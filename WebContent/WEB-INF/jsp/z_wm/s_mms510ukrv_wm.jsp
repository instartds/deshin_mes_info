...
<t:appConfig pgmId="s_mms510ukrv_wm">
	<t:ExtComboStore comboType="BOR120" pgmId="s_mms510ukrv_wm"/>				<!-- 사업장 -->
...
var alertWindow;			//20210128 추가: alertWindow : 경고창
var gsText			= ''	//20210128 추가: 바코드 알람 팝업 메세지
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_mms510ukrv_wmService.selectList',
			update	: 's_mms510ukrv_wmService.updateDetail',
			create	: 's_mms510ukrv_wmService.insertDetail',
			destroy	: 's_mms510ukrv_wmService.deleteDetail',
			syncAll	: 's_mms510ukrv_wmService.saveAll'
		}
	});

	Unilite.defineModel('s_mms510ukrv_wmModel1', {
		fields: [
	...
	Unilite.defineModel('s_mms510ukrv_wmRECEIVEModel', {		//미입고참조
		fields: [
			...
			//20210331 추가: 입고등록 시 매입접수등록한 고객정보 가져와서 btr100t에 insert 하도록 수정
			{name: 'PHONE'				, text: '연락처'			, type: 'string'},
			{name: 'BANK_NAME'			, text: '은행명'			, type: 'string'},
			{name: 'BANK_ACCOUNT'		, text: '계좌번호'			, type: 'string'},
			{name: 'BIRTHDAY'			, text: '생년월일'			, type: 'string'},
			{name: 'ZIP_CODE'			, text: '우편번호'			, type: 'string'},
			{name: 'ADDR1'				, text: '주소'			, type: 'string'},
			{name: 'CUSTOM_PRSN'		, text: '<t:message code="system.label.purchase.clientname" default="고객명"/>'	, type: 'string'}		//20210429 추가
		]
	});

	Unilite.defineModel('s_mms510ukrv_wmRETURNModel', {			//반품가능발주참조
		fields: [
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'				, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
			{name: 'ITEM_NAME'			, text: '<t:message code="system.label.purchase.itemname" default="품목명"/>'				, type: 'string'},
	...
	Unilite.defineModel('s_mms510ukrv_wmINSPECTModel2', {		//검사결과참조
		fields: [
			{name: 'TYPE'				, text: '구분'		, type: 'string',comboType:'AU',comboCode:'Z003'},
			{name: 'CUSTOM_PRSN'		, text: '<t:message code="system.label.purchase.clientname" default="고객명"/>'			, type: 'string'},
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'				, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
			{name: 'ITEM_NAME'			, text: '<t:message code="system.label.purchase.itemname" default="품목명"/>'				, type: 'string'},
	...
	var directMasterStore = Unilite.createStore('s_mms510ukrv_wmMasterStore1', {
		model: 's_mms510ukrv_wmModel1',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('s_mms510ukrv_wmGrid1');
	...
	var inoutNoMasterStore = Unilite.createStore('inoutNoMasterStore', {					//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read: 's_mms510ukrv_wmService.selectinoutNoMasterList'
			}
		}
		,loadStoreRecords : function() {
	...
	var noReceiveStore = Unilite.createStore('s_mms510ukrv_wmNoReceiveStore', {				//미입고참조
		model: 's_mms510ukrv_wmRECEIVEModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_mms510ukrv_wmService.selectnoReceiveList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	var returnPossibleStore = Unilite.createStore('s_mms510ukrv_wmReturnPossibleStore', {	//반품가능발주참조
		model: 's_mms510ukrv_wmRETURNModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_mms510ukrv_wmService.selectreturnPossibleList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	var inspectResultStore2 = Unilite.createStore('s_mms510ukrv_wmInspectResultStore', {	//검사결과참조(무검사참조겸용)
		model		: 's_mms510ukrv_wmINSPECTModel2',
		autoLoad	: false,
		uniOpt		: {
			isMaster	: true,			// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read : 's_mms510ukrv_wmService.selectinspectResultList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
...
//						UniAppManager.app.fnExchngRateO();
		...
		Unilite.popup('AGENT_CUST', {
			...
			listeners: {
				change: function(field, newValue, oldValue, eOpts) {
					...
					s_mms510ukrv_wmService.getWhCellCode(param, function(provider, response) {
						if(!Ext.isEmpty(provider) && !gsQueryFlag) {
							...
							var whCellStore2 = panelResult2.getField('WH_CELL_CODE').getStore();
			...
			},{	//20210128 추가: 검사번호 필드 추가 -> 바코드 스캔 시, 검사결과참조/적용 로직 타도록 로직 추가, 20210401 수정: 검사결과 데이터가 없을 경우, 미입고 참조/적용로직 타도록 수정
				fieldLabel	: '접수/검사번호',
				name		: 'INSPEC_NUM',
				xtype		: 'uniTextfield',
				readOnly	: false,
			...
			handler	: function() {
				...
				param.CALL_PGM		= 's_mms510ukrv_wm';
	...
	var panelResult = Unilite.createSearchForm('detailForm', {
		disabled	: false,
		border		: true,
		padding		: '1',
		region		: 'center',
	...
	var masterGrid = Unilite.createGrid('s_mms510ukrv_wmGrid1', {
...
//			{dataIndex: 'ADDR1'				, width:100	, hidden: false},
		...
		listeners: {
			beforeedit  : function( editor, e, eOpts ) {
				...
				if(e.record.phantom == true) {
					if(UniUtils.indexOf(e.field, ['WH_CODE'])) {
						return true;
					}
				}
		...
		setItemData: function(record, dataClear, grdRecord) {
			...
			if(dataClear) {
				...
				s_mms510ukrv_wmService.fnOrderPrice(param, function(provider, response) {
		...
		setNoreceiveData:function(record) {
			...
			grdRecord.set('ITEM_ACCOUNT'		, record['ITEM_ACCOUNT']);
			grdRecord.set('SPEC'				, record['SPEC']);
			grdRecord.set('ITEM_STATUS'			, Ext.isEmpty(record['ITEM_STATUS']) ? '1': record['ITEM_STATUS']);	//20210331 수정
			...
			//20210414 추가: 품목이 "불량"인 경우  금액 있으면 기표대상, 진단가로 없으면 기표대상 아님 가단가로 설정
			if(record.ITEM_STATUS == '2') {
				grdRecord.set('ACCOUNT_YNC'		, record['ORDER_O'] == 0 ? 'N':'Y');		//기표대상 여부
				grdRecord.set('PRICE_YN'		, record['ORDER_O'] == 0 ? 'N':'Y');		//단가형태
			}
		...
		setInspectData:function(record) {
			...
			//20201222 추가: 품목이 "불량"인 경우 기표대상 'N', 단가형태 '가단가', 단가/금액 0으로 set되도록 수정, 20210204 수정: 금액 있으면 기표대상, 진단가로 없으면 기표대상 아님 가단가로 설정
			if(record.ITEM_STATUS == '2') {
				grdRecord.set('ACCOUNT_YNC'		, record['ORDER_O'] == 0 ? 'N':'Y');		//기표대상 여부
				grdRecord.set('PRICE_YN'		, record['ORDER_O'] == 0 ? 'N':'Y');		//단가형태
			}
			...
			s_mms510ukrv_wmService.taxType(param, function(provider, response) {
				if(!Ext.isEmpty(provider)) {
	...
	var inoutNoMasterGrid = Unilite.createGrid('s_mms510ukrv_wminoutNoMasterGrid', {	//조회버튼 누르면 나오는 조회창
		layout : 'fit',
	...
	var noreceiveGrid = Unilite.createGrid('s_mms510ukrv_wmNoreceiveGrid', {			//미입고참조
		layout		: 'fit',
		...
		columns:  [
			...
			{ dataIndex: 'CUSTOM_PRSN'			, width:100},							//20210429 추가: 고객명
...
//			this.deleteSelectedRow();
	...
	var returnpossibleGrid = Unilite.createGrid('s_mms510ukrv_wmReturnpossibleGrid', {	//반품가능발주참조
		layout		: 'fit',
	...
	var inspectresultGrid2 = Unilite.createGrid('s_mms510ukrv_wmInspectResultGrid2', {	//검사결과참조
		region		: 'east' ,
		layout		: 'fit',
		...
		selModel: Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false,
			listeners: {
				beforeselect: function(rowSelection, record, index, eOpts) {
					var beforeSelected	= rowSelection.selected.items;
					var gsBlSum			= panelResult.getValue('BlSumInoutO');
					var amt				= record.get('ORDER_LOC_O');
					gsBlSum				= gsBlSum + amt;
					panelResult.setValue('BlSumInoutO', gsBlSum);
				},
				select: function(grid, selectRecord, index, rowIndex, eOpts ) {
			...
			referNoReceiveWindow = Ext.create('widget.uniDetailWindow', {
					...
					handler	: function() {
						...
						if(Ext.isEmpty(panelSearch.getValue('CUSTOM_CODE'))) {
							...
							if(panelSearch.getValue('CUSTOM_CODE') != selCustomCode) {
								Unilite.messageBox('한개의 거래처만 입력할 수 있습니다.');
								return false;
							}
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
						...
						noreceiveSearch.getField('TYPE').setValue('R1');
						noReceiveStore.loadStoreRecords();
					}
				}
			})
	...
	function openReturnPossibleWindow() {		//반품가능발주참조
		if(Ext.isEmpty(panelSearch.getValue('CUSTOM_CODE'))) {
			Unilite.messageBox('거래처를 선택해주세요.');
			return false;
		}
		if(!UniAppManager.app.checkForNewDetail()) return false;
			...
			referInspectResultWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
						...
						inspectresultSearch.getField('TYPE').setValue('R');
...
//				cbStore.loadStoreRecords(whCode);			//20201020 주석
		...
		setDefault: function() {
			...
			field.fireEvent('changedivcode', field, UserInfo.divCode, null, null, "DIV_CODE");
			//20210602 추가: 기본값 설정로직 추가(입고담당, 입고창고, 입고창고CELL)
			panelSearch.setValue('INOUT_PRSN'	, BsaCodeInfo.gsInOutPrsn);
			panelResult2.setValue('INOUT_PRSN'	, BsaCodeInfo.gsInOutPrsn);
			panelSearch.setValue('WH_CODE'		, BsaCodeInfo.gsDefaultWhCode);
			panelResult2.setValue('WH_CODE'		, BsaCodeInfo.gsDefaultWhCode);
			panelSearch.setValue('WH_CELL_CODE'	, BsaCodeInfo.gsDefaultWhCellCode);
			panelResult2.setValue('WH_CELL_CODE', BsaCodeInfo.gsDefaultWhCellCode);
		...
		isQtyCheck:function(record) {
			...
			if(record.get("INOUT_Q") == '0' && record.get("INOUT_TYPE_DETAIL") != '91') {
				Unilite.messageBox('<t:message code="system.message.purchase.message074" default="입고량을 입력 하십시오."/>');
				return false;
			}
			var recordSts = "N";
			...
			if(BsaCodeInfo.gsInvstatus == '+') {
				if(record.get("STOCK_CARE_YN") == 'Y') {
					...
					|| (record.get("INOUT_Q") < record.get("ORIGINAL_Q")) && recordSts == "U") {
						dInoutQ = 0
						dOriginalQ = 0;
						var findRecs = directMasterStore.findBy(function(rec,id) {
					...
					if((dStockQ - dOriginalQ) < dInoutQ * (-1)) {
						Unilite.messageBox('<t:message code="system.message.purchase.message069" default="(-) 입고 수량은 재고량을 초과할 수 없습니다."/>'+" : " + (dStockQ - dOriginalQ));
						return false;
					}
				}
			}
				...
				case "MAKE_DATE" :
					if(!Ext.isEmpty(record.get('ITEM_CODE'))) {
						if(!Ext.isEmpty(newValue)) {
							...
							s_mms510ukrv_wmService.selectExpirationdate(params, function(provider, response) {
...
//					cbStore.loadStoreRecords(newValue);			//20201020 주석
	...
	function fnEnterInspecNumBarcode(newValue) {
		...
		Ext.each(detailRecords, function(detailRecord,i) {
			...
			|| detailRecord.get('ORDER_NUM').toUpperCase() == InspecNum.toUpperCase()) {	//20210401 추가: 접수등록(개인) 데이터도 확인
				beep();
				gsText	= '동일한 접수/검사번호가 이미 등록되었습니다.';
				flag	= false;
		...
		if(flag) {
			var param = {
				...
				ITEM_LEVEL1		: panelResult2.getValue('ITEM_LEVEL1')		//20210319 추가
			}
			//검사결과참조 쿼리 호출
			s_mms510ukrv_wmService.selectinspectResultList(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
					panelSearch.setValue('CUSTOM_CODE'	, provider[0].CUSTOM_CODE);
					...
					panelSearch.setValue('MONEY_UNIT'	, provider[0].MONEY_UNIT);
					...
					salesCommonService.fnExchgRateO(param, function(provider2, response) {
						if(!Ext.isEmpty(provider2)) {
							if(provider2.BASE_EXCHG == "1" && !isIni && !Ext.isEmpty(panelSearch.getValue('MONEY_UNIT')) && panelSearch.getValue('MONEY_UNIT') != "KRW") {
								Unilite.messageBox('<t:message code="system.message.purchase.datacheck002" default="환율정보가 없습니다."/>')
							}
							panelSearch.setValue('EXCHG_RATE_O'	, provider2.BASE_EXCHG);
					...
					//20210401 수정: 검사번호에 대한 데이터가 없을 때, 매입접수(개인) 데이터 확인하도록 로직 수정
...
//					return false;
					...
					var param = {
						...
						CREATE_LOC		: panelResult2.getValues().CREATE_LOC,
						TYPE			: 'R1',					//매입접수: 'P' - 구매발주
						RECEIPT_NUM		: InspecNum,
						ITEM_LEVEL1		: panelResult2.getValue('ITEM_LEVEL1')		//20210319 추가
					}
					//미입고참조(매입접수(개인) 쿼리 호출
					s_mms510ukrv_wmService.selectnoReceiveList(param, function(provider, response){
						if(!Ext.isEmpty(provider)){
							panelSearch.setValue('CUSTOM_CODE'	, provider[0].CUSTOM_CODE);
							panelSearch.setValue('CUSTOM_NAME'	, provider[0].CUSTOM_NAME);
							panelResult2.setValue('CUSTOM_CODE'	, provider[0].CUSTOM_CODE);
							...
							salesCommonService.fnExchgRateO(param, function(provider2, response) {
								if(!Ext.isEmpty(provider2)) {
									...
									Ext.each(provider, function(record,i) {
										UniAppManager.app.onNewDataButtonDown();
										masterGrid.setNoreceiveData(record);
									});
								} else {
									Ext.each(provider, function(record,i) {
										UniAppManager.app.onNewDataButtonDown();
							...
							Unilite.messageBox('입력하신 바코드에 해당하는 데이터가 존재하지 않습니다.');
							Ext.getBody().unmask();
							panelResult2.setValue('INSPEC_NUM', '');
							panelResult2.getField('INSPEC_NUM').focus();
							return false;
