...
<%
//일일근태내역 조회
request.setAttribute("PKGNAME","Unilite_app_gtt100skrv");
%>
...
function appMain() {
	...
	Unilite.defineModel('${PKGNAME}model', {
	    fields: [
					...
					,{name: 'ORDER_VEHICLE_CODE'    ,text:'운행차량코드(현금)'	,type : 'string'  }
					,{name: 'ORDER_VEHICLE_NAME'    ,text:'운행차량(현금)'	,type : 'string'  }
					,{name: 'CARD_BASE_COUNT'    	,text:'운행대수(카드)'	,type : 'string'  }
	...
	Unilite.defineModel('${PKGNAME}gcdModel', {
	    fields: [
					 {name: 'VEHICLE_NAME'    		,text:'차량'			,type : 'string'  } 
					,{name: 'VEHICLE_CODE'    		,text:'차량'			,type : 'string'  } 
					,{name: 'VEHICLE_REGIST_NO'    	,text:'차량등록번호'	,type : 'string'  } 
					,{name: 'TOTAL_AMOUNT'   		,text:'수입금'			,type : 'uniPrice'}	
			]
	});
	
	var opStore =  Unilite.createStore('${PKGNAME}opStore',{
         model: '${PKGNAME}gcdModel',
         autoLoad: false,
            uniOpt : {
            ...
            proxy: {
                type: 'direct',
                api: {
                	   read : 'gtt100skrvService.opDriver'
                }
            },
			loadStoreRecords: function(record)	{
				var param= {
					'DIV_CODE': record.get('DIV_CODE'),
					'OPERATION_DATE': UniDate.getDateStr(record.get('OPERATION_DATE')),
					'DRIVER_CODE':record.get('DRIVER_CODE')
				};
				if(Ext.isDefined(param['DRIVER_CODE']))	{
					this.load({params: param});
				}
			}
		});
	var opGrid = Unilite.createGrid('${PKGNAME}opGrid', {
        layout : 'fit', 
        store:opStore,
    	region:'north',
    	title:'승무내역 차량',
		...
		var gcdStore =  Unilite.createStore('${PKGNAME}gcdStore',{
         model: '${PKGNAME}gcdModel',
         autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결 
            ...
            proxy: {
                type: 'direct',
                api: {
                	   read : 'gtt100skrvService.gcdDriver'
                }
            },
			loadStoreRecords: function(record)	{
				var param= {
	...
	var gcdGrid = Unilite.createGrid('${PKGNAME}gcdGrid', {
        layout : 'fit',        
    	region:'center',
    	store:gcdStore,
    	flex:.5,
	...
	var panelSearch = Unilite.createSearchPanel('${PKGNAME}searchForm',{
				...
				{	
			    	...
			    	items:[{	    
			        	...
			        	handler: function()	{
			        		...
			        		Ext.getBody().mask();
			        		gtt100skrvService.summary(params, function(provider, response)	{
     ...
     var masterGrid = Unilite.createGrid('${PKGNAME}grid', {
		...
		columns:[
			...
			{dataIndex:'DRIVER_NAME'		,width: 70, summaryType:'count',
			 summaryRenderer:function(value, summaryData, dataIndex, metaData ) {
              	var me = this;
			...
			{dataIndex:'VEHICLE_REGIST_NO'	,width: 100 },
			{dataIndex:'ORDER_VEHICLE_NAME'	,width: 110 ,
			 renderer: function(value, metaData, record) {
             	var rv = value;
             	if(record.get('VEHICLE_CODE') != record.get('ORDER_VEHICLE_CODE'))	{
             		var rv = '<div style="color:red">'+value+'</div>';
             	}
             	return rv;
             }
			},
			{dataIndex:'CARD_BASE_COUNT'	,width: 110 ,
			 renderer: function(value, metaData, record) {
             	var rv = value;
             	if(record.get('VEHICLE_CODE') != record.get('ORDER_VEHICLE_CODE'))	{
             		var rv = '<div style="color:red">'+value+'</div>';
		...
		listeners:{
			onGridDblClick:function(grid, record, cellIndex, colName)	{
				if(colName == 'CARD_BASE_COUNT')	{
					opGrid.reset();
	                gcdGrid.reset()
					gcdStore.loadStoreRecords(record);
					opStore.loadStoreRecords(record)
					if(!gcdWindow)	{
						gcdWindow = Ext.create('widget.uniDetailWindow', {
			                title: '운행대수 상세보기',
			                width: 300,			                
			                height: 300,
			                layout:'border',
