...
<t:appConfig pgmId="s_str104ukrv_yp"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="s_str104ukrv_yp" /> 			<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_str104ukrv_ypService.selectDetailList',
			update: 's_str104ukrv_ypService.updateDetail',
			create: 's_str104ukrv_ypService.insertDetail',
			destroy: 's_str104ukrv_ypService.deleteDetail',
			syncAll: 's_str104ukrv_ypService.saveAll'
		}
	});
...
//    });
	 ...
	 */
	//좌측 마스터 서치폼 정의
	var masterForm = Unilite.createSearchPanel('s_str104ukrv_ypMasterForm', {
		title: '검색조건',
...
//			},
	...
	}); //End of var masterForm = Unilite.createForm('s_str104ukrv_ypMasterForm', {
...
//                  UniAppManager.app.fnExchngRateO();
			...
			handler: function() {
                ...
                var win = Ext.create('widget.CrystalReport', {
                    url: CPATH+'/z_yp/s_str104cukrv_yp.do',
                    prgID: 's_str104ukrv_yp',
                    extParam: param
                });
                win.center();
                win.show();
...
//					   	alert(labelText+Msg.sMB083);
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('s_str104ukrv_ypDetailModel', {
	    fields: [
	             {name: 'INOUT_CODE'                    , text:'출고처'                , type: 'string', allowBlank: false},
...
//	    		 	{name: 'TRANS_RATE'					, text:'입수'     				, type: 'uniQty', defaultValue: 1, allowBlank: false},
	    		 ...
	    		 {name: 'SALE_CUST_CD'				    , text:'매출처명'      				, type: 'string', allowBlank: false},////매출처 defaultValue 다시 분석
...
//	    		 {name: 'PURCHASE_TYPE'	    			, text:'매입조건'			     	, type: 'string', comboType:'AU', comboCode:'YP09', editable: false},
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('s_str104ukrv_ypDetailStore', {
		model: 's_str104ukrv_ypDetailModel',
		autoLoad: false,
		uniOpt: {
		...
		loadStoreRecords: function() {
			var param		= masterForm.getValues();
			var inoutNum	= masterForm.getValue('INOUT_NUM').split(',');
			...
			this.load({
				...
				callback : function(records,options,success)	{
					if(success)	{
						masterForm.getField('INOUT_DATE').setReadOnly(true);
						masterForm.getField('SALE_DATE').setReadOnly(true);
						masterForm.getField('INOUT_PRSN').setReadOnly(true);
...
//			console.log('first record2 ORDER_UNIT_O'+ firstRecord2.get('ORDER_UNIT_O'));
 	...
 	//마스터 그리드 정의
    var detailGrid = Unilite.createGrid('s_str104ukrv_ypGrid', {
        layout: 'fit',
        region:'center',
        uniOpt: {
       ...
       features: [ {id : 'masterGridSubTotal', ftype: 'uniGroupingsummary', showSummaryRow: true },
    	...
    	store: detailStore,
        columns: [
        		  {dataIndex: 'INOUT_CODE'                   ,       width:133, hidden: false},
				  {dataIndex: 'CUSTOM_NAME'				    ,		width:133, hidden: false},
...
//				  {dataIndex: 'SALE_BASIS_P'				,		width:90 },
				  ...
				  {dataIndex: 'ORDER_STOCK_Q'               ,       width:120, summaryType: 'sum' },
...
//				  {dataIndex: 'SALES_TYPE'				    ,		width:70 },
				  ...
				  {dataIndex: 'SALE_CUSTOM_CODE'            ,       width:66, hidden: false },
				  ...
				  {dataIndex: 'PLAN_NUM'					,		width:115 },
				  {dataIndex: 'ORDER_NUM'					,		width:115 },
				  {dataIndex: 'ISSUE_REQ_NUM'				,		width:115 },
				  {dataIndex: 'BASIS_NUM'					,		width:115 },
				  {dataIndex: 'PAY_METHODE1'	    		,		width:115 },
...
//				grdRecord.set('SALE_BASIS_P'	,record['SALE_BASIS_P']);
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 's_str104ukrv_ypService.selectOrderNumMasterList'
                }
            }
            ,loadStoreRecords : function()	{
...
//		  	masterForm.setValue('CREATE_LOC', record.get('CREATE_LOC'));
	...
	//검색창 그리드 정의
    var orderNoDetailGrid = Unilite.createGrid('s_str104ukrv_yporderNoDetailGrid', {
        // title: '기본',
        layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
...
//					    		orderNoSearch.setValue('ORDER_TYPE',masterForm.getValue('ORDER_TYPE'));
    ...
    //출하지시 참조 모델 정의
	Unilite.defineModel('s_str104ukrv_ypRequestModel', {
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('s_str104ukrv_ypRequestStore', {
			model: 's_str104ukrv_ypRequestModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 's_str104ukrv_ypService.selectRequestiList'
                }
            },
            listeners:{
            	load:function(store, records, successful, eOpts)	{
	...
	//출하지시 참조 그리드 정의
    var requestGrid = Unilite.createGrid('s_str104ukrv_ypRequestGrid', {
        // title: '기본',
        layout : 'fit',
    	store: requestStore,
        ...
        columns:  [{ dataIndex: 'CUSTOM_NAME'		    	,  width: 120 },
        		   ...
        		   { dataIndex: 'STOCK_UNIT'            	,  width: 66, hidden: true },
        		   { dataIndex: 'DVRY_CUST_NAME'        	,  width: 113 },
        		   { dataIndex: 'WONSANGI_NUM'        	,  width: 113 },
...
//  								requestSearch.setValue('ISSUE_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
    //수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('s_str104ukrv_ypsalesOrderForm', {
        layout :  {type : 'uniTable', columns : 2},
        items :[
        	Unilite.popup('DIV_PUMOK',{
        	fieldLabel:'품목코드' ,
    ...
    //수주(오퍼) 참조 모델 정의
	Unilite.defineModel('s_str104ukrv_ypsalesOrderModel', {
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('s_str104ukrv_ypsalesOrderStore', {
			model: 's_str104ukrv_ypsalesOrderModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 's_str104ukrv_ypService.selectSalesOrderList'
                }
            }
    ...
    //수주(오퍼) 참조 그리드 정의
    var salesOrderGrid = Unilite.createGrid('s_str104ukrv_ypsalesOrderGrid', {
        // title: '기본',
        layout : 'fit',
...
//			cbStore.loadStoreRecords();
			//TEST
//			masterForm.setValue('INOUT_NUM', '01S20171030003,01S20171030005,01S20171030006');
//			panelResult.setValue('INOUT_NUM', '01S20171030003,01S20171030005,01S20171030006');
        ...
        processParams: function(params) {
            ...
            if(params.PGM_ID == 's_srq100ukrv_yp' || params.PGM_ID == 'srq100ukrv') { //출하지시등록에서 링크 넘어올시
                ...
                Ext.each(params.record, function(rec,i){
                    if(i == 0){
                        masterForm.setValue('CUSTOM_CODE'   , rec.get('CUSTOM_CODE'));
                        masterForm.setValue('CUSTOM_NAME'   , rec.get('CUSTOM_NAME'));
                        masterForm.setValue('DIV_CODE'      , formPram.DIV_CODE);
                        masterForm.setValue('MONEY_UNIT'    , rec.get('MONEY_UNIT'));
                        masterForm.setValue('CREATE_LOC'    , rec.get('REF_LOC'));
                        ...
                        //출고일에 넘어온 출하지시일 SET
                        masterForm.setValue('INOUT_DATE'    , formPram.SHIP_DATE);
                        panelResult.setValue('INOUT_DATE'   , formPram.SHIP_DATE);
                        //매출일에 넘어온 출하지시일 SET
                        masterForm.setValue('SALE_DATE'     , formPram.SHIP_DATE);
                        panelResult.setValue('SALE_DATE'    , formPram.SHIP_DATE);
                    }
                    //출하지시등록에서 넘어오면 출고일, 수불담당자 수정가능하도록 변경
                    masterForm.getField('INOUT_DATE').setReadOnly(false);
                    masterForm.getField('SALE_DATE').setReadOnly(false);
                    ...
                    var gubun = 'FEFER';
                    var nationInout = '1';
                    var sale_date = masterForm.getValue('SALE_DATE');
...
//            	s_str104ukrv_ypService.deptWhcode(param, function(provider, response){
...
//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_str104ukrv_ypAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config)	{
			var fp = Ext.getCmp('s_str104ukrv_ypFileUploadPanel');
