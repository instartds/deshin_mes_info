...
<t:appConfig pgmId="w_sof101ukrv_yp"  >
...
<t:ExtComboStore comboType="BOR120" pgmId="w_sof101ukrv_yp"/><!-- 사업장	-->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'w_sof101ukrv_ypService.selectDetailList',
			update	: 'w_sof101ukrv_ypService.updateDetail',
			create	: 'w_sof101ukrv_ypService.insertDetail',
			destroy	: 'w_sof101ukrv_ypService.deleteDetail',
			syncAll	: 'w_sof101ukrv_ypService.saveAll'
		}
	});
	 ...
	 */
	Unilite.defineModel('w_sof101ukrv_ypDetailModel', {
		fields: [
			...
			{name: 'SO_KIND'				, text:'<t:message code="unilite.msg.fSbMsgS0070"	default="주문구분"/>'			, type: 'string', allowBlank: false , comboType: 'AU', comboCode: 'S065', defaultValue: '15'},
...
//			{name: 'SALE_Q'					, text:'SALE_Q'																	, type: 'int', defaultValue: 0},
			...
			{name: 'ORDER_Q'				, text:'<t:message code="unilite.msg.sMS543" default="수주량"/>'					, type: 'uniQty', allowBlank: true, defaultValue: 0},
	...
	//수주정보스토어
	var detailStore = Unilite.createStore('w_sof101ukrv_ypDetailStore', {
		model	: 'w_sof101ukrv_ypDetailModel',
		autoLoad: false,
		uniOpt	: {
...
//				this.fnOrderAmtSum();
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('w_sof101ukrv_ypGrid');
	 ...
	 */
	var panelSearch = Unilite.createSearchPanel('w_sof101ukrv_ypMasterForm', {
...
//				hidden		: true,
				...
				handler	: function() {
					if(!Ext.isEmpty(panelSearch.getValue('ORDER_NUM') && panelSearch.getValue('ORDER_REQ_YN') == "N")){
						...
						var me = this;
						w_sof101ukrv_ypService.insertPurchaseRequest(param, function(provider, response) {
							if(provider){
		...
		api: {
			load	: 'w_sof101ukrv_ypService.selectMaster',
			submit	: 'w_sof101ukrv_ypService.syncForm'
		},
...
//		  me.setAllFieldsReadOnly(true);
	...
	}); //End of var panelSearch = Unilite.createForm('w_sof101ukrv_ypMasterForm', {
...
//			}
	 ...
	 */
	var detailGrid = Unilite.createGrid('w_sof101ukrv_ypGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
					...
					'onSelected': {
						fn: function(records, type) {
							...
							Ext.each(records, function(record,i) {
								...
								w_sof101ukrv_ypService.getCustomItemCode(param, function(provider, response){
...
//						detailGrid.setItemData(null,true, detailGrid.uniOpt.currentRecord);
			...
			{dataIndex: 'ORDER_Q'			, width: 100	, hidden: true	, summaryType: 'sum'},
			...
			{dataIndex: 'DVRY_CUST_NAME'	, width: 100	, hidden: true	,
			 editor: Unilite.popup('DELIVERY_G',{
				listeners:{
					'onSelected': {
						fn: function(records, type  ){
							//var grdRecord = Ext.getCmp('w_sof101ukrv_ypGrid').uniOpt.currentRecord;
...
//			{dataIndex: 'ISSUE_REQ_Q'		, width: 90		, hidden: true},
		...
		listeners: {
			...
			beforeedit  : function( editor, e, eOpts ) {
                ...
                if (UniUtils.indexOf(e.field, ['ORDER_Q1', 'ORDER_Q2', 'ORDER_Q3', 'ORDER_Q4', 'ORDER_Q5', 'ORDER_Q6', 'ORDER_Q7'])){
                    return true;
                }else{
                    return false;
                }
...
/*		setEstiData:function(record) {											//견적참조 사용 안 함
		...
		setExcelData: function(record, i) {
			var grdRecord = this.getSelectedRecord();
			grdRecord.set('ORDER_NUM'			, panelSearch.getValue('ORDER_NUM'));
			grdRecord.set('SCM_FLAG_YN'			, 'N');
			...
			grdRecord.set('ORDER_UNIT'			, record['ORDER_UNIT']);
			grdRecord.set('ORDER_Q1'			, record['ORDER_Q1']);
			grdRecord.set('ORDER_Q2'			, record['ORDER_Q2']);
			grdRecord.set('ORDER_Q3'			, record['ORDER_Q3']);
			grdRecord.set('ORDER_Q4'			, record['ORDER_Q4']);
			grdRecord.set('ORDER_Q5'			, record['ORDER_Q5']);
			grdRecord.set('ORDER_Q6'			, record['ORDER_Q6']);
			grdRecord.set('ORDER_Q7'			, record['ORDER_Q7']);
			grdRecord.set('OUT_DIV_CODE'		, record['DIV_CODE']);
...
/*				"DVRY_DATE1"		: grdRecord.get('DVRY_DATE1'),
			...
			w_sof101ukrv_ypService.getItemCode(param, function(provider, response) {
				if(!Ext.isEmpty(provider)){
					...
					w_sof101ukrv_ypService.getItemInfo(param, function(provider, response) {
...
//								, BsaCodeInfo.gsMoneyUnit
	...
	var orderNoSearch = Unilite.createSearchForm('orderNoSearchForm', {
		...
		items: [{
			...
			Unilite.popup('AGENT_CUST',{fieldLabel:'<t:message code="unilite.msg.sMSR213" default="거래처"/>' , validateBlank: false,colspan:2, readOnly: true,
				listeners:{
					applyextparam: function(popup){
						popup.setExtParam({'AGENT_CUST_FILTER':  ['1','3']});
						popup.setExtParam({'CUSTOM_TYPE':  ['1','3']});
...
//			Unilite.popup('AGENT_CUST',{fieldLabel:'프로젝트' , valueFieldName:'PROJECT_NO', textFieldName:'PROJECT_NAME', validateBlank: false}),
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'w_sof101ukrv_ypService.selectOrderNumMasterList'
			}
		},
	...
	//검색 그리드(마스터)
	var orderNoMasterGrid = Unilite.createGrid('w_sof101ukrv_ypOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
		...
		returnData: function(record) {
			...
			fnGetCalDate(record.get('CAL_NO'), record.get('DVRY_DATE'));
			...
			UniAppManager.app.onQueryButtonDown(record.get('DVRY_DATE'));
		}
	});
	//검색 모델(디테일)
	...
	var orderNoDetailStore = Unilite.createStore('orderNoDetailStore', {
		...
		proxy: {
			type: 'direct',
			api	: {
				read	: 'w_sof101ukrv_ypService.selectOrderNumDetailList'
			}
		},
		loadStoreRecords : function()  {
			var param		= orderNoSearch.getValues();
	...
	//검색 그리드(디테일)
	var orderNoDetailGrid = Unilite.createGrid('w_sof101ukrv_ypOrderNoDetailGrid', {
		layout	: 'fit',
	...
	/** 사업장별 영업담당 정보
	 */
	var divPrsnStore = Unilite.createStore('w_sof101ukrv_yp_DIV_PRSN', {
...
/*	// 견적을 참조하기 위한 Search Form, Grid, Inner Window 정의
	var estimateSearch = Unilite.createSearchForm('estimateForm', {
		...
		items :[
			...
			{fieldLabel: '<t:message code="unilite.msg.sMS573" default="영업담당"/>'  , name: 'ESTI_PRSN',	xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('w_sof101ukrv_yp_DIV_PRSN') }
		]
	});
	//견적참조 모델
	Unilite.defineModel('w_sof101ukrv_ypESTIModel', {
		fields: [
	...
	//견적참조 스토어
	var estimateStore = Unilite.createStore('w_sof101ukrv_ypEstiStore', {
		model: 'w_sof101ukrv_ypESTIModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'w_sof101ukrv_ypService.selectEstiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	...
	//견적참조 그리드
	var estimateGrid = Unilite.createGrid('w_sof101ukrv_ypEstimateGrid', {
		// title: '기본',
		layout : 'fit',
		store: estimateStore,
...
/*	// 수주이력을 참조하기 위한 Search Form, Grid, Inner Window 정의
	...
	//수주이력 모델
	Unilite.defineModel('w_sof101ukrv_ypRefModel', {
		fields: [
	...
	//수주이력 스토어
	var refStore = Unilite.createStore('w_sof101ukrv_ypRefStore', {
			model: 'w_sof101ukrv_ypRefModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'w_sof101ukrv_ypService.selectRefList'
	...
	//수주이력 그리드
	var refGrid = Unilite.createGrid('w_sof101ukrv_ypRefGrid', {
		// title: '기본',
		layout : 'fit',
		store: refStore,
		selModel :	Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false }),
...
*/
	...
	//엑셀참조
	Unilite.Excel.defineModel('excel.w_sof101ukrv_yp.sheet01', {
		fields: [
	...
	function openExcelWindow() {
		...
		if(!excelWindow) {
			...
			excelWindow = Ext.create( appName, {
				excelConfigName: 'w_sof101ukrv_yp',
				width	: 600,
				height	: 400,
				modal	: false,
				extParam: {
				...
				grids: [{
					...
					title		: '수주정보',
					useCheckbox	: true,
					model		: 'excel.w_sof101ukrv_yp.sheet01',
					readApi		: 'w_sof101ukrv_ypService.selectExcelUploadSheet1',
					columns		: [
				...
				onApply:function()  {
					...
					Ext.each(records, function(record,i){
						UniAppManager.app.onNewDataButtonDown();
						detailGrid.setExcelData(record.data, i);
					});
...
//			}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('w_sof101ukrv_ypAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
...
//				params.rtnRecord.set('PRICE_TYPE'	,provider['PRICE_TYPE']);
		...
		fnGetCustCredit: function(divCode, customCode, sDate, moneyUnit){
			...
			w_sof101ukrv_ypService.getCustCredit(param, function(provider, response) {
	...
	function fnGetCalNo(newValue) {
		...
		w_sof101ukrv_ypService.getCalNo(param, function(provider, response){
			if(!Ext.isEmpty(provider)) {
...
//				fnGetCalDate(provider[0].CAL_NO);
	...
	function fnGetCalDate(newValue, dvryDate) {
		if(!Ext.isEmpty(newValue)) {
			...
			w_sof101ukrv_ypService.getCalDate(param, function(provider, response){
				if(!Ext.isEmpty(provider)) {
					fnSetValue(provider);
				}
			});
