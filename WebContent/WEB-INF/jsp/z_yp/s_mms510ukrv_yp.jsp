...
<t:appConfig pgmId="s_mms510ukrv_yp"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_mms510ukrv_yp" /> <!-- 사업장 -->
...
function appMain() {
	...
	var cbStore = Unilite.createStore('s_s_mms510ukrv_yp_ypComboStoreGrid',{
		autoLoad: false,
		uniOpt: {
...
//	var sumtypeLot = BsaCodeInfo.gsSumTypeLot == "Y"?true:false;
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_mms510ukrv_ypService.selectList',
			update: 's_mms510ukrv_ypService.updateDetail',
			create: 's_mms510ukrv_ypService.insertDetail',
			destroy: 's_mms510ukrv_ypService.deleteDetail',
			syncAll: 's_mms510ukrv_ypService.saveAll'
		}
	});

	Unilite.defineModel('s_mms510ukrv_ypModel1', {
		fields: [
...
//			{name: 'PURCHASE_RATE'		  , text: 'PURCHASE_RATE'		, type: 'int'}
	...
	Unilite.defineModel('s_mms510ukrv_ypNoReceiveModel', {		 //미입고참조
		fields: [
...
//			{name: 'TRNS_RATE'				, text: '변환계수'			, type: 'uniQty'},
	...
	Unilite.defineModel('s_mms510ukrv_ypRETURNModel', {		 //반품가능발주참조
		fields: [
			{name: 'GUBUN'					, text: '선택'			, type: 'string'},
			{name: 'ITEM_CODE'				, text: '품목코드'			, type: 'string'},
			{name: 'ITEM_NAME'				, text: '품목명'			, type: 'string'},
...
//			{name: 'TRNS_RATE'				, text: '변환계수'			, type: 'uniQty'},
	...
	Unilite.defineModel('s_mms510ukrv_ypReceiveModel', {		 //입고참조
		fields: [
			{name: 'GUBUN'					, text: '선택'			, type: 'string'},
			{name: 'ITEM_CODE'				, text: '품목코드'			, type: 'string'},
			{name: 'ITEM_NAME'				, text: '품목명'			, type: 'string'},
...
//			{name: 'TRNS_RATE'				, text: '변환계수'			, type: 'int'},
	...
	Unilite.defineModel('s_mms510ukrv_ypSCMREFModel1', {		 //검사결과참조
		fields: [
			{name: 'GUBUN'	 				, text: '선택'			, type: 'string'},
...
//			{name: 'TRNS_RATE'				, text: '입수'			, type: 'int'},
	...
	Unilite.defineModel('s_mms510ukrv_ypINSPECTModel2', {	 //검사결과참조
		fields: [
			{name: 'GUBUN'					, text: '선택'			, type: 'string'},
			{name: 'ITEM_CODE'				, text: '품목코드'			, type: 'string'},
			{name: 'ITEM_NAME'				, text: '품목명'			, type: 'string'},
...
//			{name: 'TRNS_RATE'				, text: '변환계수'			, type: 'int'},
	...
	var directMasterStore1 = Unilite.createStore('s_mms510ukrv_ypMasterStore1', {
		model: 's_mms510ukrv_ypModel1',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('s_mms510ukrv_ypGrid1');
	...
	});//End of var directMasterStore1 = Unilite.createStore('s_mms510ukrv_ypMasterStore1', {
	...
	var inoutNoMasterStore = Unilite.createStore('inoutNoMasterStore', {	//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read: 's_mms510ukrv_ypService.selectinoutNoMasterList'
			}
		}
		,loadStoreRecords : function()	{
	...
	var noReceiveStore = Unilite.createStore('s_mms510ukrv_ypNoReceiveStore', {//미입고참조
			model: 's_mms510ukrv_ypNoReceiveModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_mms510ukrv_ypService.selectnoReceiveList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	var returnPossibleStore = Unilite.createStore('s_mms510ukrv_ypReturnPossibleStore', {//반품가능발주참조
			model: 's_mms510ukrv_ypRETURNModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_mms510ukrv_ypService.selectreturnPossibleList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	var receiveStore = Unilite.createStore('s_mms510ukrv_ypReceiveStore', {// 입고참조
			model: 's_mms510ukrv_ypReceiveModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_mms510ukrv_ypService.selectReceiveList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	var scmRefStore = Unilite.createStore('s_mms510ukrv_ypScmRefStore', {//검사결과참조
			model: 's_mms510ukrv_ypSCMREFModel1',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_mms510ukrv_ypService.selectScmRefList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	var inspectResultStore2 = Unilite.createStore('s_mms510ukrv_ypInspectResultStore', {//검사결과참조
			model: 's_mms510ukrv_ypINSPECTModel2',
			autoLoad: false,
			uniOpt : {
				isMaster: true,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_mms510ukrv_ypService.selectinspectResultList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
...
//	});
	...
	var masterGrid = Unilite.createGrid('s_mms510ukrv_ypGrid1', {
		// for tab
		layout: 'fit',
		region: 'center',
		excelTitle: '입고등록',
							...
							fn: function(records, type) {
								...
								Ext.each(records, function(record,i) {
									if(i==0) {
										...
										grdRecord.set('WONSANGI_NUM'	, record.ORIGIN);
			...
			beforeedit  : function( editor, e, eOpts ) {
				...
				if(e.record.data.ORDER_NUM != ''){
					...
					if(UniUtils.indexOf(e.field, ['INOUT_P','ORDER_UNIT_Q','INOUT_I','ACCOUNT_YNC' , 'WH_CODE'
												 ,'PRICE_YN','MONEY_UNIT','EXCHG_RATE_O', 'WONSANGI_NUM'])){
						return true;
					}
					if(UniUtils.indexOf(e.field, ['ITEM_STATUS'])){
						if(BsaCodeInfo.gsProcessFlag == "PG"){
		...
		setItemData: function(record, dataClear, grdRecord) {
			var grdRecord = this.getSelectedRecord();
	   		if(dataClear) {
				...
				s_mms510ukrv_ypService.fnOrderPrice(param, function(provider, response)	{
...
//			grdRecord.set('PURCHASE_RATE'		, record['PURCHASE_RATE']);
			...
			s_mms510ukrv_ypService.taxType(param, function(provider, response)	{
				if(!Ext.isEmpty(provider)){
	...
	});//End of var masterGrid = Unilite.createGrid('s_mms510ukrv_ypGrid1', {

	var inoutNoMasterGrid = Unilite.createGrid('s_mms510ukrv_ypinoutNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		layout : 'fit',
...
//		  		'WH_CODE':record.get('WH_CODE'),
	...
	var noreceiveGrid = Unilite.createGrid('s_mms510ukrv_ypNoreceiveGrid', {//미입고참조
		layout : 'fit',
	...
	var returnpossibleGrid = Unilite.createGrid('s_mms510ukrv_ypReturnpossibleGrid', {//반품가능발주참조
		layout: 'fit',
	...
	var receiveGrid = Unilite.createGrid('s_mms510ukrv_ypReceiveGrid', {//입고참조
		layout: 'fit',
...
//	var scmRefGrid= Unilite.createGrid('s_mms510ukrv_ypScmRefGrid1', {//업체출고 참조(SCM)
//		region: 'center' ,
//		layout: 'fit',
//		store: scmRefStore,
//		uniOpt: {
...
//	});
	...
	var inspectresultGrid2 = Unilite.createGrid('s_mms510ukrv_ypInspectResultGrid2', {//검사결과참조
		region: 'east' ,
		layout : 'fit',
...
//	}
	...
	Unilite.Main({
		...
		onNewDataButtonDown: function()	{
			if(!this.checkForNewDetail()) return false;
				...
				if(BsaCodeInfo.gsGwYn == 'Y') {
					 console.log("[gsCertType1]" + gsCertType);
					var r = {
	...
	//				  PURCHASE_RATE:	  0
					 ...
					 console.log("[gsCertType2]" + gsCertType);
					var r = {
						ACCOUNT_YNC:		accountYnc,
						INOUT_TYPE:		 inoutType,
						INOUT_CODE_TYPE:	inoutCodeType,
...
//			}
		...
		selectOrderPrice: function(flag, grdRecord, orderUnit){
			...
			s_mms510ukrv_ypService.selectGetOrderPrice(param, function(provider, response){
				if(Ext.isEmpty(provider)) {
					alert(Msg.sMM329);
					return false;
...
//			return toSave;
		...
		fnAutoLotNo: function(newValue){
			...
			s_mms510ukrv_ypService.getCertType(param, function(provider, response)	{
				if(!Ext.isEmpty(provider)) {
					gsCertType = provider;
				} else {
					gsCertType = 0;
...
//					break;
   ...
   function getPjtNoPopupEditor(){
    ...
    function getFarmPopupEditor(){
       var editField = Unilite.popup('FARM_INOUT_G', {
                    textFieldName   : 'FARM_CODE',
                    DBtextFieldName : 'FARM_CODE',
                    autoPopup       : true,
                    listeners       : {
...
//                                      newRecord.set('ORDER_SEQ'   , orderSeq);
                        ...
                        applyextparam: function(popup){
                            ...
                            if(!Ext.isEmpty(record)) {
                                ...
                                popup.setExtParam({'ORDER_UNIT_Q'   : record.get('ORDER_UNIT_Q')});
                                popup.setExtParam({'ORDER_UNIT'     : record.get('ORDER_UNIT')});
                                popup.setExtParam({'WONSANGI_NUM'   : record.get('WONSANGI_NUM')});
                            }
                        }
                    }
                })
