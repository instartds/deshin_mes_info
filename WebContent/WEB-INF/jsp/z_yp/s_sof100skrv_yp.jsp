...
<t:appConfig pgmId="s_sof100skrv_yp"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_sof100skrv_yp"/>		<!-- 사업장 -->
	...
	<t:ExtComboStore comboType="AU" comboCode="Z011"/>	<!-- 202109:jhj 농공산구분  -->
 ...
 */
	...
	Unilite.defineModel('s_sof100skrv_ypModel', {
		fields: [
			...
			{name: 'ORDER_ANAL'			, text: '수주번호분석'		,type:'string'},														//20210602 추가
			{name: 'PRODT_ANAL1'		, text: '품목코드분석1'		,type:'string'},														//20210602 추가
			{name: 'PRODT_ANAL2'		, text: '품목코드분석2'		,type:'string'},														//20210602 추가
			{name: 'PRODT_ANAL3'		, text: '품목코드분석3'		,type:'string'},														//20210602 추가
			{name: 'ORDER_MONTH'			,text:'수주월'								,type:'string'},
//			202109 jhj:컬럼추가
			{name: 'GOODS_DIVISION'		, text:'농공산구분'		, type: 'string', comboType: 'AU', comboCode: 'Z011', editable:false},
			{name: 'WONSANGI'			, text:'원산지'		, type: 'string'},
			{name: 'FARM_NAME'			, text:'생산자'		, type: 'string'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('s_sof100skrv_ypMasterStore1', {
		model	: 's_sof100skrv_ypModel',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {
				read: 's_sof100skrv_ypService.selectList1'
			}
		},
...
//					Ext.getCmp('sof102skrvApp').mask('<t:message code="system.label.sales.loading" default="로딩중..."/>','loading-indicator');
				...
				listeners	: {
					change: function(combo, newValue, oldValue, eOpts) {
						panelResult.setValue('GOODS_DIVISION', newValue);
					}
				}
			}]
		}],
...
//				allowBlank:false,
			...
			Unilite.popup('AGENT_CUST', {
				fieldLabel		: '<t:message code="system.label.sales.custom" default="거래처"/>',
				extParam		: {'CUSTOM_TYPE':['1','3']},				//20210819 수정: '3' -> '1', '3'
				valueFieldName	: 'CUSTOM_CODE',
				textFieldName	: 'CUSTOM_NAME',
				validateBlank	: false,
				colspan			: 2,
				listeners: {
					...
					applyextparam: function(popup) {
						popup.setExtParam({'CUSTOM_TYPE':['1','3']});			//20210819 수정: '3' -> '1', '3'
					}
				}
			}),
			Unilite.popup('DIV_PUMOK',{
				...
				listeners	: {
					change: function(combo, newValue, oldValue, eOpts) {
						panelSearch.setValue('GOODS_DIVISION', newValue);
					}
				}
			},{
				fieldLabel: '<t:message code="system.label.sales.taxabledivision" default="과세구분"/>',
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('s_sof100skrv_ypGrid1', {
		...
		tbar:[{
			...
			readOnly		: true
				...
				handler	: function() {
					...
					param["sTxtValue2_fileTitle"]='거래명세서';

					var win = '';
						 win = Ext.create('widget.ClipReport', {
							url: CPATH+'/z_yp/s_sof100clrkrv_yp.do',
							prgID: 's_sof100skrv_yp',
							extParam: param
						});
						win.center();
						win.show();
				...
				handler	: function() {
					...
					param["sTxtValue2_fileTitle"]='납품서';

					var win = '';
					 win = Ext.create('widget.ClipReport', {
						url: CPATH+'/z_yp/s_sof100clrkrv_yp2.do',
						prgID: 's_sof100skrv_yp',
						extParam: param
					});
					win.center();
				...
				handler	: function() {
					...
					var win = '';
					 win = Ext.create('widget.ClipReport', {
						url: CPATH+'/z_yp/s_sof100clrkrv_yp3.do',
						prgID: 's_sof100skrv_yp',
						extParam: param
					});
					win.center();
		...
		columns: [
			//-->	202109 jhj 양평 전용
			{dataIndex: 'ORDER_DATE'		, width: 93},	//수주일
			{dataIndex: 'CUSTOM_CODE2'		, width: 80},	//학교
			{dataIndex: 'CUSTOM_NAME2'		, width: 133},	//학교명
			{dataIndex: 'ITEM_CODE'			, width: 100, locked: false},
			{dataIndex: 'ITEM_NAME'			, width: 200, locked: false},
			{dataIndex: 'SPEC'				, width: 150, locked: false},	//규격
			{dataIndex: 'DVRY_DATE2'		, width:93},	//납기일
			{dataIndex: 'WONSANGI'			, width:110},	//원산지
			{dataIndex: 'FARM_NAME'				, width:100},	//생산자
			{dataIndex: 'CERT_NO'				, width:100},	//인증번호
			{dataIndex: 'ORDER_UNIT'		, width: 53, locked: false, align: 'center'},	//단위
			{dataIndex: 'TAX_TYPE'			, width:80, align: 'center'},	//과세구분
			{dataIndex: 'GOODS_DIVISION'	, width:80},
			{dataIndex: 'ORDER_UNIT_Q'		, width: 80, summaryType: 'sum'},	//수주량
			{dataIndex: 'ORDER_P'			, width: 106},	//단가
			{dataIndex: 'ORDER_O'			, width: 120, summaryType: 'sum'},	//금액
			//<--	202109 jhj 양평 전용
			...
			{dataIndex: 'EXCHG_RATE_O'		, width: 66, align: 'right' ,hidden: true},
			{dataIndex: 'SO_AMT_WON'		, width: 120, summaryType: 'sum' ,hidden: true},
			{dataIndex: 'ORDER_TAX_O'		, width: 100, summaryType: 'sum' ,hidden: true},
//			{dataIndex: 'ORDER_TAX_O'		, width: 100},
			...
			{dataIndex: 'SER_NO'			, width: 53, align:'center' ,hidden: true},
			...
			{dataIndex: 'PROJECT_NAME'		, width:150 ,hidden: true},
			{dataIndex: 'PO_NUM'			, width:86 ,hidden: true},
			{dataIndex: 'BOOKING_NUM'		, width:86 ,hidden: true},
			{dataIndex: 'DVRY_DATE2'		, width:93 ,hidden: true},
			{dataIndex: 'LOT_NO'			, width:93 ,hidden: true},
			{dataIndex: 'PROD_END_DATE'		, width:120 ,hidden: true},
			...
			{dataIndex: 'ORDER_PROC'		, width:100, align: 'center' ,hidden: true},	//20210521 추가
			{dataIndex: 'ORDER_STATUS'		, width:90, hidden: true},		//20210521 수정: HIDDEN
			{dataIndex: 'STATUS_NAME'		, width:90, align: 'center' ,hidden: true},	//20210521 추가
			{dataIndex: 'SORT_KEY'			, width:106, hidden: true},
			{dataIndex: 'CREATE_LOC'		, width:106, hidden: true},
			{dataIndex: 'REMARK'			, width:200 ,hidden: true},
			//20181116 추가
			{dataIndex: 'EXCHG_MONEY_UNIT'	, width: 80 , hidden: true ,hidden: true},
			{dataIndex: 'ORI_EXCHG_RATE'	, width: 120, hidden: true ,hidden: true},
			{dataIndex: 'EXCHG_ORDER_O'		, width: 120, summaryType: 'sum', hidden: true ,hidden: true},
			//20190725 고객명(RECEIVER_NAME), 송장번호(INVOICE_NUM) 추가
			{dataIndex: 'RECEIVER_NAME'		, width: 150 , hidden: false ,hidden: true},
			{dataIndex: 'INVOICE_NUM'		, width: 120 , hidden: false ,hidden: true},
			{dataIndex: 'INSERT_DB_USER'	, width: 200 , align: 'center' ,hidden: true},
			{dataIndex: 'UPDATE_DB_USER'	, width: 200 , align: 'center' ,hidden: true},
			//20191008 추가
			{dataIndex: 'PURCHASE_YN'		, width: 80, locked: false, align: 'center' ,hidden: true},
			//20210602 품목관련 추가정보 - 이노코스트
			{dataIndex: 'ORDER_ANAL_CODE'	, width: 100, locked: false, align: 'center', hidden: true},
			{dataIndex: 'ORDER_ANAL'		, width: 100, locked: false, align: 'center', hidden: true},
			{dataIndex: 'PRODT_ANAL1'		, width: 100, locked: false, align: 'center', hidden: true},
			{dataIndex: 'PRODT_ANAL2'		, width: 100, locked: false, align: 'center', hidden: true},
			{dataIndex: 'PRODT_ANAL3'		, width: 100, locked: false, align: 'center', hidden: true},
			{dataIndex: 'ORDER_MONTH'		, width: 100, locked: false, align: 'center' ,hidden: true}
			...
			afterrender: function(grid) {
				...
				this.contextMenu.add({
					...
					handler	: function(menuItem, event) {
						...
						var params = {
							sender		: me,
							'PGM_ID'	: 's_sof100skrv_yp',
							COMP_CODE	: UserInfo.compCode,
							DIV_CODE	: panelResult.getValue('DIV_CODE'),
							ORDER_NUM	: record.data.ORDER_NUM
						}
						var rec = {data : {prgID : 's_sof100ukrv_yp', 'text':''}};	// 양평 전용
						parent.openTab(rec, '/z_yp/s_sof100ukrv_yp.do', params);
					}
				});
			}
		}
