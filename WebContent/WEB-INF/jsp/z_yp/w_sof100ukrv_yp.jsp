...
<t:appConfig pgmId="w_sof100ukrv_yp"  >
...
<t:ExtComboStore comboType="BOR120" pgmId="w_sof100ukrv_yp"/><!-- 사업장	-->
</t:appConfig>
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'w_sof100ukrv_ypService.selectDetailList',
			update	: 'w_sof100ukrv_ypService.updateDetail',
			create	: 'w_sof100ukrv_ypService.insertDetail',
			destroy	: 'w_sof100ukrv_ypService.deleteDetail',
			syncAll	: 'w_sof100ukrv_ypService.saveAll'
		}
	});
	 ...
	 */
	Unilite.defineModel('w_sof100ukrv_ypDetailModel', {
		fields: [
			...
			{name: 'ORDER_UNIT_Q'			, text:'ORDER_UNIT_Q'														, type: 'int', allowBlank: false},
...
//			{name: 'TAX_TYPE'				, text:'TAX_TYPE'															, type: 'string'}
	...
	//수주정보스토어
	var detailStore = Unilite.createStore('w_sof100ukrv_ypDetailStore', {
		model	: 'w_sof100ukrv_ypDetailModel',
		autoLoad: false,
		uniOpt	: {
...
//				this.fnOrderAmtSum();
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('w_sof100ukrv_ypGrid');
	 ...
	 */
	var panelSearch = Unilite.createSearchPanel('w_sof100ukrv_ypMasterForm', {
...
//				hidden		: true,
				...
				handler	: function() {
					if(!Ext.isEmpty(panelSearch.getValue('ORDER_NUM') && panelSearch.getValue('ORDER_REQ_YN') == "N")){
						...
						var me = this;
						w_sof100ukrv_ypService.insertPurchaseRequest(param, function(provider, response) {
							if(provider){
		...
		api: {
			load	: 'w_sof100ukrv_ypService.selectMaster',
			submit	: 'w_sof100ukrv_ypService.syncForm'
		},
...
//		  me.setAllFieldsReadOnly(true);
	...
	}); //End of var panelSearch = Unilite.createForm('w_sof100ukrv_ypMasterForm', {
...
//			}
	...
	//수주정보 그리드
	var detailGrid = Unilite.createGrid('w_sof100ukrv_ypGrid', {
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
					...
					'onSelected': {
						fn: function(records, type) {
							...
							Ext.each(records, function(record,i) {
								...
								w_sof100ukrv_ypService.getCustomItemCode(param, function(provider, response){
 ...
 //						popup.setExtParam({'DIV_CODE': panelSearch.getValue('DIV_CODE')});
			...
			{dataIndex: 'DVRY_CUST_NAME'	, width: 100	, hidden: true	,
			 editor: Unilite.popup('DELIVERY_G',{
				listeners:{
					'onSelected': {
						fn: function(records, type  ){
							//var grdRecord = Ext.getCmp('w_sof100ukrv_ypGrid').uniOpt.currentRecord;
		...
		listeners: {
			...
			beforeedit  : function( editor, e, eOpts ) {
				...
				if (UniUtils.indexOf(e.field, ['ORDER_Q'])){
                    return true;
                }else{
                    return false;
                }
...
//			Unilite.popup('AGENT_CUST',{fieldLabel:'프로젝트' , valueFieldName:'PROJECT_NO', textFieldName:'PROJECT_NAME', validateBlank: false}),
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'w_sof100ukrv_ypService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function()  {
	...
	//검색 그리드(마스터)
	var orderNoMasterGrid = Unilite.createGrid('w_sof100ukrv_ypOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
	...
	var orderNoDetailStore = Unilite.createStore('orderNoDetailStore', {
		...
		proxy: {
			type: 'direct',
			api	: {
				read	: 'w_sof100ukrv_ypService.selectOrderNumDetailList'
			}
		},
		loadStoreRecords : function()  {
			var param		= orderNoSearch.getValues();
	...
	//검색 그리드(디테일)
	var orderNoDetailGrid = Unilite.createGrid('w_sof100ukrv_ypOrderNoDetailGrid', {
		layout	: 'fit',
	...
	/** 사업장별 영업담당 정보
	 */
	var divPrsnStore = Unilite.createStore('w_sof100ukrv_yp_DIV_PRSN', {
	...
	var estimateSearch = Unilite.createSearchForm('estimateForm', {
			...
			items :[	Unilite.popup('AGENT_CUST',{fieldLabel:'견적처' , validateBlank: false})
						...
						,{ fieldLabel: '<t:message code="unilite.msg.sMS147" default="견적일"/>'
						 ...
						 ,{fieldLabel: '<t:message code="unilite.msg.sMS573" default="영업담당"/>'  , name: 'ESTI_PRSN',	xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('w_sof100ukrv_yp_DIV_PRSN') }
					]

	});
	//견적참조 모델
	Unilite.defineModel('w_sof100ukrv_ypESTIModel', {
		fields: [
	...
	//견적참조 스토어
	var estimateStore = Unilite.createStore('w_sof100ukrv_ypEstiStore', {
			model: 'w_sof100ukrv_ypESTIModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'w_sof100ukrv_ypService.selectEstiList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	//견적참조 그리드
	var estimateGrid = Unilite.createGrid('w_sof100ukrv_ypEstimateGrid', {
		// title: '기본',
		layout : 'fit',
		store: estimateStore,
...
//		  estimateSearch.setValue('DIV_CODE', panelResult.getValue('DIV_CODE'));
	...
	var refSearch = Unilite.createSearchForm('RefSForm', {
			layout :  {type : 'uniTable', columns : 3},
			items :[	Unilite.popup('AGENT_CUST',{fieldLabel:'수주처' , validateBlank: false, readOnly: true,
						listeners:{
							applyextparam: function(popup){
								popup.setExtParam({'AGENT_CUST_FILTER':  ['1','3']});
								popup.setExtParam({'CUSTOM_TYPE':  ['1','3']});
	...
	//수주이력 모델
	Unilite.defineModel('w_sof100ukrv_ypRefModel', {
		fields: [
	...
	//수주이력 스토어
	var refStore = Unilite.createStore('w_sof100ukrv_ypRefStore', {
			model: 'w_sof100ukrv_ypRefModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'w_sof100ukrv_ypService.selectRefList'
	...
	//수주이력 그리드
	var refGrid = Unilite.createGrid('w_sof100ukrv_ypRefGrid', {
		// title: '기본',
		layout : 'fit',
		store: refStore,
		selModel :	Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false }),
			...
			if(!excelWindow) {
				...
				excelWindow = Ext.create( appName, {
						...
						grids: [{
								...
								readApi: 'w_sof100ukrv_ypService.selectExcelUploadSheet1',
								columns: [
											 { dataIndex: 'ITEM_CODE',		  width: 120	  }
...
//		  }
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('w_sof100ukrv_ypAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
...
//			}
		...
		/* 	panelSearch.setValue('CUSTOM_CODE', 'S0037');
			panelSearch.setValue('CUSTOM_NAME', '양평유치원/간식');
			panelResult.setValue('CUSTOM_CODE', 'S0037');
			panelResult.setValue('CUSTOM_NAME', '양평유치원/간식');
 */
		},
...
//						rtnRecord.set('EXP_ISSUE_DATE'		,'');
		...
		fnGetCustCredit: function(divCode, customCode, sDate, moneyUnit){
			...
			w_sof100ukrv_ypService.getCustCredit(param, function(provider, response) {
...
//						record.set('EXP_ISSUE_DATE'	,'');
				...
				case "CUSTOM_ITEM_NAME" :
					if(Ext.isEmpty(record.get('ITEM_CODE'))) {
						...
						w_sof100ukrv_ypService.getItemCode(param, function(provider, response) {
							if(!Ext.isEmpty(provider)) {
