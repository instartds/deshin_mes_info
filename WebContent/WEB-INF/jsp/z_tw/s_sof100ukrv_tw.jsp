...
<t:appConfig pgmId="s_sof100ukrv_tw"  >
...
<t:ExtComboStore comboType="BOR120" pgmId="s_sof100ukrv_tw"/>						<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_sof100ukrv_twService.selectDetailList',
			update	: 's_sof100ukrv_twService.updateDetail',
			create	: 's_sof100ukrv_twService.insertDetail',
			destroy	: 's_sof100ukrv_twService.deleteDetail',
			syncAll	: 's_sof100ukrv_twService.saveAll'
		}
	});
	...
	Unilite.defineModel('s_sof100ukrv_twDetailModel', {
		fields: [
	...
	// 수주정보스토어
	var detailStore = Unilite.createStore('s_sof100ukrv_twDetailStore', {
		model	: 's_sof100ukrv_twDetailModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
...
//				}
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('s_sof100ukrv_twGrid');
	...
	//미등록된 SRM 데이터
	Unilite.Excel.defineModel('s_Sof100ukrv_twModel_notRegisteredSRM', {
		fields: [
	...
	var notRegisteredSRMStore = Unilite.createStore('s_sof100ukrv_twNotRegisteredSRMStore',{
		model: 's_Sof100ukrv_twModel_notRegisteredSRM',
		uniOpt: {
...
//		groupField: ''
	...
	var masterForm = Unilite.createSearchPanel('s_sof100ukrv_twMasterForm', {
					...
					blur: function(field, The, eOpts){
						...
						if(newValue != offerNoChk && nationInout == '2' && !Ext.isEmpty(newValue)){ //해외 일 경우에만
							...
							if(!Ext.isEmpty(masterForm.getValue('OFFER_NO'))){
								s_sof100ukrv_twService.checkOfferNo(param, function(provider2, response){
			...
			// 20210224 : 카드사(CARD_CUSTOM_CODE) 추가
				...
				handler	: function() {
					if(!Ext.isEmpty(masterForm.getValue('ORDER_NUM')/* && masterForm.getValue('ORDER_REQ_YN') == "N"*/)){
						...
						var me = this;
						s_sof100ukrv_twService.insertPurchaseRequest(param, function(provider, response) {
...
//						popup.setExtParam({'CUSTOM_CODE': masterForm.getValue('CUSTOM_CODE')});
			...
			Unilite.popup('AGENT_CUST', {
				...
				handler : function() {
					...
					s_sof100ukrv_twService.soToPoRequest(param, function(provider, response) {
		...
		api: {
			load	: 's_sof100ukrv_twService.selectMaster',
			submit	: 's_sof100ukrv_twService.syncForm'
		},
	...
	}); // End of var masterForm = Unilite.createForm('s_sof100ukrv_twMasterForm', {
...
//					}
	...
	var detailForm = Unilite.createForm('s_sof100ukrv_twDetail', {
		autoScroll	: true,
		layout		: 'fit',
		...
		items		: [{
			xtype		: 'xuploadpanel',
			id			: 's_sof100ukrv_twFileUploadPanel',
...
//			this.setActiveRecord(record || null);
			...
			//첨부파일
			var fp = Ext.getCmp('s_sof100ukrv_twFileUploadPanel');
			var ordernum = panelResult.getValue('ORDER_NUM');
			if(!Ext.isEmpty(ordernum)) {
				s_sof100ukrv_twService.getFileList({DOC_NO : ordernum}, function(provider, response) {
...
//			}
			...
			detailWin = Ext.create('widget.uniDetailWindow', {
				...
				tbar	: ['->',{
					...
					handler	: function() {
						...
						s_sof100ukrv_twService.insertSOF102(param , function(provider, response){
							// 저장 완료 리턴 메세지
					...
					handler	: function() {
						...
						s_sof100ukrv_twService.selectDocCnt(param, function(provider, response){
							if(!Ext.isEmpty(provider)){
								if( provider > 0){
					...
					handler	: function() {
						...
						if(!Ext.isEmpty(detailSearch.getValue('DEL_FIDS'))){
							if(confirm('문서를 삭제 하시겠습니까?')) {
								...
								s_sof100ukrv_twService.deleteSOF102(param , function(provider, response){})
							}
						} else {
	 ...
	 */
	var detailGrid = Unilite.createGrid('s_sof100ukrv_twGrid', {
		store: detailStore,
		layout: 'fit',
		region:'center',
		selModel:'rowmodel',
...
//				param.dvryDate			= dvryDate;
				...
				s_sof100ukrv_twService.receiveSRM(param, function(provider, response){
			...
			{dataIndex: 'PURCHASE_YN',		width: 133, xtype : 'checkcolumn',
				listeners: {
					beforecheckchange: function( CheckColumn, rowIndex, checked, eOpts ){
						...
						detailGrid.mask();
						s_sof100ukrv_twService.getSupplyType(param, function(provider, response) {
			...
			{dataIndex: 'DVRY_CUST_NAME',	width: 100,
				editor: Unilite.popup('DELIVERY_G',{
						...
						listeners:{ 'onSelected': {
							fn: function(records, type  ){
							// var grdRecord =
							// Ext.getCmp('s_sof100ukrv_twGrid').uniOpt.currentRecord;
							var grdRecord = detailGrid.uniOpt.currentRecord;
...
// textFieldName:'PROJECT_NAME', validateBlank: false}),
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 's_sof100ukrv_twService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	// 검색 그리드(마스터)
	var orderNoMasterGrid = Unilite.createGrid('s_sof100ukrv_twOrderNoMasterGrid', {
	...
	var orderNoDetailStore = Unilite.createStore('orderNoDetailStore', {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 's_sof100ukrv_twService.selectOrderNumDetailList'
			}
		},
		loadStoreRecords : function() {
			var param		= orderNoSearch.getValues();
	...
	// 검색 그리드(디테일)
	var orderNoDetailGrid = Unilite.createGrid('s_sof100ukrv_twOrderNoDetailGrid', {
	...
	// 견적참조 모델
	Unilite.defineModel('s_sof100ukrv_twESTIModel', {
		fields: [
	...
	// 견적참조 스토어
	var estimateStore = Unilite.createStore('s_sof100ukrv_twEstiStore', {
		model	: 's_sof100ukrv_twESTIModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 's_sof100ukrv_twService.selectEstiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	// 견적참조 그리드
	var estimateGrid = Unilite.createGrid('s_sof100ukrv_twEstimateGrid', {
		store	: estimateStore,
		layout	: 'fit',
	...
	// 수주이력 모델
	Unilite.defineModel('s_sof100ukrv_twRefModel', {
		fields: [
	...
	// 수주이력 스토어
	var refStore = Unilite.createStore('s_sof100ukrv_twRefStore', {
		model	: 's_sof100ukrv_twRefModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 's_sof100ukrv_twService.selectRefList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	// 수주이력 그리드
	var refGrid = Unilite.createGrid('s_sof100ukrv_twRefGrid', {
		store	: refStore,
		layout	: 'fit',
		selModel: Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false }),
		uniOpt	: {
	...
	// 엑셀참조
	Unilite.Excel.defineModel('excel.sof100_tw.sheet01', {
		fields: [
			{name: 'ITEM_CODE'			, text:'<t:message code="system.label.sales.itemcode" default="품목코드"/>'						, type: 'string'},
			{name: 'QTY'				, text:'<t:message code="system.label.sales.sellingqty" default="판매수량"/>'					, type: 'uniQty', allowBlank: false, defaultValue: 0},
			{name: 'ITEM_NAME'			, text:'<t:message code="system.label.sales.itemname" default="품목명"/>'						, type: 'string'},
			{name: 'SPEC'				, text:'<t:message code="system.label.sales.spec" default="규격"/>'							, type: 'string'},
			{name: 'ORDER_P'			, text:'<t:message code="system.label.sales.sellingprice" default="판매단가"/>'					, type: 'uniUnitPrice', allowBlank: true, defaultValue: 0},
			{name: 'ORDER_UNIT'			, text:'<t:message code="system.label.sales.salesunit" default="판매단위"/>'					, type: 'string', allowBlank: false , comboType: 'AU', comboCode: 'B013'},
			{name: 'TRNS_RATE'			, text:'<t:message code="system.label.sales.containedqty" default="입수"/>'					, type: 'uniQty', allowBlank: false, defaultValue: 1},
			{name: 'ITEM_ACCOUNT'		, text:'<t:message code="system.label.sales.itemaccount" default="품목계정"/>'					, type: 'string'},
			{name: 'OUT_WH_CODE'		, text:'<t:message code="system.label.sales.issuewarehouse" default="출고창고"/>'				, type: 'string', store: Ext.data.StoreManager.lookup('whList')},
			{name: 'WH_CODE'			, text:'<t:message code="system.label.sales.mainwarehouse" default="주창고"/>'					, type: 'string'},
			{name: 'ORDER_O'			, text:'<t:message code="system.label.sales.amount" default="금액"/>'							, type: 'uniPrice', allowBlank: true, defaultValue: 0},
			{name: 'ORDER_TAX_O'		, text:'<t:message code="system.label.sales.vatamount" default="부가세액"/>'					, type: 'uniPrice', allowBlank: true, defaultValue: 0},
			{name: 'ORDER_O_TAX_O'		, text:'<t:message code="system.label.sales.salesordertotal" default="수주계"/>'				, type: 'uniPrice', allowBlank: true, defaultValue: 0},
			...
			{name: 'CARE_YN'			, text: 'CARE_YN'				, type: 'string', comboType: 'AU'	, comboCode: 'B010'},
			{name: 'CARE_REASON'		, text: 'CARE_REASON'			, type: 'string'},
			{name: 'ADDRESS1'			, text: '검가구분'	, type: 'string'},
			{name: 'ADDRESS2'			, text: '저장BIN'	, type: 'string'},
	...
	function openExcelWindow() {
		...
		var me = this;ㄴ
		var vParam = {};
		var appName = 'Unilite.com.excel.ExcelUpload'; //'Unilite.com.excel.ExcelUploadWin';
		if(!excelWindow) {
			excelWindow =  Ext.WindowMgr.get(appName);
			excelWindow = Ext.create( appName, {
				modal: false,
				excelConfigName: excelConNm, 
				extParam: {
					'PGM_ID'                : 's_sof100ukrv_tw',
					'DIV_CODE'				: masterForm.getValue('DIV_CODE'),
				...
				grids: [{
					...
					title		: '두산수주정보',
					useCheckbox	: true,
					model		: 'excel.sof100_tw.sheet01',
					readApi		: 's_sof100ukrv_twService.selectExcelUploadSheet1',
					columns		: [
...
//						}
	...
	var unregisteredSrmGrid = Unilite.createGrid('s_sof100ukrv_twUnregisteredGrid', {
...
/*			//20190925 OFFER_NO 자동채번 로직관련 수정 - - 무역이면서 자동채번이 아닐 때만 OFFER_NO 필수 체크, 원래 주석이어서 일단 주석 유지
		...
		onSaveDataButtonDown: function(config) {
			...
			if(!Ext.isEmpty(masterForm.getValue('DVRY_DATE')) && masterForm.getValue('DVRY_DATE') < masterForm.getValue('ORDER_DATE')) {
				...
				masterForm.getField('ORDER_DATE').setReadOnly(false);
				panelResult.getField('ORDER_DATE').setReadOnly(false);						
				masterForm.getField('ORDER_DATE').focus();
				return false;
			}
...
//			}			
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_sof100ukrv_twAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
...
// params.rtnRecord.set('PRICE_TYPE',provider['PRICE_TYPE']);
		...
		fnGetCustCredit: function(divCode, customCode, sDate, moneyUnit){
			...
			s_sof100ukrv_twService.getCustCredit(param, function(provider, response) {
...
//						record.set('PROD_END_DATE', record.get('DVRY_DATE'));
				...
				case "UPN_CODE" :
                    ...
                    if( masterForm.getValue('MONEY_UNIT')== "JPY") {
                        s_sof100ukrv_twService.checkUpnCode(param, function(provider2, response){
