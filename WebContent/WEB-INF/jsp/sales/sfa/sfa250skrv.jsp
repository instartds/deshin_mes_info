...
<t:appConfig pgmId="sfa250skrv"  >
<t:ExtComboStore comboType="BOR120"  pgmId="sfa250skrv"/> 			<!-- 사업장 -->  
...
function appMain() {
	 ...
	 * @type 
	 */   			 
	Unilite.defineModel('sfa250skrvModel', {
	    fields: [
	    	...
	    	{name:'SALES_PROFIT' 				, text: '<t:message code="system.label.sales.salesprofit" default="매출이익"/>'			, type: 'uniPrice'}, 	
	 ...
	 * @type 
	 */
	var MasterStore = Unilite.createStore('sfa250skrvMasterStore',{
			model: 'sfa250skrvModel',
			uniOpt : {
            ...
            proxy: {
                type: 'direct',
                api: {
               		read: 'sfa250skrvService.selectList'
                }
            },
		    ...
		    items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {				
						...
						var itemSearch = Ext.getCmp('itemhidden');
						var itemSearch2 = Ext.getCmp('itemhidden2');
						if (newValue.GUBUN == '1' ){  
...
//						html: '<b>※ 부가세 제외금액</b>',
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('sfa250skrvGrid', {
    	region: 'center' ,
        layout : 'fit',
        store : MasterStore,
        uniOpt:{
        ...
        columns:  [
			...
			{dataIndex:'PUR_PRICE' 	  							, width: 100},
			{dataIndex:'SALE_PRICE' 	  						, width: 100},
			{dataIndex:'SALE_Q' 	  							, width: 100 , summaryType: 'sum'},
			{dataIndex:'SALE_AMT_O' 			  				, width: 120 , summaryType: 'sum'},
			{dataIndex:'SAVE_MONEY' 	  						, width: 100 , summaryType: 'sum'},
			{dataIndex:'TAX_AMT_O' 				  				, width: 100 , summaryType: 'sum'},
			{dataIndex:'SALES_AMT_O' 	  						, width: 100 , summaryType: 'sum'},
			{dataIndex:'SALE_COST' 	  							, width: 100 , summaryType: 'sum'},
    ...
    Unilite.Main ({
		...
		fnInitBinding: function() {
			...
			itemSearch.setVisible(false);
			itemSearch2.setVisible(false);
			masterGrid.getColumn('ITEM_CODE').hidden = true;
			masterGrid.getColumn('ITEM_NAME').hidden = true;
