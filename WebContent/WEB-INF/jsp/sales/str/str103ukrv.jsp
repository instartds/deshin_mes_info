...
var gsQueryFlag		= false;	//20201216 추가: 창고변경 시, 창고에 설정되어 있는 기본창고cell 가져오는 로직 추가
////2015.02.10
////마감 처리 된건 수정 불가하게 수정
...
var BsaCodeInfo = {
	...
	gsSiteCode			: '${gsSiteCode}'			//20200311 추가: 사이트 구분 운영코드(B259) 가져오는 로직 추가
};
...
//20200311 추가: 사이트 구분 운영코드(B259)에 따라 라벨출력 컬럼 숨기거나 보이도록 설정하기 위한 변수 선언
var labelhiddenChk = true;
if(BsaCodeInfo.gsSiteCode == 'SHIN'/* && UserInfo.userID == 'unilite5'*/){
	labelhiddenChk = false;
}
var labelPrintWindow;//20200311 추가: 라벨출력 관련 WINDOW
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str103ukrvService.selectDetailList',
			update	: 'str103ukrvService.updateDetail',
			create	: 'str103ukrvService.insertDetail',
			destroy	: 'str103ukrvService.deleteDetail',
			syncAll	: 'str103ukrvService.saveAll'
		}
	});
...
//			holdable	: 'hold',
			listeners	: {
				change: function(combo, newValue, oldValue, eOpts) {
					//2201216 추가: 창고변경 시, 창고에 설정되어 있는 기본창고cell 가져오는 로직 추가
					var param = {
...
//			colspan	: 2,
			handler	: function() {
				...
				if(Ext.isEmpty(reportGubun) || reportGubun == 'CR'){
					...
					var rptParam				= panelResult.getValues();
					rptParam['REPORT_TYPE']		= BsaCodeInfo.gsDefaultType;

					str410skrvService.getReportFileInfo(rptParam, function(provider, response) {
						if(provider) {
...
/*		,{
		...
		api: {
			load	: 'str103ukrvService.selectMaster',
			submit	: 'str103ukrvService.syncMaster'
		},
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('str103ukrvDetailModel', {
		fields: [
...
//			{name: 'LOT_ASSIGNED_YN'		, text:'LOT_ASSIGNED_YN'		, type: 'string', defaultValue: "N"},	//lot팝업에서 선택시 Y로 set..
			...
			{name: 'CARD_CUSTOM_CODE'		, text:'<t:message code="system.label.sales.cardcustomnm" default="카드사"/>'		, type: 'string', comboType:'AU', comboCode:'A028'},
...
//			{name: 'PURCHASE_TYPE'			, text:'<t:message code="system.label.sales.purchasecondition" default="매입조건"/>'	, type: 'string', comboType:'AU', comboCode:'YP09', editable: false},
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('str103ukrvDetailStore', {
		proxy	: directProxy,
		model	: 'str103ukrvDetailModel',
		autoLoad: false,
		uniOpt	: {
...
//										return record.get('TAX_TYPE') == '1';}));
			...
			var dSaleTI		= 0;
			var dSaleNTI	= 0;
			var dZeroTI		= 0;		//20210429 추가: 영세금액
			var dTaxI 		= 0;
			...
			panelResult.setValue('TOT_SALE_ZERO_O'	, dZeroTI);									//20210429 추가: 영세총액(4)
			panelResult.setValue('TOTAL_AMT'		, dSaleTI + dSaleNTI + dTaxI + dZeroTI);	//출고총액, 20210429 수정: 영세총액 추가
...
//			console.log('first record2 ORDER_UNIT_O'+ firstRecord2.get('ORDER_UNIT_O'));
 	...
 	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('str103ukrvGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
...
//			{dataIndex: 'SALE_BASIS_P'			, width:90 },
			...
			//20190904 세액포함여부 콤보로 변경 / 수정가능하도록 변경
			{dataIndex: 'TAX_INOUT'				, width:88, hidden: false , align: 'center'},
...
//			{dataIndex: 'SALES_TYPE'			, width:70 },
			...
			{dataIndex: 'CARD_CUSTOM_CODE'		, width:66, hidden: true },  // 20210225 : CARD_CUSTOM_CODE(카드사) 추가
						...
						*/
						...
						if (UniUtils.indexOf(e.field,
											['INOUT_WGT_P', 'INOUT_VOL_P', 'UNIT_WGT', 'UNIT_VOL', 'INOUT_WGT_P', 'INOUT_VOL_P', 'UNIT_WGT', 'UNIT_VOL',
											 ...
											 'ADDRESS'
											 //, 'SALE_CUST_CD', 'DVRY_CUST_CD', 'DVRY_CUST_NAME', 'SALE_CUSTOM_CODE'
											 , 'ORDER_CUST_CD', 'PLAN_NUM', 'ORDER_NUM',
											 'ORDER_NUM', 'ISSUE_REQ_NUM', 'BASIS_NUM', 'PAY_METHODE1'	, 'LC_SER_NO', 'INOUT_NUM', 'INOUT_DATE', 'INOUT_METH',
											 'INOUT_TYPE', 'DIV_CODE', 'INOUT_CODE_TYPE', 'INOUT_CODE', 'CREATE_LOC', 'UPDATE_DB_USER',
...
//						}
					...
					if (UniUtils.indexOf(e.field,
											['INOUT_TYPE_DETAIL', 'TRANS_RATE', 'INOUT_WGT_P', 'INOUT_VOL_P', 'UNIT_WGT', 'UNIT_VOL', 'INOUT_SEQ',
											 ...
											 //, 'SALE_CUSTOM_CODE', 'SALE_CUST_CD', 'DVRY_CUST_CD', 'DVRY_CUST_NAME',
...
//				grdRecord.set('SALE_BASIS_P'	,record['SALE_BASIS_P']);
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str103ukrvService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('str103ukrvorderNoDetailGrid', {
		// title: '기본',
		layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
...
//						orderNoSearch.setValue('ORDER_TYPE',panelResult.getValue('ORDER_TYPE'));
	...
	var requestSearch = Unilite.createSearchForm('requestForm', {
		...
		//20200206 추가: 거래처 선택없이 참조하도록 로직 변경되어 추가
		Unilite.popup('AGENT_CUST',{
			fieldLabel		: '<t:message code="system.label.sales.custom" default="거래처"/>' ,
			valueFieldName	: 'CUSTOM_CODE',
			textFieldName	: 'CUSTOM_NAME',
...
//			name:'CUSTOM_NAME'
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('str103ukrvRequestModel', {
		fields: [
			...
			{name: 'ISSUE_DATE'			,text: '<t:message code="system.label.sales.issuerequestdate" default="출고요청일"/>'	, type: 'uniDate'},		//20200206 수정: 컬럼명 '납기일' -> '출고요청일'로 변경
			...
			{name: 'REMARK_INTER'		,text: '<t:message code="system.label.sales.remarkinter" default="내부기록사항"/>'			, type: 'string'},
			//20200206 추가: 거래처 선택없이 참조하도록 로직 변경되어 추가
			{name: 'CUSTOM_AGENT_TYPE'	,text: 'CUSTOM_AGENT_TYPE'	, type: 'string'},
			{name: 'CUSTOM_TAX_TYPE'	,text: 'CUSTOM_TAX_TYPE'	, type: 'string'},
			{name: 'CUSTOM_BUSI_PRSN'	,text: 'CUSTOM_BUSI_PRSN'	, type: 'string'}
		]
	});
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('str103ukrvRequestStore', {
		model: 'str103ukrvRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str103ukrvService.selectRequestiList'
			}
		},
		loadStoreRecords : function() {
				...
				if(successful) {
					...
					if(masterRecords.items.length > 0) {
						Ext.each(records, function(item, i) {
							Ext.each(masterRecords.items, function(masterRecord, i) {
								if((masterRecord.data['ISSUE_REQ_NUM'] == item.data['ISSUE_REQ_NUM'])
									&& (masterRecord.data['ISSUE_REQ_SEQ'] == item.data['ISSUE_REQ_SEQ'])
									) {
									//20200131 출하지시 참조 시, 출하지시량 남아있는 데이터는 참조팝업에 보이도록 로직 추가
									var results = detailStore.sumBy(function(detailRecord, id) {
										if(detailRecord.get('ISSUE_REQ_NUM') == masterRecord.data['ISSUE_REQ_NUM']
										&& detailRecord.get('ISSUE_REQ_SEQ') == masterRecord.data['ISSUE_REQ_SEQ']) {
											return true;
										}
									},
									['ORDER_UNIT_Q']);
									...
									if(!item.dirty) {
										if(results.ORDER_UNIT_Q >= item.data['NOT_REQ_Q']) {
											deleteRecords.push(item);
										} else {
											item.set('NOT_REQ_Q', item.get('NOT_REQ_Q') - results.ORDER_UNIT_Q);
										}
									}
								}
							});
		...
		//20200206 수정: 출하지시번호로 그루핑
		groupField: 'ISSUE_REQ_NUM'
	});
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('str103ukrvRequestGrid', {
		store	: requestStore,
		layout	: 'fit',
		...
		//20200206 그루핑 기능 추가
		features: [ {id : 'requestGridSubTotal',ftype: 'uniGroupingsummary',showSummaryRow: true },
					{id : 'requestGridTotal',	ftype: 'uniSummary',		showSummaryRow: true} ],
		columns:  [
			{ dataIndex: 'CUSTOM_NAME'			, width: 120 },
		...
		returnData: function() {
			...
			// 선택한 항목이 존재하지 않을 경우
			if(records.length < 1) {
				Unilite.messageBox('<t:message code="system.message.sales.message061" default="선택된 데이터가 없습니다."/>');	//선택된 데이터가 없습니다.
				return false;
			}
			
			//20200206 추가: 거래처 선택없이 참조하도록 로직 변경되어 추가
			panelResult.setValue('CUSTOM_CODE', records[0].get('CUSTOM_CODE'));
			panelResult.setValue('CUSTOM_NAME', records[0].get('CUSTOM_NAME'));
			CustomCodeInfo.gsAgentType		= records[0].get('CUSTOM_AGENT_TYPE');
			...
			CustomCodeInfo.gsbusiPrsn 		= records[0].get('CUSTOM_BUSI_PRSN');	//거래처의 주영업담당
			...
			referRequestWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
						//20200206 추가: 거래처 선택없이 참조하도록 로직 변경되어 추가
						if(!Ext.isEmpty(panelResult.getValue('CUSTOM_CODE')) && !Ext.isEmpty(panelResult.getValue('CUSTOM_NAME')) && panelResult.getField('CUSTOM_CODE').readOnly) {
							requestSearch.getField('CUSTOM_CODE').setReadOnly(true);
							requestSearch.getField('CUSTOM_NAME').setReadOnly(true);
						} else {
							requestSearch.getField('CUSTOM_CODE').setReadOnly(false);
							requestSearch.getField('CUSTOM_NAME').setReadOnly(false);
						}
...
//		},{
	...
	Unilite.defineModel('str103ukrvsalesOrderModel', {
		fields: [
			...
			{ name: 'CUSTOM_TAX_TYPE'	, text: 'CUSTOM_TAX_TYPE'	, type: 'string'},
			{ name: 'CUSTOM_BUSI_PRSN'	, text: 'CUSTOM_BUSI_PRSN'	, type: 'string'},
			{ name: 'R_STOCK_Q'			, text: 'R_STOCK_Q'			,type : 'string'},
			// 20210422
			{name: 'ADDRESS'			,text: '<t:message code="system.label.sales.address" default="주소"/>'				, type: 'string'}
		]
	});
	...
	var salesOrderStore = Unilite.createStore('str103ukrvsalesOrderStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str103ukrvService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function() {
	...
	var salesOrderGrid = Unilite.createGrid('str103ukrvsalesOrderGrid', {
		...
		columns:  [
			...
			{ dataIndex: 'R_STOCK_Q'			, width: 66, hidden: true },
		...
		returnData: function() {
			...
			CustomCodeInfo.gsCustCreditYn	= records[0].get('CREDIT_YN') == Ext.isEmpty(records[0].get('CREDIT_YN'))? 0 : records[0].get('CREDIT_YN');
			CustomCodeInfo.gsUnderCalBase	= records[0].get('WON_CALC_BAS');
			CustomCodeInfo.gsTaxInout 		= records[0].get('CUSTOM_TAX_TYPE');	//세액포함여부
			CustomCodeInfo.gsbusiPrsn 		= records[0].get('CUSTOM_BUSI_PRSN'); //거래처의 주영업담당
			...
			refersalesOrderWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
						//20200206 추가: 거래처 선택없이 참조하도록 로직 변경되어 추가
						if(!Ext.isEmpty(panelResult.getValue('CUSTOM_CODE')) && !Ext.isEmpty(panelResult.getValue('CUSTOM_NAME')) && panelResult.getField('CUSTOM_CODE').readOnly) {
							salesOrderSearch.getField('CUSTOM_CODE').setReadOnly(true);
							salesOrderSearch.getField('CUSTOM_NAME').setReadOnly(true);
						} else {
							salesOrderSearch.getField('CUSTOM_CODE').setReadOnly(false);
							salesOrderSearch.getField('CUSTOM_NAME').setReadOnly(false);
						}
...
//						salesOrderSearch.setValue('DVRY_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('DVRY_DATE_TO')));
	...
	var linkedStore = Unilite.createStore('str103ukrvlinkedStore', {
		model: 'str103ukrvRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,	// 상위 버튼 연결
...
//				panelRsult.setValue('INOUT_NUM', formPram.D)
				...
				Ext.each(params.record, function(rec,i){
					...
					var r = {
						...
						BILL_TYPE			: billType,
						CARD_CUSTOM_CODE	: formPram.CARD_CUSTOM_CODE,    // 20210225 CARD_CUSTOM_CODE(카드사) 추가
						...
						SPEC				: rec.get('SPEC'),
						SALE_DATE			: panelResult.getValue('SALE_DATE'),
						NATION_INOUT		: formPram.NATION_INOUT,
			...
			if(params.PGM_ID == 'srq100skrv') {
				...
				panelResult.setValue('DIV_CODE'		, record.get('ISSUE_DIV_CODE'));		//20210503 수정: params.DIV_CODE -> record.get('ISSUE_DIV_CODE')
			...
			//20210503 주석: srq100skrv와 동일한 로직으로 변경
			} else if(params.PGM_ID == 'srq100ukrv') {
				//data set 시작
				var record = params.record;
				panelResult.setValue('DIV_CODE'		, record[0].get('ISSUE_DIV_CODE'));
				panelResult.setValue('CUSTOM_CODE'	, record[0].get('CUSTOM_CODE'));
				panelResult.setValue('CUSTOM_NAME'	, record[0].get('CUSTOM_NAME'));
				var param = {						//20210504 수정: 필요한 데이터만 param으로 넘기도록 수정
					DIV_CODE		: record[0].data.DIV_CODE,
					ISSUE_REQ_NUM	: record[0].data.ISSUE_REQ_NUM
				}
				//거래처 정보에 따른 전역변수 값 수동으로 set
				popupService.agentCustPopup(param, function(records, response) {
					console.log('records : ', records);
...
//				var records = params.record;
//				var param   = params.formPram;
//
////				panelResult.setValue('DIV_CODE'		, param.DIV_CODE); 
////				2021-01-22 수주사업장과 출고사업장이 다른 경우 오류 발생, 우선 유저귀속 사업장이 세팅이 되도록 변경
//				panelResult.setValue('DIV_CODE'		, UserInfo.divCode);
//				panelResult.setValue('CUSTOM_CODE'	, param.CUSTOM_CODE);
//				panelResult.setValue('CUSTOM_NAME'	, param.CUSTOM_NAME);
//
//				Ext.each(params.record, function(rec,i){
//					var inoutSeq = detailStore.max('INOUT_SEQ');
//					if(!inoutSeq){
...
//					}
//
//					if(rec.get('MONEY_UNIT') != UserInfo.currency){
//						nation = '2';
//					} else {
//						nation = '1';
//					}
...
//						TRANS_RATE			: rec.get('TRANS_RATE'),
//						ORDER_UNIT_Q		: dDoOutQ,
//						ORDER_UNIT_P		: rec.get('ISSUE_REQ_PRICE'),
//						ORIGIN_Q			: dDoOutQ,
//						ORDER_NOT_Q			: dDoOutQ,
//						ISSUE_NOT_Q			: '0',
//						PRICE_TYPE			: rec.get('PRICE_TYPE'),
...
//						GUBUN				: rec.get('FEFER'),
//						TRANS_COST			: 0,
//						ORDER_UNIT_O		: rec.get('ISSUE_REQ_AMT'),
//						INOUT_TAX_AMT		: rec.get('ISSUE_REQ_TAX_AMT'),
//						ORDER_AMT_SUM		: orderAmySum,
//						LOT_NO				: rec.get('LOT_NO'),
//						SPEC				: rec.get('SPEC'),
//						SALE_DATE			: panelResult.getValue('SALE_DATE'),
...
//						ISSUE_REQ_NUM		: rec.get('ISSUE_REQ_NUM'),		//20210430 추가
//						ISSUE_REQ_SEQ		: rec.get('ISSUE_REQ_SEQ')		//20210430 추가
//						//ORDER_UNIT_Q		: rec.get('ISSUE_QTY')
//					}
//					detailGrid.createRow(r, 'INOUT_SEQ');
//					var record = detailGrid.getSelectedRecord();
//					UniSales.fnStockQ(record, UniAppManager.app.cbStockQ, UserInfo.compCode, record.get('DIV_CODE'), record.get('ITEM_STATUS'), record.get('ITEM_CODE'), record.get('WH_CODE'));
//				});
		...
		onNewDataButtonDown: function() {
			...
			if(!panelResult.setAllFieldsReadOnly(true)){
				...
				var taxInout = '';
				if(!Ext.isEmpty(CustomCodeInfo.gsTaxInout)){
				    taxInout = CustomCodeInfo.gsTaxInout;
				}else{
					taxInout = '1'; // 20210906 행 추가시 세액포함여부 값 들어가지 않아 금액 계산되지 않는 문제로 수정.
				}
...
//				str103ukrvService.deptWhcode(param, function(provider, response){
...
//				});
		...
		onResetButtonDown: function() {
			//20201216 추가: 창고변경 시, 창고에 설정되어 있는 기본창고cell 가져오는 로직 추가
			gsQueryFlag = false;
...
//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('str103ukrvAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
...
//			panelResult.setAllFieldsReadOnly(true);
		...
		fnTaxCalculate: function(rtnRecord, dOrderO, taxType, taxInout) {
			...
			if(sTaxInoutType=="1") {	//별도
				dOrderAmtO	= dOrderO//UniSales.fnAmtWonCalc(dOrderO, sWonCalBas, numDigitOfPrice);
				//20191001 세액 구하는 함수 생성: UserInfo.currency = 'KRW"이면 소숫점 이하 버림 -> 20191212 fnAmtWonCalc로 변경
				dTaxAmtO	= dOrderAmtO * dVatRate / 100 // UniSales.fnAmtWonCalc(dOrderAmtO * dVatRate / 100, sWonCalBas, numDigitOfPrice);
...
//				dTaxAmtO	= UniSales.fnAmtWonCalc(dAmountI - dOrderAmtO, sWonCalBas, numDigitOfPrice);
			...
			if(sTaxType == "2" || sTaxType == "3") {				//20210429 수정: 영세(sTaxType == "3") 추가
			...
			if(!panelResult.setAllFieldsReadOnly(true)){
				return false;
				 ...
				 Ext.each(records, function(record,i){
					...
					if(Ext.isEmpty(record.get('TRANS_RATE'))){
						newDetailRecords[i].set('TRANS_RATE'		, 1);
					}else{
					...
					// 20210225 : CARD_CUSTOM_CODE (카드사) 추가
					newDetailRecords[i].set('CARD_CUSTOM_CODE'	, record.get('CARD_CUSTOM_CODE'));
		...
		fnMakeSof100tDataRef: function(records) {
			...
			if(!panelResult.setAllFieldsReadOnly(true)){
				 ...
				 Ext.each(records, function(record,i){
					...
					newDetailRecords[i].set('ORDER_TYPE'		, record.get('ORDER_TYPE'));
					newDetailRecords[i].set('BILL_TYPE'			, record.get('BILL_TYPE'));
					newDetailRecords[i].set('CARD_CUSTOM_CODE'	, record.get('CARD_CUSTOM_CODE')); // 20210225 : CARD_CUSTOM_CODE(카드사) 추가
					newDetailRecords[i].set('STOCK_UNIT'		, record.get('STOCK_UNIT'));
					newDetailRecords[i].set('PRICE_YN'			, record.get('PRICE_YN'));
					...
					/* newDetailRecords[i].set('STOCK_Q'		, record.get('STOCK_Q')); 2020.03.06 LOT_NO가 자동으로 할당돼 있으면 해당 LOT재고를 가져오도록 수정
					newDetailRecords[i].set('ORDER_STOCK_Q'	, record.get('STOCK_Q') / lRate); */
					newDetailRecords[i].set('STOCK_Q'		, record.get('R_STOCK_Q'));
					newDetailRecords[i].set('ORDER_STOCK_Q'	, record.get('R_STOCK_Q') / lRate);
					newDetailRecords[i].set('LOT_YN'		, record.get('LOT_YN'));
...
//						detailStore.fnOrderAmtSum();
					...
					case "ORDER_UNIT_Q" :
						if (record.get('INOUT_TYPE_DETAIL') != '82')	//과출고환입의 경우 마이너스 수량 허용 (반품이 아닌 매출확정전 과출고에 때른 환입 유형)
						{
							if (newValue < 0 && !Ext.isEmpty(newValue)) {
								rv='<t:message code="system.message.sales.message034" default="양수만 입력가능합니다."/>';
								break;
...
//									sInout_q = newValue;	//출고량
	...
	//20200311 추가: 라벨출력 컬럼 추가에 따른 로직 신규 추가 - 신환코스텍
	function openlabelPrintWindow() {
		//if(!UniAppManager.app.checkForNewDetail()) return false;
		if(!labelPrintWindow) {
			labelPrintWindow = Ext.create('widget.uniDetailWindow', {
				title		: '<t:message code="system.label.sales.labelprint" default="라벨출력"/>',
				width		: 300,
				height		: 353,
				layout		: {type:'vbox', align:'stretch'},
				...
				listeners	: {
					...
					beforeshow: function ( me, eOpts ) {
						...
						labelPrintSearch.setValue('LABEL_QTY2'				, gsSelRecord2.BOX_Q);			//20200316 수정: 1 -> gsSelRecord2.BOX_Q
						labelPrintSearch.setValue('PRODT_DATE'				, UniDate.get('today'));
						labelPrintSearch.setValue('LABEL_TYPE'				, 1);
						labelPrintSearch.setValue('PRODT_QTY'				, gsSelRecord2.PACK_UNIT_Q);	//20200316 수정: gsSelRecord2.ORDER_UNIT_Q -> gsSelRecord2.PACK_UNIT_Q
						labelPrintSearch.setValue('LABEL_ITEM_CODE'			, gsSelRecord2.ITEM_CODE);
						labelPrintSearch.setValue('KEY_IN_ITEM_CODE'		, gsSelRecord2.ITEM_CODE);
						labelPrintSearch.setValue('KEY_IN_ITEM_NAME'		, gsSelRecord2.KEY_IN_ITEM_NAME);
						labelPrintSearch.setValue('KEY_IN_PART_ITEM_NAME'	, gsSelRecord2.KEY_IN_PART_ITEM_NAME);
					},
...
//			fieldStyle	: 'text-align: center;background-color: yellow; background-image: none;',
			...
			fieldLabel	: '<t:message code="system.label.sales.goodscode" default="제품코드"/>',
		...
		},{	fieldLabel	: '<t:message code="system.label.sales.itemname2" default="품명"/>',
		...
		},{	fieldLabel	: '<t:message code="system.label.sales.partname" default="부품명"/>',
		...
		},{	fieldLabel	: '<t:message code="system.label.sales.weight" default="중량"/>',
			name		: 'KEY_IN_WEIGHT',
			xtype		: 'uniNumberfield',
			hidden		: false,
			readOnly	: false,
		...
		},{	fieldLabel	: '<t:message code="system.label.sales.manufacturingnumber" default="제조번호"/>',
			name		: 'KEY_IN_INOUT_NUM',
			xtype		: 'uniTextfield',
			margin		: '0 0 0 0',
			hidden		: false,
			readOnly	: false,
			...
			items		: [{
				...
				hidden	: false,
				handler : function() {
					if(!labelPrintSearch.getInvalidMessage()) return;
					if(!Ext.isEmpty(labelPrintSearch.getValue('LABEL_QTY')) && !Ext.isEmpty(labelPrintSearch.getValue('LABEL_QTY2'))){
						if(labelPrintSearch.getValue('LABEL_QTY') > labelPrintSearch.getValue('LABEL_QTY2')){
							Unilite.messageBox('출력 시작페이지가 마지막페이지 보다 클 수는 없습니다.');
							return false;
						}
					}
					if(Ext.isEmpty(labelPrintSearch.getValue('LABEL_QTY')) || Ext.isEmpty(labelPrintSearch.getValue('LABEL_QTY2'))){
						Unilite.messageBox('출력 매수를 입력해주세요.');
						return false;
					}
					var selectedDetailRecords	= detailGrid.getSelectedRecords();
					var param					= panelResult.getValues();
					param['INOUT_NUM']			= gsSelRecord2.INOUT_NUM;
					param['INOUT_SEQ']			= gsSelRecord2.INOUT_SEQ;											//20200316 추가
					...
					param['sTxtValue2_fileTitle']	= '<t:message code="system.label.product.labelprint" default="라벨출력"/>';
					param['RPT_ID']					= 'pmr100clrkrv_shin';
					...
					param['KEY_IN_INOUT_NUM']		= labelPrintSearch.getValue('KEY_IN_INOUT_NUM');
					param['LABEL_LOC']				= gsLabelChk;
					var win = Ext.create('widget.ClipReport', {
						url		: CPATH+'/sales/str103clukrv_label.do',
						prgID	: 'pmr100ukrv',
						extParam: param
					});
					win.center();
