...
<t:appConfig pgmId="str410skrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="str410skrv"/>	<!-- 사업장 -->
...
function appMain() {
	 ...
	 * @type
	 */
	var directMasterStore = Unilite.createStore('str410skrvMasterStore1', {
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'str410skrvService.selectList1'
			}
		},
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('str410skrvGrid1', {
		...
		printBtn:function(record){
			...
			param['REPORT_TYPE']		= record.data.DEAL_REPORT_TYPE;

			//20210311 추가: 최우선적으로 공통코드 S148 검색해서 없으면 기존로직 수행하도록 변경 (21, 22 월드와이드 메모리에서 사용)
			str410skrvService.getReportFileInfo(param, function(provider, response) {
				if(provider) {
					param.PATH = provider;
					...
					if(record.data.DEAL_REPORT_TYPE == '15'){
						...
						var win = Ext.create('widget.ClipReport', {
							url		: CPATH+'/sales/str410clskrv_2.do',	// 20201211 수정 str410clskrv_2 -> str410clskrv_cov 로 변경
