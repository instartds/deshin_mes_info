...
<t:appConfig pgmId="str107ukrv"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="str107ukrv" />				<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str107ukrvService.selectDetailList',
			update	: 'str107ukrvService.updateDetail',
			create	: 'str107ukrvService.insertDetail',
			destroy	: 'str107ukrvService.deleteDetail',
			syncAll	: 'str107ukrvService.saveAll'
		}
	});
...
//			margin: '0 0 0 0',
			items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
								if(newValue== true){
									panelDvry.setConfig('collapsed', false);
								}else{
									panelDvry.setConfig('collapsed', true);
								}
					}
				}
			}]
...
/*		,{
		...
		api: {
			load	: 'str107ukrvService.selectMaster',
			submit	: 'str107ukrvService.syncMaster'
		},
	...
	// 운송관리여부 폼
	var panelDvry = Unilite.createSearchForm('dvryForm',{
		region		: 'north',
		layout		: {type : 'uniTable', columns : 5},
		padding		: '1 1 1 1',
		border		:true,
		...
		api: {
			submit: 'str107ukrvService.syncForm'
		}
	});
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('str107ukrvDetailModel', {
		fields: [
...
//			{name: 'PURCHASE_TYPE'			, text:'<t:message code="system.label.sales.purchasecondition" default="매입조건"/>'	, type: 'string', comboType:'AU', comboCode:'YP09', editable: false},
			...
			{name: 'NEW_PRODUCT'			, text:'신제품 여부'			, type: 'boolean'}
		]
	});
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('str107ukrvDetailStore', {
		proxy	: directProxy,
		model	: 'str107ukrvDetailModel',
		autoLoad: false,
		uniOpt	: {
...
//			});
			...
			paramMaster.DVRY_DATE    = UniDate.getDbDateStr(panelDvry.getValue('DVRY_DATE'));
			paramMaster.CAR_NO 		 = panelDvry.getValue('CAR_NO');
			paramMaster.DRIVER 	     = panelDvry.getValue('DRIVER');
			paramMaster.PALLET_TYPE  = panelDvry.getValue('PALLET_TYPE');
			paramMaster.REMARK 		 = panelDvry.getValue('REMARK');
			paramMaster.CARRIER_TYPE = panelDvry.getValue('CARRIER_TYPE');
			paramMaster.PALLET_CNT   = panelDvry.getValue('PALLET_CNT');
			paramMaster.DVRY_NUM     = panelDvry.getValue('DVRY_NUM');
			...
			if(inValidRecs.length == 0) {
				config = {
					...
					success: function(batch, option) {
						...
						panelDvry.setValue("DVRY_NUM", master.INOUT_NUM);
...
//			console.log('first record2 ORDER_UNIT_O'+ firstRecord2.get('ORDER_UNIT_O'));
 	...
 	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('str107ukrvGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
...
//			{dataIndex: 'PURCHASE_P'			, width:90 },
			...
			{dataIndex: 'NEW_PRODUCT'			, width:90, xtype: 'checkcolumn',id:'chkNewProduct', disabledCls : '' },
...
//				grdRecord.set('SALE_BASIS_P'	,record['SALE_BASIS_P']);
	...
	Unilite.defineModel('orderNoMasterModel', {
		fields: [{name: 'DIV_CODE'			, text: '<t:message code="system.label.sales.division" default="사업장"/>'				, type: 'string', comboType: 'BOR120'},
				 ...
				 {name: 'CAR_NO'         	, text: 'CAR_NO'				, type: 'string'},
				 {name: 'CARRIER_TYPE'   	, text: 'CARRIER_TYPE'			, type: 'string'},
				 {name: 'DRIVER'         	, text: 'DRIVER'				, type: 'string'},
				 {name: 'DRIVER_MOBILENO'	, text: 'DRIVER_MOBILENO'		, type: 'string'},
				 {name: 'PALLET_TYPE'       , text: 'PALLET_TYPE'			, type: 'string'},
				 {name: 'PALLET_CNT'        , text: 'PALLET_CNT'			, type: 'string'},
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str107ukrvService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('str107ukrvOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
		...
		columns:  [
			...
			{ dataIndex: 'DVRY_NUM'			, width: 93, hidden: true },
			...
			{ dataIndex: 'REMARK'           , width: 93, hidden: true }
		...
		returnData: function(record) {
			...
			panelDvry.setValue('DVRY_NUM'		, record.get('DVRY_NUM'));
			panelDvry.setValue('DVRY_DATE'		, record.get('DVRY_DATE'));
			panelDvry.setValue('CAR_NO'         , record.get('CAR_NO' ));
			panelDvry.setValue('CARRIER_TYPE'   , record.get('CARRIER_TYPE'));
			panelDvry.setValue('DRIVER'         , record.get('DRIVER'        ));
			panelDvry.setValue('DRIVER_MOBILENO', record.get('DRIVER_MOBILENO'));
			panelDvry.setValue('PALLET_TYPE'    , record.get('PALLET_TYPE'    ));
			panelDvry.setValue('PALLET_CNT'     , record.get('PALLET_CNT'     ));
			panelDvry.setValue('REMARK'         , record.get('REMARK'         ));
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('str107ukrvorderNoDetailGrid', {
		// title: '기본',
		layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
...
//			name:'CUSTOM_NAME'
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('str107ukrvRequestModel', {
		fields: [
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('str107ukrvRequestStore', {
		model: 'str107ukrvRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str107ukrvService.selectRequestiList'
			}
		},
		loadStoreRecords : function() {
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('str107ukrvRequestGrid', {
		store	: requestStore,
		layout	: 'fit',
...
//						requestSearch.setValue('ISSUE_DATE_FR'	, UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('str107ukrvsalesOrderForm', {
		layout :  {type : 'uniTable', columns : 3},
		items :[
			Unilite.popup('DIV_PUMOK',{
			fieldLabel:'<t:message code="system.label.sales.item" default="품목"/>' ,
...
//		},{
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('str107ukrvsalesOrderModel', {
		fields: [
			{ name: 'ORDER_NUM'			, text:'<t:message code="system.label.sales.sono" default="수주번호"/>'					,type : 'string' },
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('str107ukrvsalesOrderStore', {
		model	: 'str107ukrvsalesOrderModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str107ukrvService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function() {
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('str107ukrvsalesOrderGrid', {
		layout	: 'fit',
...
//						salesOrderSearch.setValue('DVRY_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('DVRY_DATE_TO')));
	...
	var linkedStore = Unilite.createStore('str107ukrvlinkedStore', {
		model: 'str107ukrvRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,	// 상위 버튼 연결
	...
	Unilite.Main({
		...
		borderItems	: [{
			...
			border	: false,
			items	:[
				panelResult,panelDvry, detailGrid
			]
		}],
...
//				str107ukrvService.deptWhcode(param, function(provider, response){
...
//				});
		...
		onResetButtonDown: function() {
			...
			panelResult.setAllFieldsReadOnly(false);
			panelDvry.clearForm();
			panelDvry.setAllFieldsReadOnly(false);
...
//				}
			...
			if(!detailStore.isDirty()) {
				if(panelDvry.isDirty() && !Ext.isEmpty(panelResult.getValue('INOUT_NUM'))) {
					this.fnMasterSave();
				}
			}
		},
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('str107ukrvAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config) {
			var fp = Ext.getCmp('str107ukrvFileUploadPanel');
...
//			panelResult.setValue('MONEY_UNIT',BsaCodeInfo.gsMoneyUnit);
			...
			if(BsaCodeInfo.gsDvryYn == 'Y'){
				panelDvry.setConfig('collapsed', false);
				panelResult.setValue('DVRY_MANAGE_YN', 'Y');
			}else{
				panelDvry.setConfig('collapsed', true);
			}
			panelDvry.setValue('PALLET_CNT',0);
		},
