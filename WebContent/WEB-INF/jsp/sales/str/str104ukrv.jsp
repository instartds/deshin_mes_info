...
<t:appConfig pgmId="str104ukrv"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="str104ukrv" />		<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str104ukrvService.selectDetailList',
			update	: 'str104ukrvService.updateDetail',
			create	: 'str104ukrvService.insertDetail',
			destroy	: 'str104ukrvService.deleteDetail',
			syncAll	: 'str104ukrvService.saveAll'
		}
	});
					...
					//20200107 로직 수정
		 			if(Ext.isDate(newValue)) {
						if(newValue != oldValue &&!Ext.isEmpty(newValue && !Ext.isEmpty(panelResult.getValue('DIV_CODE')))){
							UniSales.fnGetClosingInfo(
								//20200107 조회, 참조 후에도 수정가능하게 변경함으로 인해 추가로직 적용을 위한 새로운 함수 생성
...
//			colspan	: 2,
	...
	/** 디테일 정보를 가지고 있는 Grid
	 */
	Unilite.defineModel('str104ukrvDetailModel', {
		fields: [
...
//			{name: 'PURCHASE_TYPE'			, text:'<t:message code="system.label.sales.purchasecondition" default="매입조건"/>'		, type: 'string', comboType:'AU', comboCode:'YP09', editable: false},
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('str104ukrvDetailStore', {
		model	: 'str104ukrvDetailModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
			...
			if(inValidRecs.length == 0) {
				config = {
					...
					success: function(batch, option) {
						...
						//20200113 일괄 등록은 저자 후 reset하도록 수정
...
//						}
	...
	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('str104ukrvGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
...
//				grdRecord.set('SALE_BASIS_P'	,record['SALE_BASIS_P']);
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 'str104ukrvService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('str104ukrvorderNoDetailGrid', {
		store	: orderNoMasterStore,
		layout	: 'fit',
		uniOpt	: {
			useRowNumberer: false
...
//						orderNoSearch.setValue('ORDER_TYPE'		, panelResult.getValue('ORDER_TYPE'));
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('str104ukrvRequestModel', {
		fields: [
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('str104ukrvRequestStore', {
		model	: 'str104ukrvRequestModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'str104ukrvService.selectRequestiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('str104ukrvRequestGrid', {
		store	: requestStore,
		layout	: 'fit',
...
//						requestSearch.setValue('ISSUE_DATE_FR'	, UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('str104ukrvsalesOrderForm', {
		layout	: {type : 'uniTable', columns : 3},
		items	: [
			Unilite.popup('DIV_PUMOK',{
			fieldLabel		: '<t:message code="system.label.sales.item" default="품목"/>' ,
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('str104ukrvsalesOrderModel', {
		fields: [
			{ name: 'ORDER_NUM'			, text:'<t:message code="system.label.sales.sono" default="수주번호"/>'					,type : 'string' },
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('str104ukrvsalesOrderStore', {
		model	: 'str104ukrvsalesOrderModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 'str104ukrvService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function() {
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('str104ukrvsalesOrderGrid', {
...
//				};
//				str104ukrvService.deptWhcode(param, function(provider, response){
...
//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('str104ukrvAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config) {
			var fp = Ext.getCmp('str104ukrvFileUploadPanel');
...
//						}
						...
						// 20210224 : CARD_CUSTOM_CODE(카드사) 추가
