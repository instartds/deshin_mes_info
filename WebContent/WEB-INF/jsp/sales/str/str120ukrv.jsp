...
<t:appConfig pgmId="str120ukrv">
	<t:ExtComboStore comboType="BOR120" pgmId="str120ukrv"/> 				<!-- 사업장 -->
	<t:ExtComboStore comboType="AU" comboCode="B024" />						<!-- 수불담당 -->
	<t:ExtComboStore comboType="AU" comboCode="B013" />						<!-- 판매단위 -->
	<t:ExtComboStore comboType="AU" comboCode="S006" />                        <!-- 판매단위 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str120ukrvService.selectDetailList',
			update	: 'str120ukrvService.updateDetail',
			create	: 'str120ukrvService.insertDetail',
			destroy	: 'str120ukrvService.deleteDetail',
			syncAll	: 'str120ukrvService.saveAll'
		}
	});
			...
			listeners	: {
				expand: function(combo, eOpts ){
					...
					if(inoutPrsn.getValue() == "" || inoutPrsn.getValue() == null){
						Unilite.messageBox('<t:message code="system.message.sales.message053" default="담당을 먼저 선택하셔야 합니다."/>');
...
//					panelResult.getField('BARCODE').focus();
				...
				//20200130 포커스 이동로직 수정
				select: function(combo, record, eOpts){
					panelResult.getField('BARCODE').focus();
				},
				specialkey: function(field, event) {
					if(event.getKey() == event.ENTER) {
						setTimeout(function(){panelResult.getField('BARCODE').focus()}, 10);
					}
				}
			}
		},{
...
//			autoCreate	: {tag: 'input', type: 'text', size: '20', style :'IME-MODE:DISABLED' ,autocomplete: 'off', maxlength: '8'},
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('str120ukrvDetailModel', {
		fields: [
			...
			{name: 'INOUT_TYPE_DETAIL'	,text: '<t:message code="system.label.sales.trantype" default="수불유형"/>'					,type: 'string', comboType: 'AU', comboCode: 'S006', allowBlank: false},
			...
			//20191104 유통기한(MAKE_EXP_DATE) 추가
			{name: 'MAKE_EXP_DATE'		,text: '<t:message code="system.label.sales.expirationdate" default="유통기한"/>'			,type: 'uniDate'}
		]
	});

	//마스터 스토어 정의
	var detailStore = Unilite.createStore('str120ukrvDetailStore', {
		model	: 'str120ukrvDetailModel',
		autoLoad: false,
		uniOpt	: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('str120ukrvGrid');
	...
	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('str120ukrvGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
		...
		tbar	: [{
			itemId	: 'requestBtn',
			text	: '<div style="color: blue"><t:message code="system.label.sales.productionqtyrefernoinspec2" default="생산량참조(무검사품)"/></div>',
			hidden	: true,
			handler	: function() {
				openProdtWindow();
			}
			...
			itemId	: 'refBtn',
			text	: '<div style="color: blue"><t:message code="system.label.sales.inspecresultrefer" default="검사결과참조"/></div>',
			hidden	: true,
			handler	: function() {
		...
		columns: [
						...
						elm.editor.on(
							'beforequery',function(queryPlan, eOpts) {
								...
								store.filterBy(function(item){
									return item.get('option') == record.get('TO_DIV_CODE');
								})
							}
						);
			...
			//20190612 수정 가능하도록 변경
			{ dataIndex: 'TO_DIV_CODE'			, width: 100	, hidden: false },
...
//					if(e.field=='INOUT_Q'		) return false;
					if(e.field=='WH_CODE'		) return false;
					if(e.field=='WH_CELL_CODE'	) return false;		//20200714 수정: 주석 해제 - 수불데이터 처리가 안 됨
//					if(e.field=='INOUT_CODE'	) return false;
					if(e.field=='TO_DIV_CODE'	) return false;
					if(e.field=='ORG_CD'		) return false;
					if(e.field=='ORIGINAL_Q'	) return false;
...
//			var grdRecord = this.uniOpt.currentRecord;
	...
	var inNoMasterStore = Unilite.createStore('inNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api	: {
				read : 'str120ukrvService.selectInNumMasterList'
			}
		},
		loadStoreRecords : function()	{
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('str120ukrvInNoMasterGrid', {
		// title: '기본',
		layout	: 'fit',
		...
		returnData: function(record) {
			...
			panelResult.setValues({
				//20190612 추가: DIV_CODE, WH_CELL_CODE
	...
	var prodtSearch = Unilite.createSearchForm('prodtForm', {
		...
		items :[{
			...
			listeners: {
				onValueFieldChange: function(field, newValue, oldValue){
					if(!Ext.isObject(oldValue)) {
						prodtSearch.setValue('ITEM_NAME', '');
					}
				},
				onTextFieldChange: function(field, newValue, oldValue){
					if(!Ext.isObject(oldValue)) {
						prodtSearch.setValue('ITEM_CODE', '');
					}
				},
				applyextparam: function(popup){
					popup.setExtParam({'DIV_CODE': prodtSearch.getValue('DIV_CODE')});
				}
			}
		}),{
			fieldLabel: '<t:message code="system.label.sales.division" default="사업장"/>',
	...
	//생산량참조 모델 정의
	Unilite.defineModel('str120ukrvProdtModel', {
		fields: [
			{name: 'ITEM_CODE'		,text: '<t:message code="system.label.sales.item" default="품목"/>'					, type: 'string'},
			{name: 'ITEM_NAME'		,text: '<t:message code="system.label.sales.itemname" default="품목명"/>'				, type: 'string'},
			{name: 'SPEC'			,text: '<t:message code="system.label.sales.spec" default="규격"/>'					, type: 'string'},
	...
	//생산량참조 스토어 정의
	var prodtStore = Unilite.createStore('str120ukrvProdtStore', {
		model	: 'str120ukrvProdtModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy	: {
			type: 'direct',
			api	: {
				read	: 'str120ukrvService.selectProdtList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	//생산량참조 그리드 정의
	var prodtGrid = Unilite.createGrid('str120ukrvProdtGrid', {
		// title: '기본',
		layout	: 'fit',
		store	: prodtStore,
		...
		returnData: function()	{
			...
			Ext.each(records, function(record,i){
				if(Ext.isEmpty(record.data.DIV_CODE)) {
					record.data.DIV_CODE = Ext.getCmp('prodtForm').getValue('DIV_CODE');
				}
				UniAppManager.app.onNewDataButtonDown();
...
//		prodtSearch.setValue('WH_CODE'		, panelResult.getValue('WH_CODE'));
		...
		if(!referProdtWindow) {
			referProdtWindow = Ext.create('widget.uniDetailWindow', {
				title	: '<t:message code="system.label.sales.productionqtyrefernoinspec2" default="생산량참조(무검사품)"/>',
				width	: 1080,
				height	: 580,
				layout	: {type:'vbox', align:'stretch'},
	 ...
	 */
	//검사결과 참조 폼 정의
	var inspecResultSearch = Unilite.createSearchForm('str120ukrvInspecResultForm', {
		layout	: {type : 'uniTable', columns : 3},
		items	: [{
			fieldLabel: '<t:message code="system.label.sales.productiondate" default="생산일"/>',
			xtype: 'uniDateRangefield',
			...
			listeners: {
				onValueFieldChange: function(field, newValue, oldValue){
					if(!Ext.isObject(oldValue)) {
						inspecResultSearch.setValue('ITEM_NAME', '');
					}
				},
				onTextFieldChange: function(field, newValue, oldValue){
					if(!Ext.isObject(oldValue)) {
						inspecResultSearch.setValue('ITEM_CODE', '');
					}
				},
				applyextparam: function(popup){
					popup.setExtParam({'DIV_CODE': inspecResultSearch.getValue('DIV_CODE')});
				}
			}
		}),{
			fieldLabel: '<t:message code="system.label.sales.division" default="사업장"/>',
	...
	//검사결과 참조 모델 정의
	Unilite.defineModel('str120ukrvInspecResultModel', {
		fields: [
			{ name: 'ITEM_CODE'			, text:'<t:message code="system.label.sales.item" default="품목"/>'					,type : 'string' },
			{ name: 'ITEM_NAME'			, text:'<t:message code="system.label.sales.itemname" default="품목명"/>'				,type : 'string' },
			{ name: 'SPEC'				, text:'<t:message code="system.label.sales.spec" default="규격"/>'					,type : 'string' },
			...
			{ name: 'SORT_KEY'			, text:'SORT_KEY'		,type : 'string' },
			{ name: 'INSPEC_NUM'		, text:'<t:message code="system.label.sales.inspecno" default="검사번호"/>'				,type : 'string' },
			{ name: 'INSPEC_SEQ'		, text:'<t:message code="system.label.sales.seq" default="순번"/>'					,type : 'string' },
			//20191104 유통기한(MAKE_EXP_DATE) 추가
			{ name: 'MAKE_EXP_DATE'		, text: '<t:message code="system.label.sales.expirationdate" default="유통기한"/>'		,type: 'uniDate'}
		]
	});
	//검사결과 참조 스토어 정의
	var inspecResultStore = Unilite.createStore('str120ukrvInspecResultStore', {
		model	: 'str120ukrvInspecResultModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,			// 상위 버튼 연결
			editable	: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read	: 'str120ukrvService.inspecResult'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	...
	//검사결과 참조 그리드 정의
	var inspecResultGrid = Unilite.createGrid('str120ukrvInspecResultGrid', {
		// title: '기본',
		layout	: 'fit',
		...
		returnData: function()	{
			...
			Ext.each(records, function(record,i){
				if(Ext.isEmpty(record.data.DIV_CODE)) {
					record.data.DIV_CODE = Ext.getCmp('str120ukrvInspecResultForm').getValue('DIV_CODE');
				}
				UniAppManager.app.onNewDataButtonDown();
	...
	Unilite.Main({
		...
		fnInitBinding: function() {
			...
			if(BsaCodeInfo.gsInspecFlag=="Y")	{
				requestBtn.setText('<div style="color: blue"><t:message code="system.label.sales.productionqtyrefernoinspec2" default="생산량참조(무검사품)"/></div>');
				requestBtn.show();
				refBtn.show();
			} else {
				requestBtn.setText('<div style="color: blue"><t:message code="system.label.sales.productionqtyrefer" default="생산량참조"/></div>');
				requestBtn.show();
//				refBtn.show();
//			}
...
//				detailGrid.createRow(r, 'ITEM_CODE', detailGrid.getStore().getCount() - 1);
				detailGrid.createRow(r, 'ITEM_CODE', - 1);		//대량 데이터 있을 경우, 바코드 입력 시 바코드필드에 포커스가 가지 않음 -> 제일 위에 행 추가하도록 수정
				panelResult.setAllFieldsReadOnly(true);
		},
...
//			}
		...
		checkForNewDetail:function() {
			if(BsaCodeInfo.gsAutoType=='N' && Ext.isEmpty(panelResult.getValue('INOUT_NUM')))	{
				Unilite.messageBox('<t:message code="system.label.sales.receiptno" default="입고번호"/>:<t:message code="system.message.sales.datacheck001" default="필수입력 항목입니다."/>');
				return false;
			}
			/**
			 * 마스터 데이타 수정 못 하도록 설정
...
//			WH_CODE	: panelResult.getValue('WH_CODE'),
		...
		str120ukrvService.getItemCode(param, function(provider, response){
			if(!Ext.isEmpty(provider) && provider[0].ITEM_CODE){
				barcodeItemCode = provider[0].ITEM_CODE;
				...
				if(flag) {
					...
					str120ukrvService.getItemInfo(param, function(provider, response){
						if(!Ext.isEmpty(provider)){
							if(provider[0].INSPEC_YN == 'Y') {			//품질(검사)대상이 'Y'일 경우, 검사참조 쿼리 실행
								...
								str120ukrvService.inspecResult(yParam, function(yProvider, yResponse){
									if(!Ext.isEmpty(yProvider)){
										...
										beep();
										gsText = '<t:message code="system.message.sales.message018" default="해당 BARCODE에 대한 입고가능한 데이터가 없습니다."/>';
										openAlertWindow(gsText);
										Ext.getBody().unmask();
										panelResult.getField('BARCODE').focus();
										return false;
								...
								str120ukrvService.selectProdtList(nParam, function(nProvider, nResponse){
...
//						}
						...
						if(newValue > notinQ + originQ ){
							rv = '<t:message code="system.message.sales.message052" default="입고량은 미입고량을 초과할수 없습니다."/>'
							break;
						} else {
...
//						}
					...
					case "INOUT_CODE" :	//작업장
							...
							if(Ext.isEmpty(wkDivCode)) {
								Unilite.messageBox('작업장의 사업장 코드를 가져올 수 없습니다. 작업장 정보를 확인하여 주십시오.');
								break;
							}
							...
							//20190612 주석
//							record.set('DIV_CODE', wkDivCode);	////맞나???? -> 그럴리가 있나?
							break;
