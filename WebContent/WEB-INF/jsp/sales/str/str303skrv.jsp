...
<t:appConfig pgmId="str303skrv" >
	<t:ExtComboStore comboType="BOR120" pgmId="str303skrv" />	<!-- 사업장 -->
...
function appMain() {
	 ...
	 * @type
	 */
	Unilite.defineModel('str303skrvModel', {
		fields:  [
...
//			{name: 'UPDATE_DB_TIME'		,text: '<t:message code="system.label.sales.entrydate" default="등록일"/>'						,type: 'string'}
	...
	var directMasterStore1 = Unilite.createStore('str303skrvMasterStore',{
		model	: 'str303skrvModel',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 'str303skrvService.selectList1'
			}
		},
	...
	var masterGrid = Unilite.createGrid('str303skrvGrid1', {
...
//			{ dataIndex: 'SORT_KEY'			,width: 120	, hidden: true}
	...
	Unilite.Main ({
		...
		fnInitBinding: function() {
			...
			str303skrvService.userWhcode({}, function(provider, response) {
			...
			if(!UserInfo.appOption.collapseLeftSearch)	{
				panelSearch.onLoadSelectText('ITEM_LEVEL1');;
			}else {
	...
	function summaryHandle(type){
		...
		if(type == 'Y'){
			directMasterStore1.setConfig('groupField', 'ITEM_CODE');
			viewNormal.getFeature('masterGridTotal').toggleSummaryRow(true);
			viewNormal.getFeature('masterGridSubTotal').toggleSummaryRow(true);
		}else{
			directMasterStore1.setConfig('groupField', '');
