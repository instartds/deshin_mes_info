...
<t:appConfig pgmId="str105ukrv"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="str105ukrv" /> 				<!-- 사업장 -->  
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str105ukrvService.selectList'/*,
			update	: 'str105ukrvService.updateDetail',
			create	: 'str105ukrvService.insertDetail',
			destroy	: 'str105ukrvService.deleteDetail',
			syncAll	: 'str105ukrvService.saveAll'*/
		}
	});
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'str105ukrvService.selectList2',
			update	: 'str105ukrvService.updateDetail2',
			create	: 'str105ukrvService.insertDetail2',
			destroy	: 'str105ukrvService.deleteDetail2',
			syncAll	: 'str105ukrvService.saveAll2'
		}
	});
...
//			holdable	: 'hold',
			listeners	: {
				change: function(combo, newValue, oldValue, eOpts) {
					//20200108 로직 추가
					if(barcodeStore.data.length > 0) {
						var records = barcodeStore.data.items;
...
//								Unilite.messageBox('선택된 수불데이터가 없습니다.');
...
//							}
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('str105ukrvDetailModel', { 
		fields: [
...
//			{name: 'PURCHASE_TYPE'			, text:'<t:message code="system.label.sales.purchasecondition" default="매입조건"/>'				, type: 'string'		, comboType:'AU', comboCode:'YP09', editable: false},
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('str105ukrvDetailStore', {
		model	: 'str105ukrvDetailModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
...
//				panelResult.setValue('TOT_QTY'			,dtotQty);   							//수량총계
	...
	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('str105ukrvGrid', {
		store	: detailStore,
		itemId	: 'str105ukrvGrid',
		selModel: 'rowmodel',
		layout	: 'fit',
		region	: 'center',
		flex	: 1,
...
//			grdRecord.set('ORDER_UNIT_Q'		, record['R_ALLOC_Q']);
	...
	var barcodeStore = Unilite.createStore('str105ukrvBarcodeStore', {
		model	: 'str105ukrvDetailModel',
		proxy	: directProxy2,
		autoLoad: false,
		uniOpt	: {
...
//						})
	...
	var barcodeGrid = Unilite.createGrid('str105ukrvBarcodeGrid', {
		store	: barcodeStore,
		itemId	: 'str105ukrvBarcodeGrid',
		layout	: 'fit',
		region	: 'south',
		split	: true,
		flex	: 0.5,
...
//			{dataIndex: 'PURCHASE_TYPE'			,width:66	,hidden: true },
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'str105ukrvService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('str105ukrvOrderNoMasterGrid', {
		// title: '기본',
		layout	: 'fit',	   
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('str105ukrvorderNoDetailGrid', {
		layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
			useRowNumberer: false
...
//						orderNoSearch.setValue('ORDER_TYPE',panelResult.getValue('ORDER_TYPE'));
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('str105ukrvRequestModel', {
		fields: [
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('str105ukrvRequestStore', {
		model: 'str105ukrvRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'str105ukrvService.selectRequestiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('str105ukrvRequestGrid', {
		// title: '기본',
		layout	: 'fit',
		store	: requestStore,
...
//						requestSearch.setValue('ISSUE_DATE_FR'	, UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('str105ukrvsalesOrderForm', {
		layout :  {type : 'uniTable', columns : 2},
		items :[
			Unilite.popup('DIV_PUMOK',{
			fieldLabel:'<t:message code="system.label.sales.item" default="품목"/>' ,
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('str105ukrvsalesOrderModel', {
		fields: [
			{ name: 'ORDER_NUM'				, text:'<t:message code="system.label.sales.sono" default="수주번호"/>'			,type : 'string' },
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('str105ukrvsalesOrderStore', {
		model	: 'str105ukrvsalesOrderModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy	: {
			type: 'direct',
			api: {
				read	: 'str105ukrvService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function()	{
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('str105ukrvsalesOrderGrid', {
		// title: '기본',
		layout : 'fit',
...
//				panelResult.setAllFieldsReadOnly(true);
		...
		onDeleteDataButtonDown: function() {
			...
			if(activeGridId == 'str105ukrvGrid' && !Ext.isEmpty(selRow) && selRow.phantom === true)	{			//detailGrid 삭제 함수 호출
				fnDeleteDetail(selRow);
				
			} else if(activeGridId != 'str105ukrvGrid' && !Ext.isEmpty(selRow2) && selRow2.phantom === true) {	//barcodeGrid 삭제 함수 호출
				fnBarcodeGrid(selRow, selRow2)
					
			} else if (confirm('<t:message code="system.message.base.confirm001" default="현재행을 삭제 합니다. 삭제 하시겠습니까?"/>')) {
				if( activeGridId == 'str105ukrvGrid' && !Ext.isEmpty(selRow)) {				//detailGrid 삭제 함수 호출
				...
				} else if(activeGridId != 'str105ukrvGrid' && !Ext.isEmpty(selRow2)) {		//barcodeGrid 삭제 함수 호출
...
//				gsLotNoS = ''
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('str105ukrvAdvanceSerch');	
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		setDefault: function() {
			...
			str105ukrvService.deptWhcode(param, function(provider, response){
...
//				dTaxAmtO = 	 UniSales.fnAmtWonCalc(dAmountI - dOrderAmtO, sWonCalBas, numDigitOfPrice);
			...
			//자사화폐가 "KRW"일 때, 금액 소숫점 첫째자리에서 반올림 -> 20191212 동일로직 위에 추가: 주석
...
//			var dOrderQ = Unilite.nvl(rtnRecord.get('ORDER_Q'), 0);
		...
		fnExchngRateO:function(isIni, refProvider) {
			...
			salesCommonService.fnExchgRateO(param, function(provider, response) {
				if(!Ext.isEmpty(provider)){
					//20200113 !isLoad 추가
					if(!isLoad && provider.BASE_EXCHG == "1" && !isIni  && !Ext.isEmpty(panelResult.getValue('MONEY_UNIT')) && panelResult.getValue('MONEY_UNIT') != BsaCodeInfo.gsMoneyUnit){
...
//						panelResult.getField('MONEY_UNIT').focus();
					...
					if(barcodeStore.data.length > 0) {
						var detailRecords = barcodeStore.data.items;
	...
	function fnEnterIssueBarcode(newValue) {
		...
		if(flag) {
			...
			//출하지시참조 조회쿼리 호출
			str105ukrvService.selectRequestiList(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
...
//					panelResult.getField('BARCODE')focus();
					...
					Unilite.messageBox('<t:message code="system.label.sales.message002" default="입력하신 출하지시번호의 데이터가 존재하지 않습니다."/>');
					Ext.getBody().unmask();
					panelResult.setValue('ISSUE_BARCODE', '');
					panelResult.getField('ISSUE_BARCODE').focus();
					return false;
...
//				Unilite.messageBox('품목 정보가 없습니다.');
				beep();
...
//				Unilite.messageBox('단가 정보가 없습니다.');
				beep();
...
//							Unilite.messageBox(provider[i].ERR_MSG);
							beep();
...
//						var numDigitOfPrice = UniFormat.Price.length - UniFormat.Price.indexOf(".");
	...
	var activeGridId = 'str105ukrvGrid';
}
</script>