...
<t:appConfig pgmId="srq100ukrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="srq100ukrv" />		<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'srq100ukrvService.selectList',
			update	: 'srq100ukrvService.updateDetail',
			create	: 'srq100ukrvService.insertDetail',
			destroy	: 'srq100ukrvService.deleteDetail',
			syncAll	: 'srq100ukrvService.saveAll'
		}
	});
...
//			me.setValue('EXCHAGE_RATE',		record.get('EXCHANGE_RATE'));
	...
	//마스터 모델 정의
	Unilite.defineModel('srq100ukrvDetailModel', {
		fields: [
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('srq100ukrvDetailStore', {
		model	: 'srq100ukrvDetailModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
...
//							inValidRecs = [].concat(inValidRecs, [record])
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('srq100ukrvGrid');
...
//				var record = detailStore.data.items[0];
	...
	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('srq100ukrvGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
			...
			{dataIndex: 'CUSTOM_NAME'					, width: 133 ,
				editor: Unilite.popup('CUST_G',{
					...
					listeners:{
						'onSelected': {
							fn: function(records, type  ){
								var grdRecord = Ext.getCmp('srq100ukrvGrid').uniOpt.currentRecord;
...
/*							{
	...
	var requestNoMasterStore = Unilite.createStore('requestNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'srq100ukrvService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//참조내역 모델 정의
	Unilite.defineModel('srq100ukrvReferModel', {
		fields: [
			{name: 'CUSTOM_CODE'		,text: '<t:message code="system.label.sales.client" default="고객"/>'					, type: 'string'},
			{name: 'CUSTOM_NAME'		,text: '<t:message code="system.label.sales.client" default="고객"/>'					, type: 'string'},
			{name: 'ITEM_CODE'			,text: '<t:message code="system.label.sales.item" default="품목"/>'					, type: 'string'},
	...
	//참조내역 스토어 정의 (수주참조)
	var referStore = Unilite.createStore('srq100ukrvReferStore', {
		model	: 'srq100ukrvReferModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'srq100ukrvService.selectReferList'
			}
		},
		loadStoreRecords : function() {
	...
	//참조내역 그리드 정의
	var referGrid = Unilite.createGrid('srq100ukrvReferGrid', {
		store	: referStore,
		layout	: 'fit',
...
//						referSearch.clearForm();
	...
	//참조내역 모델 정의
	Unilite.defineModel('srq100ukrvReferSCMModel', {
		fields: [
			{name: 'GUBUN'				,text: '<t:message code="system.label.sales.classfication" default="구분"/>'		, type: 'string'},
			{name: 'CUSTOM_CODE'		,text: '<t:message code="system.label.sales.client" default="고객"/>'				, type: 'string'},
			{name: 'CUSTOM_NAME'		,text: '<t:message code="system.label.sales.client" default="고객"/>'				, type: 'string'},
	...
	//참조내역 스토어 정의
	var referSCMStore = Unilite.createStore('srq100ukrvReferSCMStore', {
		model	: 'srq100ukrvReferSCMModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'srq100ukrvService.selectReferSCMList'
			}
		},
		loadStoreRecords : function() {
			var param = referSearch.getValues();
	...
	//참조내역 그리드 정의
	var referSCMGrid = Unilite.createGrid('srq100ukrvRefeSCMrGrid', {
		store	: referSCMStore,
		layout	: 'fit',
		selModel:	Ext.create('Ext.selection.CheckboxModel', { checkOnly : false, toggleOnClick:false, mode: 'SIMPLE' }),
		uniOpt	: {
...
//					srq100ukrvService.selectReferList(param, function(provider, response){
...
//			}
		...
		processParams: function(params) {
			...
			if(params.PGM_ID == 'sof100ukrv') { //수주등록 에서 링크넘어올시
				...
				srq100ukrvService.selectReferList(formPram, function(provider, response){
					if(provider) {
...
//			}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('srq100ukrvAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
