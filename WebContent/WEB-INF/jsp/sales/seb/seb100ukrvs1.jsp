<%@page language="java" contentType="text/html; charset=utf-8"%>
	var multiCompCodeProxy1;
	<c:if test="${multiCompCode == 'true'}">
	multiCompCodeProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	var seb100ukrvProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	Unilite.defineModel('seb100ukrvsModel1', {
		fields: [
			...
			{name: 'REF_CODE1'			, text: '적용범위(이상)'	, type: 'float', decimalPrecision: 2, format:'0,000.00'},
			{name: 'REF_CODE2'			, text: '적용범위(미만)'	, type: 'float', decimalPrecision: 2, format:'0,000.00'},
	...
	var seb100ukrvStore1 = Unilite.createStore('seb100ukrvStore1',{
		model	: 'seb100ukrvsModel1',
		proxy	: Ext.isEmpty(multiCompCodeProxy1) ? seb100ukrvProxy1:multiCompCodeProxy1,
		autoLoad: false,
		uniOpt	: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				panelDetail.down('#basisInfo').uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		listeners: {
			load: function (store, records) {
				if(records && records.length > 0) {
					var mGrid	= panelDetail.down('#seb100ukrvGrid1');
	...
	var basisInfo = {
		...
		items	: [{
			...
			items		: [{
				...
				xtype		: 'uniGridPanel',
				itemId		: 'seb100ukrvGrid1',
				store		: seb100ukrvStore1,
				autoScroll	: false,
				dockedItems	: [{
					xtype	: 'toolbar',
					dock	: 'top',
