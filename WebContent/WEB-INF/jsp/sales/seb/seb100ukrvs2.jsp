<%@page language="java" contentType="text/html; charset=utf-8"%>
	...
	multiCompCodeProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	var seb100ukrvProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	Unilite.defineModel('seb100ukrvsModel2', {
		fields: [
			{name: 'REF_CODE10'			, text: '<t:message code="system.label.base.classfication" default="구분"/>'			, type: 'string'	, allowBlank: false, comboType: 'AU', comboCode: 'SE03'},
			...
			{name: 'REF_CODE1'			, text: '<t:message code="system.label.base.reference" default="참조"/>1'				, type: 'string'},
			{name: 'REF_CODE2'			, text: '15G미만(%)'	, type: 'float', decimalPrecision: 2, format:'0,000.00'},							//20210629 수정: 소숫점 2자리로 변경
			{name: 'REF_CODE3'			, text: '15G이상(%)'	, type: 'float', decimalPrecision: 2, format:'0,000.00'},							//20210629 수정: 소숫점 2자리로 변경
			{name: 'REF_CODE4'			, text: '팔레트'		, type: 'float', decimalPrecision: 2, format:'0,000.00'},							//20210907 수정: 소숫점 2자리로 변경
...
//			{name: 'REF_CODE10'			, text: '<t:message code="system.label.base.reference" default="참조"/>10'			, type: 'string'},
	...
	var seb100ukrvStore2 = Unilite.createStore('seb100ukrvStore2',{
		model	: 'seb100ukrvsModel2',
		proxy	: Ext.isEmpty(multiCompCodeProxy2) ? seb100ukrvProxy2:multiCompCodeProxy2,
		autoLoad: false,
		uniOpt	: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				panelDetail.down('#standardProfitRate').uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		listeners: {
			load: function (store, records) {
				if(records && records.length > 0) {
					var mGrid	= panelDetail.down('#seb100ukrvGrid2');
	...
	var standardProfitRate = {
		itemId	: 'standardProfitRate',
		layout	: {type: 'vbox', align:'stretch'},
		items	: [{
			title		: '표준이익율',
			itemId		: 'tab_standardProfitRate',
			xtype		: 'container',
			...
			items		: [{
				...
				xtype		: 'uniGridPanel',
				itemId		: 'seb100ukrvGrid2',
				store		: seb100ukrvStore2,
				autoScroll	: false,
				dockedItems	: [{
					xtype	: 'toolbar',
					dock	: 'top',
				...
				columns: [
					...
					{dataIndex : 'REF_CODE3'		, flex: 2		, align: 'center'},
					{dataIndex : 'REF_CODE4'		, flex: 2		, align: 'center'},			//20210907 수정: , width: 110, hidden: true -> , flex: 2, align: 'center'
