<%@page language="java" contentType="text/html; charset=utf-8"%>
	var multiCompCodeProxy8;
	<c:if test="${multiCompCode == 'true'}">
	multiCompCodeProxy8 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	var seb100ukrvProxy8 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
	...
	Unilite.defineModel('seb100ukrvsModel8', {
		fields: [
			...
			{name: 'REF_CODE1'			, text: '비중적용'		, type: 'string', comboType: 'AU', comboCode: 'B010'},								//20210629 수정: 콤보로 변경
			{name: 'REF_CODE2'			, text: '<t:message code="system.label.base.reference" default="참조"/>2'	, type: 'string'},
	...
	var seb100ukrvStore8 = Unilite.createStore('seb100ukrvStore8',{
		model	: 'seb100ukrvsModel8',
		proxy	: Ext.isEmpty(multiCompCodeProxy8) ? seb100ukrvProxy8:multiCompCodeProxy8,
		autoLoad: false,
		uniOpt	: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				panelDetail.down('#chargingUnit').uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		listeners: {
			load: function (store, records) {
				if(records && records.length > 0) {
					var mGrid	= panelDetail.down('#seb100ukrvGrid8');
	...
	var chargingUnit = {
		...
		items	: [{
			...
			items		: [{
				...
				xtype		: 'uniGridPanel',
				itemId		: 'seb100ukrvGrid8',
				store		: seb100ukrvStore8,
				autoScroll	: false,
				dockedItems	: [{
					xtype	: 'toolbar',
					dock	: 'top',
				...
				columns: [
					...
					{dataIndex : 'REF_CODE1'		, flex: 4		, align: 'center'},
