...
<t:appConfig pgmId="ssa450skrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="ssa450skrv"/>		<!-- 사업장 -->
...
function appMain() {
	...
	Unilite.defineModel('Ssa450skrvModel1', {
		fields: [
			...
			{name: 'NEW_PRODUCT'		,text: '신제품여부'					,type: 'string'},
...
//			{name: 'CONSIGNMENT_FEE'	,text: '수수료(위탁)'	,type: 'uniPrice'},
	 ...
	 * @type
	 */
	var directMasterStore = Unilite.createStore('ssa450skrvMasterStore1',{
...
//			}
	...
	var panelSearch = Unilite.createSearchPanel('searchForm', {
		...
		items		: [{
			...
			Unilite.popup('AGENT_CUST',{
				...
				id				: 'ssa450skrvvCustPopup',
				listeners		: {
					onValueFieldChange: function(field, newValue, oldValue){
						panelResult.setValue('MANAGE_CUSTOM', newValue);

						if(!Ext.isObject(oldValue)) {
							panelSearch.setValue('MANAGE_CUSTOM_NAME', '');
							panelResult.setValue('MANAGE_CUSTOM_NAME', '');
						}
					},
					onTextFieldChange: function(field, newValue, oldValue){
						panelResult.setValue('MANAGE_CUSTOM_NAME', newValue);

						if(!Ext.isObject(oldValue)) {
							panelSearch.setValue('MANAGE_CUSTOM', '');
							panelResult.setValue('MANAGE_CUSTOM', '');
						}
					},
					applyextparam: function(popup){
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('ssa450skrvGrid1', {
		...
		columns	: [
			...
			{ dataIndex: 'NEW_PRODUCT'			, width: 100, align: 'center'},     //신제품여부
	...
	Unilite.Main({
		...
		fnInitBinding: function(params) {
			...
			//20210802 추가: 초기화 로직 추가
			panelSearch.getField('SALE_YN').setValue('A');
			panelSearch.getField('ENCLUDE_YN').setValue('Y');
...
//				panelResult.getField('NATION_INOUT').setValue('1');
		...
		onPrintButtonDown: function() {
			...
			win = Ext.create('widget.ClipReport', {
				url		: CPATH+'/sales/ssa450clskrv.do',
				prgID	: 'ssa450skrv',
				extParam: param
			});
			win.center();
