...
<t:appConfig pgmId="ssa671ukrv"  >  
    <t:ExtComboStore comboType="BOR120" pgmId="ssa671ukrv" />           <!-- 사업장 -->  
</t:appConfig>
<script type="text/javascript" >
///// 최종마감일이 없을시  rdo disabled처리 해야함 ssa671ukrv.htm  283줄
function appMain() { 
    var gsMonth = '';
    var orgInfo = '';
...
//              tdAttrs:{'align':'center'},                             
                handler : function() {
                    ...
                    if(UniAppManager.app.fnDateDIffCal()){
                        ...
                        ssa671ukrvService.insertMaster(param, function(provider, response)  {
                            if(provider){
...
//          panelSearch.getValue('t2');WORK_DATE
//          panelSearch.setValue('WORK_DATE', '201502');
//          Unilite.messageBox(panelSearch.getValue('WORK_DATE'));
//          UniAppManager.app.fnSetArEnvironm();
...
//          panelSearch.setValue('WORK_DATE', '');
            ...
            var param = panelSearch.getValues();
            ssa671ukrvService.selectMaster1(param, function(provider1, response)    {   //잔액이월Control에서 월마감(최종마감,기초잔액)년월 조회
                ssa671ukrvService.selectMaster2(param, function(provider2, response)    {   //채권누계테이블에서 해당 사업장의 최종마감월과 최초마감월 조회
...
//                      if(provider1.data.LAST_YYYYMM != '000000') panelSearch.setValue('END_DATE', Ext.Date.format(UniDate.extParseDate(provider1.data.LAST_YYYYMM + '01'), 'Y.m'));
                    ...
                    ssa671ukrvService.selectOrgInfo(param, function(provider, response) {   //사업자번호 조회
        ...
        fnChangeclosedYN: function(newValue){   //작업선택 radio 변경시
            ...
            if(newValue.CLOSED_YN == 'F'){  //마감
                ...
                if(StrDate1 < sRealBaseDate){
                    Unilite.messageBox(Msg.sMB108 + ' ' + sRealBaseDate + Msg.sMB109)    //기초반영월이 ?월이므로 취소가 불가능합니다. 
        ...
        fnDateDIffCal: function(){  
            ...
            if(!Ext.isEmpty(sRealBaseDate)){
                if(sRealBaseDate > sWorkDate){  //기초잔액년월 >이월작업년월
                    Unilite.messageBox(Msg.sMS328);//이월작업년월은 기초잔액반영년월보다 작을 수 없습니다.
                ...
                if(!panelSearch.getValue('CLOSED_YN')){     //취소일경우
                    if(sWorkDate != sBDate){    //이월작업년월 != 최초잔액반영월
                        if(sWorkDate > sEndDate){   //이월작업년월 > 최종마감년월
                            Unilite.messageBox(Msg.sMS331);  //이월취소작업시 이월작업년월은 최종마감년월보다 클 수 없습니다.
                            panelSearch.setValue('WORK_DATE', gsMonth);
                            return false;
                        }                       
                    }
                    ...
                    if(sWorkDate < sEndDate){   //이월작업년월 < 최종마감년월
                        Unilite.messageBox(Msg.sMS332);  //이월작업시 이월작업년월은 최종마감년월보다 작을 수 없습니다.
                        panelSearch.setValue('WORK_DATE', gsMonth);
                        return false;
                    }
                }               
            ...
            if(!Ext.isEmpty(panelSearch.getValue('AR_LAST_MONTH'))){
                if(sWorkDate > sArEndDate){ //이월작업년월 > 최종채권마감년월
                    Unilite.messageBox(Msg.sMS330);  //이월작업년월은 채권최종이월년월보다 클 수 없습니다.
                    panelSearch.setValue('WORK_DATE', gsMonth);
                    return false;
                }
            }
            ...
            if(sWorkDate > today){
                Unilite.messageBox(Msg.sMS329);  //이월작업년월은 현재년월보다 클 수 없습니다.
                panelSearch.setValue('WORK_DATE', gsMonth);
                return false;
            }
