...
<t:appConfig pgmId="sof198skrv"  >

	<t:ExtComboStore comboType="BOR120" pgmId="sof198skrv" /> 					<!-- 사업장 -->  
		...
		loadStoreRecords: function() {
			...
			this.load({
				...
				callback:function(responseText, arg)	{
					loadData = [];
					Ext.each(arg._response.result, function(data){
						loadData.push({
							'품목코드' : data.ITEM_CODE,
							'품목명' : data.ITEM_NAME,
							'거래처코드' : data.CUSTOM_CODE1,
							'거래처명' : data.CUSTOM_NAME1,
							'수주액'	: data.ORDER_O,
							'수주일': data.ORDER_DATE,
							'수주월': Ext.Date.format(UniDate.extParseDate(data.ORDER_DATE),'Y.m'),
							'영업담당명':data.ORDER_PRSN_NM,
							'영업담당코드':data.ORDER_PRSN,
						})	
					});
	...
	var masterGrid = Unilite.createGrid('sof100skrvGrid1', {
				...
				componentConfig : {
		    	    ...
		    	    addFields :[{
						...
						addfunction : function(record) {
							return Math.round(record.get("ORDER_O")/masterGrid.store.sum("ORDER_O")*10000)/100;
						}
					},{
						text : '수주월',
						name : 'ADD_FIELD2',
		...
		tbar:[{
			...
			handler : function(){
				...
				var win  = masterGrid.uniOpt.pivot.pivotWin;
				//var fieldStore = grid.setPivotFieldsStore();
				 
						if(!win) {
							
							win = Ext.create('widget.uniDetailWindow', {
								title: 'DATA 피벗',
								width : '100%',
								height : '100%',
								...
								items: [{
						    		...
						    		id    :  masterGrid.getId()+'Pivot',
						    		grid  : masterGrid,
						    		store : masterGrid.store,
						    		scrollable : true,
						    		initRows: ["품목명"],
						    	    initCols: ["거래처명"],
						    	    initVals:["수주액"],
								...
								listeners : {beforehide: function(me, eOpt) {
											 ...
											 show: function( panel, eOpts ) {
												 var pivotComp = Ext.getCmp( masterGrid.getId()+'Pivot');
												 pivotComp.onLoadRender();
											 } 
								}
							});
						}
						masterGrid.uniOpt.pivot.pivotWin = win;
						win.data = masterGrid.getStore().getData().items;
						win.center();
						win.show(); 
