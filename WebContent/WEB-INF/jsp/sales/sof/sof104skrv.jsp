...
<t:appConfig pgmId="sof104skrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="sof104skrv"/>		<!-- 사업장 -->
 ...
 */
	...
	Unilite.defineModel('Sof104skrvModel', {
		fields: [
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('sof104skrvMasterStore1', {
		model	: 'Sof104skrvModel',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {
				read: 'sof104skrvService.selectList1'
			}
		},
			...
			items:[
				...
				listeners: {
					...
					applyextparam: function(popup){
						...
						popup.setExtParam({multiSelectItemAccount: true});
						popup.setExtParam({'ITEM_ACCOUNT': ['00','10']});
...
//					Ext.getCmp('sof102skrvApp').mask('<t:message code="system.label.sales.loading" default="로딩중..."/>','loading-indicator');
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						panelResult.setValue('ORDER_UNIT', newValue);
					}
				}
			}]
		}],
...
//				allowBlank:false,
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						panelSearch.setValue('ORDER_UNIT', newValue);
					}
				}
			}
		]
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('sof104skrvGrid1', {
		...
		columns: [
			...
			{dataIndex: 'TRANS_RATE'		, width: 60, align: 'right',
				summaryRenderer:function(value, summaryData, dataIndex, metaData ) {
					return Unilite.renderSummaryRow(summaryData, metaData, '중계', '총합');
			}},
			{dataIndex: 'ORDER_UNIT_Q'		, width: 80, summaryType: 'avg'},
