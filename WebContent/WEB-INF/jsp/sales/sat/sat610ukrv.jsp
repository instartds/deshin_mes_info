...
<t:appConfig pgmId="sat610ukrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="sat610ukrv" />				<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'sat610ukrvService.selectList',
			create	: 'sat610ukrvService.insertDetail',
			destroy	: 'sat610ukrvService.deleteDetail',
			syncAll	: 'sat610ukrvService.saveAll'
		}
	});
	...
	//마스터 모델
	Unilite.defineModel('sat610ukrvModel',{
		fields: [
 			...
 			, {name : 'BASIS_NUM'       , text : '출고번호'     	, type : 'string'   , editable : false }
 			, {name : 'BASIS_SEQ'       , text : '출고순번'      	, type : 'int'      , editable : false }
	...
	// 마스터 스토어 정의
	var masterStore = Unilite.createStore('sat610ukrvStore',{
		model	: 'sat610ukrvModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
		...
		saveStore: function() {
			...
			if((inValidRecs.length == 0)) {
				...
				var grid = Ext.getCmp('sat610ukrvGrid');
	...
	var panelResult =  Unilite.createForm('resultForm',{
		...
		layout : {type : 'uniTable', columns : 3, tableAttrs:{width : 1100}},
		padding:'1 1 1 1',
		border:true,
		items: [{
			fieldLabel	: '사업장',
			...
			fieldLabel	: '출고자',
			readOnly    : true,
			valueFieldName:'OUT_USER',
			textFieldName:'OUT_USER_NAME',
			readOnly    : true
		}),{
			fieldLabel	: '배송방법',
			name		: 'DELIVERY_METH',
		...
		api: {
			load: 'sat610ukrvService.selectMaster',
			submit: 'sat610ukrvService.syncMaster'				
		},
		listeners: {
	...
	// 마스터 그리드
	var masterGrid = Unilite.createGrid('sat610ukrvGrid',{
		store	: masterStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
		...
		if(!outWin)	{
			Unilite.defineModel('outModel',{
				fields: [
					...
					, {name : 'USE_FR_DATE'         , text : '사용기간FR'     	, type : 'uniDate' }
					, {name : 'USE_FR_TIME'         , text : '사용기간FR'     	, type : 'string' }
					, {name : 'USE_TO_DATE'         , text : '사용기간TO'	   	, type : 'uniDate' } 
				    ...
				    , {name : 'OUT_USER'	    	, text : '출고자'	   		, type : 'string' } 
				    , {name : 'OUT_USER_NAME'	    , text : '출고자명'	   	, type : 'string' }
				    , {name : 'MOVE_YN'	    		, text : '이동/연장여부'	   	, type : 'string' } 
				]
			});
					...
					Unilite.createGrid('', {
						...
						store: Unilite.createStoreSimple('outStore', {
							...
							proxy: Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
								api: {
									read : 'sat610ukrvService.selectOutList'
								}
							}),
							loadStoreRecords : function() {
						...
						columns: [  
							...
							, {dataIndex : 'SAT600T_COUNT'      , width : 70  }
							, {dataIndex : 'USE_FR_DATE'        , width : 100 , align:'center'}
							, {dataIndex : 'USE_TO_DATE'        , width : 100 , align:'center'}
							, {dataIndex : 'BASIS_NUM'          , width : 110 }
							, {dataIndex : 'MOVE_YN'          , width : 100 , align:'center'}
						]
						,listeners: {									   
						...
						,returnData: function() {
							...
							if(Ext.isEmpty(record)) {
								...
								UniAppManager.app.loadOutDetail(record.data);
							}
						}
					})
	...
	Unilite.Main( {
		...
		fnInitBinding: function(param) {
			...
			this.setDefault();
			if(param && !Ext.isEmpty(param.INOUT_NUM)){
		...
		onNewDataButtonDown: function(param) {
			var r = {
				 ...
				 'BASIS_NUM'  : param.BASIS_NUM,
				 'BASIS_SEQ'  : param.BASIS_SEQ,
				 'INOUT_NUM'  : panelResult.getValue("INOUT_NUM"),
		...
		loadOutDetail : function(param) {
			masterStore.loadData({});
			var sParam  = {
				DIV_CODE : param.DIV_CODE,
				BASIS_NUM : param.BASIS_NUM,
				MOVE_YN : param.MOVE_YN
			}
			sat610ukrvService.selectOutDetailList(sParam, function(responseText)	{
