...
<%
// 고객정보 팜업
request.setAttribute("PKGNAME","Unilite.app.calendar.eventEditor");
%>

<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CD46" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB23" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB46" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB47" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="B010" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB10" />
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB48" />  // 영업담당자
<t:ExtComboStore useScriptTag="false" comboType="AU" comboCode="CB50" />


Ext.define('${PKGNAME}', { 
    extend: 'Unilite.com.BaseJSPopupCalApp',
    layout: 'fit',
        ...
        me.detailForm = Unilite.createSimpleForm( '', {
		    ...
		    items: [ 
		    		...
		    		{	xtype: 'uniFieldset',
				    	...
				    	items: [ {fieldLabel: '문서번호'	,name: 'DOC_NO'		,  hidden:true}
		    					,{fieldLabel: 'Action_type'	,name:'ACTION_TYPE'	,  value:'N_PLAN', hidden:true} //	(N:신규, U:수정, D:삭제)
					    		,Unilite.popup('CLIENT_PROJECT2', {fieldLabel:'고객명', showValue : false, width:280, allowBlank:false, extParam : {RDO : '2'} ,
					    								listeners: {
												                'onSelected': function(records, type) {
												                    	var frm = me.detailForm;
												                    	var rec = records[0];
					...
					,{	xtype:'uniFieldset',
				    	...
				    	items: [	    
							 ...
							 ,{fieldLabel: '계획일자'
								               ...
								               ,items: [ {name:'PLAN_DATE',  width:90	, xtype: 'uniDatefield',  allowBlank: true, fieldCls :'x-form-field x-form-required-field'} 
					    							 	,{name:'PLAN_TIME',  width:90	, xtype: 'uniTimefield',  fieldCls:'x-form-field x-form-required-field'}
								                ]		               
							            	 }
				...
				items :[   
		     			...
		     			,{fieldLabel: '실행일자' , colspan:2
						               ...
						               ,defaults: { hideLabel: true}
						               ,items: [ {name:'RESULT_DATE',  width:90		, xtype: 'uniDatefield', allowBlank: true, fieldCls:'x-form-field x-form-required-field'} 
			    							 	,{name:'RESULT_TIME',  width:90		, xtype: 'uniTimefield', allowBlank: true, fieldCls:'x-form-field x-form-required-field'} 
						                ]		               
					            	 }
						               ...
						               ,items: [ 
									            ...
									            ,{xtype:'button',text:'+'	, width : 20
									            	, handler: function(){
									            			console.log('11');
                                                            var frm = me.detailForm;
										            		if(me.down('#FS_OPINION_STR2').isHidden()){
										            			me.down('#FS_OPINION_STR2').show();
										            		}else if(me.down('#FS_OPINION_STR3').isHidden()){
										            			me.down('#FS_OPINION_STR3').show();
										            		}else {
									            ...
									            ,{xtype:'button',text:'-'	, width : 20
									            	, handler: function(){
									            			var frm = me.detailForm;
										            		if(me.down('#FS_OPINION_STR3').isVisible()){
										            			me.down('#FS_OPINION_STR3').hide();			
										            			frm.setValue('OPINION_STR3','','WRITE_EMP3','');
										            		}else if(me.down('#FS_OPINION_STR2').isVisible()){
										            			me.down('#FS_OPINION_STR2').hide();				
    ...
    fnInitBinding : function(param) {
            console.log('cmd100ukrv param:', param);
			var me = this;
            var detailForm = me.detailForm;
			me.setToolbarButtons(['save'],false);			
			me.setToolbarButtons('delete',true);
			
			var docId = param.docId; // '{param.DOC_ID}';
			var startDate = param.startDate;// '{param.startDate}';
            var planHour = param.startHour; // '{param.startHour}';
            
			if(!Ext.isEmpty(docId)) {
                console.log('docId:',docId);
				me._loadData(docId);
			}else {
                console.log('docId id null');
                detailForm.setValue('PLAN_DATE', startDate);
                detailForm.setValue('RESULT_DATE', startDate);
	...
	_loadData: function(docId) {
			...
			detailForm.getForm().load({
				 ...
				 , success:function() {
							...
							if(detailForm.getValue('OPINION_STR3') != null && detailForm.getValue('OPINION_STR3') != ''  )	{
								if(me.down('#FS_OPINION_STR3').isHidden()){
			            			me.down('#FS_OPINION_STR3').show();
			            		}
							}
		...
		onDeleteDataButtonDown: function()	{
	       ...
	       if(confirm('현재내용을 삭제 합니다.\n 삭제 하시겠습니까?')) {
				...
				var param = {'DOC_NO' : detailForm.getValue('DOC_NO')
							 ,'FILE_NO' : detailForm.getValue('FILE_NO')};
