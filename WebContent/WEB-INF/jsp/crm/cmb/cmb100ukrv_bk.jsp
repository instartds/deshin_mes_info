...
function appMain() {
	...
	var panelSearch = Unilite.createSearchForm('searchForm',{
	            	...
	            	items : [ {fieldLabel:'고객명', name:'CLIENT_NAME',  width:240 }
		            		
	            			 , Unilite.popup('CUST',{ fieldLabel: '거래처', textFieldWidth:170, validateBlank:false})
		         ]
    });  
    ...
    var masterGrid = Unilite.createGrid('cmb100ukrvGrid', {
        ...
        columns:  [     
					...
					,{ dataIndex: 'WIFE_BIRTH_DATE'		,  width: 140 } // '베우자생일',
          ...
          /*,listeners: { selectionchange: function( grid, selected, eOpts ) {
          							Ext.getCmp('cmb100ukrvDetailForm').setActiveRecord(selected[0] || null);
			      					if(selected[0] != null && tab.getActiveTab().getId() == 'cmb100ukrvDetailForm')	{			      						
			      						Ext.getCmp('businessCardForm').clearForm();
				            			var fid = selected[0].get('BUSINESSCARD_FID');
									...
									listeners: {change : function( filefield, value, eOpts )	{
															...
															if(value !='' )	{
																...
																Ext.getCmp('saveBtn').setDisabled(false);
			       											    Ext.getCmp('saveCloseBtn').setDisabled(false);
															}
														}
									}
								  }
								  ,{ xtype: 'image', id:'cmb100ukrvBCImage', src:CPATH+'/resources/images/nameCard.jpg', width:320,	 overflow:false}
					             ]
    ...
    var detailForm = Unilite.createForm('cmb100ukrvDetailForm', {
	    ...
	    items : [{ 
        			...
        			, items :[	  {fieldLabel: '고객명',  		name: 'CLIENT_NAME' 	,maxLength:20	,allowBlank: false}
        						 ...
        						 ,{fieldLabel: '직책',  		name: 'DUTY_ANNE'		,maxLength:20	}
	    	   ...
	    	   ,{   title: '가족사항'	    	   
        			...
        			, items :[	  {fieldLabel: '생년월일',  	name: 'BIRTH_DATE'		,xtype:'uniDatefield'}
        						 ...
        						 ,{fieldLabel: '베우자생년월일',  name: 'WIFE_BIRTH_DATE'	,xtype:'uniDatefield'}
