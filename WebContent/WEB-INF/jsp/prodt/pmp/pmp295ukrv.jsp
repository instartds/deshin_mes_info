...
function appMain() {
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read:		'pmp295ukrvService.selectMaster',
			update:		'pmp295ukrvService.updateDetail',
			create:		'pmp295ukrvService.insertDetail',
			destroy:	'pmp295ukrvService.deleteDetail',
			syncAll:	'pmp295ukrvService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read:		'pmp295ukrvService.orderApply2'
		}
	});
	 ...
	 * @type
	 */
	Unilite.defineModel('pmp295ukrvMasterModel', {
	    fields: [
...
//			{name: 'PJT_CODE'         	,text: '<t:message code="system.label.product.projectno" default="프로젝트번호"/>'			,type:'string'},
	...
	Unilite.defineModel('pmp295ukrvDetailModel', {
	    fields: [
...
//			{name: 'PJT_CODE'       ,text: '<t:message code="system.label.product.projectno" default="프로젝트번호"/>'	,type:'string'}
	 ...
	 * @type
	 */
	var masterStore = Unilite.createStore('pmp295ukrvmasterStore', {
		model: 'pmp295ukrvMasterModel',
		autoLoad: false,
		uniOpt: {
	...
	var detailStore = Unilite.createStore('pmp295ukrvdetailStore', {
		model: 'pmp295ukrvDetailModel',
		autoLoad: false,
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('pmp295ukrvGrid');
	...
	var issueRequestNoMasterStore = Unilite.createStore('issueRequestNoMasterStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'pmp295ukrvService.selectOrderNumMasterList'
                }
            }
            ,loadStoreRecords : function()	{
	 ...
	 * @type
	 */
    var masterGrid = Unilite.createGrid('pmp295ukrvGrid', {
    	layout: 'fit',
        region:'center',
        uniOpt: {
...
//	        		detailStore.loadData({})
    ...
    var detailGrid = Unilite.createGrid('pmp295ukrvGrid2', {
    	layout: 'fit',
        region:'south',
        uniOpt: {
			expandLastColumn: false,
...
//			{dataIndex: 'PJT_CODE'       	, width: 100}
    ...
    var issueRequestNoMasterGrid = Unilite.createGrid('pmp295ukrvissueRequestNoMasterGrid', {
        // title: '기본',
        layout : 'fit',
    ...
    // 작업지시 참조 모델 정의
    Unilite.defineModel('pmp295ukrvWorkModel', {
    	fields: [
...
//		    {name: 'PJT_CODE'			, text: '<t:message code="system.label.product.projectno" default="프로젝트번호"/>'		, type: 'string'}
    ...
    //작업지시 참조 스토어 정의
	var WorkStore = Unilite.createStore('pmp295ukrvWorkStore', {
		model: 'pmp295ukrvWorkModel',
        autoLoad: false,
        uniOpt : {
        	isMaster: false,			// 상위 버튼 연결
        	editable: false,			// 수정 모드 사용
        ...
        proxy: {
            type: 'direct',
            api: {
            	read    : 'pmp295ukrvService.selectEstiList'
            }
        },
        listeners:{
        	load:function(store, records, successful, eOpts)	{
...
//									console.log("record :", record);
	...
	// 작업지시 그리드
	var WorkGrid = Unilite.createGrid('pmp295ukrvWorkGrid', {
    	layout : 'fit',
    	store: WorkStore,
...
//        	{ dataIndex: 'PJT_CODE'			   			, width: 100}
    ...
    function openWorkOrderWindow() {
  		...
  		WorkSearch.setValue('REF_WKORD_NUM', panelSearch.getValue('WKORD_NUM'));
...
//							WorkGrid.returnData();
		...
		//							panelSearch.setValue('REF_WKORD_NUM', record[0].get('WKORD_NUM'));
							...
							pmp295ukrvService.orderApply(param, function(provider, response){
						    	var store = masterGrid.getStore();
						    	var records = response.result;
						    	store.insert(0, records);
...
//							WorkGrid.returnData();
	...
	//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('pmp295ukrvAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config)	{
			var fp = Ext.getCmp('pmp295ukrvFileUploadPanel');
