...
<t:appConfig pgmId="pmp110ukrv">
	<t:ExtComboStore comboType="BOR120" pgmId="pmp110ukrv" />			<!-- 사업장 -->
...
var BsaCodeInfo = {
	...
	var CustomCodeInfo = {
		csPricePrecision : UniFormat.Price ? UniFormat.Price.length
				- (UniFormat.Price.indexOf(".") != -1 ? UniFormat.Price
						.indexOf(".") + 1 : UniFormat.Price.length - 2) : 2,
		csQtyPrecision : UniFormat.Qty ? UniFormat.Qty.length
				- (UniFormat.Qty.indexOf(".") != -1 ? UniFormat.Qty
						.indexOf(".") + 1 : UniFormat.Qty.length - 2) : 2
	};
...
function appMain() {
//alert(CustomCodeInfo.csQtyPrecision);
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'pmp110ukrvService.selectDetailList',
			update	: 'pmp110ukrvService.updateDetail',
			create	: 'pmp110ukrvService.insertDetail',
			destroy	: 'pmp110ukrvService.deleteDetail',
			syncAll	: 'pmp110ukrvService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'pmp110ukrvService.selectWorkNum'
		}
	});
	...
	/*var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'pmp110ukrvService.selectProgInfo'
		}
	});*/
					...
					render: function(component) {
                        component.getEl().on('dblclick', function( event, el ) {
                        	if(Ext.isEmpty(panelResult.getValue('ITEM_CODE'))){
                        		Unilite.messageBox('작업지시 품목을 먼저 입력해주세요');
						...
						if(!panelResult.uniOpt.inLoading) {
							...
							if(confirm(msg)){
								if(!Ext.isEmpty(panelResult.getValue("DIV_CODE")) && !Ext.isEmpty(panelResult.getValue("WKORD_NUM"))) {
									...
									pmp110ukrvService.closeWok(param, function(respnseText, response){
...
//			tdAttrs: {style: 'border-top: 1px solid #cccccc;padding-top: 2px;' }
	 ...
	 */
	Unilite.defineModel('pmp110ukrvDetailModel', {
		fields: [
	 ...
	 * @type
	 */
	var detailStore = Unilite.createStore('pmp110ukrvDetailStore', {
		model: 'pmp110ukrvDetailModel',
		autoLoad: false,
		uniOpt: {
...
//			})
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('pmp110ukrvGrid');
...
//				}
	 ...
	 * @type
	 */
	var detailGrid = Unilite.createGrid('pmp110ukrvGrid', {
		layout: 'fit',
		region:'center',
		uniOpt: {
...
//	});
	...
	Unilite.defineModel('pmp110ukrvProductionPlanModel', {
		fields: [
	...
	var productionPlanStore = Unilite.createStore('pmp110ukrvProductionPlanStore', {
		model: 'pmp110ukrvProductionPlanModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'pmp110ukrvService.selectEstiList'
			}
		},
		loadStoreRecords : function() {
	...
	var productionPlanGrid = Unilite.createGrid('pmp110ukrvproductionPlanGrid', {
		layout : 'fit',
...
//						productionPlanInputForm.setValue('PRODT_END_DATE',panelResult.getValue('PRODT_END_DATE'));
 ...
 // 검색 스토어(디테일)
	...
	var refLotSearch = Unilite.createSearchForm('refLotForm', {
		layout :{type : 'uniTable', columns : 3},
		items: [{
			fieldLabel: '<t:message code="system.label.product.division" default="사업장"/>',
			name: 'DIV_CODE',
    ...
    var refLotProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'pmp110ukrvService.selectRefLot'
		}
	});

	Unilite.defineModel('refLotModel', {
		fields: [
	...
	var refLotStore = Unilite.createStore('refLotStore', {
		model: 'refLotModel',
		autoLoad: false,
		uniOpt: {
			isMaster: false,		 // 상위 버튼 연결
		...
		proxy: refLotProxy,
		loadStoreRecords: function() {
			var param= refLotSearch.getValues();
			this.load({
				params : param
			});
		}
	});
	var refLotGrid = Unilite.createGrid('refLotGrid', {
		store: refLotStore,
		region: 'center',
		layout: 'fit',
		uniOpt: {
		...
		listeners: {
			onGridDblClick: function(grid, record, cellIndex, colName) {
				panelResult.setValue('LOT_NO',record.get('LOT_NO'),false);
                refLotWindow.hide();
	...
	function openRefLotWindow() {
        if(!refLotWindow) {
            refLotWindow = Ext.create('widget.uniDetailWindow', {
                title: '투입 LOT NO 팝업',
                width: 900,
                height: 400,
                layout: {type:'vbox', align:'stretch'},
                items: [refLotSearch,refLotGrid],
                tbar: ['->',{
                    itemId: 'searchBtn',
                    text: '조회',
                    handler: function() {
						if(!refLotSearch.getInvalidMessage()) return false;
                    	
                        refLotStore.loadStoreRecords();
                    },
                    disabled: false
                },{
                    itemId : 'closeBtn',
                ...
                listeners: {
                    beforehide: function(me, eOpt) {
                        refLotSearch.clearForm();
						refLotGrid.reset();
						refLotStore.clearData();
                    },
                    beforeclose: function(panel, eOpts) {
                    },
                    ...
                    beforeshow: function(panel, eOpts) {
                    	refLotSearch.setValue('DIV_CODE', panelResult.getValue('DIV_CODE'));
                    	refLotSearch.setValue('ITEM_CODE', panelResult.getValue('ITEM_CODE'));
                    	refLotSearch.setValue('ITEM_NAME', panelResult.getValue('ITEM_NAME'));
                    },
                    show: function(panel, eOpts) {
                        refLotStore.loadStoreRecords();
                    }
				...
				if(!Ext.isEmpty(provider)) {
					...
					if(count <= 0) {
						Ext.each(records, function(record,i) {
							...
							var wrokd_Prsn      = panelResult.getValue('WKORD_PRSN');
							//20190123 추가(PMP100T.CUSTOM_NAME 저장할 데이터)
...
//				Ext.getCmp('workEndYn').setReadOnly(false);
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('pmp110ukrvAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
...
//			});
		...
		confirmSaveData: function(config) {
			var fp = Ext.getCmp('pmp110ukrvFileUploadPanel');
...
//			panelResult.getField('DIV_CODE').setReadOnly(true);
	...
	//						'WORK_END_YN':		params.,
							...
							'DVRY_DATE':		params.DVRY_DATE,
	//						'CUSTOM_NAME':			params.,
	//						'PROG_UNIT':		params.,
	//						'EXCHG_TYPE':		params.,
							'WKORD_Q':			params.WK_PLAN_Q,
							'WK_PLAN_NUM':		params.WK_PLAN_NUM
	//						'PRODT_START_DATE':	params.,
	//						'PRODT_WKORD_DATE':	new Date(),
	//						'PRODT_END_DATE':	params.,
	//						'LOT_NO':			params.,
						});
	//					if(record.get('EXPIRATION_DAY') != 0){
	//						panelResult.setValue('EXPIRATION_DATE',UniDate.getDbDateStr(UniDate.add(panelResult.getValue('PRODT_WKORD_DATE'), {months: +record.get('EXPIRATION_DAY'), days:-1})));
...
//			panelResult.getField('DIV_CODE').setReadOnly(true);
	...
	//20210423 추가: 날짜 우선순위 체크로직, 20210428 수정: 수정하는 필드에 따라서 체크로직 변경하도록 수정 (FLAG)
