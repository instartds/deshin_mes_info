...
var selectedMasterGrid = 'pms401ukrvGrid';
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'pms401ukrvService.selectMaster',
			update: 'pms401ukrvService.updateDetail',
			create: 'pms401ukrvService.insertDetail',
			destroy: 'pms401ukrvService.deleteDetail',
			syncAll: 'pms401ukrvService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'pms401ukrvService.selectDetail',
			update: 'pms401ukrvService.updateDetail2',
			create: 'pms401ukrvService.insertDetail2',
			destroy: 'pms401ukrvService.deleteDetail2',
			syncAll: 'pms401ukrvService.saveAll2'
		}
	});

	var masterForm = Unilite.createSearchPanel('pms401ukrvMasterForm', {
	 ...
	 * @type
	 */
	Unilite.defineModel('pms401ukrvDetailModel', {
	    fields: [
	...
	Unilite.defineModel('pms401ukrvDetailModel2', {
	    fields: [
	    	{name: 'DIV_CODE'       		,text: '<t:message code="system.label.product.division" default="사업장"/>'				    ,type:'string'},
			{name: 'INSPEC_NUM'     		,text: '<t:message code="system.label.product.inspecno" default="검사번호"/>'				,type:'string'},
	...
	Unilite.defineModel('pms401ukrvDetailModel3', {
	    fields: [
	...
	var detailStore = Unilite.createStore('pms401ukrvDetailStore', {
		model: 'pms401ukrvDetailModel',
		autoLoad: false,
		uniOpt: {
...
//                    Ext.getCmp('ItemDeleteButton').setDisabled(true);
	...
	var detailStore2 = Unilite.createStore('pms401ukrvDetailStore2', {
		model: 'pms401ukrvDetailModel2',
		autoLoad: false,
		uniOpt: {
			isMaster: false,			// 상위 버튼 연결
			editable: true,			// 수정 모드 사용
...
//			var param= param.data.items[0];
	...
	var checkItemStore = Unilite.createStore('pms401ukrvDetailStore3', {// 이동요청
																		// 참조
			model: 'pms401ukrvDetailModel3',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            ...
            proxy: {
                type: 'direct',
                api: {
                	read: 'pms401ukrvService.selectCheckItem'
                }
            }/*
	...
	var detailGrid = Unilite.createGrid('pms401ukrvGrid', {
    	layout: 'fit',
        region:'center',
        uniOpt: {
...
//				if(selectedMasterGrid == 'pms401ukrvGrid2' && UniAppManager.app._needSave())  {
...
//       		}
    ...
    var checkItemGrid = Unilite.createGrid('pms401ukrvGrid3', {	// 검사항목 팝업
        // title: '기본',
        layout : 'fit',
...
//	    trackResetOnLoad: true,
	...
	var checkNoMasterStore = Unilite.createStore('checkNoMasterStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'pms401ukrvService.selectOrderNumMasterList'
                }
            }
            ,loadStoreRecords : function()	{
	...
	var checkNoMasterGrid = Unilite.createGrid('pms401ukrvcheckNoMasterGrid', {
        // title: '기본',
        layout : 'fit',
...
//          		   { dataIndex: 'PJT_CODE'     				,	 width: 100 }
    ...
    // 접수 참조 모델 정의
    Unilite.defineModel('pms401ukrvProductionModel', {
    	fields: [
	...
	// 접수 참조 스토어 정의
	var productionStore = Unilite.createStore('pms401ukrvProductionStore', {
			model: 'pms401ukrvProductionModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'pms401ukrvService.selectEstiList'
                }
            },
            listeners:{
            	load:function(store, records, successful, eOpts)	{
	...
	var productionGrid = Unilite.createGrid('pms401ukrvproductionGrid', {
    	layout : 'fit',
...
//		 	 		GOODBAD_TYPE: '01'
		...
		onDeleteDataButtonDown: function() {
			if(selectedMasterGrid == 'pms401ukrvGrid')   {
