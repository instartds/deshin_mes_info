...
<t:appConfig pgmId="ppl300ukrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="ppl300ukrv"/> 					 <!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'ppl300ukrvService.selectEstiList'
		}
	});
	var directProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'ppl300ukrvService.selectDetailList',
			update	: 'ppl300ukrvService.updateList',
			syncAll	: 'ppl300ukrvService.saveAll2'
		}
	});
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'ppl300ukrvService.selectApsParameter',
			update	: 'ppl300ukrvService.updateDetail',
			syncAll	: 'ppl300ukrvService.saveAll'
		}
	});
	 ...
	 * @type
	 */
	Unilite.defineModel('ppl300ukrvModel', {
	    fields: [
	 ...
	 * @type
	 */
	var apsStore = Unilite.createStore('ppl300ukrvMasterStore1',{
		model: 'ppl300ukrvModel',
		uniOpt : {
			       ...
			       // tdAttrs	: {align: 'left',width: 200},
					...
					handler	: function() {
								   openOrderInformationWindow()
									...
									/*	$.ajax({    type: "POST",
										            url : "<c:url value='/ganttchart/ganttApsData.do'/>",
										            data: param,
										            type: "POST",
									  ...
									  // scheduler.crudManager.load({'FROM_DT': $( '.from-date' ).val(), 'TO_DT':  $( '.to-date' ).val()});
					},
					disabled: false
				},{ xtype: 'button',
					itemId	: 'settingBtn',
     ...
     * @type
     */
    var masterGrid = Unilite.createGrid('ppl300ukrvGrid1', {
    	flex   : .3,
        region : 'south',
        store  : apsStore,
        split  : true,
...
//        	forceToolbarbutton:true
		...
		fnInitBinding : function(params) {
			...
			panelResult.setValue('FROM_DATE'	, UniDate.get('threeMonthsAgo'));
			panelResult.setValue('TO_DATE'		, UniDate.get('threeMonthsLater'));
			var param = {
				'OPTION_DATE'	: UniDate.getDbDateStr(UniDate.get('threeMonthsAgo')),
				'CAL_TYPE'		: '3' //주단위
			}
			prodtCommonService.getCalNo(param, function(provider, response) {
				if(!Ext.isEmpty(provider.CAL_NO)){
			...
			var param = {
				'OPTION_DATE'	: UniDate.getDbDateStr(UniDate.get('threeMonthsLater')),
				'CAL_TYPE'		: '3' //주단위
			}
			prodtCommonService.getCalNo(param, function(provider, response) {
				if(!Ext.isEmpty(provider.CAL_NO)){
...
<link rel="stylesheet" href='<c:url value="/resources/js/scheduler-4.2.1/examples/groupsummary/resources/app.css" />'  >
...
<script data-default-locale="En" type="module" src="<c:url value='/resources/js/scheduler-4.2.1/examples/groupsummary/app.module.js'   />" charset="utf-8" ></script>
