...
<t:appConfig pgmId="ppl100ukrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="ppl100ukrv"/> 			<!-- 사업장 -->
...
function appMain() {
	...
	var mrpYnStore = Unilite.createStore('ppl100ukrvMRPYnStore', {
	    fields: ['text', 'value'],
		data :  [
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'ppl100ukrvService.selectDetailList',
			update: 'ppl100ukrvService.updateDetail',
			create: 'ppl100ukrvService.insertDetail',
			destroy: 'ppl100ukrvService.deleteDetail',
			syncAll: 'ppl100ukrvService.saveAll'
		}
	});
	...
	//작업장
	Unilite.defineModel('ppl100ukrvMasterModel1', {
	    fields: [
	    	...
	    	{name: 'ITEM_CODE'      	, text: '<t:message code="system.label.product.soseq" default="수주순번"/>' 				, type: 'string'},
	    	{name: 'ITEM_INFO'     		, text: '<t:message code="system.label.product.custom" default="거래처"/>' 				, type: 'string'},
	...
	//품목
	Unilite.defineModel('ppl100ukrvMasterModel2', {
	    fields: [
	    	{name: 'COMP_CODE'      	, text: 'COMP_CODE'			, type: 'string'},
	    	{name: 'DIV_CODE'       	, text: '<t:message code="system.label.product.division" default="사업장"/>' 				, type: 'string'},
	    	{name: 'ITEM_CODE'      	, text: '<t:message code="system.label.product.item" default="품목"/>' 			, type: 'string', allowBlank: true},
	    	{name: 'ITEM_INFO'      	, text: '<t:message code="system.label.product.unit" default="단위"/>' 				, type: 'string'},
	    	{name: 'ORDER_INFO'      	, text: '<t:message code="system.label.product.soseq" default="수주순번"/>' 			, type: 'string'},
	    	{name: 'WORK_SHOP_CODE'     , text: '<t:message code="system.label.product.soqty" default="수주량"/>' 				, type: 'string'},
		    {name: 'GUBUN'          	, text: '<t:message code="system.label.product.classfication" default="구분"/>'   				, type: 'string'},
		    {name: 'TOT_Q'         	, text: '<t:message code="system.label.product.monthlytotal" default="월합계"/>' 	, type: 'uniQty'},
		    {name: 'DAY1_Q'         , text: '1<t:message code="system.label.product.day" default="일"/>' 	, type: 'uniQty'},
			{name: 'DAY2_Q'        	, text: '2<t:message code="system.label.product.day" default="일"/>' 	, type: 'uniQty'},
	...
	//마스터 스토어 정의
	var masterStore1 = Unilite.createStore('ppl100ukrvmasterStore1', {
		model: 'ppl100ukrvMasterModel1',
		autoLoad: false,
		uniOpt: {
...
//			console.log("list:", list);
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('ppl100ukrvGrid1');
	...
	//마스터 스토어 정의
	var masterStore2 = Unilite.createStore('ppl100ukrvmasterStore2', {
		model: 'ppl100ukrvMasterModel2',
		autoLoad: false,
		uniOpt: {
			isMaster: true,			// 상위 버튼 연결
			editable: true,			// 수정 모드 사용
...
//       		var list = [].concat(toUpdate, toCreate);
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('ppl100ukrvGrid2');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		}
	});

	var masterForm = Unilite.createSearchPanel('ppl100ukrvMasterForm', {
		...
		items: [{
		    ...
		    items: [{
	        	...
	        	listeners: {
					onValueFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
					...
					onTextFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
	...
	var panelResult = Unilite.createSearchForm('resultForm',{
		...
		items: [{
				...
				listeners: {
					change: function(combo, newValue, oldValue, eOpts) {
					masterForm.setValue('TYPE', newValue);
					}
				}
			}],
	    setAllFieldsReadOnly: function(b) {
     ...
     * @type
     */
    var masterGrid1 = Unilite.createGrid('ppl100ukrvGrid1', {
    	layout: 'fit',
        region:'center',
...
//		],
        columns: [
        	...
        	{text :'<t:message code="system.label.product.item" default="품목"/>',locked: true,
        		columns:[
        	...
        	{text :'<t:message code="system.label.product.sono" default="수주번호"/>',locked: true,
        		columns:[
    				{text :'<t:message code="system.label.product.deliverydate" default="납기일"/>',
		        			columns:[
		        				{dataIndex:'ORDER_INFO'       , width: 120}
		        			]
	        		}
    			]
        	},
		...
		listeners: {
			beforeedit:function(editor, e, eOpts) {
				var rowIndex = masterGrid1.getSelectedRowIndex();
				if(rowIndex % 3 == 0){
					if(!UniUtils.indexOf(e.field, [ 'GUBUN', 'TOT_Q'])){
						return true;
					}
                }
                return false;
    ...
    var masterGrid2 = Unilite.createGrid('ppl100ukrvGrid2', {
    	layout: 'fit',
        region:'center',
            enableColumnHide :false,
            sortableColumns : false,
...
//		],
        columns: [
        	...
        	{text :'<t:message code="system.label.product.itemname" default="품목명"/>',locked: true,
    			...
    			editor:Unilite.popup('DIV_PUMOK',{
    				fieldLabel: '<t:message code="system.label.product.item" default="품목"/>',
    				validateBlank:false,
    				valueFieldName:'ITEM_CODE',
            		textFieldName:'ITEM_NAME',
        	...
        	{text :'<t:message code="system.label.product.custom" default="거래처"/>',locked: true,
        		columns:[
        	...
        	{text :'<t:message code="system.label.product.workcenter" default="작업장"/>',locked: true,
        		comboType: 'WU',
        		columns:[
    				{text :'<t:message code="system.label.product.deliverydate" default="납기일"/>',
		        			columns:[
		...
		setItemData: function(record, dataClear) {
			var rowIndex = masterGrid2.getSelectedRowIndex();
       		...
       		if(dataClear) {
       			...
       			masterStore2.getAt(rowIndex+1).set('ITEM_INFO'	, "");
    			masterStore2.getAt(rowIndex+2).set('ITEM_INFO'	, "");
       		} else {
       			masterStore2.getAt(rowIndex).set('ITEM_CODE'	, record['ITEM_CODE']);
       			masterStore2.getAt(rowIndex).set('ITEM_INFO'	, record['ITEM_NAME']);
       			masterStore2.getAt(rowIndex+1).set('ITEM_INFO'	, record['SPEC']);
    			masterStore2.getAt(rowIndex+2).set('ITEM_INFO'	, record['STOCK_UNIT']);
       		}
		},
		listeners: {
			beforeedit:function(editor, e, eOpts) {
    ...
    // 수주정보 참조 모델 정의
	Unilite.defineModel('ppl100ukrvOrderModel', {
	    fields: [
	...
	//수주정보 참조 스토어 정의
	var OrderStore = Unilite.createStore('ppl100ukrvOrderStore', {
		model: 'ppl100ukrvOrderModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: true,			// 상위 버튼 연결
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('ppl100ukrvOrderGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		}
	});
				...
				items: [{
                 ...
                 items: [
	                 Unilite.popup('DIV_PUMOK',{
			        	...
			        	listeners: {
							onValueFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
									OrderSearch.setValue('ITEM_CODE_FR', newValue);
									if(!Ext.isObject(oldValue)) {
										OrderSearch.setValue('ITEM_NAME_FR', '');
									}
							},
							onTextFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
									OrderSearch.setValue('ITEM_NAME_FR', newValue);
									if(!Ext.isObject(oldValue)) {
										OrderSearch.setValue('ITEM_CODE_FR', '');
									}
							},
							applyextparam: function(popup){
								popup.setExtParam({'DIV_CODE': OrderSearch.getValue('DIV_CODE')});
                 	...
                 	Unilite.popup('DIV_PUMOK',{
			        	...
			        	listeners: {
							onValueFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
									OrderSearch.setValue('ITEM_CODE_TO', newValue);
									if(!Ext.isObject(oldValue)) {
										OrderSearch.setValue('ITEM_NAME_TO', '');
									}
							},
							onTextFieldChange: function(field, newValue, oldValue){	// 2021.08.10 표준화 작업
									OrderSearch.setValue('ITEM_NAME_TO', newValue);
									if(!Ext.isObject(oldValue)) {
										OrderSearch.setValue('ITEM_CODE_TO', '');
									}
							},
							applyextparam: function(popup){
								popup.setExtParam({'DIV_CODE': OrderSearch.getValue('DIV_CODE')});
	...
	/* 수주정보 그리드 */
	 var OrderGrid = Unilite.createGrid('ppl100ukrvOrderGrid', {
    	layout : 'fit',
    	store: OrderStore,
    ...
    //판매계획 참조 모델 정의
	Unilite.defineModel('ppl100ukrvSalesPlanModel', {
	    fields: [
	    	{name: 'GUBUN'         		, text: '<t:message code="system.label.product.selection" default="선택"/>'			, type: 'string'},
	    	{name: 'PLAN_TYPE'     		, text: '<t:message code="system.label.product.typecode" default="유형코드"/>' 		, type: 'string'},
		    {name: 'PLANTYPE_NAME' 		, text: '<t:message code="system.label.product.type" default="유형"/>'			, type: 'string'},
	...
	//판매계획 참조 스토어 정의
	var SalesPlanStore = Unilite.createStore('ppl100ukrvSalesPlanStore', {
		model: 'ppl100ukrvSalesPlanModel',
		autoLoad: false,
            uniOpt : {
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('ppl100ukrvSalesPlanGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		}
	});
	 ...
	 */
    //판매계획 참조 폼 정의
	var SalesPlanSearch = Unilite.createSearchForm('ppl100ukrvSalesPlanForm', {
    ...
    var SalesPlanGrid = Unilite.createGrid('ppl100ukrvSalesPlanGrid', {
    	// title: '기본',
        layout : 'fit',
        region:'center',
    ...
    var tab = Unilite.createTabPanel('tabPanel',{
		...
		listeners : {
		  tabChange : function ( tabPanel, newCard, oldCard, eOpts )  {
				...
				switch(newTabId){
					case 'masterGrid1' :
						masterForm.setValue('TYPE', 'W');
						panelResult.setValue('TYPE', 'W');
						masterStore1.loadStoreRecords();
						break;

					case 'masterGrid2' :
						masterForm.setValue('TYPE', 'I');
    ...
    Unilite.Main({
		...
		onNewDataButtonDown: function()	{
			...
			if(activeTabId == 'masterGrid1'){
				masterGrid1.createRow(r3, '', -1);
				masterGrid1.createRow(r2, '', -1);
				masterGrid1.createRow(r1, '', -1);
			}
			if(activeTabId == 'masterGrid2'){
				masterGrid2.createRow(r3, '', -1);
				masterGrid2.createRow(r2, '', -1);
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('ppl100ukrvAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
        ...
        checkForNewDetail:function() {
			if(Ext.isEmpty(masterForm.getValue('FR_DATE')) || Ext.isEmpty(masterForm.getValue('FR_DATE')))	{
