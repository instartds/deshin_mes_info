...
<t:appConfig pgmId="s_zcc701ukrv_kd"  >
    <t:ExtComboStore comboType="BOR120"  pgmId="s_zcc701ukrv_kd"  />             <!-- 사업장 -->
...
var selectedGrid = 's_zcc701ukrv_kdGrid2';

function appMain() {
	
    var directProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
        api: {
            read    : 's_zcc701ukrv_kdService.selectList'
        }
    });
    
    var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
        api: {
            read    : 's_zcc701ukrv_kdService.selectList1',
            update  : 's_zcc701ukrv_kdService.updateDetail1',
            create  : 's_zcc701ukrv_kdService.insertDetail1',
            destroy : 's_zcc701ukrv_kdService.deleteDetail1',
            syncAll : 's_zcc701ukrv_kdService.saveAll1'
        }
    });
    
    var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
        api: {
            read    : 's_zcc701ukrv_kdService.selectList2',
            update  : 's_zcc701ukrv_kdService.updateDetail2',
            create  : 's_zcc701ukrv_kdService.insertDetail2',
            destroy : 's_zcc701ukrv_kdService.deleteDetail2',
            syncAll : 's_zcc701ukrv_kdService.saveAll2'
        }
    });    
     ...
     */
    //그리드1, 2
    Unilite.defineModel('s_zcc701ukrv_kd1Model', {
        fields: [
            ...
            {name : 'MAKE_QTY',             text : '가공수량',        type : 'uniQty'},
            ...
            {name : 'WIRE_P',               text : '와이어단가',         type : 'uniPrice'},
            {name : 'WIRE_S_P',             text : '와이어S단가',        type : 'uniPrice'},
            {name : 'LASER_P',              text : '레이저단가',         type : 'uniPrice'},
            {name : 'COAT_P',               text : '코팅단가',          type : 'uniPrice'},
            {name : 'ETC_P',                text : '기타단가',          type : 'uniPrice'},
            {name : 'TOTAL_AMT',            text : '합계',            type : 'uniPrice'},
            {name : 'EST_P',                text : '견적단가',          type : 'uniPrice'},
            {name : 'MARGIN_AMT',           text : '마진금액',          type : 'uniPrice'},
            {name : 'TEMP_P',               text : '임시단가',          type : 'uniPrice'},
            {name : 'NEGO_P',               text : '네고단가',          type : 'uniPrice'},
            {name : 'SAMPLE_QTY',           text : '샘플수량',          type : 'uniQty'},
    ...
    //수금그리드
    Unilite.defineModel('s_zcc701ukrv_kd2Model', {
        fields: [
            {name : 'COMP_CODE',            text : '법인코드',         type : 'string', allowBlank : false},
            {name : 'DIV_CODE',             text : '사업장',          type : 'string', comboType : "BOR120"},
    ...
    //그리드1
    var directMasterStore1 = Unilite.createStore('s_zcc701ukrv_kdMasterStore1',{
            model: 's_zcc701ukrv_kd1Model',
            autoLoad: false,
            uniOpt : {
    ...
    //그리드2
    var directMasterStore2 = Unilite.createStore('s_zcc701ukrv_kdMasterStore2',{
        model: 's_zcc701ukrv_kd1Model',
        uniOpt : {
            isMaster    : true,            // 상위 버튼 연결 
...
//            var param= panelResult.getValues();
        ...
        saveStore : function() {
                ...
                if(inValidRecs.length == 0) {
                    ...
                    var grid = Ext.getCmp('s_zcc701ukrv_kdGrid2');
    ...
    //그리드3
    var directMasterStore3 = Unilite.createStore('s_zcc701ukrv_kdMasterStore3',{
        model: 's_zcc701ukrv_kd2Model',
        uniOpt : {
            isMaster    : true,            // 상위 버튼 연결 
            editable    : true,            // 수정 모드 사용 
...
//            var param= panelResult.getValues();
        ...
        saveStore : function() {
                ...
                if(inValidRecs.length == 0) {
                    ...
                    var grid = Ext.getCmp('s_zcc701ukrv_kdGrid3');
                    grid.uniSelectInvalidColumnAndAlert(inValidRecs);
                }
            }
...
//        },
        ...
        api : {
            submit: 's_zcc701ukrv_kdService.syncForm'                
        },
    ...
    //그리드1
    var masterGrid1 = Unilite.createGrid('s_zcc701ukrv_kdGrid1', { 
        layout : 'fit',   
        region : 'center',                          
        ...
        columns : [ 
            ...
            {dataIndex : 'MAKE_END_YN',         width : 90},
        ...
        listeners : {
            select : function() {
                selectedGrid = 's_zcc701ukrv_kdGrid1';
    ...
    //그리드2
    var masterGrid2 = Unilite.createGrid('s_zcc701ukrv_kdGrid2', {
        layout : 'fit',
        region:'center',
        flex: 2,
        title : '샘플비미수금',
        ...
        listeners :{
            select : function() {
                selectedGrid = 's_zcc701ukrv_kdGrid2';
            ...
            beforeedit : function( editor, e, eOpts ) {
                if(UniUtils.indexOf(e.field, ['EST_P', 'TEMP_P', 'NEGO_P', 'SAMPLE_QTY', 'DELIVERY_DATE', 'CLOSE_YN'])) 
                {
                    var record1 = masterGrid1.getSelectedRecord();
                    if(record1.get('MAKE_END_YN') == 'N') {
                        return false;
                    } else {
                        return true;
                    }
    ...
    //그리드3
    var masterGrid3 = Unilite.createGrid('s_zcc701ukrv_kdGrid3', {
        layout : 'fit',
        region:'center',
        split: true,
        flex: 1,
        ...
        listeners :{
            select : function() {
                selectedGrid = 's_zcc701ukrv_kdGrid3';
...
//                 UniAppManager.setToolbarButtons(['delete', 'deleteAll', 'newData'], true);
    ...
    Unilite.Main({
        ...
        onDeleteDataButtonDown : function() {
            if(selectedGrid == 's_zcc701ukrv_kdGrid2') {
                if(confirm('현재행을 삭제 합니다.\n 삭제 하시겠습니까?')) {
                    masterGrid2.deleteSelectedRow();
                }
            } else if(selectedGrid == 's_zcc701ukrv_kdGrid3') {
        ...
        validate: function( type, fieldName, newValue, oldValue, record, eopt, editor, e) {
            ...
            switch(fieldName) {
                ...
                case "TEMP_P" :
                    ...
                    if(record.get('SAMPLE_QTY') == 0) {
                        record.set('DELIVERY_AMT', 0);
                    } else {
                        if(record.get('TEMP_P') == 0) {
                            record.set('DELIVERY_AMT', newValue * record.get('SAMPLE_QTY'));
                        }
                        else {
                            record.set('DELIVERY_AMT', record.get('TEMP_P') * record.get('SAMPLE_QTY'));
                        }
                    }
                    break;
                ...
                case "SAMPLE_QTY" :
                    ...
                    if(newValue == 0) {
                    	...
                    	if(record.get('TEMP_P') == 0) {
                            record.set('DELIVERY_AMT', record.get('NEGO_P') * newValue);
                        }
                        else {
                            record.set('DELIVERY_AMT', record.get('TEMP_P') * newValue);
                        }
                    }
                    break;
            }
