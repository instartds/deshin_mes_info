...
<t:appConfig pgmId="s_zdd400ukrv_kd"  >
...
function appMain() {
    ...
    var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 's_zdd400ukrv_kdService.selectList',
            update: 's_zdd400ukrv_kdService.updateList',
            syncAll: 's_zdd400ukrv_kdService.saveAll'
        }
    });
	
	Unilite.defineModel('s_zdd400ukrv_kdModel', {                            
        fields: [
                ...
                if(inValidRecs.length == 0 )    {                                       
                    var config = {
                        ...
                        success: function(batch, option) {
                            ...
                            s_zdd400ukrv_kdService.getFileList({REQ_NUM : master.REQ_NUM},              //파일조회 메서드  호출(param - 파일번호) 
        	...
        	selectionchange:function( model1, selected, eOpts ) {
                ...
                if(selected.length > 0) {
                    ...
                    if(directMasterStore.getCount() > 0 && record && !record.phantom){
                        ...
                        s_zdd400ukrv_kdService.getFileList({REQ_NUM : reqNum},              //파일조회 메서드  호출(param - 파일번호)  
                            function(provider, response) {                          
                                fp.loadData(response.result);                       //업로드 파일 load해서 보여주기
                                fp.getEl().unmask();                                //mask off
...
//                var record = masterGrid.getSelectedRecord();
                    ...
                    listeners: {
                        onSelected: {
                            fn: function(records, type) {
                                ...
                                s_zdd400ukrv_kdService.selectPersonDept(param, function(provider, response)  {     
     ...
     * @type
     */     
    var detailForm = Unilite.createForm('s_zdd400ukrv_kdDetail', {
        disabled :false,
        border:true,
        padding:'1 1 1 1',
        flex:1,
        ...
        items :[
            ...
            Unilite.popup('Employee',{
                    ...
                    listeners: {
                        onSelected: {
                            fn: function(records, type) {
                                var param= Ext.getCmp('s_zdd400ukrv_kdDetail').getValues();
                                s_zdd400ukrv_kdService.selectPersonDept(param, function(provider, response)  {     
                                    if(!Ext.isEmpty(provider)){                                                
                                        detailForm.setValue('REQ_DEPT_CODE', provider[0].DEPT_CODE);  
                                        detailForm.setValue('REQ_DEPT_NAME', provider[0].DEPT_NAME);             
