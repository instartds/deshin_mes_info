...
<t:appConfig pgmId="s_hat900skr_kd">
...
function appMain() {
	 ...
	 * @type 
	 */
	Unilite.defineModel('s_hat900skr_kdModel', {
		fields: [
	 ...
	 * @type 
	 */					
	var masterStore = Unilite.createStore('s_hat900skr_kdMasterStore1',{
		model	: 's_hat900skr_kdModel',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {			
					read: 's_hat900skr_kdService.selectList'
			}
		},
		...
		listeners: {
			load: function(store, records, successful, eOpts) {
				...
				var re = panelResult.getValue('DUTY_DATE_FR');
				
				gsToday 		= UniDate.getDbDateStr(re);
...
//			}
	 ...
	 * @type 
	 */
	var masterGrid = Unilite.createGrid('s_hat900skr_kdGrid1', {
		store	: masterStore,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	Unilite.Main({
		...
		fnInitBinding : function() {
			//초기값 설정
			panelResult.setValue('DUTY_DATE_FR'	, new Date());
			panelResult.setValue('DUTY_DATE_TO' , new Date());
			...
			//초기화 시, 포커스 설정
			panelResult.onLoadSelectText('DUTY_DATE_FR');
			//버튼 설정
        ...
        requestApprove: function(){     //결재 요청
            ...
            var datefr      = UniDate.getDbDateStr(panelResult.getValue('DUTY_DATE_FR'));
            var dateto      = UniDate.getDbDateStr(panelResult.getValue('DUTY_DATE_TO'));
            ...
            var spText      = 'omegaplus_kdg.unilite.USP_HUMAN_HAT900SKR_fnQuery ' + "'" + compCode + "'" + ', ' + "'" + divCode + "'"
                            + ', ' + "'" + datefr + "'" + ', ' + "'" + dateto + "'" + ', ' + "'" + deptcodefr + "'" + ', ' + "'" + deptcodeto + "'" + ', ' + "'" + personnumbfr + "'" + ', ' + "'" + personnumbto + "'"+ ', ' + "''" 
                            + ', ' + "'" + userId + "'" + ', ' + "'ko'" + ', ' + "''";
            var spCall      = encodeURIComponent(spText); 
            
            var groupUrl = "http://gw.kdg.co.kr:8070/approval/legacyGateway.hi?viewMode=docuDraft&prg_no=hat900skr&draft_no=0&sp=EXEC " 
