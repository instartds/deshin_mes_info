...
<t:appConfig pgmId="s_hat850skr_kd">
...
function appMain() {
	 ...
	 * @type 
	 */
	Unilite.defineModel('s_hat850skr_kdModel', {
		fields: [
			...
			{name: 'DUTY_FR_0800'	,text: '~08:00'			,type: 'string'},
			{name: 'DUTY_FR_0815'	,text: '~08:15'			,type: 'string'},
			{name: 'DUTY_FR_0820'	,text: '08:15~'			,type: 'string'},
	 ...
	 * @type 
	 */					
	var masterStore = Unilite.createStore('s_hat850skr_kdMasterStore1',{
		model	: 's_hat850skr_kdModel',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {			
					read: 's_hat850skr_kdService.selectList'
			}
		},
...
//					detailForm.openCryptRepreNoPopup();
	 ...
	 * @type 
	 */
	var masterGrid = Unilite.createGrid('s_hat850skr_kdGrid1', {
		store	: masterStore,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	Unilite.Main({
		...
		requestApprove: function(){     //결재 요청
            ...
            var frdutydate  = UniDate.getDbDateStr(panelResult.getValue('DUTY_FR_DATE'));
            var todutydate  = UniDate.getDbDateStr(panelResult.getValue('DUTY_TO_DATE'));
            var deptcode    = panelResult.getValue('DEPT_CODE');
            var personnumb  = panelResult.getValue('PERSON_NUMB');
            
            
            //var record = masterGrid.getSelectedRecord();
            var groupUrl = "http://gw.kdg.co.kr:8070/approval/legacyGateway.hi?viewMode=docuDraft&prg_no=hat850skr&draft_no=0&sp=EXEC " 
                                   
            var spText      = 'omegaplus_kdg.unilite.USP_HUMAN_HAT850SKR_fnQuery ' + "'" + compCode + "'" + ', ' + "'" + divCode + "'" 
                            + ', ' + "'" + frdutydate + "'" + ', ' + "'" + todutydate + "'" + ', ' + "'" + deptcode + "'"
                            + ', ' + "'" + personnumb + "'" + ', ' + "''" + ', ' + "'" + userId + "'" + ', ' + "'ko'"
                            + ', ' + "''" + ', ' + "''" + ', ' + "''";
