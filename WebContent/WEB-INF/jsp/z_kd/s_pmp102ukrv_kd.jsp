...
<t:appConfig pgmId="s_pmp102ukrv_kd"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_pmp102ukrv_kd" /> 			<!-- 사업장 -->
...
var BsaCodeInfo = {
    ...
    gsEquipCode  : '${gsEquipCode}'             // 작업지시 설비여부
};
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_pmp102ukrv_kdService.selectDetailList',
			update: 's_pmp102ukrv_kdService.updateDetail',
			create: 's_pmp102ukrv_kdService.insertDetail',
			destroy: 's_pmp102ukrv_kdService.deleteDetail',
			syncAll: 's_pmp102ukrv_kdService.saveAll'
		}
	});

	var masterForm = Unilite.createSearchPanel('s_pmp102ukrv_kdMasterForm', {
		title: '검색조건',
		    	  ...
		    	  },
		    	  {
		    	  	...
		    	  	handler: function(){
						var param = {
							...
							"LOT_NO" : panelResult.getValue('LOT_NAME'),
							"ITEM_CODE" : panelResult.getValue('ITEM_CODE')
						}
						s_pmp102ukrv_kdService.selectLotNoCheck(param, function(provider, response)	{
							if(!Ext.isEmpty(provider)){
								Unilite.messageBox('중복된 LOT NO 입니다.');
								return false;
							}else{
...
//		    	  },
	 ...
	 * @type
	 */
	Unilite.defineModel('s_pmp102ukrv_kdDetailModel', {
	    fields: [
	...
	Unilite.defineModel('s_pmp102ukrv_kdDetailModel2', {
	    fields: [
	 ...
	 * @type
	 */
	var detailStore = Unilite.createStore('s_pmp102ukrv_kdDetailStore', {
		model: 's_pmp102ukrv_kdDetailModel',
		autoLoad: false,
		uniOpt: {
		 ...
		 * 's_pmp102ukrv_kdService.selectDetailList', update:
		 * 's_pmp102ukrv_kdService.updateDetail', create:
		 * 's_pmp102ukrv_kdService.insertDetail', destroy:
		 * 's_pmp102ukrv_kdService.deleteDetail', syncAll:
		 * 's_pmp102ukrv_kdService.syncAll' } },
		 */
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				Ext.getCmp('s_pmp102ukrv_kdGrid').getSelectionModel().selectAll();
...
/* 			s_pmp102ukrv_kdService.saveAll(paramMaster, function(provider, response)	{
					if(!Ext.isEmpty(provider)){
						sWkordNum = provider['sWkordNum'];
						topLotno = provider['topLotno'];
         ...
         config = {
                ...
                var grid = Ext.getCmp('s_pmp102ukrv_kdGrid');
	...
	var detailStore2 = Unilite.createStore('s_pmp102ukrv_kdDetailStore2', {
		model: 's_pmp102ukrv_kdDetailModel2',
		autoLoad: false,
		uniOpt: {
			//isMaster: true,			// 상위 버튼 연결
			editable: true,			// 수정 모드 사용
    ...
    var detailGrid = Unilite.createGrid('s_pmp102ukrv_kdGrid', {
    	layout: 'fit',
        region:'north',
        uniOpt: {
...
//		selModel :   Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false }),
    ...
    var detailGrid2 = Unilite.createGrid('s_pmp102ukrv_kdGrid2', {
    	layout: 'fit',
        region:'center',
        uniOpt: {
			expandLastColumn: true,
    ...
    Unilite.defineModel('s_pmp102ukrv_kdProductionPlanModel', {
    	fields: [
	    	{name: 'GUBUN'        			, text: '선택'			, type: 'string'},
	...
	var productionPlanStore = Unilite.createStore('s_pmp102ukrv_kdProductionPlanStore', {
			model: 's_pmp102ukrv_kdProductionPlanModel',
            autoLoad: false,
            uniOpt : {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 's_pmp102ukrv_kdService.selectEstiList'
                }
            },
            listeners:{
	...
	var productionPlanGrid = Unilite.createGrid('s_pmp102ukrv_kdproductionPlanGrid', {
    	layout : 'fit',
...
//	          						  'PRODT_START_DATE':record.get('PRODT_START_DATE'),	참조시 변동없어야 한다고 해서 주석 처리
...
//                			 	productionPlanStore.loadStoreRecords();
    ...
    <%@include file="./s_pmp102ukrp2v_kd.jsp"%>
      ...
      Unilite.Main ({
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_pmp102ukrv_kdAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config)	{
			var fp = Ext.getCmp('s_pmp102ukrv_kdFileUploadPanel');
