...
<t:appConfig pgmId="s_opo100ukrv_kd"  >
    <t:ExtComboStore comboType="BOR120" pgmId="s_opo100ukrv_kd"  />             <!-- 사업장 -->
...
function appMain() {
    ...
    var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 's_opo100ukrv_kdService.selectList',
            update: 's_opo100ukrv_kdService.updateDetail',
            create: 's_opo100ukrv_kdService.insertDetail',
            destroy: 's_opo100ukrv_kdService.deleteDetail',
            syncAll: 's_opo100ukrv_kdService.saveAll'
        }
    });
    /**
     *   Model 정의
     * @type
     */
    Unilite.defineModel('s_opo100ukrv_kdModel', {
        fields: [
            ...
            {name: 'ORDER_UNIT_P'       ,text: '<t:message code="system.label.purchase.price" default="단가"/>'                 ,type: 'uniUnitPrice', allowBlank: true},
            ...
            {name: 'CHILD_ITEM_CODE'    ,text: '<t:message code="system.label.purchase.childitemmaterial" default="소모자재"/>'               ,type: 'string'},
            {name: 'CHILD_NEED_QTY'     ,text: '<t:message code="system.label.purchase.requiredqty" default="소요량"/>'               ,type: 'uniQty'},
            {name: 'CHILD_UNIT_Q'       ,text: '<t:message code="system.label.purchase.childunitqty" default="단위량"/>'               ,type: 'uniQty'},
            {name: 'S_GUBUN_KD'       ,text: '<t:message code="system.label.purchase.applyseq" default="적용차수"/>'               ,type: 'string'},
            {name: 'GW_FLAG'                , text: '<t:message code="system.label.purchase.drafting" default="기안여부"/>'				       , type: 'string', comboType:'AU', comboCode:'WB17'},
            {name: 'GW_DOC'             ,text: '<t:message code="system.label.purchase.draftdocno" default="기안문서번호"/>'               ,type: 'string'},
    ...
    Unilite.defineModel('referModel', {       // 외주PL참조
        fields: [
            {name : 'CHECK',            text : '<t:message code="system.label.purchase.apply" default="적용"/>',      type: 'boolean'},
            {name : 'COMP_CODE',        text : '<t:message code="system.label.purchase.companycode" default="법인코드"/>',   type : 'string'},
            {name : 'DIV_CODE',         text : '<t:message code="system.label.purchase.division" default="사업장"/>',    type : 'string', comboType: 'BOR120'},
            {name : 'GUBUN',            text : '<t:message code="system.label.purchase.classfication" default="구분"/>',     type : 'string'},
            {name : 'PROD_ITEM_CODE',   text : '<t:message code="system.label.purchase.proditemcode" default="가공품목"/>',   type : 'string'},
            {name : 'PROD_ITEM_NAME',   text : '<t:message code="system.label.purchase.proditemcode" default="가공품목"/>',    type : 'string'},
            {name : 'STOCK_UNIT',       text : '<t:message code="system.label.purchase.inventoryunit" default="재고단위"/>',   type: 'string', comboType:'AU', comboCode:'B013'},
            {name : 'PROD_SPEC',        text : '<t:message code="system.label.purchase.spec" default="규격"/>',     type : 'string'},
            {name : 'ORDER_UNIT',       text : '<t:message code="system.label.purchase.unit" default="단위"/>',     type : 'string', comboType:'AU', comboCode:'B013'},
            {name : 'UNIT_Q',           text : '<t:message code="system.label.purchase.originunitqty" default="원단위량"/>',   type : 'uniQty'},
            {name : 'PROD_UNIT_Q',      text : '<t:message code="system.label.purchase.parentitemqty" default="모품목수"/>',   type : 'uniQty'},
            {name : 'SHARE_RATE',       text : '<t:message code="system.label.purchase.sharerate" default="차지율"/>',    type: 'float', decimalPrecision: 2, format:'0,000.0000'},
            {name : 'INPUT_Q',          text : '<t:message code="system.label.purchase.partitionqty" default="분할수량"/>(*)',   type : 'uniQty'},
            {name : 'INPUT_RATE',       text : '<t:message code="system.label.purchase.inputrate" default="분할율"/>(%)',   type : 'uniQty'},
            {name : 'ORDER_Q',          text : '<t:message code="system.label.purchase.pounitqty" default="발주수량"/>',   type : 'uniQty'},
            {name : 'CHILD_ITEM_CODE',  text : '<t:message code="system.label.purchase.childitemmaterial" default="소모자재"/>',   type : 'string'},
            {name : 'CHILD_NEED_QTY',   text : '<t:message code="system.label.purchase.childitemmaterialqty" default="소요자재량"/>',  type : 'uniQty'},
            {name : 'CHECK_YN',         text : '<t:message code="system.label.purchase.checkyn" default="체크여부"/>',   type : 'string'},
            {name : 'CALC_UNIT_PRICE',  text : '<t:message code="system.label.purchase.price" default="단가"/>(<t:message code="system.label.purchase.purchaseprice" default="구매단가"/>*차지율)',   type : 'float'},
            {name : 'ITEM_P',           text : '<t:message code="system.label.purchase.purchaseprice" default="구매단가"/>',   type : 'float'},
            {name : 'PO_AMT',           text : '<t:message code="system.label.purchase.poamount" default="발주금액"/>',   type : 'float'}
        ]
    });
    /**
     * Store 정의(Service 정의)
     * @type
     */
    var directMasterStore1 = Unilite.createStore('s_opo100ukrv_kdMasterStore1',{
        model: 's_opo100ukrv_kdModel',
        uniOpt: {
        ...
        saveStore: function() {
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('s_opo100ukrv_kdGrid');
    ...
    var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {    //조회버튼 누르면 나오는 조회창
        ...
        proxy: {
            type: 'direct',
            api: {
                read    : 's_opo100ukrv_kdService.selectOrderNumList'
            }
        },
        loadStoreRecords : function()   {
    ...
    var otherOrderStore2 = Unilite.createStore('mpo501ukrvOtherOrderStore2', {     // 구매요청등록참조
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 's_opo100ukrv_kdService.selectMre100tList'
            }
        },
        listeners:{
            load:function(store, records, successful, eOpts) {
    ...
    var referStore = Unilite.createStore('referStore', {     // 구매요청등록참조
        model: 'referModel',
        autoLoad: false,
        uniOpt : {
            isMaster: false,            // 상위 버튼 연결
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 's_opo100ukrv_kdService.selectReferList'
            }
        },
        listeners: {
            load:function(store, records, successful, eOpts) {
                ...
                /*         console.log("store.items :", store.items);
                        ...
                        Ext.each(records,
                            function(item, i)   {
                                Ext.each(masterRecords.items, function(record, i)   {
                                    console.log("record :", record);
                                    if((record.data['ITEM_CODE'] == item.data['PROD_ITEM_CODE'])){
                                        orderRecords.push(item);
                                    }
                                });
                            });
        ...
        loadStoreRecords : function()   {
            ...
            if(referSearch.setAllFieldsReadOnly(true) == false) {
                return false;
            }
    ...
    var masterForm = Unilite.createSearchPanel('searchForm', {
        title: '<t:message code="system.label.purchase.pounitcodition" default="발주조건"/>',
...
//              allowBlank: false,
            ...
            Unilite.popup('PROG_WORK_CODE', {
                fieldLabel: '<t:message code="system.label.purchase.routing" default="공정"/>',
...
//              allowBlank: false,
                ...
                listeners: {
                    onSelected: {
                        fn: function(records, type) {
                            panelResult.setValue('PROG_WORK_CODE', masterForm.getValue('PROG_WORK_CODE'));
                            panelResult.setValue('PROG_WORK_NAME', masterForm.getValue('PROG_WORK_NAME'));
                        },
                        scope: this
                    },
                    onClear: function(type) {
...
//              allowBlank: false,
                ...
                listeners: {
                    onSelected: {
                        fn: function(records, type) {
                            masterForm.setValue('PROG_WORK_CODE', panelResult.getValue('PROG_WORK_CODE'));
                            masterForm.setValue('PROG_WORK_NAME', panelResult.getValue('PROG_WORK_NAME'));
                        },
                        scope: this
                    },
                    onClear: function(type) {
                        masterForm.setValue('PROG_WORK_CODE', '');
                        masterForm.setValue('PROG_WORK_NAME', '');
                    },
                    applyextparam: function(popup){
                ...
                fieldLabel: '<t:message code="system.label.purchase.drafting" default="기안여부"/>TEMP',
                name:'GW_TEMP',
                xtype: 'uniTextfield',
                hidden: true
            },{
    ...
    var referSearch = Unilite.createSearchForm('referForm', {     // 외주PL참조
        layout: {type : 'uniTable', columns : 3},
        items :[{
                fieldLabel: '<t:message code="system.label.purchase.division" default="사업장"/>',
                name:'DIV_CODE',
...
//                                    masterGrid.setItemData(record,false, masterGrid.uniOpt.currentRecord);
                                    referSearch.setValue("ITEM_CODE",       record['ITEM_CODE']);
                                    referSearch.setValue("ITEM_NAME",       record['ITEM_NAME']);
                                    referSearch.setValue("STOCK_UNIT",      record['STOCK_UNIT']);
                        ...
                        'onClear': function(type) {
                            ...
                            referSearch.setValue("STOCK_UNIT",      '');
                        },
                        applyextparam: function(popup) {
                            popup.setExtParam({'DIV_CODE': referSearch.getValue('DIV_CODE')});
                        }
                    }
            }),{
                fieldLabel: '<t:message code="system.label.purchase.issuedqty" default="출고량"/>',
                name: 'QTY',
                xtype: 'uniNumberfield',
                decimalPrecision: 2,
                allowBlank:false,
            ...
            {
                fieldLabel: '<t:message code="system.label.purchase.unit" default="단위"/>',
                name: 'STOCK_UNIT',
                xtype: 'uniCombobox',
                comboType: 'AU',
                comboCode: 'B013',
                ...
                fieldLabel: '<t:message code="system.label.purchase.poqtytotal" default="발주량합계"/>',
                name: 'TOT_PO_QTY',
                xtype: 'uniNumberfield',
                holdable: 'hold',
                readOnly: true,
     ...
     * @type
     */
    var masterGrid= Unilite.createGrid('s_opo100ukrv_kdGrid', {
        region: 'center' ,
        layout: 'fit',
        ...
        tbar: [{
                ...
                text:'<t:message code="system.label.purchase.drafting2" default="기안"/>',
                handler: function() {
                    var param = panelResult.getValues();
                    param.DRAFT_NO = UserInfo.compCode + panelResult.getValue('ORDER_NUM');
                    if(confirm('기안 하시겠습니까?')) {
                        s_opo100ukrv_kdService.selectGwData(param, function(provider, response) {
                            if(Ext.isEmpty(provider[0]) || provider[0].GW_FLAG != '1' || provider[0].GW_FLAG != '3') {
                                panelResult.setValue('GW_TEMP', '기안중');
                                s_opo100ukrv_kdService.makeDraftNum(param, function(provider2, response) {
                                    UniAppManager.app.requestApprove1();
                                });
                            } else {
                                alert('<t:message code="system.message.purchase.message046" default="이미 기안된 자료입니다."/>');
                                return false;
                            }
                        });
                    }
                ...
                handler: function() {
                    ...
                    if (record.data.GW_DOC == "")
                    {
                        alert('<t:message code="system.message.purchase.message047" default="기안문서번호가 없습니다."/>');
                        return false;                    
                    }
                    else
                    {
            ...
            menu: Ext.create('Ext.menu.Menu', {
                items: [{
                        ...
                        {
                        itemId: 'referBtn',
                        text: '<t:message code="system.label.purchase.subcontractplentryrefer" default="외주P/L참조"/>',
                        handler: function() {
                            openReferWindow();
                        }
                        }/*, {
        ...
        columns: [
            ...
            {dataIndex:'ORDER_UNIT_Q'           , width: 95 },
            {dataIndex:'ORDER_UNIT'             , width: 80 , align: 'center'},
            {dataIndex:'UNIT_PRICE_TYPE'        , width: 90 , align: 'center'},
            {dataIndex:'ORDER_UNIT_P'           , width: 100 },
            {dataIndex:'ORDER_O'                , width: 110 },
            {dataIndex:'DVRY_DATE'              , width: 85 },
            ...
            {dataIndex:'CHILD_NEED_QTY'         , width: 110},
            {dataIndex:'CHILD_UNIT_Q'           , width: 110},
            {dataIndex:'S_GUBUN_KD'              , width: 100 ,hidden : true},
            ...
            beforeedit  : function( editor, e, eOpts ) {
                ...
                if(e.record.phantom) {
                	if(panelResult.getValue('GW_FLAG') == '1' || panelResult.getValue('GW_FLAG') == '3') {
                        ...
                        if(UniUtils.indexOf(e.field, ['ITEM_CODE','ITEM_NAME','ORDER_SEQ','S_GUBUN_KD'])) return true;
                        }
                	}
                }
                if(UniUtils.indexOf(e.field, ['ORDER_UNIT','DVRY_DATE','DVRY_TIME','ORDER_UNIT_P','MONEY_UNIT','EXCHG_RATE_O',
        ...
        setOrderData:function(record) {
            ...
            var param = {"DIV_CODE": record['DIV_CODE'],
                "DEPT_CODE": masterForm.getValue('DEPT_CODE')};
            s_opo100ukrv_kdService.callDeptInspecFlag(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
                    grdRecord.set('INSPEC_FLAG', provider['INSPEC_FLAG']);
                }
            });
...
//            grdRecord.set('WH_CODE'            , record['']);
        ...
        setReferData : function(record, gbnMax) {                     // 외주PL참조
            var grdRecord = this.getSelectedRecord();

            var param = {
                  'COMP_CODE': UserInfo.compCode
                , 'DIV_CODE' : record.DIV_CODE
                , 'ITEM_CODE': record.PROD_ITEM_CODE
            }
            s_opo100ukrv_kdService.fnGetReferData(param, function(provider, response) {
                if(!Ext.isEmpty(provider)) {

                	grdRecord.set('COMP_CODE'          , UserInfo.compCode);
                    grdRecord.set('DIV_CODE'           , record.DIV_CODE);
                    grdRecord.set('ITEM_CODE'          , provider[0].ITEM_CODE);
                    grdRecord.set('ITEM_NAME'          , provider[0].ITEM_NAME);
                    grdRecord.set('SPEC'               , provider[0].SPEC);
                    grdRecord.set('ORDER_UNIT'         , provider[0].ORDER_UNIT);
                    grdRecord.set('STOCK_UNIT'         , provider[0].STOCK_UNIT);
                    grdRecord.set('TRNS_RATE'          , provider[0].TRNS_RATE);
                    grdRecord.set('ORDER_P'            , record['CALC_UNIT_PRICE']);
                    grdRecord.set('DVRY_DATE'          , moment().add('day', provider[0].PURCH_LDTIME).format('YYYYMMDD'));
//                    grdRecord.set('WH_CODE'            , provider[0].WH_CODE);
                    grdRecord.set('ORDER_UNIT_P', record['CALC_UNIT_PRICE']);
                    ...
                    var param = {"ITEM_CODE"    : provider[0].ITEM_CODE,
                                 "CUSTOM_CODE"  : masterForm.getValue('CUSTOM_CODE'),
                                 "DIV_CODE"     : masterForm.getValue('DIV_CODE'),
                                 "MONEY_UNIT"   : masterForm.getValue('MONEY_UNIT'),
                                 "ORDER_UNIT"   : provider[0].ORDER_UNIT,
                                 "ORDER_DATE"   : masterForm.getValue('ORDER_DATE')
                                };
/*
                    mpo501ukrvService.fnOrderPrice(param, function(provider, response) {
                        if(!Ext.isEmpty(provider)){
                           ...
                           // grdRecord.set('ORDER_UNIT_P' ,   provider[0].ORDER_P);
                              grdRecord.set('ORDER_UNIT_P'  , provider['ORDER_P']);
                        }
                    });
*/
                    ...
                    grdRecord.set('ORDER_Q'            , grdRecord.get('ORDER_UNIT_Q') * grdRecord.get('TRNS_RATE'));
                    grdRecord.set('UNIT_PRICE_TYPE'    , 'Y');
                    grdRecord.set('ORDER_O'            , grdRecord.get('ORDER_UNIT_P') * grdRecord.get('ORDER_UNIT_Q'));
                    grdRecord.set('MONEY_UNIT'         , masterForm.getValue('MONEY_UNIT'));
                    grdRecord.set('EXCHG_RATE_O'       , masterForm.getValue('EXCHG_RATE_O'));
                    grdRecord.set('ORDER_LOC_P'        , grdRecord.get('ORDER_UNIT_P') * grdRecord.get('EXCHG_RATE_O'));
                    grdRecord.set('ORDER_LOC_O'        , grdRecord.get('ORDER_UNIT_P') * grdRecord.get('EXCHG_RATE_O') * grdRecord.get('ORDER_Q') * grdRecord.get('TRNS_RATE'));
            ...
            grdRecord.set('CHILD_NEED_QTY'  , record['CHILD_NEED_QTY']);
            grdRecord.set('CHILD_UNIT_Q'    , record['SHARE_RATE']);
        }
    });

    var orderNoMasterGrid = Unilite.createGrid('s_opo100ukrv_kdOrderNoMasterGrid', {        //조회버튼 누르면 나오는 조회창
        layout : 'fit',
        ...
        columns: [
            ...
            { dataIndex: 'GW_FLAG'          ,  width: 66,hidden:true},
            { dataIndex: 'PROG_WORK_CODE'   ,  width: 66,hidden:true},
            { dataIndex: 'PROG_WORK_NAME'   ,  width: 66,hidden:true},
        ...
        if(!excelWindow) {
            ...
            excelWindow = Ext.create( appName, {
                ...
                grids: [{
                    ...
                    readApi: 's_opo100ukrv_kdService.selectExcelUploadSheet1',
                    columns: [
...
//                        directMasterStore1.loadStoreRecords();
    ...
    var referGrid = Unilite.createGrid('referGrid', {    //구매요청등록참조
        layout: 'fit',
        store: referStore,
        uniOpt: {
            useGroupSummary: false,
...
//        }),
        columns: [
            {dataIndex : 'CHECK',               width: 40, xtype : 'checkcolumn', hidden : false,
                listeners:{
                    checkchange: function(CheckColumn, rowIndex, checked, eOpts) {
                        var grdRecord = referGrid.getStore().getAt(rowIndex);
                        if(checked == true) {
                            grdRecord.set('CHECK_YN', 'Y');
                        } else {
                        	grdRecord.set('CHECK_YN', '');
                        }
                    }
                }
            },
            ...
            {dataIndex : 'PROD_ITEM_NAME',      width: 140},
            {dataIndex : 'PROD_SPEC',           width: 130},
            {dataIndex : 'ORDER_UNIT',          width: 50, align: 'center'},
            {dataIndex : 'UNIT_Q',              width: 90},
            {dataIndex : 'PROD_UNIT_Q',         width: 90},
            {dataIndex : 'SHARE_RATE',          width: 80},
            {dataIndex : 'INPUT_Q',          	  width: 80},
            {dataIndex : 'INPUT_RATE',         	width: 80},
            {dataIndex : 'ORDER_Q',             width: 80},
            {dataIndex : 'CHILD_ITEM_CODE',     width: 100, hidden: true},
            {dataIndex : 'CHILD_NEED_QTY',      width: 90},
            {dataIndex : 'CHECK_YN',            width: 100, hidden: true},
            {dataIndex : 'CALC_UNIT_PRICE',     width: 100, hidden: true},
            {dataIndex : 'ITEM_P',              width: 100, hidden: true},
            {dataIndex : 'PO_AMT',              width: 100, hidden: true}
        ...
        listeners: {
            ...
            beforeedit : function( editor, e, eOpts) {
//                if ( (UniUtils.indexOf(e.field, ['ORDER_Q'])) || (UniUtils.indexOf(e.field, ['INPUT_Q'])) )
                if ( (UniUtils.indexOf(e.field, ['INPUT_Q'])) )
                {
                    if(e.record.data.GUBUN == 'LOSS') {
                    	return false;
                    } else {
                        return true;
                    }
            ...
            edit : function(editor, e, newValue, oldValue) {/*
                var fieldName = e.field;

                if(fieldName == 'ORDER_Q') {

                    var record      = referGrid.getSelectedRecord();
                    var shareRate   = e.record.get('SHARE_RATE');
                    var orderQ      = e.record.get('ORDER_Q');

                    if(shareRate == 0 || orderQ == 0) {
                        e.record.set('CHILD_NEED_QTY', 0);
                    } else if(shareRate != 0 && orderQ != 0) {
                        e.record.set('CHILD_NEED_QTY', shareRate * orderQ);
                    }

                    var store = referStore;
                    var lossQty = 0;

                    Ext.each(store.data.items, function(records, i) {
                        if(records.get('GUBUN') != 'LOSS') {
                            lossQty = lossQty + records.get('CHILD_NEED_QTY');
                        }
                    });

                    var calcQty = referSearch.getValue('QTY') - lossQty;

                    if(calcQty < 0) {
                        alert('소요자재량을 초과하였습니다.');
                        e.record.set('ORDER_Q', oldValue);
                        return false;
                    }

                    Ext.each(store.data.items, function(records, i) {
                        if(records.get('GUBUN') == 'LOSS') {
                            records.set('CHILD_NEED_QTY', calcQty);
                        }
                    });

                    referSearch.setValue('REMAIN_Q_H', calcQty);
                }
            */}
        },
        returnData: function(gbnMax)  {
        	var store = referStore;

            Ext.each(store.data.items, function(record, i) {
            	if(record.get('CHECK') == true) {
            	    if(record.get('CHILD_NEED_QTY') > 0) {
                    UniAppManager.app.onNewDataButtonDown('Y');
                    masterGrid.setReferData(record.data, gbnMax);
                  }

            	} else {
            		if(record.get('GUBUN') == 'LOSS') {
            			if(record.get('CHILD_NEED_QTY') > 0) {
                            UniAppManager.app.onNewDataButtonDown('Y');
                            masterGrid.setReferData(record.data);
            			}
            		}
            	}
            });

//            this.getStore().remove(store.data.items);
        }
    });

    function openReferWindow() {          //외주PL참조
        if(!UniAppManager.app.checkForNewDetail()) return false;
        if(!referWindow) {
            referWindow = Ext.create('widget.uniDetailWindow', {
                title: '<t:message code="system.label.purchase.subcontractplentryrefer" default="외주P/L참조"/>',
                width: 1080,
                height: 580,
                layout: {type:'vbox', align:'stretch'},
                ...
                tbar: ['->',{
                    itemId : 'resetBtn',
                    text: '<t:message code="system.label.purchase.reset2" default="초기화"/>',
                    handler: function() {
                        referSearch.setAllFieldsReadOnly(false);
                        referGrid.reset();
                        referSearch.clearForm();
                        referSearch.setValue('DIV_CODE', masterForm.getValue('DIV_CODE'));
                    ...
                    text: '<t:message code="system.label.purchase.afterapplyclose" default="적용 후 닫기"/>',
                    handler: function() {
//                        if(referSearch.getValue('REMAIN_Q_H') > 0) {
//                            alert('잔량이 존재합니다.');
//                            return false;
//                        }

                        referGrid.returnData(gbnMax);
                        referWindow.hide();
                        referGrid.reset();
                        referSearch.clearForm();
//                        directMasterStore1.loadStoreRecords();
                ...
                listeners : {
                    show: function( panel, eOpts )  {
                        referSearch.setValue('DIV_CODE', masterForm.getValue('DIV_CODE'));
                        referSearch.setValue('CUSTOM_CODE', masterForm.getValue('CUSTOM_CODE'));
                        if(!Ext.isEmpty(referSearch.getValue('ITEM_CODE'))) {
                            referStore.loadStoreRecords();
                        }
                    },
	                beforeshow: function ( me, eOpts ) {
	                	var count = directMasterStore1.getCount();
	                	var gbnKd = [];

	                	if(count > 0){
	     					   Ext.each(directMasterStore1.data.items,  function(record, index, records){

	     						  gbnKd.push(Number(record.get("S_GUBUN_KD")));

		                     })
		                     gbnMax = Ext.Array.max(gbnKd) + 1 ;
	                	}else{
	                		 gbnMax = 1;
	                	}
...
//          panelResult.setValue('DEPT_NAME',UserInfo.deptName);
        ...
        onNewDataButtonDown: function(gubunChk) {
            ...
            if(!Ext.isEmpty(param.ORDER_NUM)) {
                s_opo100ukrv_kdService.selectGwData(param, function(provider, response) {
                    if(Ext.isEmpty(provider[0]) || provider[0].GW_FLAG != '1' || provider[0].GW_FLAG != '3') {
                        ...
                        alert('<t:message code="system.message.purchase.message048" default="기안된 데이터는 삭제가 불가능합니다."/>');
                        return false;
                    }
                });
            } else {
        ...
        requestApprove1 : function(){     //결재 요청
            ...
            var spText      = 'EXEC omegaplus_kdg.unilite.USP_GW_S_OPO100UKRV_KD ' + "'" + compCode + "'" + ', ' + "'" + divCode + "'" + ', ' + "'" + orderNum + "'";
            var spCall      = encodeURIComponent(spText);

//            frm.action = '/payment/payreq.php';
         //   frm.action   = groupUrl + "viewMode=docuDraft" + "&prg_no=s_opo100ukrv_kd&draft_no=" + UserInfo.compCode + panelResult.getValue('ORDER_NUM') + "&sp=" + spCall/* + Base64.encode()*/;
          //  frm.target   = "payviewer";
         //   frm.method   = "post";
          //  frm.submit();
             var gwurl = groupUrl + "viewMode=docuDraft" + "&prg_no=s_opo100ukrv_kd&draft_no=" + UserInfo.compCode + panelResult.getValue('ORDER_NUM') + "&sp=" + spCall/* + Base64.encode()*/;
            UniBase.fnGw_Call(gwurl,frm,'GW');
        },
        requestApprove2 : function(record){     //결재 요청(View)
            var gsWin = window.open('about:blank','payviewer','width=500,height=500');
...
//            frm.action = '/payment/payreq.php';
//            frm.action   = groupUrl + "viewMode=docuDraft" + "&prg_no=s_opo100ukrv_kd&draft_no=" + '0' + "&sp=" + spCall/* + Base64.encode()*/;
            ...
            switch(fieldName) {
                ...
                case "ORDER_UNIT" :
                    var param = {
                                ...
                                , 'MONEY_UNIT'      : masterForm.getValue('MONEY_UNIT')
                                , 'ORDER_UNIT'      : newValue
                                , 'APLY_START_DATE' : UniDate.getDbDateStr(masterForm.getValue('ORDER_DATE'))
                            };

                    s_opo100ukrv_kdService.fnGetLastPriceInfo(param, function(provider, response) {
                        if(!Ext.isEmpty(provider)){
                            record.set('ORDER_UNIT_P',      provider[0].ITEM_P);
                            record.set('UNIT_PRICE_TYPE',   provider[0].PRICE_TYPE);

                            UniAppManager.app.fnCalOrderAmt(record, "P", provider[0].ITEM_P);
                        }
                    });

//                    UniAppManager.app.fnCalOrderAmt(record, "P", record.get('ORDER_UNIT_P'));
    ...
    Unilite.createValidator('validator02', {
        ...
        console.log('validate >>> ', {'type':type, 'fieldName':fieldName, 'newValue':newValue, 'oldValue':oldValue, 'record':record});
            var rv = true;
	          ...
	          var stockUnit = referSearch.getValue('STOCK_UNIT'); //재고단위
	          var outQty = referSearch.getValue('QTY'); //출고수량
            ...
            switch(fieldName) {
                case "INPUT_Q" :
                    if(newValue < 0) {
                        ...
                        var oldChildNeedQty = record.get('CHILD_NEED_QTY');
                        var oldInputRate    = record.get('INPUT_RATE');
                        var oldOrderQ       = record.get('ORDER_Q');
                	      var shareRate       = record.get('SHARE_RATE') / 100;
                        var inputQty        = newValue;
                	      var orderUnit       = record.get('ORDER_UNIT');


           			        if (stockUnit == "KG")
                        {
                          weight = outQty * shareRate;
                          poQty = weight * inputQty;
                        }
                        else
                        {
                          poQty = outQty * inputQty;
                        }

//                  			record.set('INPUT_RATE', newValue * shareRate * 100);

                  			record.set('ORDER_Q', poQty);
                        var orderQ = poQty;

                        if(shareRate == 0 || orderQ == 0) {
                            record.set('CHILD_NEED_QTY', 0);
                        } else if(shareRate != 0 && orderQ != 0) {
                            if (stockUnit == "KG")
                            {
                              record.set('CHILD_NEED_QTY', 1 * orderQ);
                            }
                            else
                            {
                              record.set('CHILD_NEED_QTY', shareRate * orderQ);
                            }

                            if (orderUnit == "EA")
                            {
                        			record.set('ORDER_Q', inputQty);
                              record.set('CHILD_NEED_QTY', inputQty / shareRate);
                            }

                        }
                        var mat_q = record.get('CHILD_NEED_QTY');

                   			record.set('INPUT_RATE', mat_q / outQty * 100);
                        ...
                        Ext.each(store.data.items, function(records, i) {
                            if(records.get('GUBUN') != 'LOSS') {
                                lossQty = lossQty + records.get('CHILD_NEED_QTY');
                                inputRate = inputRate + records.get('INPUT_RATE');
                            }
                        });
                        ...
                        if(calcQty < 0) {
                            alert('<t:message code="system.message.purchase.message052" default="소요자재량을 초과하였습니다."/>');
                            record.set('INPUT_Q',        oldValue);
                            record.set('ORDER_Q',        oldOrderQ);
                            record.set('CHILD_NEED_QTY', oldChildNeedQty);
                            record.set('INPUT_RATE',     oldInputRate);
                        ...
                        Ext.each(store.data.items, function(records, i) {
                            if(records.get('GUBUN') == 'LOSS') {
                              if (stockUnit  == "KG") {
                                records.set('ORDER_Q', calcQty);
                              }
                				      records.set('CHILD_NEED_QTY', calcQty);
                				      records.set('INPUT_RATE', 100-inputRate);
                            }
                        });
                ...
                case "ORDER_Q" :    // 입력불가처리
                    if(newValue < 0) {
                        ...
                        var oldChildNeedQty = record.get('CHILD_NEED_QTY');
                        var oldInputRate    = record.get('INPUT_RATE');
                        var oldInputQ       = record.get('INPUT_Q');

                	      var shareRate       = record.get('SHARE_RATE') / 100;
                        var orderQ          = newValue;

                        record.set('INPUT_Q', orderQ/outQty);
                        var inputQty = record.get('INPUT_Q')
                  			record.set('INPUT_RATE', inputQty * shareRate * 100);
    ...
    function fnTotPoCalc(store)
    {
      ...
      Ext.each(store.data.items, function(records, i) {
        poQty = records.get('ORDER_Q');
        poTotQty = poTotQty + poQty;

      });

      referSearch.setValue("TOT_PO_QTY", poTotQty);

      if (referSearch.getValue('STOCK_UNIT') == "M")
      {
        fnPriceCalc(store);
      }
    }
    ...
    //M 단가계산
    function fnPriceCalc(store)
    {
      var inputTotRate = 0;

      var outQty = referSearch.getValue('QTY');
      var itemP = 0;
      var j = 0;
      var totOutAmt = 0;
      var totPoAmt = 0;
      ...
      Ext.each(store.data.items, function(records, i) {
        ...
        if(records.get('GUBUN') != 'LOSS') {
          if (i == 0)
          {
            itemP = records.get('ITEM_P');
            totOutAmt = outQty * itemP;
          }
          inputTotRate = inputTotRate + records.get('INPUT_RATE');
        }
      });

      var orgAmt = totOutAmt / inputTotRate;

      Ext.each(store.data.items, function(records, i) {
        if(records.get('GUBUN') != 'LOSS') {

          inputRate = records.get('INPUT_RATE');
          poQty = records.get('ORDER_Q');

          poAmt = orgAmt * inputRate;
          poPrice = poAmt / poQty;

          totPoAmt = totPoAmt + poAmt;
          records.set('PO_AMT', poAmt);
          records.set('CALC_UNIT_PRICE', poPrice);

        }
      });

      var diff = totOutAmt - totPoAmt;
      if (diff != 0)
      {
        Ext.each(store.data.items, function(records, i) {
          if ( (records.get('GUBUN') != 'LOSS') && (i==0) ) {
            poAmt = records.get('PO_AMT');
            poQty = records.get('ORDER_Q');

            records.set('PO_AMT', poAmt-diff);
            records.set('CALC_UNIT_PRICE', (poAmt-diff)/poQty);
          }
        });
      }
...
//
//    function fnCalcOrderQ() {
//        var record = referGrid.getSelectedRecord();
//
//        var shareRate   = record.get('SHARE_RATE');
