...
<t:appConfig pgmId="s_agj105ukr_kd"  > 
...
var baseInfo = {
	...
	<%@include file="./s_accntGridConfig_kd.jsp" %> 
	/**
	 * 일반전표 Store 정의(Service 정의)
	 * @type 
	 */
	
	var directMasterStore1 = Unilite.createStore('s_agj105ukr_kdMasterStore1',getStoreConfig());
	...
	//createSearchForm
	var slipForm =  Unilite.createSearchForm('s_agj105ukr_kdSlipForm',  {		
        itemId: 's_agj105ukr_kdSlipForm',
...
/*                    comboType: 'AU',
     ...
     * @type 
     */
    var masterGrid1 = Unilite.createGrid('s_agj105ukr_kdGrid1', getGridConfig(directMasterStore1,'s_agj105ukr_kdGrid1','s_agj105ukr_kdDetailForm1', 0.6, false, '2'));

    var detailForm1 = Unilite.createForm('s_agj105ukr_kdDetailForm1',  getAcFormConfig('s_agj105ukr_kdDetailForm1',masterGrid1 ));
	
	//차대변 구분 전표
<%@include file="../../jsp/accnt/agj/agjSlip.jsp" %> 
    ...
    Unilite.Main({
		...
		onPrevDataButtonDown:  function()	{
			...
			if(slipForm.isValid() )	{
				...
				s_agj105ukr_kdService.getPrevSlipNum(param, function(provider, response){
					Ext.getBody().unmask();
					if(provider)	{
		...
		onNextDataButtonDown:  function()	{
			...
			if(slipForm.isValid())	{
				...
				s_agj105ukr_kdService.getNextSlipNum(param, function(provider, response){
					Ext.getBody().unmask();
					if(provider)	{
						panelSearch.setValue('AC_DATE', provider.EX_DATE);
						panelSearch.setValue('EX_NUM', provider.EX_NUM);
		...
		onResetButtonDown:function() {
			...
			var masterGrid1 = Ext.getCmp('s_agj105ukr_kdGrid1');
			panelSearch.reset();						
			masterGrid1.reset();
       ...
       requestApprove: function(){     //결재 요청
            ...
            {
                var groupUrl = "http://gw.kdg.co.kr:8070/approval/legacyGateway.hi?viewMode=docuDraft&prg_no=agj105ukr_2&draft_no=0&sp=EXEC "
                
            } else if (slipForm.getValue('REPORT_TYPE') == '03' || slipForm.getValue('REPORT_TYPE') == '04') {
            	
                var groupUrl = "http://gw.kdg.co.kr:8070/approval/legacyGateway.hi?viewMode=docuDraft&prg_no=agj105ukr_1&draft_no=0&sp=EXEC "
            }
...
//			record.set("CREDIT_NUM", '');    카드번호/현금영수증 팝업 띄울시 팝업 SEARCH FIELD에  값 셋팅 안되는 문제 때문에 수정 20161219
			...
			//카드과세/면세/영세
				...
				creditWIn = creditPopup (creditWIn, record, record.get('CREDIT_NUM'), "CREDIT_CODE", null, "CREDIT_NUM", null, null, null,  'VALUE');				
				record.set("REASON_CODE", '');
