...
<t:appConfig pgmId="s_str103ukrv_kd"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="s_str103ukrv_kd" /> 			<!-- 사업장 -->
...
var referStockMoveWindow;//재고이동참조
var gsRefYn = 'N'
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_str103ukrv_kdService.selectDetailList',
			update: 's_str103ukrv_kdService.updateDetail',
			create: 's_str103ukrv_kdService.insertDetail',
			destroy: 's_str103ukrv_kdService.deleteDetail',
			syncAll: 's_str103ukrv_kdService.saveAll'
		}
	});
...
//	});
	 ...
	 */
	//좌측 마스터 서치폼 정의
	var masterForm = Unilite.createSearchPanel('s_str103ukrv_kdMasterForm', {
		title: '검색조건',
...
//							]);
		...
		api: {
			load: 's_str103ukrv_kdService.selectMaster',
			submit: 's_str103ukrv_kdService.syncMaster'
		},
...
//			},
	...
	}); //End of var masterForm = Unilite.createForm('s_str103ukrv_kdMasterForm', {
				...
				change: function(combo, newValue, oldValue, eOpts) {
					masterForm.setValue('MONEY_UNIT', newValue);
					 if(newValue != 'KRW'){
	                        ...
	                        Ext.getCmp('TOT_SALE_TAXI').setConfig('decimalPrecision',6);
	                        Ext.getCmp('TOT_SALE_TAXI').focus();
	                        Ext.getCmp('TOT_SALE_TAXI').blur();
	                        Ext.getCmp('TOT_TAXI').setConfig('decimalPrecision',6);
	                        Ext.getCmp('TOT_TAXI').focus();
	                        Ext.getCmp('TOT_TAXI').blur();
	                        Ext.getCmp('TOT_SALENO_TAXI').setConfig('decimalPrecision',6);
	                        Ext.getCmp('TOT_SALENO_TAXI').focus();
	                        Ext.getCmp('TOT_SALENO_TAXI').blur();

	                    }else{
	                    	panelResult.setValue('NATION_INOUT','1');
	                    	Ext.getCmp('TOT_SALE_TAXI').setConfig('decimalPrecision',0);
	                        Ext.getCmp('TOT_SALE_TAXI').focus();
	                        Ext.getCmp('TOT_SALE_TAXI').blur();
	                        Ext.getCmp('TOT_TAXI').setConfig('decimalPrecision',0);
	                        Ext.getCmp('TOT_TAXI').focus();
	                        Ext.getCmp('TOT_TAXI').blur();
	                        Ext.getCmp('TOT_SALENO_TAXI').setConfig('decimalPrecision',0);
	                        Ext.getCmp('TOT_SALENO_TAXI').focus();
	                        Ext.getCmp('TOT_SALENO_TAXI').blur();
...
//                    url: CPATH+'/sales/str103cukrv.do',
//                    prgID: 's_str103ukrv_kd',
//                    extParam: param
//                });
//                win.center();
//                win.show();
...
//					   	alert(labelText+Msg.sMB083);
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('s_str103ukrv_kdDetailModel', {
		fields: [
				 ...
				 {name: 'ORDER_UNIT_P'					, text:'단가'	 				, type: 'uniUnitPrice', defaultValue: 0, editable: false},
				 ...
				 {name: 'ORDER_UNIT_O'					, text:'금액'	 			, defaultValue: 0  , type: 'uniFC', editable: false},
				 {name: 'ORDER_AMT_SUM'					, text:'합계금액'	 				, defaultValue: 0   , type: 'uniFC', editable: false},
...
//				 {name: 'PURCHASE_TYPE'					, text:'매입조건'				 	, type: 'string', comboType:'AU', comboCode:'YP09', editable: false},
				 ...
				 {name: 'INOUT_I'					 	, text:'환산액'			  , type: 'uniPrice'}
		]
	});
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('s_str103ukrv_kdDetailStore', {
		model: 's_str103ukrv_kdDetailModel',
		autoLoad: false,
		uniOpt: {
		...
		saveStore: function() {
			...
			Ext.each(list, function(record, index) {
				...
				if(record.get('ACCOUNT_YNC') == 'Y'){
				  if(record.get('ORDER_UNIT_P') == 0 && record.get('ORDER_UNIT_O') == 0){
                    alert("매출대상이 Yes입니다. 금액 과 단가 값을 입력해주세요.");
                    isErr = true;
                    return false;
				  }
                }
			...
			var inoutIGrid = this.sum('INOUT_I');
			var inoutIForm = panelResult.getValue('INOUT_I');

			if(	gsAllDelChk == 'N'){
				if(Number(inoutIGrid) != Number(inoutIForm)){
					alert("출고환산금액이 맞지 않습니다.\n출고환산금액을 조정하십시오.");
					return false;
				}
			}
...
//			panelResult.setValue('TOT_QTY',dtotQty);								 //수량총계
			console.log("[[환율]]" + exchgRate);
			console.log("[[출고총액]]" + panelResult.getValue('TOTAL_AMT'));
			console.log("[[환산금액합]]" + panelResult.getValue('TOTAL_AMT') * exchgRate);
			var totalAmt = panelResult.getValue('TOTAL_AMT');
			//masterForm.setValue('INOUT_I',totalAmt * exchgRate);   			//환산총액 기존 내역 주석처리
			//panelResult.setValue('INOUT_I',totalAmt * exchgRate);   			//환산총액

			masterForm.setValue('INOUT_I',dInoutITax + dInoutINoTax);   			//환산총액  폼의 환산액을
			panelResult.setValue('INOUT_I',dInoutITax + dInoutINoTax);   			//환산총액
...
//			console.log('first record2 ORDER_UNIT_O'+ firstRecord2.get('ORDER_UNIT_O'));
 	...
 	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('s_str103ukrv_kdGrid', {
		layout: 'fit',
		region:'center',
		uniOpt: {
...
//												}
												...
												if(rtnRecord.get('ORDER_UNIT_Q') < 0 && !Ext.isEmpty(rtnRecord.get('ORDER_UNIT_Q')))	{
													rv=Msg.sMB076;
													return false;
												}
												var sInout_q = rtnRecord.get('ORDER_UNIT_Q');	//출고량
												var lot_q = rtnRecord.get('TEMP_ORDER_UNIT_Q');//로트팝업에서 넘겨받는 수량

												if(!Ext.isEmpty(lot_q) && lot_q!= 0){
													if(sInout_q > lot_q){
														alert('출고량은 lot양품재고량을 초과할 수 없습니다. 양품재고량: ' + lot_q);
														rtnRecord.set('ORDER_UNIT_Q',lot_q);
														return false;
													}
												}
											});
...
//				grdRecord.set('SALE_BASIS_P'	,record['SALE_BASIS_P']);
		...
		setrequestData:function(record) {
	   		...
	   		grdRecord.set('ORDER_AMT_SUM', grdRecord.get('ORDER_UNIT_O') + grdRecord.get('INOUT_TAX_AMT')); //합계필드 자동계산
	   		var exchgRate = grdRecord.get('EXCHG_RATE_O');
	   		if(Ext.isEmpty(exchgRate)){
	   			exchgRate = 1;
	   		}
	   		var inoutVal = grdRecord.get('ORDER_AMT_SUM') * exchgRate ;
	   		grdRecord.set('INOUT_I', inoutVal.toFixed(0)); //환산액필드 자동계산
		},
		setSalesOrderData: function(record) {
			...
			var sRefCode2 = UniAppManager.app.fnGetSubCode(null, inoutTypeDetail) ;	//출고유형value의 ref2
	   		...
	   		if(record['ACCOUNT_YNC'] == "N"){
	   			...
	   			grdRecord.set('ORDER_UNIT_O'		, Math.floor(record['R_ORDER_O']));
	   			grdRecord.set('INOUT_TAX_AMT'		, Math.floor(record['R_ORDER_TAX_O']));
	   		}
	   		grdRecord.set('ORDER_AMT_SUM'			, Math.floor(record['R_ORDER_O']) + Math.floor(record['R_ORDER_TAX_O']));
	   		var exchgRate = 1;
	   		if(Ext.isEmpty(grdRecord.get('EXCHG_RATE_O'))){
	   			exchgRate = 1;
	   		}else{
	   			exchgRate =  grdRecord.get('EXCHG_RATE_O');
	   		}
	   		var inoutVal = grdRecord.get('ORDER_AMT_SUM') * exchgRate ;
	   		grdRecord.set('INOUT_I', inoutVal.toFixed(0)); //환산액필드 자동계산
	   		grdRecord.set('PRICE_TYPE'			, record['PRICE_TYPE']);
	   ...
	   setStockMoveData: function(record) {
		   var grdRecord = this.getSelectedRecord();
		   ...
		   grdRecord.set('LOT_NO'		      , record['LOT_NO']);
		   grdRecord.set('WH_CODE'		      , record['WH_CODE']);
		   //grdRecord.set('LOCATION'		      , record['LOCATION']);
		   grdRecord.set('INOUT_PRSN'	      , record['INOUT_PRSN']);
		   //grdRecord.set('REQSTOCK_Q'	      , record['REQSTOCK_Q']);
		   //grdRecord.set('OUT_STOCK_Q'	      , record['OUT_STOCK_Q']);
		   grdRecord.set('SALE_P'      	      , record['SALE_P']);
		   grdRecord.set('STOCK_Q'		      , record['STOCK_Q']);

		   grdRecord.set('ORDER_UNIT'		      , record['SALE_UNIT']);
		   grdRecord.set('ORDER_UNIT_Q'		      , record['REQSTOCK_Q']);
		   grdRecord.set('ORDER_UNIT_P'		      , record['SALE_P']);
		   grdRecord.set('ISSUE_REQ_NUM'		      , record['ISSUE_REQ_NUM']);
		   grdRecord.set('ISSUE_REQ_SEQ'		      , record['ISSUE_REQ_SEQ']);
		   grdRecord.set('WH_CELL_CODE'		      , record['WH_CELL_CODE']);
		   grdRecord.set('WH_CELL_NAME'		      , record['WH_CELL_NAME']);
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_str103ukrv_kdService.selectOrderNumMasterList'
				}
			},
			loadStoreRecords : function()	{
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('s_str103ukrv_kdOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
...
//		  	masterForm.setValue('CREATE_LOC', record.get('CREATE_LOC'));
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('s_str103ukrv_kdorderNoDetailGrid', {
		// title: '기본',
		layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
...
//								orderNoSearch.setValue('ORDER_TYPE',masterForm.getValue('ORDER_TYPE'));
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('s_str103ukrv_kdRequestModel', {
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('s_str103ukrv_kdRequestStore', {
			model: 's_str103ukrv_kdRequestModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_str103ukrv_kdService.selectRequestiList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('s_str103ukrv_kdRequestGrid', {
		// title: '기본',
		layout : 'fit',
		store: requestStore,
...
// 								requestSearch.setValue('ISSUE_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('s_str103ukrv_kdsalesOrderForm', {
		layout :  {type : 'uniTable', columns : 2},
		items :[
			Unilite.popup('DIV_PUMOK',{
			fieldLabel:'품목코드' ,
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('s_str103ukrv_kdsalesOrderModel', {
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('s_str103ukrv_kdsalesOrderStore', {
			model: 's_str103ukrv_kdsalesOrderModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_str103ukrv_kdService.selectSalesOrderList'
				}
			}
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('s_str103ukrv_kdsalesOrderGrid', {
		// title: '기본',
		layout : 'fit',
...
/**************************************************************************************************************************************/
	/**
	 * 재고이동참조하기 위한 Search Form, Grid, Inner Window 정의
	 */
	//재고이동참조 폼 정의
	 var stockMoveSearch = Unilite.createSearchForm('s_str103ukr_kdstockMoveForm', {
			layout :  {type : 'uniTable', columns : 2},
			items :[{
				xtype: 'uniCombobox',
		   		name:'DIV_CODE',
	...
	Unilite.defineModel('s_str103ukr_kdstockMoveModel', {
		fields: [
				 {name: 'ITEM_CODE'					,text: '품목코드'				, type: 'string'},
				 {name: 'ITEM_NAME'					,text: '품목명'					, type: 'string'},
				 {name: 'SPEC'								,text: '규격'						, type: 'string'},
				 ...
				 {name: 'INOUT_PRSN'				,text: '수불담당자'			,type: 'string'},
				 {name: 'REQSTOCK_Q'				,text: '요청수량'				,type: 'uniQty'},
				 {name: 'OUT_STOCK_Q'				,text: '재고수량'				, type: 'uniQty'},
				 {name: 'SALE_P'      						,text:'판매단가'  			,type:'uniPrice'},
				 {name: 'STOCK_Q'			   			,text: '현재고'		 			, type: 'uniQty'},
				 {name: 'ISSUE_REQ_NUM'			, text: '출하지시번호'		, type: 'string'},
				 {name: 'ISSUE_REQ_SEQ'				, text: '순번'					, type: 'uniQty'},
				 {name: 'SALE_UNIT'						, text: '판매단위'			, type: 'string',			comboType:'AU',			comboCode: 'B013',		displayedField: 'value'},
				 {name: 'ORDER_NUM'					, text: '수주번호'			, type: 'string'},
				 {name: 'ORDER_SEQ'					, text: 'ORDER_SEQ'	, type: 'int'},
				 {name: 'TAX_INOUT'					, text: 'TAX_INOUT'	, type: 'int'},
				 {name: 'REQSTOCK_DATE'			, text: '이동요청일'	, type: 'uniDate'}
		]
	});

	var stockMoveStore = Unilite.createStore('s_str103ukr_kdstockMoveStore', {
			model: 's_str103ukr_kdstockMoveModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_str103ukrv_kdService.selectStockMove'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts)	{
						   ...
						   if(masterRecords.items.length > 0)	{
							   ...
							   Ext.each(records,
							   			function(item, i)	{
			   								Ext.each(masterRecords.items, function(record, i)	{
			   										console.log("record :", record);
			   										//부모창에 적용된 데이타(COMP_CODE,DIV_CODE,ISSUE_REQ_NUM,ISSUE_REQ_SEQ 로 비교)는 데이타 표시하지 말것
			   										if( (record.data['ISSUE_REQ_NUM'] == item.data['ISSUE_REQ_NUM'])
			   											&& (record.data['ISSUE_REQ_SEQ'] == item.data['ISSUE_REQ_SEQ'])
			   											&& (record.data['COMP_CODE'] == item.data['COMP_CODE'])
			...
			,loadStoreRecords : function()	{
				var param= stockMoveSearch.getValues();
				console.log( param );
				this.load({
					params : param
				});
	...
	//재고이동참조 그리드 정의
	var stockMoveGrid = Unilite.createGrid('s_str103ukr_kdstockMoveGird', {
		layout : 'fit',
		store: stockMoveStore,
		selModel :   Ext.create('Ext.selection.CheckboxModel', { checkOnly : false, toggleOnClick:false, mode: 'SIMPLE' }),
			uniOpt:{
				onLoadSelectFirst : false,
				useRowNumberer: false
		...
		columns:  [
           { dataIndex: 'ITEM_CODE'			    ,  width: 80 },
				   ...
				   { dataIndex: 'OUT_STOCK_Q'		   	,  width: 80 },
				   ...
				   { dataIndex: 'TAX_INOUT'		        , width: 80   , hidden: true}
		  ]
	   ,returnData: function()	{
	   		var records = this.getSelectedRecords();
			Ext.each(records, function(record,i){
										UniAppManager.app.onNewDataButtonDown();
										detailGrid.setStockMoveData(record.data);
									});
			this.deleteSelectedRow();
			detailStore.fnOrderAmtSum();
	   	}
	...
	//재고이동참조 참조 메인
	function openStockMoveWindow() {
  		if(!UniAppManager.app.checkForNewDetail()) return false;
		if(!referStockMoveWindow) {
			referStockMoveWindow = Ext.create('widget.uniDetailWindow', {
				title: '재고이동참조',
				width: 1080,
				height: 580,
				layout:{type:'vbox', align:'stretch'},
				items: [stockMoveSearch, stockMoveGrid],
				tbar:  ['->',
										{	itemId : 'saveBtn',
											text: '조회',
											handler: function() {
												stockMoveStore.loadStoreRecords();
											},
											disabled: false
										},
										{	itemId : 'confirmBtn',
				...
				listeners : {beforehide: function(me, eOpt)	{
							 ...
							 beforeshow: function ( me, eOpts )	{
								 stockMoveSearch.setValue('DIV_CODE',masterForm.getValue('DIV_CODE'));
								 stockMoveSearch.setValue('FR_CUST_CODE',masterForm.getValue('CUSTOM_CODE'));
								 stockMoveSearch.setValue('CUSTOM_NAME',masterForm.getValue('CUSTOM_NAME'));
								 stockMoveSearch.setValue('ISSUE_DATE_TO', masterForm.getValue('INOUT_DATE'));
								 //stockMoveSearch.setValue('ISSUE_DATE_FR', masterForm.getValue('INOUT_DATE'));
 								stockMoveSearch.setValue('ISSUE_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
							 	stockMoveStore.loadStoreRecords();
							 }
				}
			})
		}
		referStockMoveWindow.center();
		referStockMoveWindow.show();
	}
...
//			panelResult.setAllFieldsReadOnly(false);
		...
		processParams: function(params) {
			...
			if(params.PGM_ID == 'sof100ukrv' || params.PGM_ID == 's_sof100ukrv_kd') { //수주등록 또는 수주등록(양평)에서 링크넘어올시
...
//				panelRsult.setValue('INOUT_NUM', formPram.D)
				...
				Ext.each(params.record, function(rec,i){
					...
					var inoutVal = (orderUnitO + orderTaxO) * rec.get('REF_EXCHG_RATE_O');
					var r = {
						INOUT_SEQ			: inoutSeq,
						INOUT_TYPE			: "2",
						INOUT_METH			: "1",
						...
						ORDER_AMT_SUM		: orderUnitO + orderTaxO,
						INOUT_I					: inoutVal.toFixed(0)
					}
...
//				s_str103ukrv_kdService.deptWhcode(param, function(provider, response){
...
//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_str103ukrv_kdAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config)	{
			var fp = Ext.getCmp('s_str103ukrv_kdFileUploadPanel');
...
//			masterForm.getField('TOT_QTY').setReadOnly(true);
			...
			masterForm.getField('INOUT_I').setReadOnly(true);
...
//			panelResult.getField('TOT_QTY').setReadOnly(true);
			...
			panelResult.getField('INOUT_I').setReadOnly(true);
		...
		fnOrderAmtCal: function(rtnRecord, sType, fieldName, nValue, taxType) {
			...
			var dOrderForO = 0;
			var dOrderForP = 0;
            var dMoneyUnit = panelResult.getValue('MONEY_UNIT');
			...
			if(sType == "P" || sType == "Q"){
				...
				if(dPriceType == "A"){
					if(dMoneyUnit == 'KRW'){
						dOrderForO = dOrderQ * Math.floor(dOrderP);
					}else{
						dOrderForO = dOrderQ * dOrderP;
					}
					console.log("[[dOrderQ]]" + dOrderQ);
					console.log("[[dOrderP]]" + dOrderP);
					console.log("[[dOrderForO1]]" + dOrderForO);
					...
					if(dMoneyUnit == 'KRW'){
						dOrderForO = dOrderQ * Math.floor(dOrderP)
						dOrderO	= dOrderQ * Math.floor(dOrderP)
					}else{
						dOrderForO = dOrderQ * dOrderP
						dOrderO	= dOrderQ * dOrderP
					}
				...
				dOrderForO = dOrderForO.toFixed(numDigitOfPrice); //20190717 금액 uniFC길이만큼 소수점 반올림처리

				rtnRecord.set('ORDER_UNIT_O', dOrderForO);
				rtnRecord.set('ORDER_AMT_SUM', dOrderForO + rtnRecord.get('INOUT_TAX_AMT'));
				console.log("[[dOrderForO2]]" + dOrderForO);
				var inoutVal = 0;
				inoutVal = rtnRecord.get('ORDER_AMT_SUM') * dExchgRate
				rtnRecord.set('INOUT_I', inoutVal.toFixed(0));//환산액 필드 계산
				...
				dOrderO.toFixed(numDigitOfPrice); //20190717 금액 uniFC길이만큼 소수점 반올림처리
				rtnRecord.set('INOUT_WGT_P', dOrderWgtP);
				rtnRecord.set('INOUT_VOL_P', dOrderVolP);
				...
				rtnRecord.set('ORDER_AMT_SUM', dOrderO + rtnRecord.get('INOUT_TAX_AMT'));
				var inoutVal = 0;
				inoutVal = rtnRecord.get('ORDER_AMT_SUM') * dExchgRate;
				rtnRecord.set('INOUT_I', inoutVal.toFixed(0));
				if(rtnRecord.get('TAX_TYPE') == '1'){
		...
		fnTaxCalculate: function(rtnRecord, dOrderO, taxType) {
			...
			var sTaxInoutType = CustomCodeInfo.gsTaxInout;
			...
			var dMoneyUnit = panelResult.getValue('MONEY_UNIT');
			var digit = UniFormat.FC.indexOf(".") == -1 ? UniFormat.FC.length : UniFormat.FC.indexOf(".") + 1;
	    	var numDigitOfPriceFc	= UniFormat.Price.length - digit;
...
//				dTaxAmtO   = UniSales.fnAmtWonCalc(dAmountI * dVatRate / 100, sWonCalBas, numDigitOfPrice);
			...
			if(dMoneyUnit == 'KRW'){
				rtnRecord.set('ORDER_UNIT_O',Math.floor(dOrderAmtO));
				rtnRecord.set('INOUT_TAX_AMT',Math.floor(dTaxAmtO));
				rtnRecord.set('ORDER_AMT_SUM', (Math.floor(dOrderAmtO) + Math.floor(dTaxAmtO)));
			}else{
				rtnRecord.set('ORDER_UNIT_O',dOrderAmtO.toFixed(0));
				rtnRecord.set('INOUT_TAX_AMT',dTaxAmtO.toFixed(0));
				rtnRecord.set('ORDER_AMT_SUM', (rtnRecord.get('ORDER_UNIT_O') + rtnRecord.get('INOUT_TAX_AMT')).toFixed(0));
			}

			var exchgRate = panelResult.getValue('EXCHG_RATE_O');				//환율
			var inoutVal = 0;
			inoutVal = rtnRecord.get('ORDER_AMT_SUM') * exchgRate;
			rtnRecord.set('INOUT_I', inoutVal.toFixed(0));
		},
...
//						detailStore.fnOrderAmtSum();
					...
					case "ORDER_UNIT_Q" :
						...
						if(!Ext.isEmpty(lot_q) && lot_q!= 0){
							if(sInout_q > lot_q){
								rv = "출고량은 lot양품재고량을 초과할 수 없습니다. 양품재고량: " + lot_q;
								break;
							}
						}
...
//						break;
					...
					case "INOUT_TAX_AMT" :
						...
						var exchgRate = 	panelResult.getValue('EXCHG_RATE_O');
						record.set('ORDER_AMT_SUM', record.get('ORDER_UNIT_O') + record.get('INOUT_TAX_AMT'));
						var inoutVal = 0;
						inoutVal = record.get('ORDER_AMT_SUM') * exchgRate;
						record.set('INOUT_I',inoutVal.toFixed(0));
						detailStore.fnOrderAmtSum();
						break;
