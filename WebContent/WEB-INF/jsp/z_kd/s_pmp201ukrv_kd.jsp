...
<t:appConfig pgmId="s_pmp201ukrv_kd"  >
    ...
    <t:ExtComboStore comboType="AU" comboCode="WB17"/>              <!-- 기안상태 -->
...
function appMain() {
    ...
    var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read:       's_pmp201ukrv_kdService.selectMaster',
            update:     's_pmp201ukrv_kdService.updateDetail',
            create:     's_pmp201ukrv_kdService.insertDetail',
            destroy:    's_pmp201ukrv_kdService.deleteDetail',
            syncAll:    's_pmp201ukrv_kdService.saveAll'
        }
    });

    var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read:       's_pmp201ukrv_kdService.orderApply2'
        }
    });
...
//              UniAppManager.setToolbarButtons('save', true);
    ...
    var panelResult = Unilite.createSearchForm('resultForm',{
      ...
      var panelResult3 = Unilite.createSearchForm('detailForm', { //createForm
            disabled: false,
            border:true,
            padding: '1',
            region: 'north',
     ...
     * @type
     */
    Unilite.defineModel('s_pmp201ukrv_kdMasterModel', {
        fields: [
            ...
            {name: 'WKORD_ITEM_CODE'        ,text: '작지품목'       ,type:'string'},
			{name: 'TOP_ITEM_NAME'        ,text: '작지품명'       ,type:'string'},
			{name: 'TOP_ITEM_SPEC'        ,text: '품번'       ,type:'string'},
...
//          {name: 'PJT_CODE'           ,text: '프로젝트번호'     ,type:'string'},
            ...
            {name: 'GW_TEXT'             ,text: '기안여부명'           ,type: 'string'},
            {name: 'GW_DOC'             ,text: '기안문서'           ,type: 'string'},
            {name: 'DRAFT_NO'           ,text: '기안번호'           ,type: 'string'},
            {name:'NOT_OUT_Q'           ,text: '미출고량'           ,type: 'uniQty', editable: false}
        ]
    });

    Unilite.defineModel('s_pmp201ukrv_kdDetailModel', {
        fields: [
            {name: 'WKORD_NUM'      ,text: '작업지시번호' ,type:'string'},
...
//          {name: 'PJT_CODE'       ,text: '프로젝트번호' ,type:'string'}
    ...
    Unilite.defineModel('issueRequestNoMasterModel', {
        fields: [
            ...
            {name: 'OUTSTOCK_REQ_DATE'  , text: '출고요청일'    , type: 'uniDate'},
            {name: 'GW_FLAG'  			, text: '기안상태'    	 , type: 'string', comboType :"AU" , comboCode : "WB17"},
            {name: 'LOT_NO'             , text: 'LOT NO'     , type: 'string'},
            {name: 'REMARK'             , text: '비고'        , type: 'string'},
     ...
     * @type
     */
    var masterStore = Unilite.createStore('s_pmp201ukrv_kdmasterStore', {
        model: 's_pmp201ukrv_kdMasterModel',
        autoLoad: false,
        uniOpt: {
                ...
                callback: function(records, operation, success){
                    ...
                    if(success){
                        if(masterGrid.getStore().getCount() == 0){
                          ...
                          var gwFlag = masterStore.data.items[0].data.GW_FLAG;
        ...
        listeners:{
            load: function(store, records, successful, eOpts) {
                /*  if(masterGrid.getStore().getCount() == 0) {
                    ...
                    Ext.getCmp('GW2').setDisabled(false);
                    panelResult3.setValue("ITEM_CODE",records[0].get('WKORD_ITEM_CODE'));
                    panelResult3.setValue("TOP_ITEM_NAME",records[0].get('TOP_ITEM_NAME'));
                    panelResult3.setValue("TOP_ITEM_SPEC",records[0].get('TOP_ITEM_SPEC'));
                    panelResult3.setValue("LOT_NO",records[0].get('LOT_NO'));
                    panelResult3.setValue("TXT_GW_FLAG",records[0].get('GW_TEXT'));
                    panelResult3.setValue("TXT_GW_DOC",records[0].get('GW_DOC'));
                    panelResult3.setValue("TXT_DRAFT_NO",records[0].get('DRAFT_NO'));


                 //조회시
                } else {
                    if(masterStore.data.items[0].data.GW_FLAG == 'Y' || masterStore.data.items[0].data.GW_FLAG == '3') {
                        Ext.getCmp('GW').setDisabled(true);
                        Ext.getCmp('GW2').setDisabled(true);
    ...
    /* var detailStore = Unilite.createStore('s_pmp201ukrv_kddetailStore', {
        model: 's_pmp201ukrv_kdDetailModel',
        autoLoad: false,
        uniOpt: {
...
//          console.log(param);
        ...
        saveStore: function() {
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('s_pmp201ukrv_kdGrid');
    ...
    var issueRequestNoMasterStore = Unilite.createStore('issueRequestNoMasterStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                    read    : 's_pmp201ukrv_kdService.selectOrderNumMasterList'
                }
            }
            ,loadStoreRecords : function()  {
     ...
     * @type
     */
    var masterGrid = Unilite.createGrid('s_pmp201ukrv_kdGrid', {
        layout: 'fit',
        region:'center',
        uniOpt: {
        ...
        tbar: [{
                ...
                handler: function() {
                    ...
                    if(confirm('기안 하시겠습니까?')) {
                        s_pmp201ukrv_kdService.selectGwData(param, function(provider, response) {
                            if(Ext.isEmpty(provider[0]) || provider[0].GW_FLAG != '1' || provider[0].GW_FLAG != '3') {
                                panelResult.setValue('GW_TEMP', '기안중');
                                s_pmp201ukrv_kdService.makeDraftNum(param, function(provider2, response)   {
                ...
                handler: function() {
                    ...
                    s_pmp201ukrv_kdService.selectDraftNo(param, function(provider, response) {
        ...
        columns: [
            ...
            {dataIndex: 'WKORD_ITEM_CODE'           , width: 100},
			{dataIndex: 'TOP_ITEM_NAME'        , width: 100},
			{dataIndex: 'TOP_ITEM_SPEC'        , width: 100},
...
//          {dataIndex: 'PJT_CODE'              , width: 100},
            ...
            {dataIndex: 'GW_TEXT'               , width: 100, hidden:true},
        ...
        viewConfig: {
	        getRowClass: function(record, rowIndex, rowParams, store){
	        	var cls = '';
	        	if(record.get('PAB_STOCK_Q') - record.get('OUTSTOCK_REQ_Q') < 0){
					cls = 'x-change-cell_1';	
				}
				return cls;
	        }
...
//              grdRecord.set('OUTSTOCK_REQ_Q'      , "");
//              grdRecord.set('CONTROL_STATUS'      , "2");
            panelResult3.setValue("ITEM_CODE",record.ITEM_CODE);
            panelResult3.setValue("TOP_ITEM_NAME",record.TOP_ITEM_NAME);
            panelResult3.setValue("TOP_ITEM_SPEC",record.TOP_ITEM_SPEC);
            panelResult3.setValue("LOT_NO",record.LOT_NO);
...
//          grdRecord.set('OUTSTOCK_REQ_DATE'   , record['OUTSTOCK_REQ_DATE']);
            grdRecord.set('CONTROL_STATUS'      , "2" );
            grdRecord.set('REF_WKORD_NUM'       , record['WKORD_NUM']);
            grdRecord.set('WKORD_ITEM_CODE'     , record['WKORD_ITEM_CODE']);
...
//          grdRecord.set('AGREE_PRSN'          , record['AGREE_PRSN']);
            ...
            grdRecord.set('NOT_OUT_Q'         , record['NOT_OUT_Q']);
...
/*     var detailGrid = Unilite.createGrid('s_pmp201ukrv_kdGrid2', {
        layout: 'fit',
        region:'south',
        uniOpt: {
            expandLastColumn: false,
...
//          {dataIndex: 'PJT_CODE'          , width: 100}
    ...
    var issueRequestNoMasterGrid = Unilite.createGrid('s_pmp201ukrv_kdissueRequestNoMasterGrid', {
        // title: '기본',
        layout : 'fit',
        ...
        columns:  [
            ...
            {dataIndex: 'GW_FLAG'           , width:100, align: 'center'},
...
//                    validateBlank:false,
            ...
            searchInfoWindow = Ext.create('widget.uniDetailWindow', {
                ...
                listeners : {
                    beforehide: function(me, eOpt)  {
                        ...
                        issueRequestNoMasterGrid.getStore().loadData({});
                    },
    ...
    // 작업지시 참조 모델 정의
    Unilite.defineModel('s_pmp201ukrv_kdWorkModel', {
        fields: [
...
//          {name: 'PJT_CODE'           , text: '프로젝트번호'        , type: 'string'}
    ...
    //작업지시 참조 스토어 정의
    var WorkStore = Unilite.createStore('s_pmp201ukrv_kdWorkStore', {
        model: 's_pmp201ukrv_kdWorkModel',
        autoLoad: false,
        uniOpt : {
            isMaster: false,            // 상위 버튼 연결
            editable: false,            // 수정 모드 사용
        ...
        proxy: {
            type: 'direct',
            api: {
                read    : 's_pmp201ukrv_kdService.selectEstiList'
            }
        },
        listeners:{
            load:function(store, records, successful, eOpts)    {
    ...
    // 작업지시 그리드
    var WorkGrid = Unilite.createGrid('s_pmp201ukrv_kdWorkGrid', {
        layout : 'fit',
        store: WorkStore,
...
//          { dataIndex: 'PJT_CODE'                     , width: 100}
        ...
        returnData: function(record)    {
            ...
            panelResult3.setValue("ITEM_CODE",record[0].get('ITEM_CODE'));
            panelResult3.setValue("TOP_ITEM_NAME",record[0].get('TOP_ITEM_NAME'));
            panelResult3.setValue("TOP_ITEM_SPEC",record[0].get('TOP_ITEM_SPEC'));
            panelResult3.setValue("LOT_NO",record[0].get('LOT_NO'));
            panelResult3.setValue("TXT_GW_FLAG",record[0].get('GW_TEXT'));
            panelResult3.setValue("TXT_GW_DOC",record[0].get('GW_DOC'));
            panelResult3.setValue("TXT_DRAFT_NO",record[0].get('DRAFT_NO'));
            var param= {
            ...
            s_pmp201ukrv_kdService.orderApply(param, function(provider, response){
...
//                          UniAppManager.setToolbarButtons(['newData'], true);
    ...
    Unilite.Main({
        id: 's_pmp201ukrv_kdApp',
        borderItems:[{
            region:'center',
            layout: 'border',
            border: false,
            items:[
                 masterGrid, panelResult,panelResult3
            ]},
            panelSearch
        ],
...
//                          ITEM_CODE: seq,
        ...
        onResetButtonDown: function() {
            ...
            panelSearch.getField('WORK_SHOP_CODE').setValue('');
            panelSearch.getField('OUTSTOCK_NUM').setValue('');
            panelSearch.getField('WORK_SHOP_CODE').setValue('');
            panelSearch.getField('REF_WKORD_NUM').setValue('');

            panelResult3.getField('TXT_GW_FLAG').setValue('');
            panelResult3.getField('TXT_GW_DOC').setValue('');
            panelResult3.getField('TXT_DRAFT_NO').setValue('');
            panelResult3.getField('ITEM_CODE').setValue('');
            panelResult3.getField('TOP_ITEM_NAME').setValue('');
            panelResult3.getField('TOP_ITEM_SPEC').setValue('');
            panelResult3.getField('LOT_NO').setValue('');
            Ext.getCmp('GW').setDisabled(true);
        },
        ...
        onDetailButtonDown: function() {
            var as = Ext.getCmp('s_pmp201ukrv_kdAdvanceSerch');
            if(as.isHidden())   {
                as.show();
            } else {
                as.hide()
        ...
        confirmSaveData: function(config)   {
            var fp = Ext.getCmp('s_pmp201ukrv_kdFileUploadPanel');
...
//            frm.action = '/payment/payreq.php';
            ...
            //frm.method   = "post";
            //frm.submit();
            var gwurl = groupUrl + "viewMode=docuDraft" + "&prg_no=s_pmp200ukrv_kd&draft_no=" + UserInfo.compCode + panelResult.getValue('OUTSTOCK_NUM') + "&sp=" + spCall/* + Base64.encode()*/;
