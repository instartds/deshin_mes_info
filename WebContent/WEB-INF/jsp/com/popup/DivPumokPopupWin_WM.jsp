...
<%
	...
	request.setAttribute("PKGNAME","Unilite.app.popup.DivPumokPopup_WM");
%>
	...
	Unilite.defineModel('${PKGNAME}.divPumokPopupModel', {
		fields: [
			...
			//20200515추가: 
			,{name: 'BIV150_G_STOCK_Q'	,text:'REMARK2'				,type : 'string'}
		]
	});
		...
		var masterGridConfig = {
			store : Unilite.createStoreSimple('${PKGNAME}.divPumokPopupMasterStore',{
				...
				proxy: {
					type: 'direct',
					api: {
						read: 'popupService.divPumokPopup_WM'
					}
				},
				listeners	: {
	...
	fnInitBinding : function(param) {
		...
		if( Ext.isDefined(param)) {
			if(Ext.isDefined(fieldTxt) && fieldTxt.isFormField) {
				...
				if(param['USE_YN'])	{
					frm.findField('USE_YN').setValue(param['USE_YN']);
				} else {
					frm.findField('USE_YN').setValue('Y');
				}
			}
			frm.setValues(param);
