...
<%
	//  사업장별 BOX 'Unilite.app.popup.DivBoxPopup' 
	request.setAttribute("PKGNAME","Unilite.app.popup.DivBoxPopup");
%>
	 ...
	 *   Model 정의 
	 */
	Unilite.defineModel('${PKGNAME}.divBoxPopupModel', {
	    fields: [ 	{name: 'BOX_CODE'    		,text:'BOX<t:message code="system.label.common.code" default="코드"/>'	,type : 'string'} 
					,{name: 'BOX_NAME'    			,text:'BOX<t:message code="system.label.common.name" default="명"/>'	,type : 'string'} 
					...
					,{name: 'TOTAL_ITEM'    	,text:'<t:message code="system.label.common.summarybox" default="집계BOX"/>'	,type : 'string'}  
...
//	    }
	    me.panelSearch = Unilite.createSearchForm('',{
	        ...
	        items: [   { fieldLabel: '<t:message code="system.label.common.division" default="사업장"/>' ,   name:'DIV_CODE'  , xtype: 'uniCombobox' , comboType:'BOR120', readOnly: true }
	                   ...
	                   ,{ fieldLabel: '<t:message code="system.label.common.account" default="계정"/>' ,   name:'ITEM_ACCOUNT'  , xtype: 'uniCombobox' , comboType:'AU' ,comboCode:'B020', value: '01', allowBlank: false, /*multiSelect : true, typeAhead: false,*/
		                    listeners:{
...
//	                   ,{ fieldLabel: 'BOX코드' ,   name:'ITEM_SEARCH' /*, allowBlank:false*/}
	                   ,{ xtype: 'uniTextfield', fieldLabel: '<t:message code="system.label.common.inquirycondition" default="조회조건"/>',     name:'TXT_SEARCH2'}
	                   ,{ fieldLabel: '<t:message code="system.label.common.ㅍagentcustomfiltertype" default="검색제외구분"/>', name:'ITEM_EXCLUDE' , hidden:true} // (DIV_CODE:BPR500T 해당 사업장에 이미등록된코드 제외, PROD_BOX_CODE: BPR500T 해당 사업장에 검색하는 등록된 코드 제외)
	                   ,{ fieldLabel: '<t:message code="system.label.common.agentcustomfilter" default="검색제외"/>', name:'PROD_BOX_CODE' , hidden:true} 
	                   ,{ fieldLabel: '<t:message code="system.label.common.agentcustomfilter" default="검색제외"/>', name:'CHILD_BOX_CODE' , hidden:true} 
...
//	                                if(newValue.RDO=='2')   {
//	                                    lblSearch.setFieldLabel( 'BOX명' );
//	                                } else if(newValue.RDO=='1'){
//	                                    lblSearch.setFieldLabel( 'BOX코드' );
//	                                }
//	                            }
//	                        }
//	                   }   
	     ...
	     */
	     ...
	     var masterGridConfig = {
            store : Unilite.createStoreSimple('${PKGNAME}.divBoxPopupMasterStore',{
							model: '${PKGNAME}.divBoxPopupModel',
				            autoLoad: false,
				            proxy: {
				                type: 'direct',
				                api: {
				                	read: 'popupService.divBoxPopup'
				                }
				            }
					}),
	        uniOpt:{
	        ...
	        selModel:'rowmodel',
	        columns:  [        
	                     { dataIndex: 'BOX_CODE',          width: 120} 
	                    ,{ dataIndex: 'BOX_NAME',          width: 240} 
                        ...
                        ,{ dataIndex: 'MAIN_CUSTOM_NAME',         width: 100  ,hidden:true}        
	        ] ,
	        listeners: {
...
//					fieldTxt.setValue(param['BOX_CODE']);
//					fieldTxt.setFieldLabel( 'BOX코드' );
//					rdo.setValue('1');
//				}else {
//					if(param['TYPE'] == 'VALUE') {
//						fieldTxt.setValue(param['BOX_CODE']);
...
//						rdo.setValue('1');
//					} else {
//						fieldTxt.setValue(param['BOX_NAME']);
//						fieldTxt.setFieldLabel( 'BOX명' );
//						rdo.setValue('2');
//					}
//				}
				var me = this;		
				...
				if(param['PROD_BOX_CODE'])	{
					frm.findField('PROD_BOX_CODE').setValue(param['PROD_BOX_CODE']); 
				}
				if(param['CHILD_BOX_CODE'])	{
					frm.findField('CHILD_BOX_CODE').setValue(param['CHILD_BOX_CODE']); 
				}
				if(param['DEFAULT_ITEM_ACCOUNT'])	{
					frm.findField('ITEM_ACCOUNT').setValue(param['DEFAULT_ITEM_ACCOUNT']); 
				}
				if(param['BOX_CODE'] || param['BOX_NAME'])	{
					this._dataLoad();
				}
			}
			frm.setValues(param);
