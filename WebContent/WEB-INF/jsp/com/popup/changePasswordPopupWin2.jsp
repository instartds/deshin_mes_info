...
<%
	...
	request.setAttribute("PKGNAME","Unilite.app.popup.changePassword2");
%>
				...
				items	: [{
					...
					items	: [{
						...
						handler	: function() {
							...
							baseCommonService.oldPwdCheck(param, function(provider, response) {
								var err = false;
...
//			padding	: '20 20 20 20',
	...
	fncheckPwd : function(newPwd) {
			...
			if(newPwd.toUpperCase().indexOf(UserInfo.userID.toUpperCase()) > -1){
				...
				baseCommonService.pwDuplicateCheck(param, function(provider, response) {
					if(!Ext.isEmpty(provider)){
					...
					if(!err1){				//비밀번호 교대사용 금지
						...
						//비밀번호 체크갯수 조회 B110
						baseCommonService.pwCheckQ(param, function(provider, response) {
							...
							var param = {"DCYCLE_CNT": dCycleCnt, "NEW_PWD": newPwd};
							baseCommonService.pwSameCheck(param, function(provider, response) {
								if(!Ext.isEmpty(provider)){
								...
								if(!err2){
									...
									baseCommonService.pwRuleCheck(param, function(provider, response) {
										var rule = [];
...
/*			if(checkVal.cntSeqChar >= 3) {
	...
	fnChangePw : function() {
		...
		var newPwd		= masterForm.getValue('NEW_PWD');
		baseCommonService.encryptionYN(param, function(provider, response) {	//비밀번호 암호화 여부
			if(!Ext.isEmpty(provider)){	//암호화 이면..
				...
				baseCommonService.encryptionSavePw(param);
				...
				baseCommonService.notEncryptionSavePw(param);
				Unilite.messageBox('<t:message code="system.message.base.message022" default="비밀번호를 변경하였습니다. 다음 로그인부터 적용됩니다."/>');
				masterForm.setValue('OLD_PWD'		, '');
				masterForm.setValue('NEW_PWD'		, '');
				masterForm.setValue('NEW_CFM_PWD'	, '');
