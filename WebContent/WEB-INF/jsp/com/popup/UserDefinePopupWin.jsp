...
<%
request.setAttribute("PKGNAME","Unilite.app.popup.UserDefinePopup");
%>
	...
	<t:ExtComboStore useScriptTag="false" comboType="BOR120" />							// 사업장 
	 ...
	 Unilite.defineModel('${PKGNAME}.UserDefinePopupModel', {
	    fields: [{name: 'USER_DEFINE_CODE' 	,text:'' 				,type:'string'	},
				 {name: 'USER_DEFINE_NAME'	,text:'' 				,type:'string'	},
...
Ext.define('${PKGNAME}', {
    ...
    me.masterGrid = Ext.create('Unilite.com.grid.UniGridPanel',{
            store: Unilite.createStoreSimple('${PKGNAME}.userDefinePopupStore',{
							model: '${PKGNAME}.UserDefinePopupModel',
					        autoLoad: false,
					        proxy: {
					            type: 'direct',
					            api: {
					            	read: 'popupService.userDefinePopup'
					            }
					        }
					}),
	        uniOpt:{
	        ...
	        selModel:'rowmodel',
	        columns:  [        
	                     { dataIndex: 'USER_DEFINE_CODE' 	,  width: 180 }
	                    ,{ dataIndex: 'USER_DEFINE_NAME'	,  width: 200 }
	                    ,{ dataIndex: 'REF_CODE1' 		,  width: 130 }
	                    ,{ dataIndex: 'REF_CODE2' 		,  width: 130 }
	        ],
			listeners: {
	...
	fnInitBinding : function(param) {
		...
		if( Ext.isDefined(param)) {
			if(Ext.isDefined(fieldTxt) && fieldTxt.isFormField) {
				if(param['TYPE'] == 'VALUE') {
					fieldTxt.setValue(param['USER_DEFINE_CODE']);
					rdo.setValue({ RDO : '1'});
				} else {
					fieldTxt.setValue(param['USER_DEFINE_NAME']);
					rdo.setValue({ RDO : '2'});
				}
			}
			me.panelSearch.setValues(param);
		}
		me.masterGrid.getColumn('USER_DEFINE_CODE').setText(param.HEADER + '<t:message code="system.label.common.code" default="코드"/>');
		me.masterGrid.getColumn('USER_DEFINE_NAME').setText(param.HEADER + '<t:message code="system.label.common.name" default="명"/>');
		this._dataLoad();
	},
