...
<%
request.setAttribute("PKGNAME","Unilite.app.popup.CostPopup");
%>
	...
	<t:ExtComboStore useScriptTag="false" comboType="BOR120" />							// 사업장 
	 ...
	 Unilite.defineModel('${PKGNAME}.CostPopupModel', {
	    fields: [{name: 'COST_CODE' 		,text:'<t:message code="system.label.common.periodcostcode" default="기간비용코드"/>' 	,type:'string'	},
				 {name: 'COST_NAME' 		,text:'<t:message code="system.label.common.periodcostname" default="기간비용"/>' 	,type:'string'	}
			]
	});
...
//                 }
    ...
    me.masterGrid = Ext.create('Unilite.com.grid.UniGridPanel',{
            store: Unilite.createStoreSimple('${PKGNAME}.custPopupStore',{
							model: '${PKGNAME}.CostPopupModel',
					        autoLoad: false,
					        proxy: {
					            type: 'direct',
					            api: {
					            	read: 'popupService.costPopup'
					            }
					        }
					}),
	        uniOpt:{
	        ...
	        selModel:'rowmodel',
	        columns:  [        
	                     { dataIndex: 'COST_CODE'	 ,  width: 120 }
	                    ,{ dataIndex: 'COST_NAME'	,  minWidth: 380, flex: 1 }	            
	        ],
			listeners: {
	...
	fnInitBinding : function(param) {
		...
		if(param['TYPE'] == 'VALUE') {
        	if(!Ext.isEmpty(param['COST_CODE'])){
        		fieldTxt.setValue(param['COST_CODE']);        	
        	}
        }else{
        	if(!Ext.isEmpty(param['COST_CODE'])){
        		fieldTxt.setValue(param['COST_CODE']);        	
        	}
        	if(!Ext.isEmpty(param['COST_NAME'])){
        		fieldTxt.setValue(param['COST_NAME']);
        	}
        }
