...
<%
	request.setAttribute("PKGNAME","Unilite.app.popup.LotPopupMulti_mit");
%>
	 ...
	 * @type
	 */
	Unilite.defineModel('${PKGNAME}.LotPopupMulti_mitModel', {
		fields: [
...
//					, tdAttrs: {style: 'border : 1px solid #ced9e7;'/*, align : 'center'*/}
			...
			items: [{
				...
				Unilite.popup('CUST',{
					...
					//20191120 멀티 LOT 선택 시 체크하기 위해서 추가
					fieldLabel	: 'ORDER_Q',
		...
		var masterGridConfig = {
			store: Unilite.createStoreSimple('${PKGNAME}.lotPopupMulti_mitMasterStore',{
				model	: '${PKGNAME}.LotPopupMulti_mitModel',
				autoLoad: false,
				proxy	: {
					type: 'uniDirect',
					api	: {
						read: 'popupService.lotNoPopup_mit'
					}
				}
			}),
			uniOpt:{
			...
			listeners: {
				onGridDblClick:function(grid, record, cellIndex, colName) {
					...
					if(1 > me.panelSearch.getValue('ORDER_Q')) {
						Unilite.messageBox('수주량보다 초과하여 지정할 수 없습니다.');
						return false;
					} else {
						me.returnData(rv);
					}
	...
	fnInitBinding : function(param) {
		...
		//20191120 멀티 LOT 선택 시 체크하기 위해서 추가
		me.panelSearch.setValue('ORDER_Q'		, param.ORDER_Q);
	...
	onSubmitButtonDown : function()	{
		...
		if(rvRecs.length > panelSearch.getValue('ORDER_Q')) {
			Unilite.messageBox('수주량보다 초과하여 지정할 수 없습니다.');
			return false;
		} else {
			me.returnData(rv);
