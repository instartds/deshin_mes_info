...
<%
// 품목팝업(대표모델)
request.setAttribute("PKGNAME","Unilite.app.popup.ItemGroupPopup");
%>
...
Unilite.defineModel('${PKGNAME}.ItemGroupPopupModel', {
fields: [ 	{name: 'ITEM_CODE'    		,text:'<t:message code="system.label.common.itemcode" default="품목코드"/>'	,type : 'string'} 
			...
			,{name: 'STOCK_UNIT'    	,text:'<t:message code="system.label.common.inventoryprice" default="재고단가"/>'	,type : 'string'} 					
		]
});
...
Ext.define('${PKGNAME}', {
    ...
    constructor : function(config) {
		...
		me.panelSearch = Unilite.createSearchForm('',{
		    ...
		    items: [    { fieldLabel: '<t:message code="system.label.common.account" default="계정"/>' ,   name:'ITEM_ACCOUNT'  , xtype: 'uniCombobox' , comboType:'AU' ,comboCode:'B020' , value:'01'}		               
 ...
 */
	    	...
	    	store: Unilite.createStoreSimple('${PKGNAME}.itemGroupPopupMasterStore',{
						model: '${PKGNAME}.ItemGroupPopupModel',
					    autoLoad: false,
					    proxy: {
					        type: 'direct',
					        api: {
					        	read: 'popupService.itemGroupPopup'
					            }
					        }
					}),
	    	uniOpt:{
		...
		if(Ext.isDefined(wParam)) {		
			if(wParam['SELMODEL'] == 'MULTI') {
				masterGrid.selModel = Ext.create("Ext.selection.CheckboxModel", { checkOnly : false });
			}
		}
    },
