...
<%
request.setAttribute("PKGNAME","Unilite.app.popup.ManagePopup");
%>
	...
	<t:ExtComboStore useScriptTag="false" comboType="BOR120" />							// 사업장 
	 ...
	 Unilite.defineModel('${PKGNAME}.ManagePopupModel', {
	    fields: [{name: 'MANAGE_CODE' 		,text:'<t:message code="system.label.common.manageitemcode" default="관리항목코드"/>' 	,type:'string'	},
				 {name: 'MANAGE_NAME'	 	,text:'<t:message code="system.label.common.manageitem" default="관리항목"/>' 		,type:'string'	},
				 {name: 'DATA_TYPE'	 		,text:'<t:message code="system.label.common.type" default="유형"/>' 			,type:'string'	},
				 {name: 'DATA_FORMAT'	 	,text:'<t:message code="system.label.common.format" default="포멧"/>' 			,type:'string', comboType:'AU', comboCode:'A147'},
...
//                 }                
    ...
    me.masterGrid = Ext.create('Unilite.com.grid.UniGridPanel',{
            store: Unilite.createStoreSimple('${PKGNAME}.custPopupStore',{
							model: '${PKGNAME}.ManagePopupModel',
					        autoLoad: false,
					        proxy: {
					            type: 'direct',
					            api: {
					            	read: 'popupService.managePopup'
					            }
					        }
					}),
	        uniOpt:{
	        ...
	        selModel:'rowmodel',
	        columns:  [        
	                     { dataIndex: 'MANAGE_CODE' 		,  width: 100 }
	                    ,{ dataIndex: 'MANAGE_NAME'	 		,  width: 180 }
	                    ,{ dataIndex: 'DATA_TYPE'	 		,  width: 80 }
	                    ,{ dataIndex: 'DATA_FORMAT'	 		,  width: 80 }
	                    ,{ dataIndex: 'DATA_LENGTH'	 		,  width: 80 }
	                    ,{ dataIndex: 'POPUP_YN'	 		,  minWidth: 80, flex: 1 }
//	                    ,{ dataIndex: 'USE_YN'	 			,  width: 80 }
	...
	fnInitBinding : function(param) {
		...
		if(param['TYPE'] == 'VALUE') {
        	...
        	if(!Ext.isEmpty(param['MANAGE_NAME'])){
        		fieldTxt.setValue(param['MANAGE_NAME']);
        	}
        }
//		var frm= me.panelSearch.getForm();		
