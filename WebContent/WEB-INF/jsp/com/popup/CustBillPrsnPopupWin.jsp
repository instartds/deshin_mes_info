...
<%
// 전자세금 계산서 담당자 팝업
request.setAttribute("PKGNAME","Unilite.app.popup.CustBillPrsnPopupWin");
%>
/**
 *   Model 정의 
 * @type 
...
Unilite.defineModel('${PKGNAME}.CustBillPrsnPopupModel', {
    fields: [ 	
...
Ext.define('${PKGNAME}', {
    ...
    constructor : function(config) {
		...
		me.panelSearch = Unilite.createSearchForm('',{
		    ...
		    items: [  
    			...
    			{ fieldLabel: '<t:message code="system.label.common.invoiceclass" default="계산서구분"/>', name:'BILL_TYPE', hidden:true} ,
		 ...
		 */
		 me.masterGrid = Unilite.createGrid('', {
			store: Unilite.createStoreSimple('${PKGNAME}.custBillPrsnPopupMasterStore',{
								model: '${PKGNAME}.CustBillPrsnPopupModel',
						        autoLoad: false,
						        proxy: {
						                type: 'direct',
						                api: {
	...
	fnInitBinding : function(param) {
		...
		var rdo = frm.getForm().findField('RDO');
		console.log("param:", param);
		if( Ext.isDefined(param)) {
			frm.setValue('SEARCH_TEXT', param['PRSN_NAME']);
			frm.setValue('CUSTOM_CODE', param['CUSTOM_CODE']);
			frm.setValue('SEARCH_TYPE', param['SEARCH_TYPE']);
			frm.setValue('BILL_TYPE', param['BILL_TYPE']);
			frm.setValue('ADD_QUERY', param['ADD_QUERY']);
		}
		frm.setValues(param);
		this._dataLoad();
	},
