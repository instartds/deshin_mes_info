...
<t:appConfig pgmId="MainPortalYSU"  >
...
<script type="text/javascript" charset="UTF-8" src='<c:url value="/api.js?group=com,main" />'></script>
...
//                            }
       ...
       //도서검색추가
         modules.push({
                xtype: 'uniImg',
                src: CPATH+'/resources/css/theme_01/module_booksearch_over'+CUR_LANG_SUFFIX+".png",
                //title: '<t:message code="main.menuTab.mymenu" />',
                width: moduleWidth,
                height: moduleHeight,
                margin: '40 20 0 20',
                listeners: {
                    el: {
                        click: function() {
                            window.open(CPATH+'/bookshop/searchBooks.do','_blank','fullscreen=yes')
                        },
			            mouseover: function(e, elem, eOpts) {
			            	elem.src = CPATH+'/resources/css/theme_01/module_booksearch'+CUR_LANG_SUFFIX+".png";
			            },
			            mouseout: function(e, elem, eOpts) {
			            	elem.src = CPATH+'/resources/css/theme_01/module_booksearch_over'+CUR_LANG_SUFFIX+".png";
			            }
                    }
                }
        });
		...
		function moduleChange(id, newTitle) {
	       ...
	       /*
            ...
            if( id != MODULE_PROCESS.ID && id != MODULE_MYMENU.ID ) {
	            ...
	            //2015.05.08 이전 클릭된 모듈메뉴가 로드되기전 변경되지 않되록 수정
            	if(leftSystemMenuB.getStore().inLoading)	{
		...
		var treeSystemMenuStore = Ext.create('Ext.data.TreeStore',{
	        ...
	        treeLoadRecords:function(param, newTitle)	{
	        	...
	        	if(!this.inLoading)	{
	        		...
	        		alert("이전 모듈 메뉴 로딩중입니다. 다시 시도하세요.")
	        	}
	        },
	        listeners: {
	        <c:choose>
				...
				urlclick : function(rec, url, item) {
					if(url && rec.get('viewYN')) {
						if (typeof url !== "undefined") {
							openTab(rec, url+"?authoUser="+(rec.get("authoUser")=="null" ? '':rec.get("authoUser")));
						} else {
...
//			},
		...
		var contentTabPanel = new Ext.TabPanel({
			...
			listeners: {
            	tabchange: function( tabPanel, tab, oldTab ) {
					...
					if(Ext.isFunction(tab.getWin))	{
						var tabBody = tab.getWin();
						if(tabBody && tabBody.UniAppManager && tabBody.UniAppManager.app) {
							var grd = tabBody.UniAppManager.app.down("uniGridPanel");
							if(Ext.isDefined(grd) && grd != null) {
								if(grd && Ext.isFunction(grd.fireEvent)) {
									grd.fireEvent('show', grd);
								}
							}
						}
					}
		...
		var panelNorth =  <%@include file="main_panelNorth_YSU.jsp" %>; // panelNorth
