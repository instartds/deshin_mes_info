...
<script type="text/javascript">
    var pgmData = ${pgmData};
    var pgmList = createPgmList(pgmData);
    function createPgmList(pgmData) {
        var list = [];
        Ext.each(pgmData, function(item, index){
            list.push(
                {"text":item.PGM_NAME,   "url":"/"+ item.LOCATION + "/" +item.PGM_ID +".do"    ,"prgID":item.PGM_ID, "viewYN":true,"authoUser":"N","leaf":true,"children":[]}
            )
        });
        return list;
    }
            ...
            items: [
                {
                    ...
                    margin: "8 0 0 35",
                    onClick:function() {
//                    	var iniUrl = '/g3erp/z_yp/w_bsa000skrv_yp.do'
//                        iniOpenTab(iniUrl);
...
//                    }
			...
			//메인 화면 초기 동작 제어
			panelNavigation.expand();
			var iniUrl = CPATH + '/z_yp/w_bsa000skrv_yp.do'
			iniOpenTab(iniUrl);

	});	// Ext.onReady();

	<%@include file="mainCommon_yp_ext.jsp" %>
