...
//			},
		...
		//tab2: portal, 20210916 주석: 조광웅 상무님 요청
...
//		tabContents.push(portal);
		...
		var contentTabPanel = new Ext.TabPanel({
			...
			region : 'center',
			padding: 0,
			maxTabNumber:30,	//	home(1) + portal(1) + program tab(28)
			id : 'contentTabPanel',
			plugins:[{
			...
			margins : '0 1 0 1',
			activeTab : 0,			//20210916 수정: 조광웅 상무님 요청 (포탈 탭 제외, 0 -> 1)
			flex:1,
			tabPosition : 'bottom',
			plain : true,
			closeAction: 'hide',
			...
			listeners: {
				...
				beforeadd:function( tabPanel , component , index , eOpts)	{
					if(tabPanel.items.length  >= tabPanel.maxTabNumber  )	{
						component.destroy();
						Unilite.messageBox('<t:message code="system.message.common.main006" default="Tab은 최대 ' + String(tabPanel.maxTabNumber) + '개까지만 가능하니 다른 Tab을 닫고 실행해주세요."/>');
						return false
					}
				}
			}
