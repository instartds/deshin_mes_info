...
<link rel="stylesheet" type="text/css" href='<c:url value="/resources/css/joinsPortal.css" />' />
...
<c:when test="${isDevelopServer }">
	...
	<script type="text/javascript" charset="UTF-8" src='<c:url value="/${ext_root}/app/Unilite/main/portal/MainPortalJoins.js" />' ></script>
...
<script type="text/javascript" charset="UTF-8" src='<c:url value="/${ext_root}/locale/${lang_prefix}-${CUR_LANG}.js" />' ></script>
...
//				            	}
				            ...
				            mouseover: function(e, elem, eOpts) {
				            	elem.src = imgSrc;//CPATH+'/resources/css/theme_01/module_'+m.id+CUR_LANG_SUFFIX+".png";
				            },
				            mouseout: function(e, elem, eOpts) {
				            	elem.src = imgOverSrc;//CPATH+'/resources/css/theme_01/module_'+m.id+'_over'+CUR_LANG_SUFFIX+".png";
				            }
				        }
				    }
				};
...
//			},
		...
		tabContents.push(<%@include file="main_homeContentsMis.jsp" %>);
			...
			listeners: {
            	...
            	beforeadd:function( tabPanel , component , index , eOpts)	{
            		if(tabPanel.items.length  >= tabPanel.maxTabNumber  )	{
            			component.destroy();
            			Ext.Msg.alert('확인', 'Tab은 최대 10개까지만 가능하니 다른 Tab을 닫고 실행해주세요.');
            			return false
            		}
            	}
			}
		...
		var panelNorth =  <%@include file="main_panelNorth_mis.jsp" %>; // panelNorth
...
//                    }
	...
	<%@include file="mainMisCommon.jsp" %>
...
<body id="ext-body" style="margin: 0; padding: 0;">
    <div id="home_notice" style="display:none;" >
	</div>
	<iframe src="about:blank" id="connectF" name="connectF" width="0" height="0" frameborder="0"></iframe>
</body>
</html>