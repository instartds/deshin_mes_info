...
function appMain() {
	 ...
	 * @type
	 */
	var masterForm = Unilite.createForm('bsa310ukrvMaster', {
		border	: true,
		region	: 'north',
		disabled: false,
		layout	: 'vbox',
		items	: [{
			...
			xtype	: 'container',
			layout	: {type: 'uniTable', columns: 2,tdAttrs: {valign:'center'}},
			defaults: {labelWidth: 100, margin:'15 0 15 20', width: 800},
			items	: [{
				xtype	: 'container',
				layout	: 'hbox',
	...
	var detailForm = Unilite.createForm('bsa310ukrvDetail', {
		...
		items	: [{
			...
			style	: {
				...
				xtype	: 'container',
				html	: '<b>※'+'<t:message code="system.label.base.lately" default="최근"/> '+BsaCodeInfo.numberPast+'<t:message code="system.label.base.passexplanation9" default="개의 비밀번호를 교대로 사용금지"/>'+'</b>',
				style	: {
					color: 'blue'
				},
				margin	: '0 0 7 20'
				...
				xtype	: 'container',
				html	: '<b>※'+'<t:message code="system.label.base.passexplanation10" default="비밀번호에 생일, 전화번호 끝 4자리 포함금지"/>'+'</b>',
				style	: {
					color: 'blue'
				},
				margin	: '0 0 7 20'
	...
	Unilite.Main({
		...
		fnSetErorrText : function(checkId, checkVal) {
			...
			case "userTelCheck":
				if(checkVal) {
					resultMsg = '<t:message code="system.message.base.passconfirm4" default="부적합 - 비밀번호에 전화번호 뒷자리 4자 포함오류"/>';
					bError = true;
				}
				break;
...
//					provider['ENCRYPT_YN'] == "Y" ? param = {"NEW_PWD": newPwd} : param = {"NEW_PWD" : newPwd.toUpperCase()};
					...
					masterForm.setValue('NEW_CFM_PWD', '');
					Ext.getCmp('new_pwd_text').setValue('');
//					if(confirm('지금 보고 있는 웹 페이지 창을 닫으려고 합니다.' + '\n' + '이 창을 닫으시겠습니까?')){
//						var tabPanel = parent.Ext.getCmp('contentTabPanel');
//						if(tabPanel) {
//							var activeTab = tabPanel.getActiveTab();
//							var canClose = activeTab.onClose(activeTab);
