...
<t:appConfig pgmId="bpr560ukrv"  >

	<t:ExtComboStore comboType="BOR120"  pgmId="bpr560ukrv"/>					<!-- 사업장 -->
...
function appMain() {
	...
	var masterSelectedGrid = 'bpr560ukrvGrid1';  // Grid1 createRow Default
	...
	var bprYnStore = Unilite.createStore('bpr560ukrvBPRYnStore', {
		fields	: ['text', 'value'],
		data	: [
	...
	var basisQtyStore = Unilite.createStore('bpr560ukrvBasisQtyStore', {
		fields	: ['text', 'value'],
		data	: [
			{'text':'1'		, 'value':'1'},
			{'text':'100'	, 'value':'100'}
	...
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
		api: {
			read	: 'bpr560ukrvService.selectList2',
			update	: 'bpr560ukrvService.updateDetail2',
			create	: 'bpr560ukrvService.insertDetail2',
			destroy	: 'bpr560ukrvService.deleteDetailGRID2',
			syncAll	: 'bpr560ukrvService.saveAll'
		}
	});
	...
	var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
		api: {
			read	: 'bpr560ukrvService.selectList3',
			update	: 'bpr560ukrvService.updateDetailGRID3',
			create	: 'bpr560ukrvService.insertDetail3',
			destroy	: 'bpr560ukrvService.deleteDetailGRID3',
			syncAll	: 'bpr560ukrvService.saveAll'
		}
	});
	...
	Unilite.defineModel('Bpr560ukrvModel2', {
		fields: [
			...
			{name: 'USE_YN'				,text: '<t:message code="system.label.base.use" default="사용"/>'						,type: 'string'  , defaultValue:'1' ,store: Ext.data.StoreManager.lookup('bpr560ukrvBPRYnStore')},
...
//			{name: 'SAMPLE_KEY'			,text: 'SAMPLE_KEY'	,type: 'string'}
	...
	Unilite.defineModel('Bpr560ukrvModel3', {
		fields: [
			...
			{name: 'MAN_HOUR'			,text: '<t:message code="system.label.base.standardtacttime" default="표준공수"/>'		,type: 'uniQty', defaultValue:0},
			{name: 'USE_YN'				,text: '<t:message code="system.label.base.use" default="사용"/>'						,type: 'string' , defaultValue:'1',store: Ext.data.StoreManager.lookup('bpr560ukrvBPRYnStore')},
			{name: 'BOM_YN'				,text: '<t:message code="system.label.base.compyn" default="구성여부"/>'				,type: 'string' , defaultValue:'1',store: Ext.data.StoreManager.lookup('bpr560ukrvBPRYnStore')},
	...
	Unilite.Excel.defineModel('excel.bpr560ukrv.sheet01', {
		fields: [
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('bpr560ukrvMasterStore1',{
...
//							Ext.getCmp('LAB_NO_POPUP').setDisabled(true);
				...
				var grid = Ext.getCmp('bpr560ukrvGrid1');
...
//					Ext.getCmp('LAB_NO_POPUP').setDisabled(false);
	...
	var directMasterStore2 = Unilite.createStore('bpr560ukrvMasterStore2',{
...
//			if(!rv) return false;
	...
	var directMasterStore3 = Unilite.createStore('bpr560ukrvMasterStore3',{
	...
	var makeProdItemBtn = Ext.create('Ext.Button',{
		...
		handler	: function()  {
			if (confirm('<t:message code="system.message.base.message008" default="모품목에 등록되지 않은 제품/반제품을 일괄등록하시겠습니까?"/>')) {
				bpr560ukrvService.makeProdItems({'DIV_CODE': panelResult.getValue('DIV_CODE')}, function(provider, response) {
...
//				padding: '0 20 0 0',
	...
	var masterGrid1 = Unilite.createGrid('bpr560ukrvGrid1', {
		// for tab
		layout : 'fit',
		region:'west',
		flex: .3,
...
//							  extParam: {DIV_CODE: UserInfo.divCode, ITEM_ACCOUNT_FILTER:['10','20'], ITEM_EXCLUDE:'DIV_CODE' ,DEFAULT_ITEM_ACCOUNT:'10'},
	...
	var masterGrid2 = Unilite.createGrid('bpr560ukrvGrid2', {
		layout : 'fit',
		region:'north',
		split: true,
		flex: .7,
		store: directMasterStore2,
		itemId:'bpr560ukrvGrid2',
...
//			'-',
			{	name: 'BASIS_QTY' ,
					...
					xtype:'uniCombobox',
					store: Ext.data.StoreManager.lookup('bpr560ukrvBasisQtyStore'),
					id:'BASIS_QTY_COMBO',
					labelWidth  : 53,
					width	   : 180,
...
//			if(!grdRecord || !Ext.isDefined(grdRecord)) grdRecord = this.getSelectedRecord();
	...
	var masterGrid3 = Unilite.createGrid('bpr560ukrvGrid3', {
		// for tab
		layout : 'fit',
		region:'center',
		split: true,
				...
				if(Ext.isEmpty(records2)){
					...
					return false;
					 ...
					 if(confirm("자품목코드와 대체품목코드를 교환하시겠습니까?")) {
						 ...
						 Ext.getBody().mask();
						 	bpr560ukrvService.changeItemData(param,function(provider, response){
		...
		if(!excelWindow) {
			...
			excelWindow = Ext.create( appName, {
				...
				grids: [{							//팝업창에서 가져오는 그리드
						...
						title		: 'BOM정보 엑셀업로드',
						useCheckbox	: false,
						model		: 'excel.bpr560ukrv.sheet01',
...
//			Ext.getCmp('LAB_NO_POPUP').setDisabled(true);
			...
			masterSelectedGrid = 'bpr560ukrvGrid1';
		...
		onNewDataButtonDown: function() {
			if(masterSelectedGrid == 'bpr560ukrvGrid1'){
			...
			else if(masterSelectedGrid == 'bpr560ukrvGrid2'){
...
//						SAMPLE_KEY		: sampleKey
			...
			else if(masterSelectedGrid == 'bpr560ukrvGrid3'){
...
//		  }
		...
		onDeleteDataButtonDown: function() {
			if(masterSelectedGrid == 'bpr560ukrvGrid1'){
				var Grid1 = UniAppManager.app.down('#bpr560ukrvGrid1');
				...
				var Grid1 = UniAppManager.app.down('#bpr560ukrvGrid2');
				...
				var Grid1 = UniAppManager.app.down('#bpr560ukrvGrid3');
		...
		rejectSave: function() {	// 저장
			if(masterSelectedGrid == 'bpr560ukrvGrid2'){
		...
		confirmSaveData: function(config)   {
			var fp = Ext.getCmp('bpr560ukrvFileUploadPanel');
