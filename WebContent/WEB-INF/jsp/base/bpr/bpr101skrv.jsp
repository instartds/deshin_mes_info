...
<t:ExtComboStore items="${COMBO_ITEM_LEVEL1}" storeId="bpr101skrvLevel1Store" />
<t:ExtComboStore items="${COMBO_ITEM_LEVEL2}" storeId="bpr101skrvLevel2Store" />
<t:ExtComboStore items="${COMBO_ITEM_LEVEL3}" storeId="bpr101skrvLevel3Store" />
</t:appConfig>
...
//	var isAllowBasisPriceChange = true;
	...
	Unilite.defineModel('bpr101skrvModel', {
		fields: [	 //BPR100T필수
				    ...
				    ,{ name: 'ITEM_LEVEL1',  			text: '<t:message code="system.label.base.majorgroup" default="대분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr101skrvLevel1Store'), child:'ITEM_LEVEL2'}        					       
				    ,{ name: 'ITEM_LEVEL2',  			text: '<t:message code="system.label.base.middlegroup" default="중분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr101skrvLevel2Store'), child:'ITEM_LEVEL3'}        
				    ,{ name: 'ITEM_LEVEL3',  			text: '<t:message code="system.label.base.minorgroup" default="소분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr101skrvLevel3Store'),parentNames:['ITEM_LEVEL1','ITEM_LEVEL2'], levelType:'ITEM'}
				    ...
				    ,{ name: 'SPEC_CONTROL_CODE',	  	text: '특정코드', 		type : 'string', maxLength:3 }				    
			  		...
			  		,{ name: 'IN_EXCESS_RATE',  		text: '<t:message code="system.label.base.overreceiptrate" default="과입고허용율"/>',  	type : 'uniPercent', defaultValue:0.00, maxLength: 3}
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'bpr101skrvService.selectDetailList',
			update: 'bpr101skrvService.updateDetail',
			create: 'bpr101skrvService.insertDetail',
			destroy: 'bpr101skrvService.deleteDetail',
			syncAll: 'bpr101skrvService.saveAll'
		}
	});
	var directMasterStore = Unilite.createStore('bpr101skrvMasterStore',{
			model: 'bpr101skrvModel',
           	autoLoad: false,
        	uniOpt : {
        	...
        	,loadStoreRecords : function()	{
				...
				bpr101skrvService.checkItemCode(checkParam, function(provider1, response)	{
...
//				var paramMaster= [];
//				var app = Ext.getCmp('bpr101skrvApp');
...
//							params: [paramMaster],
							success: function(batch, option) {
								...
								bpr101skrvService.goInterFace(param, function(provider, response)	{
								});
...
//					}
	...
	Unilite.defineModel('bpr101skrvSubModel', {
	...
	var directSubProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'bpr101skrvService.selectSubDetailList',
			update: 'bpr101skrvService.updateSubDetail',
			create: 'bpr101skrvService.insertSubDetail',
			destroy: 'bpr101skrvService.deleteSubDetail',
			syncAll: 'bpr101skrvService.subSaveAll'
		}
	});
	
	var directSubStore = Unilite.createStore('bpr101skrvSubStore', {
		model: 'bpr101skrvSubModel',
        autoLoad: false,
        uniOpt : {
...
//	    		detailForm.getField('SALE_COMMON_P').setReadOnly(false);
	 ...
	 * 검색 Form
	 */
	var sortSeqStore = Unilite.createStore('bpr101skrvSeqStore', {
	    fields: ['text', 'value'],
		data :  [
		...
		items: [{     
        	...
        	layout: {type: 'uniTable', columns: 1},
		 	items: [{
    			...
    			xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr101skrvLevel1Store'),
                child: 'ITEM_LEVEL2'
              }, {
              	name: 'ITEM_LEVEL2',
              	fieldLabel: '<t:message code="system.label.base.middlegroup" default="중분류"/>',
              	xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr101skrvLevel2Store'),
                child: 'ITEM_LEVEL3'
             	...
             	xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr101skrvLevel3Store'),
     ...
     * Master Grid
     */
	 var masterGrid = Unilite.createGrid('bpr101skrvGrid', {  
    	region:'center',
...
//					detailForm.getField('SALE_BASIS_P').setReadOnly(true);
    ...
    /*상세폼에  BPR400T그리드*/
    var subGrid = Unilite.createGrid('bpr101skrvsubGrid', {    	
    	//region:'center',
...
//					var orderUnit = record.get('ORDER_UNIT');
    ...
    var stockSearchForm = Unilite.createSearchForm('bpr101skrvstockSearchForm', {        
            layout :  {type : 'uniTable', columns : 1},
            items :[{
				name: 'DIV_CODE',
    ...
    //창고별재고 조회 모델
	Unilite.defineModel('bpr101skrvstockSearchModel', {
	...
	//창고별재고 조회 스토어
    var stockSearchStore = Unilite.createStore('bpr101skrvstockSearchStore', {
			model: 'bpr101skrvstockSearchModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'bpr101skrvService.selectStockList'
                }
            }
            ,loadStoreRecords : function()	{
				var param= {
	...
	//창고별재고 조회 그리드 
    var stockSearchGrid = Unilite.createGrid('bpr101skrvstockSearchGrid', {
        // title: '기본',
        layout : 'fit',
...
//		disabled:false,
	    ...
	    items : [{
	    		...
	    		{ 
        			...
        			, layout: { type: 'uniTable', columns: 1}
        			, height: 435
        			, items :[	 { name: 'ITEM_LEVEL1',  			fieldLabel: '<t:message code="system.label.base.majorgroup" default="대분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr101skrvLevel1Store'), child: 'ITEM_LEVEL2'}   
						  		,{ name: 'ITEM_LEVEL2',  			fieldLabel: '<t:message code="system.label.base.middlegroup" default="중분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr101skrvLevel2Store'), child: 'ITEM_LEVEL3'}    
						  		,{ name: 'ITEM_LEVEL3',  			fieldLabel: '<t:message code="system.label.base.minorgroup" default="소분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr101skrvLevel3Store'), parentNames:['ITEM_LEVEL1','ITEM_LEVEL2'],levelType:'ITEM'}
...
//						  		,{ name: 'USE_YN',  				fieldLabel: '<t:message code="system.label.base.useyn" default="사용여부"/>', 	xtype:'uniRadiogroup', comboType:'AU', comboCode:'B010', width:235, allowBlank:false}
	    	   ...
	    	   ,{   title: '제원정보'
        			...
        			, defaults: {type: 'uniTextfield', colspan: 2, enforceMaxLength: true, readOnly: true}
        			, height: 435
        			, width: 300
...
//							    ,{ name: 'SPEC_CONTROL',  	 		fieldLabel: '특정', 		xtype : 'uniRadiogroup', comboType:'AU', comboCode:'B010', width:235, allowBlank:false}
							    ...
							    ,{xtype: 'displayfield', name: ''}
								,{ name: 'SALE_UNIT',  				fieldLabel: '<t:message code="system.label.base.salesunit" default="판매단위"/>',	xtype:'uniCombobox',	comboType:'AU', comboCode:'B013' , displayField: 'value', fieldStyle: 'text-align: center;', allowBlank:false}							    
...
//        					 ,{xtype: 'uniCombobox', fieldLabel: '진열대번호', name: 'BIN_NUM', comboType:'AU', comboCode:'YP02', maxLength :10 }
							...
							Unilite.popup('BIN', { 
        					 ...
        					 ,{ name: 'IN_EXCESS_RATE',  			fieldLabel: '<t:message code="system.label.base.overreceiptrate" default="과입고허용율"/>',	xtype : 'uniNumberfield',  decimalPrecision:'2', maxLength :5}
...
//        					{ name: 'CIR_PERIOD_YN',  			fieldLabel: '유효기한관리', 	 xtype : 'uniRadiogroup', comboType:'AU', comboCode:'B010', width:260, allowBlank:false, padding: '8 0 0 0'}
        					   ...
        					   ,{ name: 'USE_BY_DATE',  			fieldLabel: '<t:message code="system.label.base.availabledate" default="유효일"/>',		 xtype:'uniNumberfield',value:'${UseByDate}', hidden: true, maxLength: 10}
//        					   ,{xtype: 'displayfield', name: ''}
...
//									SALES_TYPE : ,						
								...
								bpr101skrvService.updateSaleBasisP(param, function(provider, response)	{
	...
	Unilite.Excel.defineModel('excel.bpr101.sheet01', {
	    fields: [
			...
			{name: 'USE_YN' 		 		 ,text:'<t:message code="system.label.base.inventorymanageobject" default="재고관리대상"/>'	,type: 'string', comboType:'AU', comboCode:'B010'},			 
            ...
            if(!excelWindow) {
                ...
                excelWindow = Ext.create( appName, {
                        ...
                        grids: [{
                        		...
                        		readApi: 'bpr101skrvService.selectExcelUploadSheet1',
                        		columns: [
...
//			}
			...
			if(detailForm.getValue('ITEM_ACCOUNT') == "02" && Ext.isEmpty(detailForm.getValue('CONSIGNMENT_FEE'))){
				Unilite.messageBox('위탁수수료를 입력해 주세요.');
				detailForm.getField('CONSIGNMENT_FEE').focus();
				return false;
			}
			
			if(!Ext.isEmpty(detailForm.getValue('BUY_BIG_BOX_BARCODE')) && detailForm.getValue('BUY_RATE') <= 1){
		...
		processParams: function(params) {
			...
			if(params && params.ITEM_CODE) {
				...
				bpr101skrvService.checkItemDuplicate(param, function(provider1, response)	{							
