...
<t:ExtComboStore items="${COMBO_ITEM_LEVEL1}" storeId="bpr102ukrvLevel1Store" />
<t:ExtComboStore items="${COMBO_ITEM_LEVEL2}" storeId="bpr102ukrvLevel2Store" />
<t:ExtComboStore items="${COMBO_ITEM_LEVEL3}" storeId="bpr102ukrvLevel3Store" />
</t:appConfig>
		...
		initComponent: function() {
	    	...
	    	var searchStore  = Ext.create('Ext.data.Store',{
	            ...
	            proxy: {
	                type: 'direct',
	                api: {
	                    read : 'bpr102ukrvService.searchMenu'
	                },
		            reader: {
		                type: 'json',
				<c:choose>
...
//		    console.log(store.getProxy().getCurrentConfig());
	...
	Unilite.defineModel('bpr102ukrvModel', {
		fields: [	 //BPR100T필수
				    ...
				    ,{ name: 'ITEM_LEVEL1',  			text: '<t:message code="system.label.base.majorgroup" default="대분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel1Store'), child:'ITEM_LEVEL2'}        					       
				    ,{ name: 'ITEM_LEVEL2',  			text: '<t:message code="system.label.base.middlegroup" default="중분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel2Store'), child:'ITEM_LEVEL3'}        
				    ,{ name: 'ITEM_LEVEL3',  			text: '<t:message code="system.label.base.minorgroup" default="소분류"/>', 		type : 'string', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel3Store'),parentNames:['ITEM_LEVEL1','ITEM_LEVEL2'], levelType:'ITEM'}
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			...
			update: 'bpr102ukrvService.updateDetail',
			create: 'bpr102ukrvService.insertDetail',
			destroy: 'bpr102ukrvService.deleteDetail',
			syncAll: 'bpr102ukrvService.saveAll'
		}
	});
	var directMasterStore = Unilite.createStore('bpr102ukrvMasterStore',{
			model: 'bpr102ukrvModel',
           	autoLoad: false,
        	uniOpt : {
...
//				var paramMaster= [];
//				var app = Ext.getCmp('bpr102ukrvApp');
            	...
            	Ext.each(list, function(record, index) {
            		if(isNaN(record.get('ITEM_CODE'))){
						Unilite.messageBox('도서코드는 숫자만 입력 가능합니다.');
...
//							params: [paramMaster],
							success: function(batch, option) {
								...
								bpr102ukrvService.goInterFace(param, function(provider, response)	{
								});
...
//					}
	...
	Unilite.defineModel('bpr102ukrvSubModel', {
	...
	var directSubProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'bpr102ukrvService.selectSubDetailList',
			update: 'bpr102ukrvService.updateSubDetail',
			create: 'bpr102ukrvService.insertSubDetail',
			destroy: 'bpr102ukrvService.deleteSubDetail',
			syncAll: 'bpr102ukrvService.subSaveAll'
		}
	});
	
	var directSubStore = Unilite.createStore('bpr102ukrvSubStore', {
		model: 'bpr102ukrvSubModel',
        autoLoad: false,
        uniOpt : {
...
//			    	UniAppManager.updateStatus(msg, true);	
	 ...
	 * 검색 Form
	 */
	var sortSeqStore = Unilite.createStore('bpr102ukrvSeqStore', {
	    fields: ['text', 'value'],
		data :  [
		...
		items: [{     
        	...
        	layout: {type: 'uniTable', columns: 1},
		 	items: [{
    			...
    			xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr102ukrvLevel1Store'),
                child: 'ITEM_LEVEL2'
              }, {
              	name: 'ITEM_LEVEL2',
              	fieldLabel: '<t:message code="system.label.base.middlegroup" default="중분류"/>',
              	xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr102ukrvLevel2Store'),
                child: 'ITEM_LEVEL3'
             	...
             	xtype:'uniCombobox',
                store: Ext.data.StoreManager.lookup('bpr102ukrvLevel3Store'),
...
//				}				
    ...
    /*상세폼에  BPR400T그리드*/
    var subGrid = Unilite.createGrid('bpr102ukrvsubGrid', {    	
    	//region:'center',
...
//	    				}
    ...
    //창고별재고 조회 모델
	Unilite.defineModel('bpr102ukrvstockSearchModel', {
	...
	//창고별재고 조회 스토어
    var stockSearchStore = Unilite.createStore('bpr102ukrvstockSearchStore', {
			model: 'bpr102ukrvstockSearchModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'bpr102ukrvService.selectStockList'
                }
            }
            ,loadStoreRecords : function()	{
				var param= {
	...
	//창고별재고 조회 그리드 
    var stockSearchGrid = Unilite.createGrid('bpr102ukrvstockSearchGrid', {
        // title: '기본',
        layout : 'fit',
...
//		    					detailForm.setValue('ITEM_NAME2', newValue);
	    		...
	    		{ 
        			...
        			, layout: { type: 'uniTable', columns: 1}
        			, height: 350
        			, items :[	 { name: 'ITEM_LEVEL1',  			fieldLabel: '<t:message code="system.label.base.majorgroup" default="대분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel1Store'), child: 'ITEM_LEVEL2'}   
						  		,{ name: 'ITEM_LEVEL2',  			fieldLabel: '<t:message code="system.label.base.middlegroup" default="중분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel2Store'), child: 'ITEM_LEVEL3'}    
						  		,{ name: 'ITEM_LEVEL3',  			fieldLabel: '<t:message code="system.label.base.minorgroup" default="소분류"/>' 		,xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('bpr102ukrvLevel3Store'), parentNames:['ITEM_LEVEL1','ITEM_LEVEL2'],levelType:'ITEM'}
			...
			getBookInfoWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {beforehide: function(me, eOpt)	{
                			 ...
                			 show: function( panel, eOpts )	{                			 	
                			 	...
                			 	bpr102ukrvService.setBookInfo(param, function(provider1, response)	{							
...
//				detailForm.clearForm();		
		...
		processParams: function(params) {
			...
			if(params && params.ITEM_CODE) {
				...
				bpr102ukrvService.checkItemDuplicate(param, function(provider1, response)	{							
        	 ...
        	 params: {
				...
				UniNaverSearch.searchBookAdv(params, function(naver, item, result) {
		    		...
		    		if(result && item){
						...
						rtnRecord.set('ISBN_CODE', '');
						rtnRecord.set('ITEM_NAME', '');
						rtnRecord.set('ITEM_NAME1', '');
						rtnRecord.set('PUBLISHER', '');
						rtnRecord.set('AUTHOR1', '');
						rtnRecord.set('PUB_DATE',  '');
						rtnRecord.set('SALE_COMMON_P', '');
						rtnRecord.set('SALE_BASIS_P', '');
						bookImage.setSrc('');
					}					
//					var bookLink = detailForm.down('#bookLink');
...
//					bookLink.setValue('<a href="javascript:void(0);" onclick="javascript:'+naver.getLinkScript(item.link)+';"><u>책소개</u></a>')
        ...
        fnGetbookLink: function(isbn){
        	 params: {
				...
				UniNaverSearch.searchBookAdv(params, function(naver, item, result) {
					...
					}else if(item && !Ext.isArray(item))	{
						naver.openLink(item.link);
					}
				});
        	 }      
        }, 
        fnSetBookInfo: function(item){
			...
			var title = '';
			var author = '';
			if(!Ext.isEmpty(item.isbn)){
       			isbn = item.isbn.substring(0,10);
      		 }	
	       if(!Ext.isEmpty(item.title)){
	       		title = item.title.substring(0,100);
	       }
	       if(!Ext.isEmpty(item.author)){
			...
			if(fieldName == "PURCHASE_RATE" )	{	
					if(newValue < 0 ) {
						...
						if(!Ext.isEmpty(masterRecord.get('SALE_BASIS_P')) && masterRecord.get('SALE_BASIS_P') != 0){
							record.set('ITEM_P', Math.round(detailForm.getValue('SALE_BASIS_P') * (newValue / 100)) );
						}					
					}
