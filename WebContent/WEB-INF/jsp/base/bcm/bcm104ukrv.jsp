...
<t:appConfig pgmId="bcm104ukrv"  >
	...
	<t:ExtComboStore comboType="BOR120"  pgmId="bcm104ukrv" /> 			<!-- 사업장 -->  
...
function appMain() {
	...
	Unilite.defineModel('bcm104ukrvModel', {
...
//				 {name: '' 	  				  ,text:'대조자(업체)'		,type:'string'	}
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'bcm104ukrvService.selectDetailList',
			update: 'bcm104ukrvService.updateDetail',
			create: 'bcm104ukrvService.insertDetail',
			destroy: 'bcm104ukrvService.deleteDetail',
			syncAll: 'bcm104ukrvService.saveAll'
		}
	});	  
	 ...
	 * @type 
	 */					
	var directMasterStore = Unilite.createStore('bcm104ukrvMasterStore',{
			model: 'bcm104ukrvModel',
            autoLoad: false,
            uniOpt : {
...
//            	}
			...
			,loadStoreRecords : function()	{
				var param= Ext.getCmp('bcm104ukrvSearchForm').getValues();			
				console.log( param );
				this.load({
					params : param,
						callback : function(records, operation, success) {
							if(success)	{
								...
								if(Ext.isEmpty(record) && masterGrid.isHidden()){
									...
									bcm104ukrvService.getUnPayAmt(param, function(provider, response)	{
...
//				    }); 
	 ...
	 * @type 
	 */
	var panelSearch = Unilite.createSearchPanel('bcm104ukrvSearchForm',{
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('bcm104ukrvGrid', {
    	region:'center',
			  						...
			  						listeners: {
						                'onSelected': {
						                    fn: function(records, type  ){
						                    	var me = this;
						                    	var grdRecord = Ext.getCmp('bcm104ukrvGrid').uniOpt.currentRecord;
...
//								}) 		
         ...
         /* ,listeners: {	
          		selectionchange: function( grid, selected, eOpts ) {
          			...
          			//추가정보 Tab의 Data Load
          			Ext.getCmp('bcm104ukrvEBillDetail').setActiveRecord(selected[0] || null);  
  				}
...
//						width: 342
	    		...
	    		{ 
        			  title: '<t:message code="system.label.base.basisinfo2" default="기본정보"/>'
        			, defaults: {type: 'uniTextfield', enforceMaxLength: true, labelWidth: 140}
        			, layout: { type: 'uniTable', columns: 1}
        			, height: 520
        			, width: 370
        			, items :[{
						...
						listeners: {
							change: function(field, newValue, oldValue, eOpts) {
//								if(Ext.isEmpty(detailForm.getValue('CUST_LEVEL1'))){
		...
		//				   				}
	    	   ...
	    	   ,{   title: '업무정보'
        			...
        			, items :[{
	            		...
	            		xtype: 'radiogroup',
	            		fieldLabel: '세액계산순서',
	            		items : [{boxLabel  : '공급가',  name: 'TAX_CALC_ORDER', inputValue: '2', width:95 }
	                    		,{boxLabel  : '부가세', name: 'TAX_CALC_ORDER' , inputValue: '1'}
                    	]
                    },{
						fieldLabel: '원미만계산',
						name: 'WON_CALC_BAS',
...
//				},
	...
	//취급품목 조회 폼
	var purchaseItemSearchForm = Unilite.createSearchForm('bcm104ukrvPurchaseItemSearch', {
	...
	//취급품목 조회 모델
	Unilite.defineModel('bcm104ukrvpurchaseItemSearchModel', {
	    fields: [
	...
	//취급품목 조회 스토어
    var purchaseItemSearchStore = Unilite.createStore('bcm104ukrvpurchaseItemSearchStore', {
			model: 'bcm104ukrvpurchaseItemSearchModel',
            autoLoad: false,
            uniOpt : {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'bcm104ukrvService.selectPurchaseList'
                }
            }
            ,loadStoreRecords : function()	{
	...
	//취급품목 조회 그리드 
    var purchaseItemSearchGrid = Unilite.createGrid('bcm104ukrvpurchaseItemSearchGrid', {
        // title: '기본',
        layout : 'fit',
...
//                			 	purchaseItemSearchWindow.getEl().setXY([getWindowX,getWindowY]);
	...
	//현미지급금, 현재고금액 조회 모델
	Unilite.defineModel('bcm104ukrvUnPayAmtSearchModel', {
	    fields: [
	    	{name: 'COMP_CODE'				,text: '<t:message code="system.label.base.companycode" default="법인코드"/>'		,type: 'string'},			
	...
	//현미지급금, 현재고금액 조회 스토어
    var unPayAmtSearchStore = Unilite.createStore('bcm104ukrvUnPayAmtSearchStore', {
			model: 'bcm104ukrvUnPayAmtSearchModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 'bcm104ukrvService.getUnPayAmt'
                }
            }
            ,loadStoreRecords : function()	{
	...
	//현미지급금, 현재고금액 조회 그리드 
    var unPayAmtSearchGrid = Unilite.createGrid('bcm104ukrvunPayAmtSearchGrid', {
        // title: '기본',
        layout : 'fit',
...
//								});
		...
		,onSaveAsExcelButtonDown: function() {
			var masterGrid = Ext.getCmp('bcm104ukrvGrid');
		...
		onNewDataButtonDown : function()	{
        	 ...
        	 var r = {
				...
				VAT_RATE: vatRate,
				TRANS_CLOSE_DAY: transCloseDay,
				COLLECT_CARE: CollectCare,
				NATION_CODE: nationCode,
				MONEY_UNIT: moneyUni,
				WON_CALC_BAS: wonCalcBas,
				BILL_TYPE: billType
	        };
...
//			bcm104ukrvService.chkPK(param, function(provider, response)	{
//				if(!Ext.isEmpty(provider)){
//					Unilite.messageBox('')
//				}else{
...
//				}				
//			});			
			if(!Ext.isEmpty(detailForm.getValue('CUSTOM_CODE')) && detailForm.getValue('CUSTOM_CODE').length != 6){
				Unilite.messageBox('거래처코드 6자리를 입력해 주세요.');
...
//				if(!selRow.phantom && Ext.isEmpty(selRow.get('MODIFY_REASON'))){
//					Unilite.messageBox('변경사유를 입력해 주세요');
//				}else{
					masterGrid.deleteSelectedRow();
//				}				
