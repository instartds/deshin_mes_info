...
<%
//노무기간별 조회
request.setAttribute("PKGNAME","Unilite_app_gac300skrv");
%>
<t:appConfig pgmId="gac300skrv"  >
...
function appMain() {
	Unilite.defineModel('${PKGNAME}Model', {
	    ...
	    fields: [
			...
			{name: 'ROUTE_GROUP_NAME'       ,text:'노선그룹명'					,type : 'string'},
			...
			{name: 'ACCIDENT_DATE'          ,text:'사고일'					,type : 'uniDate'},
			{name: 'VEHICLE_NAME'           ,text:'차량명'					,type : 'string'},
			{name: 'ACCIDENT_DIV'           ,text:'사고구분코드'				,type : 'string'},
			{name: 'ACCIDENT_DIV_NAME'      ,text:'사고구분'					,type : 'string'},
			{name: 'ACCIDENT_PLACE'         ,text:'사고장소'					,type : 'string'},
			{name: 'DRIVER_FAULT'           ,text:'운전자과실코드'				,type : 'string'},
			{name: 'DRIVER_FAULT_NAME'      ,text:'운전자과실'					,type : 'string'},
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'gac300skrvService.selectList'/*,
			update: 'gac300skrvService.updateDetail',
			create: 'gac300skrvService.insertDetail',
			destroy: 'gac300skrvService.deleteDetail',
			syncAll: 'gac300skrvService.saveAll'*/
		}
	});
	...
	var panelSearch = Unilite.createSearchPanel('${PKGNAME}SearchForm',{
		...
		items: [{	
	    	...
	    	items:[{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {						
						panelResult.setValue('ROUTE_GROUP', newValue);
					}
				}
			},{	    
				fieldLabel: '노선',
	...
	var panelResult = Unilite.createSearchForm('resultForm',{
		...
		items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {						
						panelSearch.setValue('ROUTE_GROUP', newValue);
					}
				}
			},{	    
				fieldLabel: '노선',
     ...
     var masterGrid = Unilite.createGrid('${PKGNAME}Grid', {
		...
		columns:[
			...
			{dataIndex:'ACCIDENT_NUM'                    ,width: 100 
			,summaryRenderer:function(value, summaryData, dataIndex, metaData ) {
			       return Unilite.renderSummaryRow(summaryData, metaData, '', '합계');
            }},
			{dataIndex:'ROUTE_GROUP'                     ,width: 66  , hidden: true},
			{dataIndex:'ROUTE_GROUP_NAME'                ,width: 72  },
			{dataIndex:'ROUTE_CODE'                      ,width: 66  },
			{dataIndex:'ROUTE_NUM'                       ,width: 66  },
			{dataIndex:'DRIVER_CODE'                     ,width: 60  },
			{dataIndex:'NAME'            		         ,width: 66  },
			{dataIndex:'EXPERIENCE_PERIOD'               ,width: 66  },
			{dataIndex:'JOIN_DATE'                       ,width: 88  },
			{dataIndex:'ACCIDENT_DATE'                   ,width: 88  },
			{dataIndex:'VEHICLE_NAME'                    ,width: 44  },
			{dataIndex:'ACCIDENT_DIV'                    ,width: 88  , hidden: true},
			{dataIndex:'ACCIDENT_DIV_NAME'               ,width: 66  },
			{dataIndex:'ACCIDENT_PLACE'                  ,width: 240  },
			{dataIndex:'DRIVER_FAULT'                    ,width: 88  , hidden: true},
			{dataIndex:'DRIVER_FAULT_NAME'               ,width: 100  },
			{ 
	         	text:'인적집계',
	         		columns: [
