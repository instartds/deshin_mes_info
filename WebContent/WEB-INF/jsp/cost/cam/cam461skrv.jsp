...
<t:appConfig pgmId="cam461skrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="cam461skrv" /> 				<!-- 사업장 -->
...
function appMain() {
   var cam461skrvModel=Unilite.defineModel('cam461skrvModel', {
   	fields: [
   ...
   var cam461skrvStore=Unilite.createStore('Store', {
   		model: 'cam461skrvModel',
		autoLoad: false,
		uniOpt: {
		...
		proxy: {
			type: 'direct',
			api: {
				read    : 'cam461skrvService.selectList'
   ...
   /**
	...
	var panelSearch = Unilite.createSearchPanel('searchForm', {
		title: '검색조건',
		api: {
         		 load:'cam461skrvService.selectWORK_SEQ'
				},
	...
	var masterGrid = Unilite.createGrid('tio110skrvGrid1', {
    	...
    	store: cam461skrvStore,
        columns: [
        	{dataIndex: 'PROD_ITEM_CODE'			, width: 146 ,locked: false,
			...
			{dataIndex: 'TOTAL_AMT'					, width: 146,
				summaryType: 'count',
		        summaryRenderer: function(value, summaryData, dataIndex, metaData) {
		        	var matDAmtOne = metaData.record.get("MAT_DAMT_ONE"),
		            ...
		            return Ext.util.Format.number((matDAmtOne + matDAmtTwo + matIAmtOne + matIAmtTwo + laborDAmt + laborIAmt + expenseDAmt + expenseIAmt) , UniFormat.Price);
		        }
		    },
			{dataIndex: 'PER_UNIT_COST'				, width: 200,
				summaryType: 'count',
		        summaryRenderer: function(value, summaryData, dataIndex, metaData) {
		        	var matDAmtOne = metaData.record.get("MAT_DAMT_ONE"),
		        	    matDAmtTwo = metaData.record.get("MAT_DAMT_TWO"),
		            ...
		            return (prodtQ && prodtQ > 0) ? Ext.util.Format.number(((matDAmtOne + matDAmtTwo + matIAmtOne + matIAmtTwo + laborDAmt + laborIAmt + expenseDAmt + expenseIAmt) / prodtQ), UniFormat.Price) : 0 ;
		        }
			}
			/*(SUM(SUM_F.MAT_DAMT_ONE)
			  +SUM(SUM_F.MAT_DAMT_TWO)
			  +SUM(SUM_F.MAT_IAMT_ONE)
			  +SUM(SUM_F.MAT_IAMT_TWO)
			  +SUM(SUM_F.LABOR_DAMT)
			  +SUM(SUM_F.LABOR_IAMT)
			  +SUM(SUM_F.EXPENSE_DAMT)
			  +SUM(SUM_F.EXPENSE_IAMT))/SUM(SUM_F.PRODT_Q) As PER_UNIT_COST
			*/
		]
    });
	Unilite.Main( {
