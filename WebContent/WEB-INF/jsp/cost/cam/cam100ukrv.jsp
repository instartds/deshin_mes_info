...
function appMain() {
	...
	var directMasterStore1 = Unilite.createStore('cam100skrvMasterStore1',{
		...
		proxy: {
			type: 'direct',
			api: {			
				read: 'cam100ukrvService.selectIDGb'
			}
		},
	...
	var directMasterStore2 = Unilite.createStore('cam100skrvMasterStore2',{
		...
		proxy: {
			type: 'direct',
			api: {			
				read: 'cam100ukrvService.selectProdItem'
			}
		},
		loadStoreRecords : function()	{
			if(!costPoolCode || costPoolCode.length == 0)	{
	...
	var directMasterStore3 = Unilite.createStore('cam110ukrvMasterModel3',{
        ...
        proxy: {
            type: 'direct',
            api: {			
                read: 'cam100ukrvService.selectStatusList'                	
            }
        },
		loadStoreRecords : function()	{
	 ...
	 * @type 
	 */
	var panelSearch = Unilite.createForm('cam100ukrv', {
		disabled: false,
		region:'north',
		padding:'10 0 5 0',
	...
	var costPanel1 = Unilite.createForm('cam100ukrvCostPanel1', {
		disabled: false,
		title:'원가계산',
		collapsible :true,
			...
			items: [{
				...
				handler: function() {
					if(costPanel1.isValid())	{
						...
						cam100ukrvService.executeProcessCost(param, 
	...
	var costPanel2 = Unilite.createForm('cam100ukrvCostPanel2', {
		disabled: false,
		title:'원가마감',
		collapsible :true,
		collapseDirection  : 'top',
			...
			items: [{
				...
				handler: function() {
					if(costPanel2.isValid())	{
						...
						param.WORK_GUBUN = 'P';
						Ext.getBody().mask('실행중...','loading-indicator');
						cam100ukrvService.executeCloseCost(param, 
							function(provider, response) {
								if(provider) {	
    ...
    Unilite.Main( {
		...
		getLastWorkInfo:function(divCode)	{
			cam100ukrvService.selectLastWorkInfo({'DIV_CODE':divCode}, function(reponseText){
				if(reponseText) {
		...
		getLastCloseInfo:function(divCode)	{
			cam100ukrvService.selectLastCloseInfo({'DIV_CODE':divCode}, function(reponseText){
				if(reponseText) {
