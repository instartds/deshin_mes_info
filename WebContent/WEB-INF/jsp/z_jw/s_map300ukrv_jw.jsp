<%@page language="java" contentType="text/html; charset=utf-8"%>
	<t:appConfig pgmId="s_map300ukrv_jw">
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {	
			read	: 's_map300ukrv_jwService.selectList',
			update: 's_map300ukrv_jwService.updateList',
			syncAll : 's_map300ukrv_jwService.saveAll'
		}
	});
	...
	* @type 
	*/
	Unilite.defineModel('s_map300ukrv_jwModel', {
		fields: [
	...
	});//End of Unilite.defineModel('s_map300ukrv_jwModel', {
	...
	* @type 
	*/			
	var directMasterStore1 = Unilite.createStore('s_map300ukrv_jwMasterStore1',{
		model	: 's_map300ukrv_jwModel',
		proxy	: directProxy,
		uniOpt	: {
...
//			})
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('s_map300ukrv_jwGrid');
	...
	});//End of var directMasterStore1 = Unilite.createStore('s_map300ukrv_jwMasterStore1',{
	...
	*/
	 ...
	 * @type 
	 */
	var masterGrid = Unilite.createGrid('s_map300ukrv_jwGrid1', {
		...
		columns: [
			...
			//20181017 추가
			{dataIndex:'ITEM_WIDTH'	 		, width: 66, hidden: true},
			{dataIndex:'SQM'	 			, width: 66, hidden: true}
		],
		listeners:{
	...
	});//End of var masterGrid = Unilite.createGrid('s_map300ukrv_jwGrid1', {



	Unilite.Main({
		id			: 's_map300ukrv_jwApp',
		borderItems	: [{
			layout	: 'border',
			region	: 'center',
			border	: 'false',
		...
		onPriceApply: function(){
			...
			Ext.each(records,function(record, index, records){
				...
				if(masterForm.getValue('MONEY_UNIT') == BsaCodeInfo.gsDefaultMoney){
					...
					record.set('ORDER_UNIT_I'		, record.get('SQM') * record.get('ORDER_UNIT_P'));
					...
					record.set('ORDER_UNIT_FOR_O'	, record.get('SQM') * record.get('ORDER_UNIT_P') / record.get('EXCHG_RATE_O'));
					...
					record.set('INOUT_I'			, record.get('INOUT_P') * record.get('SQM'));
					record.set('ORDER_UNIT_FOR_P'	, masterForm.getValue('ORDER_UNIT_FOR_P'));
					record.set('ORDER_UNIT_FOR_O'	, record.get('SQM') * record.get('ORDER_UNIT_FOR_P'));
					record.set('INOUT_FOR_P'		, masterForm.getValue('ORDER_UNIT_FOR_P'));
					record.set('INOUT_FOR_O'		, record.get('ORDER_UNIT_FOR_O'));
				}
				
				record.set('APPLY_YN', 'Y');
				if(record.get('PRICE_YN') == 'N'){
		...
		onExchgRatekApply:	function(){
			...
			Ext.each(records,function(record, index, records){
				...
				orderUnitQ = orderUnitQ * record.get('TRNS_RATE') * itemWidth / 1000;
				...
				if(record.get('EXCHG_RATE_O') != 0){ 
					...
					record.set('ORDER_UNIT_I'	, orderUnitQ * record.get('ORDER_UNIT_P'));
