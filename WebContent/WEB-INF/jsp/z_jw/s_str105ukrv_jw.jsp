...
<t:appConfig pgmId="s_str105ukrv_jw"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="s_str105ukrv_jw" /> 				<!-- 사업장 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_str105ukrv_jwService.selectList'/*,
			update	: 's_str105ukrv_jwService.updateDetail',
			create	: 's_str105ukrv_jwService.insertDetail',
			destroy	: 's_str105ukrv_jwService.deleteDetail',
			syncAll	: 's_str105ukrv_jwService.saveAll'*/
		}
	});
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_str105ukrv_jwService.selectList2',
			update	: 's_str105ukrv_jwService.updateDetail2',
			create	: 's_str105ukrv_jwService.insertDetail2',
			destroy	: 's_str105ukrv_jwService.deleteDetail2',
			syncAll	: 's_str105ukrv_jwService.saveAll2'
		}
	});
					...
					fn: function(records, type) {
						...
						if(BsaCodeInfo.gsOptDivCode == "1"){	//출고사업장과 동일
							...
							if(Ext.isEmpty(saleDivCode)){//거래처의 영업담당자가 있는지 체크
								...
								alert('<t:message code="system.message.sales.message065" default="영업담당자 정보가 존재하지 않습니다."/>');	//영업담당자정보가 존재하지 않습니다.
								return false;
							}
						}
...
//						var whStore = panelResult.getField('ALL_CHANGE_WH_CODE').getStore();
				...
				specialkey:function(field, event)	{
					if(event.getKey() == event.ENTER) {
						...
						if(!Ext.isEmpty(newValue)) {
							...
							//선택된 데이터 체크 - 1.선택된 detailGrid 행 존재 확인
							if(Ext.isEmpty(detailRecord)) {
								beep();
								gsText = '<t:message code="system.message.sales.datacheck016" default="선택된 자료가 없습니다."/>';
								openAlertWindow(gsText);
//								alert('선택된 수불데이터가 없습니다.');
								panelResult.setValue('BARCODE', '');
								return false;
							}
							//선택된 데이터 체크 - 2.선택된 행과 바코드의 품목일치여부 확인
//							if(detailRecord.get('ITEM_CODE').toUpperCase() == newValue.split('|')[0].toUpperCase()) {
								detailGrid.focus();
...
//						'QUE VO INDUSTRIAL PARK, PHUONGLIEU COMMUNE, QUE VO DISTRICT, BAC NINH PROVINCE, VIET NAM';
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('s_str105ukrv_jwDetailModel', {
		fields: [
...
//			{name: 'PURCHASE_TYPE'			, text:'<t:message code="system.label.sales.purchasecondition" default="매입조건"/>'				, type: 'string'		, comboType:'AU', comboCode:'YP09', editable: false},
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('s_str105ukrv_jwDetailStore', {
		model	: 's_str105ukrv_jwDetailModel',
		proxy	: directProxy,
		autoLoad: false,
		uniOpt	: {
...
//				panelResult.setValue('TOT_QTY'			,dtotQty);   							//수량총계
	...
	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('s_str105ukrv_jwGrid', {
		store	: detailStore,
		itemId	: 's_str105ukrv_jwGrid',
		selModel: 'rowmodel',
		layout	: 'fit',
		region	: 'center',
		flex	: 1,
					...
					handler	: function() {
						if(BsaCodeInfo.useLotAssignment == "Y"){
							if(Ext.isEmpty(panelResult.getValue('WH_CODE'))){
								alert('<t:message code="system.message.sales.message057" default="출고창고를 선택해 주세요."/>');
								return false;
							}else if(Ext.isEmpty(panelResult.getValue('WH_CELL_CODE'))){
								alert('<t:message code="system.message.sales.message058" default="출고창고 Cell을 선택해 주세요."/>');
								return false;
							}
						}
...
//					}
					if(BsaCodeInfo.gsSumTypeCell == 'Y' && Ext.isEmpty(e.record.data.WH_CELL_CODE)){
						alert('<t:message code="system.message.sales.message059" default="출고창고 CELL코드를 입력하십시오."/>');
						return false;
					}
					if(Ext.isEmpty(e.record.data.ITEM_CODE)){
						alert('<t:message code="system.message.sales.message024" default="품목코드를 입력하십시오."/>');
						return false;
					}
				}
...
//			grdRecord.set('ORDER_UNIT_Q'		, record['R_ALLOC_Q']);
	...
	var barcodeStore = Unilite.createStore('s_str105ukrv_jwBarcodeStore', {
		model	: 's_str105ukrv_jwDetailModel',
		proxy	: directProxy2,
		autoLoad: false,
		uniOpt	: {
			...
			if(inValidRecs.length == 0) {
				config = {
					...
					success: function(batch, option) {
						...
						var param= panelResult.getValues();
						s_str105ukrv_jwService.spReceiving(param, function(provider, response){
							if(!Ext.isEmpty(provider)){
								 	panelResult.setValue('INOUT_NUM', provider['INOUT_NUM']);
...
//						})
	...
	var barcodeGrid = Unilite.createGrid('s_str105ukrv_jwBarcodeGrid', {
		store	: barcodeStore,
		itemId	: 's_str105ukrv_jwBarcodeGrid',
		layout	: 'fit',
		region	: 'south',
		split	: true,
		flex	: 0.5,
...
//			{dataIndex: 'PURCHASE_TYPE'			,width:66	,hidden: true },
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 's_str105ukrv_jwService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('s_str105ukrv_jwOrderNoMasterGrid', {
		// title: '기본',
		layout	: 'fit',
...
//			panelResult.setValue('CREATE_LOC'	, record.get('CREATE_LOC'));
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('s_str105ukrv_jworderNoDetailGrid', {
		layout : 'fit',
		store: orderNoMasterStore,
		uniOpt:{
			useRowNumberer: false
...
//						orderNoSearch.setValue('ORDER_TYPE',panelResult.getValue('ORDER_TYPE'));
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('s_str105ukrv_jwRequestModel', {
		fields: [
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('s_str105ukrv_jwRequestStore', {
		model: 's_str105ukrv_jwRequestModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_str105ukrv_jwService.selectRequestiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('s_str105ukrv_jwRequestGrid', {
		// title: '기본',
		layout	: 'fit',
		store	: requestStore,
		...
		returnData: function(issueBarcode)	{
			var records = this.getSelectedRecords();
			if(!Ext.isEmpty(issueBarcode)) {				//출하지시번호 바코드 입력 시,
				records = issueBarcode;
			}
			Ext.each(records, function(record,i){
				UniAppManager.app.onNewDataButtonDown();
				if(!Ext.isEmpty(issueBarcode)) {			//출하지시번호 바코드 입력 시,
					detailGrid.setrequestData(record);
...
//						requestSearch.setValue('ISSUE_DATE_FR'	, UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('s_str105ukrv_jwsalesOrderForm', {
		layout :  {type : 'uniTable', columns : 2},
		items :[
			Unilite.popup('DIV_PUMOK',{
			fieldLabel:'<t:message code="system.label.sales.item" default="품목"/>' ,
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('s_str105ukrv_jwsalesOrderModel', {
		fields: [
			{ name: 'ORDER_NUM'				, text:'<t:message code="system.label.sales.sono" default="수주번호"/>'			,type : 'string' },
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('s_str105ukrv_jwsalesOrderStore', {
		model	: 's_str105ukrv_jwsalesOrderModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy	: {
			type: 'direct',
			api: {
				read	: 's_str105ukrv_jwService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function()	{
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('s_str105ukrv_jwsalesOrderGrid', {
		// title: '기본',
		layout : 'fit',
...
//  					salesOrderSearch.setValue('DVRY_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('DVRY_DATE_TO')));
	...
	Unilite.Main({
		...
		fnInitBinding: function(params) {
			...
			panelResult.getField('ISSUE_BARCODE').focus();		//Reset 버튼 클릭 시, 출하지시번호에 포커스 강제 지정
...
//				panelResult.setAllFieldsReadOnly(true);
		...
		onNewDataButtonDown2: function(newValue) {
			...
			if (Ext.isEmpty(selectedRecord)) {
				...
				if(Ext.isEmpty(BARCODE_KEY)) {
					if(!Ext.isEmpty(sofInfo) && sofInfo != 0) {					//수주 참조일 경우,
						BARCODE_KEY = sofInfo;

					} else if(!Ext.isEmpty(srqInfo) && srqInfo != 0) {			//출하지시 참조일 경우,
						BARCODE_KEY = srqInfo;
		...
		onDeleteDataButtonDown: function() {
			if(gsMonClosing == "Y" || gsDayClosing == "Y"){	//마감여부 check
				alert('<t:message code="system.message.sales.message054" default="마감된 월(일)에 대해서는 자료의 수정/추가/삭제가 불가능합니다."/>'); //마감된 월(일)에 대해서는 자료의 수정/추가/삭제가 불가능합니다.
				return false;
			}
			...
			if(Ext.isEmpty(selRow) && Ext.isEmpty(selRow2)) {
				alert('<t:message code="system.message.sales.datacheck016" default="선택된 자료가 없습니다."/>');					//선택된 자료가 없습니다.
				return false;
			}
			if(activeGridId == 's_str105ukrv_jwGrid' && !Ext.isEmpty(selRow) && selRow.phantom === true)	{			//detailGrid 삭제 함수 호출
				fnDeleteDetail(selRow);

			} else if(activeGridId != 's_str105ukrv_jwGrid' && !Ext.isEmpty(selRow2) && selRow2.phantom === true) {	//barcodeGrid 삭제 함수 호출
				fnBarcodeGrid(selRow, selRow2)

			} else if (confirm('system.message.sales.confirm001')) {
				if( activeGridId == 's_str105ukrv_jwGrid' && !Ext.isEmpty(selRow)) {				//detailGrid 삭제 함수 호출
					if(BsaCodeInfo.gsInoutAutoYN == "N" && selRow.get('ACCOUNT_Q') > 0) {	//동시매출발생이 아닌 경우,매출존재체크 제외
						alert('<t:message code="system.message.sales.message055" default="매출이 진행된 건은 수정/삭제할 수 없습니다."/>');													//매출이 진행된 건은 수정/삭제할 수 없습니다.
						return false;
					}
					if(selRow.get('SALE_C_YN') == "Y"){
						alert('<t:message code="system.message.sales.message056" default="계산서가 마감된 건은 수정/삭제가 불가능합니다."/>');													//계산서가 마감된 건은 수정/삭제가 불가능합니다.
						return false;
					}
					fnDeleteDetail(selRow);

				} else if(activeGridId != 's_str105ukrv_jwGrid' && !Ext.isEmpty(selRow2)) {		//barcodeGrid 삭제 함수 호출
...
//				gsLotNoS = ''
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_str105ukrv_jwAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		setDefault: function() {
			...
			s_str105ukrv_jwService.deptWhcode(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
...
//			panelResult.getField('TOT_QTY').setReadOnly(true);
		...
		checkForNewDetail:function() {
			if(BsaCodeInfo.gsAutoType=='N' && Ext.isEmpty(panelResult.getValue('CUSTOM_CODE')))	{
				alert('<t:message code="system.label.sales.custom" default="거래처"/>:<t:message code="system.message.sales.datacheck001" default="필수입력 항목입니다."/>');
				return false;
			}
...
//						panelResult.getField('MONEY_UNIT').focus();
	...
	function fnEnterIssueBarcode(newValue) {
		...
		if(flag) {
			...
			//출하지시참조 조회쿼리 호출
			s_str105ukrv_jwService.selectRequestiList(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
...
//					panelResult.getField('BARCODE')focus();
					...
					alert('<t:message code="system.label.sales.message002" default="입력하신 출하지시번호의 데이터가 존재하지 않습니다."/>');
					Ext.getBody().unmask();
					panelResult.setValue('ISSUE_BARCODE', '');
					panelResult.getField('ISSUE_BARCODE').focus();
					return false;
	...
	function fnEnterBarcode(newValue) {
		...
		if(selectedRecord.get('INOUT_METH') == '2') {
			//품목정보 체크
			if(Ext.isEmpty(selectedRecord.get('ITEM_CODE'))) {
//				alert('품목 정보가 없습니다.');
				beep();
				gsText = '<t:message code="system.label.sales.message003" default="입력하신 품목 정보가 없습니다."/>';
				openAlertWindow(gsText);
				//해당 컬럼에 포커싱 작업 추후 진행
//				detailGrid.getColumn('ORDER_UNIT_Q').focus(detailGrid.getSelectedRowIndex());
				panelResult.setValue('BARCODE', '');
				panelResult.getField('BARCODE').focus();
				return false;
			}
...
/*			//출고량 체크
			if(Ext.isEmpty(selectedRecord.get('ORDER_UNIT_Q')) || selectedRecord.get('ORDER_UNIT_Q') == 0) {
				alert('출고수량이  없습니다.');
				//해당 컬럼에 포커싱 작업 추후 진행
//				detailGrid.getColumn('ORDER_UNIT_Q').focus(detailGrid.getSelectedRowIndex());
				return false;
			...
			//단가정보 체크
			if(selectedRecord.get('ACCOUNT_YNC') == 'Y' &&(Ext.isEmpty(selectedRecord.get('ORDER_UNIT_P')) || selectedRecord.get('ORDER_UNIT_P') == 0)) {
//				alert('단가 정보가 없습니다.');
				beep();
...
//				detailGrid.getColumn('ORDER_UNIT_Q').focus(detailGrid.getSelectedRowIndex());
		...
		var barcodeLotNo= newValue.split('|')[1];
		var orderUnitQ	= newValue.split('|')[2];
		...
		//재고 테이블 검색하여 재고존재확인 / FIFO 로직 구현해야 함
//		if(Ext.isEmpty(gsLotNoS)) {				//임시테이블명 생성 - 다른 로직으로 대체
//			gsLotNoS = fnMakeTmpTable();
//		}
...
//							alert(provider[i].ERR_MSG);
						...
						if(selectedRecord.get('ITEM_CODE').toUpperCase() == provider[i].NEWVALUE.split('|')[0]) {
							...
							barcodeStore.filterBy(function(record){			//다시 필터 set
								return record.get('BARCODE_KEY') == selectedRecord.get('myId');
							})
							...
							if(flag) {
								UniAppManager.app.onNewDataButtonDown2(provider[i].NEWVALUE);
								return;
							}
...
//						break;
	...
	var activeGridId = 's_str105ukrv_jwGrid';
}
</script>