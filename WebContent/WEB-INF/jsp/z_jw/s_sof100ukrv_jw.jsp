...
<t:appConfig pgmId="s_sof100ukrv_jw"  >
...
<t:ExtComboStore comboType="BOR120" pgmId="s_sof100ukrv_jw"/><!-- 사업장    -->
...
function appMain() {
    ...
    //미등록된 SRM 데이터
    Unilite.Excel.defineModel('s_sof100ukrv_jwModel_notRegisteredSRM', {
        fields: [
    ...
    var notRegisteredSRMStore = Unilite.createStore('s_sof100ukrv_jwNotRegisteredSRMStore',{
        model: 's_sof100ukrv_jwModel_notRegisteredSRM',
        uniOpt: {
...
//      groupField: ''
     ...
     */
    var masterForm = Unilite.createSearchPanel('s_sof100ukrv_jwMasterForm', {
            ...
            items: [{
                ...
                listeners: {
                    change: function(field, newValue, oldValue, eOpts) {
                        ...
                        if(masterForm.getValue('NATION_INOUT') == '2') {    // 국외일시만
                                                                            // 무역폼
                                                                            // 활성화
                            // 무역폼 readOnly: false
...
//                        }
                            ...
                            var nationInout = masterForm.getValue("NATION_INOUT");
                                if(newValue != offerNoChk && nationInout == '2' && !Ext.isEmpty(newValue)){ //해외 일 경우에만
                                    ...
                                    if(! Ext.isEmpty(masterForm.getValue('OFFER_NO'))){
                                        s_sof100ukrv_jwService.checkOfferNo(param, function(provider2, response){
                                                    if(!Ext.isEmpty(provider2)){
                                                        checkOfferNo = provider2.data.OFFER_NO;
                                                        alert('[' + checkOfferNo + ']' + ' <t:message code="system.message.sales.message028" default="OFFER번호가 존재합니다."/>');
                                                         return false;
                                                    }
                                            });
                                    }
            ...
            Unilite.popup('AGENT_CUST', {
                 ...
                 * 'DEPT_CODE', textFieldName: 'DEPT_NAME', holdable: 'hold',
                 * listeners: { onSelected: { fn: function(records, type) {
                 * panelResult.setValue('DEPT_CODE',
                 * masterForm.getValue('DEPT_CODE'));
                 * panelResult.setValue('DEPT_NAME',
                 * masterForm.getValue('DEPT_NAME')); }, scope: this }, onClear:
                 * function(type) { panelResult.setValue('DEPT_CODE', '');
                 * panelResult.setValue('DEPT_NAME', ''); }, applyextparam:
                 ...
                 * ,{ fieldLabel: '<t:message code="system.label.sales.department" default="부서"/>', name: 'DEPT_CODE', xtype:
                 * 'uniTextfield', listeners: { change: function(field,
                ...
                margin: '0 0 0 95',
                xtype: 'button',
                text: '<t:message code="system.label.sales.purchaserequestinfoapply" default="구매요청정보반영"/>',
                ...
                handler: function() {
                    if(!Ext.isEmpty(masterForm.getValue('ORDER_NUM') && masterForm.getValue('ORDER_REQ_YN') == "N")){
                        var param = {
                            ...
                            OrderNum: masterForm.getValue('ORDER_NUM'),
                            DivCode: masterForm.getValue('DIV_CODE')
                        }
                        var me = this;
                        s_sof100ukrv_jwService.insertPurchaseRequest(param, function(provider, response) {
...
//                            popup.setExtParam({'CUSTOM_CODE': masterForm.getValue('CUSTOM_CODE')});
            ...
            }),
            {
                ...
                handler : function() {
                    ...
                    s_sof100ukrv_jwService.soToPoRequest(param, function(provider, response) {
                        if(!Ext.isEmpty(provider)){
                            if(provider=='Y'){
                                Ext.Msg.alert('<t:message code="system.label.sales.confirm" default="확인"/>','<t:message code="system.message.sales.aftersave002" default="완료되었습니다."/>');
                                UniAppManager.app.onQueryButtonDown();

                            } else {
                                Ext.Msg.alert('<t:message code="system.label.sales.confirm" default="확인"/>','<t:message code="system.message.sales.message125" default="작업 중 오류가 발생했습니다."/>');
                            }
                        }
                        Ext.getBody().unmask();
                    });
                ...
                }),
            {
                ...
                margin:'0 0 0 10',
                html:'<div>'+Ext.util.Format.number(BsaCodeInfo.gsApp2AmtInfo, UniFormat.Price) + BsaCodeInfo.gsMoneyUnit + ' ' + '<t:message code="system.label.sales.below" default="이하"/>' + '</div>'
            }, {
        ...
        api: {
            load: 's_sof100ukrv_jwService.selectMaster',
            submit: 's_sof100ukrv_jwService.syncForm'
        ...
        fnCreditCheck: function() {
            if(CustomCodeInfo.gsCustCrYn=='Y' && BsaCodeInfo.gsCreditYn=='Y') {
                if(this.getValue('TOT_ORDER_AMT') > this.getValue('REMAIN_CREDIT')) {
                    alert('<t:message code="system.message.sales.datacheck002" default="해당 업체에 대한 여신액이 부족합니다. 추가여신액 설정을 선행하시고 작업하시기 바랍니다."/>');
                    return false;
                }
            }
            return true;
...
// me.setAllFieldsReadOnly(true);
    ...
    }); // End of var masterForm = Unilite.createForm('s_sof100ukrv_jwMasterForm', {
...
// holdable: 'hold',
         ...
         // holdable: 'hold',
             ...
             * panelResult.getValue('DEPT_CODE'));
             * masterForm.setValue('DEPT_NAME',
             * panelResult.getValue('DEPT_NAME')); }, scope: this }, onClear:
             * function(type) { masterForm.setValue('DEPT_CODE', '');
             * masterForm.setValue('DEPT_NAME', ''); }, applyextparam:
             * function(popup){ var authoInfo = pgmInfo.authoUser;
             * //권한정보(N-전체,A-자기사업장>5-자기부서) var deptCode = UserInfo.deptCode;
             * //부서정보 var divCode = ''; //사업장
             *
...
// }
             ...
             * ,{ fieldLabel: '<t:message code="system.label.sales.department" default="부서"/>', name: 'DEPT_CODE', xtype: 'uniTextfield',
             *
             * listeners: { change: function(field, newValue, oldValue, eOpts) {
             * masterForm.setValue('DEPT_CODE', newValue); } } }
             */,{
            fieldLabel: '<t:message code="system.label.sales.taxincludedflag" default="세액포함여부"/>',
            name: 'TAX_INOUT',
            xtype: 'uniRadiogroup',
...
//            height: 47,
    ...
    var detailForm = Unilite.createForm('s_sof100ukrv_jwDetail', {
        autoScroll:true,
        layout : 'fit',
        ...
        items :[{
                    xtype:'xuploadpanel',
                    id : 's_sof100ukrv_jwFileUploadPanel',
...
//
                    ...
                    //첨부파일
                    var fp = Ext.getCmp('s_sof100ukrv_jwFileUploadPanel');
                    var ordernum = panelResult.getValue('ORDER_NUM');
                    if(!Ext.isEmpty(ordernum)) {
                        s_sof100ukrv_jwService.getFileList({DOC_NO : ordernum},
                ...
                detailWin = Ext.create('widget.uniDetailWindow', {
                    ...
                    tbar:  ['->',      {   itemId : 'confirmBtn',
                                            ...
                                            handler: function() {
                                                ...
                                                s_sof100ukrv_jwService.insertSOF102(param , function(provider, response){})
                                            },
                                            disabled: false
                                        },
                                        {   itemId : 'confirmCloseBtn',
                                            ...
                                            handler: function() {
												...
												s_sof100ukrv_jwService.selectDocCnt(param, function(provider, response){
													if(!Ext.isEmpty(provider)){
														if( provider > 0){
                                            ...
                                            handler: function() {
                                                ...
                                                if(!Ext.isEmpty(detailSearch.getValue('DEL_FIDS'))){
                                                    if(confirm('문서를 삭제 하시겠습니까?')) {
                                                        ...
                                                        s_sof100ukrv_jwService.deleteSOF102(param , function(provider, response){})
                                                    }
                                                } else {
                                                    alert('삭제할 문서가 없습니다.');
                                                    return false;
...
// }
     ...
     */
    // 수주정보 모델
    Unilite.defineModel('s_sof100ukrv_jwDetailModel', {
        fields: [
            ...
            {name: 'ORDER_O'                , text:'<t:message code="system.label.sales.amount" default="금액"/>'      , allowBlank: true , defaultValue: 0 , type: 'uniPrice'},
            ...
            {name: 'ORDER_O_TAX_O'          , text:'<t:message code="system.label.sales.sototalsum" default="수주합계"/>'               , type: 'uniPrice', defaultValue: 0, editable:false},
            ...
            {name: 'PROD_END_DATE'          , text:'<t:message code="system.label.sales.productionfinishdate" default="생산완료일"/>'           , type: 'uniDate'},
    ...
    var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 's_sof100ukrv_jwService.selectDetailList',
            update: 's_sof100ukrv_jwService.updateDetail',
            create: 's_sof100ukrv_jwService.insertDetail',
            destroy: 's_sof100ukrv_jwService.deleteDetail',
            syncAll: 's_sof100ukrv_jwService.saveAll'
        }
    });
    // 수주정보스토어
    var detailStore = Unilite.createStore('s_sof100ukrv_jwDetailStore', {
        model: 's_sof100ukrv_jwDetailModel',
        autoLoad: false,
        uniOpt: {
         ...
         * 's_sof100ukrv_jwService.selectDetailList', update:
         * 's_sof100ukrv_jwService.updateDetail', create:
         * 's_sof100ukrv_jwService.insertDetail', destroy:
         * 's_sof100ukrv_jwService.deleteDetail', syncAll:
         * 's_sof100ukrv_jwService.syncAll' } },
         */
...
//            	}
                ...
                if(record.get('ACCOUNT_YNC') == 'Y') {
                    if(record.get('ORDER_P')== 0){
                        alert((index + 1) + '<t:message code="system.message.sales.message060" default="행의 입력값을 확인해 주세요."/>\n' + '<t:message code="system.message.sales.message071" default="매출대상은 단가가 필수 입력값 입니다."/>');
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('s_sof100ukrv_jwGrid');
     ...
     */
    var detailGrid = Unilite.createGrid('s_sof100ukrv_jwGrid', {
        layout: 'fit',
        region:'center',
        uniOpt: {
...
//                      param.dvryDate          = dvryDate;
                ...
                s_sof100ukrv_jwService.receiveSRM(param, function(provider, response){
            ...
            xtype: 'button',
            itemId:'procTool',
            text: '<t:message code="system.label.sales.processbutton" default="프로세스..."/>',
            iconCls: 'icon-link',
            menu: Ext.create('Ext.menu.Menu', {
                items: [
                    /*
                     * { itemId: 'reqIssueLinkBtn', text: '출하지시등록', handler:
                     * function() { var params = { 'ORDER_NUM' :
                     * masterForm.getValue('ORDER_NUM') } var rec = {data :
                     * {prgID : 'srq101ukrv', 'text':'출하지시등록'}};
                     * parent.openTab(rec, '/sales/srq101ukrv.do', params); //
                     * srq100ukrv의 fnInitBinding(params)에서 처리되도록 구현되어야 함.
                     * (cmb200ukrv.jsp 참고)
                     *  } } ,
                     */
                        {
                            ...
                            if(detailStore.isDirty()){
                                   alert('<t:message code="system.message.sales.message032" default="저장작업 선행후 처리하시기 바랍니다."/>');
                                   return false;
                            }
                            if(detailStore.getCount() != 0){
                                   var params = {
                                    ...
                                    var rec = {data : {prgID : 'srq100ukrv', 'text':''}};
                        ...
                        },
                {
                     ...
                     * , { itemId: 'saleLinkBtn', text: '매출등록', handler:
                     * function() { var params = { ORDER_NO :
                     * masterForm.getValue('ORDER_NO') } var rec = {data :
                     * {prgID : 'ssa100ukrv', 'text':''}}; parent.openTab(rec,
                     * '/sales/ssa100ukrv.do', params); } }
                     */]
            })
        }],
        store: detailStore,
            ...
            {dataIndex: 'DVRY_CUST_NAME',   width: 100,
                editor: Unilite.popup('DELIVERY_G',{
                            ...
                            listeners:{ 'onSelected': {
                                fn: function(records, type  ){
                                // var grdRecord =
                                // Ext.getCmp('s_sof100ukrv_jwGrid').uniOpt.currentRecord;
                                var grdRecord = detailGrid.uniOpt.currentRecord;
...
// grdRecord.set('REF_EXCHG_RATE_O', params.record[0].data.);
    ...
    var orderNoSearch = Unilite.createSearchForm('orderNoSearchForm', {
        ...
        items: [{
            ...
            },{fieldLabel: '<t:message code="system.label.sales.pono" default="발주번호"/>'           , name: 'PO_NUM'},
            {
                fieldLabel: '<t:message code="system.label.sales.salescharge" default="영업담당"/>',
                name: 'ORDER_PRSN',
                xtype:'uniCombobox',
            ...
            Unilite.popup('AGENT_CUST',{fieldLabel:'<t:message code="system.label.sales.custom" default="거래처"/>' , validateBlank: false,
                listeners:{
                    applyextparam: function(popup){
                        popup.setExtParam({'AGENT_CUST_FILTER':  ['1','3']});
                        popup.setExtParam({'CUSTOM_TYPE':  ['1','3']});
...
// textFieldName:'PROJECT_NAME', validateBlank: false}),
            ...
            {fieldLabel: '<t:message code="system.label.sales.offerno" default="OFFER번호"/>'           , name: 'OFFER_NO'},
            {
            ...
            },{name: 'REMARK'   ,fieldLabel: '<t:message code="system.label.sales.remarks" default="비고"/>' , xtype : 'uniTextfield', width: 325}  /*
                 * , Unilite.popup('DEPT', { fieldLabel: '<t:message code="system.label.sales.department" default="부서"/>', valueFieldName:
    ...
    var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                    read    : 's_sof100ukrv_jwService.selectOrderNumMasterList'
                }
            }
            ,loadStoreRecords : function()  {
    ...
    // 검색 그리드(마스터)
    var orderNoMasterGrid = Unilite.createGrid('s_sof100ukrv_jwOrderNoMasterGrid', {
        // title: '기본',
        layout : 'fit',
    ...
    var orderNoDetailStore = Unilite.createStore('orderNoDetailStore', {
            ...
            proxy: {
                type: 'direct',
                api: {
                    read    : 's_sof100ukrv_jwService.selectOrderNumDetailList'
                }
            },
            loadStoreRecords : function()  {
                var param= orderNoSearch.getValues();
    ...
    // 검색 그리드(디테일)
    var orderNoDetailGrid = Unilite.createGrid('s_sof100ukrv_jwOrderNoDetailGrid', {
        layout : 'fit',
...
// //openNationWindow
// //해외추가 메인
// function openNationWindow() {
// if(!nationWindow) {
// nationWindow = Ext.create('widget.uniDetailWindow', {
// title: '해외추가',
// width: 1000,
// height: 180,
// layout: {type:'vbox', align:'stretch'},
...
// text: '적용',
// handler: function() {
// UniAppManager.app.setNationInfoData();
// nationWindow.hide();
// },
// disabled: false
// }, '->',{
...
// show: function( panel, eOpts ) {
// var record = detailGrid.getSelectedRecord();
// nationSearch.setValue('CUSTOM_CODE'
// ,masterForm.getValue('CUSTOM_CODE_TEMP'));
// nationSearch.setValue('CUSTOM_NAME'
// ,masterForm.getValue('CUSTOM_NAME_TEMP'));
// nationSearch.setValue('DATE_DEPART' ,masterForm.getValue('DATE_DEPART'));
// nationSearch.setValue('DATE_EXP' ,masterForm.getValue('DATE_EXP'));
// nationSearch.setValue('PAY_METHODE1' ,masterForm.getValue('PAY_METHODE1'));
// nationSearch.setValue('PAY_TERMS' ,masterForm.getValue('PAY_TERMS'));
// nationSearch.setValue('PAY_DURING' ,masterForm.getValue('PAY_DURING'));
// nationSearch.setValue('TERMS_PRICE' ,masterForm.getValue('TERMS_PRICE'));
// nationSearch.setValue('COND_PACKING' ,masterForm.getValue('COND_PACKING'));
// nationSearch.setValue('METH_CARRY' ,masterForm.getValue('METH_CARRY'));
// nationSearch.setValue('METH_INSPECT' ,masterForm.getValue('METH_INSPECT'));
// nationSearch.setValue('DEST_PORT' ,masterForm.getValue('DEST_PORT'));
// nationSearch.setValue('DEST_PORT_NM' ,masterForm.getValue('DEST_PORT_NM'));
// nationSearch.setValue('SHIP_PORT' ,masterForm.getValue('SHIP_PORT'));
// nationSearch.setValue('SHIP_PORT_NM' ,masterForm.getValue('SHIP_PORT_NM'));
// nationSearch.setValue('BANK_CODE' ,masterForm.getValue('BANK_SENDING'));
// }
// }
// })
// }
...
// var nationSearch = Unilite.createSearchForm('nationSearchForm', { // 해외추가 팝업창
// layout: {type : 'uniTable', columns : 3},
// items:[
// {
// layout: {type:'uniTable', column:2},
...
// ],
// api: {
// load: 's_sof100ukrv_jwService.selectNation',
// submit: 's_sof100ukrv_jwService.syncForm2'
// }
// });
    ...
    var estimateSearch = Unilite.createSearchForm('estimateForm', {
            ...
            layout :  {type : 'uniTable', columns : 2},
            items :[    Unilite.popup('AGENT_CUST',{fieldLabel:'<t:message code="system.label.sales.estimateplace" default="견적처"/>' , validateBlank: false})
                        ,{ fieldLabel: '<t:message code="system.label.sales.estimateno" default="견적번호"/>'  ,       name: 'ESTI_NUM'}
                        ,{ fieldLabel: '<t:message code="system.label.sales.estimatedate" default="견적일"/>'
                         ...
                         ,{fieldLabel: '<t:message code="system.label.sales.salescharge" default="영업담당"/>'  , name: 'ESTI_PRSN',    xtype:'uniCombobox', store: Ext.data.StoreManager.lookup('SOF100UKRV_DIV_PRSN') }
                    ]

    });
    // 견적참조 모델
    Unilite.defineModel('s_sof100ukrv_jwESTIModel', {
        fields: [
    ...
    // 견적참조 스토어
    var estimateStore = Unilite.createStore('s_sof100ukrv_jwEstiStore', {
            model: 's_sof100ukrv_jwESTIModel',
            autoLoad: false,
            uniOpt : {
                isMaster: false,            // 상위 버튼 연결
                editable: false,            // 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                    read    : 's_sof100ukrv_jwService.selectEstiList'
                }
            },
            listeners:{
                load:function(store, records, successful, eOpts)    {
    ...
    // 견적참조 그리드
    var estimateGrid = Unilite.createGrid('s_sof100ukrv_jwEstimateGrid', {
        // title: '기본',
        layout : 'fit',
        store: estimateStore,
...
// estimateSearch.setValue('DIV_CODE', masterForm.getValue('DIV_CODE'));
//
...
// estimateSearch.setValue('FR_ESTI_DATE', UniDate.get('startOfMonth',
// panelResult.getValue('ORDER_DATE')) );
...
// estimateSearch.setValue('DIV_CODE', panelResult.getValue('DIV_CODE'));
    ...
    var refSearch = Unilite.createSearchForm('RefSForm', {
            ...
            items :[    Unilite.popup('AGENT_CUST',{fieldLabel:'<t:message code="system.label.sales.soplace" default="수주처"/>' , validateBlank: false,
                        ...
                        ,Unilite.popup('DIV_PUMOK',{validateBlank: false,
                        listeners: {
                            applyextparam: function(popup){
                                popup.setExtParam({'DIV_CODE': masterForm.getValue('DIV_CODE')});
                            }
                        ...
                        ,{ fieldLabel: '<t:message code="system.label.sales.sodate" default="수주일"/>'
                         ...
                         },{fieldLabel: '<t:message code="system.label.sales.lasthistory" default="최근이력"/>'      , name: 'RDO_YN',   xtype:'uniRadiogroup', comboType:'AU', comboCode:'B131' , width:230, allowBlank:false, value:'Y'},
                         {
                            fieldLabel: '납기일',
                            xtype: 'uniDateRangefield',
    ...
    // 수주이력 모델
    Unilite.defineModel('s_sof100ukrv_jwRefModel', {
        fields: [
    ...
    // 수주이력 스토어
    var refStore = Unilite.createStore('s_sof100ukrv_jwRefStore', {
            model: 's_sof100ukrv_jwRefModel',
            autoLoad: false,
            uniOpt : {
                isMaster: false,            // 상위 버튼 연결
                editable: false,            // 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                    read    : 's_sof100ukrv_jwService.selectRefList'
    ...
    // 수주이력 그리드
    var refGrid = Unilite.createGrid('s_sof100ukrv_jwRefGrid', {
        // title: '기본',
        layout : 'fit',
        store: refStore,
        selModel :   Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false }),
    ...
    function openRefWindow() {
        ...
        //refSearch.setValue('ORDER_PRSN', masterForm.getValue('ORDER_PRSN'));
                        ...
                        grids: [
                             {
                                ...
                                readApi: 's_sof100ukrv_jwService.selectExcelUploadSheet1',
                                columns: [
                                            ...
                                            this.contextMenu.add(
                                                {
                                                    ...
                                                    handler: function(menuItem, event) {
                                                        ...
                                                        var params = {
                                                            ...
                                                            sender: me,
                                                            action: 'excelNew',
                                                            _EXCEL_JOBID: excelWindow.jobID,            // SOF112T
                                                                                                        // Key1
                                                            _EXCEL_ROWNUM: record.get('_EXCEL_ROWNUM'), // SOF112T
                                                                                                        // Key2
    ...
    var unregisteredSrmGrid = Unilite.createGrid('s_sof100ukrv_jwUnregisteredGrid', {
        // title: '기본',
        layout  : 'fit',
...
// UniAppManager.app.fnStockQ(dGridRecord);
...
// }
  ...
  /*        if(masterForm.getValue('NATION_INOUT') == 2 && Ext.isEmpty(masterForm.getValue('OFFER_NO'))) {
        ...
        onSaveDataButtonDown: function(config) {
            ...
            if(masterForm.getValue('NATION_INOUT') == 2 && Ext.isEmpty(masterForm.getValue('OFFER_NO'))) {
                alert('<t:message code="system.message.sales.datacheck003" default="OFFER번호는 필수입니다."/>');
                return true;
            }
            if(masterForm.getValue('NATION_INOUT') == 2){
                if(!panelTrade.getInvalidMessage()) return;
...
// }
            ...
            if(isDraftFlag === false)   { // 수동승인인 경우
                ...
                if(Ext.isEmpty(masterForm.getValue("APP_1_ID")))    {
                    alert('<t:message code="system.message.sales.datacheck004" default="1차 승인자는 필수 입력입니다."/>');
                    masterForm.getField("APP_1_NM").focus();
                    return false;
                }else if(amt > BsaCodeInfo.gsApp1AmtInfo)   {
                    if(Ext.isEmpty(masterForm.getValue("APP_2_ID")))    {
                        alert('<t:message code="system.message.sales.datacheck005" default="2차 승인자는 필수 입력입니다."/>');
                        masterForm.getField("APP_2_NM").focus();
                        return false;
                    }else  if(amt > BsaCodeInfo.gsApp2AmtInfo)  {
                        if(Ext.isEmpty(masterForm.getValue("APP_3_ID")))    {
                            alert('<t:message code="system.message.sales.datacheck006" default="3차 승인자는 필수 입력입니다."/>');
        ...
        onSaveDataButtonDown2: function(config) {   // masterForm.fnCreditCheck()
                                                    // 없는용..삭제시 호출
...
// }
        ...
        onDetailButtonDown: function() {
            var as = Ext.getCmp('s_sof100ukrv_jwAdvanceSerch');
            if(as.isHidden())   {
                as.show();
            } else {
                as.hide()
...
// }
        ...
        checkForNewDetail:function() {
            if(BsaCodeInfo.gsAutoType=='N' && Ext.isEmpty(masterForm.getValue('ORDER_NUM')))    {
                alert('<t:message code="system.label.sales.sono" default="수주번호"/>:<t:message code="system.message.sales.datacheck001" default="필수입력 항목입니다."/>');
                return false;
            }
...
//            var numDigitOfPrice = UniFormat.Price.length - UniFormat.Price.indexOf(".");
            ...
            if(sTaxInoutType=="1") {
                ...
                if(UserInfo.compCountry == 'CN') {
                    ...
                    //20191212 세액계산 수정: '2' -> CustomCodeInfo.gsUnderCalBase
                    dTaxAmtO   = UniSales.fnAmtWonCalc(dTaxAmtO, CustomCodeInfo.gsUnderCalBase, numDigitOfPrice);
                }
                ...
                if(UserInfo.compCountry == 'CN') {
                    ...
                    //20191212 세액계산 수정: '2' -> CustomCodeInfo.gsUnderCalBase
                    dTemp      = UniSales.fnAmtWonCalc((dAmountI / ( dVatRate + 100 )) * 100, CustomCodeInfo.gsUnderCalBase, numDigitOfPrice);
                    dTaxAmtO   = UniSales.fnAmtWonCalc(dTemp * dVatRate / 100, CustomCodeInfo.gsUnderCalBase, numDigitOfPrice);
                }
        ...
        fnCheckNum: function(value, record, fieldName, fieldLabel) {
            ...
            if(record.get("PRICE_YN") == "1" || record.get("ACCOUNT_YNC")=="N") {
                ...
                if(value < 0)   {
                    alert(fieldLabel+'<t:message code="system.message.sales.message031" default="은"/> '+'<t:message code="system.message.sales.message034" default="양수만 입력가능합니다."/>');
                    r=false;
                    return r;
                }else if(value == 0)    {
                    if(fieldName == "ORDER_TAX_O")  {
                        if(BsaCodeInfo.gsVatRate != 0)  {
                            alert(fieldLabel+': <t:message code="system.message.sales.datacheck009" default="0보다 큰 값이 입력되어야 합니다."/>');
                            r=false;
                        }
                    }else {
                        alert(fieldLabel+': <t:message code="system.message.sales.datacheck009" default="0보다 큰 값이 입력되어야 합니다."/>');
...
// params.rtnRecord.set('PRICE_TYPE',provider['PRICE_TYPE']);
        ...
        fnGetCustCredit: function(divCode, customCode, sDate, moneyUnit){
            ...
            s_sof100ukrv_jwService.getCustCredit(param, function(provider, response) {
            ...
            Ext.each(records, function(record,i){
                if(!Ext.isEmpty(record.get('DVRY_DATE'))) {
                    if(record.get('DVRY_DATE') < UniDate.getDbDateStr(masterForm.getValue('ORDER_DATE'))) {
                        alert('<t:message code="system.label.sales.deliverydate" default="납기일"/>: ' + '<t:message code="system.message.sales.message037" default="수주일 이후이어야 합니다."/>');
                        flag = false;
                        return false;
                    }else {
               ...
               // detailGrid.setExcelData(record.data);
                ...
                UniSales.fnGetItemInfo(newDetailRecords[i]
                                , UniAppManager.app.cbGetItemInfo
                                , 'I'
                                , UserInfo.compCode
                                , masterForm.getValue('CUSTOM_CODE')
...
// record.set('DISCOUNT_RATE', 0);
                ...
                case "TRANS_RATE" :    //입수
                    ...
                    if(dStockQ > 0) {
                        if(dStockQ > dOrderQ * lTrnsRate) {                 // 재고량 > 수주량
                            record.set('PROD_SALE_Q', 0);
                            record.set('PROD_Q', 0);
                            record.set('PROD_END_DATE', '');
                        } else if(dStockQ <= dOrderQ * lTrnsRate)   {       // 재고량 <= 수주량
                            dProdSaleQ= (dOrderQ * lTrnsRate - dStockQ) / lTrnsRate ;
                            dProdQ= (dOrderQ * lTrnsRate - dStockQ);
