...
function appMain() {
	...
	Unilite.defineModel('had619ukrEmpModel', {
    	fields: [
	...
	var directEmpStore = Unilite.createStore('had619ukrEmpStore', {
		model: 'had619ukrEmpModel',
		uniOpt: {
		...
		proxy: {
			type: 'uniDirect',
			api: {
				read   : 'had619ukrService.selectEmpList'
			}
		},
	...
	var empGrid = Unilite.createGrid('had619ukrEmpGrid', {
		layout	: 'fit',
		region	: 'center',
		flex	: 1,
		border	: 1,
		...
		fnSetClose : function() {
			...
			had619ukrService.fnSetClose(param, function(provider, response) {
    ...
    Unilite.defineModel('had619ukr600Model', {
        fields: [
               {name:'PERSON_NUMB'               	, text:'사번'  						, type:'string'}
     ...
     * @type
     */
    var direct600Store = Unilite.createStore('had619ukr600Store',{
            model: 'had619ukr600Model',
            autoLoad: false,
            folderSort: true,
            uniOpt : {
            ...
            proxy: Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
       			api: {
                       read : 'had619ukrService.selectFormHad600'
                       ,update : 'had619ukrService.update600'
                       ,create : 'had619ukrService.insert600'
                       ,destroy: 'had619ukrService.deleteAll'
                       ,syncAll: 'had619ukrService.save600All'
            ...
            listeners:{
                load: function ( store, records, successful, operation, eOpts )    {
                    ...
                    if(records && records.length > 0)    {
                        ...
                        //중소기업취업청년 감면세율 정보  중소기업청년 세액감면 액이 존재 할때만 표시 
                        /* if(!Ext.isEmpty(direct400Store.getAt(0)))	{
                        	if(Ext.isEmpty(records[0].get('YOUTH_DED_I')) || records[0].get('YOUTH_DED_I')==0)	{
	                        	var record400 = direct400Store.getAt(0);
	                        	if(record400.get("YOUTH_EXEMP_RATE") != 0 )	{
		                        	record400.set("YOUTH_EXEMP_RATE", 0);
		                        	var form400_tab10 = Ext.getCmp('tab10Form400');
		                        	form400_tab10.setActiveRecord(record400);
	                        	}
                        	}
                        } */
                ...
                update:function( store, record, operation, modifiedFieldNames, details, eOpts )    {
                    ...
                    //중소기업취업청년 감면세율 정보  중소기업청년 세액감면 액이 존재 할때만 표시 
                    /* if(!Ext.isEmpty(direct400Store.getAt(0)))	{
                    	if(Ext.isEmpty(record.get('YOUTH_DED_I')) || record.get('YOUTH_DED_I')==0)	{
                        	var record400 = direct400Store.getAt(0);
                        	if(record400.get("YOUTH_EXEMP_RATE") != 0 )	{
	                        	record400.set("YOUTH_EXEMP_RATE", 0);
	                        	var form400_tab10 = Ext.getCmp('tab10Form400');
            ...
            ,saveStore : function()    {
	            ...
	            if(list.length > 0)	{
	            	...
	            	if(me.getData() && me.getData().items)	{
	                	...
	                	Ext.getBody().mask();
		            	had619ukrService.update600_3(param, function(responseText, response){
		            		Ext.getBody().unmask();
		                    direct400Store.saveStore();
		            	})
	            	} else {
        ...
        Unilite.defineModel('had619ukr400Model', {
        fields: [
              {name:'PERSON_NUMB'                 ,text:'사번'                                ,type:'string'}
             ,{name:'NAME'                        ,text:'성명'                                ,type:'string'}
             ,{name:'DEPT_CODE'                   ,text:'부서코드'                             ,type:'string'}
        ...
        var direct400Store = Unilite.createStore('had619ukr400Store',{
            model: 'had619ukr400Model',
            autoLoad: false,
            folderSort: true,
            uniOpt : {
                isMaster: true,            // 상위 버튼 연결
            ...
            proxy: Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
                api: {
                    read : 'had619ukrService.selectFormHad400'
                    ,update : 'had619ukrService.update400'
                    ,create : 'had619ukrService.insert400'
                    ,syncAll: 'had619ukrService.save400All'
                }
            }),
            listeners:{
                load: function ( store, records, successful, operation, eOpts )    {
                    ...
                    if(records && records.length > 0)    {
                        ...
                        // ToDo : panel Result 에 had400t 조회 결과 매핑하고
                        //	소스 전체 뒤져서 panelSearch.getValue('PERSON_NUMB') =>
                        //			 panelResult.getValue('PERSON_NUMB') 로 변경해야 함.
                      ...
                      //중소기업취업청년 감면세율 정보  중소기업청년 세액감면 액이 존재 할때만 표시 
                        /* if(!Ext.isEmpty(direct600Store.getAt(0)))	{
                        	var record600 = direct600Store.getAt(0);
                        	if(Ext.isEmpty(record600.get('YOUTH_DED_I')) || record600.get('YOUTH_DED_I')==0)	{
                        		var record400 = store.getAt(0);
                        		if(record400.get("YOUTH_EXEMP_RATE") != 0)	{
                        			record400.set("YOUTH_EXEMP_RATE", 0);
                        			form400_tab10.setActiveRecord(record400);
                        		}
...
//						panelResult.setValue('HALFWAY_TYPE', newValue.HALFWAY_TYPE);
					}
				}
	        },{
				fieldLabel: '정산여부',
				...
				listeners: {
					change : function(rb, newValue, oldValue, options) {
//						panelResult.setValue('ADJUST_YN', newValue.ADJUST_YN);
					}
				}
	        }//,
...
//			listeners: {
//				change : function(rb, newValue, oldValue, options) {
//					panelSearch.setValue('HALFWAY_TYPE', newValue.HALFWAY_TYPE);
//				}
//			}
//        },{
//			fieldLabel: '정산여부',
...
//			listeners: {
//				change : function(rb, newValue, oldValue, options) {
//					panelSearch.setValue('ADJUST_YN', newValue.ADJUST_YN);
//				}
//			}
        }],
    ...
    var tab3 = Ext.create('Ext.container.Container',{
        ...
        items: [{
            ...
            layout: {
                type: 'table',
                columns:2, 
                tdAttrs:{style:'vertical-align:top;height:100%;height:255px;'},
                tableAttrs:{height:255}
            },
            scrollable: true,
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'&nbsp;국민연금', style: 'text-align:left;vertical-align:middle;',height:52},
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'&nbsp;주택임차차입금', style: 'text-align:left', rowspan: 2, tdAttrs: {width:100, style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'component', html:'&nbsp;원리금상환액(대출기관)', style: 'text-align:left', tdAttrs: {width:250, style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_TOT_I', holdable:'hold'},

                        { xtype: 'button', text:'원리금상환액(거주자) 입력', style: 'text-align:center', width: '90%', itemId:'btnHouseLoan',
                          handler:function()    {
                             if(panelResult.isValid())    {
                        ...
                        //{ xtype: 'component', html:'&nbsp;원리금상환액(거주자)', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;  '}},
                        //{ xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_TOT_I_2'},

                        { xtype: 'component', html:'&nbsp;장기주택</br>&nbsp;저당차입금</br>&nbsp;이자상환액', style: 'text-align:left;', rowspan: 9, tdAttrs: {style: 'border : 1px solid #ced9e7;  '}},
                        { xtype: 'component', html:'&nbsp;15년미만(600만원한도)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I_2', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;15년~29년(1,000만원한도)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7;  '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;30년이상(1,500만원한도)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7;  '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I_3', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;고정금리비거치상환대출', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7;  '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'FIXED_RATE_LOAN', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;기타대출(500만원한도)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'ETC_LOAN', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;15년이상(고정금리이면서 비거치상환)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I_6', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;15년이상(고정금리이거나 비거치상환)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I_7', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;15년이상(그밖의대출)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_AMOUNT_INTER_I_8', holdable:'hold'},

                            { xtype: 'component', html:'&nbsp;10년이상(고정금리이거나 비거치상환)', style: 'text-align:left;', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'&nbsp;신용카드(전통시장ㆍ대중교통 제외)', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7; '}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly: true, name: 'CARD_USE_I'},

                        { xtype: 'component', html:'&nbsp;직불카드등(전통시장ㆍ대중교통 제외)', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly: true, name: 'DEBIT_CARD_USE_I'},

                        { xtype: 'component', html:'&nbsp;현금영수증(전통시장ㆍ대중교통 제외)', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly: true, name: 'CASH_USE_I'},
                        
                        { xtype: 'component', html:'&nbsp;도서ㆍ공연ㆍ박물관ㆍ미술관 사용액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly: true, name: 'BOOK_CONCERT_USE_I'},

                        { xtype: 'component', html:'&nbsp;전통시장사용분', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly: true, name: 'TRA_MARKET_USE_I'},

                        { xtype: 'component', html:'&nbsp;대중교통이용분', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;', height:40}},
                        { xtype: 'component', html:'&nbsp;사용금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'대중교통을 신용카드, 직불ㆍ선불카드, 현금영수증으로 사용한 금액', style: 'text-align:left;', tdAttrs:{height:40}}
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'2017년', tdAttrs: { width:50}, rowspan: 2},
                        ...
                        { xtype: 'component', html:'2018년',  rowspan: 2},
                        ...
                        { xtype: 'component', html:'&nbsp;출연금', colspan:2},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'STAFF_STOCK_I', holdable:'hold'},

                        { xtype: 'component', html:'&nbsp;출연금(벤처기업)', colspan:2},
                        ...
                        { xtype: 'component', html:'&nbsp;고용유지중소기업<br/>&nbsp;근로자 소득공제', style: 'text-align:left',  height:40},
                        { xtype: 'component', html:'&nbsp;임금감소액', colspan: 2},
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'세액</br>감면', rowspan: 6, tdAttrs: {style: 'border : 1px solid #ced9e7;', width:50}},
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'세액<br/>감면', rowspan: 6, tdAttrs: {style: 'border : 1px solid #ced9e7;', width:50}},
                        { xtype: 'component', html:'&nbsp;소득세법', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;', width:150}},
                        ...
                        { xtype: 'component', html:'&nbsp;외국인기술자', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'SKILL_DED_I'},
                        { xtype: 'component', html:'외국인 기술자에 대한 소득세 면제', style: 'text-align:left'},

                        { xtype: 'component', html:'&nbsp;중소기업취업청년', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'YOUTH_DED_I'},
                        { xtype: 'component', html:'중소기업 취업 청년에 대한 소득세 감면 ', style: 'text-align:left'},

                        { xtype: 'component', html:'&nbsp;성과공유중소기업<br/>&nbsp;경영성과급', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;', height:38}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'MANAGE_RESULT_REDU_I'},
                        { xtype: 'component', html:'성과공유 중소기업에 종사하는 근로자가 수령하는 경영성과급 ', style: 'text-align:left'},

                        { xtype: 'component', html:'&nbsp;중소기업핵심인력<br/>&nbsp;성과보상기금 ', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;', height:38}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'CORE_COMPEN_FUND_REDU_I'},
                        ...
                        { xtype: 'component', html:'&nbsp;조세조약', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'button', text:'과학기술인공제 입력', style: 'text-align:center', tdAttrs: {height:58}, width: '90%', itemId:'btnPension',
                          handler:function()    {
                             if(panelResult.isValid())    {
                        ...
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'SCI_DEDUC_I', tdAttrs: {height:76}},

                        { xtype: 'button', text:'근로자퇴직급여보장법 입력', style: 'text-align:center',tdAttrs: {height:58}, width: '90%', itemId:'btnPension1',
                          handler:function()    {
                             if(panelResult.isValid())    {
                        ...
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'RETIRE_PENS_I', tdAttrs: {height:76}},

                        { xtype: 'button', text:'연금저축계좌 입력', style: 'text-align:center', tdAttrs: {height:58}, width: '90%', itemId:'btnPension2',
                          handler:function()    {
                             if(panelResult.isValid())    {
                        ...
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'PENS_I', tdAttrs: {height:76}}
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'&nbsp;정치자금기부금', rowspan: 2, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;', width:200}},
                        { xtype: 'component', html:'10만원이하', tdAttrs: {style: 'border : 1px solid #ced9e7;', width:150}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'POLICY_INDED'},

                        { xtype: 'component', html:'10만원초과', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'POLICY_GIFT_I'},

                        { xtype: 'component', html:'&nbsp;법정기부금', rowspan: 3, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'이월액(소득공제)', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'LEGAL_GIFT_I_PREV'},
                        
                        { xtype: 'component', html:'이월액(세액공제)', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'LEGAL_GIFT_I_PREV_14'},

                        { xtype: 'component', html:'기부금액', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'LEGAL_GIFT_I'},

                        /*{ xtype: 'component', html:'&nbsp;특례기부금</br>&nbsp;(공익신탁기부금제외)', rowspan: 2, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;  width: 6%'}},
                        { xtype: 'component', html:'전년도이월액', tdAttrs: {style: 'border : 1px solid #ced9e7;  width: 4%'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'PRIV_GIFT_I_PREV'},

                        { xtype: 'component', html:'기부금액', tdAttrs: {style: 'border : 1px solid #ced9e7;  width: 4%'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'PRIV_GIFT_I'},

                        { xtype: 'component', html:'&nbsp;공익신탁기부금', rowspan: 2, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;  width: 6%'}},
                        { xtype: 'component', html:'전년도이월액', tdAttrs: {style: 'border : 1px solid #ced9e7;  width: 4%'}},
                        ...
                        */
                        { xtype: 'component', html:'&nbsp;우리사주조합기부금', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'기부금액', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', readOnly:true, name: 'STAFF_GIFT_I'},

                        { xtype: 'component', html:'&nbsp;지정기부금(종교단체외)', rowspan: 3, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'이월액(소득공제)', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        ...
                        { xtype: 'component', html:'&nbsp;  종교단체기부금', rowspan: 3, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'이월액(소득공제)', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
            ...
            items: [
                {
                    ...
                    items:[
                        ...
                        { xtype: 'component', html:'&nbsp;납세조합공제', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'공제세액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'P3_TAX_DED_I', holdable: 'hold'},

                        { xtype: 'component', html:'&nbsp;주택차입금', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'이자상환액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'HOUS_INTER_I', holdable: 'hold'},

                        { xtype: 'component', html:'&nbsp;외국납부', rowspan: 2, style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'component', html:'소득금액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
                        { xtype: 'uniNumberfield', value:'0', suffixTpl: '원', name: 'FORE_INCOME_I', holdable: 'hold'},

                        { xtype: 'component', html:'납부세액', style: 'text-align:left', tdAttrs: {style: 'border : 1px solid #ced9e7;'}},
    ...
    var search2 = Unilite.createSearchForm('search2',{
        ...
        xtype: 'container',
         flex: 1,
        api: {
            load: 'had619ukrService.selectFormData02'
        },
        trackResetOnLoad: true,
        layout:{type:'vbox', align:'stretch', defaultMargins: '0 0 5 0'},
        items: [{
...
//        tabRotation: 0,
        items: [{
             ...
             ,id: 'had619ukrTab01'
             ,scrollable: true
             //,tabConfig:{style:{background:'#ccffcc'}}
             ,items:[tab1]
        },{
             title: '추가'	//	추가공제
             ,id: 'had619ukrTab02'
...
//    });
    ...
    Unilite.Main( {
        borderItems: [{
             ...
             border: false,
             items:[
					//panelResult, gridPanel, formPanel
        ...
        onDeleteDataButtonDown: function() {
            ...
            if(!Ext.isEmpty(mRow)) {
				...
				had619ukrService.deleteAll(param, function(responseText, response) {
					if(responseText){
        ...
        fnCollectData:function(cType){
               ...
               //Unilite.messageBox(infoForm400.getValue('HALFWAY_TYPE'));
               Ext.getBody().mask('Loading');
               had619ukrService.selectSummaryData(paramData, function(responseText, response){
        ...
        fnDisplayData1: function(record, sCollectType) {
            ...
            //일반정보
            record400.set('HOUSEHOLDER_YN',(record["HOUSEHOLDER_YN"]=="1" || record["HOUSEHOLDER_YN"]==true )? true:false )        ;        //세대주여부 1:세대주,2:세대주아님
            ...
            record400.set("HALFWAY_TYPE", (record["HALFWAY_TYPE"]=="Y"||record["HALFWAY_TYPE"]==true )? true:false);

			
            if(record["FOREIGN_NUM"] != "" )  {                                                                        //외국인단일세율(외국인여부)
                //foreSingleYn.setDisabled(false);
            	record400.set("FORE_SINGLE_YN", record["FORE_SINGLE_YN"]);
            } else {
                //foreSingleYn.setDisabled(true);
                record400.set("FORE_SINGLE_YN", false);
            }
			
            //record400.set("FOREIGN_DISPATCH_YN", record["FOREIGN_DISPATCH_YN"]);
            record400.set("FOREIGN_DISPATCH_YN", (record["FOREIGN_DISPATCH_YN"]=="Y"||record["FOREIGN_DISPATCH_YN"]==true)? true:false);    //외국법인소속 파견근로자 여부
            ...
            if(sCollectType == "QUERY" || sCollectType == "REFER" || sCollectType == "INCOME_DED" ) {
                ...
                record400.set("NOW_TAX_INVENTION_I"       , record["NOW_TAX_INVENTION_I"]);    //[숨김]주(현)직무발명보상금과세분
                ...
                record400.set("OLD_TAX_INVENTION_I"      , record["OLD_TAX_INVENTION_I"]) ;    //[숨김]종(전)직무발명보상금과세
            ...
            //그밖의소득공제-기타
            if(sCollectType == "QUERY") {
                record400.set("COMP_PREMINUM"    , (record["COMP_PREMINUM"]=="1" || record["COMP_PREMINUM"]==true) ? true:false);
...
//                 record400.set("INVESTMENT_I6"        , Unilite.nvl(record["INVESTMENT_I6"]         ,0));    //[입력]투자조합출자(2014년 50%해당분)
            ...
            if(sCollectType == "QUERY" || sCollectType == "REFER" )	{
	            record400.set("INVESTMENT_I"        , Unilite.nvl(record["INVESTMENT_I"]          ,0));    //[입력]투자조합출자(2012년이전 10%해당분)
	            record400.set("INVESTMENT_I2"        , Unilite.nvl(record["INVESTMENT_I2"]         ,0));    //[입력]투자조합출자(2012년 20%해당분)
	            record400.set("INVESTMENT_I3"        , Unilite.nvl(record["INVESTMENT_I3"]         ,0));    //[입력]투자조합출자(2012년 20%해당분)
	            record400.set("INVESTMENT_I4"        , Unilite.nvl(record["INVESTMENT_I4"]         ,0));    //[입력]투자조합출자(2013년 10%해당분)
	            record400.set("INVESTMENT_I2"        , Unilite.nvl(record["INVESTMENT_I2"]         ,0));    //[입력]투자조합출자(2013년 30%해당분)
	            record400.set("INVESTMENT_I5"        , Unilite.nvl(record["INVESTMENT_I5"]         ,0));    //[입력]투자조합출자(2014년 10%해당분)
            ...
            if(sCollectType == "QUERY") {
                ...
                record400.set("MANAGE_RESULT_REDU_I"          ,0);                                                    //성과공유중소기업경영성과급
                record400.set("CORE_COMPEN_FUND_REDU_I"          ,0);                                                    //중소기업핵심인력성과보상기금   
        ...
        fnSetClear1:function(sCollectType) {
            ...
            if(sCollectType == "QUERY" || sCollectType == "REFER" || sCollectType == "INCOME_DED" ) {
                ...
                record400.set("NOW_TAX_INVENTION_I"        , 0);    //[숨김]주(현)직무발명보상금과세분
                ...
                record400.set("OLD_TAX_INVENTION_I"         , 0);    //[숨김]종(전)직무발명보상금과세
        ...
        fnCalculateTax:function()    {
               ...
               Ext.getBody().mask('Loading');
               had619ukrService.selectCalculateTax(paramData, function(responseText, response){
                   console.log("responseText", responseText);
        ...
        fnDisplayData2:function(record)    {
            ...
            record600.set("HALFWAY_TYPE"              , (record["HALFWAY_TYPE"]=="Y" 		|| record["HALFWAY_TYPE"]==true) ? true:false);                                //[숨김]중도퇴사자정산
            record600.set("HOUSEHOLDER_YN"            , (record["HOUSEHOLDER_YN"]=="1" 		|| record["HOUSEHOLDER_YN"]==true) ? true:false);                            //[숨김]세대주여부    1:세대주, 2:세대주 아님
            record600.set("FOREIGN_DISPATCH_YN"       , (record["FOREIGN_DISPATCH_YN"]=="Y" || record["FOREIGN_DISPATCH_YN"]==true) ? true:false);                        //[숨김]외국법인소속 파견근로자 여부
            record600.set("FORE_SINGLE_YN"            , (record["FORE_SINGLE_YN"]=="Y" 		|| record["FORE_SINGLE_YN"]==true)? true:false);                            //[숨김]외국인단일세율
            ...
            record600.set("TAX_INVENTION_I"            , Unilite.nvl(record["TAX_INVENTION_I"]        ,0));    //[숨김]임원퇴직한도초과액
            ...
            record600.set("NOW_TAX_INVENTION_I"       , Unilite.nvl(record["NOW_TAX_INVENTION_I"]   ,0));    //[숨김]주(현)직무발명보상금과세분
            ...
            record600.set("OLD_TAX_INVENTION_I"       , Unilite.nvl(record["OLD_TAX_INVENTION_I"]   ,0));    //[숨김]종(전)직무발명보상금과세
            ...
            record600.set("MANAGE_RESULT_REDU_I"       , Unilite.nvl(record["MANAGE_RESULT_REDU_I"]           ,0));    //성과공유중소기업경영성과급
            record600.set("CORE_COMPEN_FUND_REDU_I"    , Unilite.nvl(record["CORE_COMPEN_FUND_REDU_I"]           ,0));    //중소기업핵심인력성과보상기금  
        ...
        fnSetClear2:function()    {
            ...
            record600.set("NOW_TAX_INVENTION_I"          , 0);    //[숨김]주(현)직무발명보상금과세분
            ...
            record600.set("OLD_TAX_INVENTION_I"       	 , 0);    //[숨김]종(전)직무발명보상금과세
            ...
            record600.set("MANAGE_RESULT_REDU_I"           , 0);    //성과공유중소기업경영성과급
            record600.set("CORE_COMPEN_FUND_REDU_I"        , 0);    //중소기업핵심인력성과보상기금
                ...
                taxCommunityWin= Ext.create('widget.uniDetailWindow', {
                    ...
                    items: [{
                            ...
                            items:[{
                                ...
                                xtype:'uniDetailForm',
                                itemId:'had619NapForm',
                                disabled:false,
                                height:340,
                                api:{
                                    load:had619ukrService.selectNap,
                                    submit:had619ukrService.napSyncMaster
                                },
                        ...
                        items :  [
                             {
                                ...
                                handler: function() {
                                    ...
                                    had619ukrService.napDeleteAll(searchForm.getValues(), function(){
                             ...
                             },
                             {
                                ...
                                handler: function() {
                                    var had619NapForm =  taxCommunityWin.down('#had619NapForm');
                                    if( had619NapForm.isDirty())          {
                                        var searchForm =  taxCommunityWin.down('#search');
                                        taxCommunityWin.mask();
                                        had619NapForm.submit({
                                            params: searchForm.getValues(),
                                            success:function()    {
                                                //Unilite.messageBox('저장되었습니다.');
                                            	taxCommunityWin.changes = true;
                                                taxCommunityWin.unmask();
                                                had619NapForm.resetDirtyStatus();
                                            },
                                            failure:function()    {
                                                taxCommunityWin.unmask();
                                            }
                    ...
                    listeners : {
                        ...
                        show: function( panel, eOpts )    {
                            ...
                            var form =  taxCommunityWin.down('#had619NapForm');
            ...
            if(!familyWin)    {
                ...
                Unilite.defineModel('had960ukrModel', {
                    fields: [
                        ...
                        {name: 'DEBIT_CARD_USE_I'       , text: '직불카드 등'            , type: 'uniPrice', defaultValue:0},
		             ...
		             , {name:'BOOK_CONCERT_CARD_I'          , text:'신용카드'                 , type:'uniPrice'    ,defaultValue:0}
		             , {name:'BOOK_CONCERT_CASH_I'          , text:'현금영수증'               , type:'uniPrice'    ,defaultValue:0}
		             , {name:'BOOK_CONCERT_DEBIT_I'         , text:'직불카드 등'               , type:'uniPrice'    ,defaultValue:0}
                    ]
                });
                            ...
                            //기타소득내역
                                Unilite.createGrid('had960ukrGrid', {
                                            ...
                                            handler:function()    {
                                                ...
                                                if(selRecord.length > 0)	{
                                                	/* if(selRecord[0].get('REL_CODE') == "0") {
                                                		Unilite.messageBox("근로자 본인은 삭제하실 수 없습니다.");
                                                		return;
                                                	} */
                                                    ...
                                                    if(!Ext.isEmpty(responseText))    {
                                                        if(confirm(Msg.fsbMsgH0360))    {
                                                            ...
                                                            had960ukrService.selectLast(form.getValues(), function(responseText, response){
                                                                if(Ext.isEmpty(responseText)) {
                                                                    if(confirm("당해년도 데이터가 존재합니다. 데이터를 삭제하고 실행하시겠습니까?"+"\n"+"단, 전년도내역에서 만20세 이하의 거주자 직계비속은 당해년도 기준으로 조정됩니다."))	{
                                    ...
                                    columns:  [
                                        ...
                                        {dataIndex: 'SERIOUS_SICK_MED_I', id:'med_gr3'   , width:100},
                                        {dataIndex: 'REAL_LOSS_MED_INSUR_I', id:'med_gr4'   , flex:1},
                                            ...
                                            switch(field)    {
                                                ...
                                                case "DEFAULT_DED_YN":
                                                    ...
                                                    if (relationCode == "4" || relationCode == "5" || relationCode == "6"  )    { //2-1) 직계비속, 형제자매
                                                        ...
                                                        if( dAge > dAgeLimit ) {
                                                            Unilite.messageBox(Msg.sMH9037);
                                                            record.set(field, false);
                                                            this.fnAutoSetSamePerson(record, field, false);
                                                            this.getView().refresh();
                                                            return;
                                                        ...
                                                        //'2-2) 직계존속
                                                            ...
                                                            if( gender == "1" || gender == "5" ) {
                                                                ...
                                                                if( dAge < dAgeLimit ) {
                                                                    Unilite.messageBox(Msg.sMH9038)
                                                                    record.set(field, false);
                                                                    this.fnAutoSetSamePerson(record, field, false);
                                                                    this.getView().refresh();
                                                                    return;
                                                            ...
                                                            if( gender == "2" || gender == "6" ) {
                                                                ...
                                                                if( dAge < dAgeLimit ) {
                                                                    Unilite.messageBox(Msg.sMH9039)
                                                                    record.set(field, false);
                                                                    this.fnAutoSetSamePerson(record, field, false);
                                                                    this.getView().refresh();
                                                                    return;
                                                        ...
                                                        //'2-3) 위탁아동
                                                            ...
                                                            if( dAge >= dAgeLimit ) {
                                                                Unilite.messageBox(Msg.fsbMsgH0297);
                                                                record.set(field, false);
                                                                this.fnAutoSetSamePerson(record, field, false);
                                                                this.getView().refresh();
                                                                return;
                                                ...
                                                case "BRING_CHILD_DED_YN": //'자녀양육비공제
                                                    ...
                                                    if (relationCode != "4" && relationCode != "8"  ) {	//	직계비속, 위탁아동, 입양자(?)
                                                    	if(sAfterValue) {
                                                    ...
                                                    if(!deformDedYn) {
                                                    	if( dAge <= 6 &&  sAfterValue ) {
	                                                    	if(confirm("자녀세액공제는 만 7세 이상 자녀(만 7세 미만 취학 아동 포함)만 가능합니다. 그래도 체크 하시겠습니까?"))	{
	                                                        	// 자녀양육비공제는 6세이하만 가능합니다
	                                                        	record.set(field, true);
	                                                    		this.fnAutoSetSamePerson(record, field, true);
	                                                    		this.getView().refresh();
		                                                        return ;
                                                ...
                                                case "WOMAN_DED_YN"    : //'부녀자공제
                                                    ...
                                                    if( gender != "2" && gender != "6" && sAfterValue ) {
                                                        Unilite.messageBox(Msg.fsbMsgH0106);
                                                        record.set(field, false);
                                                        this.fnAutoSetSamePerson(record, field, false);
                                                        this.getView().refresh();
                                                        return;
                                                    ...
                                                    if(baseForm400.getValue("EARN_INCOME_I") > 30000000) {
														if( sAfterValue  ) {
	                                                        Unilite.messageBox("총급여액이 3천만원 이하인 경우만 체크할 수 있습니다.");
	                                                        record.set(field, false);
	                                                        this.fnAutoSetSamePerson(record, field, false);
	                                                        this.getView().refresh();
	                                                        return;
                                                ...
                                                case "ONE_PARENT_DED_YN"://'한부모소득공제
                                                    ...
                                                    if( record.get("WOMAN_DED_YN") && sAfterValue) {
                                                        Unilite.messageBox(Msg.fsbMsgH0383);
                                                        record.set(field, false);
                                                        this.fnAutoSetSamePerson(record, field, false);
                                                        this.getView().refresh();
                                                        return;
                                                ...
                                                case "MANY_CHILD_DED_YN"://'다자녀추가공제
                                                    ...
                                                    if( record.get("LIVE_GUBUN") == "2" && sAfterValue) {
                                                        Unilite.messageBox(Msg.fsbMsgH0107);
                                                        record.set(field, false);
                                                        this.fnAutoSetSamePerson(record, field, false);
                                                        this.getView().refresh();
                                                        return;
                                                    ...
                                                    if( relationCode != "4"  && sAfterValue) {
                                                        Unilite.messageBox(Msg.fsbMsgH0280);
                                                        record.set(field, false);
                                                        this.fnAutoSetSamePerson(record, field, false);
                                                        this.getView().refresh();
                                                        return;
                                                    ...
                                                    if( dAge > dAgeLimit  &&  !record.get("DEFORM_DED_YN")  && sAfterValue) {
                                                        Unilite.messageBox(Msg.sMH1578);
                                                        record.set(field, false);
                                                        this.fnAutoSetSamePerson(record, field, false);
                                                        this.getView().refresh();
                                                        return;
                                    ...
                                    listeners:{
                                    	beforeedit:function(editor, context, eOpts)	{
                                    		...
                                    		if(familyWin.openType == 'E' && UniUtils.indexOf(context.field, ['REL_CODE']))	{
                                    			return false;
                                    		}
                                    		...
                                    		if(context.record.get("SEQ_NUM") =='2'  && UniUtils.indexOf(context.field, ['CASH_USE_I']))   {
                                    			return false;
                                    		}
                        ...
                        beforeshow: function( panel, eOpts )    {
                           ...
                           /* var validationEdu = Unilite.createValidator('validatorEdu', {
                            ...
                            if(gbCloseYn) {
                                ...
                                //grid.getColumn("BRING_CHILD_DED_YN").setDisabled(gbCloseYn);
                                //grid.getColumn("MANY_CHILD_DED_YN").setDisabled(gbCloseYn);
                            }
                            else {
                                grid.getColumn("HOUSEHOLDER_YN").setDisabled(gbCloseYn);
                                grid.getColumn("WOMAN_DED_YN").setDisabled(gbCloseYn);
            		...
            		validate: function( type, fieldName, newValue, oldValue, record, ePanel, editor, e) {
            			...
            			var rv = true;
                            ...
                            switch(fieldName)    {
                                ...
                                break;
                                case "REL_CODE":
                                	if(record.get("DEFAULT_DED_YN") && newValue =="3")	{
                                		var data = had960ukrStore.getData();
                                       	var message = "";
                                       	Ext.each(data.items, function(item, idx){
                                       		if(item.get("ONE_PARENT_DED_YN"))	{
                                       			message = "배우자공제와 한부모공제는 중복적용 할 수 없습니다.";
                                       		}
                                       	})
                                        if(message != "")	{
                                		...
                                		if(record.get("EDU_USE_I") != 0)	{
                                			record.set("EDU_USE_I", 0); 
                                			grid.fnAutoSetSamePerson(record.obj, "EDU_USE_I", 0);
                                		}
                                		if(record.get("EDU_CODE") != '')	{
                                			record.set("EDU_CODE", ''); 
                                			grid.fnAutoSetSamePerson(record.obj, "EDU_CODE", '');
                                		}
                                ...
                                case "EDU_CODE":
                                     if( sAfterValue == "" )   {
                                        ...
                                        if( !record.get("DEFORM_DED_YN") && sAfterValue == "5" )   {
                                            rv = Msg.fsbMsgH0142;
                                            record.set(fieldName, "");
                                            sAfterValue = "";
                                            grid.fnAutoSetSamePerson(record.obj,fieldName, "");
                                        }
...
//                                     if(sAfterValue == 0 && record.get("FIELD_EDUC_I") == 0)   {
//                                         record.set("EDU_CODE", "");
...
//                                     }
                                    ...
                                    /* if( sAfterValue == 0 &&
                                    	record.get("DEFORM_EDU_USE_I") == 0 &&
                                    	record.get("FIELD_EDUC_I") == 0 &&
                                    	record.get("EDU_PRINCIPAL_RTN_I") == 0 &&
                                    	record.get("UNIFORM_USE_I") == 0 )
                                    {
                                    	record.set("EDU_CODE", '');
                                    } */
                                break;
                                ...
                                case "BIRTH_ADOPT_CODE"://'출산입양자공제
                                    ...
                                    grid.fnAutoSetSamePerson(record.obj, fieldName, newValue);
                                    //record.set("BIRTH_ADOPT_DED_YN", !Ext.isEmpty(sAfterValue));
                        ...
                        if(inValidRecs.length == 0) {
                            config = {
                                        ...
                                        success: function(batch, option) {
                                            ...
                                            // 난임수술비, 의료비 반영
                                            //had619ukrService.updateMedI(medDocWin.paramData, function(){
                                            	medDocWin.unmask();
                                            //})
                                        }
                                };
                                            ...
                                            if(!me.familyPopupWin)    {
                                                ...
                                                var humanFamilyPopupStore = Unilite.createStore('humanFamilyPopupStore',{
                                                    ...
                                                    proxy: {
                                                        type: 'uniDirect',
                                                        api: {
                                                            read : 'had619ukrService.selectFamliy'
                                                        }
                                                    },
                                                    loadStoreRecords: function(){
...
//                                                                        }
                    ...
                    listeners : {
                        ...
                        beforeshow: function( panel, eOpts )    {
                            ...
                            had619ukrService.selectMedicalDed(medSummaryParams, function(responseText, response)    {
                                if(responseText)    {
                        ...
                        switch(fieldName) {
                            ...
                            case  "MED_CODE" :
                            	...
                            	if(newValue == "S" && record.get("REL_CODE") != "0") {	//	본인의료비
                        	  	...
                        	  	break;
                            /* case "SEND_USE_I" :
                            	if(record.get("MED_CODE") == "B") {
									if(newValue > 2000000) {
										Unilite.messageBox("산후조리원비는 200만원을 초과 입력할 수 없습니다.");
										record.set("SEND_USE_I", "2000000");
										return;
									}
                            	}
                            	break; */
            ...
            if(!pdfWin)    {
                ...
                var hadPdfStore = Unilite.createStore('hadPdfStore',{
                    ...
                    proxy: {
                        type: 'uniDirect',
                        api: {
                            read : 'had619ukrService.selectPdfList'
                            ,update : 'had619ukrService.updatePdf'
                            /*,create : 'had619ukrService.insert'
                            ,destroy : 'had619ukrService.delete'*/
                            ,syncAll: 'had619ukrService.saveAllPdf'
                        }
                ...
                pdfWin= Ext.create('widget.uniDetailWindow', {
                    ...
                    items: [{
                          ...
                          ]},
                          {
                            ...
                            xtype:'uniDetailForm',
                            api: {
                                submit: had619ukrService.pdfUploadFile
                            },
                            style:{
                                'background':'#fff'
                            },
                           ...
                           // margine:'3 3 3 3',
                            disabled:false,
                          	items:[
			                            	...
			                            	uploadForm.submit(
												{
													params: params,
												 	...
												 	success: function(form, action)	{
												 		pdfWin.mask();
												 		had619ukrService.pdfRead(action.params, function(responseText, response){
												 			pdfWin.unmask();
															if(!response.message) {
                                ...
                                height:140,
                                layout:{type:'uniTable', columns:7, tableAttrs:{cellspacing:5}},
                                margine:'3 3 3 3',
                                items:[{
                                    xtype:'component',
                                    colspan:7,
                                    ...
                                    listeners:{
                                    	render:function(c)	{
                                    		c.getEl().on('click', function(component){
                                    			hadPdfStore.refreshFilter('G407Y');
                                    			var grid = pdfWin.down('#hadPdfGrid');
                                    			grid.getColumn('DATA_NAME').hide();
                                    			grid.getColumn('CARD_TYPE_NAME').show();
                                    			grid.getColumn('SBDY_APLN_SUM').hide();
                            ...
                            api:{
                            	load:'had619ukrService.selectPDFSummary'
                            }
						},{
                            xtype:'container',
                            flex:1,
                    ...
                    applyData:function()	{
                    	pdfWin.mask();
                    	had619ukrService.spYearTaxPdf(pdfWin.paramData, function(responseText, response){
                    		console.log("responseText : ", responseText);
                    		console.log("response : ", response);
                    		Unilite.messageBox("PDF자료가 반영되었습니다.");
                    		pdfWin.unmask();
                    		pdfWin.mask();
                    		had619ukrService.batchSummaryData(pdfWin.batchParamData, function(){
                        		pdfWin.unmask();
