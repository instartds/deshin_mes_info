...
function appMain() {
	...
	var cbStore2 = Unilite.createStore('hat510ukrsComboStoreGrid2',{
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'hat500ukrService.getComboList2'
			}
		},
...
//			lazyRender	: true,
	 ...
	 * @type
	 */
	Unilite.defineModel('Hat500ukrModel', {
		fields: fields
	});//End of Unilite.defineModel('Hat500ukrModel', {

	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'hat500ukrService.selectList',
			//create	: 'hat500ukrService.updateList',
			update	: 'hat500ukrService.insertList',
			destroy	: 'hat500ukrService.deleteList',
			syncAll	: 'hat500ukrService.saveAll'
		}
	});
	 ...
	 * @type
	 */
	var store_pay_code0 = Unilite.createStore('hat500ukrMasterStore1', {
		model	: 'Hat500ukrModel',
		proxy	: directProxy,
		uniOpt	: {
...
/*
	...
	var storeForLoad = Unilite.createStore('hat500ukrMasterStore1', {
		model	: 'Hat500ukrModel',
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('hat500ukrGrid1', {
...
//					});
				...
				listeners : {
					...
					hide: function ( me, eOpts ) {
						if(me.fileIds != null && me.fileIds.length > 0) {
							...
							panelSearch.setValue('PGM_ID', 'hat500ukr');				  // 초기값 세팅

							var param = Ext.getCmp('searchForm').getValues();
							hat500ukrService.secomUpload(param, function(provider, response)  {
								if(!Ext.isEmpty(provider)) {
									panelSearch.setValue('FR_DATE', provider.RETURN_DATE);
									panelResult.setValue('FR_DATE', provider.RETURN_DATE);
									masterGrid.getEl().unmask();

									Unilite.messageBox('업로드 완료.');
									UniAppManager.app.onQueryButtonDown();
			...
			switch(fieldName) {
				case "WORK_TEAM" :
					if(!Ext.isEmpty(newValue) && dutyRule == 'Y' && Ext.isEmpty(record.get("DUTY_CODE")) && record.get("FLAG") == 'N') {
                    	...
                    	Ext.getBody().mask();
						hat500ukrService.getDutycodeTime(param, function(provider, response) {
                        	Ext.getBody().unmask();
							if(!Ext.isEmpty(provider)){
				...
				listeners	: {
					...
					click	: function(button, event, eOpts) {
						...
						if(confirm(closeYn + '<t:message code="system.message.human.message139" default="을(를) 진행하시겠습니까?"/>')) {
							...
							hat500ukrService.procClosing(param, function(provider, response) {
								if(response.result){
	...
	function checkValidaionGrid(store) {
		...
		var grid			= Ext.getCmp('hat500ukrGrid1');
	...
	var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create	: 'hat500ukrService.runClosing2',
			syncAll	: 'hat500ukrService.procClosingAll'
		}
	});
