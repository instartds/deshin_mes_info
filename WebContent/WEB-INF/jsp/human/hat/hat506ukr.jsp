...
var secomUploadWindow;	//SECOM 파일 업로드
function appMain() {
	...
	var cbStore2 = Unilite.createStore('hat510ukrsComboStoreGrid2',{
		autoLoad: false,
		uniOpt	: {
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'hat506ukrService.getComboList2'
			}
		},
...
//			lazyRender	: true,
	 ...
	 * @type
	 */
	Unilite.defineModel('Hat506ukrModel', {
		fields: fields
	});//End of Unilite.defineModel('Hat506ukrModel', {

	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'hat506ukrService.selectList',
			create	: 'hat506ukrService.updateList',
			update	: 'hat506ukrService.insertList',
			destroy	: 'hat506ukrService.deleteList',
			syncAll	: 'hat506ukrService.saveAll'
		}
	});
	 ...
	 * @type
	 */
	var store_pay_code0 = Unilite.createStore('hat506ukrMasterStore1', {
		model	: 'Hat506ukrModel',
		proxy	: directProxy,
		uniOpt	: {
...
//				dutyCodeStore.loadStoreRecords();
	...
	});//End of var store_pay_code0 = Unilite.createStore('hat506ukrMasterStore1', {
/*
	var directProxyLoad = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'hat506ukrService.selectList'
		}
	});
	var storeForLoad = Unilite.createStore('hat506ukrMasterStore1', {
		model	: 'Hat506ukrModel',
		proxy	: directProxyLoad,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
			deletable	: false,	// 삭제 가능 여부
		...
		listeners: {
			load: function(store) {
				if (this.getCount() > 0) {
					var record = store.getData();
					var loadData = Ext.Array.push(store.data.filterBy(function(record) {return record.get('FLAG') != 'N' } ).items);
					store_pay_code0.loadData(loadData);
					Ext.each(record.items, function(rec, idx){
						if(rec.get('FLAG') == 'N'){
							var newRecord =  Ext.create(store_pay_code0.model);
							if(rec.data)	{
								Ext.each(Object.keys(rec.data), function(key){
									newRecord.set(key, rec.data[key]);
								});
								newRecord.phantom = true;
								store_pay_code0.insert(idx, newRecord)
							}
						}
					});
					masterGrid.unmask();
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('hat506ukrGrid1', {
		...
		viewConfig:{
			getRowClass: function(record, rowIndex, rowParams, store){
				var cls = '';
			  	if((Ext.isEmpty(record.get('DUTY_FR_D')) && !Ext.isEmpty(record.get('DUTY_TO_D'))) || (!Ext.isEmpty(record.get('DUTY_FR_D')) && Ext.isEmpty(record.get('DUTY_TO_D')))){
...
//	 			}
		...
		setExcelData: function(record) {	//엑셀 업로드 참조
			...
			var r = {
				...
				'DUTY_YYYYMMDD' : record['DUTY_YYYYMMDD'],
				...
				'HIDDEN_FLAG'   : record['HIDDEN_FLAG']
				}
			var time = {}
			// 출퇴근시간
			if (dutyRule == 'Y') {
				time = {
					...
					'DUTY_FR_D'     : record['DUTY_FR_D'],
					'DUTY_FR_H'     : record['DUTY_FR_H'],
					'DUTY_FR_M'     : record['DUTY_FR_M'],
					'DUTY_TO_D'     : record['DUTY_TO_D'],
					'DUTY_TO_H'     : record['DUTY_TO_H'],
					'DUTY_TO_M'     : record['DUTY_TO_M'],
					'MODI_REASON'   : record['MODI_REASON'],
					'CLOSE_YN'      : record['CLOSE_YN']
					}
			// 근무시간		
			} else{
				time = {
				...
				Ext.each(colData, function(item, index){
					var tempData = {};
					tempData['TIMEF' + item.SUB_CODE] = record['TIMEF'+ item.SUB_CODE];
					tempData['TIMEC' + item.SUB_CODE] = record['TIMEC'+ item.SUB_CODE];
					tempData['TIMEN' + item.SUB_CODE] = record['TIMEN'+ item.SUB_CODE];
					tempData['TIMET' + item.SUB_CODE] = record['TIMET'+ item.SUB_CODE];
					tempData['TIMEM' + item.SUB_CODE] = record['TIMEM'+ item.SUB_CODE];
			       
			       $.extend(time, tempData);
				});
			}
	...
	});//End of var masterGrid = Unilite.createGrid('hat506ukrGrid1', {
...
//					});
	...
	// 엑셀 업로드 model
	Unilite.Excel.defineModel('excel.hat506ukr.sheet01', {
		fields: fields
	});

	// 엑셀 팝업 open
	function openSecomUploadWindow() {
		
		var me = this;
		var vParam = {};
		var appName = 'Unilite.com.excel.ExcelUpload';

		if(!secomUploadWindow) {
			secomUploadWindow =  Ext.WindowMgr.get(appName);
			secomUploadWindow = Ext.create( appName, {
					modal: false,
					excelConfigName: 'hat506ukr',
					extParam: {
						'PGM_ID': 'hat506ukr'
					},
					grids: [{
							...
							title: 'SECOM파일 업로드',
							useCheckbox: true,
							model : 'excel.hat506ukr.sheet01',
							readApi: 'hat506ukrService.selectExcelUploadSheet1',
							columns: /*columns0[createGridColumn(data);*/
								[{dataIndex: 'COMP_CODE'      , width: 93     , hidden: true},	// 법인코드
								{dataIndex: 'DIV_CODE'        , width: 93     , hidden: true},	// 법인코드
								{dataIndex: 'WORK_TEAM'	      , width: 100},   		  // 근무조
								{dataIndex: 'PERSON_NUMB'     , width: 100},      	  // 사번
								{dataIndex: 'NAME'	      	  , width: 100},          // 성명
								{dataIndex: 'DEPT_CODE'	      , width: 100    , hidden: true},  // 부서코드
								{dataIndex: 'DEPT_NAME'	      , width: 100},          // 부서
								{dataIndex: 'POST_CODE'       , width: 100},   		  // 직위코드
								{dataIndex: 'DUTY_YYYYMMDD'   , width: 100}, 		  // 근태일자
								{dataIndex: 'DUTY_CODE'  	  , width: 100    , hidden: true},  // 근태코드

								{dataIndex: 'DUTY_FR_D'  	  , width: 100},          // 출근시간
								{dataIndex: 'DUTY_FR_H'  	  , width: 100},          // 출근시간
								{dataIndex: 'DUTY_FR_M'  	  , width: 100},          // 출근시간
								{dataIndex: 'DUTY_TO_D'   	  , width: 100},          // 퇴근시간
								{dataIndex: 'DUTY_TO_H'   	  , width: 100},          // 퇴근시간
								{dataIndex: 'DUTY_TO_M'   	  , width: 100},          // 퇴근시간
								{dataIndex: 'MODI_REASON'     , width: 100},   		  // 수정사유
								{dataIndex: 'PAY_CODE'     	  , width: 100   , hidden: true},  // 급여지급방식
								{dataIndex: 'CLOSE_YN'     	  , width: 100   , hidden: true},  // 마감 여부
								{dataIndex: 'HIDDEN_FLAG'     , width: 100   , hidden: true}
					...
					onApply:function()  {
						...
						// masterGrid에 엑셀Grid 데이터세팅
						Ext.each(records, function(record,i){
							masterGrid.setExcelData(record.data);
					    });
					    
					    // 상단 조회조건 세팅
					    panelSearch.setValue('FR_DATE', records[0].get('DUTY_YYYYMMDD') );
					    panelSearch.setValue('DIV_CODE', records[0].get('DIV_CODE'));
					    panelResult.setValue('FR_DATE', records[0].get('DUTY_YYYYMMDD') );
					    panelResult.setValue('DIV_CODE', records[0].get('DIV_CODE'));
		...
		secomUploadWindow.center();
		secomUploadWindow.show();
			...
			switch(fieldName) {
				case "WORK_TEAM" :
					if(!Ext.isEmpty(newValue) && dutyRule == 'Y' && Ext.isEmpty(record.get("DUTY_CODE")) && record.get("FLAG") == 'N') {
                    	...
                    	Ext.getBody().mask();
						hat506ukrService.getDutycodeTime(param, function(provider, response) {
                        	Ext.getBody().unmask();
	...
	function createModelField(colData) {
		...
		if (dutyRule == 'Y') {
			fields.push({name: 'DUTY_CODE'	, text: '<t:message code="system.label.human.dutytype" default="근태구분"/>'	, type: 'string', store : Ext.StoreManager.lookup('hat510ukrsComboStore')});
			...
			fields.push({name: 'NUMC'		, text: '<t:message code="system.label.human.dutytype" default="근태구분"/>'	, type: 'string', store : Ext.StoreManager.lookup('hat510ukrsComboStore')});
		...
		if (dutyRule == 'Y') {
			columns.push({dataIndex: 'DUTY_CODE'		, width: 96, text: '<t:message code="system.label.human.dutytype" default="근태구분"/>', style: 'text-align: center', editor:
				...
				renderer: function (value, metaData, record, rowIndex, colIndex, store){
					var store = Ext.StoreManager.lookup('hat510ukrsComboStore');
				...
				listeners	: {
					...
					click	: function(button, event, eOpts) {
						...
						if(confirm(closeYn + '<t:message code="system.message.human.message139" default="을(를) 진행하시겠습니까?"/>')) {
							...
							hat506ukrService.procClosing(param, function(provider, response) {
								if(response.result){
	...
	function checkValidaionGrid(store) {
		...
		var grid			= Ext.getCmp('hat506ukrGrid1');
	...
	var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create	: 'hat506ukrService.runClosing2',
			syncAll	: 'hat506ukrService.procClosingAll'
		}
	});
