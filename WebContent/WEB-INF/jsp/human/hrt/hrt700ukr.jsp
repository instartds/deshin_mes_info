...
function appMain() {
	 ...
	 * @type 
	 */
	Unilite.defineModel('hrt700ukrModel', {
		fields : fields
	});
    ...
    */
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'hrt700ukrService.selectList',
			update  : 'hrt700ukrService.updateList',
			create  : 'hrt700ukrService.insertList',
			destroy : 'hrt700ukrService.deleteList',
			syncAll : 'hrt700ukrService.saveAll'
		}
	}); 
	 ...
	 * @type 
	 */
	var directMasterStore1 = Unilite.createStore('hrt700ukrMasterStore1',{
		model	: 'hrt700ukrModel',
		uniOpt	: {
	 ...
	 * @type 
	 */
	var masterGrid = Unilite.createGrid('hrt700ukrGrid1', {
...
//		}],
	...
	Unilite.Excel.defineModel('excel.hrt700ukr.sheet01', {
		fields: [
	...
	Unilite.Main( {
		...
		fnGetCloseAll : function() {
			...
			hrt700ukrService.fnGetCloseAll(param, function(provider, response) {
				...
				if(!Ext.isEmpty(provider)) {
					...
					if(bCloseAllYN == "Y") {
						...
						if(Ext.getStore('hrt700ukrMasterStore1').getCount() > 0)
							UniAppManager.setToolbarButtons(['deleteAll'],true);
					}
				}
			});
		...
		fnSetCloseAll : function(closeType) {
			...
			hrt700ukrService.fnSetCloseAll(param, function(provider, response) {
				console.log("provider : ", provider);
				if(provider){
					if(closeType == 'Y')
		...
		if(!excelWindow) { 
			...
			excelWindow = Ext.create( appName, {
				...
				grids: [{							//팝업창에서 가져오는 그리드
						...
						title		: '엑셀업로드',
						useCheckbox	: false,
						model		: 'excel.hrt700ukr.sheet01',
						readApi		: 'hrt700ukrService.selectExcelUploadSheet1',
						columns		: [ 
				...
				onApply:function()  {
					...
					if (!Ext.isEmpty(records)) {
						...
						excelUploadFlag = "Y"
						hrt700ukrService.selectExcelUploadSheet1(param, function(provider, response){
