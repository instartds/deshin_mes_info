...
//                        alert(Ext.getCmp('optPrintGb').getValue().optPrint);
                        ...
                        hum973rkrService.fnHum970ini(param, function(provider, response){           // 증명번호 최신화
...
//						if(Ext.getCmp('optPrintGb').getChecked()[0].inputValue == '3') {
	...
	Unilite.Main({
		...
		fnInitBinding : function() {
			...
			panelResult2.setValue('SEQ_COUNT',1);
...
//			var param = {"S_COMP_CODE": UserInfo.compCode };
        ...
        onPrintButtonDown: function() {
			...
			param.ENCRYPT = Ext.getCmp('rdoEncrypt').getChecked()[0].inputValue;
            param.PGM_ID = 'hum973rkr';  //프로그램ID
            param.sTxtValue2_fileTitle = '경력증명서';
            
			if(doc_Kind == '0') {
            	param.sTxtValue2_fileTitle = '경력증명서';
				
				var win = Ext.create('widget.ClipReport', {
					url: CPATH+'/human/s_hum970clrkrv_sd.do',
					prgID: 's_hum970rkr_sd',
					extParam: param
				});
				win.center();
				win.show();
			...
			if(doc_Kind == '1') {
				...
				param.sTxtValue2_fileTitle = '재직증명서';
				var win = Ext.create('widget.ClipReport', {
					url: CPATH+'/human/s_hum973clrkrv_sdc.do',
					prgID: 'hum973rkr',
					extParam: param
				});
	            win.center();
			...
			if(doc_Kind == '2') {
				...
				var seqCount = 0;
				if(Ext.isEmpty(param2.SEQ_COUNT)){
					seqCount = 0;
				}else{
					seqCount = param2.SEQ_COUNT;
				}
...
//				   DOC_KIND		: Ext.getCmp('optPrintGb').getChecked()[0].inputValue,		  // bParam(0) // 증명서 종류 
			...
			/*hum973rkrService.fnHum970nQ(bParam, function(provider, response){			// 증명번호 최신화
				if(!Ext.isEmpty(provider)  && provider == true){
					var win = Ext.create('widget.PDFPrintWindow', {
			            url: CPATH+'/hum/hum973rkrPrint.do',
			            prgID: prgID1,
			            extParam: bParam
		            });
		            win.center();
...
/*		onResetButtonDown: function() {
	    ...
	    fnUpdateChanges : function() {
	    	...
	    	param.CERTI_TYPE = Ext.getCmp('optPrintGb').getValue().optPrint;
//	    	var param  = Ext.getCmp('resultForm').getValues();			// 상단 증명서 종류 
...
//	    	param.optPrintGb = bParam0;
	    	...
	    	hum973rkrService.insertDetail(param, function(provider, response){
	    		hum973rkrService.fnHum970ini(param, function(provider2, response){
