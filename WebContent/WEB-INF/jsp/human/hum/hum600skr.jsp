...
function appMain() {
	 ...
	 */
	//1: 해당자별
	Unilite.defineModel('hum600skrModel1', {
		fields: [
	...
	//2: 만기도래현황별
	Unilite.defineModel('hum600skrModel2', {
		fields: [
			{name: 'COMP_CODE'		 	,text: 'COMP_CODE'				,type: 'string'},
			{name: 'DIV_CODE'			,text: '사업장코드'				,type: 'string'},
			{name: 'PERSON_NUMB'		,text: '사원번호'					,type: 'string'},
	 ...
	 */			
	//1: 해당자별		
	var masterStore1 = Unilite.createStore('hum600skrMasterStore1',{
		model	: 'hum600skrModel1',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {			
					read: 'hum600skrService.selectList'
			}
		},
	...
	//2: 만기도래현황별	
	var masterStore2 = Unilite.createStore('hum600skrMasterStore2',{
		model	: 'hum600skrModel2',
		uniOpt	: {
			isMaster	: true,				// 상위 버튼 연결 
			editable	: false,			// 수정 모드 사용 
			deletable	: false,			// 삭제 가능 여부 
...
//							dataForm.setValue('S_DEPT_CODE', records[0].DEPT_CODE);
	 ...
	 */
	//1: 해당자별
	var masterGrid1 = Unilite.createGrid('hum600skrGrid1', {
		store	: masterStore1,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	//2: 만기도래현황별
	var masterGrid2 = Unilite.createGrid('hum600skrGrid2', {
		store	: masterStore2,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	var tab = Unilite.createTabPanel('hum600skrTab',{		
		region		: 'center',
		activeTab	: 0,
		border		: false,
		items		: [{
	...
	Unilite.Main({
		...
		onQueryButtonDown : function()	{
			...
			//1: 해당자별
			if (activeTab == 'hum600skrTab1'){
				masterStore1.loadStoreRecords();

			//2: 만기도래현황별
			} else if (activeTab == 'hum600skrTab2'){
