...
<t:appConfig pgmId="s_hum520skr_kva">
...
function appMain() {
	 ...
	 */
	//1: 근속년수별
	Unilite.defineModel('s_hum520skr_kvaModel1', {
		fields: [
	...
	//2: 연령별
	Unilite.defineModel('s_hum520skr_kvaModel2', {
		fields: [
			{name: 'COMP_CODE'		 	,text: 'COMP_CODE'		,type: 'string'},
			{name: 'DEPT_CODE'			,text: '부서코드'			,type: 'string'},
			{name: 'DEPT_NAME'			,text: '부서명'			,type: 'string'},
	...
	//3: 학력별
	Unilite.defineModel('s_hum520skr_kvaModel3', {
		fields: [
			{name: 'COMP_CODE'		 	,text: 'COMP_CODE'		,type: 'string'},
			{name: 'DEPT_CODE'			,text: '부서코드'			,type: 'string'},
			{name: 'DEPT_NAME'			,text: '부서명'			,type: 'string'},
	...
	//4: 부서별
	Unilite.defineModel('s_hum520skr_kvaModel4', {
		fields: [
			{name: 'COMP_CODE'		 	,text: 'COMP_CODE'		,type: 'string'},
			{name: 'DEPT_CODE'			,text: '부서코드'			,type: 'string'},
			{name: 'DEPT_NAME'			,text: '부서명'			,type: 'string'},
	 ...
	 */			
	//1: 근속년수별		
	var masterStore1 = Unilite.createStore('s_hum520skr_kvaMasterStore1',{
		model	: 's_hum520skr_kvaModel1',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {			
					read: 's_hum520skr_kvaService.selectList'
			}
		},
	...
	//2: 연령별	
	var masterStore2 = Unilite.createStore('s_hum520skr_kvaMasterStore2',{
		model	: 's_hum520skr_kvaModel2',
		uniOpt	: {
			isMaster	: true,				// 상위 버튼 연결 
			editable	: false,			// 수정 모드 사용 
			deletable	: false,			// 삭제 가능 여부 
	...
	//3: 학력별	
	var masterStore3 = Unilite.createStore('s_hum520skr_kvaMasterStore3',{
		model	: 's_hum520skr_kvaModel3',
		uniOpt	: {
			isMaster	: true,				// 상위 버튼 연결 
			editable	: false,			// 수정 모드 사용 
			deletable	: false,			// 삭제 가능 여부 
	...
	//4: 부서별
	var masterStore4 = Unilite.createStore('s_hum520skr_kvaMasterStore4',{
		model	: 's_hum520skr_kvaModel4',
		uniOpt	: {
			isMaster	: true,				// 상위 버튼 연결 
			editable	: false,			// 수정 모드 사용 
			deletable	: false,			// 삭제 가능 여부 
...
//							dataForm.setValue('S_DEPT_CODE', records[0].DEPT_CODE);
	 ...
	 */
	//1: 근속년수별
	var masterGrid1 = Unilite.createGrid('s_hum520skr_kvaGrid1', {
		store	: masterStore1,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	//2: 근속년수별
	var masterGrid2 = Unilite.createGrid('s_hum520skr_kvaGrid2', {
		store	: masterStore2,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	//3: 학력별
	var masterGrid3 = Unilite.createGrid('s_hum520skr_kvaGrid3', {
		store	: masterStore3,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	//4: 부서별
	var masterGrid4 = Unilite.createGrid('s_hum520skr_kvaGrid4', {
		store	: masterStore4,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {	
	...
	var tab = Unilite.createTabPanel('s_hum520skr_kvaTab',{		
		region		: 'center',
		activeTab	: 0,
		border		: false,
		items		: [{
		...
		listeners:{
			tabchange: function( tabPanel, newCard, oldCard, eOpts )	{
				if(newCard.getItemId() == 's_hum520skr_kvaTab1')	{
					masterStore1.loadStoreRecords();
				}else if(newCard.getItemId() == 's_hum520skr_kvaTab2') {
                    masterStore2.loadStoreRecords();
                }else if(newCard.getItemId() == 's_hum520skr_kvaTab3') {
	...
	Unilite.Main({
		...
		onQueryButtonDown : function()	{
			...
			//1: 근속년수별
			if (activeTab == 's_hum520skr_kvaTab1'){
				masterStore1.loadStoreRecords();

			//2: 연령병
			} else if (activeTab == 's_hum520skr_kvaTab2'){
				masterStore2.loadStoreRecords();
				
			//3: 학력별 
			} else if (activeTab == 's_hum520skr_kvaTab3'){
				masterStore3.loadStoreRecords();
