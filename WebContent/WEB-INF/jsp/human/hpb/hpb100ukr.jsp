...
<t:appConfig pgmId="hpb100ukr"  >
	<t:ExtComboStore comboType="AU" comboCode="A118" /> <!-- 소득자타입 -->
	<t:ExtComboStore comboType="AU" comboCode="HS04" /> <!-- 사업소득구분 -->
	...
	<t:ExtComboStore comboType="AU" comboCode="H238" /> <!-- 업무구분 -->
	<t:ExtComboStore comboType="BOR120"  pgmId="hpb100ukr"  /> 			<!-- 사업장 -->	
...
function appMain() {     
    ...
    Unilite.defineModel('hpb100ukrGubunModel', {
	    fields: [ 
	...
	var GetGubunStore = Unilite.createStore('hpb100ukrGubunStore',{
		model:'hpb100ukrGubunModel',
        proxy: {
           type: 'direct',
            api: {			
                read: 'hpb100ukrService.getGubun'                	
            }
        },
        uniOpt: {
	 ...
	 * @type 
	 */
	Unilite.defineModel('hpb100ukrModel', {
	   fields: [
			...
			{name: 'NATION_CODE'			, text: '국적'				, type: 'string',allowBlank:false},
			...
			{name: 'DWELLING_CODE'			, text: '거주지국'				, type: 'string',allowBlank:false},
			...
			{name: 'BUSI_TYPE'				, text: '업무구분'				, type: 'string',  comboType :'AU' ,   comboCode:'H238'},
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
        	read : 'hpb100ukrService.selectDetailList',
        	update: 'hpb100ukrService.updateDetail',
			create: 'hpb100ukrService.insertDetail',
			destroy: 'hpb100ukrService.deleteDetail',
			syncAll: 'hpb100ukrService.saveAll'
        }
	});
	 ...
	 * @type 
	 */					
	var directMasterStore = Unilite.createStore('hpb100ukrMasterStore1',{
		model: 'hpb100ukrModel',
		uniOpt: {
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('hpb100ukrGrid1', {
    	itemId:'masterGrid',
    	layout : 'fit',
        region : 'west',
        flex  : 1,
        ...
        columns: [        
			...
			{dataIndex: 'DWELLING_CODE'			, width: 100 , hidden :true },
 	    ...
 	    listeners: {
 	    	...
 	    	selectionchangerecord:function(selected)	{
          		...
          		/*
          		if(!Ext.isEmpty(selected.data.REPRE_NUM)){
          			detailForm.setValue('REPRE_NUM_EXPOS', '***************');
          		}else{
          			detailForm.setValue('REPRE_NUM_EXPOS', '');
          		}
          		
          		if(!Ext.isEmpty(selected.data.FOREIGN_NUM)){
    ...
    var detailForm = Unilite.createForm('hpb100ukrDetail', {
    	disabled :false,
		...
		Unilite.popup('CUST', {
			...
			name: 'USER_YN'
		 	...
		 	width: 510
	  	 	...
	  	 	padding: 20,
	  	 	layout: {type:'uniTable', columns:1,
	  	 		tableAttrs:{style: 'border : 1px solid #99bce8; padding: 10px;'}
	  	 	},
	  	 	colspan: 2,
	  	 	items:[{
	  	 		xtype: 'component',
	  	 		...
	  	 		xtype: 'component',
	  	 		html: '1. 비거주자의 경우 영문성명 기재 시 여권상의 영문 성명 전부를 기재하십시오.'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '&nbsp;'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '2. 비거주자의 경우 외국법인은 영문 법인명 기재 시 정식명칭 전부를 기재하십시오.'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '&nbsp;&nbsp;&nbsp;&nbsp;일반적으로 머리글자를 사용하는 경우에는 머리글자 뒤에 괄호로 정식명칭 전부를 기재하십시오.'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '&nbsp;'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '3. 영문 주소는 번지(number), 거리(street), 시(city), 도(state), 우편번호(postal zone), 국가(country)'
	  	 	},{
	  	 		xtype: 'component',
	  	 		html: '&nbsp;&nbsp;&nbsp;&nbsp;순으로 기재해 주십시오.'
	  	 	}]
...
//			var record = this;
//							
//			var params = {'REPRE_NO':detailForm.getValue('REPRE_NUM'), 'GUBUN_FLAG': '3', 'INPUT_YN': 'Y'};
...
//			var record = this;
//							
//			var params = {'FOREIGN_NUM':detailForm.getValue('FOREIGN_NUM'), 'GUBUN_FLAG': '4', 'INPUT_YN': 'Y'};
...
//				var params = {'BANK_ACCOUNT':detailForm.getValue('BANK_ACCOUNT'), 'GUBUN_FLAG': '2', 'INPUT_YN': 'Y'};
