...
// 	console.log(colData);
	 ...
	 * @type 
	 */
	Unilite.defineModel('Hpa956skrModel', {
	    fields: fields
	 ...
	 * @type 
	 */					
	var masterStore = Unilite.createStore('hpa956skrMasterStore',{
		model: 'Hpa956skrModel',
		uniOpt: {
        ...
        proxy: {
            type: 'direct',
            api: {			
                read: 'hpa956skrService.selectList'                	
            }
        }/*, //(사용 안 함 : 쿼리에서 처리)
        listeners : {
	        load : function(store) {
...
//			width:300,
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('hpa956skrGrid1', {
    	layout	: 'fit',
    	region	: 'center',
    	store	: masterStore,
        columns	: columns,
	...
	Unilite.Main({
		...
		onQueryButtonDown : function()	{		
			if(!UniAppManager.app.checkForNewDetail()){
				...
				var param = Ext.getCmp('searchForm').getValues();
				hpa956skrService.selectColumns2(param, function(provider, response) {
...
/*	// Grid 의 summary row의  표시 /숨김 적용 (사용 안 함 : 쿼리에서 처리)
	...
	function createModelField(colData) {
		...
		var fields = [
			...
			{name: 'TAX_EXEMPTION_I'	, text: '<t:message code="system.label.human.taxexemptiontoti" default="비과세총액"/>'	, type: 'uniPrice'},
	...
	function createGridColumn(colData) {
		...
		var columns = [
   			{dataIndex: 'COMP_CODE'				, text: 'COMP_CODE'		, width: 86		, locked: false, hidden:true},
//   			{dataIndex: 'PAY_YYYYMM'		, text: '<t:message code="system.label.human.payyyyymm" default="급여년월"/>'		, width: 86		, locked: false	, style: 'text-align: center', 
...
//	            }
//			}, 				
//			{dataIndex: 'SUPP_NAME'			, text: '<t:message code="system.label.human.supptype" default="지급구분"/>'		, width: 100	, locked: false	, style: 'text-align: center',
//				renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
...
//	            }
//			}, 		
			{dataIndex: 			'DIV_CODE'			, text: '<t:message code="system.label.human.division" default="사업장"/>'		, width: 120	, locked: false , style: 'text-align: center'},				
			...
			{dataIndex: 			'PERSON_NUMB'		, text: '<t:message code="system.label.human.personnumb" default="사번"/>'		, width: 100	, locked: false	, style: 'text-align: center'},
			{dataIndex: 			'NAME'				, text: '<t:message code="system.label.human.name" default="성명"/>'				, width: 80	, locked: false	, style: 'text-align: center'},
						
			Ext.applyIf({dataIndex: 'SUPP_TOTAL_I'		, text: '<t:message code="system.label.human.payamounti" default="지급총액"/>'		, width: 110	, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			...
			Ext.applyIf({dataIndex: 'REAL_AMOUNT_I'		, text: '<t:message code="system.label.human.realamounti" default="실지급액"/>'	, width: 110	, locked: false	, style: 'text-align: center',
				renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
			...
			//TAX_EXEMPTION_I
			Ext.applyIf({dataIndex: 'TAX_EXEMPTION_I'	, text: '<t:message code="system.label.human.taxexemptiontoti" default="비과세총액"/>'	, width: 100		, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex: 'BUSI_SHARE_I'		, text: '<t:message code="system.label.human.socbusisharei" default="사회보험부담금"/>'	, width: 110		, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex: 'WORKER_COMPEN_I'	, text: '<t:message code="system.label.human.workercompeni" default="사업자산재보험부담금"/>'	, width: 145	, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price })
		];
