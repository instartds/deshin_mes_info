...
<t:appConfig pgmId="s_pmr350skrv_sh"  >
...
function appMain() {
	 ...
	 * @type 
	 */
	Unilite.defineModel('s_pmr350skrv_shModel', {
		fields : fields
	}); //End of Unilite.defineModel('s_pmr350skrv_shModel', {
	 ...
	 * @type 
	 */
	var directMasterStore1 = Unilite.createStore('s_pmr350skrv_shMasterStore1',{
		model	: 's_pmr350skrv_shModel',
		autoLoad: false,
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api: {
				read: 's_pmr350skrv_shService.selectList'
			}
		},
	 ...
	 * @type 
	 */
	var masterGrid1 = Unilite.createGrid('s_pmr350skrv_shGrid1', {
...
//				'x-change-cell_light', 'x-change-cell_normal', 'x-change-cell_dark'
	...
	});	//End ofvar masterGrid1 = Unilite.createGrid('s_pmr350skrv_shGrid1', {



	Unilite.Main({
		id			: 's_pmr350skrv_shApp',
		borderItems	: [{
			region	: 'center',
			layout	: 'border',
			border	: false,
	...
	function createModelField(colData) {
		var fields = [
			...
			{name: 'MOLD_CODE'     	,text: '금형번호'	, type: 'string'},
			...
			{name: 'BAD_Q'			,text: '<t:message code="system.label.product.defectitemqty" default="분류소계"/>'			,type: 'uniQty'},
			{name: 'BAD_RATE'		,text: '<t:message code="system.label.product.defectrateppm" default="불량률"/>'		,type: 'uniER'},
	...
	function createGridColumn(colData) {
		...
		var columns = [
			...
			{dataIndex: 'WORK_SHOP_NAME'	, width: 60	, locked: true, align: 'center'},
			{dataIndex: 'PRODT_DATE'		, width: 90		, locked: true	, align: 'center'},
			{dataIndex: 'PROG_WORK_CODE'	, width: 80		, hidden: true},
			{dataIndex: 'PROG_WORK_NAME'	, width: 80		, locked: true, align: 'center'},
			{dataIndex: 'EQUIP_NAME'	    , width: 80		, locked: true, align: 'center'},
			{dataIndex: 'MOLD_CODE'	        , width: 80		, locked: true},			
			{dataIndex: 'WKORD_NUM'	        , width: 100	, locked: true, align: 'center'},
...
//		array1[2] = {dataIndex: 'BAD_AMT'	, width:95 ,summaryType: 'sum'};
		array1[1] = {dataIndex: 'BAD_AMT'	, width:95 ,summaryType: 'sum', hidden: true};
		Ext.each(colData, function(item, index){
			if(index == 0){
