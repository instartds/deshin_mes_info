...
<t:appConfig pgmId="s_otr110ukrv_sh"  >
...
//    });
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read    : 's_otr110ukrv_shService.selectMaster',
			update  : 's_otr110ukrv_shService.updateDetail',
			create  : 's_otr110ukrv_shService.insertDetail',
			destroy : 's_otr110ukrv_shService.deleteDetail',
			syncAll : 's_otr110ukrv_shService.saveAll'
		}
	});
    ...
    var searchForm = Unilite.createSearchForm('s_otr110ukrv_shSearchForm', {	// 메인
        region  : 'north',
	...
	var masterForm = Unilite.createSearchPanel('s_otr110ukrv_shMasterForm', {	// 메인
    ...
    var otherorderSearch = Unilite.createSearchForm('otherorderForm', {		//예약참조
            ...
            items :[
				...
				Unilite.popup('DIV_PUMOK',{
                    ...
                    colspan		: 2				//20200226 수정: 검색 필드 추가로 디자인 상 수정
				}),{
					fieldLabel: '<t:message code="system.label.purchase.subcontractor" default="외주처"/>',
					xtype: 'uniTextfield',
					name: 'CUSTOM_CODE',
					...
					xtype: 'uniTextfield',
					name: 'ORDER_NUM'
				},{	//20200226 추가: 예약참조 팝업 검색조건 "수주번호" 추가
					fieldLabel: '수주번호',
					xtype:'uniTextfield',
					name:'S_ORDER_NUM'
				}
	...
	Unilite.defineModel('S_otr110ukrv_shModel', {		// 메인
	    fields: [
...
//	    	{name: 'WH_CELL_NAME'		         ,text: '<t:message code="system.label.purchase.issuewarehousecell" default="출고창고Cell"/>' 	,type: 'string',maxLength:20},
	...
	Unilite.defineModel('S_otr110ukrv_shOTHERModel', {	//예약참조
	    fields: [
	    	{name: 'ORDER_NUM'		 	    	, text: '<t:message code="system.label.purchase.pono" default="발주번호"/>'    		, type: 'string'},
			...
			{name: 'SOF_CUSTOM_NAME'			,text: '수주처명'			,type:'string'},
			{name: 'SOF_ITEM_NAME'				,text: '수주품목명'			,type:'string'},
			{name: 'OUT_SEQ'					,text: '<t:message code="system.label.purchase.outseq" default="출고순번"/>'			,type:'string'},
	...
	Unilite.defineModel('S_otr110ukrv_shOTHERModel2', {	//반품가능예약참조
	    fields: [
	    	{name: 'ORDER_NUM'		 			, text: '<t:message code="system.label.purchase.pono" default="발주번호"/>'    		, type: 'string'},
	    	{name: 'ORDER_SEQ'		 			, text: '<t:message code="system.label.purchase.poseq" default="발주순번"/>'    		, type: 'string'},
	    	{name: 'ORDER_DATE'					, text: '<t:message code="system.label.purchase.podate" default="발주일"/>'    		, type: 'uniDate'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('S_otr110ukrv_shMasterStore1',{		// 메인
		model: 'S_otr110ukrv_shModel',
		uniOpt: {
...
//                }
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('s_otr110ukrv_shGrid');
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {	// 검색버튼 조회창
            ...
            proxy: {
                type: 'direct',
                api: {
                	read: 's_otr110ukrv_shService.selectDetail'
                }
            },
            loadStoreRecords : function()	{
	...
	var otherOrderStore = Unilite.createStore('s_otr110ukrv_shOtherOrderStore', {	//예약참조
			model: 'S_otr110ukrv_shOTHERModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read : 's_otr110ukrv_shService.selectDetail2'
                }
            },
            listeners:{
            		   		...
            		   		Ext.each(records,
	            		   		function(item, i) {
			   						Ext.each(masterRecords.items, function(record, i) {
			   								...
			   								&& (record.data['BASIS_SEQ'] == item.data['OUT_SEQ'])
			   								){
	...
	var otherOrderStore2 = Unilite.createStore('s_otr110ukrv_shotherOrderStore2', {//반품가능예약참조
			model: 'S_otr110ukrv_shOTHERModel2',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read    : 's_otr110ukrv_shService.selectDetail3'
                }
            },
            listeners:{
            	load:function(store, records, successful, eOpts) {
    ...
    var masterGrid = Unilite.createGrid('s_otr110ukrv_shGrid', {		// 메인
    	// for tab
        layout : 'fit',
        region : 'center',
    	uniOpt: {
						...
						editor: Unilite.popup('DIV_PUMOK_G', {
							...
							listeners: {
								...
								applyextparam: function(popup){
							        ...
							        popup.setExtParam({'DIV_CODE': Ext.getCmp("s_otr110ukrv_shMasterForm").getValue("DIV_CODE")});
					            }
							}
						})
					 },
...
//               		 { dataIndex: 'WH_CELL_NAME'		    , 	width:100, hidden: true},
        			 ...
        			 { dataIndex: 'LOSS_Q'			  ,     width:100, hidden: true},		//20200226 수정: BsaCodeInfo.gsBoxYN == 'Y' ? false : true -> true
		...
		setReservationData: function(record) {						// 예약참조 셋팅
			...
			grdRecord.set('DIV_CODE'			, record['OUT_DIV_CODE']);
...
//			grdRecord.set('WH_CODE'				, record['WH_CODE']); 메인상단에 있는 값으로 셋팅
			...
			grdRecord.set('BASIS_SEQ'			, record['OUT_SEQ']);
...
//			grdRecord.set('WH_NAME'				, record['WH_CODE']);
			...
			//20200226 추가: 수주번호, 수주처명, 품목명 set하는 로직 추가
...
//						             				selectRec.set('WH_CELL_NAME', '');
    ...
    var orderNoMasterGrid = Unilite.createGrid('s_otr110ukrv_shOrderNoMasterGrid', {		// 검색팝업창
        // title: '기본',
        layout : 'fit',
    ...
    //预定参考
    var otherorderGrid = Unilite.createGrid('s_otr110ukrv_shotherorderGrid', {	//예약참조
        // title: '기본',
        layout : 'fit',
        ...
        columns:  [
					 ...
					 { dataIndex: 'OUT_SEQ'           		,  width:66, align:'center', hidden: false},
					 ...
					 { dataIndex: 'NOT_OUTSTOCK'		    ,  width: 96},
              		 { dataIndex: 'OUT_DATE'           		,  width:80, hidden: false},
    ...
    //退货可能预定参考
    var otherorderGrid2 = Unilite.createGrid('s_otr110ukrv_shotherorderGrid2', {  //반품가능예약참조
        // title: '기본',
        layout : 'fit',
...
//            cbStore.loadStoreRecords();
		...
		confirmSaveData: function(config)	{	// 저장하기전 원복 시키는 작업
			var fp = Ext.getCmp('s_otr110ukrv_shFileUploadPanel');
				...
				Ext.Ajax.request({
				    ...
				    success : function(response){
				        if(response.status == "200"){
				        	...
				        	var selectRecord = Ext.getCmp("s_otr110ukrv_shGrid").getSelectedRecord();
						    if(selectRecord){
			...
			switch(fieldName) {
				...
				case "PACK_UNIT_Q":
					if(BsaCodeInfo.gsBoxYN == 'Y'){
	                    ...
	                    //20200316 수정: 조건과 관계없이 LOSS_Q 계산하지 않음
