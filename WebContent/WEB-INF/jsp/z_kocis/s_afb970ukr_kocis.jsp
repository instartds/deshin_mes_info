...
<t:appConfig pgmId="s_afb970ukr_kocis"  >
...
function appMain() {
    ...
    var detailListStore = Unilite.createStore('afb970ukrDetailListStore', {
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 's_afb970ukrkocisService.selectDetailList'                 
            }
        },
        ...
        tbar:[{
            ...
            handler: function() {
            	...
            	var param = {
                    ...
                    AC_MONTH: panelResult.getValue('AC_MONTH')
                    
                }
                Ext.getCmp('pageAll').getEl().mask('월마감 중...','loading-indicator');
                s_afb970ukrkocisService.closeMonth(param, function(provider, response)  {                           
                    if(!Ext.isEmpty(provider)){
                        UniAppManager.updateStatus("해당월이 마감되었습니다.");
...
//                thAttrs: { align : 'center'}
        ...
        api: {
            load: 's_afb970ukrkocisService.selectForm'  
        }
    });
...
//                    detailForm.setHidden(false);
            ...
            panelSearch.setValue('AC_MONTH',(new Date().getMonth()+1) < 10 ? '0'+ (new Date().getMonth()+1) : (new Date().getMonth()+1));
            panelResult.setValue('AC_YEAR',new Date().getFullYear());
            panelResult.setValue('AC_MONTH',(new Date().getMonth()+1) < 10 ? '0'+ (new Date().getMonth()+1) : (new Date().getMonth()+1));
