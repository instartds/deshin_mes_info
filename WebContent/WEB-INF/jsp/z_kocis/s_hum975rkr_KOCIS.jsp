...
<t:appConfig pgmId="s_hum975rkr_KOCIS"  >
...
function appMain() {
	...
	var directSubProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_hum975rkrService_KOCIS.selectList',
			update: 's_hum975rkrService_KOCIS.updateDetail',
			create: 's_hum975rkrService_KOCIS.insertDetail',
			destroy: 's_hum975rkrService_KOCIS.deleteDetail',
			syncAll: 's_hum975rkrService_KOCIS.saveAll'
		}
	});
		...
		items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {	
						if(Ext.getCmp('optPrintGb').getChecked()[0].inputValue == '1' || Ext.getCmp('optPrintGb').getChecked()[0].inputValue == '2') {
	...
	Unilite.Main({
		...
		fnInitBinding : function() {
			...
			s_hum975rkrService_KOCIS.fnHum975ini(param, function(provider, response){			// 증명번호 최신화
		...
		fnDeleteAllData : function()	{
			...
			var param = {"S_COMP_CODE": UserInfo.compCode};	
			s_hum975rkrService_KOCIS.fnDeleteAllData(param, function(provider, response){		
			});
		},
	    ...
	    fnUpdateChanges : function() {
	    	...
	    	s_hum975rkrService_KOCIS.insertProfNum(param, function(provider, response){	
	    		s_hum975rkrService_KOCIS.fnHum975ini(param, function(provider2, response){
