...
<t:appConfig pgmId="s_bor120ukrv_KOCIS"  >
...
function appMain() {
	...
	Unilite.defineModel('bor120ukrvModel', {
	    fields: [
            {name : 'DIV_CODE'          , text : '기관코드'         , type : 'string', allowBlank:false, isPk:true, pkGen:'user'},
            {name : 'DIV_NAME'          , text : '기관명'           , type : 'string', allowBlank:false }, 
            {name : 'REPRE_ROLE'        , text : '관서장직명'        , type : 'string' },
            {name : 'REPRE_NAME'        , text : '관서장성명'        , type : 'string' },
            {name : 'SUB_REPRE_ROLE'    , text : '담당자직명'        , type : 'string' },
            {name : 'SUB_REPRE_NAME'    , text : '담당자성명'        , type : 'string' },
            {name : 'ACCNT_GENERAL_NO'  , text : '일반회계증명자번호'  , type : 'string' },
            {name : 'FIN_GENERAL_NO'    , text : '기금증명자번호'     , type : 'string' },
            {name : 'CURR_UNIT'         , text : '현지환율단위'      , type : 'string' },
            {name : 'IMGE'              , text : '직인이미지'        , type : 'string' },
            {name : 'LOGO_IMGE'         , text : 'Logo이미지'       , type : 'string' },
        ...
        /*    
	    	...
	    	{name: 'DIV_CODE'    		,text:'사업장코드'			,type : 'string', allowBlank:false ,allowBlank:false, isPk:true, pkGen:'user'} 
					,{name: 'DIV_NAME'    		,text:'사업장명'			,type : 'string', allowBlank:false } 
					,{name: 'DIV_FULL_NAME'    	,text:'사업장전명'			,type : 'string'} 
					,{name: 'COMPANY_NUM'    	,text:'사업자등록번호'		,type : 'string'} 
					,{name: 'REPRE_NAME'    	,text:'대표자명'			,type : 'string'} 
					,{name: 'REPRE_NO'    		,text:'주민등록번호(DB)'	,type : 'string'} 
					,{name: 'REPRE_NO_EXPOS'    ,text:'주민등록번호'		,type : 'string', defaultValue:'*************'} 
					...
					,{name: 'SAFFER_TAX'    	,text:'신고세무서'			,type : 'string'} 
					,{name: 'SAFFER_TAX_NM'    	,text:'신고세무서명'		,type : 'string'} 
					,{name: 'BILL_DIV_CODE'    	,text:'신고사업장코드'		,type : 'string', allowBlank:false } 
					,{name: 'BILL_DIV_NAME'    	,text:'신고사업장명'		,type : 'string'} 
					//20161024 추가 / 순서 변경
					,{name: 'HANDPHONE'    		,text:'사업장 휴대전화'		,type : 'string'} 
					,{name: 'EMAIL'    			,text:'사업자EMAIL'		,type : 'string'} 
					,{name: 'SUB_DIV_NUM'    	,text:'종사업자일련번호'		,type : 'string'}
					,{name: 'HOMETAX_ID'    	,text:'홈택스ID'			,type : 'string'} 
					,{name: 'BANK_CODE'    		,text:'은행코드'			,type : 'string'} 
					,{name: 'BANK_NAME'    		,text:'은행명'			,type : 'string'} 
					,{name: 'BANK_BOOK_NUM'   	,text:'계좌번호'			,type : 'string'} 
					,{name: 'TAX_NAME'    		,text:'세무대리인 성명'		,type : 'string'} 
					,{name: 'TAX_NUM'    		,text:'세무대리인 사업자번호'	,type : 'string'} 
					,{name: 'TAX_TEL'    		,text:'세무대리인 전화번호'	,type : 'string'} 
					,{name: 'TAX_ADDR'    		,text:'세무대리인 주소'		,type : 'string'} 
					,{name: 'COMP_CODE'    		,text:'법인코드'			,type : 'string', allowBlank:false	, defaultValue:UserInfo.compCode} 
					,{name: 'USE_YN'    		,text:'사용유무'			,type : 'string', allowBlank:false	, comboType:'AU'	, comboCode: 'A004'	, defaultValue:'Y'}*/
			]
	});
	
  	var directProxy= Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			 read   : 's_bor120ukrvService_KOCIS.selectList'
        	,update : 's_bor120ukrvService_KOCIS.updateMulti'
			,create : 's_bor120ukrvService_KOCIS.insertMulti'
			,destroy: 's_bor120ukrvService_KOCIS.deleteMulti'
			,syncAll: 's_bor120ukrvService_KOCIS.saveAll'
		}
	});
        ...
        listeners: {
			...
			items: [{
	            ...
	            items: [{
	        		...
	        		items: [
	        			{fieldLabel: '기관', name:'DIV_CODE', xtype: 'uniCombobox', comboType:'BOR120',
		        		listeners: {
    ...
    var panelResult = Unilite.createSearchForm('bor120ukrvResultForm',{
		...
		items: [{
        		xtype: 'container',
        		layout: {type: 'uniTable', columns: 1},
        		items: [{fieldLabel: '기관', name:'DIV_CODE', xtype: 'uniCombobox', comboType:'BOR120',
	        		listeners: {
					change: function(field, newValue, oldValue, eOpts) {						
    ...
    var masterGrid = Unilite.createGrid('bor120ukrvGrid', {
		...
		columns:[	
            {dataIndex: 'DIV_CODE'          ,  width: 100 ,isLink:true}, 
            {dataIndex: 'DIV_NAME'          ,  width: 130},
            {dataIndex: 'REPRE_ROLE'        ,  width: 120},
            {dataIndex: 'REPRE_NAME'        ,  width: 120},
            {dataIndex: 'SUB_REPRE_ROLE'    ,  width: 120},
            {dataIndex: 'SUB_REPRE_NAME'    ,  width: 120},
            {dataIndex: 'ACCNT_GENERAL_NO'  ,  width: 150},
            {dataIndex: 'FIN_GENERAL_NO'    ,  width: 140},
            {dataIndex: 'CURR_UNIT'         ,  width: 120},
            {dataIndex: 'IMGE'              ,  width: 120, hidden:true},
            {dataIndex: 'LOGO_IMGE'         ,  width: 120, hidden:true},
            {dataIndex: 'ADDR'              ,  width: 230},
            {dataIndex: 'COMP_CODE'         ,  width: 120, hidden:true}

				/*
					
						
							{ dataIndex: 'DIV_CODE',  width: 80 ,isLink:true} 
    ...
    var detailForm = Unilite.createForm('bor120ukrvDetailForm', {
	    ...
	    items : [{ 
				    ...
				    , items : [	
			             {name: 'COMP_CODE'         ,fieldLabel: '회사코드', value:UserInfo.compCode, hidden:true} 
                        ,{name: 'DIV_CODE'          ,fieldLabel: '기관코드', allowBlank:false}
                        ,{name: 'DIV_NAME'          ,fieldLabel: '기관명' , allowBlank:false}
                        ,{name: 'REPRE_ROLE'        ,fieldLabel: '관서장직명'} 
                        ,{name: 'REPRE_NAME'        ,fieldLabel: '관서장성명'} 
                        ,{name: 'SUB_REPRE_ROLE'    ,fieldLabel: '담당자직명'} 
                        ,{name: 'SUB_REPRE_NAME'    ,fieldLabel: '담당자성명'} 
                        ,{name: 'ACCNT_GENERAL_NO'  ,fieldLabel: '일반회계증명자번호'} 
                        ,{name: 'FIN_GENERAL_NO'    ,fieldLabel: '기금증명자번호'} 
                        ,{name: 'CURR_UNIT'         ,fieldLabel: '현지환율단위'} 
                        ,{name: 'ADDR'              ,fieldLabel: '주소'}
                        ,{
                            xtype   : 'container',
                            layout  : {type: 'uniTable', columns: 4, tdAttrs: {class:'photo-background', valign:'bottom'}},
...
//                            colspan : 2,
                            items   : [{
                            	xtype: 'component',
                            	html:'직인 이미지',
                            	tdAttrs: {style: 'font: "굴림" 12px ', valign:'top'},
                            	padding : '3 0 0 46'
                        	},{
                        	   fieldLabel  : '직인 이미지',
                                title       : '직인 이미지',
...
//                            colspan : 2,
            ...
            //                      }
				  ...
				  /*  			{name: 'COMP_CODE'    		,fieldLabel: '회사코드', value:UserInfo.compCode, allowBlank:false , hidden:true} 
				    			,{name: 'DIV_CODE'    		,fieldLabel: '사업장코드',  allowBlank:false }								
								,{name: 'DIV_FULL_NAME'    	,fieldLabel: '사업장전명'	 } 
								,{name: 'DIV_NAME'    		,fieldLabel: '사업장명'	, allowBlank:false  } 
								...
								,{name: 'REPRE_NAME'    	,fieldLabel: '대표자명'	 } 
								,{ 
									fieldLabel:'주민등록번호',
									name :'REPRE_NO_EXPOS',
									xtype: 'uniTextfield',
								...
								,{name: 'REPRE_NO'    		,fieldLabel: '주민등록번호(DB)'	 ,colspan:2, hidden: true
    								,
									listeners : {
										blur: function(field, The, eOpts)	{
											var newValue = field.getValue().replace(/-/g,'');
								...
								,{name: 'ADDR'    			,fieldLabel: '주소'	 , width:520, colspan:2} 
								,{name: 'TELEPHON'    		,fieldLabel: '전화번호'	 } 
								,{name: 'FAX_NUM'    		,fieldLabel: 'FAX번호'	 }			
								, Unilite.popup('SAFFER_TAX',{ valueFieldName:'SAFFER_TAX', textFieldName:'SAFFER_TAX_NM',  colspan:2})
								,{name: 'BILL_DIV_CODE'    	,fieldLabel: '신고사업장'	 ,xtype: 'uniCombobox', comboType:'BOR120',value:UserInfo.compCode , allowBlank:false} 
								,{name: 'HANDPHONE'    		,fieldLabel: '사업장 휴대전화'	 }
								,{name: 'EMAIL'    			,fieldLabel: '사업자 E-MAIL'	 }
				    			,{name: 'SUB_DIV_NUM'    	,fieldLabel: '종사업자 일련번호'	 }
				    			,{
								    xtype: 'radiogroup',
								    fieldLabel: '사용유무',
								    allowBlank: false,
					...
					,items:[
				    			 {name: 'TAX_NAME'    			,fieldLabel: '성명'	 } 
								,{name: 'TAX_NUM'    	,fieldLabel: '사업자번호'	 ,colspan:2, maxLength:21,
								...
								,{name: 'TAX_TEL'    			,fieldLabel: '전화번호'	 } 
								,{name: 'TAX_ADDR'    			,fieldLabel: '주소'	 } 	
...
//					}
					...
					if (selected.data.IMGE) {
                        detailForm.down('#EmpImg1').getEl().dom.src = CPATH + "/uploads/images/" + selected.data.DIV_CODE + '?_dc=' + selected.data.CHANGE_YN;
                        
                    } else {
                        detailForm.down('#EmpImg1').getEl().dom.src = CPATH + '/resources/images/human/noPhoto.png';
                    }
                    
                    if (selected.data.LOGO_IMGE) {
                        detailForm.down('#EmpImg2').getEl().dom.src = CPATH + "/uploads/images/" + selected.data.DIV_CODE + '_logo' + '?_dc=' + selected.data.CHANGE_YN;
                        
                    } else {
                        detailForm.down('#EmpImg2').getEl().dom.src = CPATH + '/resources/images/human/noPhoto.png';
                    }
				},
				hide:function()	{
					masterGrid.show();
    ...
    var photoForm1 = Ext.create('Unilite.com.form.UniDetailForm',{ 
        xtype       : 'uniDetailForm',
        disabled    : false,
        fileUpload  : true,
        itemId      : 'photoForm1',
            ...
            uploadWin = Ext.create('Ext.window.Window', {
                ...
                items       : [ 
                    ...
                    {   
                        ...
                        //url:  CPATH+'/fileman/upload.do',
                        api         : {
                            submit: s_bor120ukrvService_KOCIS.photoUploadFile1
                        },
                        items       :[{ 
                                xtype       : 'filefield',
                                fieldLabel  : '사진',
                ...
                afterSavePhoto: function()  {
                    var photoForm1 = uploadWin.down('#photoForm1');
                    photoForm1.clearForm();
                    uploadWin.hide();
                },
                tbar:['->',{    
                    xtype   : 'button', 
                        ...
                        if(detailForm.getForm().isDirty())  {
                            ...
                            var config  = {
                                success : function(form, action)    {
                                    var photoForm1 = uploadWin.down('#photoForm1').getForm();
            ...
            uploadWin = Ext.create('Ext.window.Window', {
                ...
                items       : [ 
                    photoForm2,
                    {
                        ...
                        //url:  CPATH+'/fileman/upload.do',
                        api         : {
                            submit: s_bor120ukrvService_KOCIS.photoUploadFile2
                        },
                        items       :[{ 
                                xtype       : 'filefield',
                                fieldLabel  : '사진',
                ...
                afterSavePhoto: function()  {
                    var photoForm2 = uploadWin.down('#photoForm');
                    photoForm2.clearForm();
                    uploadWin.hide();
                },
                tbar:['->',{    
                    xtype   : 'button', 
                    ...
                    handler : function()    {
                        ...
                        if(detailForm.getForm().isDirty())  {
                            ...
                            var config  = {
                                success : function(form, action)    {
                                    var photoForm2 = uploadWin.down('#photoForm2').getForm();
                                    param.ITEM_CODE = gsItemCode;
