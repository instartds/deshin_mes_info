...
<t:appConfig pgmId="s_hpa955skr_KOCIS"  >
...
// 	console.log(colData);
	...
	var masterStore = Unilite.createStore('hpa955skrMasterStore',{
        ...
        proxy: {
            type: 'direct',
            api: {			
                read: 's_hpa955skrService_KOCIS.selectList'                	
            }
        }/*, //(사용 안 함 : 쿼리에서 처리)
        listeners : {
	        load : function(store) {
...
//			}
		...
		onQueryButtonDown : function()	{		
			if(!UniAppManager.app.checkForNewDetail()){
				...
				var param = Ext.getCmp('searchForm').getValues();
				s_hpa955skrService_KOCIS.selectColumns2(param, function(provider, response) {
					...
					if (selValue == 'dept') {
						masterGrid.getColumn('DEPT_NAME').setText('부서');
					} else {
						masterGrid.getColumn('DEPT_NAME').setText('직위명');
					}
				});
...
/*	// Grid 의 summary row의  표시 /숨김 적용 (사용 안 함 : 쿼리에서 처리)
	...
	function createModelField(colData) {
		...
		var fields = [
			{name: 'PAY_YYYYMM'			, text: '구분'			, type: 'string'},
			{name: 'PAY_YYYYMM'			, text: '급여년월'			, type: 'string'},
			{name: 'SUPP_NAME'			, text: '지급구분'			, type: 'string'},
			...
			{name: 'BUSI_SHARE_I'		, text: '사회보험부담금'		, type: 'uniPrice'},
			{name: 'WORKER_COMPEN_I'	, text: '산재보험부담금'		, type: 'uniPrice'}
		];
					
		Ext.each(colData, function(item, index){
			var name = index < 20 ? 'S'+(index + 1) : 'G'+(index + 1) 
			fields.push({name: name, text: ' ', type:'uniPrice' });
...
//			if (item.WAGES_NAME == null || item.WAGES_NAME == '') {
	...
	function createGridColumn(colData) {
		...
		var columns = [
   			{dataIndex: 'GUBUN'				, text: '구분'		, width: 86		, locked: false, hidden:true},
   			{dataIndex: 'PAY_YYYYMM'		, text: '급여년월'		, width: 86		, locked: false	, style: 'text-align: center', 
			...
			{dataIndex: 'SUPP_NAME'			, text: '지급구분'		, width: 100	, locked: false	, style: 'text-align: center',
				renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
			...
			{dataIndex: 'DIV_CODE'			, width: 120		, text: '기관', locked: false, sortable: false	, id: 'a2'	, style: 'text-align: center',
				renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
			...
			{dataIndex: 			'DEPT_CODE'			, text: '부서코드'		, width: 100	, locked: false , hidden:true},
			{dataIndex: 			'DEPT_NAME'			, text: '부서'		, width: 160	, locked: false	, hidden: true,style: 'text-align: center'},
			Ext.applyIf({dataIndex: 'PERSON_CNT'		, text: '인원'		, width: 86		, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex: 'SUPP_TOTAL_I'		, text: '지급총액'		, width: 100	, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex: 'DED_TOTAL_I'		, text: '공제총액'		, width: 100	, locked: false	, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex:'REAL_AMOUNT_I'		, text: '실지급액'		, width: 100	, locked: false	, style: 'text-align: center',
				renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
			...
			Ext.applyIf({dataIndex: 'BUSI_SHARE_I'		, text: '사회보험부담금'	, width: 130	, locked: false	, hidden : true, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price }),
			Ext.applyIf({dataIndex: 'WORKER_COMPEN_I'	, text: '산재보험부담금'	, width: 130	, locked: false	, hidden : true, style: 'text-align: center'}, {align: 'right' , xtype:'uniNnumberColumn', format: UniFormat.Price })
		];
		...
		Ext.each(colData, function(item, index){
			...
			if (item == null){
//			if (item.WAGES_NAME == null || item.WAGES_NAME == '') {
				columns.push({dataIndex: dataIndex		, text: ''		, style: 'text-align: center'	,width: 100		, hidden:true});	
			} else {
