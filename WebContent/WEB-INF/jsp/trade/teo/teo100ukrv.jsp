...
<t:appConfig pgmId="teo100ukrv"  >
<t:ExtComboStore comboType="BOR120"  pgmId="teo100ukrv"/> 			<!-- 사업장 -->  
...
function appMain() { 	
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {/*
			read: 'teo100ukrvService.selectMaster',
			update: 'teo100ukrvService.updateDetail',
			create: 'teo100ukrvService.insertDetail',
			destroy: 'teo100ukrvService.deleteDetail',
			syncAll: 'teo100ukrvService.saveAll'
		*/}
	});
	
	var masterForm = Unilite.createSearchPanel('teo100ukrvMasterForm',{		// 메인
...
//						field.fireEvent('changedivcode', field, newValue, oldValue, eOpts);//panelResult의 필터링 처리 위해..
				...
				fieldLabel: '<t:message code="system.label.trade.charger" default="담당자"/>',
				name:'REQ_PRSN',	
				xtype: 'uniCombobox', 
				holdable: 'hold',
				comboType:'AU',
...
//						field.fireEvent('changedivcode', field, newValue, oldValue, eOpts);
	 ...
	 * @type 
	 */
	Unilite.defineModel('teo100ukrvModel', {		// 메인
	    fields: [  	  
	    	{name: 'REQSTOCK_NUM'				,text: '이동요청번호'			,type: 'string'},
		    {name: 'REQSTOCK_SEQ'				,text: '순번'					,type: 'int', allowBlank: false},
		    {name: 'OUT_DIV_CODE'				,text: '출고사업장'				,type: 'string', xtype: 'uniCombobox', comboType: 'BOR120', allowBlank: false},
		    {name: 'OUT_WH_CODE'				,text: '출고창고'				,type: 'string',store: Ext.data.StoreManager.lookup('whList'), allowBlank: false},
		    {name: 'OUT_WH_NAME'				,text: '출고창고'				,type: 'string',store: Ext.data.StoreManager.lookup('whList')},
		    {name: 'OUT_WH_CELL_CODE'			,text: '출고창고Cell코드'		,type: 'string'},
		    ...
		    {name: 'WH_CODE'					,text: '받을창고'				,type: 'string',store: Ext.data.StoreManager.lookup('whList')},   
		    {name: 'WH_CELL_CODE'				,text: '받을창고 Cell코드'		,type: 'string'},
		    {name: 'REQ_PRSN'					,text: '<t:message code="system.label.trade.charger" default="담당자"/>'				    ,type: 'string', comboType:'AU', comboCode:'B024'},
	...
	}); //End of Unilite.defineModel('teo100ukrvModel', {
	...
	Unilite.defineModel('orderNoMasterModel', {		// 검색조회창
	    fields: [
	    	...
	    	{name: 'WH_CODE'					, text: '받을창고코드'		, type: 'string'},
	    	{name: 'WH_NAME'					, text: '받을창고'			, type: 'string'},
	    	{name: 'WH_CELL_CODE'				, text: '받을창고Cell코드'	, type: 'string'},
	    	{name: 'WH_CELL_NAME'				, text: '받을창고Cell'		, type: 'string'},
	    	...
	    	{name: 'REQSTOCK_NUM'				, text: '출고번호'			, type: 'string'},
	    	{name: 'REQSTOCK_DATE'				, text: '출고일'			, type: 'string'},
	    	{name: 'REQSTOCK_SEQ'				, text: '출고순번'			, type: 'string'}
		]
	});
	
	Unilite.defineModel('teo100ukrvOTHERModel', {	//가용재고 참조 
	    fields: [
			...
			{name: 'NOT_IN_Q'		   	, text: '입고예정'    		, type: 'string'},
			{name: 'NOT_OUT_Q'		   	, text: '출고예정'    		, type: 'string'},
			{name: 'SAFE_STOCK_Q'	   	, text: '안전재고'    		, type: 'string'},
			{name: 'USE_STOCK_Q'	   	, text: '가용재고'    		, type: 'string'}
		]
	});
	 ...
	 * @type 
	 */					
	var directMasterStore1 = Unilite.createStore('teo100ukrvMasterStore1',{		// 메인
		model: 'teo100ukrvModel',
		uniOpt : {
	...
	});		// End of var directMasterStore1 = Unilite.createStore('teo100ukrvMasterStore1',{
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {	// 검색 팝업창
            ...
            proxy: {
                type: 'direct',
                api: {
                	read: 'teo100ukrvService.selectDetail'
                }
            },
            loadStoreRecords: function() {
	...
	var otherOrderStore = Unilite.createStore('teo100ukrvOtherOrderStore', {//가용재고 참조
			model: 'teo100ukrvOTHERModel',
            autoLoad: false,
            uniOpt : {
            	isMaster: false,			// 상위 버튼 연결
            	editable: false,			// 수정 모드 사용
            ...
            proxy: {
                type: 'direct',
                api: {
                	read: 'teo100ukrvService.selectDetail2'                	
                }
            },
            listeners:{
    ...
    var masterGrid = Unilite.createGrid('teo100ukrvGrid', {		// 메인
    	// for tab    	
        layout : 'fit',
        region:'center',
    	uniOpt: {
    ...
    });	//End of   var masterGrid1 = Unilite.createGrid('teo100ukrvGrid1', {
	
    var orderNoMasterGrid = Unilite.createGrid('teo100ukrvOrderNoMasterGrid', {	// 검색팝업창
        // title: '기본',
        layout : 'fit',       
    ...
    var otherOrderGrid = Unilite.createGrid('teo100ukrvotherOrderGrid', {	//가용재고 참조
        // title: '기본',
        layout : 'fit',
...
//						orderNoSearch.clearForm();
    ...
    Unilite.Main( {
		...
		fnInitBinding: function() {
			...
			teo100ukrvService.userWhcode({}, function(provider, response)	{
		...
		confirmSaveData: function(config)	{	// 저장하기전 원복 시키는 작업
			var fp = Ext.getCmp('teo100ukrvFileUploadPanel');
		...
		fnQtySet : function(record) {
			...
			teo100ukrvService.QtySet(param, function(provider, response)	{
				if(!Ext.isEmpty(provider)){
