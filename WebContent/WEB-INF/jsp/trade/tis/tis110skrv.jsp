...
<t:appConfig pgmId="tis110skrv" >
	<t:ExtComboStore comboType="BOR120" pgmId="tis110skrv" /> 				<!-- 사업장 -->
...
function appMain() {
	...
	Unilite.defineModel('tis110skrvModel', {
	    fields: [
...
/*hidden*/  {name: 'CUSTOM_CODE'		, text: '거래처코드'	, type: 'string'}, 
	    	{name: 'ORDER_PRSN'			, text: '담당자명'		, type: 'string' , comboType: 'AU', comboCode: 'H008'},
/*hidden*/  {name: 'PRSN_NAME'			, text: '<t:message code="system.label.trade.charger" default="담당자"/>'		, type: 'string'},
	    	...
	    	{name: 'BL_DATE'         	, text: '<t:message code="system.label.trade.shipmentdate" default="선적일"/>'		, type: 'uniDate'},
	    	...
	    	{name: 'BL_QTY'             , text: '구매단위량'	, type: 'uniQty'},
	    	...
	    	{name: 'BL_P'            	, text: '선적외화단가'		, type: 'uniUnitPrice'},
	    	{name: 'BL_O'         		, text: '선적외화금액'		, type: 'uniQty'},
...
/*hidden*/  {name: 'ORDER_SEQ'			, text: '발주순번'		, type: 'string'},
	...
	Unilite.defineModel('tis110skrvModel2', {
	    fields: [
	    	{name: 'CUSTOM_NAME'		, text: '거래처명'		, type: 'string'},
/*hidden*/  {name: 'CUSTOM_CODE'		, text: '거래처코드'	, type: 'string'}, 
	    	{name: 'ITEM_CODE'			, text: '<t:message code="system.label.trade.itemcode" default="품목코드"/>'		, type: 'string'},
...
/*hidden*/  {name: 'ORDER_SEQ'			, text: '발주순번'		, type: 'string'},
	 ...
	 * @type 
	 */					
	var directMasterStore1 = Unilite.createStore('tis110skrvMasterStore1', {
		model: 'tis110skrvModel',
		uniOpt: {
		...
		proxy: {
			type: 'direct',
			api: {			
				read: 'tis110skrvService.selectList'                	
			}
		},
	...
	var directMasterStore2 = Unilite.createStore('tis110skrvMasterStore2', {
		model: 'tis110skrvModel2',
		uniOpt: {
        	isMaster: true,			// 상위 버튼 연결 
        	editable: false,			// 수정 모드 사용 
        	deletable: false,			// 삭제 가능 여부 
	...
	var directMasterStore3 = Unilite.createStore('tis110skrvMasterStore3', {
		model: 'tis110skrvModel3',
		uniOpt: {
        	isMaster: true,			// 상위 버튼 연결 
        	editable: false,			// 수정 모드 사용 
        	deletable: false,			// 삭제 가능 여부 
...
/**2021-08-19 죄회쿼리에 맞게 주석처리
		,{
				...
				}),
			{
				fieldLabel: '통관일',
				xtype: 'uniDateRangefield',
				startFieldName: 'FRBLDATE',
				endFieldName: 'TOBLDATE',
				width:315
			},(
				Unilite.popup('PRO',{
					fieldLabel: '관리번호', 
     ...
     * @type 
     */
	var masterGrid = Unilite.createGrid('tis110skrvGrid1', {
		layout: 'fit',
		region: 'center',
		excelTitle: '발주현황조회',
		uniOpt: {
        ...
        columns: [
			...
			{dataIndex: 'BL_DATE'			, width: 93 },
			{dataIndex: 'INVOICE_DATE'		, width: 93 },
			{dataIndex: 'ORDER_DATE'		, width: 93 },
			{dataIndex: 'DVRY_DATE'			, width: 73 ,align: 'center'},
			{dataIndex: 'BL_QTY'			, width: 93,summaryType: 'sum'},
			...
			{dataIndex: 'BL_P'				, width: 93 },
			{dataIndex: 'ORDER_O'			, width: 93 },
			{dataIndex: 'BL_O'				, width: 93 ,align: 'center',summaryType: 'sum'},
			{dataIndex: 'MONEY_UNIT'		, width: 100 ,align: 'center'},
			{dataIndex: 'EXCHG_RATE'		, width: 100 ,align: 'center'},
			{dataIndex: 'BL_I'				, width: 106,summaryType: 'sum'},
			{dataIndex: 'LC_SER_NO'			, width: 120 ,align: 'center'},
			{dataIndex: 'LC_DATE'			, width: 80},
			{dataIndex: 'SO_QTY'			, width: 66,summaryType: 'sum'},
			{dataIndex: 'INSTOCK_Q'			, width: 133,summaryType: 'sum'},
			...
			{dataIndex: 'BL_SER'			, width: 120},
			{dataIndex: 'BL_NO'				, width: 80},
			{dataIndex: 'SO_SER_NO'			, width: 120},
			{dataIndex: 'SO_SER'			, width: 80,hidden:true},
    ...
    var masterGrid2 = Unilite.createGrid('tis110skrvGrid2', {
		layout: 'fit',
		region: 'center',
		excelTitle: '발주현황조회',
		uniOpt: {
     ...
     var masterGrid3 = Unilite.createGrid('tis110skrvGrid3', {
		layout: 'fit',
		region: 'center',
		excelTitle: '발주현황조회',
		uniOpt: {
	...
	var tab = Unilite.createTabPanel('tabPanel',{
        ...
        listeners:  {
            tabChange:  function ( tabPanel, newCard, oldCard, eOpts )  {
                ...
                if(newTabId == 'tis110skrvGridTab'){
					panelResult.setAllFieldsReadOnly(false);
			        masterGrid.getStore().loadStoreRecords();
			        UniAppManager.setToolbarButtons('excel',true);
				}
				if(newTabId == 'tis110skrvGridTab2' ){
					panelResult.setAllFieldsReadOnly(false);
					masterGrid2.getStore().loadStoreRecords();
					UniAppManager.setToolbarButtons('excel',true);
				} 
				if(newTabId == 'tis110skrvGridTab3' ){
					panelResult.setAllFieldsReadOnly(false);
	...
	Unilite.Main( {
		...
		onQueryButtonDown: function() {		
		    ...
		    UniAppManager.setToolbarButtons('excel',true);*/
				...
				if(activeTabId == 'tis110skrvGridTab'){
					panelResult.setAllFieldsReadOnly(false);
			         masterGrid.getStore().loadStoreRecords();
			         UniAppManager.setToolbarButtons('excel',true);
				}
				if(activeTabId == 'tis110skrvGridTab2' ){
					panelResult.setAllFieldsReadOnly(false);
					 masterGrid2.getStore().loadStoreRecords();
					 UniAppManager.setToolbarButtons('excel',true);
				} 
				if(activeTabId == 'tis110skrvGridTab3' ){
					panelResult.setAllFieldsReadOnly(false);
					 masterGrid3.getStore().loadStoreRecords();
					 UniAppManager.setToolbarButtons('excel',true);
				}
