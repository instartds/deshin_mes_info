...
function appMain() {
	 ...
	 * @type
	 */
	Unilite.defineModel('tix101ukrvModel', {
		fields: [
			{name: 'CHARGE_TYPE'		, text: '<t:message code="system.label.trade.processtype" default="진행구분"/>'		,type: 'string', comboType: "AU", comboCode: "T071"},
			{name: 'COMP_CODE'			, text: 'COMP_CODE'	,type: 'string'},
			{name: 'DIV_CODE'			, text: 'DIV_CODE'	,type: 'string'},
			{name: 'EXPORTER'			, text: '<t:message code="system.label.trade.exporter" default="수출자"/>'			,type: 'string'},
			{name: 'EXPORTER_NM'		, text: '<t:message code="system.label.trade.exportername" default="수출자명"/>'	,type: 'string'},
			{name: 'BASIC_PAPER_NO'		, text: '<t:message code="system.label.trade.basisno" default="근거번호"/>'			,type: 'string'},
			{name: 'BASE_DATE'			, text: '<t:message code="system.label.trade.basisdate" default="기준일"/>'		,type: 'string'},
	...
	Unilite.defineModel('tix101ukrvModel2', {
		fields: [
			...
			{name: 'BASIC_PAPER_NO'		, text: '<t:message code="system.label.trade.basisno" default="근거번호"/>'					,type: 'string'},
			{name: 'TRADE_CUSTOM_CODE'	, text: '<t:message code="system.label.trade.exporter" default="수출자"/>'					,type: 'string'},
			{name: 'TRADE_CUSTOM_NAME'	, text: '<t:message code="system.label.trade.exportername" default="수출자명"/>'			,type: 'string'},
			...
			{name: 'CHARGE_AMT'			, text: '<t:message code="system.label.trade.foreigncurrencyamount" default="외화금액"/>'	,type: 'uniFC'},
			...
			{name: 'CHARGE_AMT_WON'		, text: '<t:message code="system.label.trade.localamount" default="원화금액"/>'				,type: 'uniPrice'},
			{name: 'SUPPLY_AMT'			, text: '<t:message code="system.label.trade.supplyamount" default="공급가액"/>'			,type: 'uniPrice', allowBlank:true},
			...
			{name: 'TOTAL_AMOUNT'		, text: '<t:message code="system.label.trade.totalamount1" default="합계금액"/>'			,type: 'uniPrice'},
	...
	var directMasterProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'tix101ukrvService.selectMasterList',
			update	: 'tix101ukrvService.updateMaster',
//			create	: 'tix101ukrvService.insertMaster',
//			destroy	: 'tix101ukrvService.deleteMaster',
			syncAll	: 'tix101ukrvService.saveAll'
		}
	});

	var directDetailProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'tix101ukrvService.selectDetailList',
			update	: 'tix101ukrvService.updateDetail',
			create	: 'tix101ukrvService.insertDetail',
			destroy	: 'tix101ukrvService.deleteDetail',
			syncAll	: 'tix101ukrvService.saveAll'
		}
	});
	 ...
	 * @type
	 */
	var directMasterStore = Unilite.createStore('tix101ukrvMasterStore1',{
		proxy	: directMasterProxy,
		model	: 'tix101ukrvModel',
		uniOpt	: {
...
//			}
	...
	var directDetailStore = Unilite.createStore('tix101ukrvMasterStore2',{
		proxy	: directDetailProxy,
		model	: 'tix101ukrvModel2',
		uniOpt	: {
	...
	var panelSearch = Unilite.createSearchPanel('searchForm', {
		...
		items		: [{
			...
			Unilite.popup('CUST',{
				fieldLabel		: '<t:message code="system.label.trade.exporternmaircomp" default="수출자"/>',
				validateBlank	: false,
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('tix101ukrvGrid1', {
...
//			useContextMenu: true,
	 ...
	 * @type
	 */
	var detailGrid = Unilite.createGrid('tix101ukrvGrid2', {
...
//			useContextMenu		: true,
		...
		columns	: [
			...
			{dataIndex: 'CHARGE_SER'				, width: 66, hidden: true},
			...
			{dataIndex: 'SUPPLY_AMT'				, width: 100, editable: false},
			{dataIndex: 'TAX_CLS'					, width: 100},
			{dataIndex: 'VAT_AMT'					, width: 100},
			{dataIndex: 'TOTAL_AMOUNT'				, width: 100, editable: false},
		...
		listeners: {
			...
			beforeedit  : function( editor, e, eOpts ) {
				...
				if(e.record.phantom){
					...
					if (UniUtils.indexOf(e.field,["COST_DIV", "EX_DATE","EX_NUM","AGREE_YN"])) {
						return false;
					}
				}else{
		...
		onNewDataButtonDown: function() {
			...
			if(!Ext.isEmpty(pRecord)) {
				...
				var r = {
					...
					SUPPLY_AMT			: 0,
					VAT_AMT				: 0,
					TRADE_CUSTOM_CODE	: pRecord.get('EXPORTER'),
					TRADE_CUSTOM_NAME	: pRecord.get('EXPORTER_NM'),
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt, editor, e) {
			...
			switch(fieldName) {
				...
				case "CHARGE_AMT" :
					...
					record.set('SUPPLY_AMT' 	, UniMatrl.fnExchangeApply(record.get('AMT_UNIT'), record.get('CHARGE_AMT_WON') - record.get('VAT_AMT')));
					break;
				// 환율
				case "EXCHANGE_RATE" :
					//20200610 수정: 자사금액 계산시, 'JPY' 관련로직 추가
				...
				// 원화금액
				case "CHARGE_AMT_WON" :
					record.set('SUPPLY_AMT'		, newValue);                         // 공급가액 세팅
					record.set('TOTAL_AMOUNT'	, record.get('VAT_AMT') + newValue); // 부가세 세팅
					break;

				// 부가세
				case "VAT_AMT" :
					record.set('TOTAL_AMOUNT'	, record.get('SUPPLY_AMT') + newValue);
					break;
			}
			return rv;
		}
