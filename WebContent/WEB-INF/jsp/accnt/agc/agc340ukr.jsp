...
var agc340ukrRef1Window; // 검색팝업 관련

function appMain() {  

	var directProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create: 'agc340ukrService.insertDetail1',
			syncAll: 'agc340ukrService.saveAll1'
		}
	});	
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create: 'agc340ukrService.insertDetail2',
			syncAll: 'agc340ukrService.saveAll2'
		}
	});	
	var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'agc340ukrService.selectList3',
			update: 'agc340ukrService.updateDetail3', 
			syncAll: 'agc340ukrService.saveAll3'
		}
	});	
	 ...
	 * @type 
	 */					
	var directDetailStore1 = Unilite.createStore('agc340ukrMasterStore1',{
        ...
        proxy: {
           type: 'direct',
            api: {			
                read: 'agc340ukrService.selectList1'                	
            }
        },
	...
	var directDetailStore2 = Unilite.createStore('agc340ukrMasterStore1',{
        ...
        proxy: {
           type: 'direct',
            api: {			
                read: 'agc340ukrService.selectList2'                	
            }
        },
        loadStoreRecords : function()	{
			var param= Ext.getCmp('searchForm').getValues();	
	...
	var directDetailStore3 = Unilite.createStore('agc340ukrMasterStore1',{
...
//						directDetailStore3.loadStoreRecords();
		...
		listeners:{
			load: function(store, records, successful, eOpts) {		
				...
				if(SEARCH == 'SEARCH'){
					var param= Ext.getCmp('searchForm').getValues();		
					agc340ukrService.costInformation(param, function(provider, response){
...
//				UniAppManager.app.fnChangeGrid1();
	...
	var agc340ukrRef1Store = Unilite.createStore('agc340ukrRef1Store', {//검색팝업관련
        ...
        proxy: {
            type: 'direct',
            api: {
            	read: 'agc340ukrService.selectRef1'                	
            }
        },
       
        loadStoreRecords : function()	{
			var param= agc340ukrRef1Search.getValues();	
//			param.CHARGE_DIVI = gsChargeDivi;
		...
		listeners: {
           	load: function(store, records, successful, eOpts) {		
				var count = agc340ukrRef1Grid.getStore().getCount();
				if(count == 0){
	...
	var grid1SaveStore = Unilite.createStore('agc340ukrgrid1SaveStore',{  // 이익상태
...
//						directDetailStore1.commitChanges();  
	...
	var grid2SaveStore = Unilite.createStore('agc340ukrgrid2SaveStore',{ // 생산비용
...
//						directDetailStore2.commitChanges();
	...
	var cancSlipStore = Unilite.createStore('Agc340ukrCancSlipStore',{		//기표취소 관련
		proxy: {
           type: 'direct',
            api: {			
                read: 'agc340ukrService.cancSlip'                	
            }
        },
        loadStoreRecords: function() {
			var param= Ext.getCmp('searchForm').getValues();
...
//		                	UniAppManager.app.fnOpenSlip();
    ...
    var agc340ukrRef1Search = Unilite.createSearchForm('agc340ukrRef1Form', {//검색팝업관련
            	...
            	if(agc340ukrRef1Search) {
					if(newValue == null){
				    	...
				    	if(UniDate.getDbDateStr(newValue).substring(0, 6) < (UniDate.getDbDateStr(newValue).substring(0, 4) + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6))){
							agc340ukrRef1Search.setValue('ST_AC_DATE', UniDate.getDbDateStr(newValue).substring(0, 4)-1 + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6));
						}else{
							agc340ukrRef1Search.setValue('ST_AC_DATE', UniDate.getDbDateStr(newValue).substring(0, 4) + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6));
						}
					}
            	}
		    }
...
//				value: UniDate.get('today'),
    ...
    var detailGrid1 = Unilite.createGrid('agc340ukrGrid1', {
    	title: '손익현황',
    	excelTitle: '손익현황',
    	layout : 'fit',
        region : 'west',
...
//                        return '<div style= "background-color:' + '#FAF4C0' + '">' + Ext.util.Format.number(val,'0,000') + '</div>';
    ...
    var detailGrid2 = Unilite.createGrid('agc340ukrGrid2', {
    	title: '용역원가',
    	excelTitle: '용역원가',
    	layout : 'fit',
        region : 'center',
...
//	        	}
    ...
    var detailGrid3 = Unilite.createGrid('agc340ukrGrid3', {
    	title: '용역경비',
    	excelTitle: '용역경비',
    	layout : 'fit',
        region : 'east',
    ...
    var agc340ukrRef1Grid = Unilite.createGrid('agc340ukrRef1Grid', {//검색팝업 관련
        layout : 'fit',
        excelTitle: '검색팝업',
    	store: agc340ukrRef1Store,
    	uniOpt: {
		...
		listeners: {	
			onGridDblClick:function(grid, record, cellIndex, colName) {
				agc340ukrRef1Grid.returnData(record);
				agc340ukrRef1Window.hide();
				UniAppManager.setToolbarButtons(['query'],false);
				SEARCH = 'SEARCH';
...
//		  		'':record.get('EX_NUM'),
    ...
    function openAgc340ukrRef1Window() {    		//검색팝업 관련
		...
		if(!agc340ukrRef1Window) {
			agc340ukrRef1Window = Ext.create('widget.uniDetailWindow', {
                title: '검색',
                width: 700,				                
                height: 500,
                layout:{type:'vbox', align:'stretch'},
                items: [agc340ukrRef1Search, agc340ukrRef1Grid],
                tbar:  ['->',
					{	
						itemId : 'saveBtn',
						text: '조회',
						handler: function() {
							if(!agc340ukrRef1Search.getInvalidMessage()) return;
							agc340ukrRef1Store.loadStoreRecords();
						},
						disabled: false
					},{
						itemId : 'closeBtn',
...
//							draftNoSearch.clearForm();
                ...
                listeners : {
                	beforehide: function(me, eOpt)	{
		 			...
		 			show: function ( panel, eOpts )	{
		 				agc340ukrRef1Search.setValue('ST_AC_DATE',panelSearch.getValue('ST_DATE'));
		 				agc340ukrRef1Search.setValue('FR_AC_DATE',panelSearch.getValue('FR_DATE'));
		 				agc340ukrRef1Search.setValue('TO_AC_DATE',panelSearch.getValue('TO_DATE'));	
		 				agc340ukrRef1Search.setValue('AC_PROJECT_CODE',panelSearch.getValue('AC_PROJECT_CODE'));	
		 				agc340ukrRef1Search.setValue('AC_PROJECT_NAME',panelSearch.getValue('AC_PROJECT_NAME'));	
		...
		agc340ukrRef1Window.center();
		agc340ukrRef1Window.show();
    }    
	  ...
	  Unilite.Main( {
        ...
        onDeleteAllButtonDown: function() {
			...
			if(confirm('전체삭제 하시겠습니까?')) {
				...
				agc340ukrService.agc340ukrDelA(param, function(provider, response)	{							
					if(provider){	
...
//        	var sOldEditVal1 = 0;
	        ...
	        if(findAccnt == -1){
				...
				agc340ukrService.fnDispYN(param, function(provider, response)	{
					if(!Ext.isEmpty(provider)){
...
//						sOldEditVal1 = item.get('AMT_I');
		...
		fnReplaceCostAmt: function(provider){
        	...
        	UniAppManager.app.fnSetCostAmt(directDetailStore1, '2010', provider.COST_PD);  // 용역매출원가
