...
var inLoading = false;
var agreeYn = '';
var agc310ukrRef1Window; // 검색팝업 관련


function appMain() {
	var directProxy1 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create: 'agc310ukrService.insertDetail1',
			syncAll: 'agc310ukrService.saveAll1'
		}
	});	
	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create: 'agc310ukrService.insertDetail2',
			syncAll: 'agc310ukrService.saveAll2'
		}
	});	
	var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'agc310ukrService.produceBudget',
			update: 'agc310ukrService.updateDetail3',
			syncAll: 'agc310ukrService.saveAll3'
		}
	});	

	Unilite.defineModel('Agc310ukrModel1', {
	...
	Unilite.defineModel('Agc310ukrModel2', {
		fields: [
			{name: 'ACCNT_CD'			, text: 'ACCNT_CD'		, type: 'string'},
			{name: 'ACCNT_NAME'			, text: '항목명'			, type: 'string'},
	    	{name: 'AMT_I'				, text: '금액'			, type: 'uniPrice'},
	...
	Unilite.defineModel('Agc310ukrModel3', {
		fields: [
	...
	Unilite.defineModel('Agc310ukrRef1Model', {	//검색팝업관련
	    fields: [
	 ...
	 * 손익현황 저장관련
	 */
	var profitStatusSaveStore = Unilite.createStore('agc310ukrProfitStatusSaveStore',{
	 ...
	 * 제조원가 저장관련
	 */
	var produceCostSaveStore = Unilite.createStore('agc310ukrProduceCostSaveStore',{
	 ...
	 * @type 
	 */					
	var directDetailStore1 = Unilite.createStore('agc310ukrDetailStore1',{
		model: 'Agc310ukrModel1',
		uniOpt : {
        ...
        proxy: {
           type: 'direct',
            api: {			
                read: 'agc310ukrService.profitStatus'                	
            }
        },
	...
	var directDetailStore2 = Unilite.createStore('agc310ukrDetailStore2',{
		model: 'Agc310ukrModel2',
		uniOpt : {
        	isMaster: true,			// 상위 버튼 연결 
        	editable: true,			// 수정 모드 사용 
        	deletable:false,			// 삭제 가능 여부 
        ...
        proxy: {
           type: 'direct',
            api: {			
                read: 'agc310ukrService.produceCost'      	
            }
        },
		loadStoreRecords : function()	{
			var param= Ext.Object.merge(Ext.getCmp('resultForm').getValues(), Ext.getCmp('detailForm').getValues());
	...
	var directDetailStore3 = Unilite.createStore('agc310ukrDetailStore3',{
		model: 'Agc310ukrModel3',
		uniOpt : {
        	isMaster: true,			// 상위 버튼 연결 
        	editable: true,			// 수정 모드 사용 
        	deletable:false,			// 삭제 가능 여부 
...
//						directDetailStore3.loadStoreRecords();
	 ...
	 * 검색팝업 관련
	 */
	var agc310ukrRef1Store = Unilite.createStore('agc310ukrRef1Store', {//검색팝업관련
		model: 'Agc310ukrRef1Model',
        autoLoad: false,
        uniOpt : {
        	isMaster: false,			// 상위 버튼 연결
        	editable: false,			// 수정 모드 사용
        ...
        proxy: {
            type: 'direct',
            api: {
            	read: 'agc310ukrService.selectRef1'                	
            }
        },
       
        loadStoreRecords : function()	{
			var param= agc310ukrRef1Search.getValues();	
...
//	    		name: 'LINKSLIP',
				...
				listeners:{
					render: function(component) {
		                component.getEl().on('click', function( event, el ) {
							...
							agj260ukrService.cancelAutoSlip52(
		...
		items	: [{				   
				...
				handler : function() {
					...
					Ext.getBody().mask();
					agj260ukrService.spAutoSlip67(params, function(provider, response){
						if(provider && Ext.isEmpty(provider.ERROR_DESC) )	{	
				...
				handler : function() {
					...
					agj260ukrService.spAutoSlip67cancel(params, function (provider,response) {
						if(provider && Ext.isEmpty(provider.ERROR_DESC) )	{
							UniAppManager.setToolbarButtons(['deleteAll'],true);
    ...
    var agc310ukrRef1Search = Unilite.createSearchForm('agc310ukrRef1Form', {//검색팝업관련
		layout :  {type : 'uniTable', columns : 2
//			tableAttrs: { /*style: 'border : 1px solid #ced9e7;',*/width: '100%'}
		},
    	items :[{ 
            	...
            	if(agc310ukrRef1Search) {
					if(newValue == null){
				    	...
				    	if(UniDate.getDbDateStr(newValue).substring(0, 6) < (UniDate.getDbDateStr(newValue).substring(0, 4) + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6))){
							agc310ukrRef1Search.setValue('ST_AC_DATE', UniDate.getDbDateStr(newValue).substring(0, 4)-1 + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6));
						}else{
							agc310ukrRef1Search.setValue('ST_AC_DATE', UniDate.getDbDateStr(newValue).substring(0, 4) + UniDate.getDbDateStr(getStDt[0].STDT).substring(4, 6));
						}
					}
            	}
		    }
...
//				value: UniDate.get('today'),
    ...
    var detailGrid1 = Unilite.createGrid('agc310ukrGrid1', {
    	title: '손익현황',
    	region:'west',
    	titleAlign: 'center',
        layout : 'fit',
	...
	var detailGrid2 = Unilite.createGrid('agc310ukrGrid2', {
		title: '제조원가',
		region:'center',
    	titleAlign: 'center',
        layout : 'fit',
//        split:true,
	...
	var detailGrid3 = Unilite.createGrid('agc310ukrGrid3', {
		title: '제조경비',
		region:'east',
    	titleAlign: 'center',
    	split:true,
    ...
    var agc310ukrRef1Grid = Unilite.createGrid('agc310ukrRef1Grid', {//검색팝업 관련
        layout : 'fit',
        excelTitle: '검색팝업',
    	store: agc310ukrRef1Store,
    	uniOpt: {
		...
		listeners: {	
			onGridDblClick:function(grid, record, cellIndex, colName) {
				agc310ukrRef1Grid.returnData(record);
				agc310ukrRef1Window.hide();
				actionFlag='';
				saveFlag = '';
				SEARCH = 'SEARCH';
    ...
    function openAgc310ukrRef1Window() {    		//검색팝업 관련
//  		if(!UniAppManager.app.checkForNewDetail()) return false;
  	
		if(!agc310ukrRef1Window) {
			agc310ukrRef1Window = Ext.create('widget.uniDetailWindow', {
                title: '검색',
                width: 700,				                
                height: 500,
                layout:{type:'vbox', align:'stretch'},
                items: [agc310ukrRef1Search, agc310ukrRef1Grid],
                tbar:  ['->',
					{	
						itemId : 'saveBtn',
						text: '조회',
						handler: function() {
							if(!agc310ukrRef1Search.getInvalidMessage()) return;
							agc310ukrRef1Store.loadStoreRecords();
						},
						disabled: false
					},{
						itemId : 'closeBtn',
...
//							draftNoSearch.clearForm();
                ...
                listeners : {
                	beforehide: function(me, eOpt)	{
		 			...
		 			show: function ( panel, eOpts )	{
		 				agc310ukrRef1Search.setValue('ST_AC_DATE',panelResult.getValue('ST_DATE'));
		 				agc310ukrRef1Search.setValue('FR_AC_DATE',panelResult.getValue('FR_DATE'));
		 				agc310ukrRef1Search.setValue('TO_AC_DATE',panelResult.getValue('TO_DATE'));	
		 				agc310ukrRef1Search.setValue('AC_PROJECT_CODE',panelResult.getValue('AC_PROJECT_CODE'));	
		 				agc310ukrRef1Search.setValue('AC_PROJECT_NAME',panelResult.getValue('AC_PROJECT_NAME'));	
		...
		agc310ukrRef1Window.center();
		agc310ukrRef1Window.show();
    }    
	...
	/*		var param= Ext.getCmp('searchForm').getValues();		
			agc310ukrService.fnGetBaseInfo(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
				...
				agc310ukrService.costInformation(param, function(provider, response){
					if(!Ext.isEmpty(provider)){
						if(provider.AGREE_YN != 'Y' && Ext.isEmpty(provider.EX_DATE) && Ext.isEmpty(provider.J_EX_DATE)) {
							...
							agc310ukrService.agc310ukrDelA(param, function(provider, response)	{							
								if(provider){	
...
//        	var sOldEditVal1 = 0;
	        ...
	        if(findAccnt == -1){
				...
				agc310ukrService.fnDispYN(param, function(provider, response)	{
					if(!Ext.isEmpty(provider)){
...
//						sOldEditVal1 = item.get('AMT_I');
        ...
        fnReplaceCostAmt: function(provider){
        	UniAppManager.app.fnSetCostAmt(directDetailStore2, '1100', provider.COST_MT);
        	UniAppManager.app.fnSetCostAmt(directDetailStore2, '1200', provider.COST_SMT);
        	UniAppManager.app.fnSetCostAmt(directDetailStore2, '1300', provider.COST_ETCMT);
        	...
        	UniAppManager.app.fnSetCostAmt(directDetailStore2, '7000', provider.COST_PRPD);
