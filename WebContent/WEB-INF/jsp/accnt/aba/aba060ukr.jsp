...
<t:appConfig pgmId="aba060ukr"  >
	...
	<t:ExtComboStore comboType="AU" comboCode="H227" /><!--임직원구분-->
	...
	<t:ExtComboStore items="${SET_DIVI}" storeId="comboStoreSetDivi" /><!--결제및구분-->
	<t:ExtComboStore items="${AMT_DIVI}" storeId="comboStoreAmtDivi" /><!--금액구분-->
	
	<t:ExtComboStore comboType="AU" comboCode="A242" /> <!-- 외화평가구분  -->
...
function appMain() {     
	...
	var directProxy3 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
	 ...
	 var directProxy16 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
			api: {
				read 	: 'aba060ukrsService.selectList16',
				create 	: 'aba060ukrsService.insertDetail16',
				update 	: 'aba060ukrsService.updateDetail16',
				destroy	: 'aba060ukrsService.deleteDetail16',
				syncAll	: 'aba060ukrsService.saveAll16'
			}
	 });	
	...
	var directProxy17 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
			api: {
				read 	: 'aba060ukrsService.selectList17',
				create 	: 'aba060ukrsService.insertDetail17',
				update 	: 'aba060ukrsService.updateDetail17',
				destroy	: 'aba060ukrsService.deleteDetail17',
				syncAll	: 'aba060ukrsService.saveAll17'
			}
	 });
	...
	var directProxy18 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
			api: {
				read 	: 'aba060ukrsService.selectList18',
				create 	: 'aba060ukrsService.insertDetail18',
				update 	: 'aba060ukrsService.updateDetail18',
				destroy	: 'aba060ukrsService.deleteDetail18',
				syncAll	: 'aba060ukrsService.saveAll18'
			}
	 });
	...
	Unilite.defineModel('aba060ukrs4Model', {
	    fields: [{name: 'COMP_CODE'			 	,text:'법인코드'				,type : 'string'},
				 ...
				 {name: 'ACCNT'          		,text:'계정코드'				,type : 'string' , allowBlank: false},
				 {name: 'ACCNT_NAME'     		,text:'계정명'				,type : 'string'},
				 {name: 'CUSTOM_CODE'     		,text:'거래처'				,type : 'string'},	//2021.07.27 추가
				 {name: 'CUSTOM_NAME'           ,text:'거래처'				,type : 'string'},  //2021.07.27 추가
				 {name: 'UPDATE_DB_USER' 		,text:'UPDATE_DB_USER'		,type : 'string'},
				 {name: 'UPDATE_DB_TIME' 		,text:'UPDATE_DB_TIME'		,type : 'string'}
				 	 
			]
	});	
	// aba060ukrs4 store	// 기타소득
	...
	Unilite.defineModel('aba060ukrs8Model', {
	    fields: [{name: 'TRADE_DIV'					,text:'구분'			,type : 'string' , allowBlank: false ,comboType:"AU", comboCode:"T001"},
				 {name: 'CHARGE_TYPE_E'				,text:'수출진행구분'	,type : 'string' , comboType:"AU", comboCode:"T070"},
				 {name: 'CHARGE_TYPE_I'				,text:'수입진행구분'	,type : 'string' , comboType:"AU", comboCode:"T071"},
	...
	var aba060ukrs8Store = Unilite.createStore('aba060ukrs8Store',{
            ...
            saveStore : function()	{				
				var inValidRecs = this.getInvalidRecords();
				if(inValidRecs.length == 0 )	{
					var form = panelDetail.down('#tab_Trading_CostForm');
					var chk = true;
					var chkIdx ;
					if(form.getValue("TRADE_DIV") == "E")	{
						Ext.each(this.getData().items , function(record, idx){
							if(Ext.isEmpty(record.get("CHARGE_TYPE_E")))	{
								chk = false;
								chkIdx = idx+1;
								return false;
							}
						...
						if(!chk)	{
							alert(chkIdx+"행의 입력값을 확인해 주세요."+"\n"+"수줄진행구분값을 입력해 주세요.");
						}
					} else {
						Ext.each(this.getData().items , function(record, idx){
							if(Ext.isEmpty(record.get("CHARGE_TYPE_I")))	{
								chk = false;
								chkIdx = idx;
								return false;
							}
	...
	Unilite.defineModel('Aiga240ukrvModel', {
	    fields: [
	    	...
	    	{name: 'SET_DIVI'			,text: '결제및구분'				,type: 'string',store: Ext.data.StoreManager.lookup('comboStoreSetDivi')},
	 ...
	 * @type 
	 */					
	var aiga240ukrvDetailStore = Unilite.createStore('aiga240ukrvDetailStore', {
		...
		saveStore: function() {				
			...
			var form = panelDetail.down('#tab_Asst_Change_IFRS_Form');
			if(!form.getInvalidMessage()) return;
			
			var paramMaster= form.getValues();	//syncAll 수정
			var chkInvalid = false;
			Ext.each(list, function(record, idx) {
				if((record.get("ALTER_DIVI") != '40' && record.get("ALTER_DIVI") != '22') && Ext.isEmpty(record.get("SET_DIVI"))	)	{
					chkInvalid = true;
				}
			});
			if(chkInvalid)	{
				Unilite.messageBox('결제및구분을 입력하세요.');
				return;
			}
			if(inValidRecs.length == 0) {
				config = {
					...
					success: function(batch, option) {
						...
						if (aiga240ukrvDetailStore.count() == 0) {   
	 ...
	 * @type 
	 */					
	var aiga210ukrvStore = Unilite.createStore('aiga210ukrvDetailStore', {
		...
		saveStore: function() {				
			...
			var toCreate = this.getNewRecords();
       		...
       		var toDelete = this.getRemovedRecords();
       		var list = [].concat(toUpdate, toCreate);
       		var form = panelDetail.down('#tab_Asst_Dep_IFRS_Form');
			if(!form.getInvalidMessage()) return;
			var paramMaster= form.getValues();	//syncAll 수정
	...
	// aba060ukrs16 Model
	Unilite.defineModel('aba060ukrs16Model', {
	    fields: [{name: 'PAY_METHD'			,text:'운임지불방법'		,type : 'string' , allowBlank: false ,comboType:"AU", comboCode:"T025"},
				 {name: 'DR_CR'					,text:'차대구분'		,type : 'string' , allowBlank: false ,comboType:"AU", comboCode:"A001"},
				 {name: 'ITEM_ACCNT'  			,text:'품목계정'		,type : 'string' , allowBlank: false ,/*comboType:"AU", comboCode:"B020",*/store: Ext.data.StoreManager.lookup('acItemCombo')},
				 {name: 'ACCNT'  				,text:'계정코드'		,type : 'string' , allowBlank: false},
				 {name: 'ACCNT_NAME' 			,text:'계정과목'		,type : 'string'},
	...
	// aba060ukrs7 store
	var aba060ukrs16Store = Unilite.createStore('aba060ukrs16Store',{
			model: 'aba060ukrs16Model',
            autoLoad: false,
            uniOpt : {
            	isMaster: true,			// 상위 버튼 연결
            	editable: true,			// 수정 모드 사용
            ...
            saveStore : function()	{				
				...
				if(inValidRecs.length == 0 )	{
					...
					panelDetail.down('#aba060ukrs16Grid').uniSelectInvalidColumnAndAlert(inValidRecs);
				}
			},
	        loadStoreRecords : function(){
	        	var param1 = Ext.getCmp('PAY_METHD16').getValue();
            	var param= { PAY_METHD : param1 };    
		        this.load({
		           params: param
		        });   
		     }
	...
	Unilite.defineModel('aba060ukrs17Model', {
	    fields: [{name: 'GUBUN'				,text:'외화평가구분'			,type : 'string' ,comboType:"A", comboCode:"A242" , allowBlank: false},
				 {name: 'ACCNT'				,text:'계정코드'				,type : 'string', allowBlank: false},
				 {name: 'ACCNT_NAME'		,text:'계정명'				,type : 'string'},
				 {name: 'ACCNT_GAIN'		,text:'환산평가이익계정'			,type : 'string', allowBlank: false},
				 {name: 'ACCNT_GAIN_NAME'	,text:'환산평가이익계정명'		,type : 'string'},
				 {name: 'ACCNT_LOSS'		,text:'환산평가손실계정'			,type : 'string', allowBlank: false},
				 {name: 'ACCNT_LOSS_NAME'	,text:'환산평가손실계정명'		,type : 'string'}
			]
	});	
	// aba060ukrs17 store
	var aba060ukrs17Store = Unilite.createStore('aba060ukrs17Store',{
			model: 'aba060ukrs17Model',
	        autoLoad: false,
	        uniOpt : {
	        	isMaster: true,			// 상위 버튼 연결
	        	editable: true,			// 수정 모드 사용
	        ...
	        saveStore : function()	{				
				...
				if(inValidRecs.length == 0 )	{
					var config = {};
					this.syncAllDirect(config);			 //syncAllDirect		
				}else {
					panelDetail.down('#aba060ukrs17Grid').uniSelectInvalidColumnAndAlert(inValidRecs);
				}
			},
	        loadStoreRecords : function(){
	        	var param1 = Ext.getCmp('GUBUN_17').getValue();
				var param= { 'GUBUN_17' : param1};    	
		        this.load({
		           params: param
		        });
		    } 
	...
	// aba060ukrs18 Model
	Unilite.defineModel('aba060ukrs18Model', {
	    fields: [{name: 'GUBUN'					,text:'구분'			,type : 'string' , allowBlank: false ,defaultValue:'1'},
	...
	// aba060ukrs7 store
	var aba060ukrs18Store = Unilite.createStore('aba060ukrs18Store',{
			model: 'aba060ukrs18Model',
            autoLoad: false,
            uniOpt : {
            	isMaster: true,			// 상위 버튼 연결
            	editable: true,			// 수정 모드 사용
            ...
            saveStore : function()	{				
				...
				if(inValidRecs.length == 0 )	{
					...
					panelDetail.down('#aba060ukrs18Grid').uniSelectInvalidColumnAndAlert(inValidRecs);
				}
			},
	        loadStoreRecords : function(){
            	var param= panelDetail.down('#tab_pay2Form').getValues();    
		        this.load({
		           params: param
		        });   
		     }
	...
	var directProxy19 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'aiga410ukrvService.selectList',
			update: 'aiga410ukrvService.updateDetail',
			create: 'aiga410ukrvService.insertDetail',
			destroy: 'aiga410ukrvService.deleteDetail',
			syncAll: 'aiga410ukrvService.saveAll'
		}
	});	

	Unilite.defineModel('aiga410ukrvModel', {
	    fields: [
	    	{name: 'DEPT_DIVI'				,text: '구분'					,type: 'string', comboType:'AU', comboCode:'A037', allowBlank:false},
	    	{name: 'ACCNT'					,text: '국고보조금계정'				,type: 'string', allowBlank:false},
	    	{name: 'ACCNT_NAME'				,text: '국고보조금계정과목명'				,type: 'string'},
	    	{name: 'DEP_ACCNT'				,text: '감가상각비계정'				,type: 'string', allowBlank:false},
	    	{name: 'DEP_ACCNT_NAME'			,text: '감가상각비계정과목명'			,type: 'string'},
	    	{name: 'COMP_CODE'				,text: 'COMP_CODE'			,type: 'string', allowBlank:false},
	    	{name: 'INSERT_DB_USER'			,text: 'INSERT_DB_USER'		,type: 'string'},
	    	{name: 'INSERT_DB_TIME'			,text: 'INSERT_DB_TIME'		,type: 'string'},
	    	{name: 'UPDATE_DB_USER'			,text: 'UPDATE_DB_USER'		,type: 'string'},
	 ...
	 * @type 
	 */					
	var aiga410ukrvStore = Unilite.createStore('aiga410ukrvDetailStore', {
		model: 'aiga410ukrvModel',
		uniOpt: {
			isMaster: true,			// 상위 버튼 연결 
			editable: true,			// 수정 모드 사용 
			deletable: true,			// 삭제 가능 여부 
		...
		saveStore: function() {				
			...
			var toCreate = this.getNewRecords();
       		...
       		var toDelete = this.getRemovedRecords();
       		var list = [].concat(toUpdate, toCreate);
       		var form = panelDetail.down('#tab_Gov_Asst_Dep_IFRS_Form');
			if(!form.getInvalidMessage()) return;
			var paramMaster= form.getValues();	//syncAll 수정
			...
			if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aiga400ukrvGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		loadStoreRecords: function(){
    ...
    var panelDetail = Ext.create('Ext.panel.Panel', {
	    ...
	    items	: [{ 
	    	...
	    	items: [{
				...
				items:[
					...
					<%@include file="./aba060ukrs14.jsp" %>, // 자산변동(IFRS)
					<%@include file="./aba060ukrs15.jsp" %>, // 감가상각(IFRS)
					<%@include file="./aba060ukrs16.jsp" %>, // 미착
					<%@include file="./aba060ukrs11.jsp" %>, // 미착대체
					<%@include file="./aba060ukrs17.jsp" %>,  // 외화환산평가
					<%@include file="./aba060ukrs18.jsp" %>,  // 지출결의
					<%@include file="./aba060ukrs19.jsp" %>, // 국고보고금(IFRS)
			    ]
	    	 }, {
	    	 	defaults:{
					xtype:'uniDetailForm',
	    	 	...
	    	 	tabchange : function( grouptabPanel, newCard, oldCard, eOpts )	{
		    		...
		    		if(Ext.isObject(oldCard))	{
		    			 if(UniAppManager.app._needSave())	{
	    					 ...
	    					 }else if(newCard.itemId == 'tab_Asst_Change_IFRS')	{
	    						UniAppManager.setToolbarButtons(['reset','newData'], true);
	    					 } else if(newCard.itemId == 'tab_Asst_Dep_IFRS')	{
	    						UniAppManager.setToolbarButtons(['reset','newData'], true);
	    					 }  else if(newCard.itemId == 'tab_Gov_Asst_Dep_IFRS')	{
	    						UniAppManager.setToolbarButtons(['reset','newData'], true);
	    					 } else {    			 	
		    			 		UniAppManager.app.fnInitBinding();
		    			 		UniAppManager.setToolbarButtons(['reset', 'excel', 'delete'],false);
	 ...
	 Unilite.Main( {
		...
		onQueryButtonDown : function()	{	
			...
			//자산변동(IFRS)
			else if(activeTab.getItemId() == "tab_Asst_Change_IFRS")	{
				UniAppManager.setToolbarButtons(['reset', 'newData', 'delete','deleteAll','query'],true);
				aiga240ukrvDetailStore.loadStoreRecords();	
			}
			//감가상각(IFRS)
			else if(activeTab.getItemId() == 'tab_Asst_Dep_IFRS')	{
				aiga210ukrvStore.loadStoreRecords();	
			}
			
			/* 미착 */
			else if (activeTab.getItemId() == 'tab_ship'){
				UniAppManager.setToolbarButtons(['reset', 'newData', 'delete','query'],true);
				aba060ukrs16Store.loadStoreRecords();
			}
			/* 외화환산평가 */
			else if (activeTab.getItemId() == 'tab_exchangeSlip'){
				UniAppManager.setToolbarButtons(['reset', 'newData', 'delete','query'],true);
				if(!Ext.getCmp('formExchangeSlip').getInvalidMessage()){
					return false;
				}
				aba060ukrs17Store.loadStoreRecords();
			}
			/* 지출결의 */
			else if (activeTab.getItemId() == 'tab_pay2'){
				UniAppManager.setToolbarButtons(['reset', 'newData', 'delete','query'],true);
				aba060ukrs18Store.loadStoreRecords();
			}
			/* 국고보조금(IFRS) */
			else if(activeTab.getItemId() == 'tab_Gov_Asst_Dep_IFRS')	{
				aiga410ukrvStore.loadStoreRecords();	
			}
		},
		...
		onNewDataButtonDown : function()	{
			...
			if(activeTab.getId() == 'tab_inoutSlip'){
				...
				// 20210323 매입매출구분이 존재하는 경우 행 추가시 default 세팅 추가 
				var gubun = panelDetail.down('#tab_inoutSlipForm').getValue('SALE_DIVI');
			...
			//자산변동 IFRS
			else if(activeTab.getId() == "tab_Asst_Change_IFRS")	{
				var form = panelDetail.down('#tab_Asst_Change_IFRS_Form');
				if(!form.getInvalidMessage()) return;	//필수체크
	        	 ...
	        	 panelDetail.down('#aiga240ukrvGrid').createRow(r);
	        	 UniAppManager.setToolbarButtons(['reset'],true);
			}
			//감가상각(IFRS)
			else if(activeTab.getItemId() == 'tab_Asst_Dep_IFRS')	{
				...
				if(!form.getInvalidMessage()) return;	//필수체크
	        	 ...
	        	 panelDetail.down('#aiga210ukrvGrid').createRow(r);
			}
			/* 미착  추가 */
			else if(activeTab.getId() == 'tab_ship'){
				var param =  panelDetail.down('#tab_shipForm').getValues();
				var compCode = UserInfo.compCode
				var r = {
					COMP_CODE : compCode
				}
				panelDetail.down('#aba060ukrs16Grid').createRow(r ,'PAY_METHD');
				
				UniAppManager.setToolbarButtons(['reset'],true);
			}
			/* 외화환산평가 */
			else if(activeTab.getId() == 'tab_exchangeSlip'){
				var param =  panelDetail.down('#tab_exchangeSlipForm').getValues();
				var compCode = UserInfo.compCode
				var r = {
					COMP_CODE : compCode
				}
				panelDetail.down('#aba060ukrs17Grid').createRow(r , 'GUBUN');
				
				UniAppManager.setToolbarButtons(['reset'],true);
			}	
			/* 지출결의  추가 */
			else if(activeTab.getId() == 'tab_pay2'){
				var param =  panelDetail.down('#tab_pay2Form').getValues();
				var compCode = UserInfo.compCode
				var r = {
					COMP_CODE : compCode
				}
				panelDetail.down('#aba060ukrs18Grid').createRow(r ,'PAY_DIVI');
			...
			//국고보조금(IFRS)
				...
				if(!form.getInvalidMessage()) return;	//필수체크
	        	 ...
	        	 panelDetail.down('#aiga410ukrvGrid').createRow(r);
			}
		...
		onSaveDataButtonDown: function (config) {
			...
			if (activeTab.getId() == 'tab_aba100ukr'){
				...
				var taxBase =  panelDetail.down('#tab_aba100ukr').getValues().TAX_BASE;
				var billCode = panelDetail.down('#tab_aba100ukr').getValue('BILL_DIV_CODE');

				if(taxBase !='0'){
					if(billCode == null){
					Unilite.messageBox('총괄신고사업장는  필수 입력입니다.');
					panelDetail.down('#tab_aba100ukr').getField("BILL_DIV_CODE").focus();
					return false;
					}
				}	
...
//		            		alert("변경된 데이타포맷을 적용하려면 프로그램 종료후 로그인을 다시 하셔야 합니다.");
			...
			//자산변동 IFRS
				...
				aiga240ukrvDetailStore.saveStore();
			}
			//감가상각 IFRS
			else if(activeTab.getItemId() == "tab_Asst_Dep_IFRS")	{
				aiga210ukrvStore.saveStore();
			}
			/* 미착 저장 */
			else if (activeTab.getId() == 'tab_ship'){
				aba060ukrs16Store.saveStore(config);
			}
			/* 외화환산평가 저장 */
			else if (activeTab.getId() == 'tab_exchangeSlip'){
				aba060ukrs17Store.saveStore(config);
			}
			/* 지출결의 저장 */
			else if (activeTab.getId() == 'tab_pay2'){
				aba060ukrs18Store.saveStore(config);
			}
			//국고보조금 IFRS
			else if(activeTab.getItemId() == "tab_Gov_Asst_Dep_IFRS")	{
				aiga410ukrvStore.saveStore();
			}
		},
		onDeleteDataButtonDown : function()	{
			...
			//자산변동 IFRS
				...
				var grid = panelDetail.down('#aiga240ukrvGrid');
				var selRow = grid.getSelectedRecord();
			...
			//감가상각 IFRS
				...
				var grid = panelDetail.down('#aiga210ukrvGrid');
				var selRow = grid.getSelectedRecord();
			...
			/* 미착 삭제 */
				...
				var grid = panelDetail.down('#aba060ukrs16Grid');
				var selRow = grid.getSelectionModel().getSelection()[0];
			...
			/*  외화환산평가 */
				...
				var grid = panelDetail.down('#aba060ukrs17Grid');
				var selRow = grid.getSelectionModel().getSelection()[0];
			...
			/* 지출결의 삭제 */
				...
				var grid = panelDetail.down('#aba060ukrs18Grid');
				var selRow = grid.getSelectionModel().getSelection()[0];
			...
			//국고보조금 IFRS
				...
				var grid = panelDetail.down('#aiga410ukrvGrid');
				var selRow = grid.getSelectedRecord();
		...
		onResetButtonDown:function() {
			...
			//자산변동 IFRS
				...
				panelDetail.down('#tab_Asst_Change_IFRS_Form').clearForm();
				panelDetail.down('#aiga240ukrvGrid').reset();
				aiga240ukrvDetailStore.clearData();
				UniAppManager.setToolbarButtons('save', false);
			}
			//감가상각 IFRS
			else if(activeTab.getItemId() == "tab_Asst_Dep_IFRS")	{
				panelDetail.down('#tab_Asst_Dep_IFRS_Form').clearForm();
				panelDetail.down('#aiga210ukrvGrid').reset();
				aiga210ukrvStore.clearData();
				UniAppManager.setToolbarButtons('save', false);
			}
			/* 미착 */
			else if (activeTab.getItemId() == 'tab_ship'){
				panelDetail.down('#tab_shipForm').clearForm();
				panelDetail.down('#aba060ukrs16Grid').reset();
				aba060ukrs16Store.loadData({});
			}
			/* 외화환산평가*/
			else if (activeTab.getItemId() == 'tab_exchangeSlip'){
				panelDetail.down('#tab_exchangeSlipForm').clearForm();
				panelDetail.down('#aba060ukrs17Grid').reset();
				aba060ukrs17Store.loadData({});
			}
			/* 지출결의 */
			else if (activeTab.getItemId() == 'tab_pay2'){
				panelDetail.down('#tab_pay2Form').clearForm();
				panelDetail.down('#aba060ukrs18Grid').reset();
				aba060ukrs18Store.loadData({});
			}
			//국고보조금 IFRS
			else if(activeTab.getItemId() == "tab_Gov_Asst_Dep_IFRS")	{
				panelDetail.down('#tab_Gov_Asst_Dep_IFRS_Form').clearForm();
				panelDetail.down('#aiga410ukrvGrid').reset();
				aiga210ukrvStore.clearData();
				UniAppManager.setToolbarButtons('save', false);
			}
		...
		onDeleteAllButtonDown: function() {		
			var activeTab = panelDetail.down('#aba060Tab').getActiveTab();
			if(activeTab.getItemId() == "tab_Asst_Change_IFRS")	{
		 		var grid = panelDetail.down('#aiga240ukrvGrid');
				var records = aiga240ukrvDetailStore.data.items;
				var isNewData = false;
				...
				if(isNewData){								//신규 레코드들만 있을시 그리드 리셋		   
		 		...
		 		var grid = panelDetail.down('#aiga210ukrvGrid');
				var records = aiga210ukrvStore.data.items;
				var isNewData = false;
				Ext.each(records, function(record,i) {
					if(record.phantom){						//신규 레코드일시 isNewData에 true를 반환
						isNewData = true;
				...
				if(isNewData){								//신규 레코드들만 있을시 그리드 리셋		   
		 		...
		 		var grid = panelDetail.down('#aiga410ukrvGrid');
				var records = aiga410ukrvStore.data.items;
				var isNewData = false;
				Ext.each(records, function(record,i) {
					if(record.phantom){						//신규 레코드일시 isNewData에 true를 반환
						isNewData = true;
		...
		loadTabData: function(tab, itemId){
			...
			/* 매입 */
				...
				var focusField = sForm.getField('ORDER_TYPE7');
			 	focusField.focus(500);
			}
			...
			/* 자산변동(IFRS) */
			else if (tab.getItemId() == 'tab_Asst_Change_IFRS'){
				UniAppManager.setToolbarButtons(['newData', 'delete','query'],true);
				
				var sForm = panelDetail.down('#tab_Asst_Change_IFRS_Form');
				sForm.clearForm();
				
				panelDetail.down('#aiga240ukrvGrid').reset();
				aiga240ukrvDetailStore.loadData({});
			}
			/* 미착 */
			else if (tab.getItemId() == 'tab_ship'){
				UniAppManager.setToolbarButtons(['newData', 'delete','query'],true);
				
				var sForm = panelDetail.down('#tab_shipForm');
				sForm.clearForm();
				
				panelDetail.down('#aba060ukrs16Grid').reset();
				aba060ukrs16Store.loadData({});
				
				var focusField = sForm.getField('PAY_METHD');
			 	focusField.focus(500);
			}
			/* 외화환산평가 */
			else if (tab.getItemId() == 'tab_exchangeSlip'){
				UniAppManager.setToolbarButtons(['newData', 'delete','query'],true);
				
				var sForm = panelDetail.down('#tab_exchangeSlipForm');
				sForm.clearForm();
				
				panelDetail.down('#aba060ukrs17Grid').reset();
				aba060ukrs17Store.loadData({});
				
				sForm.getField('GUBUN_17').focus(500);
				
			}
			/* 지출결의 */
			else if (tab.getItemId() == 'tab_pay2'){
				UniAppManager.setToolbarButtons(['newData', 'delete','query'],true);
				
				var sForm = panelDetail.down('#tab_pay2Form');
				sForm.clearForm();
				
				panelDetail.down('#aba060ukrs18Grid').reset();
				aba060ukrs18Store.loadData({});
				
				var focusField = sForm.getField('PAY_DIVI');
			 	focusField.focus(500);
			}
	...
	Unilite.createValidator('validator01', {
		...
		grid: Ext.getCmp("aiga240ukrvGrid"),
