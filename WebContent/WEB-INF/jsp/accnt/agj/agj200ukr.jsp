...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
		 ...
		 ,syncAll:'agj200ukrService.saveAll'
		}
	});
	 ...
	 * @type 
	 */
	var directMasterStore1 = Unilite.createStore('agj200ukrMasterStore1',getStoreConfig('agj200ukrGrid1','agj200ukrDetailForm1'));
...
//				}
	 ...
	 * @type 
	 */
	var masterGrid1 = Unilite.createGrid('agj200ukrGrid1', getGridConfig(directMasterStore1,'agj200ukrGrid1','agj200ukrDetailForm1', 0.65, true,'3'));
	
	var detailForm1 = Unilite.createForm('agj200ukrDetailForm1',  getAcFormConfig('agj200ukrDetailForm1',masterGrid1 ));
...
<%@include file="./agjSlip.jsp" %> 
	 ...
	 * 매입매출전표  Store
	 */
	var activeGrid = 'agj200ukrSalesGrid'
	var directMasterStore2 = Unilite.createStore('agj200ukrMasterStore2',getStoreConfig('agj200ukrGrid2','agj200ukrSaleDetailForm'));
		
	/**
	 * 매입매출전표  Grid 정의(Grid Panel)
	 * @type 
	 */
	var masterGrid2 = Unilite.createGrid('agj200ukrGrid2', getGridConfig(directMasterStore2, 'agj200ukrGrid2','agj200ukrSaleDetailForm', 0.25, false,'3'));
		
   /**
	* 매출/매입 전표 Detail 정보 모델
	*/
 
	Unilite.defineModel('agj200ukrSaleModel', {
	 ...
	 */
		...
		api: {
			read : 'agj200ukrService.selectSalesList',
			update : 'agj200ukrService.update',
			create : 'agj200ukrService.insert',
			destroy:'agj200ukrService.delete',
			syncAll:'agj200ukrService.saveAll'
	...
	var salesStore = Unilite.createStore('agj200ukrSalesStore',{
			model: 'agj200ukrSaleModel',
			autoLoad: false,
			uniOpt : {
	 ...
	 * @type 
	 */
	var salesGrid = Unilite.createGrid('agj200ukrSalesGrid', {
		flex: 0.35,
		itemId: 'agj200ukrSalesGrid',
		uniOpt:{
				...
				items: [{
					...
					handler: function(grid, rowIndex, colIndex) {
						...
						if(record.isValid()) {
							...
							param.AC_DATE =  UniDate.getDateStr(param.AC_DATE);
							...
							accntCommonService.fnGetAutoMethod(param, function(provider, response){
								...
								if(provider.length > 0) {
									...
									Ext.each(provider, function(providerRec, idx) {
										...
										var r = {
											'AC_DATE':salesRecord.get('AC_DATE'),
											...
											'AP_DATE':UniDate.getDateStr(salesRecord.get('AC_DATE')),
		...
		listeners:{
			...
			celldblclick: function( view, td, cellIndex, record, tr, rowIndex, e, eOpts ) {
				...
				if(cellIndex==8) {
					creditWIn = creditPopup (creditWIn, record, record.get('CREDIT_CODE'), "CREDIT_CODE", null, "CREDIT_NUM", null, null, null, 'VALUE','agj200ukrSalesGrid');			
				}
			},
	...
	var saleDetailForm = Unilite.createForm('agj200ukrSaleDetailForm',  getAcFormConfig('agj200ukrSaleDetailForm',masterGrid2) );
	
	tab = Unilite.createTabPanel('agj200ukrvTab',{		
		region:'center',
		activeTab: 0,
		border: false,
		items:[
...
//			panelResult.setValue('IN_DIV_CODE'	, UserInfo.divCode);
		...
		onSaveAsExcelButtonDown: function() {
			var masterGrid1 = Ext.getCmp('agj200ukrGrid1');
			 masterGrid1.downloadExcelXml();
		},
		onQueryButtonDown : function() {
			//조회 시, 검색조건 초기화 되어 주석처리 (20170116)
//			this.onResetButtonDown();
		...
		onNewDataButtonDown : function() {
			...
			if(!this.toolbar.down("#newData").isDisabled()) {
				...
				} else if(activeTabId == 'agj200ukrvTab2' ) {
					if(activeGrid=='agj200ukrSalesGrid') {
						if(!salesStore.isDirty() && !directMasterStore2.isDirty()) {
							Ext.getBody().mask();
							agj200ukrService.getPubNum({'PUB_DATE':UniDate.getDateStr(panelSearch.getValue('AC_DATE_TO'))}, function(provider, result ) {
								var selectedRecord = salesGrid.getSelectedRecord();
								var pDate = panelSearch.getValue('AC_DATE_TO');
								var rec={
									 'AC_DATE': UniDate.getDbDateStr(pDate),
		...
		onResetButtonDown:function() {
			...
			slipGrid1.reset();
			slipGrid2.reset();
			//20200721 추가: reset 버튼 호출 시, panelSearch, panelResult, params 초기화 후 fnInitBinding 호출하는 로직 추가
		...
		setEditableGrid:function(sInputPath, btnReset ) {
			...
			if(activeTabId == 'agjTab1' ) {
				if(sInputPath == null) {
					sInputPath = (gsInputPath != null) ? gsInputPath:panelSearch.getValue("INPUT_PATH");	
				}
				...
				if (!UniUtils.indexOf(sInputPath,['Z0','Y0'])) {
					if(btnReset) {
						UniAppManager.setToolbarButtons([ 'newData','deleteAll'],false);
					}
					tempEditMode = false;
		...
		processParams: function(params) {
			...
			if(params.PGM_ID == 'agb130skr') {
			...
			else if(params.PGM_ID == 'agb280skr') {
				...
				masterGrid1.getStore().loadStoreRecords(null, function(){
					...
					gsProcessPgmId	= params.PGM_ID;
					gsSlipNum 		= params.SLIP_NUM;
					if(params.INPUT_PATH == "Y0") {
						tab.setActiveTab(tab.down('#agj200ukrvTab2'));
				...
				//전표조회에서 넘어옴
				if(params.SLIP_TYPE == '1' && params.INPUT_PATH == 'Y0'){
					tab.setActiveTab(tab.down('#agj200ukrvTab2'));
					panelSearch.setValue('AC_DATE_FR',params.AC_DATE_FR);
					panelSearch.setValue('AC_DATE_TO',params.AC_DATE_TO);
...
//					panelResult.setValue('EX_NUM',params.EX_NUM);
			...
			else if(params.PGM_ID == 'agj250skr') {
				...
				if(params.INPUT_PATH == "Y0" || params.INPUT_PATH == "Y1" ) {
					tab.setActiveTab(tab.down('#agj200ukrvTab2'));
					panelSearch.setValue('AC_DATE_FR',params.AC_DATE_FR);
					panelSearch.setValue('AC_DATE_TO',params.AC_DATE_TO);
					panelSearch.setValue('INPUT_PATH',params.INPUT_PATH);
			...
			else if(params.PGM_ID == 'atx110skr') {
			...
			else if(params.PGM_ID == 'agb210skr') {
			...
			else if(params.PGM_ID == 'agb190skr') {
				...
				//masterGrid1.get('SLIP_SEQ').focus() 해당 순번과 동일한 SLIP_SEQ 에 focus 해야함.
			}
			
			else if(params.PGM_ID == 'agb160skr') {
			...
			else if(params.PGM_ID == 'agb161skr') {
			...
			else if(params.PGM_ID == 'agb165skr') {
				
				panelSearch.setValue('AC_DATE_FR',params.ORG_AC_DATE);
				panelSearch.setValue('AC_DATE_TO',params.ORG_AC_DATE);
				panelSearch.setValue('INPUT_PATH',params.INPUT_PATH);
				panelSearch.setValue('DIV_CODE',params.DIV_CODE);
				
				panelResult.setValue('AC_DATE_FR',params.ORG_AC_DATE);
				panelResult.setValue('AC_DATE_TO',params.ORG_AC_DATE);
				panelResult.setValue('INPUT_PATH',params.INPUT_PATH);
				panelResult.setValue('DIV_CODE',params.DIV_CODE);
			...
			else if(params.PGM_ID == 'agb240skr') {
			...
			else if(params.PGM_ID == 'afs510skr') {
			...
			// 거래처원장(매입/매출)
				...
				var basisNum = params.BASIS_NUM;
				
				var arr = basisNum.split('-')
				var acDate = arr[0];
				var slipNum = arr[1];
				
				panelSearch.setValue('AC_DATE_FR',acDate);
				panelSearch.setValue('AC_DATE_TO',acDate);
				panelSearch.setValue('INPUT_PATH','');
				panelSearch.setValue('DIV_CODE',params.DIV_CODE);
				
				panelResult.setValue('AC_DATE_FR',acDate);
				panelResult.setValue('AC_DATE_TO',acDate);
				...
				masterGrid1.getStore().loadStoreRecords(null, function(){
					var selModel = masterGrid1.getSelectionModel();
					var rowIdx = masterGrid1.getStore().findBy(function(item){
						return item.get("SLIP_NUM") == gsSlipNum;
					})
					if(rowIdx) masterGrid1.select(rowIdx);
				});
			}
		...
		fnAddSlipRecord:function(){
			...
			if(selectedRecord && !needNewSlipNum) {
				...
				r = {
					...
					'AP_DATE': UniDate.getDateStr(selectedRecord.get('AC_DATE')),
				...
				if(activeTabId == 'agjTab1' ) {
					r = {
						'AC_DATE':panelSearch.getValue('AC_DATE_TO'),
						'AC_DAY': Ext.Date.format(panelSearch.getValue('AC_DATE_TO'),'d'),
						
						'OLD_AC_DATE':panelSearch.getValue('AC_DATE_TO'),
						...
						'AP_DATE': UniDate.getDateStr(panelSearch.getValue('AC_DATE_TO')),
						'AP_USER_ID': UserInfo.userID,
					...
					r = {
						...
						'INPUT_USER_ID':UserInfo.userID,
						'CHARGE_CODE':panelSearch.getValue("CHARGE_CODE"),
						'AP_DATE':UniDate.getDbDateStr(salesRecord.get('AC_DATE')),
						'AP_USER_ID':UserInfo.userID,
						'AP_CHARGE_CODE':panelSearch.getValue("CHARGE_CODE")
					};
				} 
			...
			if(!needNewSlipNum && selectedRecord   ) {
				...
				// 전표번호 생성
				agj200ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(r.AC_DATE)}, function(provider, result ) {
					var aGrid = grid;
					var store = grid.getStore();
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName) {
				...
				case 'PUB_DATE' :
					...
					Ext.getBody().mask();
					agj200ukrService.getPubNum({'PUB_DATE':UniDate.getDateStr(newValue)}, function(provider, result ) {
