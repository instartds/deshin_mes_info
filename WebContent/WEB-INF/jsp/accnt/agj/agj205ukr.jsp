...
var baseInfo = {
	 ...
	 * @type 
	 */
	var directMasterStore1 = Unilite.createStore('agj205ukrMasterStore1',getStoreConfig('agj205ukrGrid1','agj205ukrDetailForm1'));
	...
	//createSearchForm
	var slipForm =  Unilite.createSearchForm('agj205ukrSlipForm',  {		
        itemId: 'agj205ukrSlipForm',
     ...
     * @type 
     */
    var masterGrid1 = Unilite.createGrid('agj205ukrGrid1', getGridConfig(directMasterStore1,'agj205ukrGrid1','agj205ukrDetailForm1', 0.65, true, '4'));
    
    var detailForm1 = Unilite.createForm('agj205ukrDetailForm1',  getAcFormConfig('agj205ukrDetailForm1',masterGrid1 ));
...
<%@include file="./agjSlip.jsp" %> 
    ...
    Unilite.Main({
		...
		onResetButtonDown:function() {
			...
			Ext.getBody().mask();
			agj200ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(panelSearch.getValue('AC_DATE'))}, function(provider, result ) {
 				Ext.getBody().unmask();
 				slipForm.setValue('SLIP_NUM', provider.SLIP_NUM);
 			})
		},
        ...
        setEditableGrid:function(sInputPath, btnReset )	{
        	var grid = UniAppManager.app.getActiveGrid();
        	if(sInputPath == null)  sInputPath = (gsInputPath!=null) ? sInputPath:panelSearch.getValue('INPUT_PATH');
        	...
        	if (!UniUtils.indexOf(sInputPath,['Z0'])) {
				if(btnReset) {
					UniAppManager.setToolbarButtons([ 'newData'],false);
				}
				tempEditMode = false;
...
//				panelSearch.setValue('AP_STS',params.AP_STS);
				...
				masterGrid1.getStore().loadStoreRecords(null, function(provider, response){
					if(!Ext.isEmpty(provider) && provider.length > 0)	{
...
//				panelSearch.setValue('AP_STS',params.AP_STS);
				...
				panelSearch.setValue('AC_DATE',params.ORG_AC_DATE);
				slipForm.setValue('AC_DATE',params.ORG_AC_DATE);
				panelSearch.setValue('INPUT_PATH',params.INPUT_PATH);
				panelSearch.setValue('DIV_CODE',params.DIV_CODE);
				
				panelResult.setValue('AC_DATE',params.ORG_AC_DATE);
				panelResult.setValue('INPUT_PATH',params.INPUT_PATH);
				panelResult.setValue('DIV_CODE',params.DIV_CODE);
                ...
                panelSearch.setValue('SLIP_NUM',params.EX_NUM);
                panelResult.setValue('SLIP_NUM',params.EX_NUM);
                
                panelSearch.setValue('INPUT_PATH',params.INPUT_PATH);
                panelResult.setValue('INPUT_PATH',params.INPUT_PATH);
                this.onQueryButtonDown();
                
            }else if(params.PGM_ID == 'agd360skr') {
...
//			}
    	...
    	setAccntInfo:function(record, detailForm)	{
    		...
    		//관리항목 유무 
    		for(var i=1; i <= 6 ; i++)	{
				if(!Ext.isEmpty(record.get('AC_CODE'+i.toString())))	{
					chkAcCode = true;
				}
			}
			if(!chkAcCode)	{
		...
		fnAddSlipRecord:function(){
			...
			var tSlipNum = slipForm.getValue('SLIP_NUM');
			...
			var r = {
				...
				'DEPT_CODE':Ext.isEmpty(selectedRecord) ? baseInfo.gsDeptCode : selectedRecord.get('DEPT_CODE'),
				'DEPT_NAME':Ext.isEmpty(selectedRecord) ? baseInfo.gsDeptName : selectedRecord.get('DEPT_NAME'),
				...
				'CHARGE_CODE'	: slipForm.getValue('CHARGE_CODE'),//Ext.isEmpty(selectedRecord) ? slipForm.getValue('CHARGE_CODE'):selectedRecord.get('CHARGE_CODE'),
				'AP_DATE': UniDate.getDateStr(slipForm.getValue('AC_DATE')),
				'AP_USER_ID': Ext.isEmpty(selectedRecord) ? UserInfo.userID: selectedRecord.get('AP_USER_ID'),
				'AP_CHARGE_CODE':slipForm.getValue('CHARGE_CODE')
		...
		fnCopySlip: function()	{
			...
			agj200ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(UniDate.get('today'))}, function(provider, result ) {
					slipForm.setValue('SLIP_NUM', provider.SLIP_NUM)
					...
					Ext.each(records.items, function(record, idx){
						...
						record.set('SLIP_NUM',slipForm.getValue('SLIP_NUM'));
					
						record.set('OLD_AC_DATE',slipForm.getValue('AC_DATE'));
						record.set('OLD_SLIP_NUM',slipForm.getValue('SLIP_NUM'));
						/*
						record.set('AP_STS','1');
						record.set('DIV_CODE',slipForm.getValue('DIV_CODE'));
						record.set('DEPT_CODE',UserInfo.deptCode);
						record.set('DEPT_NAME',UserInfo.deptName);
						*/
						record.set('INPUT_PATH',csINPUT_PATH);
						record.set('INPUT_DIVI',gsInputDivi);
						record.set('INPUT_USER_ID',UserInfo.userID);
						record.set('CHARGE_CODE',slipForm.getValue('CHARGE_CODE'));
						record.set('AP_DATE',UniDate.getDateStr(slipForm.getValue('AC_DATE')));
						record.set('AP_USER_ID',UserInfo.userID);
						record.set('AP_CHARGE_CODE',slipForm.getValue('CHARGE_CODE'));
		...
		cbGetTaxAmtForSales:function(taxRate,  record)	{
			...
			if(sProofKind == "24" || sProofKind == "13" || sProofKind == "14" ) {
				dTmpSupplyAmt = dSupplyAmt / ((100 + taxRateNum) * 0.01);
				dTaxAmt = Math.floor(dTmpSupplyAmt * taxRateNum * 0.01);
				dSupplyAmt = dSupplyAmt - dTaxAmt;
				record.set("SUPPLY_AMT_I",dSupplyAmt);
				record.set("TAX_AMT_I",dTaxAmt);
			}else {
				dTaxAmt = Math.floor(dSupplyAmt * taxRateNum * 0.01);
				record.set("TAX_AMT_I",dTaxAmt);
			}	
		},
