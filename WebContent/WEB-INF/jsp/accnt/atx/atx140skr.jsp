...
function appMain() {
	 ...
	 * @type 
	 */
	Unilite.defineModel('atx140Model', {
		fields: [
	...
	});		// End of Ext.define('atx140skrModel', {

	/** Store 정의(Service 정의)
	 * @type 
	 */
	var directMasterStore = Unilite.createStore('atx140MasterStore1',{
		model	: 'atx140Model',
		uniOpt	: {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read: 'atx140skrService.selectList'
			}
		},
	 ...
	 * @type 
	 */
	var masterGrid = Unilite.createGrid('atx140Grid1', {
		...
		tbar:[{
			...
			handler	: function() {
				...
				if(reportGubun.toUpperCase() == 'CLIP'){
					...
					var title1	= '';
					var title2	= '';
					if (docuDivi == '2' && divi == '1') {
						title1	= '매입처별계산서합계표(갑)';
						title2	= '매입처별계산서합계표(을)';
					} else if (docuDivi == '2' && divi == '2') {
						title1	= '매출처별계산서합계표(갑)';
						title2	= '매출처별계산서합계표(을)';
					}

					param.PGM_ID				= 'atx140skr';
					...
					var win = Ext.create('widget.ClipReport', {
						url		: CPATH+'/accnt/atx140clskrPrint.do',
						prgID	: 'atx140skr',
						extParam: param
					});
					win.center();
