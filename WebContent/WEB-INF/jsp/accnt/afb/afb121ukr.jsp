...
<t:appConfig pgmId="afb121ukr"  >
	<t:ExtComboStore comboType="BOR120" pgmId="afb121ukr"/> 				<!-- 사업장		-->
...
*/
    ...
    * @type 
    */
	Unilite.defineModel('Afb121ukrModel', {
		fields: [
			...
			{name: 'ACCNT'				, text: '원계정'			, type: 'string'	, allowBlank: false },
			...
			{name: 'BUDG_YYYYMM'		, text: '예산년월'			, type: 'string'	, allowBlank: false	, editable: false },
			...
			{name: 'DIVERT_ACCNT'		, text: '전용계정'			, type: 'string' , allowBlank: false  },
			{name: 'DIVERT_ACCNT_NAME'	, text: '전용계정명'		, type: 'string' , allowBlank: false  },
			{name: 'DIVERT_DEPT_CODE'	, text: '전용부서코드'		, type: 'string' },
			{name: 'DIVERT_DEPT_NAME'	, text: '전용부서명'		, type: 'string' },
			{name: 'DIVERT_BUDG_I'		, text: '전용금액'			, type: 'uniPrice' , allowBlank: false },
			{name: 'POSSIBLE_BUDG_AMT'	, text: '전용가능금액'		, type: 'uniPrice', editable: false },
			{name: 'REMARK'				, text: '신청사유'			, type: 'string'  , allowBlank: false },
			{name: 'DIVERT_SMT_NUM'		, text: '예산전용신청번호'	, type: 'string', editable: false },
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create	: 'afb121ukrService.insertList',				
			read	: 'afb121ukrService.selectList',
			update	: 'afb121ukrService.updateList',
			destroy	: 'afb121ukrService.deleteList',
			syncAll	: 'afb121ukrService.saveAll'
		}
	});
	
	var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'afb121ukrService.insertDetailRequest',
            syncAll: 'afb121ukrService.saveAllRequest'
        }
    }); 
    ...
    * @type 
    */               
	var masterStore = Unilite.createStore('afb121ukrMasterStore',{
		model	: 'Afb121ukrModel',
		uniOpt	: {
	...
	//	    	validateBlank	: false,	    			
			...
			Unilite.popup('ACCNT',{
				...
				listeners		: {
					...
					applyextparam: function(popup){
						popup.setExtParam({ 'ADD_QUERY': "BUDG_YN = 'Y' AND BUDGCHG_YN = 'Y' AND SLIP_SW = 'Y' AND GROUP_YN = 'N'" });			//WHERE절 추카 쿼리						
...
//							popup.setExtParam({'CHARGE_CODE': gsChargeCode});				//bParam(3)
     ...
     * @type 
     */
	var masterGrid = Unilite.createGrid('afb121ukrGrid', {
		store	: masterStore,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {
...
//          disabled:true,
	 				...
	 				listeners: {'onSelected': {
							fn: function(records, type) {
								...
								var params = {
									...
									ACCNT        : grdRecord.get('ACCNT')
								}												
								afb121ukrService.getPosBudgAmt(params, function(provider, response)	{							
	...
	//							popup.setExtParam({'CHARGE_CODE': gsChargeCode});				//bParam(3)
        	...
        	{dataIndex: 'BUDG_YYYYMM'			, width: 90		, align: 'center'
        	...
        	{dataIndex: 'DIVERT_YYYYMM'			, width: 90		, align: 'center'		, hidden: true	
        	/*,renderer : function(val,metaDate,record,rowIndex,colIndex,store,view) {
					if(!Ext.isEmpty(val)){
						return  val.substring(0,4) + '.' + val.substring(4,6);
                	}
...
//									BUDG_YYYYMM  : grdRecord.get('BUDG_YYYYMM'),
//									DEPT_CODE    : grdRecord.get('DEPT_CODE'),
//									DIVERT_ACCNT : grdRecord.get('DIVERT_ACCNT')
//							
//								}												
//								afb121ukrService.getPosBudgAmt(params, function(provider, response)	{							
//									if(!Ext.isEmpty(provider)){
...
//							grdRecord.set('POSSIBLE_BUDG_AMT', '');
	...
	//							popup.setExtParam({'CHARGE_CODE': gsChargeCode});				//bParam(3)
        	...
        	{dataIndex: 'DIVERT_BUDG_I'			, width: 110},
        	{dataIndex: 'POSSIBLE_BUDG_AMT'		, width: 110},
        	{dataIndex: 'REMARK'				, width: 170},
        	{dataIndex: 'DIVERT_SMT_NUM'		, width: 110},
        	{dataIndex: 'APPR_LINE'				, width: 220},
        	{dataIndex: 'RETURN_MSG'			, width: 180}
...
/*				if(e.record.data.DOC_STATUS == '10'){
			    	if(UniUtils.indexOf(e.field, ['ACCNT','ACCNT_NAME','SEND_J_AMT_I'])){
			      		return true;
...
//			panelResult.setValue('DATE_TO', UniDate.get('today'));
		...
		onNewDataButtonDown : function() {	
			var r = {				
				...
				//BUDG_YYYYMM		: new Date().getMonth()
				//DIVERT_YYYYMM	: UniDate.get('endOfMonth')
			};
	...
	Unilite.createValidator('validator01', {
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			var rv = true;
			switch(fieldName) {
				case "BUDG_YYYYMM" :		// 예산년월(To)
					var budgYyyymm = record.get('BUDG_YYYYMM');	
					//if(BUDG_YYYYMM) {
						record.set('DIVERT_YYYYMM', budgYyyymm);
					//}					
				break;
				
				case "DIVERT_BUDG_I" :
					//var selectedRecords = masterGrid.getSelectedRecords(); //체크셀 있을 때
					
					var accnt = record.get('ACCNT')
					var accntName = record.get('ACCNT_NAME')
					if (accnt == '' || accntName == '') {
						rv = '원계정 선택 후 금액 입력이 가능합니다.'	
						break;
					}

//					var divertBudg_i = record.get('DIVERT_BUDG_I'); //현재 레코드의 금액
//					if (divertBudg_i < 0) {
//						rv = '음수는 입력할 수 없습니다.';	
//						break;
//					}
					...
					//var divertBudg_i = record.get('DIVERT_BUDG_I'); //현재 레코드의 금액
					var accnt = record.get('ACCNT')				//현재 레코드의 계정
					var docStatus = record.get('DOC_STATUS')	
					var posBudg_i = record.get('POSSIBLE_BUDG_AMT');
 
					var sumDivertBudg = 0;     //금액의 합
					var sumDivertBudgi = 0; 	//

					Ext.each(records, function(item,i){
						
						if (docStatus == item.get('DOC_STATUS')) {
							if (accnt == item.get('ACCNT')) {     //레코드의 계정과 입력하려는 금액에 있는 계정 비교
								sumDivertBudg = item.get('DIVERT_BUDG_I');
								sumDivertBudgi = sumDivertBudgi + sumDivertBudg;
							}		
						}		
					//alert("posBudg_i:::::::::::::"+posBudg_i);
						
					//alert("sumDivertBudgi:::::::::::::"+sumDivertBudgi);
					//var tot = sumDivertBudgi - oldValue + newValue ; 
					//alert("totDivertBudg:::::::::::::"+tot);
					
						if (posBudg_i < sumDivertBudgi - oldValue + newValue) {
							rv = '전용신청금액이 전용가능금액을 초과하였습니다.'
						}							
                    });
				break;				
                
                case "ACCNT" :
                    record.set('DIVERT_BUDG_I',0);
                break;
                case "ACCNT_NAME" :
                    record.set('DIVERT_BUDG_I',0);
