...
<t:appConfig pgmId="afb123ukr"  >
	<t:ExtComboStore comboType="BOR120" pgmId="afb123ukr"/> 				<!-- 사업장		-->
...
*/
    ...
    * @type 
    */
	Unilite.defineModel('Afb123ukrModel', {
		fields: [
			...
			{name: 'BUDG_YYYYMM'		, text: '예산년월(Fr)'		, type: 'string'	, editable: false   , allowBlank: false },
			...
			{name: 'DIVERT_BUDG_I'		, text: '금액'				, type: 'uniPrice' , allowBlank: false },
			{name: 'POSSIBLE_BUDG_AMT'	, text: '이월가능금액'		, type: 'uniPrice', editable: false },
			{name: 'REMARK'				, text: '신청사유'			, type: 'string'	, allowBlank: false  },
			{name: 'DIVERT_SMT_NUM'		, text: '예산이월신청번호'	, type: 'string', editable: false },
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create	: 'afb123ukrService.insertList',				
			read	: 'afb123ukrService.selectList',
			update	: 'afb123ukrService.updateList',
			destroy	: 'afb123ukrService.deleteList',
			syncAll	: 'afb123ukrService.saveAll'
		}
	});
	
	var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'afb123ukrService.insertDetailRequest',
            syncAll: 'afb123ukrService.saveAllRequest'
        }
    }); 
    ...
    * @type 
    */               
	var masterStore = Unilite.createStore('afb123ukrMasterStore',{
		model	: 'Afb123ukrModel',
		uniOpt	: {
	...
	//	    	validateBlank	: false,	    			
			...
			Unilite.popup('ACCNT',{
				...
				listeners		: {
					...
					applyextparam: function(popup){
						popup.setExtParam({ 'ADD_QUERY': "BUDG_YN = 'Y' AND BUDGCAR_YN = 'Y' AND SLIP_SW = 'Y' AND GROUP_YN = 'N'" });			//WHERE절 추카 쿼리						
...
//							popup.setExtParam({'CHARGE_CODE': gsChargeCode});				//bParam(3)
     ...
     * @type 
     */
	var masterGrid = Unilite.createGrid('afb123ukrGrid', {
		store	: masterStore,
		region	: 'center',
		layout	: 'fit',
		uniOpt	: {
    				...
    				DBtextFieldName: 'ACCNT_CODE',
	 				listeners: {'onSelected': {
							fn: function(records, type) {
								...
								afb123ukrService.getPosBudgAmt(params, function(provider, response)	{							
...
//							popup.setExtParam({'CHARGE_CODE': gsChargeCode});				//bParam(3)
        	...
        	{dataIndex: 'APPR_LINE'				, width: 230},
        	{dataIndex: 'RETURN_MSG'			, width: 190}   	
        ],
        listeners: {
        	beforeedit: function(editor, e){
				if(e.record.data.DOC_STATUS == '10'){
					if(UniUtils.indexOf(e.field, ['ACCNT','ACCNT_NAME','DIVERT_YYYYMM','DIVERT_ACCNT','DIVERT_ACCNT_NAME','DIVERT_BUDG_I','REMARK'])){
			      		return true;
			    	} /*else {
			    		return false; 
			    	}*/	
...
//			panelResult.setValue('DATE_TO', UniDate.get('today'));			
		...
		onNewDataButtonDown : function() {	
			var r = {				
				...
				//DEVERT_YYYYMM	: UniDate.get('today')
			};
	...
	Unilite.createValidator('validator01', {
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			var rv = true;
			switch(fieldName) {
				case "DIVERT_YYYYMM" :		// 예산년월(To)
					var tempDate = record.get('BUDG_YYYYMM')+'01';	
						tempDate = tempDate.substring(0,4) + '/' + tempDate.substring(4,6) + '/' + tempDate.substring(6,8);
					var budgYyyymm = new Date(tempDate);
...
//					sToMonth = UniDate.add(transDate, {months: + 11});		
					var newDate = newValue + '01';	
						newDate = newDate.substring(0,4) + '/' + newDate.substring(4,6) + '/' + newDate.substring(6,8);
					var newYyyymm = new Date(newDate);
					
					if(UniDate.getDbDateStr(newYyyymm).substring(0,4) != UniDate.getDbDateStr(budgYyyymm).substring(0,4)){	//이월가능월
						rv = "당해 월을 입력하세요.";
					}					
				break;
