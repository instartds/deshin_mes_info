...
<t:appConfig pgmId="afb400ukr"  >
	...
	<t:ExtComboStore comboType="AU" comboCode="A199" /> <!-- 예산코드 Mapping항목 	 -->
</t:appConfig>
...
function appMain() {
	...
	Unilite.defineTreeModel('Afb400Model', {
	    ...
	    fields: [
			{name: 'BUDG_CODE'			, text: '예산코드'			, type: 'string', allowBlank: false, isPk:true, pkGen:'user' },
			{name: 'BUDG_NAME'			, text: '예산코드명'		, type: 'string', allowBlank: false, maxLength: 100},
			...
			{name: 'CODE_LEVEL'			, text: '예산코드레벨'		, type: 'string'},
			{name: 'TREE_LEVEL'			, text: '상위예산코드'		, type: 'string'},
	 ...
	 */					
	 var directProxy= Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			 read   : 'afb400ukrService.selectList'
        	,update : 'afb400ukrService.updateMulti'
			,create : 'afb400ukrService.insertMulti'
			,destroy: 'afb400ukrService.deleteMulti'
			,syncAll: 'afb400ukrService.saveAll'
		}
	});
...
//						})
            	...
            	items: [{
						...
						handler : function(records) {
							if(Ext.getCmp('RDO_SELECT').getChecked()[0].inputValue == '1') {
								...
								afb400ukrService.selectAfb400t(param, 
									function(provider1, response) {
										afb400ukrService.selectAfb410t(param, 
											function(provider2, response)	{
												if(!Ext.isEmpty(provider1) || !Ext.isEmpty(provider2)) {
													if(confirm(Msg.fsbMsgA0194)) {
														...
														afb400ukrService.insertDataCopy(param, function(provider, response)	{ 
															alert("완료 되었습니다.");
														});
											...
											function(provider2, response)	{
												if(!Ext.isEmpty(provider1) || !Ext.isEmpty(provider2)) {
													if(confirm(Msg.fsbMsgA0194)) {
														...
														afb400ukrService.deleteDataCopy(param, function(provider, response)	{ 
															alert("완료 되었습니다.");
														});
														lastYearCopyWindow.hide();
													}
    ...
    var masterGrid = Unilite.createTreeGrid('afb400ukrGrid', {
		...
		columns:[
        	...
        	{dataIndex: 'CTL_CAL_UNIT'				, width: 66},
        	{dataIndex: 'CTL_TERM_UNIT'				, width: 66},
...
//			{dataIndex: 'CODE_LEVEL'				, width: 66, hidden: true},
  	...
  	Unilite.Main({                                                 
		...
		onDeleteDataButtonDown : function()	{
			...
			afb400ukrService.selectAfb410tBeforeSave(params, 
