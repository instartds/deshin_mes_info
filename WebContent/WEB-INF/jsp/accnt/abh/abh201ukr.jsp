...
var abh201ukrRef1Window; // 검색팝업 관련

function appMain() {
	var directProxyAutoSign = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'abh201ukrService.insertDetailAutoSign',
            syncAll: 'abh201ukrService.saveAllAutoSign'
        }
    }); 
/*	var directMasterProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'abh201ukrService.selectMaster',
			update: 'abh201ukrService.updateMaster',
			create: 'abh201ukrService.insertMaster',
			destroy: 'abh201ukrService.deleteMaster',
			syncAll: 'abh201ukrService.saveAllMaster'
		}
	});	*/
	
	var directDetailProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'abh201ukrService.selectDetail',
			update: 'abh201ukrService.updateDetail',
			create: 'abh201ukrService.insertDetail',
			destroy: 'abh201ukrService.deleteDetail',
			syncAll: 'abh201ukrService.saveAllDetail'
		}
	});	
	
	
/*	Unilite.defineModel('abh201ukrMasterModel', {
	    fields: [
	    	{name: 'PAY_METH'		    ,text: '지급방법'		,type: 'string',allowBlank:false,comboType:'AU', comboCode:'A395'},
	...
	Unilite.defineModel('abh201ukrDetailModel', {
	    fields: [
...
//	    	{name: 'SET_METH'			,text: '결제방법'					,type: 'string',comboType:'AU', comboCode:'A395'},//ABH220T
	...
	Unilite.defineModel('Abh201ukrRef1Model', {	//검색팝업관련
	    fields: [
			{name: 'SEND_NUM'			,text: '지급번호'		,type: 'string'},
			{name: 'PAY_METH'			,text: '지급방법'		,type: 'string',comboType:'AU', comboCode:'A395'},
...
/*	var directMasterStore = Unilite.createStore('Abh201ukrDirectMasterStore',{
		model: 'abh201ukrMasterModel',
		uniOpt: {
...
//						}
                ...
                var grid = Ext.getCmp('Abh201ukrMasterGrid');
	...
	*/
	var directDetailStore = Unilite.createStore('abh220ukrDetailStore', {
		...
		saveStore: function() {				
			...
			console.log("list:", list);
			var paramMaster= panelResult.getValues();	
//			paramMaster.SEND_NUM = panelResult.getValue('SEND_NUM');
...
//							subForm1.setValue('SEND_NUM','');
                ...
                var grid = Ext.getCmp('Abh201ukrDetailGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
...
/*			if(!Ext.isEmpty(panelResult.getValue('SEND_NUM'))){
				param.SEND_NUM = panelResult.getValue('SEND_NUM');
        	}else{
        		param.SEND_NUM = '';
        	}*/
			console.log( param );
	...
	var abh201ukrRef1Store = Unilite.createStore('abh201ukrRef1Store', {//검색팝업관련
		model: 'Abh201ukrRef1Model',
        autoLoad: false,
        uniOpt : {
        	isMaster: false,			// 상위 버튼 연결
        	editable: false,			// 수정 모드 사용
        ...
        proxy: {
            type: 'direct',
            api: {
            	read: 'abh201ukrService.selectRef1'                	
            }
        },
       
        loadStoreRecords : function()	{
			var param= abh201ukrRef1Search.getValues();	
...
//	    			,'ADD_QUERY': "(SPEC_DIVI = 'K' OR SPEC_DIVI = 'K2') AND SLIP_SW = 'Y' AND GROUP_YN = 'N'"
		...
		//	    			,'ADD_QUERY': "(SPEC_DIVI = 'K' OR SPEC_DIVI = 'K2') AND SLIP_SW = 'Y' AND GROUP_YN = 'N'"
            ...
            width: 80,  
            handler: function() {
            	if(confirm('해당 이체지급번호에 대한 데이터가 삭제 됩니다.\n계속 진행 하시겠습니까?')) {
                    var param = {
                        SEND_NUM: panelResult.getValue('SEND_NUM')
                    }
                    Ext.getCmp('pageAll').getEl().mask('작업 중...','loading-indicator');
                    abh201ukrService.abh201Delete(param, function(provider, response){                           
                        if(provider){   
...
//                            detailGrid.getEl().mask('작업 중...','loading-indicator');
                            ...
                            abh201ukrService.bankNameQuery(param, function(provider, response)  {                           
                                if(provider){   
...
//                            detailGrid.getEl().mask('작업 중...','loading-indicator');
                            ...
                            abh201ukrService.abh201create(param, function(provider, response)  {                           
                                if(provider){   
                                    UniAppManager.updateStatus(Msg.sMB014);
                                    directDetailStore.loadStoreRecords();
//                                    directMasterStore.loadStoreRecords();
...
//                        }
	...
	var abh201ukrRef1Search = Unilite.createSearchForm('abh201ukrRef1Form', {//검색팝업관련
...
//			tableAttrs: { /*style: 'border : 1px solid #ced9e7;',*/width: '100%'}
    	...
    	items :[{ 
			...
			xtype: 'uniCombobox',
		    comboType:'AU',
			comboCode:'A395',   //  A101 에서 A395로 변경 됨 으로 밑에 로직 다시 확인 필요
			listeners: {
				change: function(field, newValue, oldValue, eOpts) {
					if(newValue == '10'){
						abh201ukrRef1Search.getField('BANK_CODE').setReadOnly(false);	
						abh201ukrRef1Search.getField('BANK_NAME').setReadOnly(false);
						abh201ukrRef1Search.getField('tc1').setReadOnly(true);	
						abh201ukrRef1Search.getField('tn1').setReadOnly(true);
					}else if(newValue == '20'){
						abh201ukrRef1Search.getField('tc1').setReadOnly(false);	
						abh201ukrRef1Search.getField('tn1').setReadOnly(false);
						abh201ukrRef1Search.getField('BANK_CODE').setReadOnly(true);	
						abh201ukrRef1Search.getField('BANK_NAME').setReadOnly(true);
					}else{
						abh201ukrRef1Search.getField('BANK_CODE').setReadOnly(true);	
						abh201ukrRef1Search.getField('BANK_NAME').setReadOnly(true);
						abh201ukrRef1Search.getField('tc1').setReadOnly(true);	
				    ...
				    listeners: {
				    	onSelected: {
							fn: function(records, type) {
								abh201ukrRef1Search.setValue('BANK_ACCOUNT', records[0]["DEPOSIT_NUM"]);
		                	},
							scope: this
						},
						onClear: function(type)	{
							abh201ukrRef1Search.setValue('BANK_ACCOUNT', '');
						}
					}
				}),
			{
...
/*	var masterGrid = Unilite.createGrid('Abh201ukrMasterGrid', {
        ...
        columns:[
        	...
        	{ dataIndex: 'TOT_AMT_I'			 		,  		flex:1width: 120},
...
//        	{ dataIndex: 'AGREE_YN'		 		 		,  		width: 120,hidden:false}
    ...
    var detailGrid = Unilite.createGrid('Abh201ukrDetailGrid', {
//    	id:'detailGridId',
//    	split:true,
    	features: [{
			id: 'detailGridSubTotal',	
...
//        	{ dataIndex: 'AGREE_YN'		 		 		, width: 120,hidden:false},
        	{ dataIndex: 'PRE_DATE'			 		 	, width: 90, hidden:true},
...
//        	{ dataIndex: 'SET_METH'			 		 	, width: 80},
        	...
        	{ dataIndex: 'RCPT_NAME'					, width: 120	, hidden:true},
        	{ dataIndex: 'CMS_TRANS_YN'					, width: 120	, hidden:true},
        	{ dataIndex: 'RCPT_RESULT_MSG'				, width: 120	, hidden:true},
        	{ dataIndex: 'RCPT_STATE_NUM'				, width: 120	, hidden:true},
	...
	var abh201ukrRef1Grid = Unilite.createGrid('abh201ukrRef1Grid', {//검색팝업 관련
        layout : 'fit',
        excelTitle: '검색팝업',
    	store: abh201ukrRef1Store,
    	uniOpt: {
    		onLoadSelectFirst: true  
        },
        features: [{
...
//			dock:'bottom',
		...
		listeners: {	
			onGridDblClick:function(grid, record, cellIndex, colName) {
				if(colName =="BANK_ACCOUNT_EXPOS") {
					abh201ukrRef1Grid.openCryptAcntNumPopup(record);
				}else{
				
					abh201ukrRef1Grid.returnData(record);
					abh201ukrRef1Window.hide();
...
//			textarray = divCodeRec.split("-");
	        ...
	        panelResult.setValue('SEND_NUM', record.get('SEND_NUM'));
       	},
		openCryptAcntNumPopup:function( record )	{
			if(record)	{
				var params = {'BANK_ACCOUNT': record.get('BANK_ACCOUNT'), 'GUBUN_FLAG': '2', 'INPUT_YN': 'N'}
    ...
    function openAbh201ukrRef1Window() {    		//검색팝업 관련
//  		if(!UniAppManager.app.checkForNewDetail()) return false;
  		
		if(!abh201ukrRef1Window) {
			abh201ukrRef1Window = Ext.create('widget.uniDetailWindow', {
                title: '검색',
                width: 900,				                
                height: 500,
                layout:{type:'vbox', align:'stretch'},
                items: [abh201ukrRef1Search, abh201ukrRef1Grid],
                tbar:  ['->',
					{	
						itemId : 'saveBtn',
						text: '조회',
						handler: function() {
							if(!abh201ukrRef1Search.getInvalidMessage()) return;
							abh201ukrRef1Store.loadStoreRecords();
						},
						disabled: false
					},{
						itemId : 'closeBtn',
...
//		 				abh201ukrRef1Search.setValue('ST_AC_DATE',panelSearch.getValue('ST_DATE'));
//		 				abh201ukrRef1Search.setValue('FR_AC_DATE',panelSearch.getValue('FR_DATE'));
//		 				abh201ukrRef1Search.setValue('TO_AC_DATE',panelSearch.getValue('TO_DATE'));	
//		 				abh201ukrRef1Search.setValue('AC_PROJECT_CODE',panelSearch.getValue('AC_PROJECT_CODE'));	
//		 				abh201ukrRef1Search.setValue('AC_PROJECT_NAME',panelSearch.getValue('AC_PROJECT_NAME'));	
		 				
//		 				panelResult.down('#btnAutoSlip').setHtml(Msg.sMAW034);
						abh201ukrRef1Search.setValue('SEND_DATE_FR',UniDate.get('startOfMonth'));
						abh201ukrRef1Search.setValue('SEND_DATE_TO',UniDate.get('today'));
		...
		abh201ukrRef1Window.center();
		abh201ukrRef1Window.show();
    }
...
/*			if(!UserInfo.appOption.collapseLeftSearch)	{
			...
			UniAppManager.setToolbarButtons(['query','save','delete'], false);
			
			
			this.setDefault(params);
			
			
			/*if(!Ext.isEmpty(params.SEND_NUM)){
...
//            Ext.getCmp('autoSignBtn2').setHidden(true);
		...
		processParams: function(params) {
            ...
            if(params.PGM_ID == 'abh220ukr' || params.PGM_ID == 'abh230ukr') {
                panelResult.setValue('SEND_NUM',params.SEND_NUM);
                directDetailStore.loadStoreRecords();
            }
		},
