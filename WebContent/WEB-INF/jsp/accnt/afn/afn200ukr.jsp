...
function appMain() {  
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'afn200ukrService.selectList',
			update: 'afn200ukrService.updateDetail',
			create: 'afn200ukrService.insertDetail',
			destroy: 'afn200ukrService.deleteDetail',
			syncAll: 'afn200ukrService.saveAll'
		}
	});	
	/**
	 *   Model 정의 
	 * @type 
	 */
	Unilite.defineModel('Afn200ukrModel', {
	    fields: [  	  
	 ...
	 * @type 
	 */					
	var directMasterStore = Unilite.createStore('afn200ukrMasterStore1',{
		model: 'Afn200ukrModel',
		uniOpt: {
...
//			}
    ...
    var masterGrid = Unilite.createGrid('afn200ukrGrid1', {
    	layout : 'fit',
        region : 'center',
	 ...
	 Unilite.Main( {
		...
		onDeleteDataButtonDown: function() {
			...
			if(selRow.phantom === true)	{
				...
				var params = {
					'NOTE_NUM': selRow.get("NOTE_NUM"),
					'BANK_CODE' : selRow.get("BANK_CODE")
				};
				afn200ukrService.selectCheckDelete(params, function(responseText, provider){
					if(responseText && responseText.length > 0)	{
						if(responseText[0]["NOTE_NUM"] && responseText[0]["BANK_CODE"])	{
							alert("이미 등록된 어음은 삭제할 수 없습니다.")
						} else {
							masterGrid.deleteSelectedRow();
						}
					}else {
...
//				}
	...
	var validation = Unilite.createValidator('validator01', {
        ...
        validate: function( type, fieldName, newValue, oldValue, record, detailGrid, editor, e) {
            ...
            switch(fieldName) {
                case "INSOCK_DATE" :	//  입고일
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("FLOAT_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("FLOAT_DATE")) < 0 )	{
                			alert("입고일은 발행일보다 클 수 없습니다.");
                			rv = false;
                		}
                	}
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("SET_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("SET_DATE")) < 0 )	{
                			alert("입고일은 결제일보다 클 수 없습니다.");
                			rv =  false;
                		}
                	}
                break;
                case "FLOAT_DATE" :	//발행일
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("INSOCK_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("INSOCK_DATE")) > 0 )	{
                			alert("발행일은 입고일보다 작을 수 없습니다.");
                			rv =  false;
                		}
                	}
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("SET_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("SET_DATE")) < 0 )	{
                			alert("결제일이 발행일보다 작을 수 없습니다.");
                			rv =  false;
                		}
                	}
                break;
                case "SET_DATE" : //결제일
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("INSOCK_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("INSOCK_DATE")) > 0 )	{
                			alert("결제일은 입고일보다 작을 수 없습니다.");
                			rv =  false;
                		}
                	}
                	if(!Ext.isEmpty(newValue) && !Ext.isEmpty(record.get("FLOAT_DATE")))	{
                		if(UniDate.diffDays(newValue, record.get("FLOAT_DATE")) > 0 )	{
                			alert("결제일은 발행일보다 작을 수 없습니다.");
                			rv =  false;
                		}
                	}
                break;
