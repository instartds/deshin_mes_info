...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
		read : 'acm100ukrService.selectList'
		 ,update : 'acm100ukrService.update'
		 ,create : 'acm100ukrService.insert'
		 ,destroy:'acm100ukrService.delete'
		 ,syncAll:'acm100ukrService.saveAll'
		}
	});
				...
				{		
					...
					listeners: {
						change: function(field, newValue, oldValue, eOpts) {
							panelResult.setValue('ACCOUNT_NUM', newValue);
						}
					}
				},{
					fieldLabel:'기표구분',
	...
	var panelResult = Unilite.createSearchForm('panelResultForm', {
		...
		items: [
			...
			listeners: {
				change: function(field, newValue, oldValue, eOpts) {
					panelSearch.setValue('ACCOUNT_NUM', newValue);
				}
			}
		},{
			fieldLabel:'기표구분',
				...
				items	: [{
					...
					handler	: function(){
							if(panelSearch.getForm().isValid()) {
								...
								acm100ukrService.fnAcmbankMapping(param, function(provider, response) {
...
<%@include file="./acmGridConfig.jsp" %> 	
	 ...
	 * 매입매출전표  Store
	 */
	var activeGrid = 'acm100ukrSalesGrid'
	var directMasterStore2 = Unilite.createStore('acm100ukrMasterStore2',getStoreConfig('acm100ukrGrid2','acm100ukrSaleDetailForm'));
		
	/**
	 * 매입매출전표  Grid 정의(Grid Panel)
	 * @type 
	 */
	var masterGrid2 = Unilite.createGrid('acm100ukrGrid2', getGridConfig(directMasterStore2, 'acm100ukrGrid2','acm100ukrSaleDetailForm', 0.25, false,'3'));

	var detailForm1 = Unilite.createForm('acm100ukrDetailForm1',  getAcFormConfig('acm100ukrDetailForm1',salesGrid ));
	
   /**
	* 매출/매입 전표 Detail 정보 모델
	*/
 
	Unilite.defineModel('acm100ukrSaleModel', {
		// pkGen : user, system(default)
		fields: [ 		
				  {name: 'AC_DAY'			,text:'일자'			,type : 'string'} 
				, {name: 'AUTO_NUM'	,text:'순번'	,type : 'string', editable:false}
				...
				,{name: 'IN_AMT_I'	,text:'입금액'	,type : 'uniPrice', editable:false} 
				,{name: 'OUT_AMT_I'	,text:'출금액'	,type : 'uniPrice', editable:false} 
				,{name: 'JAN_AMT_I'	,text:'잔액'	,type : 'uniPrice', editable:false} 
				,{name: 'CUSTOM_CODE'		,text:'거래처'			,type : 'string'} 
				,{name: 'CUSTOM_NAME'		,text:'거래처명'			,type : 'string'} 
				,{name: 'ACCNT'	,text:'상대계정코드'	,type : 'string', allowBlank:false} 
				,{name: 'ACCNT_NAME'	,text:'상대계정과목명'	,type : 'string', allowBlank:false}	
				...
				,{name: 'SAVE_CODE'		,text:'통장코드'	,type : 'string', editable:false} 
				,{name: 'SAVE_NAME'		,text:'통장명'	,type : 'string', editable:false} 
				,{name: 'ACCOUNT_NUM'		,text:'계좌번호'	,type : 'string', editable:false} 
				,{name: 'ACCOUNT_NUM_EXPOS'		,text:'계좌번호'	,type : 'string', editable:false, defaultValue:'***************'} 
				,{name: 'ACCOUNT_NUM_MASK'		,text:'계좌번호'	,type : 'string', editable:false, defaultValue:'***************'} 
				,{name: 'BANK_CODE'		,text:'은행'	,type : 'string', editable:false} 
				,{name: 'BANK_NAME'		,text:'은행'	,type : 'string', editable:false} 				
				,{name: 'INOUT_TIME'		,text:'승인시간'	,type : 'string', editable:false} 
				...
				,{name: 'OLD_PUB_DATE'   	,text:'OLD_PUB_DATE'	,type : 'uniDate'} 
	 ...
	 */
		...
		api: {
			read : 'acm100ukrService.selectSalesList',
			update : 'acm100ukrService.update',
			create : 'acm100ukrService.insert',
			destroy:'acm100ukrService.delete',
			syncAll:'acm100ukrService.saveAll'
	...
	var salesStore = Unilite.createStore('acm100ukrSalesStore',{
			model: 'acm100ukrSaleModel',
			autoLoad: false,
			uniOpt : {
...
//				allDeletable:true,			// 삭제 가능 여부 
	 ...
	 * @type 
	 */
	var salesGrid = Unilite.createGrid('acm100ukrSalesGrid', {
		region:'center',
		//region:'north',
		//width		: '100%',
		//height		: '50%',
		itemId: 'acm100ukrSalesGrid',
		uniOpt:{
				...
				items: [{
					...
					handler: function(grid, rowIndex, colIndex) {
						...
						if(record.isValid()) {
							...
							Ext.getBody().mask();
							acm100ukrService.fnGetAutoMethod(param, function(provider, response){
...
//											newRecord.set("ASST_DIVI", salesRecord.get("ASST_DIVI"));
										...
										//계좌정보, 은행정보, 거래처정보 값설정
										UniAppManager.app.fnSetAcCode(newRecord, "O1",  salesRecord.get("SAVE_CODE"),  salesRecord.get("SAVE_NAME"));
										UniAppManager.app.fnSetAcCode(newRecord, "A3",  salesRecord.get("BANK_CODE"),  salesRecord.get("BANK_NAME"));
...
//										UniAppManager.app.fnSetDefaultAcCodeO1(newRecord, salesRecord);
				...
				,{ dataIndex: 'OUT_AMT_I',  width: 100	, summaryType: 'sum'}
				...
				,{ dataIndex: 'ACCNT'		,width: 100 , align:'center',
					editor:Unilite.popup('ACCNT_AC_G', {
						...
						listeners:{
							scope:this,
							onSelected:function(records, type ) {	
								salesGrid.uniOpt.currentRecord.set('ACCNT', records[0].ACCNT_CODE);
								salesGrid.uniOpt.currentRecord.set('ACCNT_NAME', records[0].ACCNT_NAME);
...
//								UniAppManager.app.loadDataAccntInfo(salesGrid.uniOpt.currentRecord, detailForm1, records[0]);
			...
			,{ dataIndex: 'PROOF_KIND',  width: 110, hidden:true}			
			...
			,{ dataIndex: 'ACCOUNT_NUM_MASK',  width: 130 } 
			,{ dataIndex: 'ACCOUNT_NUM',  width: 130 , hidden:true}			
...
//					saleDetailForm.down('#formFieldArea1').removeAll();
			...
			celldblclick: function( view, td, cellIndex, record, tr, rowIndex, e, eOpts ) {
				...
				if(cellIndex==8) {
					creditWIn = creditPopup (creditWIn, record, record.get('CREDIT_CODE'), "CREDIT_CODE", null, "CREDIT_NUM", null, null, null, 'VALUE','acm100ukrSalesGrid');			
				}
			},
	...
	var saleDetailForm = Unilite.createForm('acm100ukrSaleDetailForm',  getAcFormConfig('acm100ukrSaleDetailForm',masterGrid2) );
	...
	Unilite.Main({
		...
		fnInitBinding : function(params) {
			...
			//panelSearch.setValue('AP_DATE_FR'	, UniDate.get('today'));
			...
			//panelResult.setValue('AP_DATE_FR'	, UniDate.get('today'));
...
//					salesGrid.getStore().remove(sm.getSelection());
		...
		fnAddSlipRecord:function(){
			...
			if(!needNewSlipNum && selectedRecord   ) {
				...
				// 전표번호 생성
				acm100ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(r.AC_DATE)}, function(provider, result ) {
					var aGrid = grid;
					var store = grid.getStore();
		...
		fnSetDefaultAcCodeO1:function(record, mRecord) {
			var form = this.getActiveForm();
			for(var i = 1; i <= 6; i++) {
				if(record.get('AC_CODE'+i.toString()) == "O1" ) {
					record.set('AC_DATA'+i.toString(), mRecord.get('SAVE_CODE'));
					record.set('AC_DATA_NAME'+i.toString(), mRecord.get('SAVE_NAME'));
					form.setValue('AC_DATA'+i.toString(), mRecord.get('SAVE_CODE'));
					form.setValue('AC_DATA_NAME'+i.toString(), mRecord.get('SAVE_NAME'));
				}
				if(record.get('AC_CODE'+i.toString()) == "A3" ) {
					record.set('AC_DATA'+i.toString(), mRecord.get('BANK_CODE'));
					record.set('AC_DATA_NAME'+i.toString(), mRecord.get('BANK_NAME'));
					form.setValue('AC_DATA'+i.toString(), mRecord.get('BANK_CODE'));
					form.setValue('AC_DATA_NAME'+i.toString(), mRecord.get('BANK_NAME'));
				}
			};
		},
		...
		fnProofKindPopUp:function(record, newValue, gridId) {
			...
			//현금영수증(불공제)
				...
				//creditWIn = creditPopup (creditWIn, record, record.get('ACCOUNT_NUM'), "ACCOUNT_NUM", null, null, null, null,  'VALUE');			
