...
//				deletable:false,		 // 삭제 가능 여부
			...
			,saveStore : function(config) {	
				...
				Ext.each(insertRecords, function(rec){
					...
					paramMaster = (gsNewData ? salesGrid.getStore().getAt(gsNewData).data : salesGrid.getSelectedRecord().data);
...
//				}
				...
				if(inValidRecs.length == 0 ) {
					config = {
						...
						success: function(batch, option) {
							...
							Ext.each(batch.getOperations(), function(operation, idx){
								...
								if(responseText) {
									var savedRecord = (gsNewData ? salesGrid.getStore().getAt(gsNewData) : salesGrid.getSelectedRecord());
									
									//savedRecord.set('PUB_NUM',responseText.PUB_NUM );
									//savedRecord.set('OLD_PUB_NUM',responseText.PUB_NUM );
									
									if(responseText.SLIP_YN == 'Y') {
										savedRecord.set('SLIP_NUM',responseText.SLIP_NUM );
										savedRecord.set('AC_DATE',responseText.AC_DATE );
									}
									else {
										savedRecord.set('SLIP_NUM','0' );
										savedRecord.set('AC_DATE','' );
									}
									savedRecord.set('SLIP_YN',responseText.SLIP_YN );
					...
					if(!Ext.isEmpty(checkDuplicateRecords)) {
						...
						var me = this;
						Ext.getBody().mask();
						acm200ukrService.selectDuplicate(checkDuplicateParam, function(provider, response){
							Ext.getBody().unmask();
							var vMessage = ""
	...
	function getGridConfig(store, gridID,   detailFormID, sflex, isTbar, gridType)  {
		...
		if(detailFormID == 'acm100ukrSaleDetailForm' || detailFormID == 'acm200ukrSaleDetailForm'){
			slipNumEdit = false;
		}
...
//				  },
				...
				,{ dataIndex: 'CREDIT_NUM_MASK'		,width: 150, hidden:true }
				,{ dataIndex: 'DEPT_NAME'	,width: 100 ,
				   editor:Unilite.popup('DEPT_G',{
						showValue:false,
						extParam:{'TXT_SEARCH':'', 'isClearSearchTxt':true}, 
						autoPopup: true,
						listeners:{
							'onSelected':function(records, type) {
								var aGridID = gridID;
				...
				Unilite.createGrid('', {
					...
					returnData: function() {
						...
						if(selRecord){
							...
							//1:회계전표/2:결의전표
							if(csSLIP_TYPE == "2") {
								acm200ukrService.getSlipNum({'EX_DATE':UniDate.getDbDateStr(acDate)}, function(provider2, result ) {
			...
			*/
			if(crSum > drSum) {//차변계정 생성
				...
				accntCommonService.getForeignProfitAccnt(param, function(providerRec){
					...
					if(rowIndex == 0) {
						if(viewStore) {
							rowIndex =  viewStore.data.items.length;
						} else {
							rowIndex = store.data.items.length;
						}
					}
