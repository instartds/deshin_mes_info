...
var gsInputPath		= "C2", gsInputDivi;
...
var clickedGrid = 'acm110ukrGrid1';
...
function appMain() {
	...
	var baseInfo = {
	 ...
	 var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
			api: {
				read	: 'acm110ukrService.selectList',
				update	: 'acm110ukrService.update',
				create	: 'acm110ukrService.insert',
				destroy	: 'acm110ukrService.delete',
				syncAll	: 'acm110ukrService.saveAll'
			}
		});
	var detailDirectProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'acm110ukrService.selectList4'
		}
	});
	 ...
	 * @type 
	 */
	Unilite.defineModel('acm110ukrv', {
		fields: [ 
			...
			//========================================================================================
			,{name: 'IN_AMT_I'			,text: '입금액'			,type: 'uniPrice'}
			,{name: 'OUT_AMT_I'			,text: '출금액'			,type: 'uniPrice'}
			,{name: 'JAN_AMT_I'			,text: '잔액'	 			,type: 'uniPrice'}
			,{name: 'DEPT_NAME'			,text: '귀속부서'			,type: 'string'}
			
			,{name: 'INOUT_DATE'		,text: '입출금일자'			,type: 'string'}
			,{name: 'CMS_SEQ'			,text: '자동순번'			,type: 'string'}
		]
	});
	// 미결 참조 filter
	var inOutRefFilter = new Ext.util.Filter({
		filterFn: function(item) {
			return item.get("FLAG") == "R";
		}
	});
	var directMasterStore1 = Unilite.createStore('acm110ukrMasterStore1',{
		model: 'acm110ukrv',
		autoLoad: false,
		uniOpt : {
		...
		saveStore : function(config) {
			...
			Ext.each(saveRec, function(rec, idx){
				...
				var viewinOutGrid = detailStore.getAt(viewPendIdx)
				if(viewIdx > -1)	{
			...
			if(saveRec.length > 0 && inOutGrid.getStore().count() == 0) {
				chk = false;
			...
			write: function(store, operation, eOpts) {
				...
				if(operation.success && (operation._response.method =="insert" || operation._response.method =="update" || operation._response.method =="delete" )	)	{
					...
					Ext.each(pendRecord.items, function(item, idx){
						...
						if(tRec)	{
							item.set('IN_AMT_T'		, tRec.get("IN_AMT_T"));
							item.set('OUT_AMT_T'	, tRec.get("OUT_AMT_T"));
							item.set('JAN_AMT_T'	, tRec.get("JAN_AMT_T"));
							item.set('OPR_FLAG'		, "L");
						}
					})
	...
	var directMasterStore2 = Unilite.createStore('acm110ukrMasterStore2',{
			model    : 'acm110ukrv',
			autoLoad : false,
			uniOpt   : {
	...
	Unilite.defineModel('Acm110ukrInOutModel', {
	...
	var detailStore = Unilite.createStore('acm110ukrDetailStore',{
		model: 'Acm110ukrInOutModel',
		uniOpt : {
			isMaster:   true,		// 상위 버튼 연결 
			editable:   true,		// 수정 모드 사용 
			deletable:  true,		// 삭제 가능 여부 
		...
		listeners:{
			load:function( store , records , successful , operation , eOpts ) {
				...
				Ext.each(records, function(record, idx){
					...
					masterRecords.push({
						...
						, 'DR_AMT_I'		: record.get("DR_CR") == "1" ? record.get("IN_AMT_I")  : 0
						, 'CR_AMT_I'		: record.get("DR_CR") == "2" ? record.get("OUT_AMT_I") : 0
						...
						, 'AMT_I'			: record.get("DR_CR") == "1" ? record.get("IN_AMT_I"):record.get("OUT_AMT_I")
						...
						, 'IN_AMT_I'		: record.get('IN_AMT_I')
						, 'OUT_AMT_I'		: record.get('OUT_AMT_I')
						, 'JAN_AMT_I'		: record.get('JAN_AMT_I')
			...
			update:function(store, record, operation, modifiedFieldNames, details, eOpts){
				var sRecordIdx = directMasterStore1.findBy(function(rec,id){
					return rec.get("CMS_SEQ")     == record.get("CMS_SEQ")
			...
			remove : function( store , records , index , isMove , eOpts ) {
				...
				Ext.each(records, function(record, idx){
					...
					// 참조 데이터 
					inOutRefStore.removeFilter(inOutRefFilter);
					var refDataIdx = inOutRefStore.findBy(function(fRecord){
							return fRecord.get("CMS_SEQ") == record.get("CMS_SEQ");
						});
					if(refDataIdx > -1) {
						var refData = inOutRefStore.getAt(refDataIdx);
						refData.set("FLAG","R");
					} 
					inOutRefStore.addFilter(inOutRefFilter);
				});
	...
	//createSearchForm
	var slipForm =  Unilite.createSearchForm('acm110ukrSlipForm', {
		itemId		: 'acm110ukrSlipForm',
					...
					listeners	: {
						change:function(field, newValue, oldValue) {
							...
							if(!Ext.isEmpty(newValue) &&Ext.isDate(newValue)) {
								...
								Ext.getBody().unmask();
								acm110ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(newValue)}, function(provider, result ) {
					...
					listeners	: {
						change:function(field, newValue, oldValue, eOpts) {
							...
							if(!Ext.isEmpty(value) && Ext.isDate(slipForm.getValue("AC_DATE"))) {
								...
								acm110ukrService.getSlipNum({'AC_DATE':sDate}, function(provider, result ) {
	...
	var masterGrid1 = Unilite.createGrid('acm110ukrGrid1', {
			layout : 'fit',
			flex:.6,
			uniOpt:{
				copiedRow          : true,
					...
					editor:Unilite.popup('ACCNT_AC_G', {
						...
						listeners:{
							...
							onSelected:function(records, type ) {
								var aGridID = 'acm110ukrGrid1', aDetailFormID='acm110ukrDetailForm1';
			...
			listeners:{
				...
				selectionChange: function( grid, selected, eOpts ) {
					if(selected.length == 1) {
						 ...
						 */
						var aDetailFormID='acm110ukrDetailForm1';
						var form = Ext.getCmp(aDetailFormID);
				...
				celldblclick: function( view, td, cellIndex, record, tr, rowIndex, e, eOpts ) {
					if(e.position.column.dataIndex=='CREDIT_NUM') {
						UniAppManager.app.fnProofKindPopUp(record, null, 'acm110ukrGrid1');
	...
	var detailForm1 = Unilite.createForm('acm110ukrDetailForm1',  getAcFormConfig('acm110ukrDetailForm1', masterGrid1 ));

	// 입출금내역 grid
	var inOutGrid = Unilite.createGrid('acm110ukrinOutGrid', { 
		layout    : 'fit',
		minHeight : 100,
		store     : detailStore,
		
		viewConfig:{
			itemId:'acm110ukrinOutGrid'
		},
		uniOpt:{
		...
		columns:  [
			...
			{ dataIndex: 'ACCNT'			, width: 80  , align:'center'},
			{ dataIndex: 'ACCNT_NAME'		, width: 100 , align:'center'},
			
			{ dataIndex: 'BANK_ACCOUNT'		, width: 120 },
			{ dataIndex: 'SAVE_CODE'		, width: 80  , align:'center'},
			{ dataIndex: 'SAVE_NAME'		, width: 180 },
			{ dataIndex: 'IN_AMT_I'			, width: 100 },
			{ dataIndex: 'OUT_AMT_I'		, width: 100 },
			{ dataIndex: 'JAN_AMT_I'		, width: 100 },
			{ dataIndex: 'MONEY_UNIT'		, width: 80  , align:'center'},
			
			{ dataIndex: 'INOUT_DATE'		, width: 45  , hidden:true},
			{ dataIndex: 'CMS_SEQ'			, width: 80  , hidden:true},
				...
				editor: Unilite.popup('CUST_G',{
					...
					listeners:{ 'onSelected': {
						fn: function(records, type){
							var aGridID = 'acm110ukrinOutGrid'
							var grid = Ext.getCmp(aGridID);
							var grdRecord = grid.uniOpt.currentRecord;
		...
		listeners:{
			beforeedit : function(editor, context, eOpts) {
				...
				if(context.field == "CUSTOM_CODE" || context.field == "CUSTOM_NAME" || context.field == "REMARK"
			...
			containerclick : function(view,  e , eOpts) {
				activeGrid = 'acm110ukrinOutGrid';
				clickedGrid = 'acm110ukrinOutGrid';
			},
	...
	var inOutRefForm = Unilite.createSearchForm('acm110ukrInOutRefForm', {
		itemId			: 'acm110ukrInOutRefForm',
		height			: 85,
		disabled		: false,
		trackResetOnLoad:false,
		border			: true,
	...
	Unilite.defineModel('Acm110ukrInOutRefModel', {
		fields: [
				 {name: 'BANK_ACCOUNT'			,text: '계좌번호'		,type: 'string'},
				 {name: 'SAVE_CODE'				,text: '통장코드'		,type: 'string'},
				 {name: 'SAVE_NAME'				,text: '통장명'		,type: 'string'},
	...
	var inOutRefStore = Unilite.createStore('Acm110ukrInOutRefStore',{
		model: 'Acm110ukrInOutRefModel',
		uniOpt : {
			isMaster  : false,	// 상위 버튼 연결 
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'acm110ukrService.selectInOutRef'
			}
		},
		filters: [inOutRefFilter],
		loadStoreRecords : function() {
			var form = Ext.getCmp('acm110ukrInOutRefForm');
	...
	var inOutRefGrid = Unilite.createGrid('acm110ukrRefGrid', {
		// for tab	  
		store		: inOutRefStore,
		layout		: 'fit',
		region		: 'center',
		...
		inOutApply:function(gRec, sRec, slipSeq, isNew) {
			// 체크한 row 값 세팅
			sRec = inOutGrid.createRow({"SLIP_SEQ"		: slipSeq,
										"DR_CR"			: gRec.get("IN_AMT_I") != '0' ? '1' : gRec.get("OUT_AMT_I") != 0 ? '2' : '', // 입금액  : 차변, 출금액 : 대변
										"ACCNT"			: gRec.get("ACCNT"),
										"ACCNT_NAME"	: gRec.get("ACCNT_NAME"),
										"BANK_ACCOUNT"	: gRec.get("BANK_ACCOUNT"),
										"SAVE_CODE"		: gRec.get("SAVE_CODE"),
										"SAVE_NAME"		: gRec.get("SAVE_NAME"),
										"IN_AMT_I"		: gRec.get("IN_AMT_I"),
										"OUT_AMT_I"		: gRec.get("OUT_AMT_I"),
										"JAN_AMT_I"		: gRec.get("JAN_AMT_I"),
										"MONEY_UNIT"	: gRec.get("MONEY_UNIT"),
										"BANK_CODE"		: gRec.get("BANK_CODE"),
										"BANK_NAME"		: gRec.get("BANK_NAME"),
										"INOUT_DATE"	: gRec.get("INOUT_DATE"),
										"CMS_SEQ"		: gRec.get("CMS_SEQ"),
										"COUSTOM_CODE"	: '',
										"COUSTOM_NAME"	: '',
			...
			sRec.set("OLD_SLIP_NUM"		, slipForm.getValue("SLIP_NUM"));
			sRec.set("P_ACCNT"			, "")
			...
			// accnt 정보 세팅
			UniAppManager.app.setAccntInfo(sRec);
		},
		...
		tbar: [{
			...
			align   : 'left',
			handler : function() {
				var selectedRecords = inOutRefGrid.getSelectedRecords();                    // 참조 Grid에서 체크한 rows
				var maxSlipSeq      = Unilite.nvl(directMasterStore1.max("SLIP_SEQ"),0)+1;  // 입출금내역 순번
				...
				Ext.each(selectedRecords, function(selRecord, idx){
					inOutRefGrid.inOutApply(selRecord, null, maxSlipSeq,true);  // 입출금내역 데이터 CreateRow
		...
		'->','->','->','->','->','->','->','->','->','->','->','->','->','->','->','->','->','->','->'],
		columns: [
			{ dataIndex: 'BANK_ACCOUNT'		, width: 100 , align:'center'},
			{ dataIndex: 'SAVE_CODE'		, width: 70  , align:'center'},
			...
			{ dataIndex: 'REMARK'			, width: 200 },
			{ dataIndex: 'ACCNT'			, width: 80  , align:'center'},
			{ dataIndex: 'ACCNT_NAME'		, width: 80  , align:'center'},
			{ dataIndex: 'MONEY_UNIT'		, width: 80  , align:'center'}
		],
		listeners: {
			select: function(grid, record, index, rowIndex, eOpts ){
				// 통장master에 통장정보가 없을 경우
				if(Ext.isEmpty(record.get('SAVE_CODE')) || record.get('SAVE_CODE') == ''){
					Unilite.messageBox("계좌가 등록되어있지 않습니다.\n계좌정보를 확인해주세요.");
	...
	var pend_tab = Unilite.createTabPanel('acm110ukrvTab',{	 
		region:'south',
		flex:1,
		activeTab: 0,
		border: false,
	...
	function openSearchPopup() {
		if(!searchPopup) {
				...
				var searchDirctProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
					api: {
						read : 'acm110ukrService.selectSearch'
					}
				});
				// store
					...
					items: [{
						...
						Unilite.createGrid('', {
							...
							,returnData: function() {
								...
								var girdStore = Ext.data.StoreManager.lookup('acm110ukrMasterStore2');
	...
	Unilite.Main({
		...
		onQueryButtonDown : function() {
			...
			if(pend_tab.getActiveTab().getItemId() == 'pandTab1') {
				...
				inOutRefStore.loadStoreRecords();
			}
		},
		...
		onDeleteDataButtonDown : function() {
			if(confirm('현재행을 삭제 합니다.\n 삭제 하시겠습니까?')) {
				if(clickedGrid == 'acm110ukrGrid1') {
					masterGrid1.deleteSelectedRow();
					if(masterGrid1.getStore().getCount() == 0) {
						detailForm1.down('#formFieldArea1').removeAll();
					}
				}else if(clickedGrid == 'acm110ukrinOutGrid') {
					inOutGrid.deleteSelectedRow();
				}
			}
		},
		...
		onResetButtonDown:function(params) {
			...
			var masterGrid1 = Ext.getCmp('acm110ukrGrid1');
			masterGrid1.reset();
			...
			// 입출금내역 grid
			inOutGrid.reset();
			inOutGrid.getStore().commitChanges();
			inOutRefGrid.reset();
			
			// 참조 조회조건 form
			inOutRefForm.clearForm();
			inOutRefForm.setValue('FROM_TRADE_DATE' , UniDate.get('startOfMonth'));
			inOutRefForm.setValue('TO_TRADE_DATE'   , UniDate.get('today'));
			...
			if(Ext.isEmpty(params) || (params && Ext.isEmpty(params.SLIP_NUM)))	{
				acm110ukrService.getSlipNum({'AC_DATE':UniDate.getDbDateStr(slipForm.getValue('AC_DATE'))}, function(provider, result ) {
						slipForm.setValue("SLIP_NUM", provider.SLIP_NUM); 
						panelSearch.setValue('SLIP_NUM',provider.SLIP_NUM);
				})
			}
