...
//전체선택 버튼관련 변수 선언
	...
	var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
            create	: 'agd131ukrService.runProcedure',
            syncAll	: 'agd131ukrService.callProcedure'
		}
	});	
	 ...
	 * @type 
	 */
	Unilite.defineModel('Agd131Model', {
	   fields: [
			...
			{name: 'COLLECT_DATE'  		, text: '수금일'				, type: 'uniDate'},
			{name: 'CUSTOM_CODE'		, text: '거래처코드'				, type: 'string'},
			{name: 'CUSTOM_NAME'		, text: '거래처명'				, type: 'string'},
			{name: 'COLLECT_TYPE'       , text: '수금유형'				, type: 'string',	comboType: "AU", comboCode: "S017"},
	...
	});		// End of Ext.define('Agd131ukrModel', {
	 ...
	 * @type 
	 */					
	var MasterStore = Unilite.createStore('agd131MasterStore',{
		model: 'Agd131Model',
		uniOpt: {
        ...
        proxy: {
           type: 'direct',
            api: {			
                read: 'agd131ukrService.selectList'                	
            }
        },
		...
		listeners: {
           	load: function(store, records, successful, eOpts) {
				...
				Ext.each(records, function(record, i){	
	        		 amountTotI = record.get('COLLECT_AMT') +amountTotI	
		    	}); 
	    		panelSearch.setValue('TOT_SELECTED_AMT', amountTotI);
...
//           		if(gsConfirmYN == "Y"){
    ...
    var buttonStore = Unilite.createStore('Agd131UkrButtonStore',{      
        uniOpt: {
        ...
        saveStore: function(buttonFlag) {             
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('agd131Grid1');
				...
				listeners: {
					...
					onSelected: {
						fn: function(records, type) {
							...
							panelSearch.setValue('COMPANY_NUM', records[0].COMPANY_NUM);
			  	...
			  	listeners: {
					onSelected: {
						fn: function(records, type) {
							...
							panelResult.setValue('COMPANY_NUM', panelSearch.getValue('COMPANY_NUM'));
						},
						scope: this
					},
					onClear: function(type)	{
						...
						Ext.each(records,  function(record, index, records){
							if(selDesel == 0){
								if(record.get('AP_STS') != '2' && record.get('CHOICE') == false) {
				    				...
				    				sumSaleTaxAmtI = sumSaleTaxAmtI + record.get('COLLECT_AMT');
						...
						if(panelSearch.setAllFieldsReadOnly(true)){
							...
							if(addResult.getValue('COUNT') != 0){  
								if(Ext.getCmp('rdoSelect4').getChecked()[0].inputValue == '1'){
										...
										agd131ukrService.procButton(param, 
								...
								if(Ext.getCmp('rdoSelect4').getChecked()[0].inputValue == '2'){
										var param = panelSearch.getValues();
										panelSearch.getEl().mask('로딩중...','loading-indicator');
										agd131ukrService.cancButton(param, 
											function(provider, response) {
												if(provider) {	
			  	...
			  	listeners: {
					onSelected: {
						fn: function(records, type) {
							...
							panelSearch.setValue('CUSTOM_CODE', records[0].CUSTOM_CODE);
							panelSearch.setValue('CUSTOM_NAME', records[0].CUSTOM_NAME);				 																							
							panelSearch.setValue('COMPANY_NUM', panelResult.getValue('COMPANY_NUM'));
						},
						scope: this
					},
					onClear: function(type)	{
...
/*						필수체크 팝업 2번 발생하여 주석처리
			...
			items:[/*{			   
				...
				handler : function() {
					...
					if(checkCount > 0){
       					Ext.getCmp('selDesel2').setText('전체취소');
						selDesel = 1;
						checkCount = 0;
					}else if (checkCount <= 0){
...
//				labelWidth: 60,
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('agd131Grid1', {
    	layout : 'fit',
        region : 'center',
		store: MasterStore,
    	features: [{
...
//					var grdRecord = masterGrid.getStore().getAt(rowIndex);
    				sumSaleTaxAmtI = sumSaleTaxAmtI + selectRecord.get('COLLECT_AMT');
					sumCheckedCount = sumCheckedCount + 1;
	    			panelSearch.setValue('SELECTED_AMT', sumSaleTaxAmtI)
	    			panelSearch.setValue('COUNT', sumCheckedCount)
	    			addResult.setValue('SELECTED_AMT', sumSaleTaxAmtI)
...
//	  				Ext.getCmp('selDesel2').setText('전체선택');
        ...
        columns: [        
			/*  selmodel로 로직 변경
        	{dataIndex: 'CHOICE'        	, width: 40,	locked: true,	xtype: 'checkcolumn',	align:'center',
				listeners: {
					beforecheckchange: function( CheckColumn, rowIndex, checked, eOpts ){
						...
						if(checked == true){
							...
							if(grdRecord.get('AP_STS') == '2') {
			    				...
			    				sumSaleTaxAmtI = sumSaleTaxAmtI + grdRecord.get('COLLECT_AMT');
								sumCheckedCount = sumCheckedCount + 1;
				    			panelSearch.setValue('SELECTED_AMT', sumSaleTaxAmtI)
				    			panelSearch.setValue('COUNT', sumCheckedCount)
				    			addResult.setValue('SELECTED_AMT', sumSaleTaxAmtI)
			...
			{dataIndex: 'DIV_CODE'			, width: 130/*,	locked: true*/}, 				
			{dataIndex: 'COLLECT_NUM'		, width: 106/*,	locked: true*/}, 				
			{dataIndex: 'COLLECT_DATE'  	, width: 80/*,	locked: true*/}, 				
			{dataIndex: 'CUSTOM_CODE'		, width: 100}, 				
			{dataIndex: 'CUSTOM_NAME'		, width: 180}, 				
			{dataIndex: 'COLLECT_TYPE'		, width: 93}, 				
			...
			{dataIndex: 'CHARGE_NAME'		, width: 70}, 				
