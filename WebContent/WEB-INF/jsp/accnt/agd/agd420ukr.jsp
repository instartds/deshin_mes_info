...
<script type="text/javascript" >
									...
									//선수금계정이 확정되지 않아서 선수금 팝업 추후 확정되면 applyExtparam 수정
...
var sumSaleTaxAmtI	= 0;
	...
	var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
            create	: 'agd420ukrService.runProcedure',
            syncAll	: 'agd420ukrService.callProcedure'
		}
	});	
	 ...
	 * @type 
	 */
	Unilite.defineModel('agd420ukrModel', {
	   fields: [
			...
			{name: 'RE_AC_DATE'			, text: '선수금 -> 매출 회계일자'		, type: 'string'  },
			{name: 'RE_SLIP_NUM'		, text: '선수금 -> 매출 회계번호'		, type: 'string'  },
			{name: 'INSERT_DB_USER'		, text: '입력자'					, type: 'string'  },
			{name: 'INSERT_DB_TIME'		, text: '입력일'					, type: 'uniDate' },
			{name: 'UPDATE_DB_USER'		, text: '수정자'					, type: 'string'  },
			{name: 'UPDATE_DB_TIME'		, text: '수정일'					, type: 'uniDate' }
	    ]
	});		// End of Ext.define('agd420ukrModel', {
	  
	/* Store 정의(Service 정의)
	 * @type 
	 */					
	var masterStore = Unilite.createStore('agd420ukrmasterStore',{
		model	: 'agd420ukrModel',
		uniOpt	: {
        ...
        proxy	: {
           type	: 'direct',
            api	: {			
                read: 'agd420ukrService.selectList'                	
            }
        },
    ...
    var buttonStore = Unilite.createStore('agd420ukrButtonStore',{ 
        ...
        saveStore: function(buttonFlag) {             
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('agd420ukrGrid');
		...
		items		: [{	
        	...
        	Unilite.popup('ACCNT',{
				...
				listeners: {
					...
					applyextparam: function(popup){
						popup.setExtParam({'ADD_QUERY': "PROFIT_DIVI = 'A'"});			//선수금계정이 설정되지 않아서 이부분 추후 수정
					}
				}
		    }),	{	            		
				fieldLabel	: '구분',	
...
//		,tableAttrs: {style: 'border : 1px solid #ced9e7;'/*, width: '100%'*/}
     ...
     * @type 
     */
    var masterGrid = Unilite.createGrid('agd420ukrGrid', {
		store	: masterStore,
    	layout	: 'fit',
        region	: 'center',
    	features: [
