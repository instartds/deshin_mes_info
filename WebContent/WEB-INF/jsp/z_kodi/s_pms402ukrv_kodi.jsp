...
<t:appConfig pgmId="s_pms402ukrv_kodi"  >
...
var selectedMasterGrid = 's_pms402ukrv_kodiGrid';
...
var activeGridId = 's_pms402ukrv_kodiGrid';
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_pms402ukrv_kodiService.selectMaster',
			update	: 's_pms402ukrv_kodiService.updateDetail',
			create	: 's_pms402ukrv_kodiService.insertDetail',
			destroy	: 's_pms402ukrv_kodiService.deleteDetail',
			syncAll	: 's_pms402ukrv_kodiService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_pms402ukrv_kodiService.selectDetail',
			update	: 's_pms402ukrv_kodiService.updateDetail2',
			create	: 's_pms402ukrv_kodiService.insertDetail2',
			destroy	: 's_pms402ukrv_kodiService.deleteDetail2',
			syncAll	: 's_pms402ukrv_kodiService.saveAll2'
		}
	});



	var masterForm = Unilite.createSearchPanel('s_pms402ukrv_kodiMasterForm', {
...
//					validateBlank: false,
	 ...
	 * @type
	 */
	Unilite.defineModel('s_pms402ukrv_kodiDetailModel', {
		fields: [
	...
	Unilite.defineModel('s_pms402ukrv_kodiDetailModel2', {
		fields: [
	...
	var detailStore = Unilite.createStore('s_pms402ukrv_kodiDetailStore', {
		model: 's_pms402ukrv_kodiDetailModel',
		autoLoad: false,
		uniOpt: {
	...
	var detailStore2 = Unilite.createStore('s_pms402ukrv_kodiDetailStore2', {
		model: 's_pms402ukrv_kodiDetailModel2',
		autoLoad: false,
		uniOpt: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: true,		// 수정 모드 사용
...
//			var param= param.data.items[0];
			...
			var searchParam = Ext.getCmp('s_pms402ukrv_kodiMasterForm').getValues();
...
//				}
	...
	var detailGrid = Unilite.createGrid('s_pms402ukrv_kodiGrid', {
		layout: 'fit',
		region:'center',
		uniOpt: {
		...
		listeners: {
			...
			cellclick: function() {
				selectedGrid = 's_pms402ukrv_kodiGrid';
				selectedMasterGrid = 's_pms402ukrv_kodiGrid';
...
//			grdRecord.set('INSPEC_PRSN'			, record['INSPEC_PRSN']);
	...
	var detailGrid2 = Unilite.createGrid('s_pms402ukrv_kodiGrid2', {
		layout: 'fit',
		region:'south',
		uniOpt: {
			expandLastColumn: false,
		...
		listeners: {
			...
			cellclick: function() {
				selectedGrid = 's_pms402ukrv_kodiGrid2';
				selectedMasterGrid = 's_pms402ukrv_kodiGrid2';
...
//			}
	...
	var buttonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			create	: 's_pms402ukrv_kodiService.testcodeCopy',
			syncAll	: 's_pms402ukrv_kodiService.copyAll'
		}
	});
...
//			if(inValidRecs.length == 0) {
				config = {
					params	: [paramMaster],
					success : function(batch, option) {
						Ext.getCmp('s_pms402ukrv_kodiApp').unmask();
...
//		trackResetOnLoad: true,
	...
	var checkNoMasterStore = Unilite.createStore('checkNoMasterStore', {
			...
			proxy: {
				type: 'direct',
				api: {
					read : 's_pms402ukrv_kodiService.selectOrderNumMasterList'
				}
			},
			loadStoreRecords : function()	{
	...
	var checkNoMasterGrid = Unilite.createGrid('s_pms402ukrv_kodicheckNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
...
//			{ dataIndex: 'PJT_CODE'			, width: 100 }
	...
	// 접수 참조 모델 정의
	Unilite.defineModel('s_pms402ukrv_kodiProductionModel', {
		fields: [
	...
	// 접수 참조 스토어 정의
	var productionStore = Unilite.createStore('s_pms402ukrv_kodiProductionStore', {
		model: 's_pms402ukrv_kodiProductionModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,	// 상위 버튼 연결
			editable: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_pms402ukrv_kodiService.selectEstiList'
			}
		},
		loadStoreRecords : function()	{
	...
	var productionGrid = Unilite.createGrid('s_pms402ukrv_kodiproductionGrid', {
...
//			Ext.getCmp('selectListBtn').setDisabled(true);
		...
		onNewDataButtonDown: function() {
			if(activeGridId == 's_pms402ukrv_kodiGrid' )	{
	...
	//					GOODBAD_TYPE: '01'
		...
		onDeleteDataButtonDown: function() {
			if(selectedMasterGrid == 's_pms402ukrv_kodiGrid')	{
		...
		fnGetTestlistNew:function(value) {
			...
			s_pms402ukrv_kodiService.selectTestListNew(param, function(provider, response) {
		...
		fnInspecQtyCheck: function(rtnRecord, divCode, inspecNum, inspecSeq)	{
			...
			s_pms402ukrv_kodiService.inspecQtyCheck(param, function(provider, response){
