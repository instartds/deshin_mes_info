...
var gsRecord;
var issuereqWindow; // 출고예정량 컬럼 더클클릭 시 열리는 팝업창
var issuereqRecord; // ITEM_CODE, ITEM_NAME, LOT_NO를 받아올 변수
...
function appMain() {
	...
	Unilite.defineModel('Biv300skrvModel3', {
		fields:  [
			...
			{name: 'EXPIRATION_DAY'     , text: '<t:message code="system.label.sales.availableperiod" default="유효기간"/>',            type: 'int'},
			{name: 'DIV_CODE'			, text: '<t:message code="system.label.inventory.division" default="사업장"/>',				type: 'string'}
		]
	});
...
//			{dataIndex: 'ITEM_LEVEL3'	, 		width: 100,align:'center', locked: true},
		...
		listeners:{
			...
			onGridDblClick: function(grid, record, cellIndex, colName) {
				...
				if(UniUtils.indexOf(colName, ['ISSUE_REQ_QTY'])) {
					issuereqRecord = record;
					openIssuereqWindow();
				}
			}
		}
...
//		width		: 800,
	...
	var detailDataGrid = Unilite.createGrid('detailDataGrid', {
		...
		columns: [
			...
			{dataIndex: 'EXPIRATION_DAY'    , minWidth: 100},
			{dataIndex: 'DIV_CODE'			, width: 66	, hidden: true}
		],
		listeners: {
		},
...
//			}
	...
	var issuereqSearch = Unilite.createSearchForm('issuereqForm', {
		layout	: {type : 'uniTable', columns : 4},
		items	: [{
				fieldLabel: '<t:message code="system.label.inventory.division" default="사업장"/>',
				name: 'DIV_CODE',
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
					onValueFieldChange: function( elm, newValue, oldValue ) {
						if(!Ext.isObject(oldValue)) {
							issuereqSearch.setValue('ITEM_NAME', '');
						}
					},
					onTextFieldChange: function( elm, newValue, oldValue ) {
						if(!Ext.isObject(oldValue)) {
							issuereqSearch.setValue('ITEM_CODE', '');
						}
					},
					applyextparam: function(popup){
						popup.setExtParam({'DIV_CODE': issuereqSearch.getValue('DIV_CODE')});
					}
				}
			}),{
				fieldLabel: 'LOT_NO',
                ...
                items: [{
                    ...
                    boxLabel: '미출고',
                    width: 60, name: 'ISSUE_YN',
                    inputValue: 'N',
                    checked: true
                },{
                    boxLabel: '<t:message code="system.label.inventory.issue" default="완료"/>',
                    width: 80, name: 'ISSUE_YN',
                    inputValue: 'Y'
                }]
            }
		]
	...
	Unilite.defineModel('biv301skrvIssuereqModel', {
		fields: [
			...
			{name: 'TRANS_RATE'				, text: '입수'																				, type: 'float' , decimalPrecision: 6 , format:'0,000.000000'},
			{name: 'STOCK_REQ_QTY'			, text: '출하지시량(재고단위)'																		, type: 'uniQty'},
			{name: 'ISSUE_QTY'				, text: '출고수량(판매단위)'																		, type: 'uniQty'},
			{name: 'REMAIN_ISSUE_QTY'		, text: '미출고(판매단위)'																			, type: 'uniQty'},
			{name: 'WH_NAME'				, text: '<t:message code="system.label.inventory.issuewarehouse" default="출고창고"/>'			, type: 'string'},
			{name: 'WH_CELL_NAME'			, text: '출고CELL'																			, type: 'string'},
			{name: 'INOUT_DATE'				, text: '<t:message code="system.label.inventory.issuedate" default="출고일"/>'				, type: 'uniDate'},
			{name: 'INOUT_TYPE_DETAIL_NAME'	, text: '출고유형'																				, type: 'string'},
			{name: 'REMARK'					, text: '<t:message code="system.label.inventory.remarks" default="비고"/>'					, type: 'string'},
			{name: 'REMARK_INTER'			, text: '내부비고'																				, type: 'string'}
		]
	});
	
	var issuereqStore = Unilite.createStore('biv301skrvIssuereqStore', {
		model	: 'biv301skrvIssuereqModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 'biv301skrvService.selectIssuereqList'
			}
		},
		loadStoreRecords : function() {
			var param = issuereqSearch.getValues();
			console.log(param);
			this.load({
				params : param
			});
	...
	var issuereqGrid = Unilite.createGrid('biv301skrvIssuereqGrid', {
		store	: issuereqStore,
		layout	: 'fit',
		columns	: [
			{ dataIndex: 'LOT_NO'					, width: 80},  // LOT번호
			{ dataIndex: 'CUSTOM_CODE'				, width: 100	, hidden: true}, // 거래처코드
			{ dataIndex: 'CUSTOM_NAME'				, width: 150}, // 거래처명
			{ dataIndex: 'ISSUE_REQ_DATE'			, width: 80},  // 출하지시일
			{ dataIndex: 'ISSUE_REQ_QTY'			, width: 130}, // 출하지시량
			{ dataIndex: 'ORDER_UNIT'				, width: 70		, align:'center'}, // 판매단위
			{ dataIndex: 'TRANS_RATE'				, width: 100}, // 입수
			{ dataIndex: 'STOCK_REQ_QTY'			, width: 140}, // 출하지시량(재고단위)
			{ dataIndex: 'ISSUE_QTY'				, width: 130}, // 출고수량(판매단위)
			{ dataIndex: 'REMAIN_ISSUE_QTY'			, width: 120}, // 미출고(판매단위)
			{ dataIndex: 'WH_NAME'					, width: 80		, align:'center'}, // 출고창고
			{ dataIndex: 'WH_CELL_NAME'				, width: 80}, // 출고창고 CELL
			{ dataIndex: 'INOUT_DATE'				, width: 80}, // 출고일
			{ dataIndex: 'INOUT_TYPE_DETAIL_NAME'	, width: 80		, align:'center'}, // 출고유형
			{ dataIndex: 'REMARK'					, width: 100}, // 비고
			{ dataIndex: 'REMARK_INTER'				, width: 100}  // 내부비고
		]
	});
	
	function openIssuereqWindow() {		
		if(!issuereqWindow) {
			issuereqWindow = Ext.create('widget.uniDetailWindow', {
				title	: '출하내역',
				width	: 1080,
				height	: 580,
				layout	: {type:'vbox', align:'stretch'},
				items	: [issuereqSearch, issuereqGrid],
				tbar		: ['->',{
					itemId	: 'saveBtn',
					text	: '<t:message code="system.label.sales.inquiry" default="조회"/>',
					handler	: function() {
						issuereqStore.loadStoreRecords();
					},
					disabled: false
				},{
					itemId	: 'confirmBtn',
				...
				listeners: {
					beforeshow: function( panel, eOpts ) {
						issuereqSearch.setValue('DIV_CODE'	, panelResult.getValue('DIV_CODE'));
						issuereqSearch.setValue('ITEM_CODE'	, issuereqRecord.get('ITEM_CODE'));
						issuereqSearch.setValue('ITEM_NAME'	, issuereqRecord.get('ITEM_NAME'));
						issuereqSearch.setValue('LOT_NO'	, issuereqRecord.get('LOT_NO'));
						issuereqStore.loadStoreRecords();
					}
				}
			})
		}
		issuereqWindow.center();
		issuereqWindow.show();
	}
