...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'btr171ukrvService.selectList',
			update	: 'btr171ukrvService.updateDetail',
			create	: 'btr171ukrvService.insertDetail',
			destroy	: 'btr171ukrvService.deleteDetail',
			syncAll	: 'btr171ukrvService.saveAll'
		}
	});
	...
	var otherOrderMasterSearch = Unilite.createSearchForm('otherorderMasterForm', {
		...
		Unilite.popup('DIV_PUMOK',{ // 20210811 수정: 품목 조회조건 표준화
			...
			listeners		: {
				onValueFieldChange: function( elm, newValue, oldValue ) {
					if(!Ext.isObject(oldValue)) {
						otherOrderMasterSearch.setValue('ITEM_NAME', '');
					}
				},
				onTextFieldChange: function( elm, newValue, oldValue ) {
					if(!Ext.isObject(oldValue)) {
						otherOrderMasterSearch.setValue('ITEM_CODE', '');
					}
				},
				applyextparam: function(popup){
					popup.setExtParam({'DIV_CODE': otherOrderSearch.getValue('DIV_CODE')});
	 ...
	 * @type
	 */
	Unilite.defineModel('btr171ukrvModel', {		// 대체 입/출고
		fields: [
	...
	Unilite.defineModel('btr171ukrvOTHERModel', {	// LOT NO별 참조
		fields: [
	...
	Unilite.defineModel('btr171ukrvOTHERMasterModel', {	//biv100t 참조
		fields: [
			{name: 'CHECK_FLAG'				, text: '<t:message code="system.label.inventory.selection" default="선택"/>'			, type: 'string'},
			{name: 'DIV_CODE'				, text: '<t:message code="system.label.inventory.division" default="사업장"/>'			, type: 'string', comboType: 'BOR120', defaultValue: UserInfo.divCode},
			{name: 'REF_CODE_DIV'			, text: 'REF_CODE_DIV'				, type: 'string'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('btr171ukrvMasterStore1',{	// 대체/입출고
		model: 'btr171ukrvModel',
		uniOpt: {
...
//						directMasterStore1.loadStoreRecords();
				...
				var grid = Ext.getCmp('Btr171ukrvGrid');
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {	// 품목대체번호검색
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'btr171ukrvService.selectDetailList'
			}
		},
		loadStoreRecords: function() {
	...
	var otherOrderStore = Unilite.createStore('btr171ukrvOtherOrderStore', {
		model	: 'btr171ukrvOTHERModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'btr171ukrvService.selectOrderNumMasterList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	...
	var otherOrderMasterStore = Unilite.createStore('btr171ukrvOtherOrderMasterStore', {
		model	: 'btr171ukrvOTHERMasterModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'btr171ukrvService.selectBiv100tMasterRefer'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts)	{
	 ...
	 * @type
	 */
	var masterForm = Unilite.createSearchPanel('btr171ukrvMasterForm',{
...
//			autoPopup: true,
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('btr171ukrvGrid', {		// 대체 입/출고 그리드
...
//				,OINOUT_I: record['OINOUT_P'] * (record['GOOD_STOCK_Q'] + record['BAD_STOCK_Q'])
	...
	var orderNoMasterGrid = Unilite.createGrid('btr171ukrvOrderNoMasterGrid', {
		layout	: 'fit',
	...
	var otherOrderGrid = Unilite.createGrid('btr171ukrvOtherOrderGrid', {
		layout	: 'fit',
...
//				masterGrid.setEstiData2(record.data);
	...
	var otherOrderMasterGrid = Unilite.createGrid('btr171ukrvOtherOrderMasterGrid', {
		layout	: 'fit',
...
//				masterGrid.setEstiData2(record.data);
	...
	//엑셀참조 모델
	Unilite.Excel.defineModel('excel.btr171.sheet01', {
		fields: [
		...
		if(!excelWindow) {
			...
			excelWindow = Ext.create( appName, {
				...
				grids: [{
					...
					useCheckbox	: true,
					model		: 'excel.btr171.sheet01',
					readApi		: 'btr171ukrvService.selectExcelist',
					columns		: [
