...
<t:appConfig pgmId="btr111ukrv"  >
	<t:ExtComboStore comboType="BOR120" pgmId="btr111ukrv"/>				<!-- 사업장 -->
...
var alertWindow;			//20210201 추가: alertWindow : 경고창
var gsText		= ''		//20210201 추가: 바코드 알람 팝업 메세지
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'btr111ukrvService.selectMaster',
			update	: 'btr111ukrvService.updateDetail',
			create	: 'btr111ukrvService.insertDetail',
			destroy	: 'btr111ukrvService.deleteDetail',
			syncAll	: 'btr111ukrvService.saveAll'
		}
	});
		...
		},
		{
			...
			listeners	: {
				change: function(combo, newValue, oldValue, eOpts) {
					...
					//20210128 추가: 창고 선택 시, 창고cell 자동선택(bsa225t.default_yn='y'인 데이터)
					var param = {
					...
					btr111ukrvService.getWhCellCode(param, function(provider, response) {
						if(!Ext.isEmpty(provider) && !gsQueryFlag) {
							var whCellStore1 = panelResult.getField('WH_CELL_CODE').getStore();
			...
			listeners	: {
				change: function(combo, newValue, oldValue, eOpts) {
					...
					var param = {
						DIV_CODE: panelResult.getValue('DIV_CODE'),
						WH_CODE	: panelResult.getValue('TO_WH_CODE')
					}
					btr111ukrvService.getWhCellCode(param, function(provider, response) {
						if(!Ext.isEmpty(provider) && !gsQueryFlag) {
							var whCellStore1 = panelResult.getField('TO_WH_CELL_CODE').getStore();
							whCellStore1.clearFilter(true);
							whCellStore1.filter([{
								property: 'option',
								value	: newValue
							}]);
							panelResult.getField('TO_WH_CELL_CODE').setValue(provider);
							var whCellStore2 = panelResult.getField('TO_WH_CELL_CODE').getStore();
							whCellStore2.clearFilter(true);
							whCellStore2.filter([{
								property: 'option',
								value	: newValue
...
//				colspan	 : 3,
			...
			name: 'ITEM_CODES',
			hidden: true
		},{	//20210201 추가: 이동요청번호 필드 추가 -> 바코드 스캔 시, 출하지시참조/적용 로직 타도록 로직 추가
			fieldLabel	: '이동요청번호',
			name		: 'REQSTOCK_NUM',
			xtype		: 'uniTextfield',
			readOnly	: false,
			listeners	: {
				...
				specialkey:function(field, event)	{
					if(event.getKey() == event.ENTER) {
						...
						var newValue = panelResult.getValue('REQSTOCK_NUM');
						if(!Ext.isEmpty(newValue)) {
...
//		,tdAttrs: {style: 'border : 1px solid #ced9e7;'/*, align : 'center'*/}
	 ...
	 * @type
	 */
	Unilite.defineModel('btr111ukrvModel', {	// 메인
		fields: [
			...
			{name: 'IN_Q'					,text: '<t:message code="system.label.purchase.receiptqty" default="입고량"/>'						,type: 'uniQty', editable: false},		//20210205 추가
	...
	}); //End of Unilite.defineModel('btr111ukrvModel', {
	...
	Unilite.defineModel('btr111ukrvOTHERModel', {	//이동요청 참조
		fields: [
	...
	var directMasterStore1 = Unilite.createStore('Btr111ukrvMasterStore1',{		// 메인
		model: 'btr111ukrvModel',
		uniOpt : {
				...
				config = {
					...
					success: function(batch, option) {
						...
						if(directMasterStore1.getCount() == 0){
							...
							if(gsSetField =='WM') {
								UniAppManager.setToolbarButtons(['print'], true);		//20201207 추가: 저장 후, 출력버튼 활성화
							}
		...
		listeners: {
			load: function(store, records, successful, eOpts) {
				//20201207 추가: 출력버튼 활성화(월드와이드 메모리)
				if(records.length > 0) {
					if(gsSetField =='WM') {
	...
	});		// End of var directMasterStore1 = Unilite.createStore('Btr111ukrvMasterStore1',{
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {	// 검색 팝업창
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'btr111ukrvService.selectDetail'
			}
		},
		loadStoreRecords: function() {
	...
	var otherOrderStore = Unilite.createStore('btr111ukrvOtherOrderStore', {//이동요청 참조
		model: 'btr111ukrvOTHERModel',
		autoLoad: false,
		uniOpt : {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,		// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'btr111ukrvService.selectDetail2'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
...
//							record1.set('SALE_P'		, '');
			...
			{dataIndex: 'IN_Q'					, width: 80},							//20210205 추가
			...
			beforeedit  : function( editor, e, eOpts ) {
				if (UniUtils.indexOf(e.field, ['LOT_NO'])){
					if(Ext.isEmpty(e.record.data.WH_CODE)){
						Unilite.messageBox('<t:message code="system.message.inventory.message001" default="출고창고를 입력하십시오."/>');
						return false;
					}
					if(BsaCodeInfo.gsSumTypeCell == 'Y' && Ext.isEmpty(e.record.data.WH_CELL_CODE)){
						Unilite.messageBox('<t:message code="system.message.inventory.message002" default="출고창고 Cell코드를 입력하십시오."/>');
						return false;
					}
...
//					cbStore2.loadStoreRecords(record.get('INOUT_CODE'));
				...
				if(!e.record.phantom) {
 					if(UniUtils.indexOf(e.field, ['TO_DIV_CODE', 'WH_CODE', 'WH_CELL_CODE','INOUT_CODE', 'INOUT_CODE_DETAIL', 'ITEM_CODE', 'ITEM_NAME', 'ITEM_STATUS',
												  'REQSTOCK_Q'/*, 'INOUT_Q'*/, 'INOUT_PRSN', 'LOT_NO', 'REMARK', 'PROJECT_NO']))		//20210205 주석: 출고량 수정가능하도록 변경
					{
						return false;
					} else {
						return true;
...
//				grdRecord.set('PROJECT_NO'			, record['PROJECT_NO']);
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('btr111ukrvGrid', masterGridConfig);

/*	var masterGrid = Unilite.createGrid('btr111ukrvGrid', {		// 메인
		// for tab
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
...
//				grdRecord.set('PROJECT_NO'			, record['PROJECT_NO']);
	...
	});	//End of   var masterGrid1 = Unilite.createGrid('btr111ukrvGrid1', {
*/

	var orderNoMasterGrid = Unilite.createGrid('btr111ukrvOrderNoMasterGrid', {	// 검색팝업창
		// title: '기본',
		layout : 'fit',
...
//					 { dataIndex: 'DEPT_NAME'			,  width: 133},
	...
	var otherOrderGrid = Unilite.createGrid('btr111ukrvOtherOrderGrid', {	//이동요청 참조
		// title: '기본',
		layout : 'fit',
...
//								otherOrderSearch.setValue('INOUT_PRSN'		, panelResult.getValue('INOUT_PRSN'));
								//20210415 추가: panelResult에 있는 창고 정보 SET
								otherOrderSearch.setValue('WH_CODE'			, panelResult.getValue('TO_WH_CODE'));
								otherOrderSearch.setValue('OUT_WH_CODE'		, panelResult.getValue('WH_CODE'));
								otherOrderStore.loadStoreRecords();
							 }
				}
			})
...
//			suffixTpl:'&nbsp;M',
	...
	var labelPartitonPrintStore = Unilite.createStore('labelPartitonPrintStore', {	//라벨분할출력스토어
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'btr111ukrvService.selectLabelPrintList'
			}
		},
		loadStoreRecords: function() {
	...
	var labelPartitionPrintGrid = Unilite.createGrid('btr111ukrvPartitionPrintGrid', {		//라벨분할그리드
		layout :'fit',
	...
	Unilite.Main({
		...
		fnInitBinding: function(params) {
			...
			//20210201 추가: 이동요청번호 바코드 필드 show/hidden
			if(gsSetField == 'WM') {
				panelResult.getField('REQSTOCK_NUM').setHidden(false);
			} else {
				panelResult.getField('REQSTOCK_NUM').setHidden(true);
			}
		},
		processParams: function(params) {
			if(params.PGM_ID == 'biv450ukrv') { //폐기대상창고이관조정에서 링크넘어올시
				var formPram = params.formPram;
				panelResult.setValue('DIV_CODE'	    , formPram.DIV_CODE);
				gsQueryFlag = true;		//20210128 추가
				panelResult.setValue('WH_CODE'	    , formPram.WH_CODE);
				panelResult.setValue('WH_CELL_CODE'	, formPram.WH_CELL_CODE);
				
				panelResult.setValue('TO_DIV_CODE'	, formPram.TO_DIV_CODE);
				gsQueryFlag = true;		//20210128 추가
				panelResult.setValue('TO_WH_CODE'	, formPram.TO_WH_CODE);
				panelResult.setValue('TO_WH_CELL_CODE'	, formPram.TO_WH_CELL_CODE);
				
				panelResult.setValue('INOUT_PRSN'	, formPram.INOUT_PRSN);
				panelResult.setValue('INOUT_DATE'	, formPram.BASE_DATE);
				...
				Ext.each(params.record, function(rec,i){
						...
						var itemStatus   = rec.get('GUBUN_CODE');
						var inItemStatus = rec.get('GUBUN_CODE');
						...
						var exchgRateO   = '1.00';
						var basisSeq	 = '0';
						var inoutQ		 = rec.get('STOCK_MOVE_Q')
						var goodStockQ	 = rec.get('GOOD_STOCK')
						var badStockQ	 = rec.get('BAD_STOCK')
						var orderSeq	 = '0';
						
						var itemCode	= rec.get('ITEM_CODE')
						var itemName	= rec.get('ITEM_NAME')
						var lotNo		= rec.get('LOT_NO')
						var spec		= rec.get('SPEC')
						var unit		= rec.get('STOCK_UNIT')
		...
		setDefault: function(params) {		// 기본값
			...
			if(Ext.isEmpty(params && params.PGM_ID)){
				...
				btr111ukrvService.userWhcode({}, function(provider, response) {
				...
				btr111ukrvService.deptWhcode(param, function(provider, response){
					if(!Ext.isEmpty(provider)){
						 panelResult.setValue('WH_CODE', provider['WH_CODE']);
					}
				});
...
//			var inoutDate = UniDate.get('today');
			...
			btr111ukrvService.getItemInfo(param, function(provider, response){
				if(!Ext.isEmpty(provider)) {
		...
		confirmSaveData: function(config) {	// 저장하기전 원복 시키는 작업
			var fp = Ext.getCmp('btr111ukrvFileUploadPanel');
		...
		onPrintButtonDown: function(){
			//20201207 추가: WM 출력 로직(정규로 사용 ) 추가로 분개로직 추가 
			if(gsSetField == 'KODI') {
				var param = panelResult.getValues();
				var selectedDetails = masterGrid.getSelectedRecords();
				if(Ext.isEmpty(selectedDetails)){
				...
				var param = panelResult.getValues();
				if(Ext.isEmpty(param.INOUT_NUM)) {
					Unilite.messageBox('출력할 데이터가 없습니다.');
					return false;
				}
				param.PGM_ID	= 'btr130rkrv';		//재고이동명세서 출력과 동일하게 사용
				param.MAIN_CODE	= 'I015';

				var win = Ext.create('widget.ClipReport', {
					url			: CPATH+'/btr/btr130clrkrv.do',
					prgID		: 'btr130rkrv',		//재고이동명세서 출력과 동일하게 사용
					extParam	: param,
					submitType	: 'POST'
				});
				win.center();
...
//				출력한 데이터 출력여부 UPDATE 로직 추가
		...
		fnQtySet : function(record) {
			...
			btr111ukrvService.QtySet(param, function(provider, response) {
				if(!Ext.isEmpty(provider)){
...
//					record.set('INOUT_FOR_O', provider['AVERAGE_P'] * record.get('INOUT_Q'));
		...
		fnMakeMoveRequestDataRef2: function(records) {							//20210201 추가: 이동요청바코드 로직
			if(!panelResult.getInvalidMessage()) return;						//필수체크
			var newDetailRecords	= new Array();
			var compCode			= UserInfo.compCode;
			var divCode				= panelResult.getValue('DIV_CODE');			//출고사업장
			...
			Ext.each(records, function(record,i){
				...
				panelResult.setAllFieldsReadOnly(true);
				newDetailRecords[i] = directMasterStore1.model.create( r );
				newDetailRecords[i].set('ORDER_NUM'			, record.REQSTOCK_NUM);
				newDetailRecords[i].set('ORDER_SEQ'			, record.REQSTOCK_SEQ);
				newDetailRecords[i].set('TO_DIV_CODE'		, record.DIV_CODE);
				newDetailRecords[i].set('WH_CODE'			, panelResult.getValue('WH_CODE'));
				newDetailRecords[i].set('WH_CELL_CODE'		, panelResult.getValue('WH_CELL_CODE'));
				newDetailRecords[i].set('INOUT_CODE'		, record.WH_CODE);
				newDetailRecords[i].set('INOUT_CODE_DETAIL'	, record.WH_CELL_CODE);
				newDetailRecords[i].set('DEPT_CODE'			, panelResult.getValue('DEPT_CODE'));
				newDetailRecords[i].set('DEPT_NAME'			, panelResult.getValue('DEPT_NAME'));
				...
				newDetailRecords[i].set('ITEM_STATUS'		, '1');
				newDetailRecords[i].set('INOUT_METH'		, '3');
				newDetailRecords[i].set('REQSTOCK_Q'		, record.REQSTOCK_Q);
				newDetailRecords[i].set('INOUT_Q'			, record.NOTOUTSTOCK_Q);
				newDetailRecords[i].set('GOOD_STOCK_Q'		, record.GOOD_STOCK_Q);
				newDetailRecords[i].set('BAD_STOCK_Q'		, record.BAD_STOCK_Q);
...
//							record.set('INOUT_FOR_O', provider['AVERAGE_P'] * record.get('INOUT_Q'));
				...
				case "INOUT_Q" :
					if (newValue != '' && newValue != null){
						...
						//20210205 추가: 체크로직 추가
						if(!Ext.isEmpty(record.get('IN_Q')) && record.get('IN_Q') > 0 && newValue > record.get('IN_Q')) {
							rv = '출고량은 입고량보다 클 수 없습니다.';
							break;
						}
...
if(gsSetField !='WM') {
	...
	//20210201 추가: 바코드 입력 로직 (이동요청번호)
	function fnEnterReqStockNumBarcode(newValue) {
		var detailRecords	= directMasterStore1.data.items;
		var reqStockNum		= newValue
		var flag			= true;

		//동일한 이동요청번호 입력되었을 경우 처리
		Ext.each(detailRecords, function(detailRecord,i) {
			if(detailRecord.get('ORDER_NUM').toUpperCase() == reqStockNum.toUpperCase()) {
				beep();
				gsText	= '<t:message code="system.label.sales.message001" default="동일한  이동요청번호가 이미 등록 되었습니다."/>';
				flag	= false;
				openAlertWindow(gsText);
				Ext.getBody().unmask();
				panelResult.getField('REQSTOCK_NUM').focus();
				return false;
			}
		});
		...
		if(flag) {
			...
			//검사결과참조 쿼리 호출
			btr111ukrvService.selectDetail2(param, function(provider, response){
				if(!Ext.isEmpty(provider)){
					UniAppManager.app.fnMakeMoveRequestDataRef2(provider);
					setTimeout(function() { panelResult.getField('REQSTOCK_NUM').focus();}, 1000);
				} else {
					Unilite.messageBox('<t:message code="system.label.sales.message002" default="입력하신 이동요청번호의 데이터가 존재하지 않습니다."/>');
					Ext.getBody().unmask();
					panelResult.setValue('REQSTOCK_NUM', '');
					panelResult.getField('REQSTOCK_NUM').focus();
					return false;
