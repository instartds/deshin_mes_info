...
<t:appConfig pgmId="s_biv300skrv_mit"  >

	<t:ExtComboStore comboType="BOR120"  pgmId="s_biv300skrv_mit"/> 				<!-- 사업장 -->
...
function appMain() {
	 ...
	 * @type
	 */
	Unilite.defineModel('S_Biv300skrv_mitModel', {
		fields: [
			...
			{name: 'ALLOCK_Q',		    text: '당일 투입예정수량 ',				type: 'uniQty'},
			{name: 'SAFE_STOCK_Q',		text: '안전재고',																				type: 'uniQty'},
			{name: 'AVG_3Q',			text: '평균출고량(3M)',																		type: 'uniQty'},
			{name: 'TURNOVER_DAY',		text: '회전일수',																				type: 'uniQty'},
			{name: 'SAFE_Q',			text: '안전재고량',																		type: 'uniQty'},
			
			
		]
	});

	Unilite.defineModel('S_Biv300skrv_mitModel2', {
		fields: [
	...
	Unilite.defineModel('S_Biv300skrv_mitModel3', {
		fields:  [
	...
	Unilite.defineModel('S_Biv300skrv_mitModel4', {
		fields:  [
			{name: 'DIV_CODE'		, text: '<t:message code="system.label.inventory.division" default="사업장"/>',					type: 'string'},
	...
	Unilite.defineModel('S_Biv300skrv_mitModel5', {
		fields:  [
			{name: 'DIV_CODE'		,text: '<t:message code="system.label.inventory.division" default="사업장"/>',				type: 'string'},
			{name: 'WH_CODE'		,text: '<t:message code="system.label.inventory.warehouse" default="창고"/>',				type: 'string'},
			{name: 'WH_NAME'		,text: '<t:message code="system.label.inventory.warehousename" default="창고명"/>',		type: 'string'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('s_biv300skrv_mitMasterStore1',{
		model: 'S_Biv300skrv_mitModel',
		uniOpt : {
		...
		proxy: {
			type: 'direct',
			api: {read: 's_biv300skrv_mitService.selectMaster'}
		},
	...
	var directMasterStore2 = Unilite.createStore('s_biv300skrv_mitMasterStore2',{
		model: 'S_Biv300skrv_mitModel2',
		uniOpt : {
		...
		proxy: {
			type: 'direct',
			api: {read: 's_biv300skrv_mitService.selectMaster2'}
		},
		loadStoreRecords : function() {
			var param= Ext.getCmp('searchForm').getValues();
			param.QUERY_TYPE = '2'
	...
	var directMasterStore3 = Unilite.createStore('s_biv300skrv_mitMasterStore3',{
		model: 'S_Biv300skrv_mitModel3',
		uniOpt : {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
			deletable	: false,	// 삭제 가능 여부
		...
		proxy: {
			type: 'direct',
			api: {read: 's_biv300skrv_mitService.selectMaster3'}
		},
		loadStoreRecords : function() {
			var param= Ext.getCmp('searchForm').getValues();
	...
	var directMasterStore4 = Unilite.createStore('s_biv300skrv_mitMasterStore4',{
		model: 'S_Biv300skrv_mitModel4',
		uniOpt : {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
			deletable	: false,	// 삭제 가능 여부
		...
		proxy: {
			type: 'direct',
			api: {read: 's_biv300skrv_mitService.selectMaster4'}
		},
		loadStoreRecords : function() {
			var param= Ext.getCmp('searchForm').getValues();
	...
	var directMasterStore5 = Unilite.createStore('s_biv300skrv_mitMasterStore5',{
		model: 'S_Biv300skrv_mitModel5',
		uniOpt : {
			isMaster	: false,		// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
			deletable	: false,	// 삭제 가능 여부
		...
		proxy: {
			type: 'direct',
			api: {read: 's_biv300skrv_mitService.selectMaster5'}
		},
		loadStoreRecords : function() {
			var param= Ext.getCmp('searchForm').getValues();
			...
			items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						cellSubForm.setValue('DIV_CODE', newValue);
					}
				}
			},{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						cellSubForm.setValue('WH_CODE', newValue);
					}
				}
			},{
            ...
            listeners: {
            	change : function(field, newValue, oldValue){
            		if(oldValue != newValue)	{
            			cellSubForm.setValue('WH_CELL_CODE' , '');
            		}
            	},
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('s_biv300skrv_mitGrid_1', {
//		title: '품목별',
...
//			{dataIndex: 'ITEM_LEVEL3'	, width: 100,align:'center', locked: true},
			...
			{dataIndex: 'ALLOCK_Q'      , width: 130, summaryType: 'sum'},
			...
			{dataIndex: 'AVG_3Q'		, width: 100},
			{dataIndex: 'TURNOVER_DAY'	, width: 100},
			{dataIndex: 'SAFE_Q'	, width: 100},
	...
	var masterGrid2 = Unilite.createGrid('s_biv300skrv_mitGrid_2', {
//		title: '창고별',
...
//			{dataIndex: 'ITEM_LEVEL3'	, width: 100,align:'center', locked: true},
	...
	var masterGrid3 = Unilite.createGrid('s_biv300skrv_mitGrid_3', {
//		title: 'LOT별',
	...
	var masterGrid4 = Unilite.createGrid('s_biv300skrv_mitGrid_4', {
//		title: '외주처별',
	...
	var masterGrid5 = Unilite.createGrid('s_biv300skrv_mitGrid_5', {
//		title: 'CELL별',
		region: 'center' ,
		layout : 'fit',
	...
	Unilite.Main({
		...
		fnInitBinding : function() {
			...
			s_biv300skrv_mitService.userWhcode({}, function(provider, response) {
		...
		onQueryButtonDown : function() {
			...
			if(activeTabId == 's_biv300skrv_mitGrid1'){
				directMasterStore1.loadStoreRecords();
			}
			else if(activeTabId == 's_biv300skrv_mitGrid2'){
				directMasterStore2.loadStoreRecords();
			}
			else if(activeTabId == 's_biv300skrv_mitGrid3'){
				directMasterStore3.loadStoreRecords();
			}
			else if(activeTabId == 's_biv300skrv_mitGrid4'){
				directMasterStore4.loadStoreRecords();
			}
			else if(activeTabId == 's_biv300skrv_mitGrid5'){
