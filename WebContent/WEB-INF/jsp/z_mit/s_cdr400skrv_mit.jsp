...
<t:appConfig pgmId="s_cdr400skrv_mit"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_cdr400skrv_mit" /> 				<!-- 사업장 -->
...
function appMain() {
   var s_cdr400skrv_mitModel=Unilite.defineModel('s_cdr400skrv_mitModel', {
   	fields: [
   ...
   var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 's_cdr400skrv_mitService.selectList'
        }
    });
   var s_cdr400skrv_mitStore=Unilite.createStore('Store', {	
   		model: 's_cdr400skrv_mitModel',
		autoLoad: false,
		uniOpt: {
   ...
   /**
	...
	var panelResult = Unilite.createSearchForm('resultForm',{
		...
		layout: {type: 'uniTable', columns: 4, tableAttrs:{cellpadding:1,width:'100%'}, tdAttrs: {valign:'top'}},
        ...
        viewConfig:{
                ...
                getRowClass : function(record,rowIndex,rowParams,store){
                    ...
                    if(record.get('PROD_ITEM_CODE')=="총계"){
                    	...
                    	if(Math.abs(Number(record.get('OutProdt_q')) + Number(record.get('OS_OutOutOrder_q')) + Number(record.get('OutAdjust_q')) + Number(record.get('OS_OutEtc_q')) - Number(record.get('OutFinance_q'))) > 0.000001) {
                    		//cls = 'x-grid-cell-text-highlight';
                    		cls = 'x-grid-cell-background-highlight';
                    	}
                    	else if(Math.round(Number(record.get('OutProdt_i')) + Number(record.get('OS_OutOutOrder_i')) + Number(record.get('OutAdjust_i')) + Number(record.get('OS_OutEtc_i'))) != Math.round(Number(record.get('OutFinance_i')))) {
                    		cls = 'x-grid-cell-background-highlight';
                    	}
                    }
                    return cls;
