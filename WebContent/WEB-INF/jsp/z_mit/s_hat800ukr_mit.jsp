...
<t:appConfig pgmId="s_hat800ukr_mit"  >
...
function appMain() {
	...
	var excelWindow1;
        ...
        api: {
            read    : 's_hat800ukr_mitService.selectList',
            update  : 's_hat800ukr_mitService.updateList',
            create  : 's_hat800ukr_mitService.insertList',
            destroy : 's_hat800ukr_mitService.deleteList',
            syncAll : 's_hat800ukr_mitService.saveAll'
        }
    });
	 ...
	 * @type
	 */
	Unilite.defineModel('s_hat800ukr_mitModel', {
		fields: [
			{name : 'DUTY_YYYY'			, text : '연차년도'				, type : 'string'   	},
			...
			{name : 'LONG_YEAR'         , text : '근속년수'            	, type : 'int'      , editable : true	, format:'0,000'},
			...
			{name : 'AMOUNT_I'          , text : '통상임금(일급)'       	, type : 'int'      , editable : true	, format:'0,000'},
			{name : 'AMTTOPAY'          , text : '지급금액'             	, type : 'int'      , editable : false	, format:'0,000'},
	...
	});		//End of Unilite.defineModel('s_hat800ukr_mitModel', {
	 ...
	 * @type
	 */
	var directMasterStore = Unilite.createStore('s_hat800ukr_mitMasterStore1', {
		model: 's_hat800ukr_mitModel',
		uniOpt: {
	...
	});//End of var directMasterStore1 = Unilite.createStore('s_hat800ukr_mitMasterStore1', {
...
//			xtype: 'uniTextfield',
	...
	var masterGrid = Unilite.createGrid('s_hat800ukr_mitGrid1', {
    	// for tab
    	region: 'center',
    	flex:1,
		...
		columns: [
			  {dataIndex: 'DUTY_YYYY'				, width: 70      , align : 'center', hidden :true}
	...
	});//End of var masterGrid = Unilite.createGr100id('s_hat800ukr_mitGrid1', {
	...
	Unilite.Main( {
		...
		fnInitBinding: function() {
			...
			panelResult.onLoadSelectText('DUTY_YYYY');
		},
		...
		onDeleteAllButtonDown: function() {
			if(panelResult.getInvalidMessage())	{
				if(confirm("저장된 데이터가 삭제됩니다. 그래도 하시겠습니까?’"))	{
					var param = panelResult.getValues();
					s_hat800ukr_mitService.deleteAll(param, function(responseText, response){
						if(responseText)	{
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName)	{
				case 'JOIN_DATE':
						...
						Ext.getBody().mask();
						s_hat800ukr_mitService.getLongYearSave_mit({'DUTY_DATE' : record.get("DUTY_YYYY").toString()+'01', 'JOIN_DATE': UniDate.getDbDateStr(newValue)}, function(responseText){
							Ext.getBody().unmask();
							if(responseText)	{
