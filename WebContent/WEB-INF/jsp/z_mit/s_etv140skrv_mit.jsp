<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<t:appConfig pgmId="s_etv140skrv_mit"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="s_etv140skrv_mit"  />			 <!-- 사업장 -->  
...
function appMain() {
	...
	Unilite.defineModel('s_etv140skrv_mitModel', {
		fields: [
			{name: 'ITEM_CODE'		, text:'품목코드'		, type: 'string'},
			{name: 'SPEC'			, text:'규격'			, type: 'string'},
			{name: 'PLAN_Q'			, text:'계획량'		, type: 'uniQty'},
			{name: 'PRODT_Q'		, text:'열처리 수량'		, type: 'uniQty'},
			{name: 'HIT_TEMPER'		, text:'열처리 온도'		, type: 'uniQty'},
			{name: 'HIT_TIME'		, text:'열처리 시간'		, type: 'uniQty'}
		]
	});

	Unilite.defineModel('s_etv140skrv_mitModelContents', {
		fields: [
	...
	var directMasterStore = Unilite.createStore('s_etv140skrv_mitMasterStore',{
		model: 's_etv140skrv_mitModel',
		uniOpt : {
		...
		proxy: Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
			...
			api: {			
			   read: 's_etv140skrv_mitService.selectList'
			},
			reader: {
...
//				 params :param 
	...
	var directContentsStore = Unilite.createStore('s_etv140skrv_mitContentsStore', {
		model: 's_etv140skrv_mitModelContents',
		uniOpt: {
			isMaster:  false,	// 상위 버튼 연결 
			editable:  false,	// 수정 모드 사용 
			deletable: false,	// 삭제 가능 여부 
		...
		proxy: Ext.create('Unilite.com.data.proxy.UniDirectProxy', {
			api: {
				read: 's_etv140skrv_mitService.selectContents'
			}
		}),
		loadStoreRecords : function() {
			this.load(
	...
	var masterGrid = Unilite.createGrid('s_etv140skrv_mitMasterGrid', {
		layout : 'fit',
		region: 'center',
		//width:'100%',
		...
		columns:  [ 
			...
			{ dataIndex: 'HIT_TEMPER'			, flex: 1	, minWidth: 120},
			{ dataIndex: 'HIT_TIME'				, flex: 1	, minWidth: 120}
		] 
	});
