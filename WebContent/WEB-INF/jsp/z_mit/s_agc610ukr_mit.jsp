...
<t:appConfig pgmId="s_agc610ukr_mit"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_agc610ukr_mit"/>
	<t:ExtComboStore comboType="AU" comboCode="B020"  opts="00"/>								<!-- 품목계정 -->
...
function appMain() {  
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_agc610ukr_mitService.selectList',
			update: 's_agc610ukr_mitService.updateList',
			create: 's_agc610ukr_mitService.insertList',
			destroy: 's_agc610ukr_mitService.deleteList',
			syncAll: 's_agc610ukr_mitService.saveAll'
		}
	});	
	
	Unilite.defineModel('s_agc610ukr_mitModel', {
	    fields: [  	    
	    	  ...
	    	  , {name : 'WH_CODE'                     , text : '창고'                           	, type : 'string'            , comboType:'OU'				, editable : false, child :'WH_CELL_CODE' }
	    	  , {name : 'WH_CELL_CODE'                , text : '장소'                       	    , type : 'string'            , store : Ext.data.StoreManager.lookup('whCellList') , editable : false}
	    	  ...
	    	  , {name : 'EXPIRATION_DAY'              , text : '기본유효기간'                     	, type : 'uniPrice'                                        	, editable : false}
	    	  , {name : 'INOUT_DATE'                  , text : '입고일자'                         	, type : 'uniDate'                                         	, editable : false }
	    	  , {name : 'STOCK_Q'                     , text : '재고수량'                         	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'STOCK_P'                     , text : '단가'                           	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'STOCK_I'                     , text : '재고금액'                         	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'EXPIRATION_DATE'             , text : '유효기간'                  		, type : 'uniDate'                                         	, editable : false }
	    	  , {name : 'ALLOWA_NOSELLABLE'           , text : '충당금 판매불가'                    	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'ALLOWA_WITHIN3M'             , text : '유효기간3개월이내'                  	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'ALLOWA_WITHIN6M'             , text : '유효기간6개월이내'                  	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'ALLOWA_OVER6M'               , text : '유효기간6개월이상'                  	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'ALLOWA_OVERAYR'              , text : '유효기간1년이상'                   	, type : 'uniPrice'                                       	, editable : false }
	    	  , {name : 'FLAG'                     	  , text : 'FLAG'                   		, type : 'string'                							, editable : false }
	    	  , {name : 'REMARK'                      , text : '비고'                   		    , type : 'string'                							, editable : false }
	    ]
	});
	
	var directMasterStore = Unilite.createStore('s_agc610ukr_mitMasterStore',{
		model: 's_agc610ukr_mitModel',
		uniOpt: {
			...
			load:function(store, records){
				...
				/* if(!records || (records && records.length == 0))	{
					setTimeout(function() {
						...
						s_agc610ukr_mitService.selectNewList(param,  function(responseText, response) {
    ...
    var masterGrid = Unilite.createGrid('s_agc610ukr_mitGrid', {
        ...
        columns:  [     
        	...
        	, {dataIndex : 'ALLOWA_NOSELLABLE'                , width : 130 , summaryType : 'sum'}
        	, {dataIndex : 'ALLOWA_WITHIN3M'                  , width : 130 , summaryType : 'sum'}
        	, {dataIndex : 'ALLOWA_WITHIN6M'                  , width : 130 , summaryType : 'sum'}
        	, {dataIndex : 'ALLOWA_OVER6M'                    , width : 130 , summaryType : 'sum'}
        	, {dataIndex : 'ALLOWA_OVERAYR'                   , width : 130 , summaryType : 'sum'}
		...
		onDeleteAllButtonDown: function() {
			if(panelResult.getInvalidMessage())	{
				if(confirm("저장된 데이터가 삭제됩니다. 그래도 하시겠습니까?’"))	{
					...
					s_agc610ukr_mitService.deleteAll(param, function(responseText, response){
						if(responseText)	{
