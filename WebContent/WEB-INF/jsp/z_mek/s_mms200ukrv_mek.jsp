...
<t:appConfig pgmId="s_mms200ukrv_mek"  >
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_mms200ukrv_mekService.selectList1',
			update  : 's_mms200ukrv_mekService.updateDetail',
			create  : 's_mms200ukrv_mekService.insertDetail',
			destroy : 's_mms200ukrv_mekService.deleteDetail',
			syncAll : 's_mms200ukrv_mekService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_mms200ukrv_mekService.selectList2',
			update  : 's_mms200ukrv_mekService.updateBad',
			create  : 's_mms200ukrv_mekService.insertBad',
			destroy : 's_mms200ukrv_mekService.deleteBad',
			syncAll : 's_mms200ukrv_mekService.saveAllBad'
		}
	});
	 ...
	 * master grid1 Model
	 */
	Unilite.defineModel('s_mms200ukrv_mekModel1', {
		fields: [
	 ...
	 * detail grid2 Model
	 */
	Unilite.defineModel('s_mms200ukrv_mekModel2', {
		fields: [
			{name: 'DIV_CODE'			, text: '<t:message code="system.label.purchase.division" default="사업장"/>'				, type: 'string'},
			{name: 'INSPEC_NUM'			, text: '<t:message code="system.label.purchase.inspecno" default="검사번호"/>'				, type: 'string'},
			{name: 'INSPEC_SEQ'			, text: '<t:message code="system.label.purchase.seq" default="순번"/>'					, type: 'int'},
	 ...
	 * 접수참조 grid model
	 */
	Unilite.defineModel('s_mms200ukrv_mekRECEIPTModel', {	//접수참조
		fields: [
			{name: 'CUSTOM_CODE'		, text: '<t:message code="system.label.purchase.customcode" default="거래처코드"/>'			, type: 'string'},
			{name: 'CUSTOM_NAME'		, text: '<t:message code="system.label.purchase.customname" default="거래처명"/>'			, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
	 ...
	 * master grid1 store
	 */
	var directMasterStore1 = Unilite.createStore('s_mms200ukrv_mekMasterStore1', {
		model : 's_mms200ukrv_mekModel1',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				Ext.getCmp('s_mms200ukrv_mekGrid1').uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		listeners: {
	 ...
	 * detail grid2 store
	 */
	var directMasterStore2 = Unilite.createStore('s_mms200ukrv_mekMasterStore2', {
		model: 's_mms200ukrv_mekModel2',
		uniOpt: {
	...
	var receiptNoMasterStore = Unilite.createStore('receiptNoMasterStore', {	//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_mms200ukrv_mekService.selectreceiptNumMasterList'
			}
		},
		loadStoreRecords : function()	{
	 ...
	 * 접수참조 store
	 */
	var receiptStore = Unilite.createStore('s_mms200ukrv_mekReceiptStore', {//접수참조
		model: 's_mms200ukrv_mekRECEIPTModel',
		autoLoad: false,
		uniOpt : {
			isMaster  : false,			// 상위 버튼 연결
			editable  : false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 's_mms200ukrv_mekService.selectreceiptList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts){
		...
		items :[
			Unilite.popup('CUST',{
				...
				listeners: {
					onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptSearch.setValue('CUSTOM_NAME', '');
						}
					},
					onTextFieldChange: function(field, newValue, oldValue){		// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptSearch.setValue('CUSTOM_CODE', '');
						}
					},
					applyextparam: function(popup){								// 2021.08 표준화 작업
					}
	 ...
	 * master grid
	 */
	var masterGrid = Unilite.createGrid('s_mms200ukrv_mekGrid1', {
		// for tab
		layout : 'fit',
		region : 'center',
		uniOpt: {
	...
	});//End of var masterGrid = Unilite.createGrid('s_mms200ukrv_mekGrid1', {

	/**
	 * 主列表grid2
	 */
	var masterGrid2 = Unilite.createGrid('s_mms200ukrv_mekGrid2', {
		layout: 'fit',
		region: 'south',
		uniOpt: {
			useLiveSearch : true,
	 ...
	 * query grid
	 */
	var receiptNoMasterGrid = Unilite.createGrid('s_mms200ukrv_mekOrderNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		layout : 'fit',
	 ...
	 * 접수참조 grid
	 */
	var receiptGrid = Unilite.createGrid('s_mms200ukrv_mekOtherorderGrid', {//접수참조
		layout : 'fit',
		store: receiptStore,
		uniOpt:{
			useLiveSearch		: true,
	...
	Unilite.Main({
		...
		fnInspecQtyCheck: function(rtnRecord, fieldName, oldValue, divCode, inspecNum, inspecSeq, msgFlag)	{
			...
			s_mms200ukrv_mekService.inspecQtyCheck(param, function(provider, response){
		...
		fnCheckIsModify:function(){
			var store1 = Ext.getCmp("s_mms200ukrv_mekGrid1").getStore();
			var store2 = Ext.getCmp("s_mms200ukrv_mekGrid2").getStore();
			setTimeout( function() {
				...
				case "MAKE_DATE" :
					if(!Ext.isEmpty(record.get('ITEM_CODE'))){
						if(!Ext.isEmpty(newValue)){
							...
							s_mms200ukrv_mekService.selectExpirationdate(params, function(provider, response) {
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName) {
				case "BAD_INSPEC_Q":
					...
					var grdRecord = Ext.getCmp("s_mms200ukrv_mekGrid1").getSelectedRecord();
					var divCode = masterForm.getValue('DIV_CODE');
				...
				case "INSPEC_REMARK":
					var store = Ext.getCmp("s_mms200ukrv_mekGrid2").getStore();
					UniAppManager.app.fnCheckIsModify();
					break;
	...
	function getPjtNoPopupEditor(){
		var editField = Unilite.popup('PROJECT_G',{
			...
			listeners: {
				'applyextparam': function(popup){
					var masterGrid = Ext.getCmp("s_mms200ukrv_mekGrid1");
					var selectRec = masterGrid.getSelectedRecord();
					if(selectRec){
