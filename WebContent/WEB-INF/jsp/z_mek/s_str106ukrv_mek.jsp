...
<t:appConfig pgmId="s_str106ukrv_mek"  >
	<t:ExtComboStore comboType="BOR120"  pgmId="s_str106ukrv_mek" />				<!-- 사업장 -->
...
function appMain() {
	...
	// 엑셀 업로드 모델 생성
	Unilite.Excel.defineModel('excel.s_str106ukrv_mek.sheet01', {
		fields: [
			{name: 'UDI_CODE'			, text: 'UDI 코드'		, type: 'string'},
			{name: 'LOT_NO'				, text: 'LOT 번호'		, type: 'string'},
	...
	function openExcelWindow() {
		...
		if(!excelWindow) {
			...
			excelWindow = Ext.create(appName, {
					modal: false,
					excelConfigName: 's_str106ukrv_mek',
					grids: [{
						itemId: 'grid01',
						title: '엑셀업로드',
						useCheckbox: false,
						model : 'excel.s_str106ukrv_mek.sheet01',
						readApi: 's_str106ukrv_mekService.selectExcelUploadSheet1',
						columns: [
							{dataIndex: 'UDI_CODE'				, width: 110},
							{dataIndex: 'LOT_NO'				, width: 100},
							{dataIndex: 'INOUT_Q' 				, width: 100}
						]
					}],
					extParam: {
						'ISSUE_REQ_NUM'	: panelResult.getValue('ISSUE_REQ_NUM'),
						'ISSUE_REQ_SEQ' : panelResult.getValue('ISSUE_REQ_SEQ'),
					...
					onApply:function()	{
						...
						param._EXCEL_JOBID = me.jobID
						s_str106ukrv_mekService.selectRequestExcelList(param, function(provider, response) {
							if(!Ext.isEmpty(provider)) {
								panelResult.setValue('INOUT_NUM', provider);
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_str106ukrv_mekService.selectDetailList',
			update	: 's_str106ukrv_mekService.updateDetail',
			create	: 's_str106ukrv_mekService.insertDetail',
			destroy	: 's_str106ukrv_mekService.deleteDetail',
			syncAll	: 's_str106ukrv_mekService.saveAll'
		}
	});
			...
			listeners: {
				onSelected: {
					fn: function(records, type) {
						...
						panelResult.setValue('EXCHANGE_RATE', records[0]["EXCHANGE_RATE"]);
					},
					scope: this
				},
					onClear: function(type) {
						...
						panelResult.setValue('EXCHANGE_RATE', '');
				},
...
/*		,{
		...
		api: {
			load	: 's_str106ukrv_mekService.selectMaster',
			submit	: 's_str106ukrv_mekService.syncMaster'
		},
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('s_str106ukrv_mekDetailModel', {
		fields: [
...
//			{name: 'LOT_ASSIGNED_YN'		, text:'LOT_ASSIGNED_YN'   		, type: 'string', defaultValue: "N"},	//lot팝업에서 선택시 Y로 set..
	...
	//마스터 스토어 정의
	var detailStore = Unilite.createStore('s_str106ukrv_mekDetailStore', {
		proxy	: directProxy,
		model	: 's_str106ukrv_mekDetailModel',
		autoLoad: false,
		uniOpt	: {
...
//			console.log('first record2 ORDER_UNIT_O'+ firstRecord2.get('ORDER_UNIT_O'));
 	...
 	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('s_str106ukrv_mekGrid', {
		store	: detailStore,
		layout	: 'fit',
		region	: 'center',
		uniOpt	: {
		...
		tbar: [{
			...
			handler: function() {
				if(Ext.isEmpty(panelResult.getValue('ISSUE_REQ_NUM'))) {
					Unilite.messageBox('출하지시번호를 입력한 후, 엑셀업로드 하세요.');
					return false;
				}
				openExcelWindow();
			}
...
//				grdRecord.set('WH_CELL_NAME'	,"");
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {
		...
		proxy	: {
			type: 'direct',
			api	: {
				read : 's_str106ukrv_mekService.selectOrderNumMasterList'
			}
		},
		loadStoreRecords : function() {
	...
	//검색창 그리드 정의
	var orderNoMasterGrid = Unilite.createGrid('s_str106ukrv_mekOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
	...
	//검색창 detail그리드 정의
	var orderNoDetailGrid = Unilite.createGrid('s_str106ukrv_mekorderNoDetailGrid', {
		store	: orderNoMasterStore,
		layout	: 'fit',
		uniOpt	: {
			useRowNumberer: false
...
//						orderNoSearch.setValue('ORDER_TYPE',panelResult.getValue('ORDER_TYPE'));
			...
			searchInfoWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function( panel, eOpts ) {
						requestNoSearch.setValue('ISSUE_DIV_CODE'	, panelResult.getValue('DIV_CODE'));
						requestNoSearch.setValue('ISSUE_REQ_NUM'	, panelResult.getValue('ISSUE_REQ_NUM'));
						requestNoSearch.setValue('WH_CODE'			, panelResult.getValue('WH_CODE'));
						requestNoSearch.setValue('CUSTOM_CODE'		, panelResult.getValue('CUSTOM_CODE'));
						requestNoSearch.setValue('CUSTOM_NAME'		, panelResult.getValue('CUSTOM_NAME'));
					}
				}
			})
		}
	...
	var requestNoMasterStore = Unilite.createStore('requestNoMasterStore', {
		...
		proxy: {
			type: 'direct',
			api	: {
				read: 's_str106ukrv_mekService.selectOrderNumMasterList2'
			}
		},
		loadStoreRecords : function() {
...
/*							{
	...
	var requestNoMasterGrid = Unilite.createGrid('srq100ukrvOrderNoMasterGrid', {
		...
		columns	: [
			...
			{ dataIndex: 'MONEY_UNIT'			, width: 70, hidden: true},
			{ dataIndex: 'EXCHANGE_RATE'		, width: 70, hidden: true}
		],
		listeners: {
			onGridDblClick: function(grid, record, cellIndex, colName) {
		...
		returnData: function(record) {
			...
			panelResult.setValue('CUSTOM_NAME', 	record.get('CUSTOM_NAME'));
			panelResult.setValue('MONEY_UNIT', 		record.get('MONEY_UNIT'));
			panelResult.setValue('EXCHANGE_RATE', 	record.get('EXCHANGE_RATE'));
	...
	//출하지시 참조 모델 정의
	Unilite.defineModel('s_str106ukrv_mekRequestModel', {
		fields: [
	...
	//출하지시 참조 스토어 정의
	var requestStore = Unilite.createStore('s_str106ukrv_mekRequestStore', {
		model	: 's_str106ukrv_mekRequestModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy	: {
			type: 'direct',
			api	: {
				read : 's_str106ukrv_mekService.selectRequestiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	//출하지시 참조 그리드 정의
	var requestGrid = Unilite.createGrid('s_str106ukrv_mekRequestGrid', {
		store	: requestStore,
		layout	: 'fit',
...
//						requestSearch.setValue('ISSUE_DATE_FR'	, UniDate.get('startOfMonth', salesOrderSearch.getValue('ISSUE_DATE_TO')));
	 ...
	 */
	//수주(오퍼) 참조 폼 정의
	var salesOrderSearch = Unilite.createSearchForm('s_str106ukrv_meksalesOrderForm', {
		layout	: {type : 'uniTable', columns : 3},
		items	: [
			Unilite.popup('DIV_PUMOK',{
			fieldLabel		: '<t:message code="system.label.sales.item" default="품목"/>' ,
	...
	//수주(오퍼) 참조 모델 정의
	Unilite.defineModel('s_str106ukrv_meksalesOrderModel', {
		fields: [
			{ name: 'ORDER_NUM'			, text:'<t:message code="system.label.sales.sono" default="수주번호"/>'					,type : 'string' },
	...
	//수주(오퍼) 참조 스토어 정의
	var salesOrderStore = Unilite.createStore('s_str106ukrv_meksalesOrderStore', {
		model	: 's_str106ukrv_meksalesOrderModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
			editable	: false,	// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_str106ukrv_mekService.selectSalesOrderList'
			}
		},
		loadStoreRecords : function() {
	...
	//수주(오퍼) 참조 그리드 정의
	var salesOrderGrid = Unilite.createGrid('s_str106ukrv_meksalesOrderGrid', {
...
//						salesOrderSearch.setValue('DVRY_DATE_FR', UniDate.get('startOfMonth', salesOrderSearch.getValue('DVRY_DATE_TO')));
	...
	var linkedStore = Unilite.createStore('s_str106ukrv_meklinkedStore', {
		model	: 's_str106ukrv_mekRequestModel',
		autoLoad: false,
		uniOpt	: {
			isMaster	: false,	// 상위 버튼 연결
...
//				cbStore.loadStoreRecords(whCode);
				...
				s_str106ukrv_mekService.deptWhcode(param, function(provider, response){
					if(!Ext.isEmpty(provider)){
...
//				}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_str106ukrv_mekAdvanceSerch');
			if(as.isHidden()) {
				as.show();
			} else {
				as.hide()
		...
		confirmSaveData: function(config) {
			var fp = Ext.getCmp('s_str106ukrv_mekFileUploadPanel');
