...
<t:appConfig pgmId="s_qms500ukrv_mek"  >
	...
	<t:ExtComboStore comboType="AU" comboCode="Q002" /> <!-- 품질검사여부 -->
...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_qms500ukrv_mekService.selectList1',
			create  : 's_qms500ukrv_mekService.insertMaster',
			update  : 's_qms500ukrv_mekService.updateMaster',
			destroy : 's_qms500ukrv_mekService.deleteMaster',
			syncAll : 's_qms500ukrv_mekService.saveAll'
		}
	});

	var directProxy2 = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 's_qms500ukrv_mekService.selectList2',
			update  : 's_qms500ukrv_mekService.updateBad',
			create  : 's_qms500ukrv_mekService.insertBad',
			destroy : 's_qms500ukrv_mekService.deleteBad',
			syncAll : 's_qms500ukrv_mekService.saveAllBad'
		}
	});
	 ...
	 * master grid1 Model
	 */
	Unilite.defineModel('s_qms500ukrv_mekModel1', {
		fields: [
			...
			{name: 'IN_TYPE'			, text: '입고형태'																			, type: 'string'},
			{name: 'HW'					, text: 'HW'																			, type: 'string'},
			{name: 'SW'					, text: 'SW'																			, type: 'string'},
			{name: 'COMPLETE_DATE'		, text: '검사완료일'																			, type: 'uniDate'},
			{name: 'ORI_LOT_NO'			, text: 'ORI_LOT_NO'																	, type: 'string'},
			{name: 'ORI_INSPEC_Q'		, text: 'ORI_INSPEC_Q'																	, type: 'uniQty'}
		]
	});
	 ...
	 * detail grid2 Model
	 */
	Unilite.defineModel('s_qms500ukrv_mekModel2', {
		fields: [
			{name: 'DIV_CODE'			, text: '<t:message code="system.label.purchase.division" default="사업장"/>'				, type: 'string'},
			{name: 'INSPEC_NUM'			, text: '<t:message code="system.label.purchase.inspecno" default="검사번호"/>'				, type: 'string'},
			{name: 'INSPEC_SEQ'			, text: '<t:message code="system.label.purchase.seq" default="순번"/>'					, type: 'int'},
	 ...
	 * 접수참조 grid model
	 */
	Unilite.defineModel('s_qms500ukrv_mekRECEIPTModel', {	//접수참조
		fields: [
			...
			{name: 'RECEIPT_Q'			, text: '<t:message code="" default="입고(미검사)량"/>'										, type: 'uniQty'},
			{name: 'INSPEC_Q'			, text: '<t:message code="" default="입고(미검사)량(재고)"/>'									, type: 'uniQty'},
			{name: 'WH_CODE'			, text: '<t:message code="system.label.purchase.receiptwarehouse" default="입고창고"/>'		, type: 'string'	, comboType : 'OU'},
			{name: 'REP_INOUT_NUM'		, text: '<t:message code="" default="대체번호"/>'											, type: 'string'},
			{name: 'REP_INOUT_SEQ'		, text: '<t:message code="" default="대체순번"/>'											, type: 'int'},
			{name: 'REP_ITEM_CODE'		, text: '<t:message code="" default="대체출고품목"/>'											, type: 'string'},
			{name: 'REP_ITEM_NAME'		, text: '<t:message code="" default="대체출고품목명"/>'										, type: 'string'},
			{name: 'REP_LOT_NO'			, text: '대체출고 LOT NO'																	, type: 'string'},
			...
			{name: 'ORDER_UNIT'			, text: '<t:message code="system.label.purchase.purchaseunit" default="구매단위"/>'			, type: 'string'	, comboType : 'AU'	, comboCode : 'B013'}
		]
	});
	 ...
	 * master grid1 store
	 */
	var directMasterStore1 = Unilite.createStore('s_qms500ukrv_mekMasterStore1', {
		model : 's_qms500ukrv_mekModel1',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				Ext.getCmp('s_qms500ukrv_mekGrid1').uniSelectInvalidColumnAndAlert(inValidRecs);
			}
		},
		listeners: {
	 ...
	 * detail grid2 store
	 */
	var directMasterStore2 = Unilite.createStore('s_qms500ukrv_mekMasterStore2', {
		model: 's_qms500ukrv_mekModel2',
		uniOpt: {
		...
		loadStoreRecords: function(record){
			...
			this.load({
				...
				callback : function(batch, option) {
					...
					if(!Ext.isEmpty(mRow) && mRow.phantom) {
						setTimeout(function(){
	...
	var receiptNoMasterStore = Unilite.createStore('receiptNoMasterStore', {	//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read : 's_qms500ukrv_mekService.selectreceiptNumMasterList'
			}
		},
		loadStoreRecords : function()	{
	 ...
	 * 접수참조 store
	 */
	var receiptStore = Unilite.createStore('s_qms500ukrv_mekReceiptStore', {//접수참조
		model: 's_qms500ukrv_mekRECEIPTModel',
		autoLoad: false,
		uniOpt : {
			isMaster  : false,			// 상위 버튼 연결
			editable  : false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 's_qms500ukrv_mekService.selectreceiptList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts){
	...
	var panelResult = Unilite.createSearchForm('resultForm',{
		...
		items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						UniAppManager.app.fnProcessBarcode(newValue);
					}
				}
			},{
				fieldLabel	: '<t:message code="system.label.purchase.itemcodevalue" default="아이템코드값"/>',
		...
		items: [
				...
				listeners: {
					onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptNoSearch.setValue('CUSTOM_NAME', '');
						}
					},
					onTextFieldChange: function(field, newValue, oldValue){		// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptNoSearch.setValue('CUSTOM_CODE', '');
						}
					},
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
					onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptNoSearch.setValue('ITEM_NAME', '');
						}
					},
					onTextFieldChange: function(field, newValue, oldValue){		// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptNoSearch.setValue('ITEM_CODE', '');
						}
					},
					applyextparam: function(popup){								// 2021.08 표준화 작업
		...
		items :[{
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
					onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptSearch.setValue('ITEM_NAME', '');
						}
					},
					onTextFieldChange: function(field, newValue, oldValue){		// 2021.08 표준화 작업
						if(!Ext.isObject(oldValue)) {
							receiptSearch.setValue('ITEM_CODE', '');
						}
					},
					applyextparam: function(popup){								// 2021.08 표준화 작업
						popup.setExtParam({'DIV_CODE': receiptSearch.getValue('DIV_CODE')});
					}
				}
			})
		]
	 ...
	 * master grid
	 */
	var masterGrid = Unilite.createGrid('s_qms500ukrv_mekGrid1', {
		// for tab
		layout : 'fit',
		region : 'center',
		uniOpt: {
		...
		tbar: [
			{
				...
				itemId		: 'receiptBtn',
				text		: '<div style="color: blue"><t:message code="" default="대체참조"/></div>',
				handler: function() {
		...
		columns: [
			...
			{dataIndex: 'RECEIPT_SEQ'		, width: 80 , hidden: true},
		...
		listeners: {
			...
			beforeedit  : function( editor, e, eOpts ) {
				if (UniUtils.indexOf(e.field,['IN_TYPE', 'LOT_NO', 'HW', 'SW', 'INSPEC_TYPE', 'GOODBAD_TYPE', 'END_DECISION', 'INSPEC_Q', 'GOOD_INSPEC_Q', 'BAD_INSPEC_Q', 'INSPEC_PRSN', 'MAKE_LOT_NO', 'MAKE_DATE', 'MAKE_EXP_DATE', 'COMPLETE_DATE', 'REMARK'])){
					return true;
				}
		...
		setReceiptData:function(record) {
			...
			grdRecord.set('RECEIPT_Q'			, record['RECEIPT_Q']);
			grdRecord.set('INSPEC_Q'			, record['INSPEC_Q']);
			grdRecord.set('TRNS_RATE'			, record['TRNS_RATE']);
			grdRecord.set('ORDER_NUM'			, '');
			grdRecord.set('ORDER_SEQ'			, 0);
			...
			grdRecord.set('GOOD_INSPEC_Q'		, record[('INSPEC_Q')]);
			grdRecord.set('BAD_INSPEC_Q'		, 0);
			grdRecord.set('INSTOCK_Q'			, 0);
			grdRecord.set('END_DECISION'		, '01');
			grdRecord.set('MAKE_LOT_NO'			, '');
			grdRecord.set('MAKE_DATE'			, '');
			grdRecord.set('MAKE_EXP_DATE'		, '');
			grdRecord.set('SO_NUM'				, '');
			grdRecord.set('SO_CUSTOM_NAME'		, '');
			grdRecord.set('SO_ITEM_NAME'		, '');
			
			grdRecord.set('ORI_LOT_NO'			, record['LOT_NO']);
			grdRecord.set('ORI_INSPEC_Q'		, 0);
...
//			grdRecord.set('SO_ITEM_NAME'		, record['SO_ITEM_NAME']);
	...
	});//End of var masterGrid = Unilite.createGrid('s_qms500ukrv_mekGrid1', {

	/**
	 * 主列表grid2
	 */
	var masterGrid2 = Unilite.createGrid('s_qms500ukrv_mekGrid2', {
		layout: 'fit',
		region: 'south',
		uniOpt: {
			useLiveSearch : true,
		...
		listeners: {
			beforeedit  : function( editor, e, eOpts ) {
				if (UniUtils.indexOf(e.field,['BAD_INSPEC_Q','INSPEC_REMARK','MANAGE_REMARK','SEQ'])){
					return true;
				}else{
					return false;
				}
	 ...
	 * query grid
	 */
	var receiptNoMasterGrid = Unilite.createGrid('s_qms500ukrv_mekOrderNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		layout : 'fit',
	 ...
	 * 접수참조 grid
	 */
	var receiptGrid = Unilite.createGrid('s_qms500ukrv_mekOtherorderGrid', {//접수참조
		layout : 'fit',
		store: receiptStore,
		uniOpt:{
			useLiveSearch		: true,
		...
		columns: [
			...
			{ dataIndex: 'INOUT_NUM'			, width:93 , hidden:true},
			{ dataIndex: 'INOUT_SEQ'			, width:93 , hidden:true},
			{ dataIndex: 'ITEM_CODE'			, width:120},
			{ dataIndex: 'ITEM_NAME'			, width:150},
			{ dataIndex: 'LOT_NO'				, width:150},
			{ dataIndex: 'SPEC'					, width:150},
			{ dataIndex: 'INOUT_DATE'			, width:85},
			{ dataIndex: 'RECEIPT_Q'			, width:100},
			{ dataIndex: 'INSPEC_Q'				, width:100},
			{ dataIndex: 'WH_CODE'				, width:100},
			{ dataIndex: 'REP_INOUT_NUM'		, width:120},
			{ dataIndex: 'REP_INOUT_SEQ'		, width:93 , hidden:true},
			{ dataIndex: 'REP_ITEM_CODE'		, width:120},
			{ dataIndex: 'REP_ITEM_NAME'		, width:150},
			{ dataIndex: 'REP_LOT_NO'			, width:150},
			{ dataIndex: 'INSPEC_METH_MATRL'	, width:150},
			{ dataIndex: 'TRNS_RATE'			, width:150},
			{ dataIndex: 'ORDER_UNIT'			, width:150}
		],
		listeners: {
			onGridDblClick:function(grid, record, cellIndex, colName) {
			}
		...
		returnData: function()	{
			...
			Ext.each(records, function(record,i){
				UniAppManager.app.onNewDataButtonDown(record.data.ITEM_CODE, gsOrderType);
	...
	Unilite.Main({
		...
		fnInspecQtyCheck: function(rtnRecord, fieldName, oldValue, divCode, inspecNum, inspecSeq, msgFlag)	{
			...
			s_qms500ukrv_mekService.inspecQtyCheck(param, function(provider, response){
		...
		fnCheckIsModify:function(){
			var store1 = Ext.getCmp("s_qms500ukrv_mekGrid1").getStore();
			var store2 = Ext.getCmp("s_qms500ukrv_mekGrid2").getStore();
			setTimeout( function() {
		...
		fnProcessBarcode: function(val) {
			...
			if(!Ext.isEmpty(mRow)) {
				if(parseVal.length == 2) {
					if(parseVal[0] == 'TYPE1') {
						var inType = mRow.get('IN_TYPE');
						
						if(Ext.isEmpty(inType)) {
							inType = parseVal[1];
						}
						else {
							inType = inType + ',' + parseVal[1];
						}
						
						mRow.set('IN_TYPE', inType);
					}
					else if(parseVal[0] == 'TYPE2' && parseVal[1] == 'SAVE') {
						UniAppManager.app.onSaveDataButtonDown();
					}
				}
				else if(parseVal.length == 3) {
					mRow.set('LOT_NO'	, parseVal[0]);
					mRow.set('HW'		, parseVal[1]);
					mRow.set('SW'		, parseVal[2]);
				}
				else {
				...
				if(bEditYN) {
					mRow.set('COMPLETE_DATE', UniDate.get("today"));
				}
			}
				...
				case "MAKE_DATE" :
					if(!Ext.isEmpty(record.get('ITEM_CODE'))){
						if(!Ext.isEmpty(newValue)){
							...
							s_qms500ukrv_mekService.selectExpirationdate(params, function(provider, response) {
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName) {
				case "BAD_INSPEC_Q":
					...
					var grdRecord = Ext.getCmp("s_qms500ukrv_mekGrid1").getSelectedRecord();
					var divCode = masterForm.getValue('DIV_CODE');
				...
				case "INSPEC_REMARK":
					var store = Ext.getCmp("s_qms500ukrv_mekGrid2").getStore();
					UniAppManager.app.fnCheckIsModify();
					break;
	...
	function getPjtNoPopupEditor(){
		var editField = Unilite.popup('PROJECT_G',{
			...
			listeners: {
				'applyextparam': function(popup){
					var masterGrid = Ext.getCmp("s_qms500ukrv_mekGrid1");
					var selectRec = masterGrid.getSelectedRecord();
					if(selectRec){
