...
var BsaCodeInfo = {	// 컨트롤러에서 값을 받아옴
	gsInvstatus: 		'${gsInvstatus}',		// 재고량 + 기존출고량  과 출고량 체크 관련
...
//	});
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'otr110ukrvService.selectMaster',
			update  : 'otr110ukrvService.updateDetail',
			create  : 'otr110ukrvService.insertDetail',
			destroy : 'otr110ukrvService.deleteDetail',
			syncAll : 'otr110ukrvService.saveAll'
		}
	});
	...
	var searchForm = Unilite.createSearchForm('otr110ukrvSearchForm', {	// 메인
		region  : 'north',
		...
		items: [
			//营业区
			{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						searchForm.setValue('INOUT_PRSN', '');
						masterForm.setValue('INOUT_PRSN', '');
					}
				}
			},
			//外包单位
			...
			//出库主管
			{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						var mfStore = masterForm.getField('INOUT_PRSN').store;
						store.clearFilter();
						mfStore.clearFilter();
						if(!Ext.isEmpty(searchForm.getValue('DIV_CODE'))){
							store.filterBy(function(record){
								return record.get('refCode1') == searchForm.getValue('DIV_CODE');
							});
							mfStore.filterBy(function(record){
								return record.get('refCode1') == searchForm.getValue('DIV_CODE');
							});
						}else{
							store.filterBy(function(record){
			...
			fieldLabel: '<t:message code="system.label.purchase.issuewarehousecell" default="출고창고Cell"/>',
			name: 'WH_CELL_CODE',
			xtype:'uniCombobox',
	...
	var masterForm = Unilite.createSearchPanel('otr110ukrvMasterForm', {	// 메인
			...
			//出库主管
			{
				...
				listeners: {
					change : function(field, newValue, oldValue, eOpts) {
						...
						var store = queryPlan.combo.store;
						var srStore = searchForm.getField('INOUT_PRSN').store;
						store.clearFilter();
						srStore.clearFilter();
						if(!Ext.isEmpty(masterForm.getValue('DIV_CODE'))){
							...
							srStore.filterBy(function(record){
								return record.get('refCode1') == masterForm.getValue('DIV_CODE');
							});
						}else{
							store.filterBy(function(record){
			...
			Unilite.popup('AGENT_CUST',{
				...
				listeners: {
						onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
							orderNoSearch.setValue('INOUT_CODE', newValue);
							if(!Ext.isObject(oldValue)) {
								orderNoSearch.setValue('INOUT_NAME', '');
							}
						},
						onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
							orderNoSearch.setValue('INOUT_NAME', newValue);
							if(!Ext.isObject(oldValue)) {
								orderNoSearch.setValue('INOUT_CODE', '');
							}
						}
				}
			}),{
	...
	Unilite.defineModel('Otr110ukrvModel', {		// 메인
		fields: [
			...
			{name: 'WH_CODE'			,text: '<t:message code="system.label.purchase.issuewarehouse" default="출고창고"/>' 		,type: 'string',  comboType   : 'OU', child: 'WH_CELL_CODE', allowBlank: false},
//			{name: 'WH_NAME'			,text: '<t:message code="system.label.purchase.issuewarehouse" default="출고창고"/>' 		,type: 'string'},
			// 출고창고 WH_CODE / WH_NAME 조회할때 CODE로 하면 출고창고 값 출력, NAME으로 하면 출고창고 값이 안나옴.
			{name: 'WH_CELL_CODE'		,text:'<t:message code="system.label.purchase.issuewarehousecell" default="출고창고Cell"/>'		, type: 'string', allowBlank: sumtypeCell, store: Ext.data.StoreManager.lookup('whCellList'), parentNames:['WH_CODE','DIV_CODE']},
//			{name: 'WH_CELL_NAME'		,text: '<t:message code="system.label.purchase.issuewarehousecell" default="출고창고Cell"/>' 	,type: 'string',maxLength:20},
	...
	Unilite.defineModel('Otr110ukrvOTHERModel', {	//예약참조
		fields: [
			{name: 'ORDER_NUM'			, text: '<t:message code="system.label.purchase.pono" default="발주번호"/>'						, type: 'string'},
			...
			{name: 'WH_CODE'			, text: '<t:message code="system.label.purchase.mainwarehouse" default="주창고"/>'				, type: 'string', xtype: 'uniCombobox',comboType   : 'OU'},
			//20210819 추가: WH_CELL_CODE
			{name: 'WH_CELL_CODE'		, text: '<t:message code="system.label.purchase.mainwarehouse" default="주창고"/>CELL'			, type: 'string', allowBlank: sumtypeCell, store: Ext.data.StoreManager.lookup('whCellList'), parentNames:['WH_CODE','DIV_CODE']},
	...
	Unilite.defineModel('Otr110ukrvOTHERModel2', {	//반품가능예약참조
		fields: [
			{name: 'ORDER_NUM'			, text: '<t:message code="system.label.purchase.pono" default="발주번호"/>'						, type: 'string'},
			{name: 'ORDER_SEQ'			, text: '<t:message code="system.label.purchase.poseq" default="발주순번"/>'					, type: 'string'},
			{name: 'ORDER_DATE'			, text: '<t:message code="system.label.purchase.podate" default="발주일"/>'					, type: 'uniDate'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('Otr110ukrvMasterStore1',{		// 메인
		model: 'Otr110ukrvModel',
		uniOpt: {
...
//				}
				...
				if(BsaCodeInfo.gsInvstatus == "+"){
					var sInout_q = record.get('INOUT_Q');	//출고량
					...
					if(sInout_q > (sInv_q + sOriginQ)){
						alert('<t:message code="system.message.purchase.message036" default="출고량은 재고량을 초과할 수 없습니다."/>');
						isErr = true;
						return false;
					}
				}
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('otr110ukrvGrid');
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {	// 검색버튼 조회창
			...
			proxy: {
				type: 'direct',
				api: {
					read: 'otr110ukrvService.selectDetail'
				}
			},
			loadStoreRecords : function() {
	...
	var otherOrderStore = Unilite.createStore('otr110ukrvOtherOrderStore', {	//예약참조
			model: 'Otr110ukrvOTHERModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'otr110ukrvService.selectDetail2'
				}
			},
			listeners:{
								...
								function(item, i) {
									Ext.each(masterRecords.items, function(record, i) {
											...
											&& (record.data['ORDER_SEQ'] == item.data['ORDER_SEQ'])
												...
												//20200225  예약 참조 시, 출고량 남아있는 데이터는 참조팝업에 보이도록 로직 추가
												var results = directMasterStore1.sumBy(function(detailRecord, id) {
													if(detailRecord.get('ORDER_NUM') == record.data['ORDER_NUM']
													&& detailRecord.get('ORDER_SEQ') == record.data['ORDER_SEQ']) {
														return true;
													}
												},
												['INOUT_Q']);
												...
												if(!item.dirty) {
													if(results.INOUT_Q >= item.data['NOT_OUTSTOCK']) {
														refRecords.push(item);
													} else {
														item.set('NOT_OUTSTOCK', item.data['NOT_OUTSTOCK'] - results.INOUT_Q);
														item.commit();
													}
												}
	...
	var otherOrderStore2 = Unilite.createStore('otr110ukrvotherOrderStore2', {//반품가능예약참조
			model: 'Otr110ukrvOTHERModel2',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'otr110ukrvService.selectDetail3'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	...
	var masterGrid = Unilite.createGrid('otr110ukrvGrid', {		// 메인
		// for tab
		layout : 'fit',
		region : 'center',
		uniOpt: {
		...
		tbar: [{
			...
			text: '<div style="color: blue"><t:message code="system.label.purchase.allocationrefer" default="예약참조"/></div>',
			handler: function() {
			...
			text: '<div style="color: blue"><t:message code="system.label.purchase.returnavaiableallocationrefer" default="반품가능예약참조"/></div>',
			handler: function() {
				if(masterForm.setAllFieldsReadOnly(true)){
					searchForm.setAllFieldsReadOnly(true);
						...
						editor: Unilite.popup('DIV_PUMOK_G', {
							...
							listeners: {
								...
								applyextparam: function(popup){
									...
									popup.setExtParam({'DIV_CODE': Ext.getCmp("otr110ukrvMasterForm").getValue("DIV_CODE")});
								}
							}
						})
					 },
						...
						editor: Unilite.popup('LOTNO_G', {
							...
							listeners: {
								...
								'onClear': function(type) {
									...
									rtnRecord.set('GOOD_STOCK_Q',   '');
									rtnRecord.set('BAD_STOCK_Q',   '');
								}
							}
						})
					 }, */
							...
							listeners: {
								'onSelected': {
									fn: function(records, type) {
										...
										var goodStockQ	= 0;
										var badStockQ	= 0;
										var outQ		= grdRecord.get('INOUT_Q');
										var transRate	= 1;
										Ext.each(records, function(record,i) {
											if(i==0){
												rtnRecord = grdRecord;
												...
												var inoutSeq	= 0 ;
												var sortSeq		= 0;
												inoutSeq		= directMasterStore1.max('INOUT_SEQ');
											...
											/* 	if(Ext.isEmpty(sortSeq)){
												...
												var r = {
													...
													PAB_STOCK_Q			: record['GOOD_STOCK_Q'],
													BAD_STOCK_Q			: record['BAD_STOCK_Q'],
													INOUT_Q				: inoutQ
												}
												newDetailRecords[i-1] = directMasterStore1.model.create( r );
												Ext.each(columns, function(column, index) {
													if (   column.dataIndex != 'INOUT_SEQ'
														&& column.dataIndex != 'LOT_NO'
														&& column.dataIndex != 'WH_CODE'
														&& column.dataIndex != 'WH_CELL_CODE'
														&& column.dataIndex != 'PAB_STOCK_Q'
														&& column.dataIndex != 'GOOD_STOCK_Q'
														&& column.dataIndex != 'BAD_STOCK_Q'
														&& column.dataIndex != 'INOUT_Q') {
			...
			beforeedit: function( editor, e, eOpts ) {
				if(e.record.phantom == false) {		// 신규데이터가 아닌것.
					if(UniUtils.indexOf(e.field, ['LOT_NO'])){
  					...
  					if(UniUtils.indexOf(e.field, ['INOUT_TYPE_DETAIL',  'WH_CELL_CODE', 'LOT_NO',
												  'INOUT_Q', 'ITEM_STATUS', 'REMARK', 'PROJECT_NO','PACK_UNIT_Q', 'BOX_Q', 'EACH_Q', 'LOSS_Q'])) {
						return true;
					} else {
						return false;
					}
					...
					if(UniUtils.indexOf(e.field, ['INOUT_SEQ', 'INOUT_TYPE_DETAIL', 'ITEM_CODE', 'ITEM_NAME', 'WH_CODE', 'WH_CELL_CODE',
												  'LOT_NO', 'INOUT_Q', 'ITEM_STATUS', 'REMARK', 'PROJECT_NO','PACK_UNIT_Q', 'BOX_Q', 'EACH_Q', 'LOSS_Q']))
					{
						return true;
					} else {
						return false;
...
//			grdRecord.set('WH_CODE'				, record['WH_CODE']); 메인상단에 있는 값으로 셋팅
...
//													selectRec.set('WH_CELL_NAME', '');
	...
	var orderNoMasterGrid = Unilite.createGrid('otr110ukrvOrderNoMasterGrid', {		// 검색팝업창
		layout	: 'fit',
	...
	//预定参考
	var otherorderGrid = Unilite.createGrid('otr110ukrvotherorderGrid', {		//예약참조
		layout	: 'fit',
		...
		columns	: [
			...
			{ dataIndex: 'WH_CELL_CODE'			, width: 100},			//20210819 추가
	...
	//退货可能预定参考
	var otherorderGrid2 = Unilite.createGrid('otr110ukrvotherorderGrid2', {		//반품가능예약참조
		layout	: 'fit',
	...
	function openSearchInfoWindow() {				// 검색팝업창
		if(!SearchInfoWindow) {
			SearchInfoWindow = Ext.create('widget.uniDetailWindow', {
				title: '<t:message code="system.label.purchase.subcontractissuenosearch" default="외주출고번호검색"/>',
				width: 1080,
				height: 580,
			...
			ReservationWindow = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
						...
						otherorderSearch.setValue("CLOSING_YN"		, 'A');
						otherOrderStore.loadStoreRecords();
					}
				}
			})
...
//			cbStore.loadStoreRecords();
		...
		confirmSaveData: function(config) {	// 저장하기전 원복 시키는 작업
			var fp = Ext.getCmp('otr110ukrvFileUploadPanel');
				...
				Ext.Ajax.request({
					...
					success : function(response){
						if(response.status == "200"){
							...
							var selectRecord = Ext.getCmp("otr110ukrvGrid").getSelectedRecord();
							if(selectRecord){
	...
	Unilite.createValidator('validator01', {
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName) {
				...
				case "INOUT_Q" :	// 출고량
					if(newValue < 0 && !Ext.isEmpty(newValue) && record.get('INOUT_TYPE_DETAIL') != "90" )  {
						rv= '<t:message code="system.message.purchase.message024" default="양수만 입력 가능합니다."/>';
						break;
					}
					...
					if(record.get('INOUT_METH') == "2" && BsaCodeInfo.gsUsePabStockYn == "Y"){   //예외 출고 및 가용재고체크 사용할시
						...
						var sOriginQ = record.get('ORIGINAL_Q'); //출고량(원)
						if(sInout_q > (sInv_q + sOriginQ)){
							rv='<t:message code="system.message.purchase.message014" default="출고량은 가용재고량을 초과할 수 없습니다."/>';
							break;
						}
					}
					...
					if(BsaCodeInfo.gsInvstatus == "+"){
						...
						if(sInout_q > (sInv_q + sOriginQ)){
							rv='<t:message code="system.message.purchase.message036" default="출고량은 재고량을 초과할 수 없습니다."/>';
							break;
						}
					}
					break;
...
//						UniMatrl.fnStockQ(record, UniAppManager.app.cbStockQ, UserInfo.compCode, record.get('DIV_CODE'), null, record.get('ITEM_CODE'), newValue );
				...
				case "PACK_UNIT_Q":
					if(BsaCodeInfo.gsBoxYN == 'Y'){
						...
						var sInout_q = 0;
						var noInoutQ = record.get('NOT_OUTSTOCK_Q')
						if(record.obj.phantom){
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(newValue * record.get('BOX_Q') + record.get('EACH_Q') > noInoutQ){			//출고수량 > 기출고량
									sInout_q = noInoutQ;	//출고수량 = 미납량(참조에서 가져온 미납량)
									record.set('LOSS_Q',newValue * record.get('BOX_Q') + record.get('EACH_Q') - noInoutQ);//LOSS여분 = 출고수량 - 기출고량
								} else {
							...
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(newValue * record.get('BOX_Q') + record.get('EACH_Q') > record.get('ORIGINAL_Q') + noInoutQ  ){			//출고수량 > 기출고량 + 미출고량
									sInout_q = record.get('ORIGINAL_Q')+ noInoutQ;	//입고수량 = 미납량
									record.set('LOSS_Q', newValue * record.get('BOX_Q') + record.get('EACH_Q') - ( record.get('ORIGINAL_Q') + noInoutQ ));//LOSS여분 = 출고수량 - 미출고량
								} else {
									sInout_q = newValue * record.get('BOX_Q') + record.get('EACH_Q');	//출고량
									record.set('LOSS_Q',0);
								}
				...
				case "BOX_Q":
					if(BsaCodeInfo.gsBoxYN == 'Y'){
						var sInout_q = 0;
						var noInoutQ = record.get('NOT_OUTSTOCK_Q');//미납량
						if(record.obj.phantom){
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(record.get('PACK_UNIT_Q') * newValue + record.get('EACH_Q') > noInoutQ){			//출고수량 > 기출고량
									sInout_q = noInoutQ;	//출고수량 = 미납량(참조에서 가져온 미납량)
									record.set('LOSS_Q',record.get('PACK_UNIT_Q') * newValue + record.get('EACH_Q') - noInoutQ);//LOSS여분 = 출고수량 - 기출고량
								} else {
							...
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(record.get('PACK_UNIT_Q') * newValue + record.get('EACH_Q') > record.get('ORIGINAL_Q') + noInoutQ){			//출고수량  > 출고수량  + 미출고량
									sInout_q = record.get('ORIGINAL_Q') + noInoutQ;	//출고수량 = 기존출고수량 + 미출고수량
									record.set('LOSS_Q', record.get('PACK_UNIT_Q') * newValue + record.get('EACH_Q') - (record.get('ORIGINAL_Q') + noInoutQ));//LOSS여분 = 출고수량  - (기출고량 + 미출고량
				...
				case "EACH_Q":
					if(BsaCodeInfo.gsBoxYN == 'Y'){
						var sInout_q = 0;
						var noInoutQ = record.get('NOT_OUTSTOCK_Q');//미출고량
						if(record.obj.phantom){
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(record.get('PACK_UNIT_Q') * record.get('BOX_Q') + newValue > noInoutQ){			//출고수량 > 기출고량
									sInout_q = record.get('ORIGINAL_Q');	//출고수량 = 미납량(참조에서 가져온 미납량)
									record.set('LOSS_Q', record.get('PACK_UNIT_Q') * record.get('BOX_Q') + newValue - noInoutQ);//LOSS여분 = 출고수량 - 기출고량
								} else {
							...
							if(!Ext.isEmpty(record.get('INOUT_NUM'))){
								if(record.get('PACK_UNIT_Q') * record.get('BOX_Q') + newValue > record.get('ORIGINAL_Q') + noInoutQ ){			//출고수량 > 기출고량 + 미출고량
									sInout_q = record.get('ORIGINAL_Q') + noInoutQ;	//출고수량 = 미출고량
									record.set('LOSS_Q', record.get('PACK_UNIT_Q') * record.get('BOX_Q') + newValue - (record.get('ORIGINAL_Q') + noInoutQ));//LOSS여분 = 츨고수량 - 기출고수량 + 미출고수량
