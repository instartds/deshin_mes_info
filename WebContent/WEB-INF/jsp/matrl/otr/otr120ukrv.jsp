...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read	: 'otr120ukrvService.selectMaster',
			update	: 'otr120ukrvService.updateDetail',
			create	: 'otr120ukrvService.insertDetail',
			destroy	: 'otr120ukrvService.deleteDetail',
			syncAll	: 'otr120ukrvService.saveAll'
		}
	});

	/**
	 */
	var searchForm = Unilite.createSearchForm('otr120ukrvSearchForm', {	// 메인
		region	: 'north',
			...
			Unilite.popup('AGENT_CUST',{
				...
				listeners: {
						...
						onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
							orderNoSearch.setValue('CUSTOM_NAME', newValue);
							if(!Ext.isObject(oldValue)) {
...
//				allowBlank: BsaCodeInfo.gsSumTypeCell == "Y"?false:true,
	 ...
	 * 主要的-----------------------------------------------------------------------------model
	 */
	Unilite.defineModel('Otr120ukrvModel', {		// 메인
		fields: [
...
//			{name: 'WH_CELL_CODE'		,text:'<t:message code="system.label.purchase.receiptwarehouse" default="입고창고"/>Cell'	,type: 'string'},
	 ...
	 * 未入库参照model
	 */
	Unilite.defineModel('Otr120ukrvOTHERModel', {	//미입고참조
		fields: [
	 ...
	 * 可退货发货参照model
	 */
	Unilite.defineModel('Otr120ukrvOTHERModel2', {	//반품가능발주참조
		fields: [
	 ...
	 * 检查结果参照model
	 */
	Unilite.defineModel('Otr120ukrvOTHERModel3', {	//검사결과참조
		fields: [
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'				, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
	 ...
	 * 主要的--------------------------------------------------------------------------------store
	 */
	var directMasterStore1 = Unilite.createStore('otr120ukrvMasterStore1',{		// 메인
		model: 'Otr120ukrvModel',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('otr120ukrvGrid1');
	...
	var orderNoMasterStore = Unilite.createStore('orderNoMasterStore', {		// 검색버튼 조회창
			...
			proxy: {
				type: 'direct',
				api: {
					read: 'otr120ukrvService.selectDetail'
				}
			},
			loadStoreRecords : function() {
	 ...
	 * 未入库参照store
	 */
	var otherOrderStore = Unilite.createStore('otr120ukrvOtherOrderStore', {	//미입고참조
			model: 'Otr120ukrvOTHERModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,		// 상위 버튼 연결
				editable: false,		// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'otr120ukrvService.selectDetail2'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	 ...
	 * 可退货发货参照store
	 */
	var otherOrderStore2 = Unilite.createStore('otr120ukrvOtherOrderStore2', {	//반품가능발주참조
			model: 'Otr120ukrvOTHERModel2',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'otr120ukrvService.selectDetail3'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	 ...
	 * 检查结果参照store ########
	 */
	var otherOrderStore3 = Unilite.createStore('otr120ukrvOtherOrderStore3', {	//검사결과/무검사참조 겸용
			model: 'Otr120ukrvOTHERModel3',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'otr120ukrvService.selectDetail4'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	 ...
	 * 主要的-------------------------------------------------------------------------------grid
	 */
	var masterGrid = Unilite.createGrid('otr120ukrvGrid1', {						// 메인
		layout : 'fit',
		region:'center',
		uniOpt: {
...
//			id:'inspectnoBtn',
		...
		setCheckResultData: function(record) {					// 검사결과참조 셋팅
			...
			if(BsaCodeInfo.gsSumTypeCell == 'Y') {
				//20210819 추가
				if(Ext.isEmpty(searchForm.getValue('WH_CELL_CODE'))){
	 ...
	 * 订单编号弹出grid
	 */
	var orderNoMasterGrid = Unilite.createGrid('otr120ukrvOrderNoMasterGrid', {		// 검색팝업창
		// title: '기본',
		layout : 'fit',
	 ...
	 * 未入库参照弹出grid
	 */
	var otherorderGrid = Unilite.createGrid('otr120ukrvOtherorderGrid', {			//미입고참조
		// title: '기본',
		layout : 'fit',
	 ...
	 * 可退货发货参照弹出grid
	 */
	var otherorderGrid2 = Unilite.createGrid('otr120ukrvOtherorderGrid2', {			//반품가능발주참조
		// title: '기본',
		layout : 'fit',
	 ...
	 * 检查结果参照弹出grid
	 */
	var otherorderGrid3 = Unilite.createGrid('otr120ukrvOtherorderGrid3', {			//검사결과참조
		// title: '기본',
		layout : 'fit',
...
//			cbStore.loadStoreRecords(whCode);
		...
		selectOrderPrice: function(flag, grdRecord){
			...
			otr120ukrvService.selectGetOrderPrice(param, function(provider, response){
				if(!Ext.isEmpty(provider)) {
					var dExchangeRate = 0;
		...
		selectStockQ:function(grdRecord){
			...
			otr120ukrvService.selectStockQ(param, function(provider, response){
				if(!Ext.isEmpty(provider)) {
