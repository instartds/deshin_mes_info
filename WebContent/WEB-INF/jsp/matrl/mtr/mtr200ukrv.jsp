...
//	});
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'mtr200ukrvService.selectList',
			update: 'mtr200ukrvService.updateDetail',
			create: 'mtr200ukrvService.insertDetail',
			destroy: 'mtr200ukrvService.deleteDetail',
			syncAll: 'mtr200ukrvService.saveAll'
		}
	});
   /** Model 정의
	* @type
	*/
	Unilite.defineModel('Mtr200ukrvModel', {
		fields: [
	...
	});//End of Unilite.defineModel('Mtr200ukrvModel', {
	...
	Unilite.defineModel('refSearchMasterModel', {	  // 출고요청 참조
		fields: [
			...
			{name: 'MAIN_WH_CELL_CODE'		   , text: '<t:message code="system.label.purchase.mainwarehousecellcode" default="주창고CELL코드"/>'			, type: 'string'},
			{name: 'MAIN_WH_CELL_NAME'		   , text: '<t:message code="system.label.purchase.mainwarehouse" default="주창고"/> CELL'				, type: 'string'},
	...
	Unilite.defineModel('refSearchMasterModel3', {	  // 재고 참조
		fields: [
			...
			{name: 'GOOD_STOCK_Q'		   , text: '<t:message code="system.label.purchase.goodstock" default="양품재고"/>'				, type: 'uniQty'},
			{name: 'BAD_STOCK_Q'			, text: '<t:message code="system.label.purchase.defectinventory" default="불량재고"/>'				, type: 'uniQty'},
			//20200602 추가: WH_CODE, WH_CELL_CODE
			{name: 'WH_CODE'				, text: '<t:message code="system.label.purchase.issuewarehouse" default="출고창고"/>'		, type: 'string', comboType: 'OU', child: 'WH_CELL_CODE'},
			{name: 'WH_CELL_CODE'			, text: '<t:message code="system.label.purchase.issuewarehouse" default="출고창고"/>Cell'	, type: 'string', store: Ext.data.StoreManager.lookup('whCellList'), parentNames:['WH_CODE','DIV_CODE']}
		]
	});
	...
	* @type
	*/
	var directMasterStore1 = Unilite.createStore('mtr200ukrvMasterStore1',{
		model: 'Mtr200ukrvModel',
		autoLoad: false,
		uniOpt: {
...
//								UniAppManager.setToolbarButtons('save', false);
	...
	});//End of var directMasterStore1 = Unilite.createStore('mtr200ukrvMasterStore1',{
	...
	var releaseNoMasterStore = Unilite.createStore('releaseNoMasterStore', {	// 검색팝업창
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'mtr200ukrvService.selectreleaseNoMasterList'
				}
			},
			loadStoreRecords : function()	{
	...
	var refSearchMasterStore = Unilite.createStore('refSearchMasterStore', {   //참조
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'mtr200ukrvService.selectrefList'
				}
			},
			loadStoreRecords : function()   {
	...
	var refSearchMasterStore2 = Unilite.createStore('refSearchMasterStore2', {	//참조2
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 'mtr200ukrvService.selectrefList2'
				}
			},
			loadStoreRecords : function()   {
	...
	var refSearchMasterStore3 = Unilite.createStore('refSearchMasterStore3', {	//참조2
		...
		proxy: {
			type: 'direct',
			api: {
				read	: 'mtr200ukrvService.selectrefList3'
			}
		},
		loadStoreRecords : function()   {
			...
			items: [{
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						if(newValue == '1'){
							Ext.getCmp('MoveReleaseBtn3').setDisabled(false);
							refSearch3.down('#workShopP3').setHidden(true);//작업장
							refSearch3.down('#deptP3').setHidden(false);//부서
						}else if(newValue == '3'){
							Ext.getCmp('MoveReleaseBtn3').setDisabled(false);
							refSearch3.down('#workShopP3').setHidden(false);//작업장
							refSearch3.down('#deptP3').setHidden(true);//부서
						}else{
							Ext.getCmp('MoveReleaseBtn3').setDisabled(true);
						}
					}
...
//		,tdAttrs: {style: 'border : 1px solid #ced9e7;'/*, align : 'center'*/}
		...
		*/
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
					applyextparam: function(popup){
						popup.setExtParam({'DIV_CODE': panelSearch.getValue('DIV_CODE'), 'CUSTOM_TYPE': '3'});
					}
				}
			}),{
				fieldLabel	: 'LOT NO',
...
//				tdAttrs	: {style: 'border : 1px solid #ced9e7'}
			...
			Unilite.popup('DEPT',{
				itemId : 'deptP3',
				fieldLabel		: '<t:message code="system.label.purchase.issueplace" default="출고처"/>',
				valueFieldName	: 'INOUT_CODE_D',
				textFieldName	: 'INOUT_NAME_D',
				hidden:true,
				padding			: '0 0 5 0'
			}),
			Unilite.popup('WORK_SHOP',{
				itemId : 'workShopP3',
				fieldLabel		: '<t:message code="system.label.purchase.issueplace" default="출고처"/>',
				valueFieldName	: 'INOUT_CODE_W',
				textFieldName	: 'INOUT_NAME_W',
				hidden:false,
				padding			: '0 0 5 0',
				listeners: {
					applyextparam: function(popup){
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('mtr200ukrvGrid1', {
	   // for tab
		layout: 'fit',
		region:'center',
		uniOpt: {
						  ...
						  'onClear' : function(type)	{
							...
							applyextparam: function(popup){
								...
								popup.setExtParam({'DIV_CODE': param.DIV_CODE
												 ,'FR_PRODT_DATE': UniDate.get('startOfMonth', param.INOUT_DATE)
												 ,'TO_PRODT_DATE': UniDate.get('endOfMonth',   param.INOUT_DATE)
												 });
		...
		listeners: {
			beforeedit  : function( editor, e, eOpts ) {
				...
				if(e.record.phantom){
					...
					if (UniUtils.indexOf(e.field,[
							'INOUT_Q', 'ITEM_STATUS', 'INOUT_SEQ', 'REMARK', 'ARRAY_REF_WKORD_NUM'])){
							return true;
						}
...
//			grdRecord.set('LOT_NO'					, record['WK_LOT_NO']);
	...
	});//End of var masterGrid = Unilite.createGrid('mtr200ukrvGrid1', {

	var releaseNoMasterGrid = Unilite.createGrid('mtr200ukrvReleaseNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		// title: '기본',
		layout : 'fit',
	...
	var refSearchGrid = Unilite.createGrid('mtr200ukrvRefSearchGrid', {	 // 참조
		// title: '기본',
		layout : 'fit',
		...
		columns:  [
			 ...
			 { dataIndex: 'MAIN_WH_CELL_CODE'					,  width:66, hidden: true},
			 { dataIndex: 'MAIN_WH_CELL_NAME'					,  width:100},
	...
	var refSearchGrid2 = Unilite.createGrid('mtr200ukrvRefSearchGrid2', {	 // 반품가능요청참조
		// title: '기본',
		layout : 'fit',
	...
	var refSearchGrid3 = Unilite.createGrid('mtr200ukrvRefSearchGrid3', {	 // 재고참조
		layout : 'fit',
		...
		/*	 Ext.each(records, function(record,i) {
			...
			UniAppManager.app.fnStockDataRef(records);
			this.getStore().remove(records);
		}
	});
		...
		if(!RefSearchWindow3) {
			RefSearchWindow3 = Ext.create('widget.uniDetailWindow', {
				...
				listeners : {
					...
					beforeshow: function( panel, eOpts )  {
						refSearch3.setValue('INOUT_DATE', panelResult.getValue('INOUT_DATE'));
	...
	Unilite.Main({
		...
		processParams: function(params) {
			if(params.PGM_ID == 'pmr100ukrv') {
				...
				mtr200ukrvService.selectrefList(param, function(provider, response) {
...
// 				   newDetailRecords[i].set('WH_CODE'				 , record.get('WH_CODE'));
				   ...
				   if(BsaCodeInfo.gsSumTypeCell == 'Y') {
					   ...
					   if(Ext.isEmpty(newDetailRecords[i].get('WH_CELL_CODE'))){
						  newDetailRecords[i].set('WH_CELL_CODE'		, record.get('MAIN_WH_CELL_CODE'));
					   }
				   } else {
...
//				   newDetailRecords[i].set('LOT_NO'					, record.get('WK_LOT_NO']);
				   newDetailRecords[i].set('INOUT_Q'				 , record.get('NOT_Q'));
			 ...
			 Ext.each(records, function(record,i){
				 ...
				 if(panelSearch.getValue('INOUT_CODE_TYPE') == '2') {
					...
					// newDetailRecords[i].set('LOT_NO'					, record.get('WK_LOT_NO'));
					 ...
					 newDetailRecords[i].set('ARRAY_LOT_NO'			, record.get('WK_LOT_NO'));
					 newDetailRecords[i].set('REF_GUBUN'			   , '3');
					 /*
		...
		//재고참조시
		fnStockDataRef: function(records) {
			...
			if(Ext.isEmpty(seq)){
			 ...
			 Ext.each(records, function(record,i){
				 ...
				 newDetailRecords[i] = directMasterStore1.model.create( r );
					 ...
					 newDetailRecords[i].set('COMP_CODE'			   , UserInfo.compCode);
					 newDetailRecords[i].set('DIV_CODE'				, record.get('DIV_CODE'));
					 newDetailRecords[i].set('INOUT_METH'				, '2');
					 ...
					 if (panelSearch.getValue("INOUT_CODE_TYPE") == '1'){
				 		newDetailRecords[i].set('INOUT_CODE'	   , refSearch3.getValue('INOUT_CODE_D'));
					 }else
					 {
					 	newDetailRecords[i].set('INOUT_CODE'	   , refSearch3.getValue('INOUT_CODE_W'));
					 }

					 if (panelSearch.getValue("INOUT_CODE_TYPE") == '1'){
//					 	if(!Ext.isEmpty(refSearch3.getValue('INOUT_NAME_D'))){
					 		newDetailRecords[i].set('INOUT_NAME'	   , refSearch3.getValue('INOUT_NAME_D'));
					 		newDetailRecords[i].set('INOUT_NAME2'	   , refSearch3.getValue('INOUT_NAME_D'));
//					 	}
					 }else
					 {
//					 	if(!Ext.isEmpty(refSearch3.getValue('INOUT_NAME_M'))){
					 		newDetailRecords[i].set('INOUT_NAME'	   , refSearch3.getValue('INOUT_NAME_W'));
					 		newDetailRecords[i].set('INOUT_NAME1'	   , refSearch3.getValue('INOUT_NAME_W'));
//					 	}
					 }
					...
					newDetailRecords[i].set('ITEM_STATUS'			 , record.get('ITEM_STATUS'));
			 		...
			 		}else if(record.get('ITEM_STATUS') == '2'){
	...
	console.log('validate >>> ', {'type':type, 'fieldName':fieldName, 'newValue':newValue, 'oldValue':oldValue, 'record':record});
		...
		case "INOUT_TYPE_DETAIL":
			...
			mtr200ukrvService.selectInoutType(param, function(provider, response)	{
				if(!Ext.isEmpty(provider)){
