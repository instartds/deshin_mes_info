...
<t:appConfig pgmId="mms510ukrv">
	<t:ExtComboStore comboType="BOR120" pgmId="mms510ukrv"/>				<!-- 사업장 -->
...
//	var sumtypeLot = BsaCodeInfo.gsSumTypeLot == "Y"?true:false;
	...
	Unilite.defineModel('Mms510ukrvModel1', {
		fields: [
			...
			{name: 'EXPENSE_I'		  , text: '부대비용'																			  , type: 'uniPrice'},
...
//			{name: 'PURCHASE_RATE'		, text: 'PURCHASE_RATE'	, type: 'int'}
	...
	Unilite.defineModel('mms510ukrvRECEIVEModel', {		//미입고참조
		fields: [
	...
	Unilite.defineModel('mms510ukrvRETURNModel', {		//반품가능발주참조
		fields: [
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'				, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
			{name: 'ITEM_NAME'			, text: '<t:message code="system.label.purchase.itemname" default="품목명"/>'				, type: 'string'},
	...
	Unilite.defineModel('mms510ukrvSCMREFModel1', {		//SCM참조
		fields: [
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'					, type: 'string'},
	...
	Unilite.defineModel('mms510ukrvINSPECTModel2', {	//검사결과참조
		fields: [
			{name: 'GUBUN'				, text: '<t:message code="system.label.purchase.selection" default="선택"/>'				, type: 'string'},
			{name: 'ITEM_CODE'			, text: '<t:message code="system.label.purchase.itemcode" default="품목코드"/>'				, type: 'string'},
			{name: 'ITEM_NAME'			, text: '<t:message code="system.label.purchase.itemname" default="품목명"/>'				, type: 'string'},
...
//				 }
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('mms510ukrvGrid1');
...
//					this.fnSumAmountI();
	...
	var inoutNoMasterStore = Unilite.createStore('inoutNoMasterStore', {	//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms510ukrvService.selectinoutNoMasterList'
			}
		}
		,loadStoreRecords : function() {
	...
	var noReceiveStore = Unilite.createStore('mms510ukrvNoReceiveStore', {//미입고참조
			model: 'mms510ukrvRECEIVEModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'mms510ukrvService.selectnoReceiveList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	...
	var returnPossibleStore = Unilite.createStore('mms510ukrvReturnPossibleStore', {//반품가능발주참조
			model: 'mms510ukrvRETURNModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'mms510ukrvService.selectreturnPossibleList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	...
	var scmRefStore = Unilite.createStore('mms510ukrvScmRefStore', {//업체SCM참조
			model: 'mms510ukrvSCMREFModel1',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read : 'mms510ukrvService.selectScmRefList'
				}
			},
			listeners:{
				load:function(store, records, successful, eOpts) {
	...
	var inspectResultStore2 = Unilite.createStore('mms510ukrvInspectResultStore', {//검사결과참조(무검사참조겸용)
		model: 'mms510ukrvINSPECTModel2',
		autoLoad: false,
		uniOpt : {
			isMaster: true,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read : 'mms510ukrvService.selectinspectResultList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
...
//						UniAppManager.app.fnExchngRateO();
	...
	var inoutNoSearch = Unilite.createSearchForm('inoutNoSearchForm', {		//조회버튼 누르면 나오는 조회창
		...
		items: [{
			...
			Unilite.popup('AGENT_CUST',{
				...
				listeners: {
							onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
								inoutNoSearch.setValue('INOUT_CODE', newValue);
								if(!Ext.isObject(oldValue)) {
									inoutNoSearch.setValue('INOUT_NAME', '');
								}
							},
							onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
								inoutNoSearch.setValue('INOUT_NAME', newValue);
								if(!Ext.isObject(oldValue)) {
									inoutNoSearch.setValue('INOUT_CODE', '');
								}
							}
					}
			}),
			...
			Unilite.popup('DIV_PUMOK',{
				...
				listeners: {
							onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
								inoutNoSearch.setValue('ITEM_CODE', newValue);
								if(!Ext.isObject(oldValue)) {
									inoutNoSearch.setValue('ITEM_NAME', '');
								}
							},
							onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
								inoutNoSearch.setValue('ITEM_NAME', newValue);
								if(!Ext.isObject(oldValue)) {
									inoutNoSearch.setValue('ITEM_CODE', '');
								}
							},
	...
	var noreceiveSearch = Unilite.createSearchForm('noreceiveForm', {//미입고참조
		...
		Unilite.popup('DIV_PUMOK',{
			...
			listeners: {
						onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
							noreceiveSearch.setValue('ITEM_CODE', newValue);
							if(!Ext.isObject(oldValue)) {
								noreceiveSearch.setValue('ITEM_NAME', '');
							}
						},
						onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
							noreceiveSearch.setValue('ITEM_NAME', newValue);
							if(!Ext.isObject(oldValue)) {
								noreceiveSearch.setValue('ITEM_CODE', '');
							}
						},
					applyextparam: function(popup){	// 2021.08 표준화 작업
						popup.setExtParam({'DIV_CODE': panelSearch.getValue('DIV_CODE')});
	...
	var returnpossibleSearch = Unilite.createSearchForm('returnpossibleForm', {//반품가능발주참조
			...
			Unilite.popup('DIV_PUMOK',{
					...
					listeners: {
								onValueFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
									returnpossibleSearch.setValue('ITEM_CODE', newValue);
									if(!Ext.isObject(oldValue)) {
										returnpossibleSearch.setValue('ITEM_NAME', '');
									}
								},
								onTextFieldChange: function(field, newValue, oldValue){	// 2021.08 표준화 작업
									returnpossibleSearch.setValue('ITEM_NAME', newValue);
									if(!Ext.isObject(oldValue)) {
										returnpossibleSearch.setValue('ITEM_CODE', '');
									}
								},
							applyextparam: function(popup){	// 2021.08 표준화 작업
								popup.setExtParam({'DIV_CODE': panelSearch.getValue('DIV_CODE')});
...
//			margin:'0 0 0 100',
			...
			beforeedit  : function( editor, e, eOpts ) {
				...
				if(e.record.data.ORDER_NUM != ''){
					...
					if(UniUtils.indexOf(e.field, ['ACCOUNT_YNC','ORDER_UNIT_Q','INOUT_SEQ'
												 ,'WH_CELL_CODE','INOUT_I','INOUT_P','PRICE_YN','EXCHG_RATE_O','MONEY_UNIT','MAKE_LOT_NO','MAKE_DATE','MAKE_EXP_DATE','EXPENSE_I'])){
						return true;
					}
				...
				/*	  if(e.record.data.FLAG == 'FLAG') {
					...
					if(UniUtils.indexOf(e.field, ['INOUT_P','ORDER_UNIT_Q','INOUT_I','ACCOUNT_YNC'
												 ,'PRICE_YN','MONEY_UNIT','EXCHG_RATE_O','MAKE_LOT_NO','MAKE_DATE','MAKE_EXP_DATE','EXPENSE_I'])){
						return true;
					}
					if(UniUtils.indexOf(e.field, ['ITEM_STATUS'])){
						if(BsaCodeInfo.gsProcessFlag == "PG"){
					...
					if(UniUtils.indexOf(e.field, ['LOT_NO','ORDER_UNIT_P','ORDER_UNIT_I','ORDER_UNIT_FOR_P'
												 ,'ORDER_UNIT_FOR_O','ORDER_UNIT','REMARK','PROJECT_NO','TRANS_COST','TARIFF_AMT','EXPENSE_I'])){
						return true;
					}
				...
				if(e.record.phantom == true) {
					...
					if(e.record.data.INOUT_TYPE_DETAIL == '20'){ //무상입고
						if(UniUtils.indexOf(e.field, ['ORDER_NUM', 'ORDER_SEQ'])){
							return true;
						}
					}
				}
		...
		setItemData: function(record, dataClear, grdRecord) {
			...
			if(dataClear) {
				...
				mms510ukrvService.fnOrderPrice(param, function(provider, response) {
...
//			grdRecord.set('MONEY_UNIT'			, record['MONEY_UNIT']);
			//20210121 재고단가 추가 수정
			grdRecord.set('INOUT_P'				, record['ORDER_LOC_P']/ record['TRNS_RATE']);	//단가(재고), 20200908 수정: isNaN(grdRecord.get('INOUT_I')/ grdRecord.get('INOUT_Q'))?0:grdRecord.get('INOUT_I')/ grdRecord.get('INOUT_Q')) -> record['ORDER_LOC_P']
...
//			grdRecord.set('MONEY_UNIT'			, record['MONEY_UNIT']);
			...
			if(record.ORDER_UNIT_Q != record.REMAIN_Q) {
				...
				grdRecord.set('INOUT_P'			, record['ORDER_LOC_P'] == 0 ? record['ORDER_UNIT_P'] : record['ORDER_LOC_P']/ record['TRNS_RATE']);	//20200908 수정: 조건별로 다른 값 입력되도록 수정 20210121 재고단가 입수 계산돼 들어가도록 수정
...
//			grdRecord.set('PURCHASE_RATE'		, record['PURCHASE_RATE']);
			...
			mms510ukrvService.taxType(param, function(provider, response) {
				if(!Ext.isEmpty(provider)){
...
//			grdRecord.set('MONEY_UNIT'			, record['MONEY_UNIT']);
	...
	});//End of var masterGrid = Unilite.createGrid('mms510ukrvGrid1', {

	var inoutNoMasterGrid = Unilite.createGrid('mms510ukrvinoutNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		layout : 'fit',
	...
	var noreceiveGrid = Unilite.createGrid('mms510ukrvNoreceiveGrid', {//미입고참조
		layout : 'fit',
...
//			this.deleteSelectedRow();
	...
	var returnpossibleGrid = Unilite.createGrid('mms510ukrvReturnpossibleGrid', {//반품가능발주참조
		layout: 'fit',
...
/*	var scmRefGrid= Unilite.createGrid('mms510ukrvScmRefGrid1', {//업체출고 참조(SCM)
		region: 'center' ,
		layout: 'fit',
		store: scmRefStore,
		uniOpt: {
	...
	var inspectresultGrid2 = Unilite.createGrid('mms510ukrvInspectResultGrid2', {//검사결과참조
		region: 'east' ,
		layout : 'fit',
		...
		columns:  [
			...
			{dataIndex: 'SOF_ORDER_NUM'			, width:120,hidden:false},
			{dataIndex: 'SOF_ORDER_SEQ'			, width:80,hidden:false,align:"center"},
			...
			{dataIndex: 'MAKE_DATE'			, width: 100},
			{dataIndex: 'MAKE_EXP_DATE'		, width: 100},
			{dataIndex: 'INSPEC_REMARK'  	, width:150,hidden:false},
		...
		validate: function( type, fieldName, newValue, oldValue, record, eopt) {
			...
			switch(fieldName) {
				case "MAKE_DATE" :
					if(!Ext.isEmpty(record.get('ITEM_CODE'))){
						if(!Ext.isEmpty(newValue)){
							...
							mms510ukrvService.selectExpirationdate(params, function(provider, response) {
				...
				case "ORDER_UNIT_Q" :  //입고수량(구매단위)
					...
					//2021-04-26: 환율이 jpy일 경우 /100처리, 20200609 주석부분을 취소(극동데이터 오류 발생건으로 인해)
...
/*						record.set('INOUT_P',(record.get('INOUT_FOR_P') * record.get('EXCHG_RATE_O')));	//자사단가(재고) = 재고단위단가 * 환율
	...
	/*					if(record.get('INOUT_Q') != 0){
						...
						if(record.get('EXCHG_RATE_O') != 0){
							...
							//20200908 수정: 그리드 금액변경 시, 단가 변경 되지 않도록 수정, 20210409 주석 해제: 여기는 단가 계산 함 조건임!!!!!!!
							record.set('INOUT_P'		, UniMatrl.fnExchangeApply(moneyUnit, (record.get('INOUT_FOR_P') * record.get('EXCHG_RATE_O'))));		//자사단가(재고) = 재고단위단가 * 환율, 20200908 수정: jpy 관련로직 수정
						...
						//20210409 추가: 단가 재계산만 안해야 하나 금액도 변경되지 않음, 금액만 변경되도록 로직 추가
						record.set('INOUT_FOR_O', newValue);
