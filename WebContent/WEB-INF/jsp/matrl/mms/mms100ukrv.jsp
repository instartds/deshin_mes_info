...
<t:appConfig pgmId="mms100ukrv">
	<t:ExtComboStore comboType="BOR120" pgmId="mms100ukrv" />		<!-- 사업장 -->
...
function appMain() {
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 'mms100ukrvService.selectMaster',
			update: 'mms100ukrvService.updateDetail',
			create: 'mms100ukrvService.insertDetail',
			destroy: 'mms100ukrvService.deleteDetail',
			syncAll: 'mms100ukrvService.saveAll'
		}
	});
	...
	Unilite.defineModel('Mms100ukrvModel1', {
		fields: [
	...
	Unilite.defineModel('mms100ukrvORDERModel', {	//발주참조
		fields: [
			{name: 'DIV_CODE'		, text: '<t:message code="system.label.purchase.division" default="사업장"/>'			, type: 'string' ,comboType: 'BOR120'},
			{name: 'CUSTOM_CODE'	, text: '<t:message code="system.label.purchase.customcode" default="거래처코드"/>'		, type: 'string'},
			{name: 'CUSTOM_NAME'	, text: '<t:message code="system.label.purchase.customname" default="거래처명"/>'			, type: 'string'},
			...
			{name: 'UNIT_PRICE_TYPE', text: '단가구분'				, type: 'string'},
			{name: 'ORDER_NUM'		, text: '<t:message code="system.label.purchase.pono" default="발주번호"/>'			, type: 'string'},
			{name: 'ORDER_SEQ'		, text: '<t:message code="system.label.purchase.poseq" default="발주순번"/>'			, type: 'int'},
	...
	Unilite.defineModel('mms100ukrvCOMMERCEModel', {	//무역참조
		fields: [
			{name: 'COMP_CODE'			 , text: '<t:message code="system.label.purchase.companycode" default="법인코드"/>'		, type: 'string'},
			{name: 'CUSTOM_CODE'		 , text: '<t:message code="system.label.purchase.customcode" default="거래처코드"/>'   , type: 'string'},
			{name: 'CUSTOM_NAME'		 , text: '<t:message code="system.label.purchase.customname" default="거래처명"/>'		, type: 'string'},
	...
	Unilite.defineModel('mms100ukrvVMIModel', {	//VMI참조
		fields: [
			{name: 'DIV_CODE'		, text: '<t:message code="system.label.purchase.division" default="사업장"/>'			, type: 'string' ,comboType: 'BOR120'},
			{name: 'CUSTOM_CODE'	, text: '<t:message code="system.label.purchase.customcode" default="거래처코드"/>'		, type: 'string'},
			{name: 'CUSTOM_NAME'	, text: '<t:message code="system.label.purchase.customname" default="거래처명"/>'			, type: 'string'},
	 ...
	 * @type
	 */
	var directMasterStore1 = Unilite.createStore('mms100ukrvMasterStore1', {
		model: 'Mms100ukrvModel1',
		uniOpt: {
		...
		saveStore: function() {
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('mms100ukrvGrid1');
	...
	var receiptNoMasterStore = Unilite.createStore('receiptNoMasterStore', {	//조회버튼 누르면 나오는 조회창
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectreceiptNumMasterList'
			}
		},
		loadStoreRecords: function() {
...
//				}
	...
	var orderStore = Unilite.createStore('mms100ukrvOrderStore', {//발주참조
		model: 'mms100ukrvORDERModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectorderList'
			}
		},
		listeners:{
...
//			}
	...
	var custStore = Unilite.createStore('custStore', {	//거래처참조
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectCustList'
			}
		},
		loadStoreRecords: function() {
	...
	var custStoreDetail = Unilite.createStore('custStoreDetail', {	//거래처참조
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectCustDetailList'
			}
		},
...
//			console.log( param );
	...
	var commerceStore = Unilite.createStore('mms100ukrvCommerceStore', {//무역참조
		model: 'mms100ukrvCOMMERCEModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectcommerceList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
	...
	var vmiStore = Unilite.createStore('mms100ukrvVmiStore', {//vmi참조
		model: 'mms100ukrvVMIModel',
		autoLoad: false,
		uniOpt : {
			isMaster: false,			// 상위 버튼 연결
			editable: false,			// 수정 모드 사용
		...
		proxy: {
			type: 'direct',
			api: {
				read: 'mms100ukrvService.selectvmiList'
			}
		},
		listeners:{
			load:function(store, records, successful, eOpts) {
...
//						field2.getStore().clearFilter(true);
			 ...
			 // holdable: 'hold',
				...
				colspan:2,
				items:[{ text:'<div style="color: red">시험의뢰서 출력</div>',
							xtype: 'button',
							margin: '0 0 0 15',
...
//					field2.getStore().clearFilter(true);
	 ...
	 * @type
	 */
	var masterGrid = Unilite.createGrid('mms100ukrvGrid1', {
		layout: 'fit',
		region: 'center',
		...
		columns: [
			...
			{dataIndex: 'NOR_RECEIPT_Q'			, width: 110, summaryType: 'sum'},
			{dataIndex: 'FREE_RECEIPT_Q'		, width: 110, summaryType: 'sum'},
			...
			beforeedit  : function( editor, e, eOpts ) {
				if(e.record.phantom == false )
				{
					if(e.record.data.RECEIPT_Q == e.record.data.INSPEC_Q) {
						...
						if(e.field=='NOR_RECEIPT_Q') return false;
						if(e.field=='FREE_RECEIPT_Q') return false;
						...
						if(e.field=='RECEIPT_Q') return false;
						if(e.field=='NOR_RECEIPT_Q') return true;
						if(e.field=='FREE_RECEIPT_Q') return true;
...
//					if(e.field=='RECEIPT_Q') return true;
		...
		setOrderData:function(record) { //발주참조SET
			...
			//참조시 단가, 금액이 0이고 단가형태가 진단가일 경우 (무상)접수량에 표시, 그외에 경우 (정상)접수량에 표시
			var orderP = record['ORDER_UNIT_P'];
			var orderO = record['ORDER_O'];
			var priceType = record['UNIT_PRICE_TYPE'];

			grdRecord.set('RECEIPT_Q'			, record['NOT_RECEIPT_Q']);

			if(orderP == 0 && orderO == 0 && priceType == "Y"){
			grdRecord.set('NOR_RECEIPT_Q'		, 0);
			grdRecord.set('FREE_RECEIPT_Q'		, record['NOT_RECEIPT_Q']);
			}else{
			grdRecord.set('FREE_RECEIPT_Q'		, 0);
			grdRecord.set('NOR_RECEIPT_Q'		, record['NOT_RECEIPT_Q']);
			}
		...
		setCommerceData:function(record) {	//무역참조시 set
			...
			//참조시 단가, 금액이 0이고 단가형태가 진단가일 경우 (무상)접수량에 표시, 그외에 경우 (정상)접수량에 표시
			var orderP = record['PRICE'];
			var orderO = record['AMT_O'];
			var priceType = 'Y';
	...
	var receiptNoMasterGrid = Unilite.createGrid('mms100ukrvreceiptNoMasterGrid', {		//조회버튼 누르면 나오는 조회창
		layout : 'fit',
	...
	var orderGrid = Unilite.createGrid('mms100ukrvOrderGrid', {//발주참조
		layout : 'fit',
...
//			{ dataIndex: 'DEPT_NAME'			,width:80 ,align: 'center'},
			...
			{ dataIndex: 'UNIT_PRICE_TYPE'		,width:80,hidden: true},
			{ dataIndex: 'ORDER_NUM'			,width:120},
			{ dataIndex: 'ORDER_SEQ'			,width:80},
			{ dataIndex: 'LOT_NO'				,width:100},
...
//	 		var records = this.getSelectedRecords();
	...
	var commerceGrid = Unilite.createGrid('mms100ukrvCommerceGrid', {//무역참조
		layout: 'fit',
	...
	var vmiGrid = Unilite.createGrid('mms100ukrvVmiGrid', {//vmi참조
		layout : 'fit',
		excelTitle: '<t:message code="system.label.purchase.receiptentry2" default="접수등록"/>(<t:message code="system.label.purchase.porefer" default="발주참조"/>)',
		store: vmiStore,
		uniOpt:{
...
//	 		var records = this.getSelectedRecords();
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
		 			...
		 			show: function ( me, eOpts ) {
						...
						orderSearch.setValue('ORDER_DATE_FR'	, UniDate.get('startOfMonth'));
				...
				listeners : {
					...
					beforeshow: function ( me, eOpts ) {
		 			...
		 			show: function ( me, eOpts ) {
		 				...
		 				vmiSearch.setValue('ORDER_DATE_FR'	, UniDate.get('startOfMonth'));
	...
	var detailForm = Unilite.createForm('mms100ukrvDetail', {
		autoScroll:true,
		layout : 'fit',
...
//
					...
					//첨부파일
					var fp = Ext.getCmp('mms100ukrvFileUploadPanel');
					...
					if(!Ext.isEmpty(reiceptNumSeq)) {
						mms100ukrvService.getFileList({DOC_NO : reiceptNumSeq},
...
//					}
				...
				detailWin = Ext.create('widget.uniDetailWindow', {
					...
					tbar:  ['->',	{   itemId : 'confirmBtn',
											...
											handler: function() {
												...
												mms100ukrvService.insertQMS101(param , function(provider, response){})
											},
											disabled: false
										},
										{   itemId : 'confirmCloseBtn',
											...
											handler: function() {
												...
												if(!Ext.isEmpty(detailSearch.getValue('DEL_FIDS'))){
													if(confirm('문서를 삭제 하시겠습니까?')) {
														...
														mms100ukrvService.deleteQMS101(param , function(provider, response){})
													}
												}else{
													alert('삭제할 문서가 없습니다.');
													return false;
  ...
  //			suffixTpl:'&nbsp;M',
 	...
 	var labelPartitionPrintGrid = Unilite.createGrid('mms100ukrvPartitionPrintGrid', {		//라벨분할그리드
		layout :'fit',
...
//			}
			...
			if(confirm('<t:message code="system.message.purchase.confirm001" default="현재행을 삭제 합니다. 삭제 하시겠습니까?"/>')){
				...
				Ext.each(records, function(record, i) {
					if(record.phantom == true) {
						...
						UniAppManager.app.fnInOutQtyCheck(record, divCode, receiptNum, receiptSeq);
					}
				})
		...
		fnInOutQtyCheck: function(rtnRecord, divCode, receiptNum, receiptSeq) {
			...
			mms100ukrvService.inOutQtyCheck(param, function(provider, response){
...
//					masterGrid.deleteSelectedRow();
					masterGrid.getStore().remove(rtnRecord)
				}
			})
			return isErr;
		},
		...
		fnInspecQtyCheck: function(rtnRecord, fieldName, oldValue, divCode, receiptNum, receiptSeq) {
			...
			mms100ukrvService.inspecQtyCheck(param, function(provider, response) {				//QMS200T의 검사량 가져옴
				if(!Ext.isEmpty(provider) && provider.length > 0 ) {
	...
	Unilite.createValidator('validator01', {
		...
		console.log('validate >>> ', {'type':type, 'fieldName':fieldName, 'newValue':newValue, 'oldValue':oldValue, 'record':record});
			var rv = true;
			switch(fieldName) {
				case "NOR_RECEIPT_Q" ://(정상)접수량, 과입고 가능량 체크
					if(newValue < 0 ){
						rv='<t:message code="system.label.purchase.message064" default="접수량이 0보다 작거나 데이터가 없습니다."/>';
						break;
					}
					if(newValue != "-"){
						var dOrderQ		= record.get('ORDER_UNIT_Q');												//발주량
						var totReceiptQ = newValue + record.get('FREE_RECEIPT_Q');
						var dReceiptQ	= totReceiptQ - Unilite.nvl(record.get('OLD_VAL'), 0);					    //실접수량
						var dNReceiptQ	= record.get('NOT_RECEIPT_Q');												//미접수량
						var dEnableQ	= (dOrderQ + (dOrderQ * Unilite.nvl(BsaCodeInfo.glPerCent, 0) / 100));		//접수가능량
						var dTempQ		= (dOrderQ - dNReceiptQ + dReceiptQ);										//최종실접수량
						...
						var totReceiptQS = record.get('NOR_RECEIPT_Q') + record.get('FREE_RECEIPT_Q');
						record.set('RECEIPT_Q', totReceiptQS);
						break;
						}
				...
				case "FREE_RECEIPT_Q" ://(무상)접수량, 과입고 가능량 체크
					if(newValue < 0 ){
						rv='<t:message code="system.label.purchase.message064" default="접수량이 0보다 작거나 데이터가 없습니다."/>';
						break;
					}
					if(newValue != "-"){
						...
						var dOrderQ		= record.get('ORDER_UNIT_Q');												//발주량
						var totReceiptQ = newValue + record.get('NOR_RECEIPT_Q');                                   //무상접수량 + 정상접수량
						var dReceiptQ	= totReceiptQ - Unilite.nvl(record.get('OLD_VAL'), 0);						//실접수량
						var dNReceiptQ	= record.get('NOT_RECEIPT_Q');												//미접수량
						var dEnableQ	= (dOrderQ + (dOrderQ * Unilite.nvl(BsaCodeInfo.glPerCent, 0) / 100));		//접수가능량
						var dTempQ		= (dOrderQ - dNReceiptQ + dReceiptQ);										//최종실접수량
				...
				case "MAKE_DATE" :
					if(!Ext.isEmpty(record.get('ITEM_CODE'))){
						if(!Ext.isEmpty(newValue)){
							...
							mms100ukrvService.selectExpirationdate(params, function(provider, response) {
