...
function appMain() {
    ...
    */
	...
	Unilite.defineModel('Map301ukrvModel', {
		fields: [
			...
			{name: 'INOUT_FOR_P'  		 , text: '<t:message code="system.label.purchase.receiptprice" default="입고단가"/>'		, type: 'uniUnitPrice'},
			{name: 'INOUT_FOR_O'  		 , text: '입고금액'		, type: 'uniPrice'},
			...
			{name: 'INOUT_P'  			 , text: '입고단가(원화)'	, type: 'uniUnitPrice'},
			{name: 'INOUT_I'  			 , text: '입고금액(원화)'	, type: 'uniPrice'},
			...
			{name: 'APPLY_YN'  			 , text: '조정반영여부'		, type: 'string'},
			{name: 'COMP_CODE'  		 , text: 'COMP_CODE'	, type: 'string'}

		]
	});//End of Unilite.defineModel('Map301ukrvModel', {
   var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read    : 'map301ukrvService.selectList',
			update  : 'map301ukrvService.updateList',
			syncAll : 'map301ukrvService.saveAll'
		}
	});
   /**
    * Store 정의(Service 정의)
    * @type
    */
	var directMasterStore1 = Unilite.createStore('map301ukrvMasterStore1',{
		model: 'Map301ukrvModel',
		uniOpt: {
	...
	//			})
			...
			if(inValidRecs.length == 0) {
	            ...
	            var grid = Ext.getCmp('map301ukrvGrid');
...
//				}
	...
	});//End of var directMasterStore1 = Unilite.createStore('map301ukrvMasterStore1',{
		...
		items: [
			...
			Unilite.popup('AGENT_CUST', {
		    		...
		    		items: [{
						...
						listeners: {
							change: function(field, newValue, oldValue, eOpts) {
								masterForm.setValue('PRICE_DIFF', newValue);
							}
						}
		    		}]
				...
				listeners: {
					change: function(field, newValue, oldValue, eOpts) {
						...
						if(newValue != 'KRW'){
							...
							map301ukrvService.selectTodayExchgRate(param, function(provider, response)	{
								if(!Ext.isEmpty(provider)){
									if(!Ext.isEmpty(provider[0].BASE_EXCHG)){

										panelResult.setValue('EXCHG_RATE_O',provider[0].BASE_EXCHG);
										masterForm.setValue('EXCHG_RATE_O', provider[0].BASE_EXCHG);
							...
							masterForm.setValue('EXCHG_RATE_O', '');
						}
					}
				}
						...
						handler: function() {
							if(masterForm.getValue('EXCHG_RATE_O') == '' || masterForm.getValue('EXCHG_RATE_O')== null ){
								...
								if(!directMasterStore1.fnApplyCheck()){
									if(confirm('<t:message code="unilite.msg.sMM423"/>' + '<t:message code="unilite.msg.sMM424"/>')){
										UniAppManager.app.onExchgRatekApply();
									}
								}else{
									if(confirm('<t:message code="unilite.msg.sMM430"/>' + '<t:message code="unilite.msg.sMM427"/>')){
										UniAppManager.app.onExchgRatekApply();
									}
								}
							}
			...
			},
			{
	    		...
	    		items:[
				{
					...
					listeners: {
							change: function(field, newValue, oldValue, eOpts) {
								if(newValue <= 0){
									alert('<t:message code="unilite.msg.sMB076"/>');
					...
					handler: function() {
						if(masterForm.getValue('ORDER_UNIT_FOR_P') == '' || masterForm.getValue('ORDER_UNIT_FOR_P')== null){
							alert('<t:message code="unilite.msg.sMM422"/>');
							...
							if(!directMasterStore1.fnApplyCheck()){
								...
								if(confirm('<t:message code="unilite.msg.sMM425"/>' + '<t:message code="unilite.msg.sMM426"/>' + '<t:message code="unilite.msg.sMM427"/>')){
									UniAppManager.app.onPriceApply();
								}
							}
			...
			items: [
				...
				Unilite.popup('AGENT_CUST', {
		    		...
		    		items: [{
						...
						listeners: {
							change: function(field, newValue, oldValue, eOpts) {
								panelResult.setValue('PRICE_DIFF', newValue);
							}
						}
		    		}]
		        },{
...
//       	//	this.down('#procTool').menu.down('#saleLinkBtn').setDisabled(b);
     ...
     * @type
     */
	var masterGrid = Unilite.createGrid('map301ukrvGrid1', {
		layout: 'fit',
		region:'center',
		uniOpt:{
			useLiveSearch:true,
		...
		selModel :   Ext.create('Ext.selection.CheckboxModel', { checkOnly : true, toggleOnClick:false,
            listeners: {
                select: function(grid, selectRecord, index, rowIndex, eOpts ) {
                	if(selectRecord.get('ORDER_UNIT_FOR_P') > 0) {

                		selectRecord.set('CHOICE' , 'true');
                ...
                deselect:  function(grid, selectRecord, index, eOpts ) {
                	...
                	selectRecord.set('CHOICE' , 'false');

                },selectionchange:function ( grid, selectRecord, eOpts ){
                	var i = 0;

                	Ext.each(selectRecord, function(selectRecord, i){
            			if(selectRecord.get('ORDER_UNIT_FOR_P') == 0 || selectRecord.get('INOUT_FOR_P') == selectRecord.get('ORDER_UNIT_FOR_P')) {
                    		Ext.getCmp('map301ukrvGrid1').getSelectionModel().deselect(selectRecord,i);
            			}
        			})
		...
		columns: [
			...
			{dataIndex: 'INOUT_FOR_P'  		, width: 70, hidden: false},
			{dataIndex: 'INOUT_FOR_O'  		, width: 70, hidden: false},
			{dataIndex: 'INOUT_P'  			, width: 70, hidden: true},
			{dataIndex: 'INOUT_I'  			, width: 70, hidden: true},
...
//					if(UniUtils.indexOf(e.field, ['INOUT_FOR_P'])){
//						return false;
//					}else{
						return false;
					//}
        	...
        	//masterGrid.getSelectionModel().select(e.rowIdx);
	           ...
	           var fieldName = e.field;
				 if(fieldName == "INOUT_FOR_P"){
					 if (e.originalValue != e.value &&  e.value != 0) {
				    	 e.record.set("CHOICE","true");
				     }
					 var records = directMasterStore1.data.items;
				     data = new Object();
				     data.records = [];
				     Ext.each(records, function(record, i){
				      if(record.get('CHOICE') == "true") {
				      	 data.records.push(record);
				     	}
				     });
				     if( e.value == 0 || e.value == ''){

				    	 e.record.set("INOUT_FOR_P",e.originalValue);
	...
	});//End of var masterGrid = Unilite.createGrid('map301ukrvGrid1', {
				...
				switch(fieldName) {
					case "INOUT_FOR_P" :
						 ...
						 if(newValue != ''){
							...
							record.set('INOUT_FOR_O', record.get('ORDER_UNIT_Q') * newValue);
						}
	...
	Unilite.Main({
		...
		fnInitBinding: function() {
			...
			panelResult.setValue('PRICE_DIFF', '1');
			masterForm.setValue('PRICE_DIFF', '1');
...
//			if(directMasterStore1.getTotalCount() > 0){
		...
		onPriceApply: function(){
			...
			if(i.length < 1){
				alert('<t:message code="unilite.msg.sMM428"/>');
				}
		...
		//			Math.round(record.get('INOUT_FOR_O'),CustomCodeInfo.gsUnderCalBase);
				...
				alert('<t:message code="unilite.msg.sMM429"/>');
			});
		},
		onExchgRatekApply:	function(){
			...
			Ext.each(records,  function(record, index, records){
				if(record.get('EXCHG_RATE_O') != 0){
					...
					record.set('ORDER_UNIT_I', record.get('ORDER_UNIT_Q') * record.get('ORDER_UNIT_P'));
					record.set('ORDER_UNIT_I',UniMatrl.fnExchangeApply(masterForm.getValue('MONEY_UNIT'),record.get('ORDER_UNIT_I')));
					Math.round(record.get('ORDER_UNIT_I'),CustomCodeInfo.gsUnderCalBase);
