...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 'aep110ukrService.selectDetailList',
            update: 'aep110ukrService.updateDetail',
            create: 'aep110ukrService.insertDetail',
            destroy: 'aep110ukrService.deleteDetail',
            syncAll: 'aep110ukrService.saveAll'
        }
    }); 
    
    var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep110ukrService.insertButton',
            syncAll: 'aep110ukrService.saveButtonAll'
        }
    }); 
    var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep110ukrService.insertDetailRequest',
            syncAll: 'aep110ukrService.saveAllRequest'
        }
    }); 

    Unilite.defineModel('aep110ukrSearchModel', {
        fields: [
    ...
    Unilite.defineModel('aep110ukrDetailModel', {
        fields: [
    ...
    var buttonStore = Unilite.createStore('Aep100ukrButtonStore',{      
        ...
        saveStore: function() {             
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep110ukrSearchGrid');
    ...
    var requestStore = Unilite.createStore('Aep110ukrRequestStore',{      
        uniOpt: {
            isMaster: false,            // 상위 버튼 연결 
            editable: false,            // 수정 모드 사용 
            deletable: false,           // 삭제 가능 여부 
...
//          param.FR_INPUT_DATE = UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            config = {
                ...
                success: function(batch, option) {
                    ...
                    requestMsg = requestMsg + '<subject>'+ '경비처리 실물증빙 결재' +'</subject>';
                    requestMsg = requestMsg + '</item>';
    ...
    var directSearchStore = Unilite.createStore('aep110ukrSearchStore', {
        model: 'aep110ukrSearchModel',
        uniOpt: {
            isMaster: false,         // 상위 버튼 연결 
            editable: false,            // 수정 모드 사용 
            deletable: false,           // 삭제 가능 여부 
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 'aep110ukrService.selectSearchList'                 
            }
        },
        listeners: {
            load: function(store, records, successful, eOpts) {
    ...
    var directDetailStore = Unilite.createStore('aep110ukrDetailStore', {
        model: 'aep110ukrDetailModel',
        uniOpt: {
        ...
        saveStore: function() {             
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep110ukrDetailGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
            }
        },
        listeners: {
...
//          tdAttrs: {/*style: 'border : 1px solid #ced9e7;',*/width: '100%'/*,align : 'left'*/}
    ...
    var masterForm = Unilite.createForm('masterForm',{
        region: 'north',
        layout : {type : 'uniTable', columns : 2
//            tableAttrs: { style: 'border : 1px solid #ced9e7;'/*,align:'top'*//*,height: '100%'*/}
//            trAttrs:{align:'top',height: '100%'},
//          tdAttrs: {align:'top'/*style: 'border : 1px solid #ced9e7;',width: '100%'*//*,align : 'left'*/}
        },
        padding:'1 1 1 1',
        disabled:false,
        items: [{
            ...
            items: [{ 
                ...
                Unilite.popup('ADVM_REQ_SLIP_NO',{ 
                    ...
                    listeners: {
                        onSelected: {
                            fn: function(records, type) {
                                masterForm.setValue('ADVM_REQ_EMP_ID', records[0].VENDOR_ID);                                                                                                        
                            },
                            scope: this
                        },
                        onClear: function(type) {
                            masterForm.setValue('ADVM_REQ_EMP_ID', '');  
                        },
                        applyextparam: function(popup){                         
                ...
                listeners: {
                    change: function(field, newValue, oldValue, eOpts) { 
                    	if(loadFlag != 'Y'){
                    		...
                    		masterForm.setValue('TAX_CD','NA');
                        	masterForm.setValue('ATTRIBUTE2','');
                    	}
                        if(newValue.EVDE_TYPE_CD == '50'){
                        	masterForm.down('#attribute2').setDisabled(false);
                            masterForm.down('#conVendorNm').setDisabled(false);
                            masterForm.down('#conVendorSiteCd').setDisabled(false);
                            masterForm.down('#cardNo').setDisabled(true);
                            masterForm.getField("ATTRIBUTE2").setConfig('allowBlank',true);
                            masterForm.getField("GLOBAL_ATTRIBUTE7").setConfig('allowBlank',false);
                            masterForm.getField("GLOBAL_ATTRIBUTE8").setConfig('allowBlank',false);
                            masterForm.getField("GLOBAL_ATTRIBUTE9").setConfig('allowBlank',true);
                        }else if(newValue.EVDE_TYPE_CD == '60'){
                            masterForm.down('#attribute2').setDisabled(true);
                            masterForm.down('#conVendorNm').setDisabled(true);
                            masterForm.down('#conVendorSiteCd').setDisabled(true);
                            masterForm.down('#cardNo').setDisabled(true);
                            masterForm.getField("GLOBAL_ATTRIBUTE9").setConfig('allowBlank',true);
                            masterForm.getField("ATTRIBUTE2").setConfig('allowBlank',true);
                            masterForm.getField("GLOBAL_ATTRIBUTE7").setConfig('allowBlank',true);
                            masterForm.getField("GLOBAL_ATTRIBUTE8").setConfig('allowBlank',true);
                        }else if(newValue.EVDE_TYPE_CD == '90'){
                        	masterForm.down('#attribute2').setDisabled(false);
                            masterForm.down('#conVendorNm').setDisabled(false);
                            masterForm.down('#conVendorSiteCd').setDisabled(false);
                            masterForm.down('#cardNo').setDisabled(false);
                            masterForm.getField("GLOBAL_ATTRIBUTE9").setConfig('allowBlank',false);
                            masterForm.getField("ATTRIBUTE2").setConfig('allowBlank',true);
                            masterForm.getField("GLOBAL_ATTRIBUTE7").setConfig('allowBlank',false);
                            masterForm.getField("GLOBAL_ATTRIBUTE8").setConfig('allowBlank',false);
                        }
            ...
            }),
            {
                ...
                listeners:{
                	beforequery:function( queryPlan, eOpts )    {
                        var store = queryPlan.combo.store;
                            store.clearFilter();
                        if(masterForm.down('#evdeTypeCd').getValue().EVDE_TYPE_CD == '50'){
                            store.filterBy(function(record){
                                return record.get('refCode1') == '50' || record.get('refCode1') == '60';
                            })
                        }else if(masterForm.down('#evdeTypeCd').getValue().EVDE_TYPE_CD == '60'){
                            store.filterBy(function(record){
                                return record.get('refCode1') == '60';
                            })
                        }else if(masterForm.down('#evdeTypeCd').getValue().EVDE_TYPE_CD == '90'){
                            store.filterBy(function(record){
                                return record.get('refCode1') == '90' || record.get('refCode1') == '60';
                            })
                        }
                    },
                    ...
                    change: function(field, newValue, oldValue, eOpts) {
                    	if(!Ext.isEmpty(masterForm.getValue('TAX_CD'))){
                    		if(!Ext.isEmpty(masterForm.getField("TAX_CD").selection)){
                                if(masterForm.getField("TAX_CD").selection.data.option == 'Y' ){
                                    ...
                                    masterForm.down('#attribute2').setReadOnly(false);
                                    masterForm.getField("ATTRIBUTE2").setConfig('allowBlank',false);
                                }else{
                                	masterForm.setValue('ATTRIBUTE2','');
                                    masterForm.down('#attribute2').setReadOnly(true);
                                    masterForm.getField("ATTRIBUTE2").setConfig('allowBlank',true);
                                }
                    		}else{
                    			masterForm.setValue('ATTRIBUTE2','');
...
//                        }
        ...
        api: {
          load: 'aep110ukrService.selectMasterForm'  
        }
    });
                ...
                listeners: {
                    onSelected: {
                        fn: function(records, type) {
                            ...
                            aep110ukrService.selectAccntYnData(param, function(provider, response)   {
                                if(!Ext.isEmpty(provider)){   
...
//        ,
//        api: {
//            load: 'aep110ukrService.selectForm' ,
//            submit: 'aep110ukrService.syncMaster'   
//        }
    });
    ...
    var fileUploadForm = Unilite.createSimpleForm('fileUploadForm',{
        ...
        api: {
             load	: 'aep110ukrService.getFileList',   //조회 api
             submit	: 'aep110ukrService.saveFile'       //저장 api
        }
    });  
        ...
        bbar: ['->',{
            ...
            handler: function() {
                ...
                if(searchRecords.length > 0){
                    ...
                    if(selectTrueFlag == 'Y'){
                        if(confirm('선택한 실물증빙내역을 결재요청 처리하시겠습니까?')) { 
...
//                            alert('저장로직');
            ...
            selectionchangerecord:function(selected)    {
            	masterForm.setValue('ATTRIBUTE2','');
            	masterForm.setValue('GLOBAL_ATTRIBUTE7','');
            	masterForm.setValue('GLOBAL_ATTRIBUTE8','');
            	masterForm.setValue('GLOBAL_ATTRIBUTE9','');
...
//                masterForm.setValue('ELEC_SLIP_NO',selected.data.ELEC_SLIP_NO);
    ...
    var detailGrid = Unilite.createGrid('aep110ukrDetailGrid', {
        height:300,
        width:1135,
        region: 'south',
        excelTitle: '실물증빙경비내역',
                                ...
                                if(!Ext.isEmpty(detailForm.getValue('AMTRM'))){
                                    if(detailForm.getValue('AMTRM') < detailForm.getValue('ACCT_AMT') + drAmtISum){
                                        ...
                                        if(!Ext.isEmpty(masterForm.getValue('TAX_CD'))){
                                        	if(!Ext.isEmpty(masterForm.getField("TAX_CD").selection)){
                                                if(masterForm.getField("TAX_CD").selection.data.option == 'Y' ){
                                                    if(masterForm.getValue('INVOICE_AMT') - masterForm.getValue('ATTRIBUTE2') < detailForm.getValue('ACCT_AMT') + drAmtISum){
...
//            UniAppManager.app.fnInitInputFields(params);  
...
//          detailForm.clearForm();
            ...
            if(BsaCodeInfo.paySysGubun == '1'){
                masterForm.getForm().getFields().each(function(field) {
                    ...
                    if (field.name == 'ELEC_SLIP_NO' || 
                        field.name == 'VENDOR_SITE_CD' || 
                        field.name == 'ADVM_REQ_EMP_ID' || 
...
//            masterForm.setValue('TAX_CD','');
        ...
        fnInitInputFields: function(){
            ...
            searchForm.setValue('ELEC_SLIP_TYPE_CD','A030');
...
//            Ext.getCmp('conVendor').setHidden(true);
...
//            detailForm.setValue('CONF_DRAFTER',UserInfo.personNumb);
        ...
        fileUploadLoad: function(){
            ...
            aep110ukrService.getFileList({DOC_NO : fileNO},              //파일조회 메서드  호출(param - 파일번호) 
...
//                    ,,,
        ...
        fnAcctAmtChange1: function(){   //경비처리 금액 계산 관련      
            ...
            if(!Ext.isEmpty(masterForm.getValue('TAX_CD'))){
            	if(!Ext.isEmpty(masterForm.getField("TAX_CD").selection)){
                    if(masterForm.getField("TAX_CD").selection.data.option == 'Y' ){
                        if(masterForm.getValue('INVOICE_AMT') - masterForm.getValue('ATTRIBUTE2') - drAmtISum == 0){
        ...
        fnSelectAccntData: function(param){
            aep110ukrService.selectAccntData(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
        ...
        fnPossibleBudgAmt110: function(param){
            aep110ukrService.spAccntGetPossibleBudgAmt110(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
