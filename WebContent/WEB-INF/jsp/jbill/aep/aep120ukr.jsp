...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 'aep120ukrService.selectDetailList',
            update: 'aep120ukrService.updateDetail',
            create: 'aep120ukrService.insertDetail',
            destroy: 'aep120ukrService.deleteDetail',
            syncAll: 'aep120ukrService.saveAll'
        }
    }); 
    
    var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep120ukrService.insertButton',
            syncAll: 'aep120ukrService.saveButtonAll'
        }
    }); 
    
    var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep120ukrService.insertDetailRequest',
            syncAll: 'aep120ukrService.saveAllRequest'
        }
    }); 
    
    Unilite.defineModel('aep120ukrSearchModel', {
        fields: [
    ...
    Unilite.defineModel('aep120ukrDetailModel', {
        fields: [
...
//            {name: 'AQUI_SERV'       ,text: '봉사료'           ,type: 'uniPrice'},
    ...
    var buttonStore = Unilite.createStore('Aep120ukrButtonStore',{      
        uniOpt: {
...
//          param.FR_INPUT_DATE = UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep120ukrSearchGrid');
    ...
    var requestStore = Unilite.createStore('Aep120ukrRequestStore',{      
        uniOpt: {
            isMaster: false,            // 상위 버튼 연결 
            editable: false,            // 수정 모드 사용 
            deletable: false,           // 삭제 가능 여부 
...
//          param.FR_INPUT_DATE = UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            config = {
                ...
                success: function(batch, option) {
                    ...
                    requestMsg = requestMsg + '<subject>'+ '경비처리 세금계산서(계산서) 결재' +'</subject>';
                    requestMsg = requestMsg + '</item>';
...
//                    }
    ...
    var directSearchStore = Unilite.createStore('aep120ukrSearchStore', {
        model: 'aep120ukrSearchModel',
        uniOpt: {
            isMaster: false,         // 상위 버튼 연결 
            editable: false,            // 수정 모드 사용 
            deletable: false,           // 삭제 가능 여부 
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 'aep120ukrService.selectSearchList'                 
            }
        },
    ...
    var directDetailStore = Unilite.createStore('aep120ukrDetailStore', {
        model: 'aep120ukrDetailModel',
        uniOpt: {
...
//            paramMaster.ATTRIBUTE2           = masterForm.getValue('ATTRIBUTE2'); 
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep120ukrDetailGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
            }
        },
...
//                    tdAttrs: {style: 'border : 1px solid #ced9e7;'}
            ...
            items: [{
                ...
                items:[{
                    ...
                    items: [{
                        ...
                        layout: {type: 'uniTable' , columns: 2},
                        items: [
                            Unilite.popup('CUST',{   //custPopup
                                fieldLabel: '거래처',
                                textFieldName:'FR_VENDOR_ID',
                                ...
                                listeners: {
                                    onSelected: {
                                        fn: function(records, type) {
                                            masterForm.setValue('FR_VENDOR_SITE_CD', records[0].COMPANY_NUM);
                                            masterForm.setValue('FR_VENDOR_NM', records[0].CUSTOM_NAME);
                                            masterForm.setValue('FR_VENDOR_CEO_NAME', records[0].TOP_NAME);
                                            masterForm.setValue('FR_VENDOR_ADDRESS', records[0].ADDR1);
                                            masterForm.setValue('FR_VENDOR_BUSINESS_TYPE', records[0].COMP_TYPE);
                                            masterForm.setValue('FR_VENDOR_BUSINESS_CONDITION', records[0].COMP_CLASS);
                                            
                                        },
                                        scope: this
                                    },
                                    onClear: function(type) {
                                        masterForm.setValue('FR_VENDOR_SITE_CD', '');
                                        masterForm.setValue('FR_VENDOR_NM', '');
                                        masterForm.setValue('FR_VENDOR_CEO_NAME', '');
                                        masterForm.setValue('FR_VENDOR_ADDRESS', '');
                                        masterForm.setValue('FR_VENDOR_BUSINESS_CONDITION', '');
                                        masterForm.setValue('FR_VENDOR_BUSINESS_TYPE', '');
                                    }            
                                }
                            }),
                        {
    ...
    //                        id:'globalAttribute2',
                            name:'TO_VENDOR_ID', 
                            store: BsaCodeInfo.hiddenCheck_4 == 'Y' ? Ext.data.StoreManager.lookup('divCodeList') : Ext.data.StoreManager.lookup('indusCodeList'),
                            allowBlank:false,
                            readOnly:false,
                            listeners:{
                                change: function(field, newValue, oldValue, eOpts) { 
                                var param = {
                                        "TO_VENDOR_ID": newValue
                                    };
                                    
                                    aep120ukrService.billDivCodeInfo(param, function(provider, response)   {
                                        if(!Ext.isEmpty(provider)){
                                            masterForm.setValue('TO_VENDOR_SITE_CD', provider[0].COMPANY_NUM);
                                            masterForm.setValue('TO_VENDOR_NM', provider[0].DIV_NAME);
                                            masterForm.setValue('TO_VENDOR_CEO_NAME', provider[0].REPRE_NAME);
                                            masterForm.setValue('TO_VENDOR_ADDRESS', provider[0].ADDR);
                                            masterForm.setValue('TO_VENDOR_BUSINESS_CONDITION', provider[0].COMP_CLASS);
                                            masterForm.setValue('TO_VENDOR_BUSINESS_TYPE', provider[0].COMP_TYPE);
                                        }
                                    });
...
//                        width:122,
    ...
    //                        labelWidth:150,
        ...
        //                      }
                                ...
                                //빈값에서 포커스 이동 할때 콤보 전 값 다시 살아나는 증상 확인필요
                                // allowBlank false true 등 
...
//			                }
        ...
        api: {
          load: 'aep120ukrService.selectMasterForm'  
        }
    });
            ...
            listeners: {
                onSelected: {
                    fn: function(records, type) {
                        ...
                        aep120ukrService.selectAccntYnData(param, function(provider, response)   {
                            if(!Ext.isEmpty(provider)){   
...
//        ,
//        api: {
//            load: 'aep120ukrService.selectForm' ,
//            submit: 'aep120ukrService.syncMaster'   
//        }
    });
    var fileUploadForm = Unilite.createSimpleForm('fileUploadForm',{
        ...
        api: {
             load	: 'aep120ukrService.getFileList',   //조회 api
             submit	: 'aep120ukrService.saveFile'       //저장 api
        }
    });  
...
//                padding:'1 1 1 1',
    ...
    var detailGrid = Unilite.createGrid('aep120ukrDetailGrid', {
//      split:true,
//    	flex:0.5,
//        layout: 'fit',
    	height:300,
...
//          detailForm.clearForm();
            ...
            if(BsaCodeInfo.paySysGubun == '1'){
                masterForm.getForm().getFields().each(function(field) {
                    ...
                    if (field.name == 'FR_VENDOR_ID' || //FR_VENDOR_NM
...
//            masterForm.setValue('GLOBAL_ATTRIBUTE2',masterForm.getField('GLOBAL_ATTRIBUTE2').getStore().getAt(0).get('value'));
        ...
        fnInitInputFields: function(){
            ...
            searchForm.setValue('ELEC_SLIP_TYPE_CD','A020');
...
//            detailForm.setValue('CONF_DRAFTER',UserInfo.personNumb);
        ...
        fileUploadLoad: function(){
            ...
            aep120ukrService.getFileList({DOC_NO : fileNO},              //파일조회 메서드  호출(param - 파일번호) 
...
//                    ,,,
        ...
        fnSelectAccntData: function(param){
            aep120ukrService.selectAccntData(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
        ...
        fnPossibleBudgAmt110: function(param){
            aep120ukrService.spAccntGetPossibleBudgAmt110(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
        ...
        fnSetCardRestLmt: function(param){
            aep120ukrService.setCardRestLmt(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
