...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 'aep160ukrService.selectDetailList',
            update: 'aep160ukrService.updateDetail',
            create: 'aep160ukrService.insertDetail',
            destroy: 'aep160ukrService.deleteDetail',
            syncAll: 'aep160ukrService.saveAll'
        }
    }); 
    
    var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep160ukrService.insertButton',
            syncAll: 'aep160ukrService.saveButtonAll'
        }
    }); 
    
    var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep160ukrService.insertDetailRequest',
            syncAll: 'aep160ukrService.saveAllRequest'
        }
    }); 
    
    Unilite.defineModel('aep160ukrSearchModel', {
        fields: [
            ...
            {name: 'SLIP_DESC'            ,text: '원고내역'        ,type: 'string'},
    ...
    Unilite.defineModel('aep160ukrDetailModel', {
        fields: [
            ...
            {name: 'ITEM_DESC_USE'   ,text: '원고내역'        ,type: 'string',allowBlank:false},
            ...
            {name: 'ITEM_CODE'       ,text: '매체코드'          ,type: 'string'},
//            {name: 'ITEM_NAME'       ,text: '제품명'           ,type: 'string'},     
    ...
    var buttonStore = Unilite.createStore('Aep160ukrButtonStore',{      
        uniOpt: {
...
//          param.FR_INPUT_DATE = UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep160ukrSearchGrid');
    ...
    var requestStore = Unilite.createStore('Aep160ukrRequestStore',{      
        uniOpt: {
            isMaster: false,            // 상위 버튼 연결
            editable: false,            // 수정 모드 사용
            deletable: false,           // 삭제 가능 여부
...
// UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            config = {
                ...
                success: function(batch, option) {
                    ...
                    requestMsg = requestMsg + '<subject>'+ '경비처리 원고료 결재' +'</subject>';
                    requestMsg = requestMsg + '</item>';
...
//                    }
    ...
    var directSearchStore = Unilite.createStore('aep160ukrSearchStore', {
        model: 'aep160ukrSearchModel',
        uniOpt: {
            isMaster: false,         // 상위 버튼 연결
            editable: false,            // 수정 모드 사용
            deletable: false,           // 삭제 가능 여부
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 'aep160ukrService.selectSearchList'                 
            }
        },
    ...
    var directDetailStore = Unilite.createStore('aep160ukrDetailStore', {
        model: 'aep160ukrDetailModel',
        uniOpt: {
...
//            paramMaster.ATTRIBUTE2           = masterForm.getValue('ATTRIBUTE2'); 
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep160ukrDetailGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
            }
        },
...
//                store: Ext.data.StoreManager.lookup('businessAreaList'),
    ...
    // }
        ...
        api: {
            load: 'aep160ukrService.selectMasterForm'  
        }
    });   
                ...
                listeners: {
                    onSelected: {
                        fn: function(records, type) {
                            ...
                            aep160ukrService.selectAccntYnData(param, function(provider, response)   {
                                if(!Ext.isEmpty(provider)){   
...
//                                    }
                    ...
                    applyextparam: function(popup){
                        popup.setExtParam({'ADD_QUERY': "A.SPEC_DIVI = 'Q2' AND A.SLIP_SW = 'Y' AND A.GROUP_YN = 'N'"});					//원고료
                    },
...
// allowBlank:false,
    // colspan:2
        ...
        // popup.setExtParam({'CHARGE_CODE': getChargeCode[0].SUB_CODE});
                ...
                xtype:'component',
                fieldLabel:' ',
                html:"'원고내역' 란에는 해당 예산을 사용한 목적/방법 등을 순차적으로 기입합니다.",
...
// ,
// api: {
// load: 'aep160ukrService.selectForm' ,
// submit: 'aep160ukrService.syncMaster'
// }
    });
...
// split:true,
        ...
        api: {
             load: 'aep160ukrService.getFileList',   // 조회 api
             submit: 'aep160ukrService.saveFile'      // 저장 api
        }
    });  
    
    
    
    var searchGrid = Unilite.createGrid('aep160ukrSearchGrid', {
        region: 'center',
        excelTitle: '원천세내역',
        height:250,
        uniOpt: {
...
// masterForm.setValue('TAX_CD','');
    ...
    var detailGrid = Unilite.createGrid('aep160ukrDetailGrid', {
        height:300,
        width:1135,
        region: 'south',
        excelTitle: '원천세경비내역',
        ...
        dockedItems: [{         
            ...
            items: [{
                ...
                handler : function() { 
                    ...
                    aep160ukrService.selectBudgctlYn(param, function(provider, response)   {
                        if(!Ext.isEmpty(provider)){
...
// 			detailForm.clearForm();
            ...
            SAVE_FLAG = '';
//            if(BsaCodeInfo.paySysGubun == '1'){    다시확인필요
//                masterForm.getForm().getFields().each(function(field) {
...
// detailForm.setValue('CONF_DRAFTER',UserInfo.personNumb);
       ...
       fnAcctAmtChange1: function(){   // 경비처리 금액 계산 관련
        ...
        fnSelectAccntData: function(param){
            aep160ukrService.selectAccntData(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
...
//                            }
        ...
        fnPossibleBudgAmt110: function(param){
            aep160ukrService.spAccntGetPossibleBudgAmt110(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
...
//                ITEM_NAME:      itemName,
        ...
        fnTaxCalc: function(){   // 소득세 , 지방세 , 실지급액 계산 관련  (원고매수,원고단가,원고료, 비과세, 개인구입 포함 )//개발 중 .....
...
//          var startPage  = 0;   //START_PAGE  시작 PG 
//          var totalPage  = 0;   //TOTAL_PAGE  원고매수  
//          var unitPrice  = 0;   //UNIT_PRICE  원고단가  
            var payManu   = 0;   //PAY_MANU   원고료   
//          var tssTaxFree = 0;   //TSS_TAX_FREE  비과세 
            ...
            payManu = masterForm.getValue('TOTAL_PAGE') * masterForm.getValue('UNIT_PRICE');
