...
function appMain() {
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 'aep170ukrService.selectDetailList',
            update: 'aep170ukrService.updateDetail',
            create: 'aep170ukrService.insertDetail',
            destroy: 'aep170ukrService.deleteDetail',
            syncAll: 'aep170ukrService.saveAll'
        }
    }); 
    
    var directButtonProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep170ukrService.insertButton',
            syncAll: 'aep170ukrService.saveButtonAll'
        }
    }); 
    
    var directProxyRequest = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            create: 'aep170ukrService.insertDetailRequest',
            syncAll: 'aep170ukrService.saveAllRequest'
        }
    }); 
    
    Unilite.defineModel('aep170ukrSearchModel', {
        fields: [
            ...
            {name: 'SLIP_DESC'            ,text: '인세내역'        ,type: 'string'},
    ...
    Unilite.defineModel('aep170ukrDetailModel', {
        fields: [
            ...
            {name: 'ITEM_DESC_USE'   ,text: '인세내역'        ,type: 'string',allowBlank:false},
...
//            {name: 'ITEM_NAME'       ,text: '제품명'           ,type: 'string'},     
    ...
    var buttonStore = Unilite.createStore('aep170ukrButtonStore',{      
        uniOpt: {
...
//          param.FR_INPUT_DATE = UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep170ukrSearchGrid');
    ...
    var requestStore = Unilite.createStore('aep170ukrRequestStore',{      
        uniOpt: {
            isMaster: false,            // 상위 버튼 연결
            editable: false,            // 수정 모드 사용
            deletable: false,           // 삭제 가능 여부
...
// UniDate.getDbDateStr(panelSearch.getValue('FR_INPUT_DATE'));
            config = {
                ...
                success: function(batch, option) {
                    ...
                    requestMsg = requestMsg + '<subject>'+ '경비처리 인세 결재' +'</subject>';
                    requestMsg = requestMsg + '</item>';
...
//                    requestMsg = requestMsg + '<p><a target="_blank" href="http://localhost:8080/accnt/test/extjs/req999skrTestRequest.jsp?testValue ='+aaa+' title="상세리스트보기">' + "상세리스트" + '</a></p>';
                    requestMsg = requestMsg + '</td>';
                    requestMsg = requestMsg + '</tr>';
...
//                    }
    ...
    var directSearchStore = Unilite.createStore('aep170ukrSearchStore', {
        model: 'aep170ukrSearchModel',
        uniOpt: {
            isMaster: false,         // 상위 버튼 연결
            editable: false,            // 수정 모드 사용
            deletable: false,           // 삭제 가능 여부
        ...
        proxy: {
            type: 'direct',
            api: {
                read: 'aep170ukrService.selectSearchList'                 
            }
        },
    ...
    var directDetailStore = Unilite.createStore('aep170ukrDetailStore', {
        model: 'aep170ukrDetailModel',
        uniOpt: {
...
//            paramMaster.ATTRIBUTE2           = masterForm.getValue('ATTRIBUTE2'); 
            ...
            if(inValidRecs.length == 0) {
                ...
                var grid = Ext.getCmp('aep170ukrDetailGrid');
                grid.uniSelectInvalidColumnAndAlert(inValidRecs);
            }
        },
...
//                        masterForm.setValue('CREDIT_ACCT_AMT','');
                        ...
                        UniAppManager.app.fnTaxCalc('INCOME_TAX_ATTRIBUTE13');
                  		UniAppManager.app.fnAcctAmtChange1();
		  		...
		  		listeners:{
		  			beforequery:function(queryPlan, value)	{
						var store = queryPlan.combo.getStore();
	  					...
	  					this.store.filterBy(function(record){return record.get('refCode1') == 'A'},this)
		  			}
		  		}
            },{
                xtype	: 'container',
...
//                labelWidth	: 110,
                listeners	: {
                    change: function(field, newValue, oldValue, eOpts) {
                    	UniAppManager.app.fnTaxCalc('UNIT_PRICE', newValue);
                    	UniAppManager.app.fnAcctAmtChange1();
                    }
                }
            },{
...
//                labelWidth	: 110,
                ...
                listeners	: {
                    change: function(field, newValue, oldValue, eOpts) {
//                    	UniAppManager.app.fnTaxCalc('PAY_MANU', newValue);
//                    	UniAppManager.app.fnAcctAmtChange1();
                    }
                }
            },{
                ...
                listeners	: {
                    change: function(field, newValue, oldValue, eOpts) {
                    	UniAppManager.app.fnTaxCalc('TSS_TAX_FREE', newValue);
                    	UniAppManager.app.fnAcctAmtChange1();
                    }
                }
            },{
                ...
                listeners	: {
                    change: function(field, newValue, oldValue, eOpts) {
                    	UniAppManager.app.fnTaxCalc('TSS_IND_BUY', newValue);
                    	UniAppManager.app.fnAcctAmtChange1();
                    }
                }
            },{
...
// 				setVisible(false)
    ...
    // }
        ...
        api: {
            load: 'aep170ukrService.selectMasterForm'  
        }
    });   
                ...
                listeners		: {
                    onSelected: {
                        fn: function(records, type) {
                            ...
                            aep170ukrService.selectAccntYnData(param, function(provider, response)   {
                                if(!Ext.isEmpty(provider)){   
...
//                                    }
                    ...
                    applyextparam: function(popup){
                        popup.setExtParam({'ADD_QUERY': "A.SPEC_DIVI = 'Q3' AND A.SLIP_SW = 'Y' AND A.GROUP_YN = 'N'"});					//인세
...
//                                        "C.KOSTL_CODE = '' AND C.KOSTL_TYPE IN ('ALL', 'C')" :  "C.KOSTL_CODE = " + detailForm.getValue('COST_DEPT_CD') + " AND C.KOSTL_TYPE IN ('ALL', 'C')"
//                        });
...
// allowBlank:false,
    // colspan:2
        ...
        // popup.setExtParam({'CHARGE_CODE': getChargeCode[0].SUB_CODE});
                ...
                xtype		: 'component',
                fieldLabel	: ' ',
                html		: "'인세내역' 란에는 해당 예산을 사용한 목적/방법 등을 순차적으로 기입합니다.",
...
// ,
// api: {
// load: 'aep170ukrService.selectForm' ,
// submit: 'aep170ukrService.syncMaster'
// }
    });
...
// split:true,
        ...
        api: {
             load	: 'aep170ukrService.getFileList',   // 조회 api
             submit	: 'aep170ukrService.saveFile'      // 저장 api
        }
    });  
    
    
    
    var searchGrid = Unilite.createGrid('aep170ukrSearchGrid', {
// split:true,
// layout: 'fit',
        region: 'center',
        excelTitle: '원천세내역',
...
// masterForm.setValue('TAX_CD','');
    ...
    var detailGrid = Unilite.createGrid('aep170ukrDetailGrid', {
        excelTitle	: '인세경비내역',
        region		: 'south',
        height		: 300,
        width		: 1135,
        ...
        dockedItems: [{         
            ...
            items: [{
                ...
                handler : function() { 
                    ...
                    aep170ukrService.selectBudgctlYn(param, function(provider, response)   {
                        if(!Ext.isEmpty(provider)){
...
//                detailForm.setValue('ITEM_NAME'     ,record.get('ITEM_NAME'));  
    ...
    Unilite.Main( {
        ...
        /*
			 * masterForm.mask('loading...'); var param= masterForm.getValues();
			 * 
			 * masterForm.getForm().load({ params: param, success:
			 * function(form, action) { directDetailStore.loadStoreRecords();
			 * masterForm.unmask(); }, failure: function(form, action) {
			 * masterForm.unmask(); } });
			 */
        /*
			 * var param= detailForm.getValues();
			 * 
			 * detailForm.getForm().load({ params: param, success:
			 ...
			 * 
			 * 
			 * UniAppManager.app.fileUploadLoad(); }, failure: function(form,
			 * action) { // detailForm.unmask(); // subForm1.unmask(); } });
			 * directDetailStore.loadStoreRecords();
			 * 
			 * UniAppManager.setToolbarButtons('reset',true);
			 */
...
// detailForm.setValue('CONF_DRAFTER',UserInfo.personNumb);
        ...
        fileUploadLoad: function(){
            ...
            aep160ukrService.getFileList({DOC_NO : fileNO},              //파일조회 메서드  호출(param - 파일번호) 
        ...
        fnSelectAccntData: function(param){
            aep170ukrService.selectAccntData(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
...
//                            }
        ...
        fnPossibleBudgAmt110: function(param){
            aep170ukrService.spAccntGetPossibleBudgAmt110(param, function(provider, response)   {
                if(!Ext.isEmpty(provider)){
...
// 			var aquiServ = detailForm.getValue('AQUI_SERV');
            ...
            var itemCode    = masterForm.getValue('ITEM_CODE');
...
//                ITEM_NAME:      itemName,
        ...
        fnTaxCalc: function(field, newValue){   // 소득세 , 지방세 , 실지급액 계산 관련  (지급부수,인세단가,인세, 비과세, 개인구입 포함 )//개발 중 .....
...
//          var startPage  = 0;   //START_PAGE   정가
//          var totalPage  = 0;   //TOTAL_PAGE   지급부수  
         	var unitPrice  = 0;   //UNIT_PRICE   인세단가  
            var payManu   = 0;   //PAY_MANU    인세   
//            var taxRate	   = 0;	  //PAY_MANU_RATE     인세율
//          var tssTaxFree = 0;   //TSS_TAX_FREE  비과세 
            var tssIndBuy  = 0;   //TSS_IND_BUY  개인구입 
	        ...
	        if (!(field == 'INCOME_TAX_ATTRIBUTE13' || field == 'TSS_TAX_FREE' || field == 'TSS_IND_BUY')) {  //원천세유형, 비과세, 개인구입 필드에서 함수를 호출했을 때는 상위 필드에 영향이 없어야 한다.
	            if (field == 'UNIT_PRICE') {
	            	unitPrice = newValue;
	            	
	            } else {
	            	unitPrice = masterForm.getValue('START_PAGE') * masterForm.getValue('PAY_MANU_RATE') / 100;
	            }
	            ...
	            if (field == 'PAY_MANU') {
		            ...
		            payManu = masterForm.getValue('TOTAL_PAGE') * unitPrice;
	            }
	            masterForm.setValue('UNIT_PRICE', unitPrice);
	            masterForm.setValue('PAY_MANU', payManu);

            } else {
		        payManu = masterForm.getValue('PAY_MANU');
	        }
            
            if (field == 'TSS_TAX_FREE') {				//비과세 필드에서 함수를 호출했을 때,
            	invoiceAmt = payManu + newValue
            	
            } else {
            	invoiceAmt = payManu + masterForm.getValue('TSS_TAX_FREE');
            }
            
            masterForm.setValue('INVOICE_AMT',invoiceAmt);
//            invoiceAmt = masterForm.getValue('INVOICE_AMT');
            
            if (field == 'TSS_IND_BUY') {				//개인구입 필드에서 함수를 호출했을 때,
            	tssIndBuy = newValue
            	
            } else {
            	tssIndBuy = masterForm.getValue('TSS_IND_BUY');
            }
	        ...
	        if (masterForm.getValue('INCOME_TAX_ATTRIBUTE13') == '03_K1') {		//원천세 유형이 "일용직근로소득 (일당10만원 초과 일 경우)" - (일당-100,0000 * 세율)
	            ...
	            if(incomeTaxRefCode6 == 'P1'){
	                tempCalc = payManu - incomeTaxRefCode3;
	                
	            } else if(incomeTaxRefCode6 == 'P2'){
	                tempCalc = payManu - payManu * 0.8;
            ...
            creditAcctAmt = invoiceAmt - tssIndBuy- incomeTaxAcctAmt - rsdTaxAcctAmt;
