...
<t:appConfig pgmId="s_cdr400skrv_in"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_cdr400skrv_in" /> 				<!-- 사업장 -->
	<t:ExtComboStore items="${COMBO_WH_LIST}" storeId="whList" /><!--창고-->
	<t:ExtComboStore items="${COMBO_ITEM_LEVEL1}" storeId="itemLeve1Store" />
	<t:ExtComboStore items="${COMBO_ITEM_LEVEL2}" storeId="itemLeve2Store" />
	<t:ExtComboStore items="${COMBO_ITEM_LEVEL3}" storeId="itemLeve3Store" />	
	<t:ExtComboStore comboType="AU" comboCode="CA11" />	
</t:appConfig>
<style type="text/css">
#search_panel1 .x-panel-header-text-container-default {color: #333333;font-weight: normal;padding: 1px 2px;}

.x-grid-cell-text-highlight {color: #ff0000;}
.x-grid-cell-background-highlight {background-color: #ffffc8;}
</style>
<script type="text/javascript" >

function appMain() {
   var showFinanceDefalt = '${showFinanceDefalt}' == 'Y' ;
   var s_cdr400skrv_inModel=Unilite.defineModel('s_cdr400skrv_inModel', {
   	fields: [
	    	...
	    	{name: 'InProdtReplace_q'		    , text: '수량'      		    , type: 'uniQty'},
	    	{name: 'InProdtReplace_p'	    	, text: '단가'    			, type: 'uniPrice'},
	    	{name: 'InProdtReplace_i'	    	, text: '금액'    			, type: 'uniPrice'},
	    	...
	    	{name: 'OutProdtReplace_q'	    	, text: '수량'    			, type: 'uniQty'},
	    	{name: 'OutProdtReplace_i'	    	, text: '금액'    			, type: 'uniPrice'},
   ...
   var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
        api: {
            read: 's_cdr400skrv_inService.selectList'
        }
    });
   var s_cdr400skrv_inStore=Unilite.createStore('Store', {	
   		model: 's_cdr400skrv_inModel',
		autoLoad: false,
		uniOpt: {
		...
		listeners:{
			load: function(store, records, successful, eOpts){
				if(successful){
					...
					if(panelResult.getValues().STATUS=='2' || !panelResult.getField("SHOW_OUT_FINACE").checked ){
						masterGrid.normalGrid.getHeaderContainer().down('#OurFinance_col_header').hide(); 
		                masterGrid.getColumn("OutFinance_q").hide(); //재무수량
		                masterGrid.getColumn("OutFinance_i").hide(); //재무금액
		            }else{
		            	masterGrid.normalGrid.getHeaderContainer().down('#OurFinance_col_header').show(); 
   ...
   /**
	...
	var panelSearch = Unilite.createSearchPanel('searchForm', {		
		...
		items: [{	
			...
			items: [{ 
						...
						startFieldName: 'WORK_MONTH_FR',
						endFieldName:'WORK_MONTH_TO',
						hidden: false,
						allowBlank:false,
						...
						onStartDateChange: function(field, newValue, oldValue, eOpts) {
			                	if(panelResult) {
									panelResult.setValue('WORK_MONTH_FR',newValue);
			                	}
						    },
					    onEndDateChange: function(field, newValue, oldValue, eOpts) {
					    	if(panelResult) {
					    		panelResult.setValue('WORK_MONTH_TO',newValue);
					    	}
					    }
					},
					...
					{ name: 'ITEM_ACCOUNT',
						  		...
						  		}),
						  		{
									...
									fieldLabel: '재무출고표시',		
									labelWidth : 225,
									name: 'SHOW_OUT_FINACE',
									checked: showFinanceDefalt,
									listeners: {
										change : function(field, newValue, oldValue, eOpts) {            
											panelResult.getField('SHOW_OUT_FINACE').checked = newValue;
											panelResult.getField('SHOW_OUT_FINACE').setValue(newValue)
	...
	var panelResult = Unilite.createSearchForm('resultForm',{
		...
		layout: {type: 'uniTable', columns: 6, tableAttrs:{cellpadding:1,width:'100%', border : 0}, tdAttrs: {valign:'top'}},
		...
		items: [{ 
		  		...
		  		,{
					...
					onStartDateChange: function(field, newValue, oldValue, eOpts) {
		                	if(panelSearch) {
		                		panelSearch.setValue('WORK_MONTH_FR',newValue);
		                	}
					    },
				    onEndDateChange: function(field, newValue, oldValue, eOpts) {
				    	if(panelSearch) {
				    		panelSearch.setValue('WORK_MONTH_TO',newValue);
				    	}
				    }
				}        
		  		,{
				...
				{ name: 'ITEM_ACCOUNT',
		  		...
		  		}),
		  		{
					...
					listeners: {
						change : function(field, newValue, oldValue, eOpts) {            
							panelSearch.getField('SHOW_OUT_FINACE').checked = newValue;
							panelSearch.getField('SHOW_OUT_FINACE').setValue(newValue)
	...
	var masterGrid = Unilite.createGrid('tio110skrvGrid1', {
        ...
        viewConfig:{
                ...
                getRowClass : function(record,rowIndex,rowParams,store){
                    ...
                    if(record.get('PROD_ITEM_CODE')=="총계"){
                    	...
                    	var showFinance = (panelResult.getValues().STATUS == "1" && panelResult.getField("SHOW_OUT_FINACE").checked );
                    	if(showFinance && Number(record.get('OutProdt_q')) + Number(record.get('OS_OutOutOrder_q')) != Number(record.get('OutFinance_q'))) {
                    		//cls = 'x-grid-cell-text-highlight';
                    		cls = 'x-grid-cell-background-highlight';
                    	}
                    	//	2020.07.21	JH.Lee	소수점 6째 자리에서 차이 발생. 금액은 모두 소수이하 자릿수 없는 원화로 가정하고 반올림 처리하여 비교.
                    	else if(showFinance && Math.round(Number(record.get('OutProdt_i')) + Number(record.get('OS_OutOutOrder_i'))) != Math.round(Number(record.get('OutFinance_i')))) {
                    		cls = 'x-grid-cell-background-highlight';
                    	}
                    }
                    return cls;
        ...
        columns: [
        	...
        	{text:'입고',columns:[
        		...
        		{text:'제품대체'		,columns:[{dataIndex: 'InProdtReplace_q'		, width: 90,summaryType: 'sum' },{dataIndex: 'InProdtReplace_p'		, width: 90 },{dataIndex: 'InProdtReplace_i'		, width: 110,summaryType: 'sum' }]},
        		...
        		{text:'계'  			,columns:[{dataIndex: 'InSum_q'		, width: 90,summaryType: 'sum' },{dataIndex: 'InSum_p'		, width: 90,summaryType: 'sum' },{dataIndex: 'InSum_i'		, width: 110,summaryType: 'sum' }]}
        	]},
        	{text:'출고',columns:[
        		{text:'생산출고'		,columns:[{dataIndex: 'OutProdt_q'	, width: 90,summaryType: 'sum' },{dataIndex: 'OutProdt_i'	, width: 110,summaryType: 'sum' }]},
        		{text:'제품대체'		,columns:[{dataIndex: 'OutProdtReplace_q'	, width: 90,summaryType: 'sum' },{dataIndex: 'OutProdtReplace_i'	, width: 110,summaryType: 'sum' }]},
        		...
        		{text:'재무출고'		,columns:[{dataIndex: 'OutFinance_q'	, width: 90,summaryType: 'sum' },{dataIndex: 'OutFinance_i'	, width: 110,summaryType: 'sum' }], itemId : 'OurFinance_col_header'},
	...
	Unilite.Main( {
		...
		id: 's_cdr400skrv_inApp',
		fnInitBinding: function(param) {
			if(param && param.DIV_CODE)	{
				panelResult.setValue("DIV_CODE",param.DIV_CODE);
				panelResult.setValue("WORK_MONTH_FR",param.WORK_MONTH);
				panelResult.setValue("WORK_MONTH_TO",param.WORK_MONTH);
				panelSearch.setValue("DIV_CODE",param.DIV_CODE);
				panelSearch.setValue("WORK_MONTH_FR",param.WORK_MONTH);
				panelSearch.setValue("WORK_MONTH_TO",param.WORK_MONTH);
			}
		},
	...
	function fnPriceCompute(){
		...
		var count = store.getCount();
		 for (var i = 0; i < count; i++) {
		  ...
		  //[제품대체 단가구하기]*******************************************************
		  dProdtReplace_i=record.data.InProdtReplace_i;
		  dProdtReplace_q=record.data.InProdtReplace_q;
		  if(dProdtReplace_i==0||dProdtReplace_q==0){
		  	dProdtReplace_p=0;
		  }else{
		  	dProdtReplace_p=dProdtReplace_i / dProdtReplace_q;
		  }
		  record.data.InProdtReplace_p=dProdtReplace_p;
		  //[구매입고 단가구하기]*******************************************************
	...
	function fnSumStockPriceCompute(){
		...
		var count = store.getCount();
		 for (var i = 0; i < count; i++) {
		  var record = store.getAt(i);
		//입고의 수량/금액/단가구하기]**********************************************
		  dInSum_q=record.data.InProdt_q+record.data.InProdtReplace_q+record.data.InBuy_q+record.data.InOutOrder_q+
		  record.data.InOther_q+record.data.InMove_q+record.data.InEtc_q;
		  dInSum_i=record.data.InProdt_i+record.data.InProdtReplace_i+record.data.InBuy_i+record.data.InOutOrder_i+
		...
		//[출고의 수량/금액/단가구하기]**********************************************
		  dOutSum_q=record.data.OutProdt_q+record.data.OutProdtReplace_q+record.data.OutSale_q+record.data.OutOutOrder_q+
		  record.data.OutOther_q+record.data.OutMove_q+record.data.OutDisUse_q+record.data.OutEtc_q;
		  dOutSum_i=record.data.OutProdt_i+record.data.OutProdtReplace_i+record.data.OutSale_i+record.data.OutOutOrder_i+
