...
<t:appConfig pgmId="s_srq100ukrv_in"  >
	<t:ExtComboStore comboType="BOR120" pgmId="s_srq100ukrv_in"  /> 			<!-- 사업장 -->
				...
				items:[{
							...
							handler: function() {
								...
								if(Ext.isEmpty(masterForm.getValue('ISSUE_REQ_NUM'))){
									alert('<t:message code="system.message.sales.datacheck019" default="출하지시번호가 없습니다."/>');
								}else{
...
//			me.setValue('EXCHAGE_RATE',		record.get('EXCHANGE_RATE'));
	 ...
	 */
	//마스터 모델 정의
	Unilite.defineModel('s_srq100ukrv_inDetailModel', {
		fields: [{name: 'DIV_CODE'						   , text:'<t:message code="system.label.sales.division" default="사업장"/>'		 		, type: 'string', 	defaultValue: UserInfo.divCode},
				 ...
				 {name: 'ISSUE_DATE'					   , text:'<t:message code="system.label.sales.issuedate" default="출고일"/>'		 	, type: 'uniDate', allowBlank:false},
	...
	var directProxy = Ext.create('Unilite.com.data.proxy.UniDirectProxy',{
		api: {
			read: 's_srq100ukrv_inService.selectList',
			update: 's_srq100ukrv_inService.updateDetail',
			create: 's_srq100ukrv_inService.insertDetail',
			destroy: 's_srq100ukrv_inService.deleteDetail',
			syncAll: 's_srq100ukrv_inService.saveAll'
		}
	});

	//마스터 스토어 정의
	var detailStore = Unilite.createStore('s_srq100ukrv_inDetailStore', {
		model: 's_srq100ukrv_inDetailModel',
		autoLoad: false,
		uniOpt: {
...
//								inValidRecs = [].concat(inValidRecs, [record])
					...
					if(record.get('ISSUE_REQ_QTY') > record.get('STOCK_Q')){
						alert('재고가 부족합니다.\n 순번 : ' + record.get('ISSUE_REQ_SEQ') + ', 품목코드 : '+ record.get('ITEM_CODE') +',  LOT NO : ' + record.get('LOT_NO'));
						isErr = true;
						return false;
					}
					if(record.get('LOT_YN') == 'Y' && Ext.isEmpty(record.get('LOT_NO'))){
						alert('LOT관리여부 Y일 경우, LOT NO는 필수 입력 사항입니다.');
						isErr = true;
						return false;
					}
			...
			if(inValidRecs.length == 0) {
				...
				var grid = Ext.getCmp('s_srq100ukrv_inGrid');
...
//				var record = detailStore.data.items[0];
 	...
 	//마스터 그리드 정의
	var detailGrid = Unilite.createGrid('s_srq100ukrv_inGrid', {
		layout: 'fit',
		region:'center',
		uniOpt: {
		...
		columns: [
				  ...
				  {dataIndex: 'CUSTOM_NAME'					   ,		width: 133 ,
				   editor: Unilite.popup('CUST_G',{
							...
							'onSelected': {
								fn: function(records, type  ){
									var grdRecord = Ext.getCmp('s_srq100ukrv_inGrid').uniOpt.currentRecord;
...
//				  {dataIndex: 'WH_CODE'						   ,		width: 100 },
								...
								'onSelected': {
									fn: function(records, type) {
										...
										Ext.each(records, function(record,i) {
											...
											var transRate	= rtnRecord.get('TRANS_RATE');
											...
											rtnRecord.set('STOCK_Q'			, lotStockQ/transRate);
				  ...
				  {dataIndex: 'ISSUE_REQ_AMT'				   ,		width: 106, summaryType: 'sum'   },
				  {dataIndex: 'TAX_TYPE'					   ,		width: 80, align: 'center' },
				  {dataIndex: 'ISSUE_REQ_TAX_AMT'			   ,		width: 106, summaryType: 'sum' },
				  ...
				  {dataIndex: 'LOT_YN'						   ,		width: 106 , hidden: true }
		],
		listeners: {
...
/*							{
	...
	var requestNoMasterStore = Unilite.createStore('requestNoMasterStore', {
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_srq100ukrv_inService.selectOrderNumMasterList'
				}
			}
			,loadStoreRecords : function()	{
	...
	//조회창 그리드 정의
	var requestNoMasterGrid = Unilite.createGrid('s_srq100ukrv_inOrderNoMasterGrid', {
		// title: '기본',
		layout : 'fit',
	...
	//참조내역 모델 정의
	Unilite.defineModel('s_srq100ukrv_inReferModel', {
		fields: [{name: 'CUSTOM_CODE'					,text: '<t:message code="system.label.sales.client" default="고객"/>'				, type: 'string'},
				 {name: 'CUSTOM_NAME'					,text: '<t:message code="system.label.sales.client" default="고객"/>'				, type: 'string'},
				 {name: 'ITEM_CODE'						,text: '<t:message code="system.label.sales.item" default="품목"/>'			, type: 'string'},
				 {name: 'ITEM_NAME'						,text: '<t:message code="system.label.sales.itemname" default="품목명"/>'				, type: 'string'},
	...
	//참조내역 스토어 정의 (수주참조)
	var referStore = Unilite.createStore('s_srq100ukrv_inReferStore', {
			model: 's_srq100ukrv_inReferModel',
			autoLoad: false,
			uniOpt : {
				isMaster: false,			// 상위 버튼 연결
				editable: false,			// 수정 모드 사용
			...
			proxy: {
				type: 'direct',
				api: {
					read	: 's_srq100ukrv_inService.selectReferList'
				}
			}
			,loadStoreRecords : function()	{
	...
	//참조내역 그리드 정의
	var referGrid = Unilite.createGrid('s_srq100ukrv_inReferGrid', {
		// title: '기본',
		layout : 'fit',
		store: referStore,
...
//					s_srq100ukrv_inService.selectReferList(param, function(provider, response){
			...
			if(params.PGM_ID == 'sof100ukrv') { //수주등록 에서 링크넘어올시
				...
				Ext.each(params.record, function(rec,i){
				 ...
				 var r = {
					...
					WH_CODE			:  Unilite.nvl(rec.get('WH_CODE'),rec.get('REF_WH_CODE')),
...
//			}
		...
		onDetailButtonDown: function() {
			var as = Ext.getCmp('s_srq100ukrv_inAdvanceSerch');
			if(as.isHidden())	{
				as.show();
			} else {
				as.hide()
		...
		fnTaxCalculate: function(rtnRecord, dOrderO) {
			...
			if(sTaxInoutType=="1") {
				...
				if(UserInfo.compCountry == 'CN') {
					...
					dTemp	  = UniSales.fnAmtWonCalc((dAmountI / ( dVatRate + 100 )) * 100, sUnderCalcType);	 //세액은 절사처리함.
					dTaxAmtO  = UniSales.fnAmtWonCalc(dTemp * dVatRate / 100, sUnderCalcType);   							//세액은 절사처리함.
				}
