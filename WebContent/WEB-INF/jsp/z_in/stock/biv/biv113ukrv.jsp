...
<t:appConfig pgmId="biv113ukrv"  >
<t:ExtComboStore comboType="BOR120" pgmId="biv113ukrv"/> 			<!-- 사업장 -->
...
function appMain() {
	 ...
	 * @type
	 */
	Unilite.defineModel('Biv113ukrvModel', {
	    fields: [
	 ...
	 * @type
	 */
	var directMasterStore = Unilite.createStore('biv113ukrvMasterStore1',{
			model: 'Biv113ukrvModel',
			uniOpt : {
            ...
            proxy: {
                type: 'direct',
                api: {
                	read: 'biv113ukrvService.selectMaster'
                }
            },
						...
						handler : function(records) {
							if(panelSearch.getForm().isValid()) {
								if(confirm('<t:message code="system.message.inventory.message007" default="실행하시겠습니까?"/>')) {
									...
									biv113ukrvService.WhCodeSet(param, function(provider, response) {
										...
										if(count == '') {
											...
											biv113ukrvService.insertMaster(param, function(provider, response) {
	...
	Unilite.Main( {
		...
		fnYyyymmSet : function(divCode) {		// 사업장 선택에 따라 날짜 set
			...
			biv113ukrvService.YyyymmSet(param, function(provider, response)	{
