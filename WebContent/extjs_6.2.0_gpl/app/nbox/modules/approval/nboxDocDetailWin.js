...
Ext.define('nbox.docDetailWindow',{
    ...
    initData: function(){
    	...
    	var docEditContentsHtmlEditor = nboxDocEditContentsPanel.getRegItems()['DocEditContentsHtmlEditor'];
    	var path = me.getRegItems()['Path'];
    	var gubun = me.getRegItems()['Gubun'];    	
    	var interfaceKey = me.getRegItems()['InterfaceKey'];
    	var formID;
    	var contents;
    	...
    	switch(me.getRegItems()['ActionType'])
		{
			...
			case NBOX_C_CREATE:
				if (path ==  "ERP")
				{
					nboxDocCommonService.getInterfaceForm({GUBUN : gubun, INTERFACEKEY: interfaceKey},function(provider, response) {
							formID = response.result.records.FORMID;
							contents = response.result.records.CONTENTS;
							if (formID)
							{
								nboxDocEditFormCombo.setValue(formID);
								nboxDocEditFormCombo.setDisabled(true);
							}
    ...
    formShow: function(){
		...
		switch(me.getRegItems()['ActionType'])
		{
			case NBOX_C_READ:
				switch(me.getRegItems()['Box']){
	    			...
	    			case 'XA005':  case 'XA006': case 'XA007':	case 'XA008': case 'XA009': case 'XA010': case 'XA012': case 'XA013'://예결문서, 참조, 수신, 발신, 회사결재문서함, 기결상세함, 개인결재문서함
		    			...
		    			break;
	    			case 'XA999'://결재조회만
	    				detailToolbar.setToolBars(['comment', 'close'], true);
						detailToolbar.setToolBars(['edit', 'draft', 'draftcancel','confirm', 'confirmcancel', 'return', 'returncancel', 'delete', 'prev', 'next'], false);
		    			break;
		    		default:
		    			break;
	    		}    				
			...
			case NBOX_C_CREATE:
				detailToolbar.setToolBars(['save', 'draft', 'cancel', 'close'], true);
				detailToolbar.setToolBars(['edit', 'draftcancel', 'confirm', 'confirmcancel', 'return', 'returncancel', 'doubleline', 'delete', 'preview', 'comment', 'prev', 'next'], false);
...
function openDocDetailTab_GW(gubun, interfaceKey, documentID) {
	//var documentID = 'A20170512000001';
    ...
    parent.openTab(rec1, '/nbox/' + pgmID + '.do', params);
}	
