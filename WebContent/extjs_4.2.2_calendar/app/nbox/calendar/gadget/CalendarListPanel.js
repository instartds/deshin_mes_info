 ...
 */
/**
 * @class nbox.calendar.gadget.CalendarListPanel
 * @extends Ext.Panel
 * <p>This is a {@link Ext.Panel panel} subclass that renders a list of available calendars
 * @constructor
 * @param {Object} config The config object
 * @xtype nbox.calendarlist
 */
Ext.define('nbox.calendar.gadget.CalendarListPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nbox.calendarlist',
    
    requires: [
        'Ext.XTemplate',
        'nbox.calendar.gadget.CalendarListMenu'
    ],
     ...
     * A {@link Ext.data.Store store} containing records of type {@link nbox.calendar.data.CalendarModel CalendarRecord}.
     ...
     * records of type {@link nbox.calendar.data.CalendarModel CalendarRecord}.
    ...
    refresh: function(){
        ...
        var data = [], i = 0, o = null,
            CM = nbox.calendar.data.CalendarMappings,
    ...
    toggleCalendar: function(id, commit){
        var rec = this.store.findRecord(nbox.calendar.data.CalendarMappings.CalendarId.name, id);
            CM = nbox.calendar.data.CalendarMappings,
    ...
    showCalendar: function(id, commit){
        var rec = this.store.findRecord(nbox.calendar.data.CalendarMappings.CalendarId.name, id);
        if(rec.data[nbox.calendar.data.CalendarMappings.IsHidden.name] === true){
    ...
    radioCalendar: function(id){
        var i = 0, recId,
            calendarId = nbox.calendar.data.CalendarMappings.CalendarId.name,
            recs = this.store.getRange(),
            len = recs.length;
    ...
    handleColorChange: function(menu, id, colorId, origColorId){
        var rec = this.store.findRecord(nbox.calendar.data.CalendarMappings.CalendarId.name, id);
        rec.data[nbox.calendar.data.CalendarMappings.ColorId.name] = colorId;
        rec.commit();
    },
    ...
    showEventMenu : function(el, xy){
        var id = this.getCalendarId(el.parent('li')),
            rec = this.store.findRecord(nbox.calendar.data.CalendarMappings.CalendarId.name, id);
            colorId = rec.data[nbox.calendar.data.CalendarMappings.ColorId.name];
            
        if(!this.menu){
            this.menu = Ext.create('nbox.calendar.gadget.CalendarListMenu');
