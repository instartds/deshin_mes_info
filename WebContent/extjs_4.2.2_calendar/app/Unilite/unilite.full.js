...
//		        return record ? record.get(combo.displayField) : combo.valueNotFoundText;
		        ...
		        if(combo.child) {
					...
					combo.on('change', function(combo, newValue, oldValue, eOpts) {	
						if(combo && combo.child && combo.ownerCt.grid.getSelectedRecord())	{
		        			combo.ownerCt.grid.getSelectedRecord().set(combo.child, '')
						}
					}, this);
				}
...
function uniDirectExceptionProcessor(event) {
	...
	if(Ext.isDefined(event.transaction) && Ext.isDefined(event.transaction.action) && event.transaction.action == 'mainBoardService')	{
		Ext.getBody().unmask();
		return;
	}
...
Ext.define('Unilite.com.grid.feature.UniSummary', {
     ...
     */
    //dock: undefined,
	dock: 'bottom', // 설정 후 합계표시 토글 시키면 row 가 사라짐
...
Ext.define('Unilite.com.grid.UniGridPanel', {
	...
	uniOpt:{
		...
		useToggleSummary: false,		//합계표시 아이콘 사용 여부
...
//		config.plugins.push(bufferedPlugin);
	...
	initComponent : function() {
        ...
        */
         ...
         * for Toggle Summary
         */
    	if(!Ext.isEmpty(tbar) && this.uniOpt.useToggleSummary) {
...
Ext.define('Unilite.com.data.UniStore', {
    ...
    ,_onBeforeLoad:function(store, operation, eOpts)	{
    	...
    	store.readParams = operation.params;
    }
