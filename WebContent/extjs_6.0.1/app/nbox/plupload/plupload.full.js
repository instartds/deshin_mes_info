...
(function() {
	...
	var g = {
		...
		getPos : function(p, t) {
			...
			if (p && p.getBoundingClientRect && g.ua.ie
		...
		g.extend(this, {
			...
			init : function() {
					...
					for (F = 0; F < G.length; F++) {
						...
						if (J && !J.test(E.name)) {
							D.trigger("Error", {
										...
										message : g
												.translate("File extension error."),
										file : E
									});
							continue
						}
				...
				B.bind("UploadProgress", function(D, E) {
							E.percent = E.size > 0 ? Math.ceil(E.loaded
									/ E.size * 100) : 100;
							o()
						});
	...
	c.runtimes.Gears = c.addRuntime("gears", {
		...
		init : function(l, n) {
			...
			l.bind("PostInit", function() {
				...
				c.addEvent(b.getElementById(p.browse_button), "click",
						function(u) {
							var t = [], r, q, s;
							u.preventDefault();
							if (g) {
			...
			l.bind("UploadFile", function(u, r) {
				...
				if (q && /\.(png|jpg|jpeg)$/i.test(r.name)) {
					f[r.id] = a(f[r.id], q, /\.png$/i.test(r.name)
									? "image/png"
									: "image/jpeg")
				}
				...
				function p() {
					...
					function A(E) {
						...
						if (y) {
							...
							c.each(c.extend(B, u.settings.multipart_params),
									function(L, K) {
										D
												.append(G
														+ J
					...
					if (r.status == c.DONE || r.status == c.FAILED
							|| u.state == c.STOPPED) {
						return
					}
					if (o) {
					...
					h.open("POST", z);
					if (!y) {
						h.setRequestHeader("Content-Disposition",
	...
	d.runtimes.Silverlight = d.addRuntime("silverlight", {
		...
		init : function(p, q) {
			...
			d.extend(o.style, {
						...
						height : "100px",
						overflow : "hidden",
						opacity : p.settings.shim_bgcolor || b.documentMode > 8
								? ""
								: 0.01
					});
			...
			o.innerHTML = '<object id="'
					...
					+ p.settings.multi_selection + '"/></object>';
			function j() {
			...
			p.bind("Silverlight:Init", function() {
				...
				p.bind("Refresh", function(t) {
					...
					if (u) {
						...
						w = d.getSize(u);
						d.extend(b.getElementById(t.id
				...
				p.bind("Silverlight:UploadChunkSuccessful", function(t, w, u,
								z, y) {
				...
				p.bind("UploadFile", function(t, v) {
							...
							j().UploadFile(s[v.id], t.settings.url, c({
										...
										mime : d.mimeTypes[v.name.replace(
												/^.+\.([^.]+)/, "$1")
												.toLowerCase()]
												|| "application/octet-stream",
	...
	d.runtimes.Flash = d.addRuntime("flash", {
		...
		init : function(n, p) {
			...
			n.bind("Flash:Init", function() {
				var s = {}, r;
				try {
					o().setFileFilters(n.settings.filters,
		...
		init : function(g, j) {
			...
			function b() {
				...
				g.bind("PostInit", function() {
					var o, m = d.drop_element, q = g.id + "_droptarget", l = document
							.getElementById(m), n;
					function r(t, s) {
					...
					a.addEvent(document.getElementById(d.browse_button),
							"click", function(y) {
					...
					function s(u, w) {
						...
						e.Uploader.upload({
							...
							progressCallback : function(z) {
								var y, x = 0;
								p[u] = parseInt(z.filePercent * v.size / 100,
										10);
	...
	l.runtimes.Html5 = l.addRuntime("html5", {
		getFeatures : function() {
			...
			return {
				...
				canSendBinary : t,
				cantSendBlobInFormData : !!(l.ua.gecko && k.FormData
						&& k.FileReader && !FileReader.prototype.readAsArrayBuffer)
						|| l.ua.android,
				progress : u,
				chunks : q,
		...
		init : function(s, u) {
			...
			function r(z) {
				...
				for (w = 0; w < z.length; w++) {
					...
					A = l.guid();
					d[A] = x;
					y.push(new l.File(A, x.fileName || x.name, x.fileSize
									|| x.size))
				}
				if (y.length) {
			...
			s.bind("Init", function(A) {
				var K, J, G = [], z, H, w = A.settings.filters, x, F, v = m.body, I;
				K = m.createElement("div");
				K.id = A.id + "_html5_container";
				l.extend(K.style, {
							position : "absolute",
				...
				no_type_restriction : for (z = 0; z < w.length; z++) {
					...
					for (H = 0; H < x.length; H++) {
						...
						F = l.mimeTypes[x[H]];
						if (F && l.inArray(F, G) === -1) {
							G.push(F)
						}
					}
				}
				K.innerHTML = '<input id="'
						+ s.id
						+ '_html5"  style="font-size:999px" type="file" accept="'
						+ G.join(",")
						+ '" '
						+ (s.settings.multi_selection
								&& s.features.multi_selection
								? 'multiple="multiple"'
								: "") + " />";
				K.scrollTop = 100;
				I = m.getElementById(s.id + "_html5");
				...
				I.onchange = function D() {
					r(this.files);
					if (!l.ua.ie) {
						this.value = ""
					} else {
						var y = this.cloneNode(true);
						this.parentNode.replaceChild(y, this);
						y.onchange = D;
						y = null
					}
				};
				J = m.getElementById(A.settings.browse_button);
				if (J) {
					var C = A.settings.browse_button_hover, E = A.settings.browse_button_active, B = A.features.triggerDialog
							? J
							: K;
					if (C) {
			...
			s.bind("Refresh", function(v) {
						...
						if (w) {
							x = l.getPos(w,
									m.getElementById(v.settings.container));
							y = l.getSize(w);
			...
			s.bind("UploadFile", function(v, x) {
				...
				function z(C) {
					var F = 0, E = 0;
					function D() {
						var L, P, N = 1, O, K, M, H, G = v.settings.url;
						function J(S) {
							if (t.sendAsBinary) {
								t.sendAsBinary(S)
							} else {
						...
						function I(R) {
							...
							if (t.upload) {
								t.upload.onprogress = function(X) {
									x.loaded = Math.min(x.size, E + X.loaded
													- V);
							...
							G = l.buildUrl(v.settings.url, l.extend(O,
											v.settings.multipart_params));
							t.open("post", G, true);
							t.setRequestHeader("Content-Type",
									"application/octet-stream");
						...
						if (x.status == l.DONE || x.status == l.FAILED
								|| v.state == l.STOPPED) {
							return
						}
						O = {
						...
						if (y.chunk_size && x.size > y.chunk_size
						...
						if (y.chunk_size && q.chunks) {
							O.chunk = F;
							O.chunks = N
						}
						if (v.settings.multipart && q.multipart
								&& typeof(L) !== "string" && k.FileReader
								&& q.cantSendBlobInFormData && q.chunks
				...
				B = d[x.id];
				if (q.jpgresize && v.settings.resize
						&& /\.(png|jpg|jpeg)$/i.test(x.name)) {
					e.call(v, x, v.settings.resize, /\.png$/i.test(x.name)
									? "image/png"
									: "image/jpeg", function(C) {
								if (C.success) {
		...
		init : function(f, g) {
			f.bind("Init", function(p) {
				...
				function u() {
					...
					y.innerHTML = '<iframe id="' + p.id + '_iframe" name="'
							+ p.id + '_iframe" src="' + h
