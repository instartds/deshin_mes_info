 ...
 */
/**
 * @class nbox.calendar.CalendarPanel
 ...
 * {@link #Ext.data.Store store} config containing records of type {@link nbox.calendar.data.EventModel EventRecord}.</p>
 * @constructor
 * @param {Object} config The config object
 * @xtype nbox.calendarpanel
 */
 
 var uniText = {
    cal_filter_private: '개인',
	cal_filter_department: '부서',
	cal_filter_all: '전사',
	...
	cal_calType_MP: '계획',
	cal_calType_MR: '결과',
	cal_calType_MPR: '전체(계획+결과)',
	
	cal_move_prev: '이전',
	cal_move_next: '다음'
};
    
Ext.define('nbox.calendar.CalendarPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.nbox.calendarpanel',
    
    requires: [
        'Ext.layout.container.Card',
        'nbox.calendar.view.Day',
        'nbox.calendar.view.Week',
        'nbox.calendar.view.Month',
        'nbox.calendar.view.MultiDay',
        'nbox.calendar.view.MultiWeek'
    ],
     ...
     * active {@link nbox.calendar.view.AbstractCalendar view} at any time using the {@link #activeView} property.
     */
    /*
     ...
     * The {@link Ext.data.Store store} which is bound to this calendar and contains {@link nbox.calendar.data.EventModel EventModels}.
     ...
     * The {@link Ext.data.Store store} which is bound to this calendar and contains {@link nbox.calendar.data.CalendarModel CalendarModelss}.
     ...
     * A config object that will be applied to all {@link nbox.calendar.view.AbstractCalendar views} managed by this CalendarPanel. Any
     * options on this object that do not apply to any particular view will simply be ignored.
     */
    /**
     * @cfg {Object} dayViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.view.Day DayView} managed by this CalendarPanel.
     */
    /**
     * @cfg {Object} multiDayViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.view.MultiDay MultiDayView} managed by this CalendarPanel.
     */
    /**
     * @cfg {Object} weekViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.view.Week WeekView} managed by this CalendarPanel.
     */
    /**
     * @cfg {Object} multiWeekViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.view.MultiWeek MultiWeekView} managed by this CalendarPanel.
     */
    /**
     * @cfg {Object} monthViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.view.Month MonthView} managed by this CalendarPanel.
     */
    /**
     * @cfg {Object} editViewCfg
     * A config object that will be applied only to the {@link nbox.calendar.form.EventDetails EventEditForm} managed by this CalendarPanel.
     */
    
    /**
     * A reference to the {@link nbox.calendar.view.AbstractCalendar view} that is currently active.
     * @type {nbox.calendar.view.AbstractCalendar}
     * @property activeView
     */
    ...
    _qryTypeChange: function(type) {
    	//console.log("type", type)
    	this.uniQryType = type;
    	this._refresh();
            
    },
    _qryInOutTypeChange: function(type) {
    	//console.log("type", type)
    	this.uniInOutType = type;
    	this._refresh();
    ...
    // AbstractCalendar.getStoreParams
    uniQryType : 'C',		// 전사     
    uniInOutType : '2',		// 1:내부 2:외부(detault) 
    // private
    initComponent : function(){
        ...
        this.tbar.items.push({
		    xtype: 'button',
		    text: uniText.cal_filter_private,
		    toggleGroup: 'QRY_TYPE',
		    enableToggle: true,
            listeners    : {
		    ...
		    xtype: 'button',
		    text: uniText.cal_filter_department,
		    toggleGroup: 'QRY_TYPE',
		    enableToggle: true,
            listeners    : {
                toggle : function(btn, pressed) {
		    ...
		    xtype: 'button',
		    text: uniText.cal_filter_inner,
		    toggleGroup: 'QRY_INOUT',
		    enableToggle: true,
		    pressed:false,
            listeners    : {
                toggle : function( btn, pressed ) {
                	if(pressed) {
	                    me._qryInOutTypeChange('1');
                	}
                }
            }
		}, {
		    xtype: 'button',
		    text: uniText.cal_filter_outer,
		    toggleGroup: 'QRY_INOUT',
		    enableToggle: true,
		    pressed:true,
            listeners    : {
        ...
        if(this.showNavNextPrev){
            this.tbar.items.push({id: this.id+'-tb-prev', text:uniText.cal_move_prev, handler: this.onPrevClick, scope: this, iconCls: 'x-tbar-page-prev'});
            this.tbar.items.push({id: this.id+'-tb-next', text:uniText.cal_move_next, handler: this.onNextClick, scope: this, iconCls: 'x-tbar-page-next', iconAlign: 'right'});
        }
        if(this.showRefresh) {
	        this.tbar.items.push(' ','-',' ');
	        this.tbar.items.push({id: this.id+'-refresh', text: '', handler: function() { this._refresh();}, scope: this, iconCls: 'icon-cal-refresh'});
        }
        if(this.showNavJump){
        ...
        this.addEvents({
             ...
             * @param {nbox.calendar.CalendarPanel} this
             ...
             * @param {nbox.calendar.CalendarPanel} this
             * @param {nbox.CalendarView} view The view being activated (any valid {@link nbox.calendar.view.AbstractCalendar CalendarView} subclass)
             ...
             * (by default, an instance of {@link nbox.calendar.form.EventDetails}). Handling code should hide the active
             ...
             * @param {nbox.calendar.CalendarPanel} this The CalendarPanel
             * @param {nbox.calendar.view.AbstractCalendar} view The currently active {@link nbox.calendar.view.AbstractCalendar CalendarView} subclass
             ...
             * {@link nbox.calendar.view.AbstractCalendar CalendarView} subclasses in that it provides a default implementation (showing
             ...
             * @param {nbox.calendar.CalendarPanel} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that was resized
...
//        }, this.editViewCfg));
     ...
     * @param {nbox.calendar.data.EventModel} record The event record to edit
     * @return {nbox.calendar.CalendarPanel} this
     */
    ...
    setStartDate: function(dt){
        //nbox.log('setStartDate (CalendarPanel');
        this.startDate = dt;
     ...
     * {@link nbox.calendar.view.AbstractCalendar CalendarView}.
     * @return {nbox.calendar.view.AbstractCalendar} The active view
     */
