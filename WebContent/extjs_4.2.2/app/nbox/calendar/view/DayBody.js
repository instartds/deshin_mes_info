 ...
 */
/**
 * @class nbox.calendar.view.DayBody
 * @extends nbox.calendar.view.AbstractCalendar
 ...
 * nbox.calendar.view.Day DayView} which aggregates this class and the {@link 
 * nbox.calendar.view.DayHeader DayHeaderView} into the single unified view
 * presented by {@link nbox.calendar.CalendarPanel CalendarPanel}.</p>
 * @constructor
 * @param {Object} config The config object
 */
Ext.define('nbox.calendar.view.DayBody', {
    extend: 'nbox.calendar.view.AbstractCalendar',
    alias: 'widget.nbox.daybodyview',
    
    requires: [
        'Ext.XTemplate',
        'nbox.calendar.template.DayBody',
        'nbox.calendar.data.EventMappings',
        'nbox.calendar.dd.DayDragZone',
        'nbox.calendar.dd.DayDropZone'
    ],
    ...
    initComponent : function(){
        ...
        this.addEvents({
             ...
             * @param {nbox.calendar.view.DayBody} this
             * @param {nbox.calendar.data.EventModel} rec The original {@link
             * nbox.calendar.data.EventModel record} for the event that was resized
             ...
             * @param {nbox.calendar.view.DayBody} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel
    ...
    initDD : function(){
        ...
        /*this.dragZone = Ext.create('nbox.calendar.dd.DayDragZone', this.el, Ext.apply({
            // disabled for now because of bugs in Ext 4 ScrollManager:
            //containerScroll: true
        }, cfg));
        
        this.dropZone = Ext.create('nbox.calendar.dd.DayDropZone', this.el, cfg);*/
    },
    
    //private
    refresh : function(reloadData){
        //nbox.log('refresh (DayBodyView)');
    ...
    afterRender : function(){
        if(!this.tpl){
            this.tpl = Ext.create('nbox.calendar.template.DayBody', {
                id: this.id,
    ...
    onEventResize : function(rec, data){
        if(this.fireEvent('beforeeventresize', this, rec, data) !== false){
            var D = Extensible.Date,
                start = nbox.calendar.data.EventMappings.StartDate.name,
                end = nbox.calendar.data.EventMappings.EndDate.name;
...
//                '</tpl>'
        ...
        if(!this.eventTpl){
            this.eventTpl = !(Ext.isIE || Ext.isOpera) ? 
                Ext.create('Ext.XTemplate',
                    '<div id="{_elId}" class="{_extraCls} ext-cal-evt ext-cal-evr icon-cal-{calendarId}" ',
                ...
                : Ext.create('Ext.XTemplate',
                    '<div id="{_elId}" class="ext-cal-evt {_extraCls} icon-cal-{calendarId}" ',
     ...
     * {@link nbox.calendar.data.EventModel}) to populate the calendar views with <strong>all-day</strong> events. 
    ...
    getEventAllDayTemplate : function(){
        if(!this.eventAllDayTpl){
            ...
            tpl = !(Ext.isIE || Ext.isOpera) ? 
                Ext.create('Ext.XTemplate',
                    '<div class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evr icon-cal-{calendarId}" ',
                            'style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                        body,
                    '</div>'
                ) 
                : Ext.create('Ext.XTemplate',
                    '<div class="ext-cal-evt" ',
                            'style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                        '<div class="{_extraCls} {spanCls} ext-cal-evo icon-cal-{calendarId}">',
    ...
    getTemplateEventData : function(evt){
        ...
        if(this.calendarStore && evt[M.CalendarId.name]){
            var rec = this.calendarStore.findRecord(nbox.calendar.data.CalendarMappings.CalendarId.name, 
                    evt[M.CalendarId.name]);
                
            if(rec){
                colorCls = 'x-cal-' + rec.data[nbox.calendar.data.CalendarMappings.ColorId.name];
            }
        }
    ...
    getTemplateEventBox : function(evt){
        var heightFactor = this.hourHeight / this.hourIncrement,
            start = evt[nbox.calendar.data.EventMappings.StartDate.name],
            end = evt[nbox.calendar.data.EventMappings.EndDate.name],
    ...
    renderItems: function(){
        ...
        for(; i<l; i++){
            var evt = evts[i].data, 
                evt2 = null, 
                dt = evt[nbox.calendar.data.EventMappings.StartDate.name].getDate();
    ...
    onClick : function(e, t){
        if(this.dragPending || nbox.calendar.view.DayBody.superclass.onClick.apply(this, arguments)){
