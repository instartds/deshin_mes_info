...
Ext.define('Extensible.calendar.template.Month', {
     ...
     */
    dayHeaderTitleFormat: 'Y F j일 (l)', //'l, F j, Y',
...
Ext.define('Extensible.calendar.dd.DropZone', {
    ...
    onNodeOver : function(n, dd, e, data){
        ...
        var today = D.today();
        // console.log("start:", D.diffDays(today, n.date  ) , data.eventStart, "n.date :   ", n.date  )
        if(false) { // D.diffDays(today, n.date ) < 0 ) {
        	var msg = "계획을 과거 날자로 이동할 수 없습니다.  ";
        	data.proxy.updateMsg(msg);
        	this.dropAllowed = false;
        	//return false;
        } else {
        	this.dropAllowed = Ext.baseCSSPrefix + 'dd-drop-ok';//'x-dd-drop-ok';
        	//console.log("dropAllowed:" , this.dropAllowed);
        }
...
viewConfig: {
    ...
    getStoreParams : function(){
        ...
        params.QRY_TYPE = this.up('panel').uniQryType;
...
//            this.editWin = Ext.create('Extensible.calendar.form.EventDetail', {
                id: 'ext-cal-editwin',
    ...
    showEventEditor: function(rec, animateTarget){
        ...
        //return this;
    	//this._openPopup(rec);
    	if(Ext.isDefined(this.showEventEditorType))	{
    		if(this.showEventEditorType == 'link')	{
    			this.onEventEditor(rec);
    		}else if('popup')	{
    ...
    _openPopup: function(rec) {
    	...
    	var sParam = Extensible.param(param);
    ...
    setExtParams:function(paramMap)	{
    	var me = this;
    	
    	Ext.apply(me.extParams, paramMap);
    }
});/*!
 * Extensible 1.5.2
 * Copyright(c) 2010-2013 Extensible, LLC
...
Ext.define('Extensible.calendar.view.Month', {
    ...
    getEventTemplate : function(){
        if(!this.eventTpl){
	        ...
	        tpl = !(Ext.isIE || Ext.isOpera) ? 
				Ext.create('Ext.XTemplate',
                    '<div class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evr icon-cal-{calendarId}" title="{Title}">',
		                body,
		            '</div>'
		        ) 
				: Ext.create('Ext.XTemplate',
		            '<tpl if="_renderAsAllDay">',
                        '<div class="{_extraCls} {spanCls} icon-cal-{calendarId} ext-cal-evt ext-cal-evo ">',
		                    '<div class="ext-cal-evm icon-cal-ie">',
		                        '<div class="ext-cal-evi icon-cal-ie">',
		            '</tpl>',
		            '<tpl if="!_renderAsAllDay">',
    ...
    getTemplateEventData : function(evt){
        ...
        data._hasOption = evt['TOPTIONSTR']  != '';
...
Ext.define('Extensible.calendar.UniCalendarPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.extensible.uniCalendarPanel',
    ...
    initComponent : function(){
        ...
        if(this.showNavJump){
            this.tbar.items.push(this.jumpToText);
            this.tbar.items.push({id: this.id+'-tb-jump-dt', xtype: 'uniDatefield', width: 120, showToday: false});
            this.tbar.items.push({id: this.id+'-tb-jump', text: this.goText, handler: this.onJumpClick, scope: this});
        }
