...
function webViewerInitialized(file) {
  ...
  var queryString = "?"+file;//document.location.search.substring(1);
  var params = (0, _ui_utils.parseQueryString)(queryString)
...
PDFPrintService.prototype = {
  ...
  useRenderedPage: function useRenderedPage(printItem) {
    ...
    if (isIE && !isEdge)//isIE==브라우저가 IE인지 확인.
	{
	    scratchCanvas.style.width = '98%';
	    scratchCanvas.style.height = '98%';
    ...
    if (isIE && !isEdge)
	{
    	...
    	pageStyleSheet.textContent = '@page {size: A4 landscape  !important; margin:3.3mm  !important;}';
...
var PDFPageView = function () {
  ...
  _createClass(PDFPageView, [{
    ...
    value: function cssTransform(target) {
      ...
      var cssTransform = '';
      if (isIE  && !isEdge)	{
      	cssTransform = 'rotate(' + relativeRotation + 'deg) ' + 'scale(1,1)';
      }else	{
    ...
    value: function paintOnCanvas(canvasWrapper) {
      ...
      if (isIE  && !isEdge)//isIE==브라우저가 IE인지 확인.
	  {
	      canvas.style.width = '100%';//viewport.width + 'px'; //changed
	      canvas.style.height = '100%';//viewport.height + 'px'; //changed
...
function webViewerLoad() {
  ...
  pdfjsWebApp.PDFViewerApplication.run(config, reportUrl);
}
