...
class SetDefault():
    ...
    def __init__(self,aps_key, comp_code, div_code, work_shop_code, prog_work_code,
                        split_type ,validate, schedule_num,
                        debug,order_num,order_seq,start_day):
...
class CODI_LP():
    ...
    def __init__(self):
        ...
        self.this_year = int(base.start_day[0:4])
        self.this_month = int(base.start_day[4:6])
        self.this_day = int(base.start_day[6:8])

        self.cycle_time = 0  # 참고용

    def connect_mssql(self):
        try:
            server = 'tcp:demo.unilite.co.kr,7304'
            user = 'unilite'
            password = 'UNILITE'
            db = 'OMEGAPLUS_KODI2'
            ...
            print(f"MS SQL Connection Error : [{e}]")
            return False
    ...
    def cal_lp(self,  order_table, fac_capa, equ_item_capa, fix_prod_0 , var_type,repeat):
        ...
        if base.DEBUG :
            ...
            print("[cal_lp]============equ_item_capa table ==================")
            print(equ_item_capa)

            print("[cal_lp]============fac_capa table ==================")
            print(fac_capa)

            fac_capa.to_excel('lp_fac_capa_' + base.work_shop_code + '.xlsx',
        ...
        # Var = LpVariable.dicts("Var", (n_order, n_facility), cat=var_type , lowBound=0)
        ...
        # for r in n_order:
        #     model += lpSum(Var[r][c] for c in n_facility) == 1

        # 변수명 prod_1_1   prod_var[r][c]
        #   sum ( order[] / cycle_time[] * prod_var )  <= capa[]
        # 제품의 총 생산 시간 보다  설비 최대 가동 시간이 커야 한다
        for c in n_facility:
            # 0830 막음
            # model += lpSum(prod_var[r][c] * order_table.iloc[r-1]['AMOUNT'] for r in n_order) \
            #          <= lpSum( prod_var[r][c] * equ_item_capa.iloc[c-1][r-1] for r in n_order )
        #
        # for c in n_facility:
        #     equ_code = fac_capa.iloc[c-1]['EQU_CODE']
        #     model += lpSum(Var[r][c] * order_table.iloc[r - 1]['AMOUNT'] for r in n_order) \
        #              <= lpSum(Var[r][c] * item_capa.loc[equ_code]['ITEM_CAPA'] for r in n_order)
            # 0824 추가
            model += lpSum( prod_var[r][c] * order_table.iloc[r - 1]['AMOUNT'] for r in n_order ) <= fac_capa.iloc[c - 1][
                'ALL_CAPA']

        if base.DEBUG :
            model.writeLP(f"{base.aps_key}_{base.work_shop_code}.lp")
    ...
    def get_order(self,ms_con ):
        ...
        if ms_con:
            ...
            sql = "SELECT COMP_CODE , DIV_CODE , WORK_SHOP_CODE , WK_PLAN_NUM, ITEM_CODE , WK_PLAN_Q  AS AMOUNT,"\
                    ...
                    "AND SCHEDULE_NUM = '{}' AND ORDER_NUM = '{}'  AND SEQ = '{}'" \
                    .format(base.comp_code,base.div_code,base.work_shop_code,base.schedule_num,base.order_num,base.order_seq)
    ...
    def get_fac_basecapa_list(self,ms_con,order_df,repeat):
        ...
        if ms_con:
            ...
            #       "( SELECT ITEM_CODE FROM T_PPL100T WHERE SCHEDULE_NUM = '{}' )"\
            #     .format(base.comp_code,base.div_code,base.work_shop_code,base.prog_work_code,base.schedule_num )

            sql = "SELECT EQU_CODE, MIN(STD_PRODT_Q) as ONE_CAPA FROM PBS410T WITH(NOLOCK) "\
                  "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' AND WORK_SHOP_CODE = '{}' " \
                  "AND PROG_WORK_CODE = '{}' AND ITEM_CODE IN " \
                  "( SELECT ITEM_CODE FROM T_PPL100T WITH(NOLOCK) WHERE SCHEDULE_NUM = '{}' " \
                  " AND ORDER_NUM = '{}' AND SEQ = '{}'  ) " \
                  "GROUP BY EQU_CODE "\
                .format(base.comp_code,base.div_code,base.work_shop_code,base.prog_work_code,base.schedule_num,base.order_num,base.order_seq )

            fac_basecapa_data = pd.read_sql_query(sql, ms_con)

            equ_item_capa = pd.DataFrame()
            for ii in range(len(fac_basecapa_data)) :
                equ_code = fac_basecapa_data.iloc[ii]['EQU_CODE']
                sql = "select  std_prodt_q * {} as '{}' from pbs410t WITH(NOLOCK) where work_shop_code = '{}' " \
                      "and equ_code = '{}' and item_code in " \
                      "( select item_code from t_ppl100t WITH(NOLOCK) where order_num = '{}') "\
                    .format(repeat, ii,base.work_shop_code,equ_code,base.order_num)
                item_capa = pd.read_sql_query(sql,ms_con)
                item_capa = item_capa.transpose()
                equ_item_capa = equ_item_capa.append(item_capa)
            if base.DEBUG:
                print("equ_item_capa")
                print(equ_item_capa)

            return fac_basecapa_data ,equ_item_capa
        else:
            return -1
    ...
    def fix_prod_0_fac(self,ms_con, order_df, fac_list ):
        ...
        for ii in range(len(order_df)):   ## 오더 제품 전체
            ...
            sql = "SELECT  equ_code FROM pbs410t WITH(NOLOCK) WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                  "AND WORK_SHOP_CODE = '{}' AND PROG_WORK_CODE = '{}' "\
                   "AND ITEM_CODE IN (SELECT ITEM_CODE FROM T_PPL100T WITH(NOLOCK) WHERE SCHEDULE_NUM ='{}' " \
                  "AND ORDER_NUM = '{}' AND SEQ = '{}'  ) " \
                   "EXCEPT " \
                   "SELECT equ_code FROM PBS410T WITH(NOLOCK) WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                  "AND WORK_SHOP_CODE = '{}' AND PROG_WORK_CODE = '{}'  AND ITEM_CODE ='{}' " \
                .format(base.comp_code,base.div_code,base.work_shop_code,base.prog_work_code,base.schedule_num,
                        base.order_num,base.order_seq, base.comp_code,base.div_code,base.work_shop_code,base.prog_work_code,item_code)
            ...
            try:
                ...
                print(f" [fix_prod_0_fac] Exception Error {e}")
    ...
    def get_next_date(self,ms_con,equ_code,start_date):
        ...
        if ms_con:
            ...
            sql = "SELECT WORK_DATE , (CASE WHEN OVER_TIME_YN = 'N' THEN STD_CAPA_TIME ELSE STD_CAPA_TIME + OVER_CAPA_TIME  END) " \
                  "AS STD_CAPA_TIME  FROM PBS510T WITH(NOLOCK) " \
                  "WHERE COMP_CODE ='{}' AND DIV_CODE = '{}' AND EQU_CODE = '{}' " \
                  "AND WORK_DATE = " \
                  "( SELECT MIN(WORK_DATE) AS START_DATE  FROM PBS510T WITH(NOLOCK) " \
                  "WHERE COMP_CODE ='{}' AND DIV_CODE = '{}' AND EQU_CODE = '{}' " \
                  "AND WORK_TYPE = '2' AND  WORK_DATE > '{}'  and STD_CAPA_TIME  > 0 )" \
    ...
    def get_setup_time(self,ms_con,equ_code,item_code):
        ...
        if ms_con:
            try:
                curs = ms_con.cursor()
                sql = "SELECT ACT_SET_M , ACT_OUT_M  FROM PBS410T WITH(NOLOCK) "\
    ...
    def get_late_date(self,ms_con,equ_code,start_day,start_time):
        ...
        if ms_con:
            ...
            try:
                curs = ms_con.cursor()
                # 생산계획 테이블에서 해당 설비, 해당 제품의 확정된 생산계획의 마지막 종료일자와 시간을 가져온다
                sql = " SELECT MAX(PLAN_END_DATE) AS END_DATE FROM PPL350T WITH(NOLOCK) " \
                      "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                ...
                if len(row) > 0:
                    ...
                    if end_date < start_day :
                        ...
                        # 최종 생산 시간
                        sql = " SELECT MAX(PLAN_END_TIME) AS SUM_TIME  FROM PPL350T WITH(NOLOCK) " \
                              "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
    ...
    def get_over_limit_time(self,ms_con,equ_code,end_date):
        ...
        if ms_con:
            try:
                ...
                sql = " SELECT OVER_TIME_YN FROM PBS510T WITH(NOLOCK) " \
                      "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
    ...
    def get_next_start_date_2(self,ms_con, equ_code,item_code,daily_plan,start_day,start_time):

        if ms_con:
            curs = ms_con.cursor()
            # 생산계획 테이블에서 확정된 해당 제품, 해당 설비의 제일 마지막 일자와 시간을 가져온다
            sql = "select max(plan_end_date) AS plan_end_date , max(plan_end_time) as plan_end_time from ppl350t WITH(NOLOCK) " \
                  "where comp_code = '{}' and div_code = '{}' and equ_code = '{}'  AND CONFIRM_YN in( 'Y','P' )" \
                  "and plan_end_date = ( select max(plan_end_date)  from ppl350t WITH(NOLOCK) " \
                  "where comp_code = '{}' and div_code = '{}' and equ_code = '{}' AND CONFIRM_YN in( 'Y','P' ) )" \
                .format(base.comp_code,base.div_code,equ_code,base.comp_code,base.div_code,equ_code)
            try:
                if base.DEBUG :
                ...
                if row[0] is not None:
                    ...
                    if last_end_date < start_day :
                        last_end_date = start_day
                ...
                pass
            if base.DEBUG:
                print(f"last_end_date : [{start_day}] [{last_end_date}]")
                ...
                if d_equ_code == equ_code :
                    ...
                    if d_end_date == str(last_end_date) :
                        ...
                        if d_end_time > last_end_time:
                            last_end_time = d_end_time
        if base.DEBUG:
            print(f"last_end_date :[{last_end_date}] [{last_end_time}]")
    ...
    def get_BATCH_PRODT_YN(self , ms_con, equ_code, item_code):
        ...
        if ms_con:
            ...
            curs = ms_con.cursor()
            sql = " SELECT BATCH_PRODT_YN FROM PBS410T WITH(NOLOCK) " \
                  "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
    ...
    def get_overtime_YN(self , ms_con, equ_code, day):
        ...
        if ms_con:
            ...
            sql = " SELECT over_time_yn FROM PBS510T WITH(NOLOCK) " \
                  "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                  "AND EQU_CODE = '{}' AND work_date = '{}'  " \
                .format(base.comp_code, base.div_code, equ_code, day)
            ...
            try:
                if batch_prodt_yn =='Y':  # 기본 시간 체크 공정인 경우 - 제조공정
                    ...
                    if diff == 'SAME_ITEM' :   # 바로 앞 제품이 같은 제품
                        sql = " SELECT MULTI_PRODT_CT as plan_time FROM PBS410T WITH(NOLOCK) " \
                              "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                              "AND EQU_CODE = '{}' AND WORK_SHOP_CODE = '{}'  " \
                              "AND PROG_WORK_CODE = '{}' AND ITEM_CODE = '{}' " \
                            .format(base.comp_code, base.div_code,equ_code, base.work_shop_code, base.prog_work_code,item_code)
                    ...
                    if diff == 'SAME_ITEM' :   # 바로 앞 제품이 같은 제품
                        sql = " SELECT ( net_ct_s * {} ) / 60 as plan_time FROM PBS410T WITH(NOLOCK) " \
                              "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                              "AND EQU_CODE = '{}' AND WORK_SHOP_CODE = '{}'  " \
                              "AND PROG_WORK_CODE = '{}' AND ITEM_CODE = '{}' " \
                        ...
                        sql = " SELECT ( ( net_ct_s * {} ) / 60 ) + act_set_m as plan_time FROM PBS410T WITH(NOLOCK) " \
                              "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                              "AND EQU_CODE = '{}' AND WORK_SHOP_CODE = '{}'  " \
                              "AND PROG_WORK_CODE = '{}' AND ITEM_CODE = '{}' " \
                            .format(plan_qty,base.comp_code, base.div_code,equ_code, base.work_shop_code, base.prog_work_code,item_code)
    ...
    def get_tomorrow_capa(self, ms_con, start_day, equ_code):
        ...
        if ms_con:
            ...
            sql = "SELECT WORK_DATE , (CASE WHEN OVER_TIME_YN = 'N' THEN STD_CAPA_TIME ELSE STD_CAPA_TIME + OVER_CAPA_TIME END) " \
                  "AS STD_CAPA_TIME  FROM PBS510T WITH(NOLOCK) " \
                  "WHERE COMP_CODE ='{}' AND DIV_CODE = '{}' AND EQU_CODE = '{}' AND WORK_DATE > '{}' AND WORK_TYPE = '2' " \
                  "and STD_CAPA_TIME > 0 "\
    ...
    def set_daily_plan(self,ms_con,plan_df,batch_prodt_yn):
        ...
        daily_plan = plan_df.sort_values( by=['equ_code','start_date','item_code'], ignore_index=True )
        ...
        if batch_prodt_yn == 'Y':   #제조
            ...
            for ii in range(len(daily_plan)):
                ...
                start_day = daily_plan.iloc[ii]['start_date']   # 시작일자
                start_time = daily_plan.iloc[ii]['start_time']   # 시작시간
                ...
                if equ_code != old_equ_code :  # 해당 설비 처음 생산 계획이면,
                    last_end_date , last_end_time = self.get_late_date(ms_con, equ_code,start_day,start_time) #그 전에 확정된 설비 생산계획의 제일 마지막 종료일자를 가져온다
                else :  # 설비가 같으면
            ...
            for ii in range(len(daily_plan)):
                ...
                start_day = daily_plan.iloc[ii]['start_date']   # 주문 수량
                start_time = daily_plan.iloc[ii]['start_time']   # 주문 수량
                ...
                while remain_qty > 0 :
                    ...
                    start_day , use_time = self.get_next_start_date_2(ms_con, equ_code,item_code,plan_table,start_day,start_time)
                    # 해당 날짜의 총 작업 시간  450
                    ...
                    if remain_plan_time <= over_limit_time  :
        ...
        # print(plan_table)
        if base.DEBUG :
            plan_table.to_excel('daily_plan_'+base.aps_key+ '_' +base.work_shop_code+'.xlsx', sheet_name='daily_plan', index=False)

        return plan_table

    def get_erp_start_day(self,ms_con,item_code):

        if ms_con:
            curs = ms_con.cursor()
            sql ="select prodt_end_date , prodt_time from t_ppl100t WITH(NOLOCK) " \
                 "where comp_code = '{}' and div_code = '{}' and work_shop_code = '{}' " \
                 "and item_code = '{}' and schedule_num = '{}' AND ORDER_NUM = '{}' AND SEQ = '{}' ".\
                format(base.comp_code,base.div_code,base.work_shop_code,item_code,base.schedule_num,base.order_num,base.order_seq)
            curs.execute(sql)
            row = curs.fetchone()
            if base.DEBUG:
                print(f"query erp_start_day : [{base.work_shop_code}] [{item_code}] [{base.schedule_num}] [{base.order_num}] [{row}] ")
            if row[0] is not None:
                start_day = row[0]
                start_time = row[1]
            else:
    ...
    def get_plan_start_day(self, ms_con, equ_code):

        if ms_con:
            curs = ms_con.cursor()
            sql = "select max(plan_end_date) , max(plan_end_time) from ppl350t WITH(NOLOCK) " \
                  "where comp_code = '{}' and div_code = '{}' and work_shop_code = '{}' " \
                  "and equ_code = '{}' and confirm_yn in ( 'Y','P')" \
                  "and plan_end_date = " \
                  "( select max(plan_end_date) from ppl350t WITH(NOLOCK) " \
                  "where comp_code = '{}' and div_code = '{}' and work_shop_code = '{}' " \
                  "and equ_code = '{}' and confirm_yn in ( 'Y','P' ) )". \
                format(base.comp_code, base.div_code, base.work_shop_code, equ_code,base.comp_code, base.div_code, base.work_shop_code, equ_code)
            curs.execute(sql)
            row = curs.fetchone()
            if row[0] is not None:  #
                start_day = row[0]
    ...
    def get_cal_start_day(self, ms_con, equ_code, start_day):

        if ms_con:
            curs = ms_con.cursor()
            sql = "select work_date , " \
                  "(case when over_time_yn = 'N' then std_capa_time else std_capa_time + over_capa_time end) " \
                  "as std_capa_time  , over_time_yn from pbs510t WITH(NOLOCK) " \
                  "where comp_code = '{}' and div_code = '{}' and work_date >= '{}' " \
                  "and equ_code = '{}' and work_type ='2' and std_capa_time > 0  ". \
                format(base.comp_code, base.div_code,  start_day, equ_code)
            curs.execute(sql)
            row = curs.fetchone()
            if row[0] is not None: #
                start_day = row[0]
                ...
                # 시작 시간을 계산한다. 쿼리 값은 가용 합계 시간이므로, 기본 시간 - 가용시간 = 가능한 시작시간이 된다.
                if over_time == 'N':
                    start_time = base.DAY_CAPA - start_time
                else:
                    start_time = base.NIGHT_CAPA - start_time
            else:
                start_day = '00000000'
                start_time = 0
                over_time = 'N'
    ...
    def get_cal_next_day(self, ms_con, equ_code, start_day):
    ...
    def get_equ_start_day(self,ms_con, equ_code,item_code):

        if ms_con:
            erp_start_day ,erp_start_time = self.get_erp_start_day(ms_con,item_code) # 오메가에서 제공하는 해당 아이템 가능한 시작시간
            plan_start_day ,plan_start_time = self.get_plan_start_day(ms_con,equ_code) # 생산계획에서 해당 설비의 가능한 시작시간

            if base.DEBUG:
                print(f"get_equ_start_day [{erp_start_day}] [{plan_start_day}] ")

            if erp_start_day == plan_start_day:
                start_day = erp_start_day
                if erp_start_time > plan_start_time :
                    start_time = erp_start_time
                else:
                    start_time = plan_start_time
            elif erp_start_day > plan_start_day:
                start_day = erp_start_day
                start_time = erp_start_time
            else:
                start_day = plan_start_day
                start_time = plan_start_time

            cal_start_day ,cal_start_time = self.get_cal_start_day(ms_con,equ_code,start_day) # 설비 캘린더에서 가능한 시작시간

            if start_day == cal_start_day:
                if start_time < cal_start_time :
                    start_time = cal_start_time
            elif start_day < cal_start_day:
                start_day = cal_start_day
                start_time = cal_start_time
            else:
                pass
            if base.DEBUG:
                print(f"erp start day : [{item_code}] [{erp_start_day}] [{erp_start_time}]")
                print(f"plan start day : [{equ_code}] [{plan_start_day}] [{plan_start_time}]")
                print(f"calendar start day : [{equ_code}] [{cal_start_day}] [{cal_start_time}]")

            # 위 세 시간중 가장 늦은 시간이 해당 설비에서 해당 아이템을 시작할 수 있는 가능한 시간

            # 해당 일자에 시작 가능한 시간이 근무시간 초과 되면( 주간인 경우 450 시간 이후, 야근인 경우 630 시간 이후이면,
            # 설비 캘린더에서 가능한 다음일자와 시간을 가져온다
            over_time_YN = self.get_overtime_YN(ms_con, equ_code, start_day)

            if over_time_YN == 'N':
                if start_time >= base.DAY_CAPA:
                    start_day,start_time = self.get_cal_next_day(ms_con, equ_code, start_day)
            else:
                if start_time >= base.NIGHT_CAPA:
                    start_day,start_time = self.get_cal_next_day(ms_con, equ_code, start_day)

            # 이번주 월요일
            this_monday = format(self.GetWeekFirstDate(datetime(self.this_year, self.this_month, self.this_day)), "%Y%m%d")

            # 시작일자가 이번주 월요일 전이면, 이번주 월요일 이후 설비 가능한 일자로 변경
            if start_day < this_monday:
                start_day = this_monday
                start_day ,start_time = self.get_cal_start_day(ms_con,equ_code,start_day) # 설비 캘린더에서 가능한 시작시간
    ...
    def split_first_plan(self,ms_con, assign_list, order_df, fac_capa_df):
            ...
            for i in range(assign_len):   # 할당된 제품과 설비 리스트
                ...
                if assign_ratio > 0.0:  # 제품별 ( 한 제품이 두 번 나올수 있음 )  최대값은 1 ( 한 제품 한 설비 ) 1보다 작으면 한 설비 이상에 할당된다.
                    ...
                    sql = "select std_prodt_q ,std_men, net_ct_s , act_set_m , act_out_m , batch_prodt_yn " \
                          "from pbs410t WITH(NOLOCK) where comp_code = '{}' and div_code = '{}' and work_shop_code = '{}' " \
                    ...
                    if batch_prodt_yn =='Y':   # 제조 공정
                        ...
                        equ_start_day, equ_start_time = self.get_equ_start_day(ms_con, equ_code, item_code)
                        ...
                        for pc in range(product_count):  # 설비 1회 가동시마다 계산
                            ...
                            plan_list = [base.comp_code, base.div_code, plan_no , order_no ,order_seq,base.work_shop_code ,
                                         base.prog_work_code, equ_code, base.aps_key ,item_code ,order_amount,
                                             due_date , plan_qty , plan_cnt , equ_start_day,equ_start_time,
                                             equ_start_day,equ_start_time,plan_time, cycle_time,act_set_m,act_out_m,
                                             work_men,base.validate ]
                            # print( plan_list )
                        ...
                        product_amount = round(order_amount * assign_ratio)  # 해당 설비 해당 배정된 양만큼의 제품 생산 수량 계산
                        equ_start_day , equ_start_time = self.get_equ_start_day(ms_con, equ_code,item_code)
                        equ_one_capa = self.get_daily_capa( ms_con, equ_code, cycle_time,equ_start_day,item_code)
                        if base.DEBUG:
                            print(f"equ one capa : [{equ_one_capa}]  [{equ_code}] [{cycle_time}]  [{item_code}]  [{equ_start_day}]")
                        plan_time = int( product_amount / float(cycle_time) )
                        ...
                        plan_list = [base.comp_code, base.div_code, plan_no, order_no, order_seq,
                                     ...
                                     equ_start_day, equ_start_time,equ_start_day, equ_start_time, plan_time,
        ...
        if base.DEBUG :
            ...
            plan_df.to_excel('split_first_'+base.aps_key+ '_' +base.work_shop_code+'.xlsx', sheet_name='split_plan', index=False)
    ...
    def run_aps(self):
        ...
        if base.DEBUG :
            print(f"1.aps_key = {base.aps_key}")
            print(f"2.comp_code = {base.comp_code}")
            print(f"3.div_code = {base.div_code}")
            print(f"4.work_shop_code = {base.work_shop_code}")
            print(f"5.prog_work_code = {base.prog_work_code}")
            print(f"6.schedule_num = {base.schedule_num}")
            print(f"7.split_type = {base.split_type}")
            print(f"8.validate = {base.validate}")
            print(f"9.debug = {base.debug}")
            print(f"10.order_num = {base.order_num}")
            print(f"11.order_seq = {base.order_seq}")

        ms_con = self.connect_mssql()

        if ms_con:
            if base.split_type == 'C':
                var_type = 'Continuous'
            else:
                var_type = 'Binary'
            ...
            fac_capa_list ,equ_item_capa = self.get_fac_basecapa_list(ms_con,order_df,1) # 사용 가능한 전체 설비 리스트
        ...
        # 설비 capa 가 하루에 다 안 될 수 있기에, 1시간씩 누적하면서 조건을 만족하는지 loop
            ...
            while True:
                ...
                fac_capa_list, equ_item_capa = self.get_fac_basecapa_list(ms_con, order_df,repeat)  # 사용 가능한 전체 설비 리스트

                fac_capa_list = fac_capa_list.copy()

                all_capa = fac_capa_list['ONE_CAPA'].iloc[::] * repeat

                fac_capa_list.insert(len(fac_capa_list.columns), 'ALL_CAPA', all_capa)

                status , assign_list = self.cal_lp( order_df, fac_capa_list,equ_item_capa, fix_prod_0,var_type,repeat)
            ...
            final_assign = copy.deepcopy(assign_list)
...
# ============================= BALANCE CHECK
            ...
            if base.balance == 'Y':
                ...
                repeat = 1
                if base.DEBUG:
                    print("Balance EQU Start")
                max_loc = fac_capa_list['ALL_CAPA'].argmax()
                max_capa = fac_capa_list.loc[max_loc]['ALL_CAPA']
                fac_capa_list = fac_capa_list.copy()
                fac_capa_list.loc[max_loc , 'ALL_CAPA'] = round( max_capa * 0.95 )
                # print(f"adj loc : [{fac_capa_df.loc[max_loc]['ALL_CAPA']}]")
...
#============================= N E W
            ...
            plan_df = self.split_first_plan(ms_con, final_assign, order_df, fac_capa_list)
                ...
                sql = "INSERT INTO PPL350T (COMP_CODE, DIV_CODE, WK_PLAN_NUM, ORDER_NUM, SEQ, WORK_SHOP_CODE, EQU_CODE,"\
                      ...
                      " '{}' , '{}' , '{}' , '{}' , {} , {} , {} , {} ,{} ,'{}','{}' )"\
                        .format (base.comp_code,base.div_code,plan_df.iloc[ii]['plan_no'],plan_df.iloc[ii]['order_no'],
                                 ...
                                 plan_df.iloc[ii]['work_men'],base.validate ,plan_df.iloc[ii]['aps_key'])
                if base.DEBUG:
                    print(sql)
                curs.execute(sql)
            ms_con.commit()
...
if __name__ == "__main__":
    ...
    split_type =  sys.argv[7]   # 이것도 필요함 . 제조 는 'B'  , 포장은 'C' 가 될 수 있음
    validate = sys.argv[8]      # 이것도 일단 필요함  . 추후 사용 예정 ( 'Y','N','P' ) - 일단은 'Y' 나 'P' 만 사용
    # remaining_time = sys.argv[10]      # 다음날로 이월이 가능한 최소 근무 근무시간
    debug = sys.argv[9]      # debug 용 , default = 'N'
    order_num = sys.argv[10]  # t_ppl100t   order_num
    order_seq = sys.argv[11]  # t_ppl100t   order_seq
    start_day = sys.argv[12]  # t_ppl100t   order_seq

    # if base.DEBUG:
    print(f"\n\n sys.argv  {sys.argv[1]} {sys.argv[2]} {sys.argv[3]} {sys.argv[4]} {sys.argv[5]}"
              f" {sys.argv[6]} {sys.argv[7]}  {sys.argv[8]} {sys.argv[9]} {sys.argv[10]} {sys.argv[11]} \n\n")
