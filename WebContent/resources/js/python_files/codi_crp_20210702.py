...
class CODI_LP():
    ...
    def __init__(self):
        ...
        self.cycle_time = 0  # 참고용, 실행시킨 이후 수신받은 틱 수
    ...
    def get_std_capa_time(self,ms_con,equ_code,start_date):
        ...
        if ms_con:
            ...
            if row == None:
                ...
                std_capa = SetDefault.MIXER_DAY_CAPA  #테이블에서 못 읽은 경우에 self.get_base_capa()
                return std_capa
    ...
    def get_next_start_date(self,ms_con, equ_code,end_date,total_time,plan_time,item_code):
        ...
        if total_time + plan_time > std_capa_time + over_limit_time :  # 현재 날짜 capa + 허용 오버시간 보다 생산시간이 크면
            ...
            end_time = self.get_plan_time(ms_con, equ_code,item_code, 'DIFF')
    ...
    def  get_plan_time(self,ms_con, equ_code,item_code,diff):
        ...
        if ms_con:
            ...
            if batch_prodt_yn =='N':  # 기본 시간 체크 공정인 경우
                # 생산계획 테이블에서 해당 설비, 해당 제품의 확정된 생산계획의 마지막 종료일자와 시간을 가져온다
                ...
                if diff == 'SAME_ITEM' :   # 바로 앞 제품이 같은 제품
                    sql = " SELECT ( net_ct_s * std_prodt_q ) / 60 as plan_time FROM PBS410T " \
                          "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                          "AND EQU_CODE = '{}' AND WORK_SHOP_CODE = '{}'  " \
                          "AND PROG_WORK_CODE = '{}' AND ITEM_CODE = '{}' " \
                        .format(base.comp_code, base.div_code,equ_code, base.work_shop_code, base.prog_work_code,item_code)
                else:    # 바로 앞 제품이 다른 제품
                    sql = " SELECT ( act_ct_s * std_prodt_q ) / 60 as plan_time FROM PBS410T " \
                          "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                          "AND EQU_CODE = '{}' AND WORK_SHOP_CODE = '{}'  " \
                          "AND PROG_WORK_CODE = '{}' AND ITEM_CODE = '{}' " \
                        .format(base.comp_code, base.div_code,equ_code, base.work_shop_code, base.prog_work_code,item_code)
            ...
            try:
                ...
                plan_time = SetDefault.MIXER_TIME  # 오류시 기본 시간으로 계산
    ...
    def set_daily_plan(self,ms_con,plan_df):
        ...
        for ii in range(len(daily_plan)):
            ...
            if ii == 0:  # 처음 시작은 무조건 다른 제품
                plan_time = self.get_plan_time(ms_con,  equ_code, item_code, 'DIFF')
            else:

                if daily_plan.iloc[ii - 1]['item_code'] == item_code:
                    plan_time = self.get_plan_time(ms_con,  equ_code,item_code, 'SAME_ITEM')
                else:
                    plan_time = self.get_plan_time(ms_con,equ_code,item_code, 'DIFF')
            ...
            start_date, start_time , end_date, end_time = \
                self.get_next_start_date(ms_con, equ_code, last_end_date,last_end_time,plan_time,item_code)
