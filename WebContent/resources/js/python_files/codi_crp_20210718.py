...
class SetDefault():
    ...
    ITEM_CHNG_TIME = 30   # 제품 교체 청소 시간   ( DB 에 없을 경우 사용 )

    def __init__(self,aps_key, comp_code, div_code, work_shop_code, prog_work_code,
                        split_type ,validate, schedule_num,base_start_day,debug):
...
class CODI_LP():
    ...
    def cal_lp(self,  order_table, fac_capa, fix_prod_0 ,fix_prod_1, var_type):
    ...
    def get_fac_basecapa_list(self,ms_con):
        ...
        if ms_con:
            ...
            sql = "SELECT DISTINCT WORK_SHOP_CODE, EQU_CODE , STD_MEN, NET_CT_S, ACT_SET_M, STD_PRODT_Q as ONE_CAPA FROM PBS410T "\
                  "WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' AND WORK_SHOP_CODE = '{}' " \
        ...
        for ii in range(len(order_df)):   ## 오더 제품 전체
            ...
            sql = "SELECT  equ_code FROM pbs410t WHERE COMP_CODE = '{}' AND DIV_CODE = '{}' " \
                  "AND WORK_SHOP_CODE = '{}' AND PROG_WORK_CODE = '{}' "\
                   "AND ITEM_CODE IN (SELECT ITEM_CODE FROM T_PPL100T WHERE SCHEDULE_NUM ='{}') " \
                   "EXCEPT " \
                    ...
                    for k in range(len(row)):
                        ...
                        for jj in range(len(fac_list)):
                            if fac_list.iloc[jj]['EQU_CODE'] == found_fac:
                                ...
                                if base.debug == 'Y':
                                    print( f"found fix 0 : {item_code} : [{found_fac}] [{ii+1}] [{jj+1}]  " )
            except:
                pass
        return fix_prod_0
            ...
            try:
                ...
                for jj in range(len(fac_list)):
                    if fac_list.iloc[jj]['EQU_CODE'] == found_fac:
                        ...
                        if base.debug == 'Y':
                            print( f"found fix 1 : {item_code} : [{found_fac}] [{ii+1}] [{jj+1}]  " )
            except:
                pass
        return fix_prod_1
    ...
    def get_setup_time(self,ms_con,equ_code,item_code):
        ...
        if ms_con:
            try:
                ...
                # (CASE WHEN over_time_yn = 'N' THEN std_capa_time ELSE over_capa_time END) as capa
                # FROM     PBS510T
                sql = "SELECT ACT_SET_M as SETUP_TIME FROM PBS410T "\
                ...
                # std_capa = int(row[0][0] )
                if base.debug == 'Y':
                    print(f" ===   setup time  OK : {row}   ===")
                    print(f" row[0][0]  : {row[0][0]}")
                return row[0][0]
            except:
                if base.debug == 'Y':
                    print(f" ===  setup time  None : {row}   ===")
    ...
    def get_daily_capa(self, ms_con, equ_code, cycle_time , start_date,item_code):
        ...
        setup_time = self.get_setup_time( ms_con, equ_code, item_code)
        work_time = ( work_time - int(setup_time) ) * 60    # 분을 초로 환산  27,000  450분 ( 준비시간 제외 )
    ...
    def set_daily_plan(self,ms_con,plan_df,batch_prodt_yn):
        ...
        plan_table = pd.DataFrame( columns = ("comp_code","div_code","plan_no","order_no","order_seq","work_shop_code",
        ...
        if batch_prodt_yn == 'Y':
            ...
            for ii in range(len(daily_plan)):
                ...
                while remain_qty > 0 :
                    # 작업 할 수 있는 가장 늦은 작업 일자와 사용 시간을 가져온다
                    start_day , use_time = self.get_next_start_date_2(ms_con, equ_code,item_code,plan_table)
                    # 해당 날짜의 총 작업 시간  450 분
                    work_time = self.get_std_capa_time( ms_con,equ_code,start_day)
                    # 해당 공정에 준비 시간 가져옴
                    setup_time = self.get_setup_time(ms_con,equ_code,item_code)
                    # 해당 작업일에 남아 있는 시간 계산
                    if old_item_code != item_code and old_equ_code == equ_code:
                        # 같은 설비에 품목이 교체 되면, 품목 교체 시간을 SETUP 시간에 더 해준다
                        use_time = use_time + base.ITEM_CHNG_TIME # 품목 교체 시간
                    remain_plan_time = work_time - use_time

                    if base.debug == 'Y':
                        print(f" Time : [{equ_code}] [{item_code}] [{start_day}] [{use_time}] [{work_time}] [{setup_time}]")
                    # 남은 시간이 준비시간 보다 적으면 날짜를 해당 설비 다음 일자로 , 생산 가능 시간은 다음 날 전체 시간 - 준비시간
                    # 생산 가능 시간 = plan_time
                    if remain_plan_time <= setup_time  :
                        start_day , work_time  = self.get_tomorrow_capa(ms_con,start_day,equ_code)
                        plan_time = work_time - setup_time
                        start_plan_time = 0
                    else:
                        plan_time = remain_plan_time - setup_time
                    ...
                    if plan_qty > remain_qty:
                        plan_qty = remain_qty
                        plan_time = int ( ( remain_qty  * cycle_time ) / 60 )
                        remain_qty = 0
                    else:
                        plan_time = int ( ( plan_qty  * cycle_time ) / 60 )
                    ...
                    end_plan_time = start_plan_time + plan_time

                    plan_list = [base.comp_code, base.div_code, plan_no, order_no, order_seq, base.work_shop_code,
                                 base.prog_work_code, equ_code, base.aps_key, item_code, order_amount,
                                 due_date, plan_qty, plan_cnt, start_day, start_plan_time,
                                 start_day, end_plan_time, plan_time, cycle_time, work_men, 'P']
    ...
    def split_first_plan(self,ms_con, assign_list, order_df, fac_capa_df):
        ...
        plan_df = pd.DataFrame( columns = ("comp_code","div_code","plan_no","order_no","order_seq","work_shop_code","prog_work_code",
        ...
        if assign_len > 0:
            ...
            for i in range(assign_len):   # 할당된 제품과 설비 리스트
                ...
                if assign_ratio > 0.0:  # 제품별 ( 한 제품이 두 번 나올수 있음 )  최대값은 1 ( 한 제품 한 설비 ) 1보다 작으면 한 설비 이상에 할당된다.
                    ...
                    setup_time = fac_capa_df.iloc[equ_no]['ACT_SET_M']
                    ...
                    if batch_prodt_yn =='Y':   # 제조 공정
                        ...
                        for pc in range(product_count):  # 설비 1회 가동시마다 계산
                            ...
                            plan_list = [base.comp_code, base.div_code, plan_no , order_no ,order_seq,base.work_shop_code ,
                                         base.prog_work_code, equ_code, base.aps_key ,item_code ,order_amount,
                                             ...
                                             base.base_start_day,0,plan_time, cycle_time, work_men,'P' ]
                            # print( plan_list )
                        ...
                        plan_list = [base.comp_code, base.div_code, plan_no, order_no, order_seq,
                                     ...
                                     base.base_start_day, 0,base.base_start_day, 0, plan_time, cycle_time, work_men, 'P']
                        # print( plan_list )
    ...
    def run_aps(self):
        ...
        # 설비 capa 가 하루에 다 안 될 수 있기에, 1시간씩 누적하면서 조건을 만족하는지 loop
            ...
            while True:
                ...
                status , assign_list = self.cal_lp( order_df, fac_capa_df, fix_prod_0,fix_prod_1,var_type)
            ...
            for ii in range(len(plan_df)):
                sql = "INSERT INTO PPL350T (COMP_CODE, DIV_CODE, WK_PLAN_NUM, ORDER_NUM, SEQ, WORK_SHOP_CODE, EQU_CODE,"\
                      ...
                      "VALUES ( '{}' , '{}' , '{}' , '{}' , {} , '{}' , '{}' , '{}' , '{}' , {} , '{}' , {} , {}, "\
                      " '{}' , '{}' , '{}' , '{}' , {} , {} , {} , '{}' )"\
                        .format (base.comp_code,base.div_code,plan_df.iloc[ii]['plan_no'],plan_df.iloc[ii]['order_no'],plan_df.iloc[ii]['order_seq'],plan_df.iloc[ii]['work_shop_code'],
...
if __name__ == "__main__":
    ...
    debug = sys.argv[10]      # debug 용 , default = 'N'

    print(f" sys.argv  {sys.argv[1]} {sys.argv[2]} {sys.argv[3]} {sys.argv[4]} {sys.argv[5]}"
          f" {sys.argv[6]} {sys.argv[7]}  {sys.argv[8]} {sys.argv[9]} {sys.argv[10]}")
