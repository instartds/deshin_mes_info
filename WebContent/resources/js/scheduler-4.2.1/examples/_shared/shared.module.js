import { LocaleManager, Localizable, AsyncHelper, AjaxHelper, DomHelper, Button, Popup, Tooltip, Toast, Widget, BrowserHelper, EventHelper, ResizeHelper, VersionHelper, DataGenerator, Fullscreen, Events, GlobalEvents, Rectangle } from '../../build/scheduler.module.js';
...
var siteUrl = window.location.href;
	siteUrl = siteUrl.substring(0,siteUrl.indexOf('/prodt/'))
const
    ...
    ignoreUserActionsRe = new RegExp(`(${ignoreUserActionsDemos.join('|')})`);
...
class Shared extends Localizable(Events()) {
    ...
    loadHints() {
        ...
        AjaxHelper.get(siteUrl + '/resources/js/scheduler-4.2.1/examples/groupsummary/meta/hints.json', { parseJson : true }).then(response => {
    ...
    loadDescription() {
        const
            ...
            url    = siteUrl + '/resources/js/scheduler-4.2.1/examples/groupsummary/app.config.json';
    ...
    notifyIfLaterVersionExists(response) {
        const latestVersion = response.parsedJson && response.parsedJson.name;
...
setTimeout(async() => {
    ...
    window.taskboard = bryntum.query('taskboard', true);
