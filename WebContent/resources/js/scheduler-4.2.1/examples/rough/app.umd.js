...
var scheduler = new Scheduler({
  ...
  listeners: {
    ...
    renderEvent: function renderEvent(_ref) {
      var scheduler = _ref.source,
      ...
      var roughData = element.roughData,
          _ref2 = roughData || {},
          canvas = _ref2.canvas,
          roughCanvas = _ref2.roughCanvas,
          isNew = false;

      var width = renderData.width,
          height = renderData.height; // Create a rough.canvas upon first rendering the event, storing it for later retrieval
      ...
      if (!canvas) {
        ...
        roughCanvas = roughData.roughCanvas = rough.canvas(canvas); // New canvas, no need to clear it further down
      ...
      if (roughData.width !== width || roughData.height !== height || roughData.eventId !== eventRecord.id) {
        ...
        canvas.style.height = height + 'px'; // Clear anything previously drawn, unless it is a brand new canvas
