import { ResourceModel, EventModel, DateHelper, Scheduler, Toast, MessageDialog, StringHelper } from '../../build/scheduler.module.js';
    ...
    listeners : {
        ...
        async beforeEventDropFinalize({ source : scheduler, context }) {
            if (scheduler.confirmationsEnabled) {
                ...
                const
                    namesInQuotes = context.draggedRecords.map(eventRecord => `"${StringHelper.encodeHtml(eventRecord.name)}"`),
