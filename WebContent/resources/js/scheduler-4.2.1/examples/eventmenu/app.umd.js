...
var scheduler = new Scheduler({
    ...
    eventMenu: {
      ...
      processItems: function processItems(_ref5) {
        ...
        if (eventRecord.eventType === 'activity') {
          ...
          items.editEvent = false; // Add a "Done" item
  ...
  eventRenderer: function eventRenderer(_ref8) {
    var eventRecord = _ref8.eventRecord,
        renderData = _ref8.renderData;
    var status = ''; // Canceled meetings have their own styling and icon
    ...
    return "".concat(status).concat(StringHelper.encodeHtml(eventRecord.name));
  }
}); // Loading data using Ajax, as an alternative to using our stores or CrudManager to load

scheduler.maskBody('Loading JSON data');
AjaxHelper.get('data/data.json', {
  parseJson: true
}).then(function (response) {
  var data = response.parsedJson;
