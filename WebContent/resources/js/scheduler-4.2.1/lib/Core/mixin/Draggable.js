...
export default Target => class Draggable extends (Target || Base) {
    ...
    beforeDrag(drag) {
        ...
        return !dragSelector || Boolean(drag.element.closest(dragSelector));
    }
    ...
    updateOverItem(overItem, oldOverItem) {
        const { dragItemOverCls } = this;

        if (oldOverItem) {
            dragItemOverCls && oldOverItem.classList.remove(dragItemOverCls);
            this.onDragItemMouseLeave?.(this.enterLeaveEvent, oldOverItem);
        }
        if (overItem) {
            dragItemOverCls && overItem.classList.add(dragItemOverCls);
            this.onDragItemMouseEnter?.(this.enterLeaveEvent, overItem);
        }
    }
    ...
    onDragPointerDown(event) {
        ...
        if (!drag) {
            ...
            // If dragging is to begin, begin it.
            // The Context consults our beforeDrag handler, and if that succeeds, the Context
            // injects itself into here as our draggable.
            if (drag) {
                new DragContext(drag).begin();
            }
        }
