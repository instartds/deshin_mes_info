...
export default class DragHelper extends Base.mixin(Events, DragHelperContainer, DragHelperTranslate) {
    ...
    onPointerDown(event) {
        ...
        const handled = me.mode === 'container' ? me.grabContainerDrag(event) : me.mode.startsWith('translate') ? me.grabTranslateDrag(event) : false;
            ...
            if (!context.started) {
                if (me.trigger('beforeDragStart', { context, event }) !== false) {
                    ...
                    me.scrollManager?.startMonitoring(ObjectHelper.merge({
                        ...
                        callback : config => {
                            if (context?.element && me.lastMouseMoveEvent) {
                    ...
                    context.outermostEl = (event.target.closest('.b-outer') || document.body);
    ...
    update(event, scrollManagerConfig) {
        ...
        let target = !event.isScroll ? event.target : DomHelper.elementFromPoint(event.clientX, event.clientY);
