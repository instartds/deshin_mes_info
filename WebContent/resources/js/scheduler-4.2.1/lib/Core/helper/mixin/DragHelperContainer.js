...
export default Target => class DragHelperContainer extends (Target || Base) {
    ...
    startContainerDrag(event) {
        ...
        (floatRootOwner?.floatRoot || DomHelper.getRootElement(dragElement)).appendChild(div);
    ...
    updateContainerDrag(event) {
        ...
        const
            containerElement = DomHelper.getAncestor(context.targetElement, me.containers, 'b-grid'),
            willLoseFocus    = context.dragging && context.dragging.contains(DomHelper.getActiveElement(context.dragging));
