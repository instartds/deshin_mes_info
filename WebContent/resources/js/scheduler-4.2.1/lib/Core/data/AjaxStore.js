 ...
 *
 * ## Update
 * Posts array of JSON data containing modified records to {@link #config-updateUrl}. By default, only changed fields and
 * any fields configured with {@link Core.data.field.DataField#config-alwaysWrite} are sent.
...
export default class AjaxStore extends Store {
    ...
    internalLoad(params, eventName, successFn) {
        const
            me        = this,
            allParams = ObjectHelper.assign({}, me.params, params),
            url       = me.buildReadUrl(),
            event     = { action : 'read' + eventName, params : allParams, url };
        ...
        return url ? new Promise((resolve, reject) => {
            ...
            AjaxHelper.get(event.url + me.buildQueryString(event.url, allParams), ObjectHelper.assign(options, me.fetchOptions))
                .then(async response => {
                    const
                    ...
                    if (success) {
                        ...
                        if (me.isPaged) {
                            if (me.remoteTotal >= 0) {
                                const
                                    requestedPage = me.pageParamName ? allParams[me.pageParamName] : allParams[me.pageStartParamName] / me.pageSize + 1;
    ...
    async loadChildren(parentRecord) {
        return this.internalLoad({ [this.parentIdParamName] : parentRecord.id }, 'Children', event => {
    ...
    async commit() {
        ...
        if (!changes) {
            ...
            if (me.modified.count) {
                me.modified.forEach(record => record.clearChanges(false));
                me.modified.clear();
            }
    ...
    commitAdded(commitState) {
        ...
        return added.count && me.createUrl ? new Promise((resolve) => {
            const
                toAdd          = added.values.map(r => r.persistableData);
            ...
            AjaxHelper.post(me.createUrl + me.buildQueryString(me.createUrl, me.params), dataToSend, ObjectHelper.assign(options, me.fetchOptions)).then(response => {
                const
                    data    = response.parsedJson,
                    isArray = Array.isArray(data),
            ...
            result       = modified.length && me.updateUrl ? new Promise(resolve => {
                ...
                AjaxHelper.post(
                    ...
                    if (success) {
                        // Copy updated fields and updated ID back into records.
                        // This also calls clearChanges on each record.
                        me.processReturnedData(modified, isArray ? data : data[me.responseDataProperty], true);
