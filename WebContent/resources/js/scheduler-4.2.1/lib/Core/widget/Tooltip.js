...
export default class Tooltip extends Popup {
    ...
    updateAnchorToTarget(anchorToTarget) {
        if (!anchorToTarget) {
            this.anchor = false;
        }
    }
    ...
    async handleForElementOver(event, newTarget) {
        ...
        if (me.trigger('pointerOver', { event, target : newTarget }) === false) {
            ...
            // Abort any in-flight animated hide, and show immediately.
            // This is needed when allowOver is false (which is the default), and mouseovering
            // hides, but that immediately causes the mouse to be over another delegate
            // which jumps right in here.
            if (!me.hideOnDelegateChange && me.element.classList.contains('b-hiding')) {
            ...
            if ((abortedHide || me.isVisible) && !me.hideOnDelegateChange) {
                ...
                // updateContent may have yielded empty content which hides us
                if (!me.hidden) {
                    me.alignTo({
                ...
                if (me.hideOnDelegateChange) {
                    // Hide with no animation if we are configured to hide between delegates
                    me.hide(false);
                }
                me.updateActiveTarget(newTarget);
    ...
    get hasContent() {
        return Boolean(this._html !== '' && (typeof this.html === 'string' && this.html.length) || this.items.length);
    }
    ...
    changeHtml(htmlOrPromise) {
        ...
        if (htmlOrPromise === false) {
            VersionHelper.deprecate('Grid', '5.0.0', 'Setting false to indicate loading is deprecated, in favor of `showAsyncMessage`. Please see https://bryntum.com/docs/grid/#Grid/guides/upgrades/3.0.1.md for more information');
    ...
    onMouseMove(event) {
        ...
        if (!me._hidden) {
            ...
            const
                isElementOut = me.forSelector && me.activeTarget && !isHiding && (target.nodeType === Node.ELEMENT_NODE) && !target.matches(me.forSelector) && !(me.allowOver && me.element.contains(target)),
