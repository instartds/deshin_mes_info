...
export default class Header extends Bar {
    ...
    template() {
        ...
        return TemplateHelper.tpl`
            <div class="b-grid-header-scroller b-grid-header-scroller-${region}">
                <div data-reference="headersElement" class="b-grid-headers b-grid-headers-${region}" data-region="${region}" data-max-depth="${me.maxDepth}" data-reference="headersElement"></div>
            </div>
        `;
    }
    ...
    contentTemplate(column) {
        ...
        if (column.hidden) {
            ...
            /* eslint-disable */
            return TemplateHelper.tpl`
                <div class="b-grid-header" data-column="${column.field || ''}" data-column-id="${column.id}" ${column.isFocusable ? 'tabindex="0"' : ''} ${column.tooltip ? `data-btip="${column.tooltip}"` : ''}>
