...
export default class RowManager extends InstancePlugin {
    ...
    static get pluginConfig() {
        return {
            chain : [
                'getRowById', 'getRecordCoords', 'getRow', 'getRowFor', 'getRowFromElement', 'destroy'
            ],
            assign : [
                'rowHeight', 'topRow', 'bottomRow', 'firstVisibleRow', 'lastVisibleRow', 'firstFullyVisibleRow'
            ]
        };
    }
     ...
     */
    get firstVisibleRow() {
        return this.rows.find(r => r.bottom > this.grid.scrollable.y);
    }

    get firstFullyVisibleRow() {
        return this.rows.find(r => r.top >= this.grid.scrollable.y);
    }
