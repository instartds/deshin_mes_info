...
export default class RegionResize extends InstancePlugin {
    ...
    startMove(splitterElement, clientX) {
        ...
        const
            availableWidth = subGrid.element.offsetWidth + nextSubGrid.element.offsetWidth;
    ...
    onCollapseClick(subGrid, splitterEl) {
        ...
        if (regions[0] === region) {
            const
                lastSubGrid = grid.getSubGrid(regions[1]);
    ...
    onElementTouchStart(event) {
        const
            target = event.target.closest('.b-grid-splitter');
    ...
    onSubGridCollapse(subGrid) {
        const
            splitterEl = this.grid.resolveSplitter(subGrid),
