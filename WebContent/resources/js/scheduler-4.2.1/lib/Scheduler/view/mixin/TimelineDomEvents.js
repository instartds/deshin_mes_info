    ...
    onElementMouseButtonEvent(event) {
        ...
        if (target) {
            ...
            if (!contextMenu || event.type !== contextMenu.triggerEvent) {
                const column = this.isVertical ? this.verticalTimeAxisColumn : this.timeAxisColumn;
                // TODO 5.0 change to this.trigger
                column.trigger('timeAxisHeader' + StringHelper.capitalize(event.type), {
