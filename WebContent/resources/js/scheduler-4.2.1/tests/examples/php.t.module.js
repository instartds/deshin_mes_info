StartTest(t => {
    ...
    t.it('Dragging to a different resource should work', t => {
        t.chain(
            ...
            { waitFor : () => scheduler.eventStore.first.resourceId === 3, desc : 'Successfully dragged' },
    ...
    t.it('Dragging to the same resource should work', t => {
        t.chain(
            ...
            { waitFor : () => scheduler.eventStore.first.startDate - new Date(2018, 4, 21, 7) === 0, desc : 'Successfully dragged' },
            () => {
                t.selectorCountIs('.b-sch-event', '.b-scheduler', 9, 'Rendered elements match real number in the store');
            }
        );
    ...
    t.it('Creating a new event should work', t => {
        const
            elements   = Array.from(document.querySelectorAll('.b-sch-event-wrap')),
    ...
    t.it('Creating a new event, modifying and removing it and existing one should work', t => {
        ...
        t.chain(
            // Create Event
            { drag : '.b-sch-timeaxis-cell', fromOffset : [2, 260], by : [100, 0] },
            { type : 'Test' },
            {
