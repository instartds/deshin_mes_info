...
StartTest(t => {
    ...
    async function setup(config = {}) {
        ...
        scheduler.on({
            ...
            eventdragstart({ source, eventRecords }) {
                if (source instanceof Scheduler &&
                    eventRecords instanceof Array &&
                    eventRecords[0] instanceof EventModel) {
                    fired.eventdragstart++;
                }
            },
            eventdrop({ source, eventRecords, isCopy }) {
                if (source instanceof Scheduler &&
                    eventRecords instanceof Array &&
                    eventRecords[0] instanceof EventModel &&
                    // The 'isCopy' argument
    ...
    t.it('Clock in tooltip is updated on drag', async t => {
        ...
        t.chain(
            { drag : '.b-sch-event', by : [[step * 2, 0]], dragOnly : true },
            ...
            { mouseUp : null },
            { drag : '.b-milestone', by : [[step, 0]], dragOnly : true },
            { waitForSelector : '.b-sch-tip-valid' },
            next => {
