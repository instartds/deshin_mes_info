import { Scheduler, EventModel } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Basic use cases', t => {
        ...
        scheduler.on({
            beforeeventresize({ source, eventRecord, event }) {
                t.ok(
                    source instanceof Scheduler &&
                    eventRecord instanceof EventModel &&
                    event instanceof MouseEvent,
                    'Correct event signature of `beforeeventresize`'
                );

                fired.beforeeventresize++;
            },
            eventresizestart({ source, eventRecord }) {
            ...
            eventpartialresize({ source, eventRecord, startDate, endDate, element }) {
                if (fired.eventpartialresize === 0) {
                    t.ok(
                        ...
                        'Correct event signature of `eventpartialresize`'
                    );
        ...
        t.chain(
            ...
            { drag : '.event1', offset : ['100%-5', '50%'], by : [100, 0] },
            ...
            { drag : '.event1', offset : [3, 5], by : [-100, 0] },
            ...
            (next, el) => {
                ...
                record.resizable = 'end';

                // changing data currently rerenders events, creating a new element. cannot reuse el here
            ...
            { drag : '.event1', offset : ['100%-3', 5], by : [100, 0] },
            ...
            { moveMouseTo : '.event1', desc : 'Hover event node to force resize handles' },

            { doubleClick : '.event1', offset : ['100%-3', 5] }
        );
    });
    ...
    t.it('Should finalize if mouse only moves a little', t => {
        ...
        t.chain(
            ...
            { drag : '.event1', offset : ['100%-3', 5], by : [3, 0] }
        );
    });
    ...
    t.xit('Should handle moving mouse outside of scheduling view', t => {
        scheduler = t.getScheduler({
            ...
            eventStore : t.getEventStore({
                data : [
                    { id : 1, startDate : '2011-01-05', endDate : '2011-01-06', resourceId : 'r3' }
                ]
            }),
            features : {
                eventResize : {
        ...
        t.chain(
            ...
            { drag : '.b-sch-event', offset : [3, 8], by : [0, 1], dragOnly : true },
    ...
    t.it('Should not crash when clicking resizer element', t => {
        ...
        t.chain(
            { waitForEventsToRender : null },
            { moveMouseTo : '.b-sch-event' },
            { click : '.b-sch-event', offset : [3, '50%'], desc : 'Should not crash when clicking resizer element' }
        );
    });
});
