import { DateHelper, Rectangle } from '../../build/scheduler.module.js';
...
StartTest(async t => {
    ...
    let testSteps = [
        // moving forward in time
        {
            drag : () => {
                const
                    gridRectangle  = Rectangle.from(scheduler.timeAxisSubGridElement),
        ...
        next => {
            t.isApprox(
                DateHelper.getDurationInUnit(event.meta.modified.startDate, event.startDate, scheduler.timeAxis.mainUnit),
                1,
                1,
            ...
            t.isApprox(
                DateHelper.getDurationInUnit(event.meta.modified.endDate, event.endDate, scheduler.timeAxis.mainUnit),
                1,
                1,
                'Event moved approximately on 1 unit'
            );
