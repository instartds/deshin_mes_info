import { PresetManager, TimeAxisViewModel, TimeAxis, DateHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    t.it('forceFit and columns need to be fitted, weekAndDayLetter', t => {
        ...
        t.is(
            viewModel.getDistanceBetweenDates(colCfg[1][0].start, colCfg[1][6].end),
            viewModel.getDistanceBetweenDates(colCfg[0][0].start, colCfg[0][0].end),
    ...
    t.it('no forceFit but snap and the columns do not consume the available width => they should grow', t => {
        ...
        // Since columns consume all space, each day will be 1/7 of the width
    ...
    t.it('no forceFit but snap and the resolution is 1 day', t => {
        PresetManager.registerPreset('weekAndDayLetter', {
            tickWidth           : 139,  // Something not evenly divisible by 7
            rowHeight           : 24,
