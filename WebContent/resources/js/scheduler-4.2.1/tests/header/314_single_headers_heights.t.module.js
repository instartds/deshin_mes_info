import { PresetManager, DateHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.is(
        normalHeaderEl.offsetHeight,
        lockedHeaderEl.offsetHeight,
        'Both headers have the same height');

    // irrelevant, they are in the same div in vanilla
    ...
    t.isApprox(document.querySelector('.b-grid-body-container').offsetTop, lockedHeaderEl.offsetHeight, 'The body starts after header, not in the middle of it');
    ...
    t.is(
        document.querySelector('.b-grid-headers-normal').offsetTop,
        document.querySelector('.b-grid-headers-locked').offsetTop,
        'Both headers have the same height after preset switch'
    );

    // TODO: PORT ? should header height autoadjust? currently scheduler determines header height
    ...
    t.isApprox(
        document.querySelector('.b-grid-body-container').getBoundingClientRect().top,
        document.querySelector('.b-grid-headers-normal').getBoundingClientRect().height,
        'The body starts after header, not in the middle of it'
    );
});
