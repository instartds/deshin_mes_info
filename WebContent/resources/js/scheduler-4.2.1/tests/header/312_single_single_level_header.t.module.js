import { PresetManager } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should draw single header', t => {
        ...
        scheduler = t.getScheduler({
            ...
            columns : [
                ...
                { text : 'Id', sortable : true, width : 100, hidden : true, field : 'id', locked : true }
            ]
        });

        let headerScrollerElement = document.querySelector('.b-grid-header-scroller-normal'),
    ...
    t.it('Time axis header should fill header container', t => {
        ...
        t.isApprox(document.querySelector('.b-sch-header-timeaxis-cell').offsetHeight, document.querySelector('.b-sch-header-row').offsetHeight, 'Height of the timeaxis matches header container');
    ...
    t.it('Two level time axis header should fill header container', t => {
        ...
        let validateHeaderHeight = () => {
            t.isApprox(document.querySelector('.b-sch-header-row-1 .b-sch-header-timeaxis-cell').offsetHeight,
                Math.floor(scheduler.headerHeight / 2), 1, 'Height of the timeaxis matches top header container');

            t.isApprox(document.querySelector('.b-sch-header-row-0 .b-sch-header-timeaxis-cell').offsetHeight,
                Math.floor(scheduler.headerHeight / 2), 1, 'Height of the timeaxis matches middle header container');
        };
