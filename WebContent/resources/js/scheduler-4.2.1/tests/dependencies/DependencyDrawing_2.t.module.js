import { DomHelper, DependencyStore, Scheduler, ProjectModel, ArrayHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should refresh depdencies on scheduler resize', t => {
        ...
        t.chain(
            { waitForDependencies : null },
            {
                waitFor : () => {
                    const
                        eventBox      = t.rect('[data-event-id="1"]'),
                        [dependencyEl]  = t.query('[depId="1"]');

                    if (eventBox && dependencyEl) {
                        const box = t.getSVGBox(dependencyEl);
    ...
    t.it('Should draw dependency line if dependency dates are outside of the view', t => {
        scheduler = t.getScheduler({
            dependencyStore : t.getDependencyStore({
                data : [
                    { id : 1, from : 1, to : 2, fromSide : 'right', toSide : 'left' },
                    { id : 2, from : 1, to : 2, fromSide : 'top', toSide : 'top' },
                    { id : 3, from : 1, to : 2, fromSide : 'bottom', toSide : 'bottom' }
                ]
            }),
            eventStore : t.getEventStore({
                data : [
                    { id : 1, startDate : '2017-12-01 10:00', endDate : '2017-12-01 12:00', resourceId : 'r2' },
                    { id : 2, startDate : '2017-12-03 10:00', endDate : '2017-12-03 12:00', resourceId : 'r2' }
                ]
            }),
        ...
        scheduler = t.getScheduler({
            dependencyStore : t.getDependencyStore({
                data : [
                    ...
                    { id : 2, from : 1, to : 2, fromSide : 'top', toSide : 'top' },
                    { id : 3, from : 1, to : 2, fromSide : 'bottom', toSide : 'bottom' },
                    { id : 4, from : 1, to : 3, fromSide : 'left', toSide : 'right' },
                    { id : 5, from : 1, to : 3, fromSide : 'top', toSide : 'top' },
                    { id : 6, from : 1, to : 3, fromSide : 'bottom', toSide : 'bottom' }
                ]
            }),
            eventStore : t.getEventStore({
                data : [
                    { id : 1, startDate : '2017-12-02 10:00', endDate : '2017-12-02 12:00', resourceId : 'r2' },
                    { id : 2, startDate : '2017-12-03 10:00', endDate : '2017-12-03 10:00', resourceId : 'r2' },
                    { id : 3, startDate : '2017-12-01 10:00', endDate : '2017-12-01 10:00', resourceId : 'r2', iconCls : 'b-fa b-fa-search' }
                ]
            }),
            resourceStore : t.getResourceStore2({}, 2),
            features      : {
        ...
        t.chain(
            ...
            () => {
                ...
                DomHelper.forEachSelector(document, dependencySelector, el => {
                    ...
                    t.is(gotRect.y, expectedRects[id].y, `Top position is ok for dep ${id}`);
                    // Dep now goes to the likely position outside of view, either need to hardcode expectations or
    ...
    t.it('Should live update dependencies when dragging events', t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 'r1', name : 'A', startDate : new Date(2018, 8, 2), duration : 1, cls : 'event1' },
                { id : 2, resourceId : 'r1', name : 'B', endDate : new Date(2018, 8, 8), duration : 1, cls : 'event2' }
            ],
        ...
        t.chain(
            { drag : '.event2', by : [0, 300], dragOnly : true },
    ...
    t.it('Should remove dependency lines when records are removed from store', t => {
        scheduler = t.getScheduler({
            dependencyStore : t.getDependencyStore({
                data : [
                    { id : 1, from : 1, to : 2, fromSide : 'top', toSide : 'top' },
                    { id : 2, from : 1, to : 2, fromSide : 'bottom', toSide : 'bottom' },
                    { id : 3, from : 1, to : 3, fromSide : 'left', toSide : 'right' }
                ]
            }),
            eventStore : t.getEventStore({
                data : [
                    { id : 1, startDate : '2017-12-02 10:00', endDate : '2017-12-02 12:00', resourceId : 'r2' },
                    { id : 2, startDate : '2017-12-03 10:00', endDate : '2017-12-03 10:00', resourceId : 'r2' },
                    { id : 3, startDate : '2017-12-01 10:00', endDate : '2017-12-01 10:00', resourceId : 'r2' }
                ]
            }),
            resourceStore : t.getResourceStore2({}, 2),
            features      : {
    ...
    t.xit('Dependencies should be rendered properly after add/remove followed by redraw', t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 'r1', name : 'A', startDate : new Date(2018, 8, 2), duration : 1 },
                { id : 2, resourceId : 'r1', name : 'B', endDate : new Date(2018, 8, 8), duration : 1 }
            ],
    ...
    t.it('Dependency repainted properly when moving successor outside of the view', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            events : [
                { id : 1, resourceId : 1, name : 'Event 1', startDate : '2017-01-16', endDate : '2017-01-18' },
                { id : 2, resourceId : 1, name : 'Event 2', startDate : '2017-01-19', endDate : '2017-01-21' }
            ],
            dependencies : [
                { id : 1, from : 1, to : 2 }
            ],
        ...
        const
            dep = scheduler.dependencies[0],
            event = scheduler.eventStore.getById(2);

        let newStart, el, box, newLeft, refreshDependencySpy;

        t.chain(
            // First dependency draw must take place for us to begin test
            { waitForDependencies : null },
            ...
            },
            {
                ...
                waitFor : () => refreshDependencySpy.calls.count() === 2,
                desc    : 'First refresh'
            },
            async() => {
            ...
            },
            {
                ...
                waitFor : () => refreshDependencySpy.calls.count() === 4
            },
            () => {
                // New rendering positions it "correctly" outside of view
                newLeft = /*scheduler.timeAxisViewModel.totalSize*/2560 + scheduler.timeAxisSubGridElement.getBoundingClientRect().left;
