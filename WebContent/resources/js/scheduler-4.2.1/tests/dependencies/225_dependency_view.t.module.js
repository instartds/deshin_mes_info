import { BrowserHelper, EventStore, ArrayHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should maintain a steady number of drawn dependencies on scroll', t => {
        ...
        let lineCount, drawnDepLen;

        // The line count must remain stable during scroll
        t.chain(
    ...
    !BrowserHelper.isIE11 && t.it('Should fire dependencymouseover/dependencymouseout events', t => {
        ...
        t.chain(
            ...
            { moveCursorTo : '.b-sch-dependency', offset : [-5, '50%'] },
    ...
    t.it('Dependency painted ok on the last rendered row', async t => {
        scheduler = await t.getSchedulerAsync({
            events       : [],
            resources    : ArrayHelper.populate(50, i =>  ({ id : i + 1, name : i + 1 })),
            dependencies : [],
            features     : {
                eventTooltip : false,
                dependencies : true
        ...
        t.chain(
...

            {
                waitFor : () => {
                    const
                        dependency = document.querySelector('.b-sch-dependency'),
    ...
    t.it('Should not draw dependency to missing event', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            events : [
                { id : 1, resourceId : 1, name : 'A', startDate : '2011-01-06', endDate : '2011-01-08' },
                { id : 2, resourceId : 1, name : 'B', startDate : '2011-01-10', endDate : '2011-01-12' }
            ]
        }, 25);
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : '.b-sch-dependency:not(.b-sch-released)', desc : 'Dependency not drawn' }
        );
    });
    ...
    !BrowserHelper.isIE11 && t.it('Should show tooltip when hovering a dependency line', t => {
        ...
        t.chain(
            ...
            { moveCursorTo : '.b-sch-dependency', offset : ['50%-3', '50%-3'] },
    ...
    t.it('Should not crash nor draw dependency to event of non-existing resource', t => {
        scheduler = t.getScheduler({
            ...
            resources : [
                { id : 1, name : 'Dude' },
                { id : 2, name : 'Other Dude' }
            ],
            events : [
                { id : 1, resourceId : 1, name : 'A', startDate : '2011-01-06', endDate : '2011-01-08' },
                { id : 2, resourceId : 2, name : 'A', startDate : '2011-01-06', endDate : '2011-01-08' }
            ]
        });
    ...
    t.it('Should be possible to disable creating dependencies while still drawing them', t => {
        scheduler = t.getScheduler({
            ...
            events : [
                { id : 1, resourceId : 1, cls : 'foo', startDate : '2011-01-06', endDate : '2011-01-08' },
                { id : 2, resourceId : 1, cls : 'bar', startDate : '2011-01-06', endDate : '2011-01-08' }
            ]
        });
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : '.b-sch-terminal', desc : 'Dependency terminals not shown' },
            ...
            { waitForSelector : '.b-sch-terminal', desc : 'Dependency terminals hown' }
        );
    });
    ...
    t.it('Should respect Scheduler readOnly state', t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : '.b-sch-terminal', desc : 'Dependency terminals not shown when Scheduler is readOnly' },
            ...
            { waitForSelector : '.b-sch-terminal', desc : 'Dependency terminals shown when Scheduler is not readOnly' }
        );
    });
    ...
    t.it('Should clear dependencies when event store is cleared', t => {
        ...
        t.chain(
            ...
            next => {
                scheduler.eventStore.add({ resourceId : 'r1', startDate : '2011-01-04', endDate : '2011-01-06' });
                scheduler.eventStore.add({ resourceId : 'r2', startDate : '2011-01-04', endDate : '2011-01-06' });
    ...
    t.it('Should not be affected by XSS injection', t => {
        ...
        t.chain(
            { waitForDependencies : null },
            async() => applyPointerHook,
            { moveCursorTo : '.b-sch-dependency[depId="1"]', offset : [0, '50%'] },
            { moveCursorTo : '.b-sch-dependency[depId="2"]', offset : [0, '50%'] },
            () =>  document.head.removeChild(style)
        );
    });
});
