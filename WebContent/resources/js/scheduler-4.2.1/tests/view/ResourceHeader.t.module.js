import { Rectangle, DomHelper, EventHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    async function waitForHeader(t, resourceId, left) {
        await t.waitFor(() => {
            const
                headerElement = document.querySelector(`.b-resourceheader-cell[data-resource-id="${resourceId}"]:not(.b-released)`),
    ...
    t.it('Displaying icon', async t => {
        scheduler = await t.getVerticalSchedulerAsync({
            resources : [
                { id : 'r1', iconCls : 'b-fa b-fa-bus', name : 'Bus' },
                { id : 'r2', iconCls : 'b-fa b-fa-car', name : 'Car' }
            ]
        });
    ...
    t.it('headerRenderer config, as template', async t => {
        scheduler = await t.getVerticalSchedulerAsync({
            resourceColumns : {
                headerRenderer : ({ resourceRecord }) => `<div>${resourceRecord.id}</div>`
            }
    ...
    t.it('Filling available width', async t => {
        // TODO: Fix IE11 test
        // https://github.com/bryntum/bryntum-suite/issues/2266
        ...
        t.chain(
            // FFs ResizeObserver seems to be triggered async somehow, have to give it time to settle
            { waitFor : 100 },
            ...
            next => {
                assertHeaderElement(t, 'r2', 100, 100);
                assertEventElement(t, 3, 100, 300, 100, 200);
    ...
    t.it('Fitting available width', async t => {
        ...
        scheduler.resourceStore.add([
            { id : 'r10', name : 'Resource 10' },
            { id : 'r11', name : 'Resource 11' }
        ]);
    ...
    t.it('Should not instantly reload images with invalid resourceImagePath or defaultResourceImageName', async t => {
        ...
        const checkResourceImages = (resourceImagePath, defaultResourceImageName, resourceImageExtension, resourceName, checkImageName, expectedNbrOfErrorEvents) => [
            async() => {
                errorCount = 0;
