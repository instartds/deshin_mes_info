...
StartTest(t => {
    ...
    t.it('Should handle timeAxis#include#day for "larger" ticks', async t => {
        ...
        t.it('Should stretch to fill excluded space, Su, Th, Fr, Sa', async t => {
            await getScheduler({
                timeAxis : {
                    include : {
                        day : { from : 1, to : 4 } // Exclude Su, Th, Fr, Sa -> Showing Mo, Tu, We
                    }
                },
    ...
    t.it('Should handle weekStartDay = 1', async t => {
        ...
        t.it('Should not draw fully excluded events', async t => {
            await getSch({
                events : [
                    { id : 'sunday', resourceId : 'r1', name : 'Sunday', startDate : new Date(2019, 1, 17), duration : 24, durationUnit : 'h' },
                    { id : 'saturday', resourceId : 'r1', name : 'Saturday', startDate : new Date(2019, 1, 16), duration : 24, durationUnit : 'h' },
                    { id : 'weekend', resourceId : 'r1', name : 'Weekend', startDate : new Date(2019, 1, 16), duration : 48, durationUnit : 'h' }
                ]
            });
    ...
    t.it('Should reconfigure time axis when applying working time', async t => {
        ...
        scheduler.workingTime  = { fromDay : 1, toDay : 6 };
    ...
    t.it('Should update header when changing working time config', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            events     : [
                { id : 1, resourceId : 'r1', startDate : '2021-04-28 12:00', endDate : '2021-04-28 14:00' }
            ]
        });
