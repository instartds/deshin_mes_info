import { Scheduler, DateHelper, Rectangle, DomHelper, TabPanel, ArrayHelper, Popup, CSSHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Animation state should persist until the last requested animated update has finished', async t => {
        ...
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '2018-01-01', endDate : '2018-01-03' },
                { id : 2, resourceId : 2, startDate : '2018-01-02', endDate : '2018-01-04' }
            ],
    ...
    t.it('Horizontal scroll: Virtual event rendering', async t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '2018-01-01', endDate : '2018-01-03' },
                { id : 2, resourceId : 1, startDate : '2018-03-01', endDate : '2018-03-03' }
            ],
    ...
    t.it('Horizontal scroll: Should rerender event body correctly', async t => {
        scheduler = new Scheduler({
            ...
            eventRenderer : ({ eventRecord }) => eventRecord.name,  // disable encodeHtml
            ...
            events : [
                ...
                { id : 2, resourceId : 1, startDate : '2018-01-28T00:00', endDate : '2018-01-31T00:00', name : '2' },
                {
                    id         : 3,
                    resourceId : 2,
                    startDate  : '2018-01-01T00:00',
                ...
                { id : 4, resourceId : 2, startDate : '2018-01-28T00:00', endDate : '2018-01-31T00:00', name : '2' }
            ]
        });
    ...
    t.it('Should preserve viewportCentre when zooming', async t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '2018-01-11 10:00', endDate : '2018-01-11 11:00' },
                { id : 2, resourceId : 2, startDate : '2018-01-11 12:00', endDate : '2018-01-11 13:00' }
            ]
        });
        ...
        const
            center                = scheduler.viewportCenterDate,
            schedulerHeaderHeight = scheduler.timeAxisSubGrid.header.element.offsetHeight;

        scheduler.zoomIn();

        // Note that we zoom far out here, because at the very zoomed out scales,
    ...
    t.it('Adding rows to empty scheduler should work', async t => {
        ...
        const
            { resourceStore, rowManager } = scheduler,
        ...
        // Apparently, per Mats, Schedule->Event links *should* be permanently broken on resource removal.
    ...
    t.xit('Should correctly update events on event mutation when scrolled', async t => {
        ...
        t.chain(
            ...
            async() => {
                const
                    scrollTop               = scheduler.scrollable.y,
                    ...
                    renderedEventRectangles = renderedEventElements.map(el => Rectangle.from(el));
                ...
                t.is(scheduler.scrollable.y, scrollTop, 'Scroll position stable');

                // The only change to any Rectangle is that the mutated one should have changed width.
                renderedEventRectangles[eventElementIndex].width = eventElement.offsetWidth;

                // All rendered event elements should have exactly the same rectangles as before.
    ...
    t.xit('Should not respond to changes to events which are outside of the rendered area', async t => {
        ...
        const
            ...
            scrollNext         = () => {
                ...
                // If scroll moved, we're not at the end, so go again soon.
                ...
                // And the number of rendered events should stay stable
        ...
        // Kick off a scroll through the scheduler
        scrollNext();
    });
    ...
    t.it('Should render event correctly if time axis includes a DST transition', async t => {
        ...
        const
            eventBarEl = document.querySelector(scheduler.eventSelector),
    ...
    t.it('Element clearing upon repaint', async t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '2018-01-01', endDate : '2018-01-02' },
                { id : 2, resourceId : 2, startDate : '2018-01-01', endDate : '2018-01-02' },
                { id : 3, resourceId : 2, startDate : '2018-01-03', endDate : '2018-01-04' }
            ]
        });
        ...
        t.chain(
            async() => {
                t.is(scheduler.timeAxisSubGridElement.querySelectorAll(':not(.b-released) > .b-sch-label.b-sch-label-right').length, 3, 'There are 3 labels to begin with');

                // This event's element will be placed into the cache
                await eventStore.getAt(0).setStartDate(new Date(2020, 11, 31));

                t.is(scheduler.timeAxisSubGridElement.querySelectorAll(':not(.b-released) > .b-sch-label.b-sch-label-right').length, 2, 'There are 2 labels after moving one event outside of TimeAxis');
                // All event elements on a row get tossed out when an event is deleted,
    ...
    t.it('Appending to scheduler container should trigger timelineviewportresize', async t => {
        ...
        // Acquiring a scrollbar will change the viewport size
    ...
    t.it('Should position correctly for monthAndYear', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            //events
            events : [
                { id : 'lastJan', resourceId : 'r1', name : 'Last Jan', startDate : new Date(2019, 0, 31), duration : 24, durationUnit : 'h' },
                { id : 'firstFeb', resourceId : 'r1', name : 'First Feb', startDate : new Date(2019, 1, 1), duration : 24, durationUnit : 'h' },
                { id : 'lastFeb', resourceId : 'r1', name : 'Last Feb', startDate : new Date(2019, 1, 28), duration : 24, durationUnit : 'h' }
            ]
        });
            ...
            items            : [
                {
                    ...
                    resources : ArrayHelper.populate(5, i => ({ id : 'r' + i, name : 'Resource ' + i })),
                    events    : [
                        {
                            id         : 'e1',
                            resourceId : 'r0',
    ...
    if (new Date() > new Date(2022, 2, 1)) {
        ...
        // https://app.assembla.com/spaces/bryntum/tickets/8411
    ...
    t.describe('Row reordering', async t => {
        t.it('Many resources', async t => {
            scheduler = await t.getSchedulerAsync({
                ...
                columns : [
                    { text : 'Name', width : 100, field : 'name', locked : true },
                    { text : 'Id', field : 'id', locked : true }
                ],
                ...
                events : [
                    { id : 1, resourceId : 'r1', startDate : '2018-01-01', endDate : '2018-01-03' }
                ]
            });

            t.chain(
                { drag : '.b-grid-cell', to : '.b-sch-event-wrap' }
            );
        });
    ...
    t.it('New columns should default to locked region', async t => {
        ...
        const [col] = scheduler.columns.add({ field : 'new', text : 'new' });
    ...
    t.it('Should display zoomed event bars which are too wide for the browser as clipped just beyond the viewport boundaries', async t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '2018-04-01', endDate : '2018-08-01' }
            ]
        });
        ...
        await t.waitFor(() => document.querySelector('.b-sch-event-wrap').offsetWidth > scheduler.timeAxisColumn.width);
    ...
    t.it('Should use DateHelper.defaultFormat by default', async t => {
        ...
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : '10.20.19', endDate : '10.25.19' }
            ]
        });
    ...
    t.it('Should not transition row height initially', async t => {
        ...
        scheduler = await t.getSchedulerAsync({
            useInitialAnimation : false,
            events              : [
                { id : 1, name : 'Event 1', startDate : '2011-01-04', duration : 2, resourceId : 'r1' },
                { id : 2, name : 'Event 2', startDate : '2011-01-04', duration : 2, resourceId : 'r1' }
            ]
        });
