import { Scheduler, DomHelper, ArrayHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    if (DomHelper.scrollBarWidth !== 0) {
        ...
        t.it('Should refresh fake scroller when adding first record', async t => {
            ...
            scheduler.resourceStore.add({ id : 1 });
        ...
        t.it('Should not get stuck in infinite scrollbar toggling', async t => {
            ...
            t.firesOk(scheduler.timeAxisViewModel, { update : '<=1' });

            t.chain(
                // Need to give it some time to toggle back and forth (or rather not toggle if fix is working)
                { waitFor : 500 }
            );
        });
    }
    ...
    t.it('Should correctly update scroll range when adding resources in succession', async t => {
        ...
        for (let i = 0; i < 50; i++) {
            scheduler.resourceStore.add({ name : `r${i + 11}` });
        }
    ...
    t.it('should draw events correctly after scrolling into view', async t => {
        scheduler = await t.getSchedulerAsync({
            columns : [
                { text : 'Machines', field : 'name', width : 130 }
            ],
    ...
    t.it('Scrolling a focused event quickly out of view', async t => {
        ...
        t.isCalledNTimes('focusCell', scheduler, 0, 'Scrolling does not cause cell focus');

        // https://github.com/bryntum/support/issues/263
        ...
        t.chain(
            ...
            { drag : () => resizeEventEl, offset : ['100%-3', 5], by : [100, 0] },
    ...
    t.it('Should respect time axis incremement setting when calling scrollToNow', async t => {
        ...
        await scheduler.scrollToNow({ block : 'center' });
