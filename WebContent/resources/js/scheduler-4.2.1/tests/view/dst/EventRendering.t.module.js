import { DateHelper } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Time range duration is ok when dragging over DST', async t => {
        ...
        t.chain(
            { drag : '.spring', by : [-scheduler.tickSize, 0] },
            async() => {
            ...
            { drag : '.autumn', by : [-scheduler.tickSize, 0] },
            async() => {
    ...
    t.it('Event duration is ok when dragging over DST', async t => {
        ...
        await t.dragBy({ source : '.event-2', delta : [tickSize, 0] });

        t.hasApproxWidth('.event-2', tickSize * 2, gap, 'Event 2 width is ok');

        await t.dragBy({ source : '.event-1', delta : [tickSize, 0] });

        t.hasApproxWidth('.event-1', tickSize * 2, gap, 'Event 1 width is ok');

        scheduler.setTimeSpan(autumnStartDate, autumnEndDate);

        await t.dragBy({ source : '.event-4', delta : [tickSize, 0] });

        t.hasApproxWidth('.event-4', tickSize * 2, gap, 'Event 4 width is ok');

        await t.dragBy({ source : '.event-3', delta : [tickSize, 0] });
    ...
    t.it('Events are rendered correctly with uneven ticks caused by DST transition', async t => {
        ...
        const
            event1 = scheduler.eventStore.getById(1),
        ...
        [event1, event2].forEach(record => {
            ...
            t.isApproxPx(box.left, scheduler.getCoordinateFromDate(record.startDate, { local : false }), gap, `Event ${record.id} start date is ok`);
            t.isApproxPx(box.right, scheduler.getCoordinateFromDate(record.endDate, { local : false }), gap, `Event ${record.id} end date is ok`);
        });
        ...
        scheduler.setTimeSpan(
            new Date(autumnStartDate.getFullYear(), autumnStartDate.getMonth(), autumnStartDate.getDate() - 1),
            new Date(autumnEndDate.getFullYear(), autumnEndDate.getMonth(), autumnEndDate.getDate() + 2)
        );
