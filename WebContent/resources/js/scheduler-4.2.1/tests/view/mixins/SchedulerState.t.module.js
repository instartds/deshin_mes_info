...
StartTest(t => {
    ...
    t.it('Should restore scheduler state if start/end was not provided in config', async t => {
        ...
        scheduler.resourceStore.data = [{ id : 1, name : 'Albert' }];
        scheduler.eventStore.data = [{ id : 1, resourceId : 1, startDate : new Date(), endDate : DateHelper.add(new Date(), 2, 'd') }];
    ...
    t.it('Should restore start/end dates', t => {
        ...
        const { startDate, endDate } = scheduler;

        delete scheduler.config.startDate;
        delete scheduler.config.endDate;
    ...
    t.it('Should change view preset correctly after restoring state', async t => {
        ...
        await scheduler.timeAxisSubGrid.header.scrollable.await('scrollEnd', { checkLog : false });

        scheduler.viewPreset = { base : 'weekAndDay', options : { startDate : newStart, endDate : newEnd, centerDate }};
