import { Scheduler, DateHelper } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should allow key activation', async t => {
        ...
        t.chain(
            ...
            next => {
                const secondRec = scheduler.eventStore.getAt(1);
    ...
    t.it('Should omit events outside the TimeAxis', async t => {
        ...
        t.is(scheduler.getPrevious(a3), a2, 'assignment2 is previous from assignment3');

        // Push e2 out of the TimeAxis so that it is not navigable
        await a2.event.setStartDate(DateHelper.add(a2.event.startDate, 6, 'month'), true);
    ...
    t.it('Clicking on half-visible event should not scroll it', async t => {
        ...
        t.chain(
            // It's hanging off the left edge, so click what we can see.
            // This must not scroll it fully into view. https://github.com/bryntum/support/issues/105
            { click : '.b-sch-event', offset : ['85%', '50%'] },
    ...
    t.it('Derendering an event which is scrolled out of the rendered block should focus the view', async t => {
        ...
        const firstEventEl = scheduler.getElementFromEventRecord(scheduler.eventStore.first);

        // Focus should be thrown up to the main Scheduler element when the focused event scrolls out of view
        t.chain(
            { click : '.b-sch-event' },
    ...
    t.it('Should navigate events on filtered time axis', async t => {
        scheduler = await getScheduler({
            ...
            endDate   : '2011-03-06',
            events    : [
                { id : 1, cls : 'event1', resourceId : 1, startDate : '2011-01-04', endDate : '2011-01-04 17:00:00', name : 'Visible' },
                { id : 2, cls : 'event2', resourceId : 1, startDate : '2011-01-04 18:00:00', endDate : '2011-01-04 20:00:00', name : 'Filtered' },
                { id : 3, cls : 'event3', resourceId : 1, startDate : '2011-01-05', endDate : '2011-01-05 17:00:00', name : 'Visible' },
                { id : 4, cls : 'event4', resourceId : 1, startDate : '2011-03-03', endDate : '2011-03-03 17:00:00', name : 'Far right' }
            ],
            workingTime : {
                fromDay  : 1,
                toDay    : 6,
        ...
        t.chain(
            { click : '.b-sch-event' },
            { type : '[RIGHT]' },
            { waitForSelector : '.event3.b-sch-event-selected', desc : 'Navigated to event 3 successfully' },
            { type : '[RIGHT]' },
            { waitForSelector : '.event4.b-sch-event-selected', desc : 'Navigated to event 4 successfully' }
        );
    });
    ...
    t.it('Programmatic focus through scrollEventIntoView should activate event', async t => {
        ...
        t.chain(
            async() => scheduler.scrollEventIntoView(scheduler.eventStore.first, {
                focus : true
            }),

            {
                waitFor : () => {
                    return scheduler.activeAssignment === scheduler.assignmentStore.first &&
                        document.activeElement === document.querySelector('.b-sch-event.event1').parentNode;
                },
                desc : 'event1 is focused'
            }
        );
    ...
    t.it('Should skip filtered events during navigation', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.event3.b-sch-event-selected', desc : 'Navigated to previous event' },

            { type : '[RIGHT]' },

            { waitForSelector : '.event5.b-sch-event-selected', desc : 'Navigated to next event' }
        );
    });
});
