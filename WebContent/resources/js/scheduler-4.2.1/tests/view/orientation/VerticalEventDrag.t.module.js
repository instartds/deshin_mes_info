import { Rectangle } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Valid drag', async t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', by : [150, 0], desc : 'Dragging right' },
            ...
            { drag : '[data-event-id="1"]', by : [0, 200], desc : 'Dragging down' },
    ...
    t.it('Drag triggering scroll', async t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', to : '.b-grid-body-container', toOffset : [200, '100%-10'], desc : 'Dragging far down', dragOnly : true },
    ...
    t.it('Multi drag', async t => {
        ...
        t.chain(
            ...
            { drag : '[data-event-id="2"]', by : [200, 0], desc : 'Dragging right' },
    ...
    t.it('Should support constrainDragToResource', async t => {
        ...
        t.chain(
            { drag : '[data-event-id="2"]', by : [150, 0], desc : 'Dragging right' }
        );
    });
    ...
    t.it('Multi drag including events below the rendered block', async t => {
        ...
        t.chain(
            () => scheduler.scrollEventIntoView(event1001),

            { click : '[data-event-id="1001"]' },

            () => scheduler.scrollEventIntoView(event1),

            { click : '[data-event-id="1"]', options : { ctrlKey : true } },
            ...
            { drag : '[data-event-id="1"]', by : [200, 0], desc : 'Dragging right' },

            next => {
                // Element snap into position under "Resource 2"
                assertEventElement(t, 1, 100, 150, 150, 100);

                // The one way down outside the tendered block is no longer there
                assertEventElement(t, 1001, null);

                // Elements are correctly assigned to "Resource 2"
    ...
    t.it('Should work on scrolled page', async t => {
        ...
        t.chain(
            ...
            { drag : '[data-event-id="2"]', by : [0, -50], offset : ['50%', '80%'] },
