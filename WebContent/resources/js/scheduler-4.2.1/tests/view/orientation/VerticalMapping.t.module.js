import { ArrayHelper, DateHelper, Rectangle, DomHelper } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.beforeEach(async(t, next) => {
        ...
        scheduler = await t.getVerticalSchedulerAsync({
            resources : ArrayHelper.populate(1000, i => ({ id : 'r' + (i + 1), name : 'Resource ' + (i + 1) })),
    ...
    t.it('Resource mapping', t => {
        function assertEventResource(eventId) {
            t.is(
                scheduler.resolveResourceRecord(document.querySelector(`[data-event-id="${eventId}"]`)).id,
                scheduler.eventStore.getById(eventId).resourceId,
                'Correct for event ' + eventId
            );
        }
        ...
        function assertResource(x, index) {
            t.is(
                scheduler.resolveResourceRecord(document.querySelector('.b-sch-timeaxis-cell'), [x, 0]),
                scheduler.resourceStore.getAt(index),
                'Correct for x ' + x
            );
        }
    ...
    t.it('should support getStartEndDatesFromRectangle in vertical mode', async t => {
        ...
        scheduler.resources = [{ id : 'r1', name : 'foo' }];
        scheduler.events = [
            {
                id         : 1,
                resourceId : 'r1',
        ...
        t.chain(
            ...
            async() => {
                const
                    rect = Rectangle.from(document.querySelector('.b-sch-event:not(.b-released)'), scheduler.foregroundCanvas),
