...
Class('BryntumGridTest', {
  ...
  methods: {
    getRemoteGrid: function getRemoteGrid(cfg) {
      ...
      generator === null || generator === void 0 ? void 0 : generator.reset();
      cfg = cfg || {};
      cfg.store = {
        fields: [{
          name: 'fullName',
    ...
    checkGridSanity: function checkGridSanity(grid, next) {
      ...
      var _loop = function _loop() {
        ...
        if ((_w = w) !== null && _w !== void 0 && (_w$endsWith = _w.endsWith) !== null && _w$endsWith !== void 0 && _w$endsWith.call(_w, 'px')) {
          w = parseInt(w);
        ...
        if (column.flex) {
          ...
          if (footerEl && !domFlex.test(footerEl.style.flex)) {
            ...
            if (column.minWidth) {
              var _minW$endsWith, _minW;

              var minW = column.minWidth;

              if ((_minW$endsWith = (_minW = minW).endsWith) !== null && _minW$endsWith !== void 0 && _minW$endsWith.call(_minW, 'px')) {
                minW = parseInt(minW);
              }
            ...
            if (grid.fillLastColumn && column.isLastInSubGrid) {
              ...
              if (othersFlexed) {
                if (headerEl && headerEl.offsetWidth !== w) {
                  _this.fail("Grid ".concat(grid.id, ", header[").concat(i, "] does not match its configured width ").concat(headerEl.offsetWidth, " !== ").concat(w));
                }

                if (footerEl && _this.isElementVisible(footerEl) && footerEl.offsetWidth !== w) {
                  _this.fail("Grid ".concat(grid.id, ", footer[").concat(i, "] does not match its configured width ").concat(footerEl.offsetWidth, " !== ").concat(w));
                }
              ...
              // `this.isElementVisible` call breaks the `element` getter of the grid if console is opened in IE11
      ...
      if (!grid.hideHeaders) {
        for (i = 0; i < rowCount; i++) {
          ...
          for (var col = 0; col < columnCount; col++) {
            ...
            var validDelta = this.browser.msie ? 2 : 1;
      ...
      // `grid.element` is undefined after `this.isElementVisible` call above if console is opened in IE11


      var gridEl = grid.element || grid._element; // Check post render height cache (Edge sometimes gives 1.2, but looks visually ok)
      // Scaling (The scaleToFitWidth config) causes this to fail in IE (Trident engine).

      if (!(navigator.userAgent.match(/trident/i) && gridEl.style.transform.indexOf('scale') > -1)) {
    ...
    waitForRowsVisible: function waitForRowsVisible(grid, next) {
      ...
      var root = ((_grid = grid) === null || _grid === void 0 ? void 0 : _grid.element) || this.global.document.body;
      return this.waitFor(function () {
    ...
    getFirstRenderedRow: function getFirstRenderedRow(grid) {
      var root = (grid === null || grid === void 0 ? void 0 : grid.element) || this.global.document.body,
    ...
    assertFooterPosition: function assertFooterPosition(doc) {
      ...
      if (!bowser.msie) {
        ...
        var fromPoint = doc.elementFromPoint(footerElBox.left + footerElBox.width / 2, footerElBox.top + footerElBox.height / 2),
            el = fromPoint === null || fromPoint === void 0 ? void 0 : fromPoint.closest('.b-export-footer');
    ...
    assertGridHeader: function assertGridHeader(doc) {
      var headerEl = doc.querySelector('.b-export-header'),
          ...
          headerBox = headerEl === null || headerEl === void 0 ? void 0 : headerEl.getBoundingClientRect(),
      ...
      var cellEl = doc.elementFromPoint(gridHeaderBox.left + gridHeaderBox.width / 2, gridHeaderBox.bottom + 1),
          ...
          rowBox = rowEl === null || rowEl === void 0 ? void 0 : rowEl.getBoundingClientRect();
        ...
        rows.forEach(function (rowEl, index) {
          ...
          if (index === 0) {
            ...
            if (assertStartsWithRow) {
              ...
              var headerElBox = (_doc$querySelector = doc.querySelector('.b-export-header')) === null || _doc$querySelector === void 0 ? void 0 : _doc$querySelector.getBoundingClientRect();

              if (headerElBox && !(rowElBox.top <= headerElBox.bottom && rowElBox.bottom >= headerElBox.bottom)) {
            ...
            if (index === rows.length - 1) {
              if (assertEndsWithRow) {
                ...
                var footerElBox = (_doc$querySelector2 = doc.querySelector('.b-export-footer')) === null || _doc$querySelector2 === void 0 ? void 0 : _doc$querySelector2.getBoundingClientRect();

                if (footerElBox && !(Math.round(rowElBox.bottom) >= Math.round(footerElBox.top))) {
