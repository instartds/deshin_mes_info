import { EventModel, ResourceModel, ResourceStore, EventStore, ProjectModel } from '../../build/scheduler.module.js';

StartTest(async t => {
    t.ok(EventModel.fieldMap.startDate, 'Event#startDate');
    t.ok(EventModel.fieldMap.endDate, 'Event#endDate');

    // Differs for core and basic engines
    ...
    class MyResource extends ResourceModel {
        static get fields() {
            return [
                ...
                { name : 'name', dataSource : 'myName' },
                { name : 'customName', dataSource : 'name' }
            ];
        }
    }

    const myResourceFields = MyResource.$meta.fields.defs;
    ...
    class MyEvent extends EventModel {
        static get fields() {
            ...
            return [
                { name : 'name', dataSource : 'myName' },
                { name : 'startDate', dataSource : 'myStartDate', type : 'date' },
                { name : 'customStartDate', dataSource : 'startDate', type : 'date' },
                { name : 'resourceId', dataSource : 'myResourceId' }
            ];
        }
    }
    ...
    const eventStore = new EventStore({
        modelClass : MyEvent,
        data       : [
            { myResourceId : 'c1', myName : 'Linda', myStartDate : '2010-11-09', endDate : '2010-12-09' },
            { myResourceId : 'c2', myName : 'Foo', myStartDate : '2010-11-09', endDate : '2010-12-09' }
        ]
    });
    ...
    t.is(resourceStore.first.events[0], eventStore.first, 'resource#events located the correct event');

    // TODO: Mention in changelog that this is no longer valid
