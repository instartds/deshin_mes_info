...
StartTest(function (t) {
  ...
  //  - update event
  //  - change event id
  var eventStore, eventStoreData, dependencyStore, dependencyStoreData;
  ...
  function setup() {
    ...
    dependencyStoreData = []; // Only half of events will have dependencies, this leads to the case when cache might
  ...
  t.it('Cache should be filled upon store refresh/load', function (t) {
    ...
    t.ok(eventStore.originalCount > 0, 'Events are loaded'); // Querying dependencies for every event
  ...
  t.it('Cache should be updated upon dependency store change', function (t) {
    var eventIds, event1, event2, event3, eventDeps1, eventDeps2, eventDeps3, dependency, stats, statsBefore, statsAfter;
    stats = CacheUtil.stats['EventDependencyCache'];
    dependencyStore.data = dependencyStoreData;
    eventStore.data = eventStoreData;
    ...
    t.is(statsAfter.miss, statsBefore.miss, 'Adding the dependency updates the cache accordingly and availd full scan'); // Changing dependency target
    ...
    t.is(statsAfter.miss, statsBefore.miss, 'Changing the dependency target updates the cache accordingly and availd full scan'); // Changing dependency source
    ...
    t.is(statsAfter.miss, statsBefore.miss, 'Changing the dependency source updates the cache accordingly and availd full scan'); // Remove dependency
    ...
    t.is(statsAfter.miss, statsBefore.miss, 'Removing the dependency updates the cache accordingly and availd full scan'); // Clear dependency store
  ...
  t.it('Cache should be updated upon event store change', function (t) {
    var event1, event2, eventDeps1, eventDeps2, dependency, stats, statsBefore, statsAfter;
    stats = CacheUtil.stats['EventDependencyCache'];
    dependencyStore.data = dependencyStoreData;
    eventStore.data = eventStoreData; // Adding an event
    ...
    t.is(statsAfter.miss, statsBefore.miss, "Requesting dependencies of the new event doesn't make attempt to do full scan of the dependency store"); // Changing event id
    ...
    t.is(statsAfter.miss, statsBefore.miss, "Requesting dependencies of the event with id changed doesn't make attempt to do full scan of the dependency store"); // Removing event
