import { AssignmentStore, ResourceStore, EventStore, ProjectModel } from '../../build/scheduler.module.js';

StartTest(t => {
    const
        monday  = new Date(2015, 2, 16),
    ...
    function getProject() {
        return new ProjectModel({
            eventStore : new EventStore({
                ...
                data                  : [{
                    id : 1, name : 'Event 1', startDate : monday, endDate : sunday
                }, {
                    id : 2, name : 'Event 2', startDate : monday, endDate : sunday
                }]
            }),
    ...
    t.describe('Scheduler data model must support events assignment to multiple resources', t => {
        t.it('Should properly report resources an event assigned to', t => {
            const
                { eventStore, resourceStore } = getProject(),
                [e1, e2] = eventStore,
                [r1, r2] = resourceStore;
        ...
        t.it('Should properly report events a resource assigned to', t => {
            const { eventStore, resourceStore } = getProject();
        ...
        t.it('Should properly split a multi-resource event', async t => {
            const
                project = getProject(),
                { eventStore, assignmentStore } = project,
                [event1] = eventStore;

            // Need to await to have calculated values on split
