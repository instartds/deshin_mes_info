import { Model } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    class BaseModel extends Model {
        static get fields() {
            return [
                { name : 'id', dataSource : 'Usual' },
                { name : 'Field1', persist : true },
                { name : 'Field2', persist : true },
                { name : 'FieldFoo', persist : true }
            ];
        }
    }
    ...
    t.it('Model.set should return array of modified fields', function(t) {
        ...
        t.isDeeply(instance.set('Field1', 'foo'), { Field1 : { value : 'foo' } }, 'Object returned');
    });

    class SubModel extends BaseModel {
        static get fields() {
            return [
                { name : 'subField1', dataSource : 'field1', persist : false },
                { name : 'subField2', dataSource : 'field2' },
                { name : 'Field3', type : 'int', defaultValue : 53 },
                { name : 'FieldFoo', persist : false }
            ];
        }
    ...
    t.it('Testing the base model (the one with customizableFields property)', function(t) {
        ...
        // where would that field come from??
    ...
    t.it('Testing the sub model (the one inheriting from base model, and w/o customizableFields)', function(t) {
        ...
        const subModelFields = SubModel.fields.reverse();

        // TODO: PORT are now adding fields, should replace??
