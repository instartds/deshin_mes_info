import { ResourceStore, ResourceTimeRangeStore } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.beforeEach(t => {
        ...
        rangeStore = new ResourceTimeRangeStore({
            resourceStore,
            data : [
                { id : 1, resourceId : 1, name : 'First zone' },
                { id : 2, resourceId : 1, name : 'Second zone' },
                { id : 3, resourceId : 2, name : 'Third zone' },
                { id : 4, resourceId : 3, name : 'Forth zone' }
            ]
        });
    ...
    t.it('Resource relation sanity checks', t => {
        t.isDeeply(rangeStore.map(r => r.resource), [resourceStore.first, resourceStore.first, resourceStore.last, { id : 3, placeHolder : true }], 'Correct resource initially');
        t.isDeeply(resourceStore.first.timeRanges, [range1, range2], 'Correct ranges initially');
        t.isDeeply(resourceStore.last.timeRanges, [range3], 'Correct ranges initially');

        // Reassign using relation setter
        range1.resource = resourceStore.last;
        t.is(range1.resourceId, resourceStore.last.id, 'Assigning to resource worked');

        // Reassign using relation key
        range2.resourceId = 2;
    ...
    t.it('Adding range should populate `resource` & `timeRanges`', t => {
        const [zone5] = rangeStore.add({ id : 5, resourceId : 1, name : 'Fifth zone' });
