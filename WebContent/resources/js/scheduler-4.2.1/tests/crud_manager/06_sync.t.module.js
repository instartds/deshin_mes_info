import { AbstractCrudManager, Store, CrudManager, JsonEncoder, ResourceModel, EventModel, ProjectModel } from '../../build/scheduler.module.js';

StartTest(t => {
    t.diag('Here we test sync() method of AbstractCrudManager class');

    let sent, received, someStore1, someStore2, crud, added1, added2, response;
        ...
        sendRequest(config) {
            ...
            window.setTimeout(() => {
                ...
                if (this.testRequestMethod === 'failure') {
                    ...
                    }, null, { type : config.type, id : config.id });
                }
                else {
                    ...
                    }, null, { type : config.type, id : config.id }).then(() => {
                        if (r && r.success) {
    ...
    t.it('Delays the sending of sync packet while previous is not responded', t => {
        initTestData(t);
        ...
        // add one more record
        const a = someStore1.add({ ff1 : 'another new', ff2 : 'another new' });
        ...
        t.waitFor(
            () => someStore1.modified.count === 0 &&
                someStore2.modified.count === 0 &&
                sent === 2 &&
                received === 2,
            () => {
                t.it('Applies response to someStore1', t => {
                    ...
                    // to get better debugging info from TC on fail
                    if (newRec) {
    ...
    t.it('sync should work with fields with a dataSource', async t => {
        class MyEventRec extends EventModel {
            static get fields() {
                return [
                    { name : 'name', type : 'string', dataSource : 'eventDisplayName' }
                ];
            }
        }
        ...
        t.mockUrl('test-sync-with-dataSource', {
            responseText : JSON.stringify({
                ...
                events  : {
                    rows : [
                        { id : 1, eventDisplayName : 'First event name after sync' }
                    ]
                }
            })
        });
        ...
        t.is(crud.eventStore.first.name, 'First event start name');

        // This will be attempted to be synced, but the server will override, returning its own new name
    ...
    t.it('Should support suspendAutoSync/resumeAutoSync', async t => {
        ...
        // this should trigger a schedule of sync() which is delayed by 0ms
        crud.resumeAutoSync();
    ...
    t.it('Should not call sync if resumeAutoSync is called with false', async t => {
        ...
        crud.resumeAutoSync(false);

        // wait a bit to allow setTimeout 0 to expire
        await t.waitFor(50);
