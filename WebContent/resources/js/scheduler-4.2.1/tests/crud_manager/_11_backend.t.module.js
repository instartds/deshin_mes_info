import { AjaxHelper, CrudManager } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    const setup = (fn, testConfig) => {
        ...
        AjaxHelper.get(testConfig.resetUrl).then(fn, () => t.fail('Reset failed'));
    ...
    t.it('Should be possible to save some resources and events', t => {
        t.chain(
            ...
            next => {
                const addedResources = resourceStore.add([{ name : 'resource1' }, { name : 'resource2' }]);
            ...
            next => {
                ...
                const addedResources = resourceStore.add([{ name : 'resource3' }]);
