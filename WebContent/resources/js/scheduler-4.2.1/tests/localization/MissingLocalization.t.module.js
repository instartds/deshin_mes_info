...
StartTest('All locales should have corresponding to English locale translations', t => {
    const
        ...
        universalList = Object.assign({
            Core : []
        }, testConfig.universalList);

    // Append De locale from window
    if (window.bryntum?.locales?.De) {
    ...
    function assertES6(t, asserted, path = '') {
        Object.keys(asserted).forEach(localeKey => {
            const
                currentPath = path ? `${path}.${localeKey}` : localeKey,
                value       = asserted[localeKey];
            // if not found in master
            if (typeof value === 'function') {
                if (/=>/.test(value) || /`/.test(value)) {
                    t.fail(`ES6 Syntax found in '${currentPath}' in ${localeName}.`);
                }
            }
    ...
    Object.keys(locales).forEach(name => {
        t.it(`Check ${name} locale`, t => {
            ...
            t.ok(locale.desc, `Locale description is specified for ${name}`);

            // We suppose to use ES5 format for De localization
            !testConfig.skipES6Check && locale === locales.De && assertES6(t, locale);
