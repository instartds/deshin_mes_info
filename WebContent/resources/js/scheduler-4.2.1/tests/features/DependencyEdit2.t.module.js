import { DependencyBaseModel } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    async function setup(t, config = {}) {
        ...
        scheduler = t.getScheduler(Object.assign({
            ...
            events : [
                { id : 1, resourceId : 1, startDate : new Date(2018, 8, 17), duration : 2, name : 'task 1' },
                { id : 2, resourceId : 1, startDate : new Date(2018, 8, 24), duration : 2, name : 'task 2' }
            ],
    ...
    t.it('Should redraw dependency on dependency type change via data API', async t => {
        ...
        t.chain(
            ...
            (next) => {
                ...
                t.isApprox(taskRect.left, svgRect.left + lastPoint.x, 0.5, 'Marker points to task 2 start');
            ...
            () => {
                ...
                t.isApprox(svgRect.left + lastPoint.x, taskRect.right, 0.5, 'Marker points to task 2 end');
            }
        );
    });
    ...
    t.it('Should redraw dependency on dependency type change via editor with just created task', async t => {
        ...
        t.chain(
            ...
            // Creating new event
            { drag : '.b-grid-row[data-index=0] .b-sch-timeaxis-cell', offset : [250, '50%'], by : [100, 0] },
            ...
            // Changing dependency type
            { dblclick : dep },
            { waitForSelector : '.b-popup .b-header-title:contains(Edit dependency)', desc : 'Popup shown' },
            { click : '.b-icon-picker' },
