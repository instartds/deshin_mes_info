StartTest(t => {
    ...
    t.it('Should allow user copy, cut and paste using context menu', t => {
        ...
        t.chain(
            ...
            { waitForSelector : '[data-event-id=5] .b-cut-item', desc : 'Event Assignment 5 style was changed to "cut" when using context menu' },
            { contextmenu : '[data-id="r4"]', offset : [150, '50%'] },
            ...
            { contextmenu : '[data-id="r4"]', offset : [450, '50%'] },
            { waitForSelector : '.b-schedule-selected-tick', desc : 'The cell where the event will be pasted was highlighed when clicked' },
            { click : '.b-menuitem[data-ref="pasteEvent"]' },
            { waitForProjectReady : scheduler },
            () => {
    ...
    t.it('Should allow user copy, cut and paste using CTRL+C/CTRL+X and CTRL+V', t => {
        ...
        t.chain(
            ...
            { type : '[X]', options : { ctrlKey : true } },
            { waitForSelector : '[data-event-id=1] .b-cut-item', desc : 'Event Assignment 1 style was changed to "cut" when using CTRL+X' },
            { click : '[data-id="r6"]', offset : [350, '50%'] },
            next => {
            ...
            { diag : 'Multiple cut' },
            { click : '[data-event-id=3]', options : { ctrlKey : true } },
            { click : '[data-event-id=4]', options : { ctrlKey : true } },
            { type : '[X]', options : { ctrlKey : true } },
            { waitForSelector : '[data-event-id=3] .b-cut-item', desc : 'Event Assignment 3 style was changed to "cut" when using CTRL+X' },
            { waitForSelector : '[data-event-id=4] .b-cut-item', desc : 'Event Assignment 4 style was changed to "cut" when using CTRL+X' },
            { click : '[data-id="r1"]', offset : [250, '50%'] },
            next => {
                const rect = t.rect('.b-schedule-selected-tick');
                t.ok(rect.left + 45 == t.currentPosition[0] && rect.top + 23 == t.currentPosition[1], 'The cell where the event will be pasted was highlighed when clicked');
                next();
            ...
            { click : '.b-grid-row[data-id="r5"]', offset : [250, '50%'] },
            next => {
                const rect = t.rect('.b-schedule-selected-tick');
                t.ok(rect.left + 45 == t.currentPosition[0] && rect.top + 23 == t.currentPosition[1], 'The cell where the event will be pasted was highlighed when clicked');
                next();
            ...
            { click : '[data-event-id=5]', options : { ctrlKey : true } },
            { type : '[C]', options : { ctrlKey : true } },
            { click : '[data-id="r3"]', offset : [550, '50%'] },
            next => {
                const rect = t.rect('.b-schedule-selected-tick');
                t.ok(rect.left + 45 == t.currentPosition[0] && rect.top + 23 == t.currentPosition[1], 'The cell where the event will be pasted was highlighed when clicked');
                next();
