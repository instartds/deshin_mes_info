import { ResourceStore, Scheduler } from '../../build/scheduler.module.js';

StartTest(t => {
    let scheduler, resourceStore, eventStore, resources;
    ...
    t.it('Append nodes and collapse/expand after works fine', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-grid-cell:contains(child)', desc : 'Child is visible initially' },

            { diag : 'Collapsing...' },

            () => scheduler.features.tree.collapse(resource),

            { waitForSelectorNotFound : '.b-grid-cell:contains(child)', desc : 'Child got hidden' },

            { diag : 'Expanding...' },

            () => scheduler.features.tree.expand(resource),
            ...
            { waitForSelector : '.b-grid-cell:contains(child)', desc : 'Child got visible' }
        );
    });
    ...
    t.it('Events are rendered correctly after collapse/expand', async t => {
        scheduler = new Scheduler({
            ...
            events : [
                { id : 1, resourceId : 111, name : 'Moving out', startDate : '2020-04-27', duration : 1 },
                { id : 2, resourceId : 112, name : 'Moving in', startDate : '2020-04-29', duration : 1 },
                { id : 3, resourceId : 121, name : 'Moving in', startDate : '2020-04-28', duration : 1 },
                { id : 4, resourceId : 122, name : 'Moving out', startDate : '2020-04-30', duration : 1 }
            ],
            ...
            columns : [
                { type : 'tree', field : 'name', text : 'Name', width : 200 }
            ]
        });
    ...
    t.it('Should be possible to undo event removing when tree feature is used and assignment is set using resourceId field', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            events : [
                { id : 1, resourceId : 1, name : 'Test', startDate : '2011-01-04', endDate : '2011-01-05' }
            ],
    ...
    t.it('Should be possible to undo event removing when tree feature is used and assignment is set as a record in AssignmentStore', async t => {
        scheduler = await t.getSchedulerAsync({
            ...
            events : [
                { id : 1, name : 'Test', startDate : '2011-01-04', endDate : '2011-01-05' }
            ],
