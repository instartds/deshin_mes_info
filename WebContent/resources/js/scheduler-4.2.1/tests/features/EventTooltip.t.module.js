import { AjaxHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('should encode event name to avoid XSS issues', async t => {
        ...
        await t.waitForSelector('.b-sch-event-content:contains(Foo)');

        // Ensure that default event renderer properly encodes event name:
        ...
        // Ensure that EventTooltip also properly encodes event name:
    ...
    t.it('Should support disabling', t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-sch-event-tooltip', desc : 'Tooltip shown' }
        );
    });
    ...
    t.it('Should support async tooltip by returning Promise', t => {
        scheduler = t.getScheduler({
            ...
            features : {
                eventTooltip : {
                    template : ({ eventRecord }) => AjaxHelper.get('./fakeServer').then(response => response.text())
                }
            }
        });
    ...
    t.it('Should support using "on"/"un" function for registering/unregistering events', async t => {
        ...
        // Add listener
        scheduler.features.eventTooltip.on({ beforeShow });
        await t.moveMouseTo('.b-sch-event');
        await t.waitForSelector('.b-tooltip');
        t.ok(called, 'listener fn was called');

        // Remove listener
        scheduler.features.eventTooltip.un({ beforeShow });
        called = false;
