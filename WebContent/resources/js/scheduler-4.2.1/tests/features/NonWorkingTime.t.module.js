import { LocaleManager, BrowserHelper } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Weekends should be configurable. Weekends are Sat - Sun', async(t) => {
        ...
        const
            firstDay      = t.query('.b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index="6"]')[0], // Saturday header
            secondDay     = t.query('.b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index="7"]')[0], // Sunday header
            interval      = t.query('.b-sch-nonworkingtime[data-id=nonworking-2]')[0], // Second interval
    ...
    t.it('Weekends should be configurable. Weekends are Fri - Sat', async(t) => {
        ...
        const
            firstDay      = t.query('.b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index="5"]')[0], // Friday header
            secondDay     = t.query('.b-sch-header-row-1 .b-sch-header-timeaxis-cell[data-tick-index="6"]')[0], // Saturday header
    ...
    t.it('Should be possible to update weekends dynamically', async(t) => {
        ...
        LocaleManager.locale.DateHelper.nonWorkingDays = { 0 : true, 1 : true };
        LocaleManager.applyLocale(LocaleManager.locale.localeName, true);
    ...
    t.it('Should work in non-english locale', async t => {
        ...
        LocaleManager.locale = window.bryntum.locales.Ru;
        // Non-working time generates new calendar intervals, so need to wait when propagation is finished
        ...
        LocaleManager.locale = window.bryntum.locales.En;
        // Non-working time generates new calendar intervals, so need to wait when propagation is finished
        await scheduler.project.commitAsync();
