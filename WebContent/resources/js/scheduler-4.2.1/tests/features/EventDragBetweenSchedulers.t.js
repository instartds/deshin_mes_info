...
StartTest(t => {
    ...
    t.it('Should trigger scroll manager of source scheduler', async t => {
        ...
        t.chain(
            { drag : '.b-sch-event', to : '#first', dragOnly : true, toOffset : ['100%-20', '100%-20'] },

            {
                waitFor : () =>
                    Math.abs(scheduler.subGrids.normal.scrollable.x - scheduler.subGrids.normal.scrollable.maxX) <= 1 &&
                    Math.abs(scheduler.scrollable.y - scheduler.scrollable.maxY) <= 1
            },
    ...
    t.it('Should still behave normal for drag drop inside 1 scheduler', async t => {
        ...
        t.chain(
            { drag : '#first .b-sch-event', by : [100, 10] }
        );
    });
    ...
    t.it('Should just result in an update action if target event store is the same after drag drop', async t => {
        ...
        // And added to scheduler2
        t.wontFire(scheduler2.eventStore, 'remove');
        t.firesOnce(scheduler2.eventStore, 'add');

        t.chain(
            { drag : '#first .b-sch-event', to : '#second .b-sch-timeaxis-cell', toOffset : [105, 25] },
            () => {
    ...
    t.it('Should remove dragged event from source store and add to new event store if target event store is not the same', async t => {
        ...
        t.chain(
            { drag : '#first .b-sch-event', to : '#second .b-sch-event', dragOnly : true },
            ...
            { waitForSelectorNotFound : '#first ' + scheduler.unreleasedEventSelector, desc : 'Top empty' },
            ...
            next => {
                ...
                t.is(scheduler2.eventStore.last.startDate, scheduler2.eventStore.first.startDate, 'Same start date');
                // t.is(scheduler2.eventStore.last.isPhantom, true, 'Newly added event should be phantom if target event store is different');
    ...
    t.it('Should support drag to a new scheduler and back', async t => {
        ...
        t.chain(
            { drag : `#first ${scheduler.eventSelector}`, to : '#second', dragOnly : true, toOffset : ['100%-25', '100%-25'] },

            {
                waitFor : () =>
                    Math.abs(scheduler2.subGrids.normal.scrollable.x - scheduler2.subGrids.normal.scrollable.maxX) <= 1 &&
                    Math.abs(scheduler2.scrollable.y - scheduler2.scrollable.maxY) <= 1
            },
            ...
            { drag : `#second ${scheduler.unreleasedEventSelector} .b-sch-event`, to : '#first', dragOnly : true, toOffset : ['100%-25', '100%-25'] },
    ...
    t.it('Should activate scroll manager when dragging twice in same scheduler with cloneTarget', async t => {
        ...
        t.chain(
            ...
            { drag : '#first .b-sch-event', to : '#first', dragOnly : true, toOffset : ['100%-20', '50%'] },
    ...
    t.it('Should call validity checker on currently hovered Grid', async t => {
        ...
        t.chain(
            { drag : '#first .b-sch-event', to : '#second .b-sch-event' }
        );
    });
    ...
    t.it('Should not make event disappear when drag and dropping another event from the same row', async t => {
        ...
        t.chain(
            // drop on non valid area on the first grid
            { drag : '#second .b-sch-event:contains(test)', to : '#first .b-sch-header-timeaxis-cell' },
            { waitForSelector : '#second .b-sch-event:contains(test)' },

            // drop on valid area on the first grid
            { drag : '#second .b-sch-event:contains(bar)', to : '#first .b-sch-event' },
    ...
    t.it('Should constrain dragging to the union of all partnered Schedulers', async t => {
        ...
        t.chain(
            // Drag event downwards to the bottom of the second scheduler
            { drag : () => '[data-event-id="1"]', to : () => scheduler2.bodyContainer, toOffset : ['50%', '100%-10'], dragOnly : true },
    ...
    t.it('Should stop scrolling when moving over a new Scheduler', async t => {
        ...
        t.chain(
            ...
            async() => t.ok(scheduler.scrollManager.monitoring.has(scheduler.subGrids.normal.element), 'Active scrolling'),

            // Wait for top scheduler to start scrolling
            { moveCursorTo : scheduler2.bodyContainer, offset : ['50%', 10] },

            async() => t.notOk(scheduler.scrollManager.monitoring.has(scheduler.subGrids.normal.element), 'No active scrolling'),

            { waitFor : () => !scheduler.scrolling, desc : 'Stopped scrolling after hovering new scheduler' },
            { mouseup : null }
        );
    });
});
