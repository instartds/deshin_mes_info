import { Scheduler, DomHelper, AssignmentStore } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    async function getMultiScheduler(config = {}) {
        scheduler = new Scheduler(Object.assign({
            ...
            events : [
                { id : 1, name : 'test event', startDate : new Date(2017, 0, 1, 10), endDate : new Date(2017, 0, 1, 12) }
            ],
    ...
    t.it('Editing a multi assigned event should update all instances', async t => {
        ...
        t.chain(
            ...
            { type : 'Fun[ENTER]', target : 'input:focus' },
            ...
            { type : '09[ENTER]', clearExisting : true },
    ...
    t.it('Deleting a multi assigned event should remove all instances', async t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : scheduler.unreleasedEventSelector, desc : 'All elements gone' }
        );
    });
    ...
    t.it('Should support multiple resources w/ assignment store', async t => {
        await getMultiScheduler({
            ...
            assignments : [
                { resourceId : 1, eventId : 1 },
                { resourceId : 2, eventId : 1 }
            ]
        }, false);

        const
            event         = scheduler.eventStore.first,
            { eventEdit } = scheduler.features;
    ...
    t.it('Should select correct resource in event editor resource combo when creating new event using assignment store', async t => {
        ...
        t.chain(
            { dblClick : '.b-sch-timeaxis-cell', offset : [1, 1] },
            { type : 'foo', target : 'input:focus' },
    ...
    t.it('Adding 1 new assignment should trigger one assignment store change events', async t => {
        ...
        // Add + commitAsync
        t.firesOk(scheduler.assignmentStore, { change : 2 });
    ...
    t.it('Removing one existing resource assignment of a multi assigned event + adding 1 new should trigger 2 assignment store change events', async t => {
        ...
        // there is inline data and first project commit which clears changes is triggered from there.
        ...
        t.chain(
            ...
            async() => {
                ...
                resourceCombo.value = ['r1', 'r3'];

                // Add + remove + engine commit
