import { DateHelper, DomHelper, BrowserHelper, Override, DataGenerator, RandomGenerator, PaperFormat, RowsRange, Rectangle, ScheduleRange } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should export visible rows/schedule when scrolled to the bottom', async t => {
        async function generateResources(resourceCount = 100, eventCount = 5) {
            ...
            return { events, resources, schedulerEndDate };
        }

        const { events, resources } = await generateResources();
        ...
        const
            { startDate, endDate } = scheduler.visibleDateRange,
        ...
        await new Promise(resolve => {
            t.setIframe({
                ...
                onload(doc, frame) {
                    ...
                    const
                        tickCount       = DateHelper.getDurationInUnit(startDate, endDate, 'd'),
                        tickWidth       = scheduler.tickSize,
                        lockedGridWidth = scheduler.subGrids.locked.scrollable.scrollWidth,
                        normalGridWidth = tickCount * tickWidth,
                        splitterWidth   = scheduler.resolveSplitter('locked').offsetWidth,
