import { PaperFormat, DateHelper, DomHelper, Override, DataGenerator, RandomGenerator, Rectangle } from '../../../build/scheduler.module.js';
...
StartTest(t => {
        ...
        t.chain(
            async() => {
                ...
                async function assertColumns(exporterType) {
                    ...
                    const
                        expectedNormalWidth = scheduler.timeAxisColumn.width,
    ...
    t.it('Should keep region size', async t => {
        ...
        const
            normalScrollWidth = scheduler.timeAxisSubGrid.scrollable.scrollWidth,
        ...
        async function assertExport(config, locked, normal, total) {
            ...
            const
                { document, iframe } = await t.setIframeAsync({
                    html   : pages[0].html,
                    height : paperHeight
                }),
    ...
    t.it('Should export with collapsed regions', async t => {
        function assertRegions(doc, locked, normal) {
            const
                splitterEl = doc.querySelector('.b-grid-splitter'),
        ...
        await Promise.all([
            ...
            scheduler.await('timelineViewportResize', { checkLog : false }),
        ...
        let { document, iframe } = await t.setIframeAsync({
            height : 1123,
            html   : pages[0].html
        });
