...
StartTest({ defaultTimeout : 90000 }, t => {
    ...
    t.it('Should target resource row if center point of event bar is inside the row', t => {
        ...
        t.chain(
            { drag : scheduler.eventSelector, by : [0, 30] },
    ...
    t.it('Should move event to same resource as target event if dropped on an event', t => {
        ...
        t.chain(
            { drag : scheduler.eventSelector + ':contains(two)', to : scheduler.eventSelector + ':contains(one)' },
    ...
    t.it('Should not allow dragging if readOnly', async t => {
        ...
        t.chain(
            { drag : scheduler.eventSelector, by : [-10, 0] },
            (next, element) => {
    ...
    t.it('Should be able to abort a drag when using AssignmentStore', t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', by : [65, 0], dragOnly : true },
    ...
    t.it('Should be able to reassign when using AssignmentStore', t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', by : [0, 120] },

            () => {
                t.isDeeply(scheduler.assignmentStore.map(r => r.resourceId), ['r3', 'r2'], 'Assignment updated');

                t.isGreater(
                    document.querySelector('.r3').getBoundingClientRect().top,
                    document.querySelector('.r2').getBoundingClientRect().bottom,
                    'Moved below'
                );
            }
        );
    ...
    t.it('Dragging event outside of timeline when using AssignmentStore', t => {
        ...
        t.chain(
            { drag : '[data-event-id="1"]', by : [-400, 0], dragOnly : true },
    ...
    t.it('Dragging event to overlay splitter', t => {
        ...
        // The bug was because the elementFromPoint of the left end of the dragged
        ...
        t.chain(
            { drag : '[data-event-id=1]', by : [-99, 0] },

            { drag : '[data-event-id=1]', by : [0, scheduler.rowHeight] },
    ...
    t.it('Invalid drop should make no influence on event changing', t => {
        ...
        t.isFiredWithSignature(scheduler, 'afterEventDrop', function({ valid }) {
            return !valid;
        }, 'Drop operation must be invalid for this test');

        t.chain(
            // Make invalid drop by dragging outside the view
            next => t.dragTo('[data-event-id=1]', '.b-grid-cell', next, null, null, null, null, [-10, '50%']),
            ...
            { click : '.b-sch-header-row-main .b-sch-header-timeaxis-cell', desc : 'No exceptions expected' }
        );
    });
    ...
    t.it('Should be able to invalidate drag in `beforeEventDropFinalize` listener', t => {
        ...
        scheduler = t.getScheduler({
            listeners : {
                beforeEventDropFinalize({ event, context }) {
                    context.valid = false;
                    t.is(event.target.innerText.trim(), 'Assignment 2', 'Correct target element');
                    t.is(context.targetEventRecord.id, 2, 'Correct target event record');
                    t.is(context.newResource.id, 'r2', 'Correct target resource record');
