import { Scheduler, PresetManager } from '../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    function assertColumnLines(t, skipExists = false) {
        const
            bottomCells = t.query(`.b-sch-header-row-1 .b-sch-header-timeaxis-cell`),
        ...
        if (bottomCells.length > 1) {
            ...
            bottomCells.forEach((element, i) => {
                if (i) {
                    const
                        line = document.querySelector(`.b-column-line[data-line="line-${element.dataset.tickIndex}"]`),
                        left = element.getBoundingClientRect().left;

                    // Some lines might be replaced by major lines, thus the check that line exists
        ...
        if (topCells.length > 1) {
            ...
            topCells.forEach((element, i) => {
                if (i) {
                    const
                        line = document.querySelector(`.b-column-line-major[data-line="major-${element.dataset.tickIndex}"]`),
                        left = element.getBoundingClientRect().left;
    ...
    t.it('Should align when resizing viewport', async t => {
        ...
        t.chain(
            { waitForEvent : [scheduler, 'horizontalscroll'], trigger : () =>  t.setWindowSize(900, 768) },
            ...
            { waitForEvent : [scheduler, 'horizontalscroll'], trigger : () =>  t.setWindowSize(1200, 768) },
            ...
            { waitForEvent : [scheduler, 'horizontalscroll'], trigger : () =>  t.setWindowSize(1024, 768) },
    ...
    t.it('Should update column lines when reconfiguring working time', async t => {
        ...
        scheduler.workingTime = { fromDay : 1, toDay : 5 };
        ...
        scheduler.workingTime = { fromDay : 1, toDay : 4 };
        ...
        scheduler.workingTime = { fromDay : 1, toDay : 3 };
        ...
        scheduler.workingTime = { fromDay : 1, toDay : 2 };
