import { EventStore, ResourceStore, Base } from '../../../build/scheduler.module.js';
...
StartTest(t => {
    ...
    t.it('Should render occurrences for newly created recurring event', t => {
        ...
        t.chain(
            { doubleclick : '.b-sch-timeaxis-cell', offset : [5, 5] },
    ...
    t.it('Custom dialog shows up correctly when editing newly created event', t => {
        ...
        t.chain(
            ...
            {
                waitFor : () => scheduler.features.eventEdit.recurrenceEditor?.containsFocus
            },

            next => {
                const
                    { eventEdit }        = scheduler.features,
                    { recurrenceEditor } = eventEdit,
                    { x, y }             = recurrenceEditor;

                // Find correct center
                recurrenceEditor.alignTo({ target : window, align : 'c-c' });

                // Must have been centered by our default settings.
                t.isApprox(recurrenceEditor.x, x, 1);
                t.isApprox(recurrenceEditor.y, y, 1);

                // The frequency has been loaded correctly
    ...
    t.it('Should support multi resource assignment for recurring events', t => {
        ...
        scheduler = t.getScheduler({
            ...
            events    : [
                ...
                { id : 2, name : 'Bar', startDate : '2018-06-14', endDate : '2018-06-15', recurrenceRule : 'FREQ=DAILY;INTERVAL=2', cls : 'sch-event2' }
            ],
            resources : [
