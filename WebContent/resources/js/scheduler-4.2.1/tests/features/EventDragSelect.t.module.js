import { DomHelper } from '../../build/scheduler.module.js';
...
StartTest({ defaultTimeout : 90000 }, t => {
    ...
    t.it('Should select events during drag', async t => {
        ...
        t.chain(
            { drag : '.two', fromOffset : ['0%-20', '100%+10'], by : [50, -50], dragOnly : true },
    ...
    t.it('Should deselect selected events during drag', async t => {
        ...
        t.chain(
            ...
            { drag : '.two', fromOffset : ['0%-20', '100%+10'], by : [50, -175], dragOnly : true },
            ...
            next => {
                ...
                const gridRect = scheduler.timeAxisSubGrid.element.getBoundingClientRect();

                // tops should match... give or take fractional pixels
    ...
    t.it('Should support disabling', async t => {
        scheduler = await getScheduler({ events : [] });

        scheduler.features.eventDragSelect.disabled = true;

        t.chain(
            { drag : '.b-sch-timeaxis-cell', by : [50, 50], dragOnly : true },
    ...
    t.it('Should not trigger selection when dragging outside schedule area', async t => {
        ...
        t.chain(
            { drag : '.b-grid-cell:contains(Mike)', by : [200, 100], dragOnly : true },
    ...
    t.it('Should focus last selected event on mouse up', async t => {
        ...
        t.chain(
            { drag : '.b-sch-timeaxis-cell', by : [200, 100], offset : [1, 1] },
    ...
    t.it('Should be possible to click-select an event after a drag select with mouse up outside schedule element', async t => {
        ...
        t.chain(
            { drag : '.b-sch-timeaxis-cell', to : [1, 1], offset : [1, 1] },
