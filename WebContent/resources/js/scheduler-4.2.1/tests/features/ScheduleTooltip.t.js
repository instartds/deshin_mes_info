import '../../lib/Core/localization/En.js';
...
StartTest(t => {
    ...
    t.it('Should be displayed when hovering empty area', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-sch-scheduletip:not(.b-hidden)', desc : 'Waiting for tooltip' },
            ...
            { waitForSelectorNotFound : '.b-sch-scheduletip:not(.b-hidden)', desc : 'Waiting for tooltip' },
    ...
    t.it('Should not be displayed when hovering event', async t => {
        ...
        t.chain(
            ...
            { waitForSelectorNotFound : '.b-sch-scheduletip:not(.b-hidden)', desc : 'Not shown' }
        );
    });
    ...
    t.it('Should not be displayed when dragging an event', async t => {
        ...
        t.chain(
            // Position the mouse close to the event we're going to drag
            // so that we can begin the drag before the ScheduleTooltip
            // has a chance to hide
            { moveMouseTo : '.b-sch-event', offset : ['50%', '0%-1'] },

            { waitForSelector : '.b-sch-scheduletip:not(.b-hidden)', desc : 'Shown' },
            ...
            { drag : '.b-sch-event', offset : ['50%', '0%+1'], by : [-50, 0], dragOnly : true },
    ...
    t.it('Should still be displayed when scheduler is readOnly', async t => {
        ...
        t.chain(
            { moveMouseTo : [0, 0] },
            { moveMouseTo : '.b-sch-timeaxis-cell', offset : [10, 10] },
    ...
    t.it('Clock template should respect time axis resolution', async t => {
        ...
        t.chain(
            ...
            { waitForEvent : [scheduler, 'presetChange'], trigger : () => scheduler.zoomOutFull() },

            // Don't use `offset` here, because it will fail in FF
            { moveMouseTo : '.b-sch-timeaxis-cell' },

            { waitForSelector : '.b-sch-scheduletip:not(.b-hidden) .b-sch-clock-day', desc : 'Waiting for cell tooltip' },

            { mouseDown : null },
            { moveMouseBy : [[100, 0]] },
            { waitForSelector : '.b-tooltip:not(.b-hidden) .b-sch-clock-day', desc : 'Waiting for drag tooltip' },
            { mouseUp : null },

            { waitForEvent : [scheduler, 'presetChange'], trigger : () => scheduler.zoomToLevel('hourAndDay') },

            { moveMouseTo : '.b-grid-header' },

            { waitForSelectorNotFound : '.b-sch-scheduletip:not(.b-hidden)', desc : 'Waiting for cell tooltip to hide' },

            { moveMouseTo : '.b-sch-timeaxis-cell' },

            { waitForSelector : '.b-sch-scheduletip:not(.b-hidden) .b-sch-clock-hour', desc : 'Waiting for cell tooltip' },

            { mouseDown : null },
            { moveMouseBy : [[100, 0]] },
            { waitForSelector : '.b-tooltip:not(.b-hidden) .b-sch-clock-hour', desc : 'Waiting for drag tooltip' },
            { mouseUp : null }
        );
    });
    ...
    t.it('Should be able to fully control the HTML shown in the tooltip using generateTipContent', t => {
        ...
        t.chain(
            { moveMouseTo : '.b-sch-timeaxis-cell', offset : [0, 0] },
            { moveMouseTo : '.b-sch-timeaxis-cell', offset : [10, 10] },
