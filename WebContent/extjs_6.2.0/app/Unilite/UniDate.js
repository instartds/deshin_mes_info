...
Ext.define('Unilite.UniDate', {
	...
	get:function(type, basisDate) {
		...
		}else if(type == 'startOfLastMonth'){			//전월 1일
			rv = dt.add('month',-1).startOf("month").format(format);
		}else if(type == 'startOfNextMonth'){
			rv = dt.add('month',1).startOf("month").format(format);
		}else if(type == 'endOfTwoNextMonth'){
			rv = dt.add('month',2).endOf("month").format(format);
		...
		}else if(type == 'twelveMonthsAgo'){
			dt.add('year',-1).format(format);
			rv = dt.add('month',+1).format(format);  //12달만 보여주기 위해..
		}else if(type == 'sixDayAgo'){
			rv = dt.add('day',-6).format(format);	//6일전(오늘 포함)
		}else if(type == 'sixDayLater'){
			rv = dt.add('day',+6).format(format);	//6일후(오늘 포함)
		}else if(type == 'todayOfNextMonth'){
			rv = dt.add('month',+1).format(format);
		}else if(type == 'threeMonthsLater'){		//3개월 후 - 20200212 추가
			rv = dt.add('month',+3).format(format);
		}else if(type == 'endOfNextMonth'){			//익월말일 - 20200309 추가
			rv = dt.add('month', 1).endOf("month").format(format);
		}else if(type == 'sixMonthsAgo'){			//6개월 전 - 20210507 추가
			rv = dt.add('month', -6).startOf("month").format(format);
		}
