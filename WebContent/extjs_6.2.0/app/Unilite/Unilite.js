...
//		        return record ? record.get(combo.displayField) : combo.valueNotFoundText;
		        ...
		        var parentValue = '';
		        if(combo && combo.parentFieldName)	{
		        	parentValue = record.get(combo.parentFieldName);
		        }
		            ...
		            if (!record || !record.isModel) {
		            	if(!Ext.isEmpty(parentValue))	{
		            		if(combo && combo.store)	{
		            			...
		            			combo.store.filterBy(function(rec){return rec.get('option') == parentValue}, combo.store);
		            		}
		            	}
	...
	form: {
		createCombobox : function(field) {
			...
			if(!Ext.isEmpty(field.autoSelect)) {
				Ext.apply(comboConfig, {'autoSelect': field.autoSelect})
			}
			var combo ;
			if(Ext.isDefined(field.multiSelect)) {
...
//						config.validations.push({'type': 'presence', 'field': field.name});
	...
	*/
	...
	,isGrandSummaryRow:function (summaryData, metaData) {
		...
		if(Ext.isDefined(metaData.record.ownerGroup)){
			return false;
		} else {
			return true;
		}		
	...
	focusNextField: function(field, selectText, e) {
		...
		var upEl = field.up();
		var upElXtype = (upEl && upEl.xtype) ? upEl.xtype: "";  // Grid가 formpanel 내부에 구성된 경우 체크

		if(form && field.isFormField && upElXtype != "celleditor") { //form 내 필드인 경우
			...
			if(focusable && focusable.el && Ext.isDefined(focusable.up('form'))) {
				...
				if(focusable)	{
					this._targetFocus(field, focusable, selectText);
					...
					if(targetField) {
						Ext.fly(targetField).focus();
						if(selectText && focusable.getXType() != "radiofield" && focusable.getXType() != "checkboxfield" ) {
				...
				editablePGM = false;
				if(Ext.isDefined(PGM_ID) && PGM_ID.indexOf("ukr")>0)	{
					editablePGM = true;
				}
				...
				if(!editablePGM && !isPopup)	{
					if(Ext.isDefined(UniAppManager) && Ext.isDefined(UniAppManager.app))	{
						...
						UniAppManager.app._clickToolBarButton('query')
					}
				} else {
			...
			if(grid) {
				if (e.getKey() === Ext.EventObjectImpl.RIGHT || e.getKey() === Ext.EventObjectImpl.LEFT) {
					...
					e.stopEvent();
				}/*else if(e.getKey() == Ext.EventObjectImpl.ENTER && field.getXType() !='uniPopupColumn'  && field.getXType() !='uniCombobox')	{
					
					if(grid.editing || ( grid.up('grid') && grid.up('grid').editing)) {
						e.keyCode = Ext.EventObjectImpl.TAB;
						grid.fireEvent('specialkey', null, field, e);
					} 
				}else if(e.getKey() == Ext.EventObjectImpl.ENTER && field.getXType() =='uniCombobox')	{
					e.keyCode = Ext.EventObjectImpl.TAB;
					e.shiftKey = false;
					if(grid.editing || ( grid.up('grid') && grid.up('grid').editing)) {
						grid.editing.completeEdit();
						var view = grid.editingPlugin.view;
	...
	_targetFocus:function(field, focusable, selectText)	{
		if(focusable.el)	{
			if(focusable.xtype == 'uniTagfield')	{
				targetField = focusable.el.down('.x-tagfield-input-field');
			}else {
			...
			if(targetField) {
				...
				Ext.fly(targetField).focus();
				if(selectText && focusable.getXType() != "radiofield" && focusable.getXType() != "checkboxfield" ) {
					//setTimeout(function(){targetField.dom.select();},10);
				}
    ...
    // get number of decimal places to shift
	    var exp = 1;
	    if(a.toString().length - b.toString().length > 0) {
	    	exp = a.toString().length ;
		}else {
	    	exp = b.toString().length ;
	    }
	    
	   function makeInt (num) {
	       var value = Math.round(num * Math.pow(10, exp));
    ...
    // get number of decimal places to shift
    	...
    	if(sA.indexOf(".") == -1 && sB.indexOf(".") == -1)	{
    		return (a*b);	
    	}
    	
    	var aDp = (sA.indexOf(".") == -1) ? sA.length : sA.indexOf("."), 
    		bDp = (sB.indexOf(".") == -1) ? sB.length : sB.indexOf(".");
    	
	    var exp = 1;
	   
	    if( (sA.length-aDp) - (sB.length-bDp) > 0) {
	    	exp = (sA.length-aDp)+1 ;
		}else {
	    	exp = (sB.length-bDp)+1 ;
	    }
	...
	/**
	 * float 연산 오류 보정함수
	 * @param {} a : {"number": 10, "format": UniFormat.Qty or "0,000.00"}
	 * @param {} b : {"number": 19.99, "format": UniFormat.UnitPrice or "0,000.000"}
	 * @return {}
	 */
	 multiplyFormat:function (a, b) {

	    var aFormat, aNumber,bFormat,bNumber
	    if( !isNaN(parseFloat(a)) && isFinite(a) &&  !isNaN(parseFloat(a)) && isFinite(b))	{
	    	return 	multiply(a,b);
	    }
	    
	    if(a && a.number && a.format &&  !isNaN(parseFloat(a.number)) && isFinite(a.number) )	{
	    	aFormat = a.format;
	    	aNumber = a.number;
	    }else {
	    	console.log('Unilite.multiply param 오류');
	    }
	    
	    if(b && b.number && b.format &&  !isNaN(parseFloat(b.number)) && isFinite(b.number) )	{
	    	bFormat = b.format;
	    	bNumber = b.number;
	    }else {
	    	console.log('Unilite.multiply param 오류');
	    ...
	    if(aFormat.toString().length - bFormat.toString().length > 0) {
        
	    	exp = aFormat.indexOf('.') > -1 ? aFormat.length-1 - aFormat.indexOf('.') : 0;
		}else {
	    	exp = bFormat.indexOf('.') > -1 ? bFormat.length-1 - bFormat.indexOf('.') : 0; ;
	    }
	   ...
	   return makeFloat(makeInt(aNumber) * makeInt(bNumber));
	},
	
	divisionFormat:function (a, b) {
	...
	/**
	 * 
	 * @param {} message : 기본 메세지
	 * @param {} messsageDesc : 상세내용
	 * @param {} title : 에러유형("에러", "경고", "정보"
	 * @param {} opt : { 'showDetail' : false }
	 */
	messageBox:function(message, messsageDesc, title, opt){
		if(Ext.isEmpty(title))	{
			title="";
		}
		if(!opt)	{
			opt = {'showDetail' : false }
		}
		if(messsageDesc){
			messsageDesc = messsageDesc.replace(/<br\s*[\/]?>/gi, "\n");
		}
		var messageWin = Ext.create({
			    xtype: 'window',
			    height: opt.showDetail ? 300:150,
			    width: 512,
			    minWidth:512,
			    maxWidth:512,
			    title: title,
				...
				items:[{
					...
					items:[{
	                	xtype:'button',
	                	width:100,
	                	text: opt.showDetail ?  UniUtils.getLabel('system.label.commonJS.exception.detailClose','상세내용 접기') : UniUtils.getLabel('system.label.commonJS.exception.detail','상세내용 보기'),
	                	enableToggle : true,
	                	hidden : opt.showDetail ?  !opt.showDetail : Ext.isEmpty(messsageDesc),
	                	toggleHandler:function(btn, state)	{
	                		var txtForm = messageWin.down('#messageComponent');
	                		var txtBox = txtForm.getField('messageDetail');
	                		if(state){
		                		messageWin.setHeight(300);
		                		txtBox.setVisible(true);
		                		btn.setText(UniUtils.getLabel('system.label.commonJS.exception.detailClose','상세내용 접기'));
	                		} else {
	                			messageWin.setHeight(150);
		                		txtBox.setVisible(false);
		                		btn.setText(UniUtils.getLabel('system.label.commonJS.exception.detail','상세내용 보기'));
	                		}
	                	}
	                },{
	                	xtype:'component',
				...
				listeners:{
					...
					show:function(){
						if(messageWin.down('#comonMessageCloseBtn'))	{
							setTimeout(function(){messageWin.down('#comonMessageCloseBtn').getEl().focus();}, 10);
						}
					}
				}
			}); 
...
function uniDirectExceptionProcessor(event) {
	...
	if(event && event.xhr && event.xhr.request && event.xhr.request.jsonData && event.xhr.request.jsonData.action == "badgeService")	{
		return;
	}
	if( event.type == "exception" ) {
		if (event.message == "InvalidSessionException") {
			...
			var messageArr, vMessage1="", vMessage2="" ;
			var rexp = /\/n/g ;
			var sWHere = (event && event.where) ? event.where.replace(rexp, "<br/>") :"";
			if(event.where)	{
				messageArr =  event.where.split("||");
			} else {
				messageArr = event.where;
			}
			if(Ext.isEmpty(messageArr) || messageArr == "&nbsp;")	{
				vMessage1 = "서버에서 오류가 발생되었습니다. 관리자에게 문의하세요.";
			} else {
				vMessage1 = (messageArr[0]) ? messageArr[0].replace(/\/n/g, "<br/>"):"";
				vMessage2 = (messageArr[1]) ? messageArr[1].replace(/<br \/>/g, "\n"):"";
				if(messageArr[2]) vMessage2 += messageArr[2];
			}
			...
			var messageWin = Ext.create({
				...
				items:[{
					...
					items:[{
	                	xtype:'button',
	                	width:100,
	                	text:UniUtils.getLabel('system.label.commonJS.exception.detail','상세내용 보기'),
	                	enableToggle : Boolean ,
	                	hidden : Ext.isEmpty(vMessage2),
	                	toggleHandler:function(btn, state)	{
	                		var txtForm = messageWin.down('#messageComponent');
	                		var txtBox = txtForm.getField('messageDetail');
	                		if(state){
		...
		if(Ext.isEmpty(event.where))	{
			vMessage = UniUtils.getLabel('system.message.commonJS.exception.connectionError', "서버연결이 잠시 중단되었습니다. 잠시 후에 다시 시도해 보세요.");
		} 
		Ext.MessageBox.show({
            title:  CommonMsg.errorTitle.ERROR,
            msg: vMessage,
