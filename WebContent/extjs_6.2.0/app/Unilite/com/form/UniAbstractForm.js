 ...
 */
 Ext.define('Unilite.com.form.UniAbstractForm', {
	...
	fieldDefaults : {
		msgTarget : 'qtip',
		labelAlign : 'right',
		blankText : UniUtils.getMessage('system.message.commonJS.form.fieldblankText','값을 입력해 주세요!'),
		labelWidth : 90,
		//width:250,
		labelSeparator : "",
...
//		this.addEvents('uniOnChange');
		var me = this;
		me.on("beforeaction", me._onBeforeAction, me);
        me.callParent();
	},
	_onBeforeAction:function(form, action){
		if(MODIFY_AUTH == "false")	{
			if(action.type.indexOf("submit") > -1)	{
				Unilite.messageBox( UniUtils.getMessage('system.message.commonJS.modifyAuth','자료 수정 권한이 없습니다.'));
	...
	setActiveRecord: function(record) {
		...
		if(record && record.$className === 'Unilite.com.ValidateService.ExtRec')	{
			record = record.obj;
		}
...
//				field.addEvents('uniOnChange');
				...
				}else if ('radiofield' == field.xtype || 'checkboxfield' == field.xtype)  {
					field.on('change', function(field, newValue, oldValue, eOpts) {
							console.log('radiogroup change');
							form._onFieldBlurFunction('change',field, newValue);
					}, this );
	...
	getInvalidMessage:function()	{
		...
		if(invalid.length > 0) {
			...
			if(Ext.isDefined(invalid.items[0]['fieldLabel'])) {
				var labelText = invalid.items[0]['fieldLabel']+UniUtils.getMessage('system.message.commonJS.form.invalidText','은(는) 필수입력 항목입니다.');
			} else if(Ext.isDefined(invalid.items[0].ownerCt)) {
				var labelText = invalid.items[0].ownerCt['fieldLabel']+UniUtils.getMessage('system.message.commonJS.form.invalidText','은(는) 필수입력 항목입니다.');
			}
