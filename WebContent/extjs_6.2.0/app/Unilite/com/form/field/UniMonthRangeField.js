...
Ext.define('Unilite.com.form.field.UniMonthRangeField', {
	 ...
	 */
    presetDates: [
        {text:UniUtils.getLabel('','이번달'), startDate: moment().startOf('month'),                      	endDate: moment().endOf('month')},
        {text:UniUtils.getLabel('','최근6개월'), startDate: moment().add('month',-6).startOf('month'),    endDate: moment()},
        {text:UniUtils.getLabel('','금년도'), startDate: moment().startOf('year'),    					endDate: moment().endOf('year')},
	 ...
	 */    
    presetFutureDates: [
		{text:UniUtils.getLabel('','한달후'), startDate: moment().add('month',1).startOf('month'),    endDate: moment().add('month',1).endOf('month')},
		{text:UniUtils.getLabel('','두달후'), startDate: moment().add('month',2).startOf('month'),    endDate: moment().add('month',2).endOf('month')},
    	...
    	var toolBar = {
    		...
    		items: [    
    			...
    			{xtype:'button', text: UniUtils.getLabel('system.label.commonJS.datefield.btnFuture','미래'),iconCls: 'menu-menuShow',menu: menuFuture},
	    	 	{xtype:'button', text:UniUtils.getLabel('system.label.commonJS.datefield.thisMonth','이번달'), colspan:2,  handler:function(){me.setValuesAndGo(UniDate.get("today"), UniDate.get("today"));}},
	    	 	'->',
	...
	setDD:function(field)	{
		...
		if(Ext.isEmpty(this.startDD) && Ext.isEmpty(this.endDD) )	{
			if(field)	{
				if(field.name == this.startFieldName)	{
					var sValue = this.startDateField.getValue();
					if(sValue) {
						if(!moment.isMoment(sValue)) {
						 	sValue =moment(UniDate.extParseDate(sValue));
				...
				if(field.name == this.endFieldName)	{
					var eValue = this.endDateField.getValue();
					if(eValue) {
						if(!moment.isMoment(eValue)) {
							eValue =moment(UniDate.extParseDate(eValue));
					...
					eValue = UniDate.get('endOfMonth', sValue);
					me._fixedEndDate = eValue;
				}
			}
		}
