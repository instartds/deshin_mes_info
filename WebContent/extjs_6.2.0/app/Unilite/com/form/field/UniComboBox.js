...
Ext.override(Ext.view.BoundListKeyNav, {
	...
	/*	var e = arguments[0];
        ...
        if(!checkSearchField)	{
	        ...
	        if(field && field.getValue() && !Ext.isArray(field.getValue()))	{
...
//		}
    ...
    onItemMouseDown: function(view, record, item, index, event) {
        ...
        if(view.up() && view.up().multiSelect)	{
        	var combo = view.up();
        	var select = false;
        	var removeValues =[];
        	if(event.shiftKey)	{
	        	var combo = view.up();
	        	if(Ext.isDefined(view.shiftStartIndex) && view.shiftStartIndex > -1)	{
	        		if(view.getSelectionModel() && view.getSelectionModel().store && view.getSelectionModel().store.data && view.getSelectionModel().store.data.items)	{
	        			if(combo.getValue().indexOf(record.get(combo.valueField)) == -1){
	        				select = true;
	        			} 
		        		if(view.shiftStartIndex > index )	{
		        			Ext.each(view.getSelectionModel().store.data.items, function(item, idx){
		        				if( index < idx && idx < view.shiftStartIndex ){
		        					if(select)	{
		        						combo.addValue(item.data[combo.valueField]);
		        					} else {
		    	        				removeValues.push(item);
		        					}
		        				}
		        			})
		        		} else {
		        		...
		        		if(removeValues.length > 0)	{
		        			...
		        			Ext.each(values,function(value, idx){
		        				var addValue = true;
		        				Ext.each(removeValues,function(rItem, idx){
		        					if(value == rItem.get(combo.valueField)){
		        						addValue = false;
		        					}
		        				})
		        				if(addValue)	{
...
//        }
        ...
        if (me.store.getCount() > 0 || me.getPicker().emptyText) {
            me.expand();
        } else {
            me.collapse();
        }
...
//    growToLongestValue: true,
    ...
    constructor : function(config){    
        ...
        } else if(Ext.isDefined(config.valueWidth) && config.valueWidth > 40)	{
        	valueCss = 'uni_combo_value_none_width';
        }
		
        var valueStyle = '';
        if(Ext.isDefined(config.valueWidth) && config.valueWidth > 40)	{
	...
	initComponent: function () {
        ...
        var valueCss = "uni_combo_value";
	 	if(me.includeMainCode)	{
			valueCss = "uni_combo_value_wide";
        }else if(Ext.isDefined(me.valueWidth) && me.valueWidth > 40)	{
        	valueCss = 'uni_combo_value_none_width';
        }
		
        var valueStyle = '';
        if(Ext.isDefined(me.valueWidth) && me.valueWidth > 40)	{
...
//        me.addEvents("changedivcode");
        ...
        me.on('specialkey', me._onSpecialkey, me);
        /*
		...
		if(this.child || me.parentNames) {
			...
			if(me.childCombo || me.parentNames) {
				...
				me.on('change', function(combo, newValue, oldValue, eOpts) {	
					if(combo.childCombo && combo.childCombo.value) {
...
//	},
	_onExpand: function(combo) {
		...
		var me = this;
		if(picker.getEl())	{	
			picker.getEl().on('mouseover',function(elm){
	        	if(elm.target )	{
	        		elm.target.focus();
	        	}
	        });

			/*picker.enableKeyEvents = true
			picker.getEl().on('keypress ',function(elm, event, target){
				if(elm.target )	{
			...
			picker.getEl().on('keydown ',function(elm, event, target){
				if(elm.target )	{
		...
		picker.shiftStartIndex = -1;
		var valueSize = 0;
		if(me.valueWidth > 40)	{	// value part default size : 40
			valueSize = me.valueWidth - 40;
		}
		var scrollWidth = 0;
		if(me.store && me.store.getCount() > 20)	{
			scrollWidth = 25
		}
		if(combo.pickerWidth)	{
			...
			var growLen = this.charlen(combo.getGrowWidth()) - ((combo.getEl().getWidth()-40)/7);
			
			if(growLen > 0) { 
				if(combo.ownerForm)	{
					picker.setWidth(combo.bodyEl.getWidth() + (growLen*7)+10+valueSize+40 +scrollWidth);
					picker.setBorder( 1 );
				...
				picker.setWidth(combo.bodyEl.getWidth()+40 + valueSize+scrollWidth);
			}
		}
		/*if(me.store.getCount() == 1)	{
			var item = me.store.getAt(0);
			if(item.get(me.displayField) == combo.value || item.get(me.valueField) == combo.value)	{
        		picker.select(0);
        		picker.onFocusLeave();
			}
        	return;
	...
	onBeforequery : function(queryPlan, eOpts) { 
    	...
    	//20200812 combo.store.filter('option', pValue); option 으로 지정된 내용을 콤보에서 설정된 값으로 사용하기 위해 변경(default는 option)
    	var parentFilterName = 'option';
    	if(!Ext.isEmpty(combo.parentFilterName))	{
    		parentFilterName = combo.parentFilterName;
    	}
    	...
    	if ( combo.parentFieldName ) {
    		...
    		if(combo.ownerForm) {	// form일경우
    			...
    			//locked columns 이 있는 Grid의 경우 ownerCt 이용
    			pValue = (Ext.isEmpty(grid.uniOpt)) ? grid.ownerCt.uniOpt.currentRecord.get(combo.parentFieldName):grid.uniOpt.currentRecord.get(combo.parentFieldName);
    		}
    		if( Ext.isDefined(combo.parentNames) &&  Ext.isEmpty(pValue))	{
    			...
    			if(Ext.isArray(pValue))	{
    				...
    				combo.store.filterBy(function(record){
    					return UniUtils.indexOf(record.get(parentFilterName), pValue);
    				});
    			} else {
    				combo.store.filter(parentFilterName, pValue);	//Parent 필드값으로 필터를 설정 
    			}
				return true;
    		} else {
...
//    },
	...
	_onSpecialkey:function(field, event, eOpts){
		var me = this;
    	...
    	if(event.getKey() == 13)	{
			...
			if( upElXtype == "celleditor" || upElXtype == "editor") { 
				...
				view.setActionableMode(false, view.actionPosition);
			
			}else if( event.shiftKey && !event.ctrlKey && !event.altKey) {
				Unilite.focusPrevField(field, true, event);
	    	}else if( !event.shiftKey && !event.ctrlKey && !event.altKey){
	    		Unilite.focusNextField(field, true, event);
	    	}
    	}
	...
	_onSelect:function(field, event, eOpts){
		console.log(event);
	}
});
