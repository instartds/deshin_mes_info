...
Ext.define('Ext.overide.grid.NavigationModel', {
    ...
    initKeyNav: function(view) {
        ...
        nav = new Ext.util.KeyNav({
            ...
            113: me.onKeyF2,
            tab: me.onKeyTab,
            112:me.onFunctionKey,
            114:me.onFunctionKey,
            115:me.onFunctionKey,
           
            117:me.onFunctionKey,
            118:me.onFunctionKey,
            119:me.onFunctionKey,
            120:me.onFunctionKey,
            A: {
                ctrl: true,
    ...
    onFunctionKey:function(e)	{
    	
    	switch(e.getKey())	{
	    	case 112: if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {
	    				UniAppManager.app.onManualButtonDown();
	    			  }
	    	break;
	        case 114:if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {
	        			UniAppManager.app._clickToolBarButton('reset');
	        		  }
	    	break;
	        case 115:if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {
	        			UniAppManager.app._clickToolBarButton('newData');
	        		  }
	    	break;
	    	...
	    	break;
	        case 118: if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {
	        			UniAppManager.app._clickToolBarButton('save');
	        		  }
	    	break;
	        case 119: if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {	// only F8 (인쇄(미리보기))
                		var btn = UniAppManager.app.getTopToolbar().getComponent('print');
                		if(btn && btn.isVisible())	{
                			UniAppManager.app._clickToolBarButton('print');
                		} else {
                			UniAppManager.app._clickToolBarButton('printPreview');
                		}
                		e.stopEvent();
                	 } else if( !e.shiftKey && e.ctrlKey && !e.altKey ) {	// Ctrl + F8 (인쇄)
                		UniAppManager.app._clickToolBarButton('directPrint');
                		e.stopEvent();
                	 }
	    	break;
	        case 120: if( !e.shiftKey && !e.ctrlKey && !e.altKey ) {
	        			UniAppManager.app.onCloseButtonDown();
	        		  }
	    	break;
    	}
    },
    ...
    onKeyEnter:function(keyEvent)	{
    	...
    	if(view.ownerGrid.uniOpt.useNavigationModel)	{
    		if(UserInfo && UserInfo.appOption && UserInfo.appOption.downEnterKey == "D") {
   ...
   onKeyRight : function(keyEvent) {
		var position = keyEvent.position, view = position.view;
		console.log("#################### EVENT id : ", view.ownerGrid.id);
		console.log("#################### EVENT uniOpt : ",
				view.ownerGrid.uniOpt.enterKeyCreateRow);
		if (keyEvent.keyCode == 13 && view.store.uniOpt.editable
				&& keyEvent.position.isLastColumn()
				&& !keyEvent.position.column.isLocked()
				&& view.ownerGrid.uniOpt.enterKeyCreateRow) {
			Ext.getBody().mask();
    ...
    onKeyTab: function(keyEvent) {
        ...
        if (focusTarget) {
            ...
            //text 선택되도록 추가
            setTimeout(function(){
            	if(focusTarget && Ext.isFunction(focusTarget.select)) {
            		focusTarget.select();
            	}
            },20);
            //text 선택되도록 추가 끝
    ...
    onKeyF2: function(keyEvent) {
        ...
        //grid.setActionableMode(!actionableMode, actionableMode ? null : UniAppManager.app.onQueryButtonDown());
        grid.setActionableMode(!actionableMode, actionableMode ? null : UniAppManager.app._clickToolBarButton('query')); 
    }
});
/*
 * Ext.define('Ext.overide.grid.CellEditor', { override: 'Ext.grid.CellEditor',
 * 
 * onSpecialKey: function(field, event) {
 * 
 * var me = this, key = event.getKey(), complete = me.completeOnEnter && key ===
 * event.ENTER, cancel = me.cancelOnEsc && key === event.ESC, view =
 * me.editingPlugin.view;
 * 
 * 
 * if (complete || cancel) { // Do not let the key event bubble into the
 * NavigationModel after we're don processing it. // We control the navigation
 * action here; we focus the cell. event.stopEvent();
 *  // Maintain visibility so that focus doesn't leak. // We need to direct
 * focusback to the owning cell. if (complete) { me.completeEdit(true); } else
 * if (cancel) { me.cancelEdit(true); }
 * 
...
Ext.define('Ext.overide.grid.plugin.Editing', {
	...
	onCellClick: function(view, cell, colIdx, record, row, rowIdx, e) {
        ...
        if(editor && editor.el){
	  	    // input text 선택
        	if(view.ownerGrid.uniOpt && view.ownerGrid.uniOpt.nonTextSelectedColumns && view.ownerGrid.uniOpt.nonTextSelectedColumns.indexOf(view.getColumnManager().columns[colIdx].dataIndex) < 0)	{
				var targetField = editor.el.down('.x-form-field');
				if(targetField) {
					targetField.focus(10);		
					// cell 에서 클릭할 경우만 선택되고, editor rendering 후에는 선택되지 않도록 조건 추가
					if(e.target && e.target.nodeName =="DIV")	{  
						setTimeout(function(){
							if(targetField.dom && Ext.isFunction(targetField.dom.select))
...
Ext.define('Unilite.com.grid.UniAbstractGridPanel', {
							...
							beforeedit: function(editor, e, eOpts) {
								...
								if( (e.column.uniOPT && e.column.uniOPT.isPk ) || (e.column.isLink) ) {
									...
									if(targetField) {
										targetField.focus(10);						
										setTimeout(function(){
										//if(Ext.isFunction(targetField.dom.select)) targetField.dom.select() 
											}, 1000);
									}
	...
	createRow:function(grid, values, startEditColumnName, rowIndex, newRecord) {
		...
		var navi = grid.ownerGrid.getNavigationModel();
		...
		if(Ext.isFunction(view.getNavigationModel) && view.getNavigationModel())	{
			if(view.normalGrid)	{
				view.normalGrid.scrollTo(0,view.normalGrid.getScrollY(),false);
			} else {
				view.ownerGrid.scrollTo(0,view.ownerGrid.getScrollY(),false);
			}
			
			var sCell,targetFocus;
			if(view.getRow(newRecord))	{
				sCell = view.getCell(newRecord, sColumn, true);
			}
			if(sCell)	{
				targetFocus = Ext.fly(sCell).focus();
			} else {
				setTimeout(function(){
					var sCell = view.getCell(newRecord, sColumn, true);
					if(sCell)	{
						targetFocus = Ext.fly(sCell).focus();
						navi.setPosition(rowIndex, columnIndex);
					}
