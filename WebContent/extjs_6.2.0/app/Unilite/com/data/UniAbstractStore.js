...
Ext.define('Unilite.com.data.UniAbstractStore', {
            ...
            options=Ext.apply(options, {
            	callback: function(batch, option) {
            		...
            		if(batch.exceptions && batch.exceptions.length < 1) {
            			UniAppManager.updateStatus(UniUtils.getMessage('system.message.commonJS.store.saved','저장되었습니다.') );// "저장되었습니다.");
            		}
            		Ext.getBody().unmask();
            	},
    ...
    syncAllDirect: function(options) {
        ...
        if (needsSync && me.fireEvent('beforesync', operations) !== false) {
            ...
            /*if(!options.failure)	{
            	options.on("failure", Ext.getBody().unmask(), me)
            }*/
            options=Ext.apply(options, {
            	callback: function(batch, option) {
            		console.log("callback exceptions :", batch.exceptions);
            		if(batch.exceptions && batch.exceptions.length < 1) {
            			if(option.useSavedMessage !== false)	{
            				UniAppManager.updateStatus(UniUtils.getMessage('system.message.commonJS.store.saved','저장되었습니다.'));// "저장되었습니다.");
            			}
            		}
            		if(option.delayMaskTime && option.delayMaskTime > 0)	{
            			setTimeout(function() {Ext.getBody().unmask();}, option.delayMaskTime);
            		} else {
            			Ext.getBody().unmask();
            		}
            	}/*,
