...
Ext.define('Unilite.com.data.UniStore', {
    ...
    constructor: function(config){
        ...
        me.on('beforesync', me._onBeforesync, me);
    ...
    ,_onBeforesync: function()	{
    	//자료 저장 권한
    	if(MODIFY_AUTH == "false")	{
    		Unilite.messageBox(UniUtils.getMessage('system.message.commonJS.modifyAuth', '자료수정 권한이 없습니다.'));
    		 return false;
    	}
    }
    ...
    sumBy: function(queryFn, sumCols) {
		...
		Ext.each(sumCols, function(colName) {
			...
			Ext.each(records.items, function(record){
				var col = Ext.isNumeric(record.get(colName)) ? parseFloat(record.get(colName)):0;
				results[colName] = parseFloat(results[colName]) +  col;
			});
		});
		return results;
    },
