...
Ext.define('Unilite.com.ValidateService',{
	...
	_fieldBeforeFn:function(field, service ) {
		var rv = true,  form=field.uniOpt.ownForm; 
		var lastValidValue = field.uniOpt.lastValidValue;
		var newValue = "";
		if(field.xtype == 'uniRadiogroup' && !Ext.isEmpty(field.getValue())) {
			newValue = field.getValue()[field.name];
		} else {
	...
	resetToLastValue : function(field){
        ...
        me.beforeReset();
        var lastValidValue = (!Ext.isEmpty(uniOpt.lastValidValue)) ? uniOpt.lastValidValue:me.originalValue;
        if(field.xtype == 'uniRadiogroup')	{
        	var boxes  = field.getBoxes();
        	for (var b = 0; b < boxes.length; b++) {
	            box        = boxes[b];
	            inputValue = box.inputValue;
				if (inputValue !== lastValidValue) {
	                box.updateValueFromDom();
	            }
        	}
        	field.publishState('value', lastValidValue);
        	setTimeout(function(){
        		me.setValue(lastValidValue);
	        	for (var b = 0; b < boxes.length; b++) {
		            box        = boxes[b];
		            inputValue = box.inputValue;
					if (inputValue == lastValidValue) {
	...
	alert: function(message, tit) {
		if(!(message == false || message == true || message == undefined))	{
			Unilite.messageBox(tit +'<br \><br \>'+ message, null, CommonMsg.errorTitle.ERROR );
		}
	}
});  // ValidateService