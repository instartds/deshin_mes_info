 ...
 */
/**
 * @class nbox.calendar.view.AbstractCalendar
 ...
 */
Ext.define('nbox.calendar.view.AbstractCalendar', {
    ...
    requires: [
        //'nbox.calendar.form.EventDetails',
        //'nbox.calendar.form.EventWindow',
        'nbox.calendar.menu.Event',
        'nbox.calendar.dd.DragZone',
        'nbox.calendar.dd.DropZone'
    ],
    
    /**
     * @cfg {Ext.data.Store} eventStore
     * The {@link Ext.data.Store store} which is bound to this calendar and contains {@link nbox.calendar.data.EventModel EventRecords}.
     ...
     * The {@link Ext.data.Store store} which is bound to this calendar and contains {@link nbox.calendar.data.CalendarModel CalendarRecords}.
     ...
     * @type nbox.calendar.CalendarPanel
     * If this view is hosted inside a {@link nbox.calendar.CalendarPanel CalendarPanel} this property will reference
     ...
     * {@link nbox.calendar.data.EventModel}) to populate the calendar views with events. Internally this method
...
viewConfig: {
     ...
     * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} being rendered
    ...
    initComponent : function(){
        ...
        this.addEvents({
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this 
             */
            eventsrendered: true,
            /**
             * @event eventclick
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that was clicked on
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that the cursor is over
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that the cursor exited
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that will be moved
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that was moved with
             ...
             * @param {nbox.calendar.view.AbstractCalendar} this
             * @param {nbox.calendar.data.EventModel} rec The {@link nbox.calendar.data.EventModel record} for the event that was deleted
             * @param {Ext.Element} el The target element
             */
    ...
    afterRender : function(){
        ...
        if(this.recurtypeStore){
        	this.setRecurtypeStore(this.recurtypeStore, true);
        }
    ...
    getStoreParams : function(){
        ...
        params.QRY_TYPEA = this.up('panel').uniQryTypeA;
        params.QRY_TYPEB = this.up('panel').uniQryTypeB;
        params.QRY_TYPEC = this.up('panel').uniQryTypeC;
        
        params.SEARCH_TEXT = this.up('panel').searchText;
        
        //params.PROJECT_OPT = this.up('panel').uniInOutType;
        // params.number = 123;
    ...
    refresh : function(reloadData){
        //nbox.log('refresh (base), reload = '+reloadData);
        if(reloadData === true){
            this.reloadStore();
        }
        this.prepareData();
            ...
            for(d = 0; d < this.dayCount; d++){
                if(evtsInView.getCount() > 0){
                    var evts = evtsInView.filterBy(function(rec){
                        var startDt = Ext.Date.clearTime(rec.data[nbox.calendar.data.EventMappings.StartDate.name], true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > rec.data[nbox.calendar.data.EventMappings.StartDate.name]));
	...
	onCalendarEndDrag : function(start, end, onComplete){
        ...
        dates[nbox.calendar.data.EventMappings.StartDate.name] = start;
        dates[nbox.calendar.data.EventMappings.EndDate.name] = end;
    ...
    onUpdate : function(ds, rec, operation){
        ...
        if(operation == Ext.data.Record.COMMIT){
            ...
            var rrule = rec.data[nbox.calendar.data.EventMappings.RRule.name];
			...
			if(this.enableFx && this.enableUpdateFx){
				this.doUpdateFx(this.getEventEls(rec.data[nbox.calendar.data.EventMappings.EventId.name]), {
                    scope: this
                });
			}
        }
    ...
    onAdd : function(ds, recs, index){
		...
		if(this.enableFx && this.enableAddFx){
			this.doAddFx(this.getEventEls(rec.data[nbox.calendar.data.EventMappings.EventId.name]), {
                scope: this
            });
		};
    },
    ...
    onRemove : function(ds, rec){
		...
		if(this.enableFx && this.enableRemoveFx){
			this.doRemoveFx(this.getEventEls(rec.data[nbox.calendar.data.EventMappings.EventId.name]), {
	            remove: true,
	            scope: this,
			...
			this.getEventEls(rec.data[nbox.calendar.data.EventMappings.EventId.name]).remove();
...
//	},
    ...
    isEventVisible : function(evt){
        ...
        if(calRec && calRec.data[nbox.calendar.data.CalendarMappings.IsHidden.name] === true){
    ...
    isOverlapping : function(evt1, evt2){
        ...
        var start1 = ev1[M.StartDate.name].getTime(),
...
//        }
    ...
    setStartDate : function(start, /*private*/reload){
        ...
        //nbox.log('setStartDate (base) '+Ext.Date.format(start, 'Y-m-d'));
...
alert('End: '+bounds.end);
	 ...
	 * of {@link #nbox.calendar.data.EventModel EventRecord} objects
	 */
    ...
    /**
     * Sets the event store used by the calendar to display {@link nbox.calendar.data.EventModel events}.
    ...
    /**
     * Sets the calendar store used by the calendar (contains records of type {@link nbox.calendar.data.CalendarModel CalendarRecord}).
     * @param {Ext.data.Store} store
     */
    ...
    getEventRecord : function(id){
        var idx = this.store.find(nbox.calendar.data.EventMappings.EventId.name, id, 
            0,     // start index
    ...
    getEventEditor : function(){
        ...
        if(!this.editWin){
            this.editWin = Ext.create('nbox.calendar.form.EventWindow', {
//            this.editWin = Ext.create('nbox.calendar.form.EventDetail', {
                id: 'ext-cal-editwin',
     ...
     * {@link nbox.calendar.form.EventWindow EventEditWindow}).
     * @param {nbox.calendar.data.EventModel} rec The event record
     ...
     * @return {nbox.calendar.view.AbstractCalendar} this
     */
    showEventEditor: function(rec, animateTarget){
    	
    	if(rec.data !== undefined) {
	    	if(rec.data.calendarId == 'H' || rec.data.calendarId == 'B')
	    		return this;
    	}
        ...
        return this;
    	//this._openPopup(rec);
        //this._getNewEventEditor(rec);
    },
    _getNewEventEditor: function(rec) {
        var me = this;
        var vParam = {};
        var appName = 'Unilite.app.calendar.eventEditor';
        console.log('_getNewEventEditor', rec);
        if(rec.get !== undefined ) {
            vParam = {
                docId: rec.get('DOC_NO'),
                startDate: rec.get('startDate'),
                startHour: rec.get('startHour'),
                type: rec.get('calendarId')
            }
        } else {
            vParam = {
                startDate: UniDate.getDateStr(rec.startDate),
                startHour: UniDate.getHHMI(rec.startDate)
            }
        }
        var fn = function() {
    ...
    _openPopup: function(rec) {
    	console.log("_openPopup:",rec.startDate, rec);
    	var width=805;
    	var height=720;
    	...
    	if(rec !== undefined ) {
    		if( rec.get !== undefined) {
    			param['DOC_ID'] = rec.get('DOC_NO');
    			param['type'] = rec.get('calendarId');
    		} else {
    			param['startDate'] = UniDate.getDateStr(rec.startDate);
    			param['startHour'] = UniDate.getHHMI(rec.startDate);
    		}
    	}
    	var sParam = nbox.param(param);
	    ...
	    var url = this.uniPopupUrl;
	    //var p = Ext.getCmp('uniCalendarPanel01');
	    this.disable();
	    console.log("showModalDialog",url+'?'+sParam, param, features );
     ...
     * {@link nbox.calendar.form.EventWindow EventEditWindow}, which will be hidden).
    ...
    showEventMenu : function(el, xy){
        if(!this.eventMenu){
            this.eventMenu = Ext.create('nbox.calendar.menu.Event', {
                listeners: {
     ...
     * @param {Object} rec The event {@link nbox.calendar.data.EventModel record}
     * @param {Object} dt The new start date
     */
    moveEvent : function(rec, dt){
        if(Extensible.Date.compare(rec.data[nbox.calendar.data.EventMappings.StartDate.name], dt) === 0){
            // no changes
            return;
        }
        if(this.fireEvent('beforeeventmove', this, rec, Ext.Date.clone(dt)) !== false){
            var diff = dt.getTime() - rec.data[nbox.calendar.data.EventMappings.StartDate.name].getTime();
            rec.beginEdit();
            rec.set(nbox.calendar.data.EventMappings.StartDate.name, dt);
            rec.set(nbox.calendar.data.EventMappings.EndDate.name, Extensible.Date.add(rec.data[nbox.calendar.data.EventMappings.EndDate.name], {millis: diff}));
            rec.endEdit();
            this.save();
