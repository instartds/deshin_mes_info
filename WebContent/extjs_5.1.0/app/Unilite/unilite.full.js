...
/**
 *  it is a bug that will be resolved in the 5.0.2 release
 *  http://www.sencha.com/forum/showthread.php?292085-Error-when-clicking-grid-with-empty-text-configured-and-no-data-in-IE9
 */
Ext.define('Ext.override.panel.Table', {
    override: 'Ext.panel.Table',
    
    handleFocusEnter: function (e) {
        var me = this,
            view = me.getView(),
            targetView, navigationModel = view.getNavigationModel(),
            lastFocused, focusPosition, br = view.bufferedRenderer,
            firstRecord;
...
Ext.define('Ext.overide.grid.plugin.CellEditing', {
    ...
    onSpecialKey: function(ed, field, e) {
        ...
        if (e.getKey() === e.TAB || e.getKey() === e.ENTER) {
            ...
            if (ed) {
                ...
                if(e.getKey() === e.ENTER){
            		if(ed && ed.editingPlugin && ed.editingPlugin.getActiveColumn()) {
            			ed.editingPlugin.getActiveColumn( ).field.focus(true);
				    }
            	
            	}
            	sm = ed.up('tablepanel').getSelectionModel();
            	var interValTime = 1;
            	// 팝업 필드의 경우 코드/코드명 가져오는 시간 기다리기 위해 interval을 줌.
            	if(field.getXType() == 'uniPopupColumn')	{
            	var chkInterval = 0;
            	chkInterval = setInterval(function(){
            											console.log("chkInterval:",chkInterval);
            											if(!Ext.getBody().isMasked()) {
            												moveCell(ed, e, sm, position, grid, view)
            												window.clearInterval(chkInterval)
            												console.log("clear chkInterval:",chkInterval);
										            	}
            										}
            										, interValTime);
	...
	function moveCell(ed, e, sm, position, grid, view)	{
	if (sm.onEditorTab) {
            
            	if(!ed.editingPlugin.context.grid.isLocked && !Ext.isDefined(grid.ownerGrid)  && position.colIdx < grid.headerCt.getVisibleGridColumns().length-1 /*|| (ed.editingPlugin.context.grid.isLocked && position.colIdx < grid.headerCt.getVisibleGridColumns().length-1 )*/ ){
	            	var r = sm.onEditorTab(ed.editingPlugin, e);
	            	...
	            	if(e.getKey() === e.ENTER){
						...
						if(grid.getItemId().indexOf("-normal") > -1)	{
							grid = grid.up('#'+grid.getItemId().replace('-normal',''));
						}
	            		if(position.isEqual(ed.editingPlugin.context))	{
	            			if('tree-grid' != ed.editingPlugin.context.grid.getXType() 
	            			   && !Ext.isDefined(grid.up('uniDetailWindow'))
	            			   && (grid.uniOpt && grid.uniOpt.enterKeyCreateRow)
		            		)	{
	                ...
	                return r;
            	}else if(  position.colIdx < grid.headerCt.getVisibleGridColumns().length-1)	{
            		var r = sm.onEditorTab(ed.editingPlugin, e);
            		...
            		var eView, eRows, eGrid ;
            		var newRecordChek = true;
            		if(view.isLockedView)	{
            			eView = grid.ownerGrid.getView().normalView;
            			eGrid = grid.ownerGrid.normalGrid;
            			eRows = grid.getSelectionModel().getSelection()[0]
            		}else {
            			eView = grid.ownerGrid.getView().lockedView ? grid.ownerGrid.getView().lockedView : grid.ownerGrid.getView();
            			eGrid = grid.ownerGrid.getView().lockedView ? grid.ownerGrid.lockedGrid : grid.ownerGrid  ;
            			
            			if(view.dataSource.getCount() == position.rowIdx+1)	{
            				if('tree-grid' != grid.getXType() 
	            			   && !Ext.isDefined(grid.up('uniDetailWindow'))
	            			   && (grid.uniOpt && grid.uniOpt.enterKeyCreateRow)
		            		)	{
            					UniAppManager.app.onNewDataButtonDown();  
            			...
            			eRows = view.dataSource.getAt(position.rowIdx+1);
            		}
            		if(newRecordChek)	{
	            		var colIdx = -1;
	            		Ext.each(eGrid.columns, function(column, idx){
	            			if(colIdx == -1 && Ext.isDefined(column.getEditor()))	{
	            				colIdx = idx;
	            			}
	            		});
	            		...
	            		ed.completeEdit();
	            		var focusPosition = new Ext.grid.CellContext(eView).setPosition({
	                        row: eRows,
	                        column: colIdx
	                    });
	                    
	            		grid.ownerGrid.getView().getNavigationModel().setPosition(focusPosition);
            		}
            		//sm.onEditorTab(ed.editingPlugin, e);
            		/*	            		
            		var normalGrid = grid.up().items.items[1];
        			var editable = true;
					var selModel = normalGrid.selModel;
					var selection = selModel.selection;
					if( selection &&  selection.column == position.colIdx &&  selection.row == position.rowIdx) {
						editable = true;
					} else {
						editable = false;
					}
					var expanderSelector = view.expanderSelector;
			        if ( editable && (!expanderSelector || !e.getTarget(expanderSelector)) ) {
			        	var	plugin = normalGrid.editingPlugin;
			        	if(plugin)	{
							if(e.getKey() == e.ENTER ){
								
				            	plugin.startEdit(normalGrid.getSelection(), normalGrid.ownerCt.getColumnManager().getHeaderIndex(0));
...
//    },
    ...
    //아무키나 누르면 Edit 모드 들어 가게
    //cellmodel -> rowmodel 로 변경하면서 필요없어짐. 2014.12.18
...
Ext.define('Unilite.com.grid.UniAbstractGridPanel', {
		...
		var editiong = Ext.create('Ext.grid.plugin.CellEditing', {
						...
						listeners: {
							...
							specialkey:function( ed, field, event, eOpts )	{
								console.log("specialkey : "+event.getKey());
							}
						}
			});
		return editiong;
	...
	setColumnInfo: function( me, col, fields) {
		...
		//locking grid 의 경우 layout:fit으로 주면 5.1에서 summary 가 제대로 안보임.
	...
	createRow:function(grid, values, startEditColumnName, rowIndex, newRecord) {
		...
		if(startEditColumnName)	{
			if(grid.editingPlugin)	{
				grid.editingPlugin.startEdit(newRecord,grid.getColumn(startEditColumnName));  
			}
		}else {
			var view = grid.getView() ;
			var lockedView;
			...
			if(view)	{
				Ext.getBody().mask();
				//var newPosition = view.getPosition(rowIndex,0);
				//view.setPosition(newPosition);
				//console.log(view.getCell(newRecord, newPosition.column));
				try{
					view.focusRow(newRecord);
				}catch(e){
				...
				Ext.getBody().unmask();
				//Ext.fly(view.getCell(newRecord, newPosition.column).dom).focus();
			}
			...
			if(view)	{
		        ...
		        /*var row = view.getNode(newRecord);

		        //Capturing the el (I need the div to do the trick)
		        var el = Ext.fly(Ext.fly(row).query('.x-grid-cell')[0]).dom;
		        el.focus(true);*/
			/*}
			*/
		/*			
  			if(rowIndex)	{	
				...
				if(view.getRow(rowIndex) && Ext.isFunction(view.getRow(rowIndex).scrollIntoView))	{
					console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  scrollIntoView / ", rowIndex);
					view.getRow(rowIndex).scrollIntoView(view.getEl(), null, true);
				}else {
					console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  scroll('bottom',1, true) / ", rowIndex);
					view.getEl().scroll('bottom',1, true);
				}
				//Ext.defer(grid.setScrollTop, 30, grid, [grid.getView().scrollState.top])
			}else {
				if(view.getRow(0) && Ext.isFunction(grid.getView().getRow(0).scrollIntoView))	{
					console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  scrollIntoView / no Selection / ", rowIndex);
					view.getRow(0).scrollIntoView(grid.getView().getEl(), null, true);
				}else {
	...
	_onCellKeyDown: function(view, cell, colIdx, record, row, rowIdx, e) {
    	...
    	if(e.getKey() == 67 && e.ctrlKey )	{
    		if(window.clipboardData)	{
				...
				hiddentextarea.dom.value = cell.firstChild.innerHTML; 
        ...
        if (editor && editable && (!expanderSelector || !e.getTarget(expanderSelector)) ) {
        	...
        	console.log("e.getKey() : ",e.getKey());
        	if(plugin)	{
	            if(me._isValuableEvenv(e))	{
	            	plugin.startEdit(record, columnHeader);
	            	plugin.getActiveColumn( ).field.setValue('');
	            }
				if(e.getKey() == 32 ){
	            	plugin.startEdit(record, columnHeader);
		            ...
		            if (pos) {
				        ...
				        if (selModel.isCellModel ) {
				            ...
				            if (editingPlugin && editingPlugin.editing) {
				            	...
				            	if(e.position.column.isTreeColumn)	{
				            		//selModel.select(selModel.nextSelection);
				            	}else {
...
//        }
		...
		// 5.0 부터는 bufferedrenderer 가 기본임.
...
//		);
	...
	initComponent : function() {
		...
		*/
        ...
        //this.on('beforehide', this._onBeforehide);
...
*/            
		...
		me.on('beforeselect', function(selModel, record, index, eOpts ) {
    		...
    		//5.1에서 locking grid의 경우 selection 이 두번 발생하는 버그가 있음.
				...
				if(me._getStoreEditable()) {
		            ...
		            if (pos) {		            			            	
				        if (selModel.isCellModel) {
				        	...
				        	if(pos.rowIdx === me.getStore().getCount()-1) {
				           		UniAppManager.app.onNewDataButtonDown();
				           	}
				        }*/
		            }
...
//					var selModel = me.getSelectionModel();					
//					if(selModel.isCellModel) {						
//						var pos = new Ext.grid.CellContext(me.getView()).setPosition({row: 0, column: me._getFirstVisibleColumnIndex()});
//			            selModel.select(pos);
//		        	} else {
//		        		selModel.select(0);
//		        	}
    ...
    _onCellKeyDown: function(view, cell, colIdx, record, row, rowIdx, e) {
        ...
        if (editor && editable && (!expanderSelector || !e.getTarget(expanderSelector)) ) {
        	...
        	if(plugin)	{
	            if(me._isValuableEvenv(e))	{
	            	plugin.startEdit(record, columnHeader);
	            	if(plugin.getActiveColumn( )) plugin.getActiveColumn( ).field.setValue('');  // beforeedit return false  오류로 조건 처리
	            }
				if(e.getKey() == 32 ){
	            	plugin.startEdit(record, columnHeader);
...
//        }
		me.getSelectionModel().select({row: nRow, column: nCol});
    ...
    _createContextMenu: function(tbar) {
    	...
    	if(me._getStoreEditable() && me.uniOpt.useContextMenu) {
    	...
    	afterrender: function(viewport, eOpts) {
            ...
            me.keyNav = new Ext.util.KeyMap({
                ...
                binding: [
                {
                    ...
                    key: Ext.EventObjectImpl.BACKSPACE,
                    fn: function(keyCode, e){                       
...
//		this.addEvents('uniOnChange');
	...
	setValue:function(name, value, silent) {
		...
		if(field.xtype == 'uniRadiogroup') {
			...
			rv= field.setValue(v);
		}else if(field.type == 'uniDateRangefield')	{
			var v = Ext.isDate(value) ? UniDate.getDateStr(value):value;
			rv= field.setValue(v);
			field.fieldContainer._updateMinMax();
