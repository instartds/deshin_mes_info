        ...
        onNodeDrop : function(target, dd, e, drag){      
        	if(drag.dragZone == 'BUS')	{
	            ...
	            Ext.each(store.data.items, function(record, idx) {
			   		if( idx >= dragIdx)	{
			   			if(idx < lastIdx)	{
			   				var nexRecord = store.getAt(idx+1); 
			   				record.set('name', nexRecord.get('name'));
			   				record.set('plateNo', nexRecord.get('plateNo'));
			   				record.set('driverPhoto', nexRecord.get('driverPhoto'));
			   				busView.refreshNode(idx);
			   			}
			   			if(idx == lastIdx)	{
			   				record.set('name',  drag.record.get('name'));
			   				record.set('plateNo',  drag.record.get('plateNo'));
			   				record.set('driverPhoto',  drag.record.get('driverPhoto'));
				...
				Ext.each(store.data.items, function(record, idx) {
					if( idx == dropIdx)	{
			   		...
			   		if( idx > dropIdx)	{
		   				tempRecord2 = record;
		   				record.set('name', tempRecord.get('name'));
		   				record.set('plateNo', tempRecord.get('plateNo'));
		   				record.set('driverPhoto', tempRecord.get('driverPhoto'));
