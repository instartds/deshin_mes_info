        ...
        getDragData: function(e) {
            ...
            if (sourceEl) {
                ...
                var r = v.dragData = {
                    ...
                    html:v.getNode(sourceEl).innerHTML,
                    record:v.getRecord(sourceEl),
                    dragView:v
                };
             	/*   
             	 var html = '<div class="patient-source"><table><tbody>' +
			                    '<tr><td class="patient-label">Name</td><td class="patient-name">&nbsp;</td></tr>' +
			                    '<tr><td class="patient-label">Address</td><td class="patient-name">&nbsp;</td></tr>' +
			                    '<tr><td class="patient-label">Telephone</td><td class="patient-name">&nbsp;</td></tr>' +
			                '</tbody></table></div>'
                Ext.get(sourceEl).update(html);
                */
                return r;
            }
        },
...
//      any data we decided to attach.
        onNodeDrop : function(target, dd, e, drag){        	
            ...
            var targetObj = v.getRecord(target);
            var draghtml =  v.getNode(target).innerHTML
            var html = drag.html;
            drag.record.data = targetObj.data;
            targetObj.data = drag.data;
        	targetEl.update(html);
        	Ext.get(drag.sourceEl).update(draghtml); 
...
//      any data we decided to attach.
        onNodeDrop : function(target, dd, e, drag){
            ...
            if (html == 'Drop Patient Here') {
                ...
                html = drag.data.name + ', ' + targetEl.dom.innerHTML;
            }

            targetEl.update(html);
            Ext.Msg.alert('Drop gesture', 'Dropped patient ' + drag.data.name +
