...
@Service("s_hum970rkr_kdService")
public class S_Hum970rkr_kdServiceImpl extends TlabAbstractServiceImpl {
	...
	public String fnHum970ini(Map param) throws Exception {
		...
		List<Map>qureyValue = (List<Map>) super.commonDao.list("s_hum970rkr_kdServiceImpl.fnHum970ini", param);
		...
		try {
			for(Map param : paramList )	{	
				if(super.commonDao.select("hum975rkrServiceImpl.selectProfNum", param) == null){
					super.commonDao.update("s_hum970rkr_kdServiceImpl.insertDetail", param);
				}
			}
		}catch(Exception e){
