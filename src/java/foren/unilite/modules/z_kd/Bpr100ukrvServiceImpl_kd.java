...
@Service("bpr100ukrvService_kd")
public class Bpr100ukrvServiceImpl_kd extends TlabAbstractServiceImpl {
	...
	public List<Map> selectDetailList(Map param, LoginVO user) throws Exception {
		return super.commonDao.list("bpr100ukrvService_kd.selectDetailList", param);
	}
	...
	public Integer insertDetail(List<Map> paramList, LoginVO user)
		...
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list(
					"bpr100ukrvService_kd.checkCompCode", compCodeMap);
			for (Map param : paramList) {
				for (Map checkCompCode : chkList) {
					param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					// Map chkItemMap = (Map)
					// super.commonDao.select("bpr100ukrvService_kd.checkItemCode",
					// param);
					// if(ObjUtils.parseInt(chkItemMap.get("CNT")) > 0) {
					// super.commonDao.insert("bpr100ukrvService_kd.updateDetail",
					// param);
					// }else {
					super.commonDao.update("bpr100ukrvService_kd.insertDetail",
							param);
				}
				...
				if (!ObjUtils.isEmpty(param.get("_EXCEL_JOBID"))) {
					super.commonDao.delete("bpr100ukrvService_kd.deleteSOF112T",
							param);
		...
		for (Map param : paramList) {
			for (Map checkCompCode : chkList) {
				...
				Map chkItemMap = (Map) super.commonDao.select(
						"bpr100ukrvService_kd.checkItemCode", param);
				if (ObjUtils.parseInt(chkItemMap.get("CNT")) > 0) {
					super.commonDao.insert("bpr100ukrvService_kd.updateDetail",
							param);
				} else {
		...
		for (Map param : paramList) {
			for (Map checkCompCode : chkList) {
				...
				List chkChildList = (List) super.commonDao.list(
						"bpr100ukrvService_kd.checkChildCode", param);
				if (chkChildList.size() > 0) {
					...
					try {
						super.commonDao.delete(
								"bpr100ukrvService_kd.deleteDetail", param);
						if (!ObjUtils.isEmpty(param.get("IMAGE_FID"))) {
