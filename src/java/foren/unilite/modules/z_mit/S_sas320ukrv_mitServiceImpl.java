...
@Service("s_sas320ukrv_mitService")
public class S_sas320ukrv_mitServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<String, Object>> selectList(Map param, LoginVO loginVO) throws Exception {		
		return (List) super.commonDao.list("s_sas320ukrv_mitServiceImpl.selectList", param);
	}
	...
	public List<Map> updateList(List<Map> paramList,  Map paramMaster, LoginVO user) throws Exception {		
		 for(Map param :paramList )	{
			 String inspecPrsn = ObjUtils.getSafeString(paramMaster.get("INSPEC_PRSN"));
			 if(ObjUtils.isEmpty(inspecPrsn))	{
				 inspecPrsn = user.getUserID();
			 }
			 param.put("INSPEC_PRSN", inspecPrsn);
			 super.commonDao.update("s_sas320ukrv_mitServiceImpl.updateList", param);
			 
			 String inspecFlag = ObjUtils.getSafeString(param.get("INSPEC_FLAG"));
			 if( inspecFlag != null && "Y".equals(inspecFlag))	{
				 param.put("AS_STATUS", "40"); 
			 } else if("R".equals(inspecFlag)) {
				 param.put("AS_STATUS", "30"); 
			 } else if("N".equals(inspecFlag)) {
				 param.put("AS_STATUS", "30"); 
			 }
			 super.commonDao.update("s_sas320ukrv_mitServiceImpl.updateStatus", param);
		 }		 
		 return paramList;
	}
