...
@Service("s_hum306ukrService_KOCIS")
public class S_Hum306ukrServiceImpl_KOCIS extends TlabAbstractServiceImpl  {
	...
	public List<Map> selectList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("s_hum306ukrServiceImpl_KOCIS.select", param);
	}
	...
	public Integer  insertDetail(List<Map> paramList, LoginVO user) throws Exception {
		...
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list("s_hum306ukrServiceImpl_KOCIS.checkCompCode", compCodeMap);
            for(Map param : paramList )  {
                //인사기본자료등록의 퇴사일로 퇴사여부 확인 후 퇴사자인 경우, 등록되지 않게 메시지 호출
                retireYN = (String) super.commonDao.select("s_hum306ukrServiceImpl_KOCIS.retireYN", param);
            ...
            for(Map param : paramList ) {                
                ...
                for(Map checkCompCode : chkList) {
                     ...
                     super.commonDao.update("s_hum306ukrServiceImpl_KOCIS.insertDetail", param);
                }
            }   
        }catch(Exception e){
            if(msgAlertYN == "Y"){
	...
	public Integer updateDetail(List<Map> paramList, LoginVO user) throws Exception {
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("s_hum306ukrServiceImpl_KOCIS.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 super.commonDao.update("s_hum306ukrServiceImpl_KOCIS.updateDetail", param);
			 }
		 }
		 return 0;
	} 
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user) throws Exception {
		Map compCodeMap = new HashMap();
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("s_hum306ukrServiceImpl_KOCIS.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 super.commonDao.update("s_hum306ukrServiceImpl_KOCIS.deleteDetail", param);
			 }
		 }
		 return 0;
	}
