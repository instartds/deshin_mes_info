...
@Service("s_mms510ukrv_jwService")
@SuppressWarnings({ "unchecked", "rawtypes" })
public class S_mms510ukrv_jwServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<String, Object>> selectList(Map param) throws Exception {
		...
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectList", param);
	}
	/**
	...
	public List<Map<String, Object>> selectList2(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectList2", param);
	}
	
	@ExtDirectMethod(group = "matrl", value = ExtDirectMethodType.STORE_READ)
	public List<Map<String, Object>> selectClipList(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectClipList", param);
	}
	/**
	 * 입고등록(SP+통합)-> 입고번호 검색
	 */
	...
	public List<Map<String, Object>> selectinoutNoMasterList(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectinoutNoMasterList", param);
	}
	...
	public List<Map<String, Object>> selectnoReceiveList(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectnoReceiveList", param);
	}
	...
	public List<Map<String, Object>> selectreturnPossibleList(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectreturnPossibleList", param);
	}
	...
	public List<Map<String, Object>> selectScmRefList(Map param) throws Exception {
		return super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectScmRefList", param);
	}
	...
	public List<Map<String, Object>> selectinspectResultList(Map param) throws Exception {
		...
		if(ObjUtils.getSafeString(param.get("WINDOW_FLAG")).equals("inspectResult")){	// 검사결과
			resultList = super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectinspectResultList1", param);
		}else{ // 무검사
			resultList = super.commonDao.list("s_mms510ukrv_jwServiceImpl.selectinspectResultList2", param);
		}
		return resultList;
	}
	...
	public Object  fnOrderPrice(Map param) throws Exception {	
		return  super.commonDao.select("s_mms510ukrv_jwServiceImpl.fnOrderPrice", param);
	}
	...
	public Object userWhcode(Map param) throws Exception {	
		return  super.commonDao.select("s_mms510ukrv_jwServiceImpl.userWhcode", param);
	}
	...
	public Object fnGetCalcTaxAmt(Map param) throws Exception {	
		return  super.commonDao.select("s_mms510ukrv_jwServiceImpl.fnGetCalcTaxAmt", param);
	}
	...
	public Object  taxType(Map param) throws Exception {	
		return  super.commonDao.select("s_mms510ukrv_jwServiceImpl.taxType", param);
	}
	...
	public List<Map> saveAll(List<Map> paramList,  Map paramMaster, LoginVO user) throws Exception {
		...
		for(Map paramData: paramList) {			
			...
			for(Map param:  dataList) {
				...
				if(!param.get("ORDER_UNIT").equals(param.get("STOCK_UNIT"))) { // 구매단위 != 재고단위
					roofcount = ObjUtils.parseDouble(param.get("ORDER_UNIT_Q")); // 발주량
					param.put("ORDER_UNIT_Q", 1); // 발주량 
					param.put("ORDER_UNIT_I", ObjUtils.parseDouble(param.get("ORDER_UNIT_I"))	/ roofcount); //자사금액
					param.put("INOUT_Q"		, ObjUtils.parseDouble(param.get("INOUT_Q"))		/ roofcount); //재고단위수량
					param.put("INOUT_I"		, ObjUtils.parseDouble(param.get("INOUT_I"))		/ roofcount); //공급가액
					param.put("INOUT_FOR_O"	, ObjUtils.parseDouble(param.get("INOUT_FOR_O"))	/ roofcount); //재고단위금액
					...
					//lot_no 존재 확인
					if(ObjUtils.isEmpty(param.get("LOT_NO")) || "*".equals(param.get("LOT_NO"))){
						autoLotNo = "Y";
					}
...
//							throw new  UniDirectValidateException("입고유형이 금액보정이 아닐때  구매단가가 0 이거나 0보다 작을 수 없습니다");
							...
							for(int i=0; i<roofcount; i++) {
								if(autoLotNo == "Y") {
									String newLotNo = (String) super.commonDao.select("s_mms510ukrv_jwServiceImpl.fnGetLotNo", param);
									param.put("LOT_NO", newLotNo);
								}
								param.put("INOUT_SEQ", inoutSeq);
								
								param.put("data", super.commonDao.insert("s_mms510ukrv_jwServiceImpl.insertLogDetail", param));
								inoutSeq++;
							}
						}
					} else {
						if("N".equals(oprFlag) || "D".equals(oprFlag)){
							...
							for(int i=0; i<roofcount; i++) {
								...
								param.put("data", super.commonDao.insert("s_mms510ukrv_jwServiceImpl.updateLogDetail", param));
								inoutSeq++;
							}
						}
					}
					...
					Double oderUnitQ	= ObjUtils.parseDouble(param.get("ORDER_UNIT_Q")); // 발주량
					Double packQty		= ObjUtils.parseDouble(param.get("PACK_QTY")); // 포장수량
					Double orderUnitI	= ObjUtils.parseDouble(param.get("ORDER_UNIT_I")); // 자사금액
					Double inoutQ		= ObjUtils.parseDouble(param.get("INOUT_Q")); // 재고단위수량
					Double inoutI		= ObjUtils.parseDouble(param.get("INOUT_I")); // 공급가액
					Double inoutForO	= ObjUtils.parseDouble(param.get("INOUT_FOR_O")); // 재고단위금액
					roofcount			= ObjUtils.parseDouble(param.get("ORDER_UNIT_Q")) / ObjUtils.parseDouble(param.get("PACK_QTY")); // 발주량 / 포장수량
					
					for(int j = 0; j < roofcount; j++) {
						param.put("ORDER_UNIT_Q", packQty); // 발주량 = 포장수량
						param.put("ORDER_UNIT_I", orderUnitI	* packQty / oderUnitQ); // 자사금액 = 자사금액 * 포장수량 / 발주량
						param.put("INOUT_Q"		, inoutQ		* packQty / oderUnitQ); // 재고단위수량 = 재고단위수량 * 포장수량 / 발주량
						param.put("INOUT_I"		, inoutI		* packQty / oderUnitQ); // 공급가액 = 공급가액 * 포장수량 / 발주량
						param.put("INOUT_FOR_O"	, inoutForO		* packQty / oderUnitQ); // 재고단위금액 = 재고단위금액 * 포장수량 / 발주량
						
						if(j == Math.ceil(roofcount) - 1) {
							param.put("ORDER_UNIT_Q", oderUnitQ - (j * packQty)); // 발주량 = 발주량 - (발주량 / 포장수량 * 포장수량)
							param.put("ORDER_UNIT_I", orderUnitI	- (j * orderUnitI	* packQty / oderUnitQ));// 자사금액 = 자사금액 - (발주량 / 포장수량 * 자사금액 * 포장수량 / 발주량)
							param.put("INOUT_Q"		, inoutQ		- (j * inoutQ		* packQty / oderUnitQ));// 재고단위수량  = 재고단위수량 - (발주량 /포장수량 * 재고단위수량 * 포장수량 / 발주량) 
							param.put("INOUT_I"		, inoutI		- (j * inoutI		* packQty / oderUnitQ));// 공급가액 = 공급가액 - (발주량 / 포장수량 * 공급가액 * 포장수량 /발주량)
							param.put("INOUT_FOR_O"	, inoutForO		- (j * inoutForO	* packQty / oderUnitQ));// 재고단위금액 = 재고단위금액 - (발주량 / 포장수량 * 재고단위금액 * 포장수량 / 발주량)
						}
...
//								throw new  UniDirectValidateException("입고유형이 금액보정이 아닐때  구매단가가 0 이거나 0보다 작을 수 없습니다");
	    ...
	    super.commonDao.queryForObject("s_mms510ukrv_jwServiceImpl.spReceiving", spParam);
...
//			dataMaster.put("INOUT_NUM", "");
	...
	public List<Map> insertDetail(List<Map> params, LoginVO user) throws Exception {
		for(Map param: params)		{
			Map<String, Object> checkSerNo =  (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkSerNo", param);
			if(!ObjUtils.isEmpty(checkSerNo))	{
				Map<String, Object> autoSerNo =  (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.autoSerNo", param);
				param.put("SER_NO", ObjUtils.getSafeString(autoSerNo.get("SER_NO")));
			}
			beforeSaveDetail(param, user, "insert");
			super.commonDao.insert("s_mms510ukrv_jwServiceImpl.insertDetail", param);
		}		
	...
	public List<Map> updateDetail(List<Map> params, LoginVO user) throws Exception {
		for(Map param: params)		{
			...
			super.commonDao.insert("s_mms510ukrv_jwServiceImpl.updateDetail", param);
		}		
		return params;
	}
	...
	public void deleteDetail(List<Map> params, LoginVO user) throws Exception {
		for(Map<String, Object> param : params)	{
			...
			super.commonDao.delete("s_mms510ukrv_jwServiceImpl.deleteDetail", param);
		}
		super.commonDao.delete("s_mms510ukrv_jwServiceImpl.checkDeleteAllDetail", params.get(0)); 
	}
	...
	private void beforeSaveDetail(Map param, LoginVO user, String saveMode) throws Exception	{
		if("Y".equals(ObjUtils.getSafeString(param.get("gsDraftFlag"))))	{
			Map<String, Object> itemInfo = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.itemInfo", param);
	...
	private void checkCompare(Map<String, Object> param, LoginVO user) throws Exception {
		Map<String, Object> checkDetailData = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkDetailData", param);
		int sCase=0;
		...
		if(!ObjUtils.isEmpty(checkDetailData))	{
	        ...
	        if(sCase > 0)	{
	        	if(!(sCase == 1 || sCase == 3 || ((sCase == 5 || sCase ==6) && (reqIssueQty > 0 ))))	{
	        		Map<String, Object> checkSaleQ = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkSaleQ", param);
	        ...
	        if(outStockQ > 0)	{
		        List<Map<String, Object>> checkListSSA110 = (List<Map<String, Object>>)super.commonDao.list("s_mms510ukrv_jwServiceImpl.checkSSA110", param);
		        if(ObjUtils.isEmpty(checkListSSA110))	{
		        	...
		        	for(Map<String, Object> checkSSA110 : checkListSSA110)	{
		        		...
		        		if(sFlg != 0)	{
		        			...
		        			super.commonDao.update("s_mms510ukrv_jwServiceImpl.updateBTR100", paramSRQ100);
		        		}
		        	}
	        ...
	        if(issueReqQ > 0)	{
	        	List<Map<String, Object>> checkListSRQ100 = (List<Map<String, Object>>)super.commonDao.list("s_mms510ukrv_jwServiceImpl.checkSRQ100", param);
		        ...
		        if(ObjUtils.isEmpty(checkListSRQ100))	{
		        	...
		        	for(Map<String, Object> checkSRQ100 : checkListSRQ100)	{
		        		...
		        		if(sFlg != 0)	{
		        			...
		        			super.commonDao.update("s_mms510ukrv_jwServiceImpl.updateSRQ100", paramSRQ100);
		        		}
		        	}
		        }
	        }
	...
	private void checkEstimate(Map param, LoginVO user) throws Exception {
		List<Map<String, Object>> params = super.commonDao.list("s_mms510ukrv_jwServiceImpl.checkEstimate", param);
		
		if(!ObjUtils.isEmpty(params))	{
			Map<String, Object> cfrmData = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkEstimateConfirm", params.get(0));
			...
			if(!ObjUtils.isEmpty(cfrmData))	{
				...
				for(Map<String, Object> estiParam : params)	{					
					super.commonDao.update("s_mms510ukrv_jwServiceImpl.updateEstimateConfirm", estiParam);
				}
			}else {
	...
	private void checkEstimateDetail(Map param, LoginVO user, String saveMode) throws Exception {
		Map<String, Object> cfrmData = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkEstiDetail", param);
		if(!ObjUtils.isEmpty(cfrmData))	{
			if(!"2".equals(cfrmData.get("CONFIRM_FLAG")))	{
				throw new  UniDirectValidateException(this.getMessage("54458", user));	/* 54458:이미 취소되었거나 확정되지 않았습니다. */
			}
			...
			super.commonDao.update("s_mms510ukrv_jwServiceImpl.updateEstimateDetailConfirm", param);
	...
	private void checkScm(Map param, LoginVO user) throws Exception {
		List<Map<String, Object>> params = super.commonDao.list("s_mms510ukrv_jwServiceImpl.checkSCM", param);
		if(!ObjUtils.isEmpty(params))	{
			Map<String, Object> baseData = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkSCMDetail", param);
			...
			if(!ObjUtils.isEmpty(baseData))	{
				...
				for(Map<String, Object> scmParam : params)	{	
					...
					Map<String, Object> dData = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkSCMDelete", scmParam);
					if(ObjUtils.isEmpty(dData))	{
						...
						if("2".equals(controlStatus))	{
							...
							super.commonDao.update("s_mms510ukrv_jwServiceImpl.updateSCM", scmParam);
						}
	...
	private String  getScm(Map param)  throws Exception {
		Map<String, Object> scm = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.getSCM", param);
		String r = "N";
	...
	private void  checkStatus(Map param, LoginVO user)  throws Exception {
		Map<String, Object> obj = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkStatus", param);
		String r = "";
	...
	private void  checkProgress(Map param, String type, LoginVO user)  throws Exception {
		...
		if("Master".equals(type))	{
			obj = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkProgressMaster", param);
		}else {
			obj = (Map<String, Object>)super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkProgressDetail", param);
		}
...
//            ZPLString +="^XZ";
    ...
    public Map checkIpAddr(Map param) throws Exception {
        ...
        return (Map) super.commonDao.select("s_mms510ukrv_jwServiceImpl.checkIpAddr", param);
