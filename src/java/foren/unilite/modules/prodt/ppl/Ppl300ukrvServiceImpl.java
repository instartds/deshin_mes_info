...
@Service("ppl300ukrvService")
@SuppressWarnings({ "rawtypes", "unchecked" })
public class Ppl300ukrvServiceImpl  extends TlabAbstractServiceImpl {
	@InjectLogger
	...
	public String selectMaster(Map param) throws Exception {
		...
		if(param.get("DUMMY_YN").equals("Y")){
			dataList = super.commonDao.list("ppl300ukrvServiceImpl.selectDummy", param);
		}else{
			dataList = super.commonDao.list("ppl300ukrvServiceImpl.selectMaster", param);
		}
		...
		timeRangesData.add("rows" , infoArray3);
		 ...
		 jsonobject.add("timeRanges", timeRangesData);
		  ...
		  dependencytList = super.commonDao.list("ppl300ukrvServiceImpl.selectDependencyList", param);
	...
	public List<Map<String, Object>>   selectDetailList(Map param) throws Exception {
		...
		if(param.get("DUMMY_YN").equals("Y")){
			dataList = super.commonDao.list("ppl300ukrvServiceImpl.selectDummyDetailList", param);
		}else{
			dataList = super.commonDao.list("ppl300ukrvServiceImpl.selectDetailList", param);
		}
		return  dataList;
	}
	...
	public String insertApsInputData(Map param, LoginVO loginVO, String path) throws Exception {
		...
		String[] orderSeqArry = null;


		String orderNums = ObjUtils.getSafeString(param.get("ORDER_NUMS"));
		if(ObjUtils.isNotEmpty(orderNums)){
			orderNumArry = orderNums.split(",");
		}
		String orderSeqs = ObjUtils.getSafeString(param.get("ORDER_SEQS"));
		if(ObjUtils.isNotEmpty(orderSeqs)){
			orderSeqArry = orderSeqs.split(",");
		}

		List<Map>  orderNumList = new ArrayList<Map>();
		List<Map>  orderSeqList = new ArrayList<Map>();
		for(int i=0; i<ObjUtils.parseInt(param.get("dataCount")); i++){
			Map map = new HashMap();
			//Map map1 = new HashMap();
			Map map2 = new HashMap();

			Object SELECT_ORDER_NUM =  orderNumArry[i] + orderSeqArry[i];
		...
		apsInputRawDataList = super.commonDao.list("ppl300ukrvServiceImpl.selectApsInputRawDataList", param);
		...
		if(apsInputRawDataList.size() > 0){
			 for(Map apsParam:  apsInputRawDataList) {
				 ...
				 super.commonDao.insert("ppl300ukrvServiceImpl.insertEstiListPlan", apsParam);
			 }
			   Map<String, Object> spParam = new HashMap<String, Object>();
				...
				super.commonDao.queryForObject("ppl300ukrvServiceImpl.spProdtPlan", spParam);
...
//				    throw new  UniDirectValidateException(this.getMessage(errorDesc, loginVO));
					...
					String   fullPath = path + "\\codi_crp_aps.py";
					String   apsKey	  = (String) super.commonDao.select("ppl300ukrvServiceImpl.selecApsKey", spParam);
...
//			        command[0]        = "C://Users/chaeseongmin/AppData/Local/Programs/Python/Python38/python.exe";
			        ...
			        command[9]		  = null;									//split_type 제조 는 'B'  , 포장은 'C' 가 될 수 있음
			        command[10] 	  = "Y";									//validate 추후 사용 예정 ( 'Y','N','P' ) - 일단은 'Y' 나 'P' 만 사용
			        command[11] 	  = (String) param.get("REMAINING_TIME");	//다음날로 이월이 가능한 최소 근무 근무시간
			        command[12] 	  = "N";									//debug YN


			        command[13] 	  = null;									//수주번호
				    command[14] 	  = null;									//수주순번

			        apsInputList 	  = super.commonDao.list("ppl300ukrvServiceImpl.selectApsInputList", spParam);
			        ...
			        for(Map apsInputParam:  apsInputList) {
					     ...
					     command[13] 	  = ObjUtils.getSafeString(apsInputParam.get("ORDER_NUM"));
					     command[14] 	  = ObjUtils.getSafeString(apsInputParam.get("SEQ"));
			             ...
			             try{
			        		...
			        		super.commonDao.update("ppl300ukrvServiceImpl.updatePST", updatePSTparam);
			        }
	...
	public List<Map> updateDetail(List<Map> params, LoginVO user) throws Exception {
		try {
			for(Map param: params)	{
					...
					super.commonDao.update("ppl300ukrvServiceImpl.updateApsParameterData", param);
		...
		try {
			for(Map param: params)	{
				...
				if(param.get("DUMMY_YN").equals("Y")){//DUMMY데이터인 경우
					super.commonDao.update("ppl300ukrvServiceImpl.updateApsDumyUpdate", param);
				}else if(saveFlag.equals("FIX")){
					super.commonDao.update("ppl300ukrvServiceImpl.updateApsFixData", param);
				}else{
					if((boolean) param.get("CONFIRM_YN")){
						super.commonDao.update("ppl300ukrvServiceImpl.updateApsConfirmData", param);
						...
						map.put("TYPE","P");
						String sAutoNo = null;
						Map<String, Object> autoNumMap = (Map<String, Object>)super.commonDao.select("ppl300ukrvServiceImpl.selectAutoNum",map);
						...
						if(autoNumMap != null){
							...
							param.put("DIV_CODE",divCode);
							super.commonDao.insert("ppl300ukrvServiceImpl.insertPpl100tApsData", param);
							super.commonDao.update("ppl300ukrvServiceImpl.updateApsWkPlanNum", param);
						}
					}else{
						super.commonDao.update("ppl300ukrvServiceImpl.updateApsConfirmData", param);
						super.commonDao.delete("ppl300ukrvServiceImpl.deletePpl100tApsData", param);
					}
	...
	public List<Map<String, Object>> selectApsParameter(Map param) throws Exception {
		return super.commonDao.list("ppl300ukrvServiceImpl.selectApsParameterList", param);
	}
	...
	public List<Map<String, Object>> selectEstiList(Map param) throws Exception {
		return super.commonDao.list("ppl300ukrvServiceImpl.selectEstiList", param);
	}
}
