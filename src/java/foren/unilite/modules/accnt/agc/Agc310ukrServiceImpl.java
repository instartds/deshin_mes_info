...
@Service("agc310ukrService")
public class Agc310ukrServiceImpl extends TlabAbstractServiceImpl {
	...
	public Object fnDispYN(Map param) throws Exception {
		return super.commonDao.select("agc310ukrServiceImpl.fnDispYN", param);
	}
	 ...
	 */
	@ExtDirectMethod(group = "accnt")
	public Object bBizrefYN(Map param) throws Exception {
		return super.commonDao.select("agc310ukrServiceImpl.bBizrefYN", param);
	}
	...
	//		param.put("dAmt7000","0");
			...
			super.commonDao.list("agc310ukrServiceImpl.fnCreateTable", param);
			...
			return super.commonDao.list("agc310ukrServiceImpl.fnMakeSQL", param);
		}else{
		
			String sMatrl_YN = "";
	
			//회계기준설정으로부터 원가대체기준 설정내용 확인
			if(super.commonDao.list("agc310ukrServiceImpl.fnGetBaseInfo", param).size()==0){
				throw new  UniDirectValidateException(this.getMessage("54302", user));
			}else{
				List<Map> option1 = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.fnGetBaseInfo", param);
	...
	//		param.get("FR_DATE")
			...
			if(sMatrl_YN.equals("1")){
				super.commonDao.list("agc310ukrServiceImpl.fnRefMatrlTbl", param);
			}
...
//		param.get("FR_DATE")
		...
		if(param.get("SEARCH").equals("SEARCH")){
			...
			return super.commonDao.list("agc310ukrServiceImpl.produceBudgetSearch", param);
		}else{
			return super.commonDao.list("agc310ukrServiceImpl.produceBudget", param);
		}
	}
	...
	public List<Map<String, Object>> selectRef1(Map param, LoginVO user) throws Exception {
		...
		return super.commonDao.list("agc310ukrServiceImpl.selectRef1", param);
	}
	...
	public Object costInformation(Map param) throws Exception {
		...
		reValue = reValue.replaceAll("\\]", "");
		param.put("DIV_CODE", reValue);
		return super.commonDao.select("agc310ukrServiceImpl.costInformation", param);
	}
	...
	public void  cancSlip(Map param, LoginVO user) throws Exception {
		...
		param.put("ACCNT_DIV_CODE", reValue);
		// Check UserId
		if(super.commonDao.list("agc310ukrServiceImpl.cancSlipCheck1", param).size()==0){ 
			throw new  UniDirectValidateException(this.getMessage("54363", user));
		}
		
		// Check Accept, Draft
		
		List<Map> cancSlipCheck2 = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.cancSlipCheck2", param);
...
//               dicMsg("A0039") & " : " & rsCost("EX_NUM")
		...
		List<Map> cancSlipCheck3 = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.cancSlipCheck3", param);
...
//              dicMsg("A0107") & " : " & Format(sInputDate, "0000-00-00")
		...
		List<Map> cancSlipCheck4 = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.cancSlipCheck4", param);
...
//          dicMsg("A0035") & " : " & rsCost("EX_NUM")
	...
	//	        dicMsg("A0035") & " : " & rsCost("EX_NUM")
		...
		super.commonDao.update("agc310ukrServiceImpl.UpdateAGJ110T", param);
		super.commonDao.update("agc310ukrServiceImpl.UpdateAGC310T", param);
	...
	public void  insertDetail1(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {		
		...
		if(super.commonDao.list("agc310ukrServiceImpl.beforeSaveCheck", dataMaster).size() > 0){
			...
			List<Map> checkOption = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.beforeSaveCheck", dataMaster);
		...
		//원가대체 마스터 삭제
		super.commonDao.delete("agc310ukrServiceImpl.deleteMaster", dataMaster);
		
		//원가대체 디테일 삭제
		super.commonDao.delete("agc310ukrServiceImpl.deleteDetail", dataMaster);
		...
		for(Map param :paramList )	{	
			if(param.get("ACCNT_CD").equals("2100")){
				dataList.put("COST_PD", param.get("AMT_I"));
			}else if(param.get("ACCNT_CD").equals("2200")){
				dataList.put("COST_GD", param.get("AMT_I"));
			}
		}
		super.commonDao.update("agc310ukrServiceImpl.insertDetail1", dataList);
	...
	public void  insertDetail2(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {		
		...
		super.commonDao.update("agc310ukrServiceImpl.insertDetail2", dataList);
	...
	public void updateDetail3(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {
		...
		for(Map param :paramList )	{	
			...
			super.commonDao.update("agc310ukrServiceImpl.updateDetail3", param);
		}
		 return;
	} 
	...
	@ExtDirectMethod(value = ExtDirectMethodType.STORE_MODIFY, group = "accnt")
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor={Exception.class})
	public void agc310ukrDelA (Map paramMaster, LoginVO user) throws Exception {
		...
		//전표생성여부, 승인여부 및 결재상신여부 확인
		if(super.commonDao.list("agc310ukrServiceImpl.beforeSaveCheck", paramMaster).size() > 0){
			...
			List<Map> checkOption = (List<Map>) super.commonDao.list("agc310ukrServiceImpl.beforeSaveCheck", paramMaster);
			covExDate = (String) checkOption.get(0).get("EX_DATE");
			covAgreeYn = (String) checkOption.get(0).get("AGREE_YN");
			covDraftYn = (String) checkOption.get(0).get("DRAFT_YN");
		...
		//원가대체 마스터 삭제
		super.commonDao.delete("agc310ukrServiceImpl.deleteMaster", paramMaster);
		
		//원가대체 디테일 삭제
		super.commonDao.delete("agc310ukrServiceImpl.deleteDetail", paramMaster);
	}
