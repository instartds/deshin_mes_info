...
@Service("s_hrt700ukr_inService")
@SuppressWarnings({ "rawtypes", "unchecked" })
public class S_hrt700ukr_inServiceImpl  extends TlabAbstractServiceImpl {
	...
	public List selectColumns(Map<String, Object> param) throws Exception {
		return (List)super.commonDao.list("s_hrt700ukr_inServiceImpl.selectColumns", param);
	}
	...
	public List<Map> selectList(Map param, LoginVO user) throws Exception {
		List<Map> paymentList = super.commonDao.list("s_hrt700ukr_inServiceImpl.checkPayment", param);
		...
		if( paymentList == null || paymentList.size() == 0 || paymentList.get(0).get("CNT_PAYMENT").toString().equals("0") ){
			...
			for(Object divCode : divList) {
				...
				// 퇴직연금 생성
				newList.addAll(super.commonDao.list("s_hrt700ukr_inServiceImpl.selectNewList", chkParam));
			}
			return newList;
		}
	}
	...
	public Integer deleteList(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {
		for(Map param : paramList ) {
			super.commonDao.update("s_hrt700ukr_inServiceImpl.deleteList", param);
		}
		return 0;
	} 
	...
	public Integer updateList(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {
		...
		for(Map param : paramList ) {
			for(Object wagesCode : wagesList) {
				...
				super.commonDao.update("s_hrt700ukr_inServiceImpl.updateList", param);
			}
		}
		return 0;
	} 
	...
	public Map<String, Object> fnGetCloseAll( Map param, LoginVO user ) throws Exception {
		return (Map)super.commonDao.select("s_hrt700ukr_inServiceImpl.fnGetCloseAll", param);
    }
	...
	public Map<String, Object> fnSetCloseAll( Map param, LoginVO user ) throws Exception {
		super.commonDao.update("s_hrt700ukr_inServiceImpl.fnSetCloseAll", param);
		return param;
    }
	...
	public void excelValidate(String jobID, Map param) throws Exception {
		...
		List<Map> getData = (List<Map>) super.commonDao.list("s_hrt700ukr_inServiceImpl.getData", param);
		...
		if(!getData.isEmpty()){
			...
			for(Map data : getData )  { 
				...
				String custExistYn =  (String) super.commonDao.select("s_hrt700ukr_inServiceImpl.checkPersonNum", param);
				if (custExistYn.equals("N")) {
					param.put("MSG", "사번 [" + data.get("PERSON_NUMB") +"]을 확인하십시오.");
					super.commonDao.update("s_hrt700ukr_inServiceImpl.insertErrorMsg", param);
				}
				
				//업로드 된 데이터의 정산일 등록여부 확인
				String retrExistYn =  (String) super.commonDao.select("s_hrt700ukr_inServiceImpl.checkRetrDate", param);
