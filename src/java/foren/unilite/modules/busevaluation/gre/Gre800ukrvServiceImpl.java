...
@Service("gre800ukrvService")
public class Gre800ukrvServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<Object, String>>  selectList(Map param) throws Exception {	
		return  super.commonDao.list("gre800ukrvServiceImpl.selectList", param);
	}
	...
	public Integer  insertDetail(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {		
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list("gre800ukrvServiceImpl.checkCompCode", compCodeMap);
			for(Map param : paramList )	{			 
				 for(Map checkCompCode : chkList) {
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 super.commonDao.update("gre800ukrvServiceImpl.insertDetail", param);
					 }
				}	
		}catch(Exception e){
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("gre800ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 super.commonDao.insert("gre800ukrvServiceImpl.updateDetail", param);
			 }
		 }
		 return 0;
	} 
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user, Map paramMaster) throws Exception {
		...
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("gre800ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 try {
						 super.commonDao.delete("gre800ukrvServiceImpl.deleteDetail", param);
