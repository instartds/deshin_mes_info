...
@Service("grc200ukrvService")
public class Grc200ukrvServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<Object, String>>  selectList(Map param) throws Exception {	
		return  super.commonDao.list("grc200ukrvServiceImpl.selectList", param);
	}
...
//		}		
	...
	public Integer  insertDetail(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {		
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list("grc200ukrvServiceImpl.checkCompCode", compCodeMap);
			for(Map param : paramList )	{			 
				 for(Map checkCompCode : chkList) {
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 super.commonDao.update("grc200ukrvServiceImpl.insertDetail", param);
					 }
				}	
		}catch(Exception e){
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("grc200ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 super.commonDao.insert("grc200ukrvServiceImpl.updateDetail", param);
			 }
		 }
		 return 0;
	} 
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user, Map paramMaster) throws Exception {
		...
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("grc200ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("BASE_YEAR", dataMaster.get("BASE_YEAR"));
					 try {
						 super.commonDao.delete("grc200ukrvServiceImpl.deleteDetail", param);
