package foren.unilite.modules.busevaluation.grp;
...
@Service("grp100ukrvService")
public class Grp100ukrvServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<Object, String>>  selectList(Map param) throws Exception {	
		return  super.commonDao.list("grp100ukrvServiceImpl.selectList", param);
	}
...
//		}		
	...
	public Integer  insertDetail(List<Map> paramList, LoginVO user, Map paramMaster) throws Exception {		
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list("grp100ukrvServiceImpl.checkCompCode", compCodeMap);
			for(Map param : paramList )	{			 
				 for(Map checkCompCode : chkList) {
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 param.put("SERVICE_YEAR", dataMaster.get("SERVICE_YEAR"));
					 super.commonDao.update("grp100ukrvServiceImpl.insertDetail", param);
					 }
				}	
		}catch(Exception e){
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("grp100ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("SERVICE_YEAR", dataMaster.get("SERVICE_YEAR"));
					 super.commonDao.insert("grp100ukrvServiceImpl.updateDetail", param);
			 }
		 }
		 return 0;
	} 
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user, Map paramMaster) throws Exception {
		...
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("grp100ukrvServiceImpl.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 param.put("SERVICE_YEAR", dataMaster.get("SERVICE_YEAR"));
					 try {
						 super.commonDao.delete("grp100ukrvServiceImpl.deleteDetail", param);
