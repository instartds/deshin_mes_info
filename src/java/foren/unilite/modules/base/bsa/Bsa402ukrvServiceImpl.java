...
@Service( "bsa402ukrvService" )
public class Bsa402ukrvServiceImpl extends TlabAbstractServiceImpl {
    ...
    public UniTreeNode selectList( Map param, LoginVO user ) throws Exception {
        List<GenericTreeDataMap> treeList = super.commonDao.list("bsa402ukrvService.selectList", param);
    ...
    public List<Map> insertPrograms( List<Map> paramList, LoginVO user ) throws Exception {
        for (Map param : paramList) {
            Map chkMap = (Map)super.commonDao.select("bsa402ukrvService.checkPK", param);
            if (ObjUtils.parseInt(chkMap.get("CNT")) > 0) {
                throw new UniDirectValidateException(this.getMessage("2627", user));
            }
            
            super.commonDao.insert("bsa402ukrvService.insertPrograms", param);
    ...
    public List<Map> deletePrograms( List<Map> paramList ) throws Exception {
        for (Map param : paramList) {
            super.commonDao.update("bsa402ukrvService.deletePrograms", param);
        }
    ...
    public List<Map> updatePrograms( List<Map> paramList ) throws Exception {
        for (Map param : paramList) {
            super.commonDao.update("bsa402ukrvService.updatePrograms", param);
        }
        tlabMenuService.reload();
        return paramList;
    }
