...
@Service( "bsa300ukrvService" )
public class Bsa300ukrvServiceImpl extends TlabAbstractServiceImpl {
	...
	public List<Map<String, Object>> selectList( Map param ) throws Exception {
		...
		List<Map<String, Object>> rv = super.commonDao.list("bsa300ukrvServiceImpl.getDataList", param);
		return rv;
	}
	...
	public List<Map> insertMulti( List<Map> paramList, LoginVO user ) throws Exception {
		...
		for (Map param : paramList) {
			//사용자 ID 중복체크
			Map<String, String> chkUniqueId = (Map<String, String>)super.commonDao.select("bsa300ukrvServiceImpl.checkUniqueID", param);
			if (chkUniqueId != null) {
			...
			if (!csINIT_SYSTEM_PWD.equals(pw)) {
				...
				param.put("NEW_PASSWORD", enc.encrypt(pw));
			}
			super.commonDao.insert("bsa300ukrvServiceImpl.insertMulti", param);

			// 누가 막았는지 모르지만 다시 풀었음.
			// BSA301T 에 저장하는게 표준임.
			// 다른 내용이 있으면 관리자에게 문의할 것.
			if (bSecurityFlag) {
				super.commonDao.insert("bsa300ukrvServiceImpl.insertBSA301T", param);
			}

			//사용자 수를 초과했거나 등록된 유저가 아닌 경우 체크
			List<Map<String, Object>> mapUserList = super.commonDao.list("bsa300ukrvServiceImpl.selectUserList", param);
	...
	public List<Map> updateMulti( List<Map> paramList, LoginVO user ) throws Exception {
		...
		for (Map param : paramList) {
			...
			logger.debug("   ######################   enc pw " + enc.encrypt(pw));
			super.commonDao.update("bsa300ukrvServiceImpl.updateMulti", param);

			if (!csINIT_SYSTEM_PWD.equals(sUserPwd)) {
				super.commonDao.update("bsa300ukrvServiceImpl.updatePassword", param);
				// 누가 막았는지 모르지만 다시 풀었음.
				// BSA301T 에 저장하는게 표준임.
				// 다른 내용이 있으면 관리자에게 문의할 것.
				if (bSecurityFlag) {
					super.commonDao.update("bsa300ukrvServiceImpl.updateBSA301T", param);
				}
				super.commonDao.update("bsa300ukrvServiceImpl.insertPasswordLog", param);
			}
		}
		return paramList;
	}
	...
	public List<Map> deleteMulti( List<Map> paramList, LoginVO user ) throws Exception {
		...
		for (Map param : paramList) {
			//20210805 추가: agj210t.INPUT_USER_ID에 있는 계정은 삭제되지 않도록 수정
			String isExistsAg210t = "";
			isExistsAg210t = (String) super.commonDao.select("bsa300ukrvServiceImpl.isExistsAg210t", param);
			if("Y".equals(isExistsAg210t)) {
				throw new  UniDirectValidateException(this.getMessage("54126", user));
			}
			super.commonDao.update("bsa300ukrvServiceImpl.deleteMulti", param);
			// 누가 막았는지 모르지만 다시 풀었음. BSA301T 에 저장하는게 표준임. 다른 내용이 있으면 관리자에게 문의할 것.
			if (bSecurityFlag) {
				super.commonDao.update("bsa300ukrvServiceImpl.deleteSecurityFlagMulti", param);
			}
		}
		return paramList;
	}
	...
	private Map<String, Object> checkEncryptYN() {
		return (Map<String, Object>)super.commonDao.select("bsa300ukrvServiceImpl.checkEncryptYN");
	}
