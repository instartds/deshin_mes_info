...
@Service("bpr560ukrvService")
@SuppressWarnings({ "rawtypes", "unchecked" })
public class Bpr560ukrvServiceImpl  extends TlabAbstractServiceImpl {
	...
	public Object selectMaster(Map param) throws Exception {
		return super.commonDao.select("bpr560ukrvService.selectMaster", param);
	}

	@ExtDirectMethod(group = "sales", value = ExtDirectMethodType.STORE_READ)
	public List<Map<String, Object>> selectList(Map param) throws Exception {
		return super.commonDao.list("bpr560ukrvService.selectList", param);
	}

	@ExtDirectMethod(group = "sales", value = ExtDirectMethodType.STORE_READ)
	public List<Map<String, Object>> selectList2(Map param) throws Exception {
		return super.commonDao.list("bpr560ukrvService.selectList2", param);
	}

	@ExtDirectMethod(group = "sales", value = ExtDirectMethodType.STORE_READ)
	public List<Map<String, Object>> selectList3(Map param) throws Exception {
		return super.commonDao.list("bpr560ukrvService.selectList3", param);
	}
	...
	@ExtDirectMethod(value = ExtDirectMethodType.STORE_SYNCALL, group = "base")
	 ...
	 if(paramList != null)	{
			...
			Map checkAllDiv = (Map) super.commonDao.select("bpr560ukrvService.checkAllDiv", paramMap);
			...
			allDivList =  super.commonDao.list("bpr560ukrvService.selectDivList", paramMap);
	...
	public void insertDetail(List<Map> paramList, LoginVO user, String allDivYn, List<Map> allDivList) throws Exception {
		...
		try {
			...
			for(Map param : paramList)	{
				duplicationChk = (int) super.commonDao.queryForObject("bpr560ukrvService.prodItemCodeDuplicationChk", param);
				if(duplicationChk > 0){
...
//						insert 
				if(allDivYn.equals("Y")){
					 super.commonDao.delete("bpr560ukrvService.deleteAllDiv", param);
					
					if(allDivList != null){
						for(Map addParams : allDivList){
							param.put("DIV_CODE", addParams.get("DIV_CODE"));
							super.commonDao.update("bpr560ukrvService.insertDetail", param);
						}
					}
	...
	public void insertDetail2(List<Map> paramList, LoginVO user, String allDivYn, List<Map> allDivList) throws Exception {
		try {
			...
			//List<Map> chkList = (List<Map>) super.commonDao.list("bpr560ukrvService.checkCompCode", compCodeMap);
			for(Map param : paramList )	{
...
//								insert 
					 if(allDivYn.equals("Y")){
						...
						if(allDivList != null){
							for(Map addParams : allDivList){
								param.put("DIV_CODE", addParams.get("DIV_CODE"));
								 super.commonDao.update("bpr560ukrvService.insertDetail2", param);
							}
						}
...
//								insert 
					 if(allDivYn.equals("Y")){
						 super.commonDao.delete("bpr560ukrvService.deleteAllDiv2", param);
						
						if(allDivList != null){
							for(Map addParams : allDivList){
								param.put("DIV_CODE", addParams.get("DIV_CODE"));
								super.commonDao.update("bpr560ukrvService.insertDetail3", param);
							}
						}
...
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 update
				 if(allDivYn.equals("Y")){
					 super.commonDao.delete("bpr560ukrvService.updateAllDiv0", param);
					
				}else{
					 super.commonDao.update("bpr560ukrvService.updateDetail", param);
				}
...
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 update
				 if(allDivYn.equals("Y")){
					 super.commonDao.delete("bpr560ukrvService.updateAllDiv", param);
					
				}else{
					 super.commonDao.update("bpr560ukrvService.updateDetail2", param);
				}
...
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 update
				 if(allDivYn.equals("Y")){
					 super.commonDao.delete("bpr560ukrvService.updateAllDiv2", param);
					
				}else{
					 super.commonDao.update("bpr560ukrvService.updateDetailGRID3", param);
				}
			// }
		 }
		 return;
	...
	public void deleteDetail(List<Map> paramList,  LoginVO user, String allDivYn) throws Exception {
		Map compCodeMap = new HashMap();
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		//List<Map> chkList = (List) super.commonDao.list("bpr560ukrvService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			// for(Map checkCompCode : chkList) {
				 //param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 delete
				 if(allDivYn.equals("Y")){
					 ...
					 super.commonDao.update("bpr560ukrvService.deleteDetail", param);
				}
...
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 delete
				 if(allDivYn.equals("Y")){
					 ...
					 super.commonDao.update("bpr560ukrvService.deleteDetailGRID2", param);
				}
			// }
		 }
		 return;
...
//					if b910  Y 면  해당 데이터로 div_code 조건 빼고 delete
				 if(allDivYn.equals("Y")){
					 ...
					 super.commonDao.update("bpr560ukrvService.deleteDetailGRID3", param);
				}
			// }
		 }
		 return;
...
//		if b910 Y 면   전사업장 insert 
		if(allDivYn.equals("Y")){
			if(allDivList != null){
				for(Map addParams : allDivList){
					param.put("DIV_CODE", addParams.get("DIV_CODE"));
					super.commonDao.update("bpr560ukrvService.makeProdItems", param);
				}
			}
	...
	public void excelValidate(String jobID, Map param) throws Exception {
		...
		List<Map> getData = (List<Map>) super.commonDao.list("bpr560ukrvService.getData", param);
		...
		if(!getData.isEmpty()){
			...
			for(Map data : getData )  {
				...
				String itemExistYn =  (String) super.commonDao.select("bpr560ukrvService.checkItem", param);
				if (itemExistYn.equals("Y")) {
					param.put("MSG", "품목코드 [" + data.get("CHILD_ITEM_CODE") +"]를 먼저 등록한 후 업로드 해 주세요.");
					super.commonDao.update("bpr560ukrvService.insertErrorMsg", param);
				}
	...
	public List<Map<String, Object>> selectExcelUploadSheet1(Map param) throws Exception {
		return super.commonDao.list("bpr560ukrvService.selectExcelUploadSheet1", param);
	}
	...
	public Object  changeItemData(Map param, LoginVO user) throws Exception {
		super.commonDao.update("bpr560ukrvService.changeItemData", param);
