...
@Service("bpr101ukrvService")
public class Bpr101ukrvServiceImpl extends TlabAbstractServiceImpl  {
    ...
    public List<Map<String, Object>> selectExcelUploadSheet1(Map param) throws Exception {
        return super.commonDao.list("bpr101ukrvService.selectExcelUploadSheet1", param);
    }
	
	public void excelValidate(String jobID, Map param) {							// 엑셀 Validate
	    logger.debug("validate: {}", jobID);
		super.commonDao.update("bpr101ukrvService.excelValidate", param);
	}
    ...
    public void saveExcelData(Map param) throws Exception {
        super.commonDao.update("bpr101ukrvService.saveExcel", param);
    }
	...
	public List<Map> selectDetailList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr101ukrvService.selectDetailList", param);
	}
	...
	public List<Map> checkItemDuplicate(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr101ukrvService.checkItemDuplicate", param);
	}
	...
	public List<Map> checkItemCode(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr101ukrvService.checkItemCode", param);
	}
	...
	public List<Map> selectStockList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr101ukrvService.selectStockList", param);
	}
	...
	public List<Map> selectSubDetailList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr101ukrvService.selectSubDetailList", param);
	}
			...
			for(Map param:  dataList) {
				...
				if(oprFlag == "N"){					
					...
					for(Map divCode : divList) {
						...
						param.put("data", super.commonDao.insert("bpr101ukrvService.insertLogDetail", param));
		...
		try {
			...
			for(Map param : paramList )	{	
				...
				if(ObjUtils.isEmpty(param.get("ITEM_CODE"))){
					List<Map> itemCode = (List<Map>) super.commonDao.list("bpr101ukrvService.getAutoItemCode", compCodeMap);
				 ...
				 for(Map checkCompCode : chkList) {
					 ...
					 for(Map divCode : divList) {
						 ...
						 super.commonDao.update("bpr101ukrvService.insertMultiLog", param);
						 param.put("DIV_CODE", insertDivCode);
					 }
	...
	public Integer updateDetail(List<Map> paramList, LoginVO user) throws Exception {
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("bpr101ukrvService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {				 
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
//				 Map chkItemMap = (Map) super.commonDao.select("bpr101ukrvService.checkItemCode", param);
//				 if(ObjUtils.parseInt(chkItemMap.get("CNT")) > 0)	{
//					 super.commonDao.insert("bpr101ukrvService.updateDetail", param);
//				 }else {
//					 super.commonDao.update("bpr101ukrvService.insertDetail", param);
//				 }
				 String divCodeList[] = ((String) param.get("DIV_CODE")).split(",");
				 for(int i = 0; i<divCodeList.length; i++){
					 param.put("DIV_CODE", divCodeList[i]);
					 ...
					 super.commonDao.update("bpr101ukrvService.updateMultiLog", param);
				 }
				 //어차피 판매단가는 별도 저장됨..
//				 if(ObjUtils.parseInt(param.get("SALE_BASIS_P")) != 0 && !"".equals(param.get("SALE_BASIS_P"))){
...
//				 }
		...
		List<Map> chkList = (List) super.commonDao.list("bpr101ukrvService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 ...
				 Map chkInOutHistoryMap = (Map) super.commonDao.select("bpr101ukrvService.chkInOutHistory", param);		//수불내역 있는지 확인 있으면 삭제 불가..
	...
	public String  updateSaleBasisP(Map spParam, LoginVO user) throws Exception {
		String keyValue = getLogKey();
		try{
			super.commonDao.update("bpr101ukrvService.updateSaleBasisP1", spParam);
			Map chkSaleBasisPDuplicateMap = (Map) super.commonDao.select("bpr101ukrvService.chkSaleBasisPDuplicate", spParam);
			if(ObjUtils.parseInt(chkSaleBasisPDuplicateMap.get("CNT")) > 0)	{	//등록된 item판매단가가있을시 update else insert
				super.commonDao.delete("bpr101ukrvService.deleteSaleBasisPDuplicate", spParam);
			}			 
			super.commonDao.update("bpr101ukrvService.updateSaleBasisP2", spParam);
			super.commonDao.update("bpr101ukrvService.updateSaleBasisP3", spParam);			
			spParam.put("KEY_VALUE", keyValue);
			spParam.put("CLASS", "U");		
			spParam.put("data", super.commonDao.insert("bpr101ukrvService.insertLogDetail", spParam)); 
			spParam.put("OPR_FLAG", "U");
			super.commonDao.update("bpr101ukrvService.updateMultiLog", spParam);
		}catch(Exception e){
			throw new  UniDirectValidateException(this.getMessage("2627", user));
		}
		return keyValue;
		...
		try {
			...
			for(Map param : paramList )	{			 
				 for(Map checkCompCode : chkList) {
					 ...
					 super.commonDao.update("bpr101ukrvService.insertSubDetail", param);
					 }
				}	
		}catch(Exception e){
			throw new  UniDirectValidateException(this.getMessage("2627", user));
		 ...
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 ...
					 super.commonDao.insert("bpr101ukrvService.updateSubDetail", param);
			 }
		 }
		 return 0;
	} 
		...
		for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 ...
				 if(ObjUtils.parseInt(chkInOutHistoryMap.get("CNT")) > 0){
					 ...
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));				 
					 try {
						 super.commonDao.delete("bpr101ukrvService.deleteSubDetail", param);						 
					 }catch(Exception e)	{
						 throw new  UniDirectValidateException(this.getMessage("547",user));
			    	}	
				 }
	...
	public Object searchMenu(Map param, LoginVO user) throws Exception {
		List<Map<String,Object>> list = super.commonDao.list("bpr101ukrvService.searchMenu", param);
	...
	public void  goInterFace(Map spParam, LoginVO user) throws Exception {
		super.commonDao.queryForObject("bpr101ukrvService.goInterFace", spParam);
	}
