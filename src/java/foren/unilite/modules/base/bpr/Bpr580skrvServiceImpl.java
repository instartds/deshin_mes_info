...
@Service("bpr580skrvService")
public class Bpr580skrvServiceImpl  extends TlabAbstractServiceImpl {
	...
	public Object selectMaster(Map param) throws Exception {
		return super.commonDao.select("bpr580skrvServiceImpl.selectMaster", param);
	}
	...
	public UniTreeNode selectList(Map param, LoginVO user) throws Exception {
		...
		if("0".equals(param.get("OPTSEL"))){
			...
			treeList  = super.commonDao.list("bpr580skrvServiceImpl.selectList_new", param);
			/*GenericTreeDataMap treeMap = (GenericTreeDataMap) super.commonDao.select("bpr580skrvServiceImpl.selectList", param);
			...
			GenericTreeDataMap treeMap = (GenericTreeDataMap) super.commonDao.select("bpr580skrvServiceImpl.selectList1", param);
			if(treeMap != null){
				treeList.add(treeMap);
	...
	private void getParent(Map param, List<GenericTreeDataMap> treeList) {
		List<GenericTreeDataMap> treeParentList = super.commonDao.list("bpr580skrvServiceImpl.getParent", param);
		if(treeParentList == null || treeParentList.size() == 0){
			treeParentList = super.commonDao.list("bpr580skrvServiceImpl.getParent1", param);
		}
		if(treeParentList != null && treeParentList.size()>0){
	...
	public void getChildren(Map param, List<GenericTreeDataMap> treeList){
		List<GenericTreeDataMap> treeChildrenList = super.commonDao.list("bpr580skrvServiceImpl.getChildren", param);
	...
	public void getSubstitute(Map param, List<GenericTreeDataMap> treeList){
		List<GenericTreeDataMap> treeChildrenList = super.commonDao.list("bpr580skrvServiceImpl.getSubstitute", param);
		if(treeChildrenList != null && treeChildrenList.size()>0){
			for(GenericTreeDataMap treeMap : treeChildrenList){
				treeList.add(treeMap);
			}
	...
	public Object chekCompCode(Map param) throws Exception {
		return super.commonDao.select("bpr580skrvServiceImpl.chekCompCode", param);
	}
