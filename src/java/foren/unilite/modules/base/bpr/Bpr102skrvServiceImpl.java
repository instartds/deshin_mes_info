...
@Service("bpr102skrvService")
public class Bpr102skrvServiceImpl extends TlabAbstractServiceImpl  {
	...
	public List<Map> selectDetailList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.selectDetailList", param);
	}
	...
	public List<Map> checkItemDuplicate(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.checkItemDuplicate", param);
	}
	...
	public List<Map> checkItemCode(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.checkItemCode", param);
	}
	...
	public List<Map> selectStockList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.selectStockList", param);
	}
	/**
	 * 매입단가 정보 조회
	 * @param param
	...
	public List<Map> selectSubDetailList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.selectSubDetailList", param);
	}
	...
	public List<Map> setBookInfo(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("bpr102skrvService.setBookInfo", param);
	}
	...
	public List<Map> saveAll(List<Map> paramList, Map paramMaster, LoginVO user) throws Exception {
		...
		for(Map paramData: paramList) {			
			...
			for(Map param:  dataList) {
				...
				if(oprFlag == "N"){					
					List<Map> divList = (List<Map>) super.commonDao.list("bpr101ukrvService.getDivList", compCodeMap);
					for(Map divCode : divList) {
						param.put("DIV_CODE", divCode.get("DIV_CODE"));
						param.put("data", super.commonDao.insert("bpr102skrvService.insertLogDetail", param));
...
//					insertDivCode  = (String) ((Map) dataListMap.get("data")).get("DIV_CODE");
	...
	public Integer  insertDetail(List<Map> paramList, LoginVO user) throws Exception {		
		try {
			Map compCodeMap = new HashMap();
			compCodeMap.put("S_COMP_CODE", user.getCompCode());
			List<Map> chkList = (List<Map>) super.commonDao.list("bpr102skrvService.checkCompCode", compCodeMap);
			for(Map param : paramList )	{
				String insertDivCode = (String) param.get("DIV_CODE");
				if(ObjUtils.isEmpty(param.get("ITEM_CODE"))){
					List<Map> itemCode = (List<Map>) super.commonDao.list("bpr102skrvService.getAutoItemCode", compCodeMap);
					param.put("ITEM_CODE", itemCode.get(0).get("ITEM_CODE"));
				}
				 for(Map checkCompCode : chkList) {
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 super.commonDao.update("bpr102skrvService.bpr100tInsertDetail", param);
					 
					 if(ObjUtils.parseInt(param.get("SALE_BASIS_P")) != 0 && !"".equals(param.get("SALE_BASIS_P"))){
						 super.commonDao.update("bpr102skrvService.bpr400tInsertDetail", param);	//판매단가 저장
					 }
					 List<Map> divList = (List<Map>) super.commonDao.list("bpr102skrvService.getDivList", compCodeMap);
					 for(Map divCode : divList) {
						 param.put("DIV_CODE", divCode.get("DIV_CODE"));
...
//						 super.commonDao.update("bpr102skrvService.bpr200tInsertDetail", param);
//					 }					 
				 }					 
			}	
		}catch(Exception e){
	...
	public Integer updateDetail(List<Map> paramList, LoginVO user) throws Exception {
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("bpr102skrvService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {				 
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
//				 Map chkItemMap = (Map) super.commonDao.select("bpr102skrvService.checkItemCode", param);
//				 if(ObjUtils.parseInt(chkItemMap.get("CNT")) > 0)	{
//					 super.commonDao.insert("bpr102skrvService.updateDetail", param);
//				 }else {
//					 super.commonDao.update("bpr102skrvService.insertDetail", param);
//				 }
				 String divCodeList[] = ((String) param.get("DIV_CODE")).split(",");
				 for(int i = 0; i<divCodeList.length; i++){
					 param.put("DIV_CODE", divCodeList[i]);
					 super.commonDao.update("bpr102skrvService.bpr200tUpdateDetail", param);//detail 저장
				 }
//				 if(ObjUtils.parseInt(param.get("SALE_BASIS_P")) != 0 && !"".equals(param.get("SALE_BASIS_P"))){
//					 Map chkItemPriceMap = (Map) super.commonDao.select("bpr102skrvService.checkItemPrice", param);
//					 if(ObjUtils.parseInt(chkItemPriceMap.get("CNT")) > 0)	{	//등록된 item판매단가가있을시 update else insert 
//						 super.commonDao.update("bpr102skrvService.bpr400tUpdateDetail", param);
//					 }else {
//						 super.commonDao.update("bpr102skrvService.bpr400tInsertDetail", param);
//				 	 }
//				 }
				 				
				 super.commonDao.update("bpr102skrvService.bpr100tUpdateDetail", param);
			 }
		 }
		 return 0;
	} 
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user) throws Exception {
		Map compCodeMap = new HashMap();
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("bpr102skrvService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 List chkChildList = (List) super.commonDao.list("bpr102skrvService.checkChildCode", param);
				 if(chkChildList.size() > 0)	{
					 ...
					 try {
						 super.commonDao.delete("bpr102skrvService.bpr200tDeleteDetail", param);
						 param.put("DIV_CODE", "");
						 Map chkItemMap = (Map) super.commonDao.select("bpr102skrvService.checkItemCode", param);
						 if(ObjUtils.parseInt(chkItemMap.get("CNT")) == 0)	{
							 super.commonDao.delete("bpr102skrvService.bpr100tDeleteDetail", param);	
						 }
					 }catch(Exception e)	{
	...
	public String  updateSaleBasisP(Map spParam, LoginVO user) throws Exception {
		String keyValue = getLogKey();
		try{
			super.commonDao.update("bpr102skrvService.updateSaleBasisP1", spParam);
			Map chkSaleBasisPDuplicateMap = (Map) super.commonDao.select("bpr102skrvService.chkSaleBasisPDuplicate", spParam);
			if(ObjUtils.parseInt(chkSaleBasisPDuplicateMap.get("CNT")) > 0)	{	//등록된 item판매단가가있을시 update else insert
				super.commonDao.delete("bpr102skrvService.deleteSaleBasisPDuplicate", spParam);
			}
			super.commonDao.update("bpr102skrvService.updateSaleBasisP2", spParam);
			super.commonDao.update("bpr102skrvService.updateSaleBasisP3", spParam);
			spParam.put("KEY_VALUE", keyValue);
			spParam.put("CLASS", "U");		
			spParam.put("data", super.commonDao.insert("bpr102skrvService.insertLogDetail", spParam));
		}catch(Exception e){
			throw new  UniDirectValidateException(this.getMessage("2627", user));
		}
		return keyValue;
		...
		try {
			...
			for(Map param : paramList )	{			 
				 for(Map checkCompCode : chkList) {
					 ...
					 super.commonDao.update("bpr102skrvService.insertSubDetail", param);
					 }
				}	
		}catch(Exception e){
			throw new  UniDirectValidateException(this.getMessage("2627", user));
		 ...
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 ...
					 super.commonDao.insert("bpr102skrvService.updateSubDetail", param);
			 }
		 }
		 return 0;
	} 
		 ...
		 for(Map param :paramList )	{
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 ...
					 try {
						 super.commonDao.delete("bpr102skrvService.deleteSubDetail", param);
	...
	public Object searchMenu(Map param, LoginVO user) throws Exception {
		List<Map<String,Object>> list = super.commonDao.list("bpr102skrvService.searchMenu", param);
	...
	public void  goInterFace(Map spParam, LoginVO user) throws Exception {
		super.commonDao.update("bpr102skrvService.goInterFace", spParam);
	}
