...
@Service("s_hum970rkr_ygService")
public class S_Hum970rkr_ygServiceImpl extends TlabAbstractServiceImpl {
	...
	public String fnHum970ini(Map param) throws Exception {
		...
		List<Map>qureyValue = (List<Map>) super.commonDao.list("s_hum970rkr_ygServiceImpl.fnHum970ini", param);
		...
		try {
			for(Map param : paramList )	{	
				if(super.commonDao.select("hum975rkrServiceImpl.selectProfNum", param) == null){
					super.commonDao.update("s_hum970rkr_ygServiceImpl.insertDetail", param);
				}
			}
		}catch(Exception e){
