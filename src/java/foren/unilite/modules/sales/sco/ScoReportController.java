...
@Controller
public class ScoReportController  extends UniliteCommonController {
    @InjectLogger
     ...
     * @throws Exception
     */
       @RequestMapping(value = "/sco/sco400rkrPrint.do", method = RequestMethod.GET)
        public ModelAndView sco400rkrPrint(ExtHtttprequestParam _req, LoginVO user, HttpServletRequest request, @RequestParam(value="reportType" , required = false, defaultValue = "pdf") String reportType) throws Exception {
           ...
           // Report와 SQL용 파라미터 구성
            ...
            JasperFactory jf  = jasperService.createJasperFactory("sco400rkr", param);
        	...
        	jflist.addAll(jasperService.selectList("sco400rkrServiceImpl.selectPrimaryDataList", dataParam));
       ...
       @RequestMapping(value = "/sco/sco410rkrPrint.do", method = RequestMethod.GET)
       public ModelAndView sco410rkrPrint(ExtHtttprequestParam _req, LoginVO user, HttpServletRequest request, @RequestParam(value="reportType" , required = false, defaultValue = "pdf") String reportType) throws Exception {
          String[] subReportFileNames = {
          };
          ...
          // Report와 SQL용 파라미터 구성
           ...
           // Jasper Factpry생성(파라미터는 prefix("P_") 뭍어 report로 전달 됨.
           // 기본 경로 체계가 아닌 경우 File 전달 방식을 이용            
           JasperFactory jf  = jasperService.createJasperFactory("sco410rkr", param);
           jf.setReportType(reportType);
           // SubReport 파일명 목록을 전달
           // 레포트 수행시 compile을 상황에 따라 수행함.
           jf.setSubReportFiles(subReportFileNames);
       	...
       	Map dataParam = new HashMap();
		    	...
		    	dataParam.put("CUSTOM_CODE"	, param.get("CUSTOM_CODE"));
		    	dataParam.put("CUSTOM_NAME"	, param.get("CUSTOM_NAME"));
		    	dataParam.put("ACCNT"		, param.get("ACCNT"));
       	//logger1.debug("jasper arry : " + arry[i]);
       	
       	jflist.addAll(jasperService.selectList("sco410skrvServiceImpl.selectList", dataParam));
