...
@Service("sbs400ukrvService")
@SuppressWarnings({"rawtypes", "unchecked"})

public class Sbs400ukrvServiceImpl extends TlabAbstractServiceImpl  {
	...
	public List<Map> selectList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("sbs400ukrvServiceImpl.selectList", param);
	}
...
//				신규 등록 전, 기 등록된 데이터와 적용 시작일 비교
				String checkData = (String) super.commonDao.select("sbs400ukrvServiceImpl.checkData", param);
				
				if (checkData.equals("N")) {
					super.commonDao.insert("sbs400ukrvServiceImpl.insertList", param);
	...
	public Integer updateList(List<Map> paramList, LoginVO user) throws Exception {
		 for(Map param :paramList )	{	
			 super.commonDao.update("sbs400ukrvServiceImpl.updateList", param);
		 }
		 return 0;
	} 
	...
	public Integer deleteList(List<Map> paramList,  LoginVO user) throws Exception {		
		 for(Map param :paramList )	{
			 try {
				super.commonDao.delete("sbs400ukrvServiceImpl.deleteList", param);
	...
	public void excelValidate(String jobID, Map param) throws Exception {
		...
		List<Map> getData = (List<Map>) super.commonDao.list("sbs400ukrvServiceImpl.getData", param);
		...
		if(!getData.isEmpty()){
			...
			for(Map data : getData )  { 
				...
				String itemExistYn =  (String) super.commonDao.select("sbs400ukrvServiceImpl.checkItem", param);
				if (itemExistYn.equals("N")) {					
					param.put("MSG", "품목코드 [" + data.get("ITEM_CODE") +"]를 먼저 등록한 후 업로드 해 주세요.");
					super.commonDao.update("sbs400ukrvServiceImpl.insertErrorMsg", param);
				}
				//업로드 된 데이터의 거래처 기등록여부 확인
				String custExistYn =  (String) super.commonDao.select("sbs400ukrvServiceImpl.checkCust", param);
	...
	public List<Map<String, Object>> selectExcelUploadSheet1(Map param) throws Exception {
		return super.commonDao.list("sbs400ukrvServiceImpl.selectExcelUploadSheet1", param);
	}
	...
	public List<Map> selectObjCustom(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("sbs400ukrvServiceImpl.selectObjCustom", param);
	}
	...
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor={Exception.class})   
	public List<Map> itemPrice(List<Map> paramList, Map paramMaster, LoginVO user) throws Exception {
		if(paramList != null)   {
			List<Map> insertList = null;

			for(Map dataListMap: paramList) {
				if(dataListMap.get("method").equals("copyItemPrice")) {
					insertList = (List<Map>)dataListMap.get("data");
				}
			}
			if(insertList != null) this.copyItemPrice(insertList, paramMaster, user);
		}
		paramList.add(0, paramMaster);
		return  paramList;
	}
	...
	@ExtDirectMethod(value = ExtDirectMethodType.STORE_MODIFY, group = "sales")
	public Integer copyItemPrice( List<Map> paramList, Map paramMaster,  LoginVO user) throws Exception {
		Map<String, Object> dataMaster = (Map<String, Object>) paramMaster.get("data");
		
		String errMsg = "";		
		try {
//			원본 거래처 데이터 존재 확인
			String checkData = (String) super.commonDao.select("sbs400ukrvServiceImpl.checkOriData", dataMaster);
			
			for(Map param: paramList)	  {
				if (checkData.equals("Y")) {
				    param.put("BASIS_DATE", dataMaster.get("BASIS_DATE"));
				    param.put("DIV_CODE", dataMaster.get("DIV_CODE"));
					super.commonDao.insert("sbs400ukrvServiceImpl.copyItemPrice", param);
					
				} else {
					errMsg = "(원본 거래처의 판매단가 정보가 존재하지 않습니다.)";
					throw new  UniDirectValidateException("");
				}
			}
		} catch(Exception e){
