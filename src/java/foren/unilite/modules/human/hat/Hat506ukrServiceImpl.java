...
@Service("hat506ukrService")
@SuppressWarnings({ "rawtypes", "unchecked" })
public class Hat506ukrServiceImpl extends TlabAbstractServiceImpl {
...
//	private Map<String, Object> dataMaster;
	...
	public String getDutyRule(String comp_code) throws Exception{
		DutyRule = (String) super.commonDao.selectByPk("hat506ukrServiceImpl.getDutyRule", comp_code);
		return DutyRule;
	}
	...
	public List getAllDutycode(Map param) throws Exception {
		return (List)super.commonDao.list("hat506ukrServiceImpl.getAllDutycode" ,param);
	}

	@ExtDirectMethod(value = ExtDirectMethodType.STORE_READ, group = "hum")
	public List getDutycodeTime(Map param) throws Exception {
		return (List)super.commonDao.list("hat506ukrServiceImpl.getDutycodeTime" ,param);
	}
	...
	public List getDutycode(Map param) throws Exception {
		return (List)super.commonDao.list("hat506ukrServiceImpl.getDutycode" ,param);
	}
...
//		return (List)super.commonDao.list("hat506ukrServiceImpl.getComboList" ,param);
//	}
	...
	public List<ComboItemModel> getComboList(Map param) throws Exception {
		return (List<ComboItemModel>) super.commonDao.list("hat506ukrServiceImpl.getComboList", param);
	}
	...
	public List<ComboItemModel> getComboList2(Map param) throws Exception {
		return (List<ComboItemModel>) super.commonDao.list("hat506ukrServiceImpl.getComboList2", param);
	}
	...
	public List<Map<String, Object>> selectList(Map param, LoginVO loginVO) throws Exception {
		...
		logger.debug(param+"");
		return (List) super.commonDao.list("hat506ukrServiceImpl.selectList", param);
	}
			...
			if (DutyRule != "") {
				if (DutyRule.equals("Y")) {
					if(param.get("FLAG").equals("N")){
						super.commonDao.update("hat506ukrServiceImpl.insertList_DutyRule_Y", param);
					}else{
						super.commonDao.update("hat506ukrServiceImpl.updateList_DutyRule_Y", param);
					}
				} else {
					// 1.기존의 데이터를 모두 삭제
					super.commonDao.delete("hat506ukrServiceImpl.deleteList_DutyRule_N", param);
					// 2. 새로운 데이터를 다시 인서트 함
					...
					for (Map item : dutyCode) {
						...
						super.commonDao.insert("hat506ukrServiceImpl.insertList_DutyRule_N01", map);
					}
					// 3. 등록 안 된 근태 0값으로 인서트함
					super.commonDao.insert("hat506ukrServiceImpl.insertList_DutyRule_N02", param);
					// 4. 근태코드가 입력 된 경우 DUTY_NUM을 1로 수정함

					if (param.get("NUMN").equals("1") && param.get("NUMC") != "") {
						super.commonDao.update("hat506ukrServiceImpl.updateList_DutyRule_N", param);
					}
				}
				param.put("FLAG", "");
	...
	public List<Map> deleteList(List<Map> paramList) throws Exception {
		for(Map param :paramList )	{
			if (DutyRule != "") {
				if (DutyRule.equals("Y")) {
					super.commonDao.update("hat506ukrServiceImpl.deleteList_DutyRule_Y", param);
				} else {
					super.commonDao.update("hat506ukrServiceImpl.deleteList_DutyRule_N", param);
				}
			}
		}
		return paramList;
	...
	public Object procClosing(Map param, LoginVO user) throws Exception {
		try {
			super.commonDao.update("hat506ukrServiceImpl.procClosing", param);
		} catch(Exception e) {
		...
		try {
			...
			while (( txtline = in2.readLine() ) != null) {
				...
				if(txtline.trim().length() > 0) {
					...
					super.commonDao.update("hat506ukrServiceImpl.insertTXT", iMap);

					row++;
				}
			}
			super.commonDao.update("hat506ukrServiceImpl.insertHAT", iMap);
			...
			super.commonDao.queryForObject("hat506ukrServiceImpl.insertSP", spParam);
	...
	public List<Map<String, Object>> selectExcelUploadSheet1(Map param) throws Exception {
		// 최근에 등록한 근태일자 조회
		String dutyDate = (String) super.commonDao.select("hat506ukrServiceImpl.selectExcelUploadSheetDutyDate", param);
		param.put("WORK_DATE", dutyDate);
		
		//근태기준(Y: 출퇴근 시간, N: 근무 시간)
		DutyRule = "".equals(DutyRule) ? getDutyRule((String)param.get("S_COMP_CODE")) : DutyRule;
		param.put("DUTY_RULE", DutyRule);
		...
		return super.commonDao.list("hat506ukrServiceImpl.selectExcelUploadSheet1", param);
	}
	...
	public void excelValidate(String jobID, Map param) {
		...
		List<Map> getData = (List<Map>) super.commonDao.list("hat506ukrServiceImpl.getData", param);
