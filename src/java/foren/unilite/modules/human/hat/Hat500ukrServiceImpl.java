...
@Service("hat500ukrService")
@SuppressWarnings({ "rawtypes", "unchecked" })
public class Hat500ukrServiceImpl extends TlabAbstractServiceImpl {
...
//	private Map<String, Object> dataMaster;
	...
	public List getDutycodeTime(Map param) throws Exception {
		return (List)super.commonDao.list("hat500ukrServiceImpl.getDutycodeTime" ,param);
	}
...
//	}
	...
	public List<ComboItemModel> getComboList(Map param) throws Exception {
		return (List<ComboItemModel>) super.commonDao.list("hat500ukrServiceImpl.getComboList", param);
	}
	...
	public List<ComboItemModel> getComboList2(Map param) throws Exception {
		return (List<ComboItemModel>) super.commonDao.list("hat500ukrServiceImpl.getComboList2", param);
	}
	...
	public List<Map<String, Object>> selectList(Map param, LoginVO loginVO) throws Exception {
		...
		logger.debug(param+"");
		return (List) super.commonDao.list("hat500ukrServiceImpl.selectList", param);
	}
			...
			if (DutyRule != "") {
				if (DutyRule.equals("Y")) {
					if(param.get("FLAG").equals("N")){
						super.commonDao.update("hat500ukrServiceImpl.insertList_DutyRule_Y", param);
					}else{
						super.commonDao.update("hat500ukrServiceImpl.updateList_DutyRule_Y", param);
					}
				} else {
					// 1.기존의 데이터를 모두 삭제
					super.commonDao.delete("hat500ukrServiceImpl.deleteList_DutyRule_N", param);
					// 2. 새로운 데이터를 다시 인서트 함
					...
					for (Map item : dutyCode) {
						...
						super.commonDao.insert("hat500ukrServiceImpl.insertList_DutyRule_N01", map);
					}
					// 3. 등록 안 된 근태 0값으로 인서트함
					super.commonDao.insert("hat500ukrServiceImpl.insertList_DutyRule_N02", param);
					// 4. 근태코드가 입력 된 경우 DUTY_NUM을 1로 수정함

					if (param.get("NUMN").equals("1") && param.get("NUMC") != "") {
						super.commonDao.update("hat500ukrServiceImpl.updateList_DutyRule_N", param);
					}
				}
				param.put("FLAG", "");
	...
	public List<Map> deleteList(List<Map> paramList) throws Exception {
		for(Map param :paramList )	{
			if (DutyRule != "") {
				if (DutyRule.equals("Y")) {
					super.commonDao.update("hat500ukrServiceImpl.deleteList_DutyRule_Y", param);
				} else {
					super.commonDao.update("hat500ukrServiceImpl.deleteList_DutyRule_N", param);
				}
			}
		}
		return paramList;
	...
	public Object procClosing(Map param, LoginVO user) throws Exception {
		try {
			super.commonDao.update("hat500ukrServiceImpl.procClosing", param);
		} catch(Exception e) {
		...
		try {
			...
			while (( txtline = in2.readLine() ) != null) {
				...
				if(txtline.trim().length() > 0) {
					...
					super.commonDao.update("hat500ukrServiceImpl.insertTXT", iMap);

					row++;
				}
			}
			super.commonDao.update("hat500ukrServiceImpl.insertHAT", iMap);
			...
			super.commonDao.queryForObject("hat500ukrServiceImpl.insertSP", spParam);
