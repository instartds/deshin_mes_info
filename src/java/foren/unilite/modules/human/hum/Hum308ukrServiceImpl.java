...
@Service("hum308ukrService")
public class Hum308ukrServiceImpl extends TlabAbstractServiceImpl  {
	...
	public List<Map> selectList(Map param, LoginVO user) throws Exception {
		return  super.commonDao.list("hum308ukrService.select", param);
	}
    ...
    public List<Map<String, Object>> selectExcelUploadSheet1(Map param) throws Exception {
        return super.commonDao.list("hum308ukrService.selectExcelUploadSheet1", param);
    }
    ...
    public List<Map<String, Object>> selectExcelUploadApply(Map param) throws Exception {
        return super.commonDao.list("hum308ukrService.selectExcelUploadApply", param);
    }
    
    public void excelValidate(String jobID, Map param) {							// 엑셀 Validate
	    logger.debug("validate: {}", jobID);
		super.commonDao.update("hum308ukrService.excelValidate", param);
	}
	...
	public Integer  insertDetail(List<Map> paramList, Map paramMaster, LoginVO user) throws Exception {		
		try {
			...
			List<Map> chkList = (List<Map>) super.commonDao.list("hum308ukrService.checkCompCode", compCodeMap);
			for(Map param : paramList )	{			 
				for(Map checkCompCode : chkList) {
					 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
					 super.commonDao.update("hum308ukrService.insertDetail", param);
					 if(dataMaster.get("CHKCNT").equals("Y")){
						 super.commonDao.update("hum308ukrService.Hum100MasterUpdate", param); // 인사마스터에 저장
					 }
				}
			}	
		}catch(Exception e){
	...
	public Integer updateDetail(List<Map> paramList, Map paramMaster, LoginVO user) throws Exception {
		...
		List<Map> chkList = (List<Map>) super.commonDao.list("hum308ukrService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 super.commonDao.update("hum308ukrService.updateDetail", param);
				 if(dataMaster.get("CHKCNT").equals("Y")){
					 super.commonDao.update("hum308ukrService.Hum100MasterUpdate", param); // 인사마스터에 저장
				 }
			 }
	...
	public Integer deleteDetail(List<Map> paramList,  LoginVO user) throws Exception {
		Map compCodeMap = new HashMap();
		compCodeMap.put("S_COMP_CODE", user.getCompCode());
		List<Map> chkList = (List) super.commonDao.list("hum308ukrService.checkCompCode", compCodeMap);
		 for(Map param :paramList )	{	
			 for(Map checkCompCode : chkList) {
				 param.put("COMP_CODE", checkCompCode.get("COMP_CODE"));
				 super.commonDao.update("hum308ukrService.deleteDetail", param);
			 }
		 }
		 return 0;
	}
